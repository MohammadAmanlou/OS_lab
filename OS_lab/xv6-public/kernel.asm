
kernel:     file format elf32-i386


Disassembly of section .text:

80100000 <multiboot_header>:
80100000:	02 b0 ad 1b 00 00    	add    0x1bad(%eax),%dh
80100006:	00 00                	add    %al,(%eax)
80100008:	fe 4f 52             	decb   0x52(%edi)
8010000b:	e4                   	.byte 0xe4

8010000c <entry>:

# Entering xv6 on boot processor, with paging off.
.globl entry
entry:
  # Turn on page size extension for 4Mbyte pages
  movl    %cr4, %eax
8010000c:	0f 20 e0             	mov    %cr4,%eax
  orl     $(CR4_PSE), %eax
8010000f:	83 c8 10             	or     $0x10,%eax
  movl    %eax, %cr4
80100012:	0f 22 e0             	mov    %eax,%cr4
  # Set page directory
  movl    $(V2P_WO(entrypgdir)), %eax
80100015:	b8 00 b0 10 00       	mov    $0x10b000,%eax
  movl    %eax, %cr3
8010001a:	0f 22 d8             	mov    %eax,%cr3
  # Turn on paging.
  movl    %cr0, %eax
8010001d:	0f 20 c0             	mov    %cr0,%eax
  orl     $(CR0_PG|CR0_WP), %eax
80100020:	0d 00 00 01 80       	or     $0x80010000,%eax
  movl    %eax, %cr0
80100025:	0f 22 c0             	mov    %eax,%cr0

  # Set up the stack pointer.
  movl $(stack + KSTACKSIZE), %esp
80100028:	bc 10 d6 10 80       	mov    $0x8010d610,%esp

  # Jump to main(), and switch to executing at
  # high addresses. The indirect call is needed because
  # the assembler produces a PC-relative instruction
  # for a direct jump.
  mov $main, %eax
8010002d:	b8 30 35 10 80       	mov    $0x80103530,%eax
  jmp *%eax
80100032:	ff e0                	jmp    *%eax
80100034:	66 90                	xchg   %ax,%ax
80100036:	66 90                	xchg   %ax,%ax
80100038:	66 90                	xchg   %ax,%ax
8010003a:	66 90                	xchg   %ax,%ax
8010003c:	66 90                	xchg   %ax,%ax
8010003e:	66 90                	xchg   %ax,%ax

80100040 <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
80100040:	f3 0f 1e fb          	endbr32 
80100044:	55                   	push   %ebp
80100045:	89 e5                	mov    %esp,%ebp
80100047:	53                   	push   %ebx

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
  bcache.head.next = &bcache.head;
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
80100048:	bb 54 d6 10 80       	mov    $0x8010d654,%ebx
{
8010004d:	83 ec 0c             	sub    $0xc,%esp
  initlock(&bcache.lock, "bcache");
80100050:	68 a0 8b 10 80       	push   $0x80108ba0
80100055:	68 20 d6 10 80       	push   $0x8010d620
8010005a:	e8 31 5a 00 00       	call   80105a90 <initlock>
  bcache.head.next = &bcache.head;
8010005f:	83 c4 10             	add    $0x10,%esp
80100062:	b8 1c 1d 11 80       	mov    $0x80111d1c,%eax
  bcache.head.prev = &bcache.head;
80100067:	c7 05 6c 1d 11 80 1c 	movl   $0x80111d1c,0x80111d6c
8010006e:	1d 11 80 
  bcache.head.next = &bcache.head;
80100071:	c7 05 70 1d 11 80 1c 	movl   $0x80111d1c,0x80111d70
80100078:	1d 11 80 
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
8010007b:	eb 05                	jmp    80100082 <binit+0x42>
8010007d:	8d 76 00             	lea    0x0(%esi),%esi
80100080:	89 d3                	mov    %edx,%ebx
    b->next = bcache.head.next;
80100082:	89 43 54             	mov    %eax,0x54(%ebx)
    b->prev = &bcache.head;
    initsleeplock(&b->lock, "buffer");
80100085:	83 ec 08             	sub    $0x8,%esp
80100088:	8d 43 0c             	lea    0xc(%ebx),%eax
    b->prev = &bcache.head;
8010008b:	c7 43 50 1c 1d 11 80 	movl   $0x80111d1c,0x50(%ebx)
    initsleeplock(&b->lock, "buffer");
80100092:	68 a7 8b 10 80       	push   $0x80108ba7
80100097:	50                   	push   %eax
80100098:	e8 93 57 00 00       	call   80105830 <initsleeplock>
    bcache.head.next->prev = b;
8010009d:	a1 70 1d 11 80       	mov    0x80111d70,%eax
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
801000a2:	8d 93 5c 02 00 00    	lea    0x25c(%ebx),%edx
801000a8:	83 c4 10             	add    $0x10,%esp
    bcache.head.next->prev = b;
801000ab:	89 58 50             	mov    %ebx,0x50(%eax)
    bcache.head.next = b;
801000ae:	89 d8                	mov    %ebx,%eax
801000b0:	89 1d 70 1d 11 80    	mov    %ebx,0x80111d70
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
801000b6:	81 fb c0 1a 11 80    	cmp    $0x80111ac0,%ebx
801000bc:	75 c2                	jne    80100080 <binit+0x40>
  }
}
801000be:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801000c1:	c9                   	leave  
801000c2:	c3                   	ret    
801000c3:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801000ca:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801000d0 <bread>:
}

// Return a locked buf with the contents of the indicated block.
struct buf*
bread(uint dev, uint blockno)
{
801000d0:	f3 0f 1e fb          	endbr32 
801000d4:	55                   	push   %ebp
801000d5:	89 e5                	mov    %esp,%ebp
801000d7:	57                   	push   %edi
801000d8:	56                   	push   %esi
801000d9:	53                   	push   %ebx
801000da:	83 ec 18             	sub    $0x18,%esp
801000dd:	8b 7d 08             	mov    0x8(%ebp),%edi
801000e0:	8b 75 0c             	mov    0xc(%ebp),%esi
  acquire(&bcache.lock);
801000e3:	68 20 d6 10 80       	push   $0x8010d620
801000e8:	e8 23 5b 00 00       	call   80105c10 <acquire>
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
801000ed:	8b 1d 70 1d 11 80    	mov    0x80111d70,%ebx
801000f3:	83 c4 10             	add    $0x10,%esp
801000f6:	81 fb 1c 1d 11 80    	cmp    $0x80111d1c,%ebx
801000fc:	75 0d                	jne    8010010b <bread+0x3b>
801000fe:	eb 20                	jmp    80100120 <bread+0x50>
80100100:	8b 5b 54             	mov    0x54(%ebx),%ebx
80100103:	81 fb 1c 1d 11 80    	cmp    $0x80111d1c,%ebx
80100109:	74 15                	je     80100120 <bread+0x50>
    if(b->dev == dev && b->blockno == blockno){
8010010b:	3b 7b 04             	cmp    0x4(%ebx),%edi
8010010e:	75 f0                	jne    80100100 <bread+0x30>
80100110:	3b 73 08             	cmp    0x8(%ebx),%esi
80100113:	75 eb                	jne    80100100 <bread+0x30>
      b->refcnt++;
80100115:	83 43 4c 01          	addl   $0x1,0x4c(%ebx)
      release(&bcache.lock);
80100119:	eb 3f                	jmp    8010015a <bread+0x8a>
8010011b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010011f:	90                   	nop
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100120:	8b 1d 6c 1d 11 80    	mov    0x80111d6c,%ebx
80100126:	81 fb 1c 1d 11 80    	cmp    $0x80111d1c,%ebx
8010012c:	75 0d                	jne    8010013b <bread+0x6b>
8010012e:	eb 70                	jmp    801001a0 <bread+0xd0>
80100130:	8b 5b 50             	mov    0x50(%ebx),%ebx
80100133:	81 fb 1c 1d 11 80    	cmp    $0x80111d1c,%ebx
80100139:	74 65                	je     801001a0 <bread+0xd0>
    if(b->refcnt == 0 && (b->flags & B_DIRTY) == 0) {
8010013b:	8b 43 4c             	mov    0x4c(%ebx),%eax
8010013e:	85 c0                	test   %eax,%eax
80100140:	75 ee                	jne    80100130 <bread+0x60>
80100142:	f6 03 04             	testb  $0x4,(%ebx)
80100145:	75 e9                	jne    80100130 <bread+0x60>
      b->dev = dev;
80100147:	89 7b 04             	mov    %edi,0x4(%ebx)
      b->blockno = blockno;
8010014a:	89 73 08             	mov    %esi,0x8(%ebx)
      b->flags = 0;
8010014d:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
      b->refcnt = 1;
80100153:	c7 43 4c 01 00 00 00 	movl   $0x1,0x4c(%ebx)
      release(&bcache.lock);
8010015a:	83 ec 0c             	sub    $0xc,%esp
8010015d:	68 20 d6 10 80       	push   $0x8010d620
80100162:	e8 69 5b 00 00       	call   80105cd0 <release>
      acquiresleep(&b->lock);
80100167:	8d 43 0c             	lea    0xc(%ebx),%eax
8010016a:	89 04 24             	mov    %eax,(%esp)
8010016d:	e8 fe 56 00 00       	call   80105870 <acquiresleep>
      return b;
80100172:	83 c4 10             	add    $0x10,%esp
  struct buf *b;

  b = bget(dev, blockno);
  if((b->flags & B_VALID) == 0) {
80100175:	f6 03 02             	testb  $0x2,(%ebx)
80100178:	74 0e                	je     80100188 <bread+0xb8>
    iderw(b);
  }
  return b;
}
8010017a:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010017d:	89 d8                	mov    %ebx,%eax
8010017f:	5b                   	pop    %ebx
80100180:	5e                   	pop    %esi
80100181:	5f                   	pop    %edi
80100182:	5d                   	pop    %ebp
80100183:	c3                   	ret    
80100184:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    iderw(b);
80100188:	83 ec 0c             	sub    $0xc,%esp
8010018b:	53                   	push   %ebx
8010018c:	e8 df 25 00 00       	call   80102770 <iderw>
80100191:	83 c4 10             	add    $0x10,%esp
}
80100194:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100197:	89 d8                	mov    %ebx,%eax
80100199:	5b                   	pop    %ebx
8010019a:	5e                   	pop    %esi
8010019b:	5f                   	pop    %edi
8010019c:	5d                   	pop    %ebp
8010019d:	c3                   	ret    
8010019e:	66 90                	xchg   %ax,%ax
  panic("bget: no buffers");
801001a0:	83 ec 0c             	sub    $0xc,%esp
801001a3:	68 ae 8b 10 80       	push   $0x80108bae
801001a8:	e8 e3 01 00 00       	call   80100390 <panic>
801001ad:	8d 76 00             	lea    0x0(%esi),%esi

801001b0 <bwrite>:

// Write b's contents to disk.  Must be locked.
void
bwrite(struct buf *b)
{
801001b0:	f3 0f 1e fb          	endbr32 
801001b4:	55                   	push   %ebp
801001b5:	89 e5                	mov    %esp,%ebp
801001b7:	53                   	push   %ebx
801001b8:	83 ec 10             	sub    $0x10,%esp
801001bb:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(!holdingsleep(&b->lock))
801001be:	8d 43 0c             	lea    0xc(%ebx),%eax
801001c1:	50                   	push   %eax
801001c2:	e8 49 57 00 00       	call   80105910 <holdingsleep>
801001c7:	83 c4 10             	add    $0x10,%esp
801001ca:	85 c0                	test   %eax,%eax
801001cc:	74 0f                	je     801001dd <bwrite+0x2d>
    panic("bwrite");
  b->flags |= B_DIRTY;
801001ce:	83 0b 04             	orl    $0x4,(%ebx)
  iderw(b);
801001d1:	89 5d 08             	mov    %ebx,0x8(%ebp)
}
801001d4:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801001d7:	c9                   	leave  
  iderw(b);
801001d8:	e9 93 25 00 00       	jmp    80102770 <iderw>
    panic("bwrite");
801001dd:	83 ec 0c             	sub    $0xc,%esp
801001e0:	68 bf 8b 10 80       	push   $0x80108bbf
801001e5:	e8 a6 01 00 00       	call   80100390 <panic>
801001ea:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801001f0 <brelse>:

// Release a locked buffer.
// Move to the head of the MRU list.
void
brelse(struct buf *b)
{
801001f0:	f3 0f 1e fb          	endbr32 
801001f4:	55                   	push   %ebp
801001f5:	89 e5                	mov    %esp,%ebp
801001f7:	56                   	push   %esi
801001f8:	53                   	push   %ebx
801001f9:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(!holdingsleep(&b->lock))
801001fc:	8d 73 0c             	lea    0xc(%ebx),%esi
801001ff:	83 ec 0c             	sub    $0xc,%esp
80100202:	56                   	push   %esi
80100203:	e8 08 57 00 00       	call   80105910 <holdingsleep>
80100208:	83 c4 10             	add    $0x10,%esp
8010020b:	85 c0                	test   %eax,%eax
8010020d:	74 66                	je     80100275 <brelse+0x85>
    panic("brelse");

  releasesleep(&b->lock);
8010020f:	83 ec 0c             	sub    $0xc,%esp
80100212:	56                   	push   %esi
80100213:	e8 b8 56 00 00       	call   801058d0 <releasesleep>

  acquire(&bcache.lock);
80100218:	c7 04 24 20 d6 10 80 	movl   $0x8010d620,(%esp)
8010021f:	e8 ec 59 00 00       	call   80105c10 <acquire>
  b->refcnt--;
80100224:	8b 43 4c             	mov    0x4c(%ebx),%eax
  if (b->refcnt == 0) {
80100227:	83 c4 10             	add    $0x10,%esp
  b->refcnt--;
8010022a:	83 e8 01             	sub    $0x1,%eax
8010022d:	89 43 4c             	mov    %eax,0x4c(%ebx)
  if (b->refcnt == 0) {
80100230:	85 c0                	test   %eax,%eax
80100232:	75 2f                	jne    80100263 <brelse+0x73>
    // no one is waiting for it.
    b->next->prev = b->prev;
80100234:	8b 43 54             	mov    0x54(%ebx),%eax
80100237:	8b 53 50             	mov    0x50(%ebx),%edx
8010023a:	89 50 50             	mov    %edx,0x50(%eax)
    b->prev->next = b->next;
8010023d:	8b 43 50             	mov    0x50(%ebx),%eax
80100240:	8b 53 54             	mov    0x54(%ebx),%edx
80100243:	89 50 54             	mov    %edx,0x54(%eax)
    b->next = bcache.head.next;
80100246:	a1 70 1d 11 80       	mov    0x80111d70,%eax
    b->prev = &bcache.head;
8010024b:	c7 43 50 1c 1d 11 80 	movl   $0x80111d1c,0x50(%ebx)
    b->next = bcache.head.next;
80100252:	89 43 54             	mov    %eax,0x54(%ebx)
    bcache.head.next->prev = b;
80100255:	a1 70 1d 11 80       	mov    0x80111d70,%eax
8010025a:	89 58 50             	mov    %ebx,0x50(%eax)
    bcache.head.next = b;
8010025d:	89 1d 70 1d 11 80    	mov    %ebx,0x80111d70
  }
  
  release(&bcache.lock);
80100263:	c7 45 08 20 d6 10 80 	movl   $0x8010d620,0x8(%ebp)
}
8010026a:	8d 65 f8             	lea    -0x8(%ebp),%esp
8010026d:	5b                   	pop    %ebx
8010026e:	5e                   	pop    %esi
8010026f:	5d                   	pop    %ebp
  release(&bcache.lock);
80100270:	e9 5b 5a 00 00       	jmp    80105cd0 <release>
    panic("brelse");
80100275:	83 ec 0c             	sub    $0xc,%esp
80100278:	68 c6 8b 10 80       	push   $0x80108bc6
8010027d:	e8 0e 01 00 00       	call   80100390 <panic>
80100282:	66 90                	xchg   %ax,%ax
80100284:	66 90                	xchg   %ax,%ax
80100286:	66 90                	xchg   %ax,%ax
80100288:	66 90                	xchg   %ax,%ax
8010028a:	66 90                	xchg   %ax,%ax
8010028c:	66 90                	xchg   %ax,%ax
8010028e:	66 90                	xchg   %ax,%ax

80100290 <consoleread>:
  }
}

int
consoleread(struct inode *ip, char *dst, int n)
{
80100290:	f3 0f 1e fb          	endbr32 
80100294:	55                   	push   %ebp
80100295:	89 e5                	mov    %esp,%ebp
80100297:	57                   	push   %edi
80100298:	56                   	push   %esi
80100299:	53                   	push   %ebx
8010029a:	83 ec 18             	sub    $0x18,%esp
  uint target;
  int c;

  iunlock(ip);
8010029d:	ff 75 08             	pushl  0x8(%ebp)
{
801002a0:	8b 5d 10             	mov    0x10(%ebp),%ebx
  target = n;
801002a3:	89 de                	mov    %ebx,%esi
  iunlock(ip);
801002a5:	e8 86 1a 00 00       	call   80101d30 <iunlock>
  acquire(&cons.lock);
801002aa:	c7 04 24 20 c5 10 80 	movl   $0x8010c520,(%esp)
801002b1:	e8 5a 59 00 00       	call   80105c10 <acquire>
        // caller gets a 0-byte result.
        input.r--;
      }
      break;
    }
    *dst++ = c;
801002b6:	8b 7d 0c             	mov    0xc(%ebp),%edi
  while(n > 0){
801002b9:	83 c4 10             	add    $0x10,%esp
    *dst++ = c;
801002bc:	01 df                	add    %ebx,%edi
  while(n > 0){
801002be:	85 db                	test   %ebx,%ebx
801002c0:	0f 8e 97 00 00 00    	jle    8010035d <consoleread+0xcd>
    while(input.r == input.w){
801002c6:	a1 00 20 11 80       	mov    0x80112000,%eax
801002cb:	3b 05 04 20 11 80    	cmp    0x80112004,%eax
801002d1:	74 27                	je     801002fa <consoleread+0x6a>
801002d3:	eb 5b                	jmp    80100330 <consoleread+0xa0>
801002d5:	8d 76 00             	lea    0x0(%esi),%esi
      sleep(&input.r, &cons.lock);
801002d8:	83 ec 08             	sub    $0x8,%esp
801002db:	68 20 c5 10 80       	push   $0x8010c520
801002e0:	68 00 20 11 80       	push   $0x80112000
801002e5:	e8 b6 41 00 00       	call   801044a0 <sleep>
    while(input.r == input.w){
801002ea:	a1 00 20 11 80       	mov    0x80112000,%eax
801002ef:	83 c4 10             	add    $0x10,%esp
801002f2:	3b 05 04 20 11 80    	cmp    0x80112004,%eax
801002f8:	75 36                	jne    80100330 <consoleread+0xa0>
      if(myproc()->killed){
801002fa:	e8 c1 3b 00 00       	call   80103ec0 <myproc>
801002ff:	8b 48 24             	mov    0x24(%eax),%ecx
80100302:	85 c9                	test   %ecx,%ecx
80100304:	74 d2                	je     801002d8 <consoleread+0x48>
        release(&cons.lock);
80100306:	83 ec 0c             	sub    $0xc,%esp
80100309:	68 20 c5 10 80       	push   $0x8010c520
8010030e:	e8 bd 59 00 00       	call   80105cd0 <release>
        ilock(ip);
80100313:	5a                   	pop    %edx
80100314:	ff 75 08             	pushl  0x8(%ebp)
80100317:	e8 34 19 00 00       	call   80101c50 <ilock>
        return -1;
8010031c:	83 c4 10             	add    $0x10,%esp
  }
  release(&cons.lock);
  ilock(ip);

  return target - n;
}
8010031f:	8d 65 f4             	lea    -0xc(%ebp),%esp
        return -1;
80100322:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80100327:	5b                   	pop    %ebx
80100328:	5e                   	pop    %esi
80100329:	5f                   	pop    %edi
8010032a:	5d                   	pop    %ebp
8010032b:	c3                   	ret    
8010032c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    c = input.buf[input.r++ % INPUT_BUF];
80100330:	8d 50 01             	lea    0x1(%eax),%edx
80100333:	89 15 00 20 11 80    	mov    %edx,0x80112000
80100339:	89 c2                	mov    %eax,%edx
8010033b:	83 e2 7f             	and    $0x7f,%edx
8010033e:	0f be 8a 80 1f 11 80 	movsbl -0x7feee080(%edx),%ecx
    if(c == C('D')){  // EOF
80100345:	80 f9 04             	cmp    $0x4,%cl
80100348:	74 38                	je     80100382 <consoleread+0xf2>
    *dst++ = c;
8010034a:	89 d8                	mov    %ebx,%eax
    --n;
8010034c:	83 eb 01             	sub    $0x1,%ebx
    *dst++ = c;
8010034f:	f7 d8                	neg    %eax
80100351:	88 0c 07             	mov    %cl,(%edi,%eax,1)
    if(c == '\n')
80100354:	83 f9 0a             	cmp    $0xa,%ecx
80100357:	0f 85 61 ff ff ff    	jne    801002be <consoleread+0x2e>
  release(&cons.lock);
8010035d:	83 ec 0c             	sub    $0xc,%esp
80100360:	68 20 c5 10 80       	push   $0x8010c520
80100365:	e8 66 59 00 00       	call   80105cd0 <release>
  ilock(ip);
8010036a:	58                   	pop    %eax
8010036b:	ff 75 08             	pushl  0x8(%ebp)
8010036e:	e8 dd 18 00 00       	call   80101c50 <ilock>
  return target - n;
80100373:	89 f0                	mov    %esi,%eax
80100375:	83 c4 10             	add    $0x10,%esp
}
80100378:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return target - n;
8010037b:	29 d8                	sub    %ebx,%eax
}
8010037d:	5b                   	pop    %ebx
8010037e:	5e                   	pop    %esi
8010037f:	5f                   	pop    %edi
80100380:	5d                   	pop    %ebp
80100381:	c3                   	ret    
      if(n < target){
80100382:	39 f3                	cmp    %esi,%ebx
80100384:	73 d7                	jae    8010035d <consoleread+0xcd>
        input.r--;
80100386:	a3 00 20 11 80       	mov    %eax,0x80112000
8010038b:	eb d0                	jmp    8010035d <consoleread+0xcd>
8010038d:	8d 76 00             	lea    0x0(%esi),%esi

80100390 <panic>:
{
80100390:	f3 0f 1e fb          	endbr32 
80100394:	55                   	push   %ebp
80100395:	89 e5                	mov    %esp,%ebp
80100397:	56                   	push   %esi
80100398:	53                   	push   %ebx
80100399:	83 ec 30             	sub    $0x30,%esp
}

static inline void
cli(void)
{
  asm volatile("cli");
8010039c:	fa                   	cli    
  cons.locking = 0;
8010039d:	c7 05 54 c5 10 80 00 	movl   $0x0,0x8010c554
801003a4:	00 00 00 
  getcallerpcs(&s, pcs);
801003a7:	8d 5d d0             	lea    -0x30(%ebp),%ebx
801003aa:	8d 75 f8             	lea    -0x8(%ebp),%esi
  cprintf("lapicid %d: panic: ", lapicid());
801003ad:	e8 de 29 00 00       	call   80102d90 <lapicid>
801003b2:	83 ec 08             	sub    $0x8,%esp
801003b5:	50                   	push   %eax
801003b6:	68 cd 8b 10 80       	push   $0x80108bcd
801003bb:	e8 00 05 00 00       	call   801008c0 <cprintf>
  cprintf(s);
801003c0:	58                   	pop    %eax
801003c1:	ff 75 08             	pushl  0x8(%ebp)
801003c4:	e8 f7 04 00 00       	call   801008c0 <cprintf>
  cprintf("\n");
801003c9:	c7 04 24 25 92 10 80 	movl   $0x80109225,(%esp)
801003d0:	e8 eb 04 00 00       	call   801008c0 <cprintf>
  getcallerpcs(&s, pcs);
801003d5:	8d 45 08             	lea    0x8(%ebp),%eax
801003d8:	5a                   	pop    %edx
801003d9:	59                   	pop    %ecx
801003da:	53                   	push   %ebx
801003db:	50                   	push   %eax
801003dc:	e8 cf 56 00 00       	call   80105ab0 <getcallerpcs>
  for(i=0; i<10; i++)
801003e1:	83 c4 10             	add    $0x10,%esp
    cprintf(" %p", pcs[i]);
801003e4:	83 ec 08             	sub    $0x8,%esp
801003e7:	ff 33                	pushl  (%ebx)
801003e9:	83 c3 04             	add    $0x4,%ebx
801003ec:	68 e1 8b 10 80       	push   $0x80108be1
801003f1:	e8 ca 04 00 00       	call   801008c0 <cprintf>
  for(i=0; i<10; i++)
801003f6:	83 c4 10             	add    $0x10,%esp
801003f9:	39 f3                	cmp    %esi,%ebx
801003fb:	75 e7                	jne    801003e4 <panic+0x54>
  panicked = 1; // freeze other CPU
801003fd:	c7 05 5c c5 10 80 01 	movl   $0x1,0x8010c55c
80100404:	00 00 00 
  for(;;)
80100407:	eb fe                	jmp    80100407 <panic+0x77>
80100409:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80100410 <consputc.part.0>:
consputc(int c) // Important function 
80100410:	55                   	push   %ebp
80100411:	89 e5                	mov    %esp,%ebp
80100413:	57                   	push   %edi
80100414:	56                   	push   %esi
80100415:	89 c6                	mov    %eax,%esi
80100417:	53                   	push   %ebx
80100418:	83 ec 1c             	sub    $0x1c,%esp
  if(c == BACKSPACE){
8010041b:	3d 00 01 00 00       	cmp    $0x100,%eax
80100420:	0f 84 62 01 00 00    	je     80100588 <consputc.part.0+0x178>
    uartputc(c);
80100426:	83 ec 0c             	sub    $0xc,%esp
80100429:	50                   	push   %eax
8010042a:	e8 61 73 00 00       	call   80107790 <uartputc>
8010042f:	83 c4 10             	add    $0x10,%esp
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80100432:	bf d4 03 00 00       	mov    $0x3d4,%edi
80100437:	b8 0e 00 00 00       	mov    $0xe,%eax
8010043c:	89 fa                	mov    %edi,%edx
8010043e:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
8010043f:	b9 d5 03 00 00       	mov    $0x3d5,%ecx
80100444:	89 ca                	mov    %ecx,%edx
80100446:	ec                   	in     (%dx),%al
  pos = inb(CRTPORT+1) << 8;
80100447:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
8010044a:	89 fa                	mov    %edi,%edx
8010044c:	c1 e0 08             	shl    $0x8,%eax
8010044f:	89 c3                	mov    %eax,%ebx
80100451:	b8 0f 00 00 00       	mov    $0xf,%eax
80100456:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80100457:	89 ca                	mov    %ecx,%edx
80100459:	ec                   	in     (%dx),%al
  pos |= inb(CRTPORT+1);
8010045a:	0f b6 c0             	movzbl %al,%eax
8010045d:	09 d8                	or     %ebx,%eax
  if(c == '\n'){
8010045f:	83 fe 0a             	cmp    $0xa,%esi
80100462:	0f 84 f8 00 00 00    	je     80100560 <consputc.part.0+0x150>
  else if(c == BACKSPACE){
80100468:	8b 3d 58 c5 10 80    	mov    0x8010c558,%edi
8010046e:	8d 0c 38             	lea    (%eax,%edi,1),%ecx
80100471:	81 fe 00 01 00 00    	cmp    $0x100,%esi
80100477:	0f 84 a3 00 00 00    	je     80100520 <consputc.part.0+0x110>
  for (int i = pos + backs; i > pos; i--)
8010047d:	39 c8                	cmp    %ecx,%eax
8010047f:	7d 1d                	jge    8010049e <consputc.part.0+0x8e>
80100481:	8d 94 09 fe 7f 0b 80 	lea    -0x7ff48002(%ecx,%ecx,1),%edx
80100488:	8d 9c 00 fe 7f 0b 80 	lea    -0x7ff48002(%eax,%eax,1),%ebx
8010048f:	90                   	nop
    crt[i] = crt[i - 1];
80100490:	0f b7 0a             	movzwl (%edx),%ecx
80100493:	83 ea 02             	sub    $0x2,%edx
80100496:	66 89 4a 04          	mov    %cx,0x4(%edx)
  for (int i = pos + backs; i > pos; i--)
8010049a:	39 d3                	cmp    %edx,%ebx
8010049c:	75 f2                	jne    80100490 <consputc.part.0+0x80>
    crt[pos++] = (c&0xff) | 0x0700;  // black on white
8010049e:	89 f1                	mov    %esi,%ecx
801004a0:	8d 58 01             	lea    0x1(%eax),%ebx
801004a3:	0f b6 f1             	movzbl %cl,%esi
801004a6:	66 81 ce 00 07       	or     $0x700,%si
801004ab:	66 89 b4 00 00 80 0b 	mov    %si,-0x7ff48000(%eax,%eax,1)
801004b2:	80 
  if(pos < 0 || pos > 25*80)
801004b3:	81 fb d0 07 00 00    	cmp    $0x7d0,%ebx
801004b9:	0f 8f 45 01 00 00    	jg     80100604 <consputc.part.0+0x1f4>
  if((pos/80) >= 24){  // Scroll up.
801004bf:	81 fb 7f 07 00 00    	cmp    $0x77f,%ebx
801004c5:	0f 8f ed 00 00 00    	jg     801005b8 <consputc.part.0+0x1a8>
801004cb:	0f b6 c7             	movzbl %bh,%eax
801004ce:	8b 3d 58 c5 10 80    	mov    0x8010c558,%edi
801004d4:	89 de                	mov    %ebx,%esi
801004d6:	89 45 e4             	mov    %eax,-0x1c(%ebp)
801004d9:	01 df                	add    %ebx,%edi
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801004db:	bb d4 03 00 00       	mov    $0x3d4,%ebx
801004e0:	b8 0e 00 00 00       	mov    $0xe,%eax
801004e5:	89 da                	mov    %ebx,%edx
801004e7:	ee                   	out    %al,(%dx)
801004e8:	b9 d5 03 00 00       	mov    $0x3d5,%ecx
801004ed:	0f b6 45 e4          	movzbl -0x1c(%ebp),%eax
801004f1:	89 ca                	mov    %ecx,%edx
801004f3:	ee                   	out    %al,(%dx)
801004f4:	b8 0f 00 00 00       	mov    $0xf,%eax
801004f9:	89 da                	mov    %ebx,%edx
801004fb:	ee                   	out    %al,(%dx)
801004fc:	89 f0                	mov    %esi,%eax
801004fe:	89 ca                	mov    %ecx,%edx
80100500:	ee                   	out    %al,(%dx)
  crt[pos + backs] = ' ' | 0x0700; //
80100501:	b8 20 07 00 00       	mov    $0x720,%eax
80100506:	66 89 84 3f 00 80 0b 	mov    %ax,-0x7ff48000(%edi,%edi,1)
8010050d:	80 
}
8010050e:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100511:	5b                   	pop    %ebx
80100512:	5e                   	pop    %esi
80100513:	5f                   	pop    %edi
80100514:	5d                   	pop    %ebp
80100515:	c3                   	ret    
80100516:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010051d:	8d 76 00             	lea    0x0(%esi),%esi
  for (int i = pos - 1; i < pos + backs; i++)
80100520:	8d 58 ff             	lea    -0x1(%eax),%ebx
80100523:	8d 94 00 00 80 0b 80 	lea    -0x7ff48000(%eax,%eax,1),%edx
8010052a:	89 de                	mov    %ebx,%esi
8010052c:	85 ff                	test   %edi,%edi
8010052e:	78 1c                	js     8010054c <consputc.part.0+0x13c>
80100530:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80100533:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80100537:	90                   	nop
      crt[i] = crt[i + 1];
80100538:	0f b7 02             	movzwl (%edx),%eax
  for (int i = pos - 1; i < pos + backs; i++)
8010053b:	83 c6 01             	add    $0x1,%esi
8010053e:	83 c2 02             	add    $0x2,%edx
      crt[i] = crt[i + 1];
80100541:	66 89 42 fc          	mov    %ax,-0x4(%edx)
  for (int i = pos - 1; i < pos + backs; i++)
80100545:	39 ce                	cmp    %ecx,%esi
80100547:	7c ef                	jl     80100538 <consputc.part.0+0x128>
80100549:	8b 45 e4             	mov    -0x1c(%ebp),%eax
    if(pos > 0) --pos;
8010054c:	85 c0                	test   %eax,%eax
8010054e:	0f 85 5f ff ff ff    	jne    801004b3 <consputc.part.0+0xa3>
80100554:	c6 45 e4 00          	movb   $0x0,-0x1c(%ebp)
80100558:	31 f6                	xor    %esi,%esi
8010055a:	e9 7c ff ff ff       	jmp    801004db <consputc.part.0+0xcb>
8010055f:	90                   	nop
    backs = 0;
80100560:	c7 05 58 c5 10 80 00 	movl   $0x0,0x8010c558
80100567:	00 00 00 
    pos += 80 - pos%80;
8010056a:	ba cd cc cc cc       	mov    $0xcccccccd,%edx
8010056f:	f7 e2                	mul    %edx
80100571:	c1 ea 06             	shr    $0x6,%edx
80100574:	8d 04 92             	lea    (%edx,%edx,4),%eax
80100577:	c1 e0 04             	shl    $0x4,%eax
8010057a:	8d 58 50             	lea    0x50(%eax),%ebx
    backs = 0;
8010057d:	e9 31 ff ff ff       	jmp    801004b3 <consputc.part.0+0xa3>
80100582:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    uartputc('\b'); uartputc(' '); uartputc('\b');      
80100588:	83 ec 0c             	sub    $0xc,%esp
8010058b:	6a 08                	push   $0x8
8010058d:	e8 fe 71 00 00       	call   80107790 <uartputc>
80100592:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80100599:	e8 f2 71 00 00       	call   80107790 <uartputc>
8010059e:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
801005a5:	e8 e6 71 00 00       	call   80107790 <uartputc>
801005aa:	83 c4 10             	add    $0x10,%esp
801005ad:	e9 80 fe ff ff       	jmp    80100432 <consputc.part.0+0x22>
801005b2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    memmove(crt, crt+80, sizeof(crt[0])*23*80);
801005b8:	83 ec 04             	sub    $0x4,%esp
    pos -= 80;
801005bb:	83 eb 50             	sub    $0x50,%ebx
    memmove(crt, crt+80, sizeof(crt[0])*23*80);
801005be:	68 60 0e 00 00       	push   $0xe60
801005c3:	89 de                	mov    %ebx,%esi
801005c5:	68 a0 80 0b 80       	push   $0x800b80a0
801005ca:	68 00 80 0b 80       	push   $0x800b8000
801005cf:	e8 ec 57 00 00       	call   80105dc0 <memmove>
    memset(crt+pos, 0, sizeof(crt[0])*(24*80 - pos));
801005d4:	b8 80 07 00 00       	mov    $0x780,%eax
801005d9:	83 c4 0c             	add    $0xc,%esp
801005dc:	29 d8                	sub    %ebx,%eax
801005de:	01 c0                	add    %eax,%eax
801005e0:	50                   	push   %eax
801005e1:	8d 84 1b 00 80 0b 80 	lea    -0x7ff48000(%ebx,%ebx,1),%eax
801005e8:	6a 00                	push   $0x0
801005ea:	50                   	push   %eax
801005eb:	e8 30 57 00 00       	call   80105d20 <memset>
801005f0:	8b 3d 58 c5 10 80    	mov    0x8010c558,%edi
801005f6:	c6 45 e4 07          	movb   $0x7,-0x1c(%ebp)
801005fa:	83 c4 10             	add    $0x10,%esp
801005fd:	01 df                	add    %ebx,%edi
801005ff:	e9 d7 fe ff ff       	jmp    801004db <consputc.part.0+0xcb>
    panic("pos under/overflow");
80100604:	83 ec 0c             	sub    $0xc,%esp
80100607:	68 e5 8b 10 80       	push   $0x80108be5
8010060c:	e8 7f fd ff ff       	call   80100390 <panic>
80100611:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100618:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010061f:	90                   	nop

80100620 <printint>:
{
80100620:	55                   	push   %ebp
80100621:	89 e5                	mov    %esp,%ebp
80100623:	57                   	push   %edi
80100624:	56                   	push   %esi
80100625:	53                   	push   %ebx
80100626:	83 ec 2c             	sub    $0x2c,%esp
80100629:	89 55 d4             	mov    %edx,-0x2c(%ebp)
  if(sign && (sign = xx < 0))
8010062c:	85 c9                	test   %ecx,%ecx
8010062e:	74 04                	je     80100634 <printint+0x14>
80100630:	85 c0                	test   %eax,%eax
80100632:	78 6d                	js     801006a1 <printint+0x81>
    x = xx;
80100634:	89 c1                	mov    %eax,%ecx
80100636:	31 f6                	xor    %esi,%esi
  i = 0;
80100638:	89 75 cc             	mov    %esi,-0x34(%ebp)
8010063b:	31 db                	xor    %ebx,%ebx
8010063d:	8d 7d d7             	lea    -0x29(%ebp),%edi
    buf[i++] = digits[x % base];
80100640:	89 c8                	mov    %ecx,%eax
80100642:	31 d2                	xor    %edx,%edx
80100644:	89 ce                	mov    %ecx,%esi
80100646:	f7 75 d4             	divl   -0x2c(%ebp)
80100649:	0f b6 92 68 8c 10 80 	movzbl -0x7fef7398(%edx),%edx
80100650:	89 45 d0             	mov    %eax,-0x30(%ebp)
80100653:	89 d8                	mov    %ebx,%eax
80100655:	8d 5b 01             	lea    0x1(%ebx),%ebx
  }while((x /= base) != 0);
80100658:	8b 4d d0             	mov    -0x30(%ebp),%ecx
8010065b:	89 75 d0             	mov    %esi,-0x30(%ebp)
    buf[i++] = digits[x % base];
8010065e:	88 14 1f             	mov    %dl,(%edi,%ebx,1)
  }while((x /= base) != 0);
80100661:	8b 75 d4             	mov    -0x2c(%ebp),%esi
80100664:	39 75 d0             	cmp    %esi,-0x30(%ebp)
80100667:	73 d7                	jae    80100640 <printint+0x20>
80100669:	8b 75 cc             	mov    -0x34(%ebp),%esi
  if(sign)
8010066c:	85 f6                	test   %esi,%esi
8010066e:	74 0c                	je     8010067c <printint+0x5c>
    buf[i++] = '-';
80100670:	c6 44 1d d8 2d       	movb   $0x2d,-0x28(%ebp,%ebx,1)
    buf[i++] = digits[x % base];
80100675:	89 d8                	mov    %ebx,%eax
    buf[i++] = '-';
80100677:	ba 2d 00 00 00       	mov    $0x2d,%edx
  while(--i >= 0)
8010067c:	8d 5c 05 d7          	lea    -0x29(%ebp,%eax,1),%ebx
80100680:	0f be c2             	movsbl %dl,%eax
  if(panicked){
80100683:	8b 15 5c c5 10 80    	mov    0x8010c55c,%edx
80100689:	85 d2                	test   %edx,%edx
8010068b:	74 03                	je     80100690 <printint+0x70>
  asm volatile("cli");
8010068d:	fa                   	cli    
    for(;;)
8010068e:	eb fe                	jmp    8010068e <printint+0x6e>
80100690:	e8 7b fd ff ff       	call   80100410 <consputc.part.0>
  while(--i >= 0)
80100695:	39 fb                	cmp    %edi,%ebx
80100697:	74 10                	je     801006a9 <printint+0x89>
80100699:	0f be 03             	movsbl (%ebx),%eax
8010069c:	83 eb 01             	sub    $0x1,%ebx
8010069f:	eb e2                	jmp    80100683 <printint+0x63>
    x = -xx;
801006a1:	f7 d8                	neg    %eax
801006a3:	89 ce                	mov    %ecx,%esi
801006a5:	89 c1                	mov    %eax,%ecx
801006a7:	eb 8f                	jmp    80100638 <printint+0x18>
}
801006a9:	83 c4 2c             	add    $0x2c,%esp
801006ac:	5b                   	pop    %ebx
801006ad:	5e                   	pop    %esi
801006ae:	5f                   	pop    %edi
801006af:	5d                   	pop    %ebp
801006b0:	c3                   	ret    
801006b1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801006b8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801006bf:	90                   	nop

801006c0 <arrow>:
static void arrow(enum Arrow arr){
801006c0:	55                   	push   %ebp
801006c1:	89 e5                	mov    %esp,%ebp
801006c3:	57                   	push   %edi
801006c4:	56                   	push   %esi
801006c5:	53                   	push   %ebx
801006c6:	83 ec 1c             	sub    $0x1c,%esp
801006c9:	89 45 e0             	mov    %eax,-0x20(%ebp)
  for (int i= 0 ; i < backs ; i ++){
801006cc:	a1 58 c5 10 80       	mov    0x8010c558,%eax
801006d1:	89 45 e4             	mov    %eax,-0x1c(%ebp)
801006d4:	85 c0                	test   %eax,%eax
801006d6:	7e 57                	jle    8010072f <arrow+0x6f>
801006d8:	31 ff                	xor    %edi,%edi
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801006da:	bb d4 03 00 00       	mov    $0x3d4,%ebx
801006df:	b8 0e 00 00 00       	mov    $0xe,%eax
801006e4:	89 da                	mov    %ebx,%edx
801006e6:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801006e7:	be d5 03 00 00       	mov    $0x3d5,%esi
801006ec:	89 f2                	mov    %esi,%edx
801006ee:	ec                   	in     (%dx),%al
801006ef:	0f b6 c8             	movzbl %al,%ecx
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801006f2:	89 da                	mov    %ebx,%edx
801006f4:	b8 0f 00 00 00       	mov    $0xf,%eax
  pos = inb(CRTPORT + 1) << 8;
801006f9:	c1 e1 08             	shl    $0x8,%ecx
801006fc:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801006fd:	89 f2                	mov    %esi,%edx
801006ff:	ec                   	in     (%dx),%al
  pos |= inb(CRTPORT + 1);
80100700:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80100703:	89 da                	mov    %ebx,%edx
80100705:	09 c1                	or     %eax,%ecx
80100707:	b8 0e 00 00 00       	mov    $0xe,%eax
    pos++;
8010070c:	83 c1 01             	add    $0x1,%ecx
8010070f:	ee                   	out    %al,(%dx)
  outb(CRTPORT + 1, pos >> 8);
80100710:	89 ca                	mov    %ecx,%edx
80100712:	c1 fa 08             	sar    $0x8,%edx
80100715:	89 d0                	mov    %edx,%eax
80100717:	89 f2                	mov    %esi,%edx
80100719:	ee                   	out    %al,(%dx)
8010071a:	b8 0f 00 00 00       	mov    $0xf,%eax
8010071f:	89 da                	mov    %ebx,%edx
80100721:	ee                   	out    %al,(%dx)
80100722:	89 c8                	mov    %ecx,%eax
80100724:	89 f2                	mov    %esi,%edx
80100726:	ee                   	out    %al,(%dx)
  for (int i= 0 ; i < backs ; i ++){
80100727:	83 c7 01             	add    $0x1,%edi
8010072a:	3b 7d e4             	cmp    -0x1c(%ebp),%edi
8010072d:	75 b0                	jne    801006df <arrow+0x1f>
  backs = 0;
8010072f:	c7 05 58 c5 10 80 00 	movl   $0x0,0x8010c558
80100736:	00 00 00 
  for ( int i = input.e ; i > input.w ; i-- ){
80100739:	8b 1d 08 20 11 80    	mov    0x80112008,%ebx
8010073f:	3b 1d 04 20 11 80    	cmp    0x80112004,%ebx
80100745:	76 2b                	jbe    80100772 <arrow+0xb2>
    if (input.buf[i - 1] != '\n'){
80100747:	83 eb 01             	sub    $0x1,%ebx
8010074a:	80 bb 80 1f 11 80 0a 	cmpb   $0xa,-0x7feee080(%ebx)
80100751:	74 17                	je     8010076a <arrow+0xaa>
  if(panicked){
80100753:	8b 15 5c c5 10 80    	mov    0x8010c55c,%edx
80100759:	85 d2                	test   %edx,%edx
8010075b:	74 03                	je     80100760 <arrow+0xa0>
  asm volatile("cli");
8010075d:	fa                   	cli    
    for(;;)
8010075e:	eb fe                	jmp    8010075e <arrow+0x9e>
80100760:	b8 00 01 00 00       	mov    $0x100,%eax
80100765:	e8 a6 fc ff ff       	call   80100410 <consputc.part.0>
  for ( int i = input.e ; i > input.w ; i-- ){
8010076a:	39 1d 04 20 11 80    	cmp    %ebx,0x80112004
80100770:	72 d5                	jb     80100747 <arrow+0x87>
  if (arr == UP){
80100772:	8b 4d e0             	mov    -0x20(%ebp),%ecx
80100775:	a1 98 25 11 80       	mov    0x80112598,%eax
8010077a:	85 c9                	test   %ecx,%ecx
8010077c:	74 50                	je     801007ce <arrow+0x10e>
  if ((arr == DOWN)&&(history.index < 9)&&(history.index + 1 < history.last )){
8010077e:	83 f8 08             	cmp    $0x8,%eax
80100781:	0f 8f b8 00 00 00    	jg     8010083f <arrow+0x17f>
80100787:	8d 50 01             	lea    0x1(%eax),%edx
8010078a:	3b 15 a0 25 11 80    	cmp    0x801125a0,%edx
80100790:	0f 8d a9 00 00 00    	jge    8010083f <arrow+0x17f>
    input = history.hist[history.index + 2 ];
80100796:	8d 70 02             	lea    0x2(%eax),%esi
80100799:	bf 80 1f 11 80       	mov    $0x80111f80,%edi
8010079e:	b9 23 00 00 00       	mov    $0x23,%ecx
    history.index ++ ;
801007a3:	89 15 98 25 11 80    	mov    %edx,0x80112598
    input = history.hist[history.index + 2 ];
801007a9:	69 f6 8c 00 00 00    	imul   $0x8c,%esi,%esi
801007af:	81 c6 20 20 11 80    	add    $0x80112020,%esi
801007b5:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
    input.e -- ;
801007b7:	8b 15 08 20 11 80    	mov    0x80112008,%edx
801007bd:	8d 42 ff             	lea    -0x1(%edx),%eax
    input.buf[input.e] = '\0';
801007c0:	c6 82 7f 1f 11 80 00 	movb   $0x0,-0x7feee081(%edx)
    input.e -- ;
801007c7:	a3 08 20 11 80       	mov    %eax,0x80112008
    input.buf[input.e] = '\0';
801007cc:	eb 35                	jmp    80100803 <arrow+0x143>
    input = history.hist[history.index ];
801007ce:	69 f0 8c 00 00 00    	imul   $0x8c,%eax,%esi
801007d4:	bf 80 1f 11 80       	mov    $0x80111f80,%edi
801007d9:	b9 23 00 00 00       	mov    $0x23,%ecx
    history.index -- ;
801007de:	83 e8 01             	sub    $0x1,%eax
801007e1:	a3 98 25 11 80       	mov    %eax,0x80112598
    input = history.hist[history.index ];
801007e6:	81 c6 20 20 11 80    	add    $0x80112020,%esi
801007ec:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
    input.e -- ;
801007ee:	8b 15 08 20 11 80    	mov    0x80112008,%edx
801007f4:	8d 42 ff             	lea    -0x1(%edx),%eax
    input.buf[input.e] = '\0';
801007f7:	c6 82 7f 1f 11 80 00 	movb   $0x0,-0x7feee081(%edx)
    input.e -- ;
801007fe:	a3 08 20 11 80       	mov    %eax,0x80112008
  for (int i = input.w ; i < input.e; i++)
80100803:	8b 1d 04 20 11 80    	mov    0x80112004,%ebx
80100809:	39 d8                	cmp    %ebx,%eax
8010080b:	76 2a                	jbe    80100837 <arrow+0x177>
  if(panicked){
8010080d:	a1 5c c5 10 80       	mov    0x8010c55c,%eax
80100812:	85 c0                	test   %eax,%eax
80100814:	74 0a                	je     80100820 <arrow+0x160>
80100816:	fa                   	cli    
    for(;;)
80100817:	eb fe                	jmp    80100817 <arrow+0x157>
80100819:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    consputc(input.buf[i]);
80100820:	0f be 83 80 1f 11 80 	movsbl -0x7feee080(%ebx),%eax
  for (int i = input.w ; i < input.e; i++)
80100827:	83 c3 01             	add    $0x1,%ebx
8010082a:	e8 e1 fb ff ff       	call   80100410 <consputc.part.0>
8010082f:	39 1d 08 20 11 80    	cmp    %ebx,0x80112008
80100835:	77 d6                	ja     8010080d <arrow+0x14d>
}
80100837:	83 c4 1c             	add    $0x1c,%esp
8010083a:	5b                   	pop    %ebx
8010083b:	5e                   	pop    %esi
8010083c:	5f                   	pop    %edi
8010083d:	5d                   	pop    %ebp
8010083e:	c3                   	ret    
8010083f:	a1 08 20 11 80       	mov    0x80112008,%eax
80100844:	eb bd                	jmp    80100803 <arrow+0x143>
80100846:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010084d:	8d 76 00             	lea    0x0(%esi),%esi

80100850 <consolewrite>:

int
consolewrite(struct inode *ip, char *buf, int n)
{
80100850:	f3 0f 1e fb          	endbr32 
80100854:	55                   	push   %ebp
80100855:	89 e5                	mov    %esp,%ebp
80100857:	57                   	push   %edi
80100858:	56                   	push   %esi
80100859:	53                   	push   %ebx
8010085a:	83 ec 18             	sub    $0x18,%esp
  int i;

  iunlock(ip);
8010085d:	ff 75 08             	pushl  0x8(%ebp)
{
80100860:	8b 5d 10             	mov    0x10(%ebp),%ebx
  iunlock(ip);
80100863:	e8 c8 14 00 00       	call   80101d30 <iunlock>
  acquire(&cons.lock);
80100868:	c7 04 24 20 c5 10 80 	movl   $0x8010c520,(%esp)
8010086f:	e8 9c 53 00 00       	call   80105c10 <acquire>
  for(i = 0; i < n; i++)
80100874:	83 c4 10             	add    $0x10,%esp
80100877:	85 db                	test   %ebx,%ebx
80100879:	7e 24                	jle    8010089f <consolewrite+0x4f>
8010087b:	8b 7d 0c             	mov    0xc(%ebp),%edi
8010087e:	8d 34 1f             	lea    (%edi,%ebx,1),%esi
  if(panicked){
80100881:	8b 15 5c c5 10 80    	mov    0x8010c55c,%edx
80100887:	85 d2                	test   %edx,%edx
80100889:	74 05                	je     80100890 <consolewrite+0x40>
8010088b:	fa                   	cli    
    for(;;)
8010088c:	eb fe                	jmp    8010088c <consolewrite+0x3c>
8010088e:	66 90                	xchg   %ax,%ax
    consputc(buf[i] & 0xff);
80100890:	0f b6 07             	movzbl (%edi),%eax
80100893:	83 c7 01             	add    $0x1,%edi
80100896:	e8 75 fb ff ff       	call   80100410 <consputc.part.0>
  for(i = 0; i < n; i++)
8010089b:	39 fe                	cmp    %edi,%esi
8010089d:	75 e2                	jne    80100881 <consolewrite+0x31>
  release(&cons.lock);
8010089f:	83 ec 0c             	sub    $0xc,%esp
801008a2:	68 20 c5 10 80       	push   $0x8010c520
801008a7:	e8 24 54 00 00       	call   80105cd0 <release>
  ilock(ip);
801008ac:	58                   	pop    %eax
801008ad:	ff 75 08             	pushl  0x8(%ebp)
801008b0:	e8 9b 13 00 00       	call   80101c50 <ilock>

  return n;
}
801008b5:	8d 65 f4             	lea    -0xc(%ebp),%esp
801008b8:	89 d8                	mov    %ebx,%eax
801008ba:	5b                   	pop    %ebx
801008bb:	5e                   	pop    %esi
801008bc:	5f                   	pop    %edi
801008bd:	5d                   	pop    %ebp
801008be:	c3                   	ret    
801008bf:	90                   	nop

801008c0 <cprintf>:
{
801008c0:	f3 0f 1e fb          	endbr32 
801008c4:	55                   	push   %ebp
801008c5:	89 e5                	mov    %esp,%ebp
801008c7:	57                   	push   %edi
801008c8:	56                   	push   %esi
801008c9:	53                   	push   %ebx
801008ca:	83 ec 1c             	sub    $0x1c,%esp
  locking = cons.locking;
801008cd:	a1 54 c5 10 80       	mov    0x8010c554,%eax
801008d2:	89 45 e0             	mov    %eax,-0x20(%ebp)
  if(locking)
801008d5:	85 c0                	test   %eax,%eax
801008d7:	0f 85 e8 00 00 00    	jne    801009c5 <cprintf+0x105>
  if (fmt == 0)
801008dd:	8b 45 08             	mov    0x8(%ebp),%eax
801008e0:	89 45 e4             	mov    %eax,-0x1c(%ebp)
801008e3:	85 c0                	test   %eax,%eax
801008e5:	0f 84 5a 01 00 00    	je     80100a45 <cprintf+0x185>
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
801008eb:	0f b6 00             	movzbl (%eax),%eax
801008ee:	85 c0                	test   %eax,%eax
801008f0:	74 36                	je     80100928 <cprintf+0x68>
  argp = (uint*)(void*)(&fmt + 1);
801008f2:	8d 5d 0c             	lea    0xc(%ebp),%ebx
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
801008f5:	31 f6                	xor    %esi,%esi
    if(c != '%'){
801008f7:	83 f8 25             	cmp    $0x25,%eax
801008fa:	74 44                	je     80100940 <cprintf+0x80>
  if(panicked){
801008fc:	8b 0d 5c c5 10 80    	mov    0x8010c55c,%ecx
80100902:	85 c9                	test   %ecx,%ecx
80100904:	74 0f                	je     80100915 <cprintf+0x55>
80100906:	fa                   	cli    
    for(;;)
80100907:	eb fe                	jmp    80100907 <cprintf+0x47>
80100909:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100910:	b8 25 00 00 00       	mov    $0x25,%eax
80100915:	e8 f6 fa ff ff       	call   80100410 <consputc.part.0>
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
8010091a:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010091d:	83 c6 01             	add    $0x1,%esi
80100920:	0f b6 04 30          	movzbl (%eax,%esi,1),%eax
80100924:	85 c0                	test   %eax,%eax
80100926:	75 cf                	jne    801008f7 <cprintf+0x37>
  if(locking)
80100928:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010092b:	85 c0                	test   %eax,%eax
8010092d:	0f 85 fd 00 00 00    	jne    80100a30 <cprintf+0x170>
}
80100933:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100936:	5b                   	pop    %ebx
80100937:	5e                   	pop    %esi
80100938:	5f                   	pop    %edi
80100939:	5d                   	pop    %ebp
8010093a:	c3                   	ret    
8010093b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010093f:	90                   	nop
    c = fmt[++i] & 0xff;
80100940:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100943:	83 c6 01             	add    $0x1,%esi
80100946:	0f b6 3c 30          	movzbl (%eax,%esi,1),%edi
    if(c == 0)
8010094a:	85 ff                	test   %edi,%edi
8010094c:	74 da                	je     80100928 <cprintf+0x68>
    switch(c){
8010094e:	83 ff 70             	cmp    $0x70,%edi
80100951:	74 5a                	je     801009ad <cprintf+0xed>
80100953:	7f 2a                	jg     8010097f <cprintf+0xbf>
80100955:	83 ff 25             	cmp    $0x25,%edi
80100958:	0f 84 92 00 00 00    	je     801009f0 <cprintf+0x130>
8010095e:	83 ff 64             	cmp    $0x64,%edi
80100961:	0f 85 a1 00 00 00    	jne    80100a08 <cprintf+0x148>
      printint(*argp++, 10, 1);
80100967:	8b 03                	mov    (%ebx),%eax
80100969:	8d 7b 04             	lea    0x4(%ebx),%edi
8010096c:	b9 01 00 00 00       	mov    $0x1,%ecx
80100971:	ba 0a 00 00 00       	mov    $0xa,%edx
80100976:	89 fb                	mov    %edi,%ebx
80100978:	e8 a3 fc ff ff       	call   80100620 <printint>
      break;
8010097d:	eb 9b                	jmp    8010091a <cprintf+0x5a>
    switch(c){
8010097f:	83 ff 73             	cmp    $0x73,%edi
80100982:	75 24                	jne    801009a8 <cprintf+0xe8>
      if((s = (char*)*argp++) == 0)
80100984:	8d 7b 04             	lea    0x4(%ebx),%edi
80100987:	8b 1b                	mov    (%ebx),%ebx
80100989:	85 db                	test   %ebx,%ebx
8010098b:	75 55                	jne    801009e2 <cprintf+0x122>
        s = "(null)";
8010098d:	bb f8 8b 10 80       	mov    $0x80108bf8,%ebx
      for(; *s; s++)
80100992:	b8 28 00 00 00       	mov    $0x28,%eax
  if(panicked){
80100997:	8b 15 5c c5 10 80    	mov    0x8010c55c,%edx
8010099d:	85 d2                	test   %edx,%edx
8010099f:	74 39                	je     801009da <cprintf+0x11a>
801009a1:	fa                   	cli    
    for(;;)
801009a2:	eb fe                	jmp    801009a2 <cprintf+0xe2>
801009a4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    switch(c){
801009a8:	83 ff 78             	cmp    $0x78,%edi
801009ab:	75 5b                	jne    80100a08 <cprintf+0x148>
      printint(*argp++, 16, 0);
801009ad:	8b 03                	mov    (%ebx),%eax
801009af:	8d 7b 04             	lea    0x4(%ebx),%edi
801009b2:	31 c9                	xor    %ecx,%ecx
801009b4:	ba 10 00 00 00       	mov    $0x10,%edx
801009b9:	89 fb                	mov    %edi,%ebx
801009bb:	e8 60 fc ff ff       	call   80100620 <printint>
      break;
801009c0:	e9 55 ff ff ff       	jmp    8010091a <cprintf+0x5a>
    acquire(&cons.lock);
801009c5:	83 ec 0c             	sub    $0xc,%esp
801009c8:	68 20 c5 10 80       	push   $0x8010c520
801009cd:	e8 3e 52 00 00       	call   80105c10 <acquire>
801009d2:	83 c4 10             	add    $0x10,%esp
801009d5:	e9 03 ff ff ff       	jmp    801008dd <cprintf+0x1d>
801009da:	e8 31 fa ff ff       	call   80100410 <consputc.part.0>
      for(; *s; s++)
801009df:	83 c3 01             	add    $0x1,%ebx
801009e2:	0f be 03             	movsbl (%ebx),%eax
801009e5:	84 c0                	test   %al,%al
801009e7:	75 ae                	jne    80100997 <cprintf+0xd7>
      if((s = (char*)*argp++) == 0)
801009e9:	89 fb                	mov    %edi,%ebx
801009eb:	e9 2a ff ff ff       	jmp    8010091a <cprintf+0x5a>
  if(panicked){
801009f0:	8b 3d 5c c5 10 80    	mov    0x8010c55c,%edi
801009f6:	85 ff                	test   %edi,%edi
801009f8:	0f 84 12 ff ff ff    	je     80100910 <cprintf+0x50>
801009fe:	fa                   	cli    
    for(;;)
801009ff:	eb fe                	jmp    801009ff <cprintf+0x13f>
80100a01:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  if(panicked){
80100a08:	8b 0d 5c c5 10 80    	mov    0x8010c55c,%ecx
80100a0e:	85 c9                	test   %ecx,%ecx
80100a10:	74 06                	je     80100a18 <cprintf+0x158>
80100a12:	fa                   	cli    
    for(;;)
80100a13:	eb fe                	jmp    80100a13 <cprintf+0x153>
80100a15:	8d 76 00             	lea    0x0(%esi),%esi
80100a18:	b8 25 00 00 00       	mov    $0x25,%eax
80100a1d:	e8 ee f9 ff ff       	call   80100410 <consputc.part.0>
  if(panicked){
80100a22:	8b 15 5c c5 10 80    	mov    0x8010c55c,%edx
80100a28:	85 d2                	test   %edx,%edx
80100a2a:	74 2c                	je     80100a58 <cprintf+0x198>
80100a2c:	fa                   	cli    
    for(;;)
80100a2d:	eb fe                	jmp    80100a2d <cprintf+0x16d>
80100a2f:	90                   	nop
    release(&cons.lock);
80100a30:	83 ec 0c             	sub    $0xc,%esp
80100a33:	68 20 c5 10 80       	push   $0x8010c520
80100a38:	e8 93 52 00 00       	call   80105cd0 <release>
80100a3d:	83 c4 10             	add    $0x10,%esp
}
80100a40:	e9 ee fe ff ff       	jmp    80100933 <cprintf+0x73>
    panic("null fmt");
80100a45:	83 ec 0c             	sub    $0xc,%esp
80100a48:	68 ff 8b 10 80       	push   $0x80108bff
80100a4d:	e8 3e f9 ff ff       	call   80100390 <panic>
80100a52:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80100a58:	89 f8                	mov    %edi,%eax
80100a5a:	e8 b1 f9 ff ff       	call   80100410 <consputc.part.0>
80100a5f:	e9 b6 fe ff ff       	jmp    8010091a <cprintf+0x5a>
80100a64:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100a6b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80100a6f:	90                   	nop

80100a70 <consoleintr>:
{
80100a70:	f3 0f 1e fb          	endbr32 
80100a74:	55                   	push   %ebp
80100a75:	89 e5                	mov    %esp,%ebp
80100a77:	57                   	push   %edi
80100a78:	56                   	push   %esi
80100a79:	53                   	push   %ebx
  int c, doprocdump = 0;
80100a7a:	31 db                	xor    %ebx,%ebx
{
80100a7c:	83 ec 28             	sub    $0x28,%esp
80100a7f:	8b 45 08             	mov    0x8(%ebp),%eax
  acquire(&cons.lock);
80100a82:	68 20 c5 10 80       	push   $0x8010c520
{
80100a87:	89 45 dc             	mov    %eax,-0x24(%ebp)
  acquire(&cons.lock);
80100a8a:	e8 81 51 00 00       	call   80105c10 <acquire>
  while((c = getc()) >= 0){
80100a8f:	83 c4 10             	add    $0x10,%esp
80100a92:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100a95:	ff d0                	call   *%eax
80100a97:	89 c6                	mov    %eax,%esi
80100a99:	85 c0                	test   %eax,%eax
80100a9b:	0f 88 bc 02 00 00    	js     80100d5d <consoleintr+0x2ed>
    switch(c){
80100aa1:	83 fe 15             	cmp    $0x15,%esi
80100aa4:	7f 5a                	jg     80100b00 <consoleintr+0x90>
80100aa6:	83 fe 01             	cmp    $0x1,%esi
80100aa9:	0f 8e b1 01 00 00    	jle    80100c60 <consoleintr+0x1f0>
80100aaf:	83 fe 15             	cmp    $0x15,%esi
80100ab2:	0f 87 a8 01 00 00    	ja     80100c60 <consoleintr+0x1f0>
80100ab8:	3e ff 24 b5 10 8c 10 	notrack jmp *-0x7fef73f0(,%esi,4)
80100abf:	80 
80100ac0:	bb 01 00 00 00       	mov    $0x1,%ebx
80100ac5:	eb cb                	jmp    80100a92 <consoleintr+0x22>
80100ac7:	b8 00 01 00 00       	mov    $0x100,%eax
80100acc:	e8 3f f9 ff ff       	call   80100410 <consputc.part.0>
      while(input.e != input.w &&
80100ad1:	a1 08 20 11 80       	mov    0x80112008,%eax
80100ad6:	3b 05 04 20 11 80    	cmp    0x80112004,%eax
80100adc:	74 b4                	je     80100a92 <consoleintr+0x22>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
80100ade:	83 e8 01             	sub    $0x1,%eax
80100ae1:	89 c2                	mov    %eax,%edx
80100ae3:	83 e2 7f             	and    $0x7f,%edx
      while(input.e != input.w &&
80100ae6:	80 ba 80 1f 11 80 0a 	cmpb   $0xa,-0x7feee080(%edx)
80100aed:	74 a3                	je     80100a92 <consoleintr+0x22>
        input.e--;
80100aef:	a3 08 20 11 80       	mov    %eax,0x80112008
  if(panicked){
80100af4:	a1 5c c5 10 80       	mov    0x8010c55c,%eax
80100af9:	85 c0                	test   %eax,%eax
80100afb:	74 ca                	je     80100ac7 <consoleintr+0x57>
80100afd:	fa                   	cli    
    for(;;)
80100afe:	eb fe                	jmp    80100afe <consoleintr+0x8e>
    switch(c){
80100b00:	81 fe e2 00 00 00    	cmp    $0xe2,%esi
80100b06:	0f 84 24 02 00 00    	je     80100d30 <consoleintr+0x2c0>
80100b0c:	81 fe e3 00 00 00    	cmp    $0xe3,%esi
80100b12:	75 34                	jne    80100b48 <consoleintr+0xd8>
      if ((history.count != 0 ) && (history.last - history.index > 0))
80100b14:	a1 9c 25 11 80       	mov    0x8011259c,%eax
80100b19:	85 c0                	test   %eax,%eax
80100b1b:	0f 84 71 ff ff ff    	je     80100a92 <consoleintr+0x22>
80100b21:	a1 a0 25 11 80       	mov    0x801125a0,%eax
80100b26:	2b 05 98 25 11 80    	sub    0x80112598,%eax
80100b2c:	85 c0                	test   %eax,%eax
80100b2e:	0f 8e 5e ff ff ff    	jle    80100a92 <consoleintr+0x22>
        arrow(DOWN);
80100b34:	b8 01 00 00 00       	mov    $0x1,%eax
80100b39:	e8 82 fb ff ff       	call   801006c0 <arrow>
80100b3e:	e9 4f ff ff ff       	jmp    80100a92 <consoleintr+0x22>
80100b43:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80100b47:	90                   	nop
    switch(c){
80100b48:	83 fe 7f             	cmp    $0x7f,%esi
80100b4b:	0f 85 17 01 00 00    	jne    80100c68 <consoleintr+0x1f8>
      if(input.e != input.w){
80100b51:	a1 08 20 11 80       	mov    0x80112008,%eax
80100b56:	3b 05 04 20 11 80    	cmp    0x80112004,%eax
80100b5c:	0f 84 30 ff ff ff    	je     80100a92 <consoleintr+0x22>
        input.e--;  
80100b62:	83 e8 01             	sub    $0x1,%eax
80100b65:	a3 08 20 11 80       	mov    %eax,0x80112008
  if(panicked){
80100b6a:	a1 5c c5 10 80       	mov    0x8010c55c,%eax
80100b6f:	85 c0                	test   %eax,%eax
80100b71:	0f 84 06 02 00 00    	je     80100d7d <consoleintr+0x30d>
80100b77:	fa                   	cli    
    for(;;)
80100b78:	eb fe                	jmp    80100b78 <consoleintr+0x108>
      if (backs > 0) {
80100b7a:	a1 58 c5 10 80       	mov    0x8010c558,%eax
80100b7f:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80100b82:	85 c0                	test   %eax,%eax
80100b84:	0f 8e 08 ff ff ff    	jle    80100a92 <consoleintr+0x22>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80100b8a:	bf d4 03 00 00       	mov    $0x3d4,%edi
80100b8f:	b8 0e 00 00 00       	mov    $0xe,%eax
80100b94:	89 fa                	mov    %edi,%edx
80100b96:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80100b97:	be d5 03 00 00       	mov    $0x3d5,%esi
80100b9c:	89 f2                	mov    %esi,%edx
80100b9e:	ec                   	in     (%dx),%al
80100b9f:	0f b6 c8             	movzbl %al,%ecx
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80100ba2:	89 fa                	mov    %edi,%edx
80100ba4:	b8 0f 00 00 00       	mov    $0xf,%eax
  pos = inb(CRTPORT + 1) << 8;
80100ba9:	c1 e1 08             	shl    $0x8,%ecx
80100bac:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80100bad:	89 f2                	mov    %esi,%edx
80100baf:	ec                   	in     (%dx),%al
  pos |= inb(CRTPORT + 1);
80100bb0:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80100bb3:	89 fa                	mov    %edi,%edx
80100bb5:	09 c1                	or     %eax,%ecx
80100bb7:	b8 0e 00 00 00       	mov    $0xe,%eax
    pos++;
80100bbc:	83 c1 01             	add    $0x1,%ecx
80100bbf:	ee                   	out    %al,(%dx)
  outb(CRTPORT + 1, pos >> 8);
80100bc0:	89 ca                	mov    %ecx,%edx
80100bc2:	c1 fa 08             	sar    $0x8,%edx
80100bc5:	89 d0                	mov    %edx,%eax
80100bc7:	89 f2                	mov    %esi,%edx
80100bc9:	ee                   	out    %al,(%dx)
80100bca:	b8 0f 00 00 00       	mov    $0xf,%eax
80100bcf:	89 fa                	mov    %edi,%edx
80100bd1:	ee                   	out    %al,(%dx)
80100bd2:	89 c8                	mov    %ecx,%eax
80100bd4:	89 f2                	mov    %esi,%edx
80100bd6:	ee                   	out    %al,(%dx)
        backs--;
80100bd7:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100bda:	83 e8 01             	sub    $0x1,%eax
80100bdd:	a3 58 c5 10 80       	mov    %eax,0x8010c558
80100be2:	e9 ab fe ff ff       	jmp    80100a92 <consoleintr+0x22>
      if ((input.e - backs) > input.w)
80100be7:	a1 58 c5 10 80       	mov    0x8010c558,%eax
80100bec:	8b 3d 08 20 11 80    	mov    0x80112008,%edi
80100bf2:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80100bf5:	29 c7                	sub    %eax,%edi
80100bf7:	3b 3d 04 20 11 80    	cmp    0x80112004,%edi
80100bfd:	0f 86 8f fe ff ff    	jbe    80100a92 <consoleintr+0x22>
80100c03:	bf d4 03 00 00       	mov    $0x3d4,%edi
80100c08:	b8 0e 00 00 00       	mov    $0xe,%eax
80100c0d:	89 fa                	mov    %edi,%edx
80100c0f:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80100c10:	be d5 03 00 00       	mov    $0x3d5,%esi
80100c15:	89 f2                	mov    %esi,%edx
80100c17:	ec                   	in     (%dx),%al
80100c18:	0f b6 c8             	movzbl %al,%ecx
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80100c1b:	89 fa                	mov    %edi,%edx
80100c1d:	b8 0f 00 00 00       	mov    $0xf,%eax
  pos = inb(CRTPORT + 1) << 8;
80100c22:	c1 e1 08             	shl    $0x8,%ecx
80100c25:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80100c26:	89 f2                	mov    %esi,%edx
80100c28:	ec                   	in     (%dx),%al
  pos |= inb(CRTPORT + 1);
80100c29:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80100c2c:	89 fa                	mov    %edi,%edx
80100c2e:	09 c1                	or     %eax,%ecx
80100c30:	b8 0e 00 00 00       	mov    $0xe,%eax
    pos--;
80100c35:	83 e9 01             	sub    $0x1,%ecx
80100c38:	ee                   	out    %al,(%dx)
  outb(CRTPORT + 1, pos >> 8);
80100c39:	89 ca                	mov    %ecx,%edx
80100c3b:	c1 fa 08             	sar    $0x8,%edx
80100c3e:	89 d0                	mov    %edx,%eax
80100c40:	89 f2                	mov    %esi,%edx
80100c42:	ee                   	out    %al,(%dx)
80100c43:	b8 0f 00 00 00       	mov    $0xf,%eax
80100c48:	89 fa                	mov    %edi,%edx
80100c4a:	ee                   	out    %al,(%dx)
80100c4b:	89 c8                	mov    %ecx,%eax
80100c4d:	89 f2                	mov    %esi,%edx
80100c4f:	ee                   	out    %al,(%dx)
        backs++;
80100c50:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100c53:	83 c0 01             	add    $0x1,%eax
80100c56:	a3 58 c5 10 80       	mov    %eax,0x8010c558
80100c5b:	e9 32 fe ff ff       	jmp    80100a92 <consoleintr+0x22>
      if(c != 0 && input.e-input.r < INPUT_BUF){
80100c60:	85 f6                	test   %esi,%esi
80100c62:	0f 84 2a fe ff ff    	je     80100a92 <consoleintr+0x22>
80100c68:	a1 08 20 11 80       	mov    0x80112008,%eax
80100c6d:	89 c2                	mov    %eax,%edx
80100c6f:	2b 15 00 20 11 80    	sub    0x80112000,%edx
80100c75:	83 fa 7f             	cmp    $0x7f,%edx
80100c78:	0f 87 14 fe ff ff    	ja     80100a92 <consoleintr+0x22>
        c = (c == '\r') ? '\n' : c;
80100c7e:	8d 48 01             	lea    0x1(%eax),%ecx
80100c81:	8b 15 5c c5 10 80    	mov    0x8010c55c,%edx
80100c87:	83 e0 7f             	and    $0x7f,%eax
        input.buf[input.e++ % INPUT_BUF] = c;
80100c8a:	89 0d 08 20 11 80    	mov    %ecx,0x80112008
        c = (c == '\r') ? '\n' : c;
80100c90:	83 fe 0d             	cmp    $0xd,%esi
80100c93:	0f 84 f3 00 00 00    	je     80100d8c <consoleintr+0x31c>
        input.buf[input.e++ % INPUT_BUF] = c;
80100c99:	89 f1                	mov    %esi,%ecx
80100c9b:	88 88 80 1f 11 80    	mov    %cl,-0x7feee080(%eax)
  if(panicked){
80100ca1:	85 d2                	test   %edx,%edx
80100ca3:	0f 84 d7 01 00 00    	je     80100e80 <consoleintr+0x410>
  asm volatile("cli");
80100ca9:	fa                   	cli    
    for(;;)
80100caa:	eb fe                	jmp    80100caa <consoleintr+0x23a>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80100cac:	be d4 03 00 00       	mov    $0x3d4,%esi
80100cb1:	b8 0e 00 00 00       	mov    $0xe,%eax
80100cb6:	89 f2                	mov    %esi,%edx
80100cb8:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80100cb9:	b9 d5 03 00 00       	mov    $0x3d5,%ecx
80100cbe:	89 ca                	mov    %ecx,%edx
80100cc0:	ec                   	in     (%dx),%al
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80100cc1:	b8 0f 00 00 00       	mov    $0xf,%eax
80100cc6:	89 f2                	mov    %esi,%edx
80100cc8:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80100cc9:	89 ca                	mov    %ecx,%edx
80100ccb:	ec                   	in     (%dx),%al
80100ccc:	be d0 07 00 00       	mov    $0x7d0,%esi
  if(panicked){
80100cd1:	8b 3d 5c c5 10 80    	mov    0x8010c55c,%edi
80100cd7:	85 ff                	test   %edi,%edi
80100cd9:	74 05                	je     80100ce0 <consoleintr+0x270>
  asm volatile("cli");
80100cdb:	fa                   	cli    
    for(;;)
80100cdc:	eb fe                	jmp    80100cdc <consoleintr+0x26c>
80100cde:	66 90                	xchg   %ax,%ax
80100ce0:	b8 00 01 00 00       	mov    $0x100,%eax
80100ce5:	e8 26 f7 ff ff       	call   80100410 <consputc.part.0>
  for (int pos = 0; pos < SCREEN_SIZE ; pos++){
80100cea:	83 ee 01             	sub    $0x1,%esi
80100ced:	75 e2                	jne    80100cd1 <consoleintr+0x261>
  if(panicked){
80100cef:	8b 0d 5c c5 10 80    	mov    0x8010c55c,%ecx
  backs = 0;
80100cf5:	c7 05 58 c5 10 80 00 	movl   $0x0,0x8010c558
80100cfc:	00 00 00 
  input.e = input.w = input.r = 0;
80100cff:	c7 05 00 20 11 80 00 	movl   $0x0,0x80112000
80100d06:	00 00 00 
80100d09:	c7 05 04 20 11 80 00 	movl   $0x0,0x80112004
80100d10:	00 00 00 
80100d13:	c7 05 08 20 11 80 00 	movl   $0x0,0x80112008
80100d1a:	00 00 00 
  if(panicked){
80100d1d:	85 c9                	test   %ecx,%ecx
80100d1f:	0f 84 fb 00 00 00    	je     80100e20 <consoleintr+0x3b0>
80100d25:	fa                   	cli    
    for(;;)
80100d26:	eb fe                	jmp    80100d26 <consoleintr+0x2b6>
80100d28:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100d2f:	90                   	nop
      if ((history.count != 0)  && (history.last - history.index < history.count))
80100d30:	8b 15 9c 25 11 80    	mov    0x8011259c,%edx
80100d36:	85 d2                	test   %edx,%edx
80100d38:	0f 84 54 fd ff ff    	je     80100a92 <consoleintr+0x22>
80100d3e:	a1 a0 25 11 80       	mov    0x801125a0,%eax
80100d43:	2b 05 98 25 11 80    	sub    0x80112598,%eax
80100d49:	39 c2                	cmp    %eax,%edx
80100d4b:	0f 8e 41 fd ff ff    	jle    80100a92 <consoleintr+0x22>
        arrow(UP);
80100d51:	31 c0                	xor    %eax,%eax
80100d53:	e8 68 f9 ff ff       	call   801006c0 <arrow>
80100d58:	e9 35 fd ff ff       	jmp    80100a92 <consoleintr+0x22>
  release(&cons.lock);
80100d5d:	83 ec 0c             	sub    $0xc,%esp
80100d60:	68 20 c5 10 80       	push   $0x8010c520
80100d65:	e8 66 4f 00 00       	call   80105cd0 <release>
  if(doprocdump) {
80100d6a:	83 c4 10             	add    $0x10,%esp
80100d6d:	85 db                	test   %ebx,%ebx
80100d6f:	0f 85 40 01 00 00    	jne    80100eb5 <consoleintr+0x445>
}
80100d75:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100d78:	5b                   	pop    %ebx
80100d79:	5e                   	pop    %esi
80100d7a:	5f                   	pop    %edi
80100d7b:	5d                   	pop    %ebp
80100d7c:	c3                   	ret    
80100d7d:	b8 00 01 00 00       	mov    $0x100,%eax
80100d82:	e8 89 f6 ff ff       	call   80100410 <consputc.part.0>
80100d87:	e9 06 fd ff ff       	jmp    80100a92 <consoleintr+0x22>
        input.buf[input.e++ % INPUT_BUF] = c;
80100d8c:	c6 80 80 1f 11 80 0a 	movb   $0xa,-0x7feee080(%eax)
  if(panicked){
80100d93:	85 d2                	test   %edx,%edx
80100d95:	0f 85 0e ff ff ff    	jne    80100ca9 <consoleintr+0x239>
80100d9b:	b8 0a 00 00 00       	mov    $0xa,%eax
80100da0:	e8 6b f6 ff ff       	call   80100410 <consputc.part.0>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
80100da5:	8b 15 08 20 11 80    	mov    0x80112008,%edx
          if (history.count < 9){
80100dab:	a1 9c 25 11 80       	mov    0x8011259c,%eax
80100db0:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80100db3:	83 f8 08             	cmp    $0x8,%eax
80100db6:	0f 8f 05 01 00 00    	jg     80100ec1 <consoleintr+0x451>
            history.hist[history.last + 1] = input;
80100dbc:	8b 3d a0 25 11 80    	mov    0x801125a0,%edi
80100dc2:	b9 23 00 00 00       	mov    $0x23,%ecx
80100dc7:	be 80 1f 11 80       	mov    $0x80111f80,%esi
80100dcc:	83 c7 01             	add    $0x1,%edi
80100dcf:	69 c7 8c 00 00 00    	imul   $0x8c,%edi,%eax
80100dd5:	89 7d e0             	mov    %edi,-0x20(%ebp)
80100dd8:	05 20 20 11 80       	add    $0x80112020,%eax
80100ddd:	89 c7                	mov    %eax,%edi
            history.count ++ ;
80100ddf:	8b 45 e4             	mov    -0x1c(%ebp),%eax
            history.hist[history.last + 1] = input;
80100de2:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
            history.last ++ ;
80100de4:	8b 7d e0             	mov    -0x20(%ebp),%edi
            history.count ++ ;
80100de7:	83 c0 01             	add    $0x1,%eax
            history.last ++ ;
80100dea:	89 3d a0 25 11 80    	mov    %edi,0x801125a0
            history.index = history.last;
80100df0:	89 3d 98 25 11 80    	mov    %edi,0x80112598
            history.count ++ ;
80100df6:	a3 9c 25 11 80       	mov    %eax,0x8011259c
          wakeup(&input.r);
80100dfb:	83 ec 0c             	sub    $0xc,%esp
          input.w = input.e;
80100dfe:	89 15 04 20 11 80    	mov    %edx,0x80112004
          wakeup(&input.r);
80100e04:	68 00 20 11 80       	push   $0x80112000
          backs = 0;
80100e09:	c7 05 58 c5 10 80 00 	movl   $0x0,0x8010c558
80100e10:	00 00 00 
          wakeup(&input.r);
80100e13:	e8 48 38 00 00       	call   80104660 <wakeup>
80100e18:	83 c4 10             	add    $0x10,%esp
80100e1b:	e9 72 fc ff ff       	jmp    80100a92 <consoleintr+0x22>
80100e20:	b8 24 00 00 00       	mov    $0x24,%eax
80100e25:	e8 e6 f5 ff ff       	call   80100410 <consputc.part.0>
  if(panicked){
80100e2a:	8b 15 5c c5 10 80    	mov    0x8010c55c,%edx
80100e30:	85 d2                	test   %edx,%edx
80100e32:	74 0c                	je     80100e40 <consoleintr+0x3d0>
80100e34:	fa                   	cli    
    for(;;)
80100e35:	eb fe                	jmp    80100e35 <consoleintr+0x3c5>
80100e37:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100e3e:	66 90                	xchg   %ax,%ax
80100e40:	b8 20 00 00 00       	mov    $0x20,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80100e45:	be d4 03 00 00       	mov    $0x3d4,%esi
80100e4a:	e8 c1 f5 ff ff       	call   80100410 <consputc.part.0>
80100e4f:	b8 0e 00 00 00       	mov    $0xe,%eax
80100e54:	89 f2                	mov    %esi,%edx
80100e56:	ee                   	out    %al,(%dx)
80100e57:	b9 d5 03 00 00       	mov    $0x3d5,%ecx
80100e5c:	31 c0                	xor    %eax,%eax
80100e5e:	89 ca                	mov    %ecx,%edx
80100e60:	ee                   	out    %al,(%dx)
80100e61:	b8 0f 00 00 00       	mov    $0xf,%eax
80100e66:	89 f2                	mov    %esi,%edx
80100e68:	ee                   	out    %al,(%dx)
80100e69:	b8 02 00 00 00       	mov    $0x2,%eax
80100e6e:	89 ca                	mov    %ecx,%edx
80100e70:	ee                   	out    %al,(%dx)
}
80100e71:	e9 1c fc ff ff       	jmp    80100a92 <consoleintr+0x22>
80100e76:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100e7d:	8d 76 00             	lea    0x0(%esi),%esi
80100e80:	89 f0                	mov    %esi,%eax
80100e82:	e8 89 f5 ff ff       	call   80100410 <consputc.part.0>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
80100e87:	83 fe 0a             	cmp    $0xa,%esi
80100e8a:	0f 84 15 ff ff ff    	je     80100da5 <consoleintr+0x335>
80100e90:	83 fe 04             	cmp    $0x4,%esi
80100e93:	0f 84 0c ff ff ff    	je     80100da5 <consoleintr+0x335>
80100e99:	a1 00 20 11 80       	mov    0x80112000,%eax
80100e9e:	8d 90 80 00 00 00    	lea    0x80(%eax),%edx
80100ea4:	39 15 08 20 11 80    	cmp    %edx,0x80112008
80100eaa:	0f 85 e2 fb ff ff    	jne    80100a92 <consoleintr+0x22>
80100eb0:	e9 f6 fe ff ff       	jmp    80100dab <consoleintr+0x33b>
}
80100eb5:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100eb8:	5b                   	pop    %ebx
80100eb9:	5e                   	pop    %esi
80100eba:	5f                   	pop    %edi
80100ebb:	5d                   	pop    %ebp
    procdump();  // now call procdump() wo. cons.lock held
80100ebc:	e9 7f 39 00 00       	jmp    80104840 <procdump>
80100ec1:	b8 20 20 11 80       	mov    $0x80112020,%eax
              history.hist[h] = history.hist[h+1]; 
80100ec6:	8d b0 8c 00 00 00    	lea    0x8c(%eax),%esi
80100ecc:	89 c7                	mov    %eax,%edi
80100ece:	b9 23 00 00 00       	mov    $0x23,%ecx
80100ed3:	05 8c 00 00 00       	add    $0x8c,%eax
80100ed8:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
            for (int h = 0; h < 9; h++) {
80100eda:	bf 0c 25 11 80       	mov    $0x8011250c,%edi
80100edf:	39 c7                	cmp    %eax,%edi
80100ee1:	75 e3                	jne    80100ec6 <consoleintr+0x456>
            history.hist[9] = input;
80100ee3:	b9 23 00 00 00       	mov    $0x23,%ecx
80100ee8:	be 80 1f 11 80       	mov    $0x80111f80,%esi
            history.index = 9;
80100eed:	c7 05 98 25 11 80 09 	movl   $0x9,0x80112598
80100ef4:	00 00 00 
            history.hist[9] = input;
80100ef7:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
            history.last = 9;
80100ef9:	c7 05 a0 25 11 80 09 	movl   $0x9,0x801125a0
80100f00:	00 00 00 
            history.count = 10;
80100f03:	c7 05 9c 25 11 80 0a 	movl   $0xa,0x8011259c
80100f0a:	00 00 00 
80100f0d:	e9 e9 fe ff ff       	jmp    80100dfb <consoleintr+0x38b>
80100f12:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100f19:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80100f20 <consoleinit>:

void
consoleinit(void)
{
80100f20:	f3 0f 1e fb          	endbr32 
80100f24:	55                   	push   %ebp
80100f25:	89 e5                	mov    %esp,%ebp
80100f27:	83 ec 10             	sub    $0x10,%esp
  initlock(&cons.lock, "console");
80100f2a:	68 08 8c 10 80       	push   $0x80108c08
80100f2f:	68 20 c5 10 80       	push   $0x8010c520
80100f34:	e8 57 4b 00 00       	call   80105a90 <initlock>

  devsw[CONSOLE].write = consolewrite;
  devsw[CONSOLE].read = consoleread;
  cons.locking = 1;

  ioapicenable(IRQ_KBD, 0);
80100f39:	58                   	pop    %eax
80100f3a:	5a                   	pop    %edx
80100f3b:	6a 00                	push   $0x0
80100f3d:	6a 01                	push   $0x1
  devsw[CONSOLE].write = consolewrite;
80100f3f:	c7 05 6c 2f 11 80 50 	movl   $0x80100850,0x80112f6c
80100f46:	08 10 80 
  devsw[CONSOLE].read = consoleread;
80100f49:	c7 05 68 2f 11 80 90 	movl   $0x80100290,0x80112f68
80100f50:	02 10 80 
  cons.locking = 1;
80100f53:	c7 05 54 c5 10 80 01 	movl   $0x1,0x8010c554
80100f5a:	00 00 00 
  ioapicenable(IRQ_KBD, 0);
80100f5d:	e8 be 19 00 00       	call   80102920 <ioapicenable>
}
80100f62:	83 c4 10             	add    $0x10,%esp
80100f65:	c9                   	leave  
80100f66:	c3                   	ret    
80100f67:	66 90                	xchg   %ax,%ax
80100f69:	66 90                	xchg   %ax,%ax
80100f6b:	66 90                	xchg   %ax,%ax
80100f6d:	66 90                	xchg   %ax,%ax
80100f6f:	90                   	nop

80100f70 <exec>:
#include "x86.h"
#include "elf.h"

int
exec(char *path, char **argv)
{
80100f70:	f3 0f 1e fb          	endbr32 
80100f74:	55                   	push   %ebp
80100f75:	89 e5                	mov    %esp,%ebp
80100f77:	57                   	push   %edi
80100f78:	56                   	push   %esi
80100f79:	53                   	push   %ebx
80100f7a:	81 ec 0c 01 00 00    	sub    $0x10c,%esp
  uint argc, sz, sp, ustack[3+MAXARG+1];
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pde_t *pgdir, *oldpgdir;
  struct proc *curproc = myproc();
80100f80:	e8 3b 2f 00 00       	call   80103ec0 <myproc>
80100f85:	89 85 ec fe ff ff    	mov    %eax,-0x114(%ebp)

  begin_op();
80100f8b:	e8 90 22 00 00       	call   80103220 <begin_op>

  if((ip = namei(path)) == 0){
80100f90:	83 ec 0c             	sub    $0xc,%esp
80100f93:	ff 75 08             	pushl  0x8(%ebp)
80100f96:	e8 85 15 00 00       	call   80102520 <namei>
80100f9b:	83 c4 10             	add    $0x10,%esp
80100f9e:	85 c0                	test   %eax,%eax
80100fa0:	0f 84 fe 02 00 00    	je     801012a4 <exec+0x334>
    end_op();
    cprintf("exec: fail\n");
    return -1;
  }
  ilock(ip);
80100fa6:	83 ec 0c             	sub    $0xc,%esp
80100fa9:	89 c3                	mov    %eax,%ebx
80100fab:	50                   	push   %eax
80100fac:	e8 9f 0c 00 00       	call   80101c50 <ilock>
  pgdir = 0;

  // Check ELF header
  if(readi(ip, (char*)&elf, 0, sizeof(elf)) != sizeof(elf))
80100fb1:	8d 85 24 ff ff ff    	lea    -0xdc(%ebp),%eax
80100fb7:	6a 34                	push   $0x34
80100fb9:	6a 00                	push   $0x0
80100fbb:	50                   	push   %eax
80100fbc:	53                   	push   %ebx
80100fbd:	e8 8e 0f 00 00       	call   80101f50 <readi>
80100fc2:	83 c4 20             	add    $0x20,%esp
80100fc5:	83 f8 34             	cmp    $0x34,%eax
80100fc8:	74 26                	je     80100ff0 <exec+0x80>

 bad:
  if(pgdir)
    freevm(pgdir);
  if(ip){
    iunlockput(ip);
80100fca:	83 ec 0c             	sub    $0xc,%esp
80100fcd:	53                   	push   %ebx
80100fce:	e8 1d 0f 00 00       	call   80101ef0 <iunlockput>
    end_op();
80100fd3:	e8 b8 22 00 00       	call   80103290 <end_op>
80100fd8:	83 c4 10             	add    $0x10,%esp
  }
  return -1;
80100fdb:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80100fe0:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100fe3:	5b                   	pop    %ebx
80100fe4:	5e                   	pop    %esi
80100fe5:	5f                   	pop    %edi
80100fe6:	5d                   	pop    %ebp
80100fe7:	c3                   	ret    
80100fe8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100fef:	90                   	nop
  if(elf.magic != ELF_MAGIC)
80100ff0:	81 bd 24 ff ff ff 7f 	cmpl   $0x464c457f,-0xdc(%ebp)
80100ff7:	45 4c 46 
80100ffa:	75 ce                	jne    80100fca <exec+0x5a>
  if((pgdir = setupkvm()) == 0)
80100ffc:	e8 ff 78 00 00       	call   80108900 <setupkvm>
80101001:	89 85 f4 fe ff ff    	mov    %eax,-0x10c(%ebp)
80101007:	85 c0                	test   %eax,%eax
80101009:	74 bf                	je     80100fca <exec+0x5a>
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
8010100b:	66 83 bd 50 ff ff ff 	cmpw   $0x0,-0xb0(%ebp)
80101012:	00 
80101013:	8b b5 40 ff ff ff    	mov    -0xc0(%ebp),%esi
80101019:	0f 84 a4 02 00 00    	je     801012c3 <exec+0x353>
  sz = 0;
8010101f:	c7 85 f0 fe ff ff 00 	movl   $0x0,-0x110(%ebp)
80101026:	00 00 00 
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80101029:	31 ff                	xor    %edi,%edi
8010102b:	e9 86 00 00 00       	jmp    801010b6 <exec+0x146>
    if(ph.type != ELF_PROG_LOAD)
80101030:	83 bd 04 ff ff ff 01 	cmpl   $0x1,-0xfc(%ebp)
80101037:	75 6c                	jne    801010a5 <exec+0x135>
    if(ph.memsz < ph.filesz)
80101039:	8b 85 18 ff ff ff    	mov    -0xe8(%ebp),%eax
8010103f:	3b 85 14 ff ff ff    	cmp    -0xec(%ebp),%eax
80101045:	0f 82 87 00 00 00    	jb     801010d2 <exec+0x162>
    if(ph.vaddr + ph.memsz < ph.vaddr)
8010104b:	03 85 0c ff ff ff    	add    -0xf4(%ebp),%eax
80101051:	72 7f                	jb     801010d2 <exec+0x162>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
80101053:	83 ec 04             	sub    $0x4,%esp
80101056:	50                   	push   %eax
80101057:	ff b5 f0 fe ff ff    	pushl  -0x110(%ebp)
8010105d:	ff b5 f4 fe ff ff    	pushl  -0x10c(%ebp)
80101063:	e8 b8 76 00 00       	call   80108720 <allocuvm>
80101068:	83 c4 10             	add    $0x10,%esp
8010106b:	89 85 f0 fe ff ff    	mov    %eax,-0x110(%ebp)
80101071:	85 c0                	test   %eax,%eax
80101073:	74 5d                	je     801010d2 <exec+0x162>
    if(ph.vaddr % PGSIZE != 0)
80101075:	8b 85 0c ff ff ff    	mov    -0xf4(%ebp),%eax
8010107b:	a9 ff 0f 00 00       	test   $0xfff,%eax
80101080:	75 50                	jne    801010d2 <exec+0x162>
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
80101082:	83 ec 0c             	sub    $0xc,%esp
80101085:	ff b5 14 ff ff ff    	pushl  -0xec(%ebp)
8010108b:	ff b5 08 ff ff ff    	pushl  -0xf8(%ebp)
80101091:	53                   	push   %ebx
80101092:	50                   	push   %eax
80101093:	ff b5 f4 fe ff ff    	pushl  -0x10c(%ebp)
80101099:	e8 b2 75 00 00       	call   80108650 <loaduvm>
8010109e:	83 c4 20             	add    $0x20,%esp
801010a1:	85 c0                	test   %eax,%eax
801010a3:	78 2d                	js     801010d2 <exec+0x162>
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
801010a5:	0f b7 85 50 ff ff ff 	movzwl -0xb0(%ebp),%eax
801010ac:	83 c7 01             	add    $0x1,%edi
801010af:	83 c6 20             	add    $0x20,%esi
801010b2:	39 f8                	cmp    %edi,%eax
801010b4:	7e 3a                	jle    801010f0 <exec+0x180>
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
801010b6:	8d 85 04 ff ff ff    	lea    -0xfc(%ebp),%eax
801010bc:	6a 20                	push   $0x20
801010be:	56                   	push   %esi
801010bf:	50                   	push   %eax
801010c0:	53                   	push   %ebx
801010c1:	e8 8a 0e 00 00       	call   80101f50 <readi>
801010c6:	83 c4 10             	add    $0x10,%esp
801010c9:	83 f8 20             	cmp    $0x20,%eax
801010cc:	0f 84 5e ff ff ff    	je     80101030 <exec+0xc0>
    freevm(pgdir);
801010d2:	83 ec 0c             	sub    $0xc,%esp
801010d5:	ff b5 f4 fe ff ff    	pushl  -0x10c(%ebp)
801010db:	e8 a0 77 00 00       	call   80108880 <freevm>
  if(ip){
801010e0:	83 c4 10             	add    $0x10,%esp
801010e3:	e9 e2 fe ff ff       	jmp    80100fca <exec+0x5a>
801010e8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801010ef:	90                   	nop
801010f0:	8b bd f0 fe ff ff    	mov    -0x110(%ebp),%edi
801010f6:	81 c7 ff 0f 00 00    	add    $0xfff,%edi
801010fc:	81 e7 00 f0 ff ff    	and    $0xfffff000,%edi
80101102:	8d b7 00 20 00 00    	lea    0x2000(%edi),%esi
  iunlockput(ip);
80101108:	83 ec 0c             	sub    $0xc,%esp
8010110b:	53                   	push   %ebx
8010110c:	e8 df 0d 00 00       	call   80101ef0 <iunlockput>
  end_op();
80101111:	e8 7a 21 00 00       	call   80103290 <end_op>
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
80101116:	83 c4 0c             	add    $0xc,%esp
80101119:	56                   	push   %esi
8010111a:	57                   	push   %edi
8010111b:	8b bd f4 fe ff ff    	mov    -0x10c(%ebp),%edi
80101121:	57                   	push   %edi
80101122:	e8 f9 75 00 00       	call   80108720 <allocuvm>
80101127:	83 c4 10             	add    $0x10,%esp
8010112a:	89 c6                	mov    %eax,%esi
8010112c:	85 c0                	test   %eax,%eax
8010112e:	0f 84 94 00 00 00    	je     801011c8 <exec+0x258>
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80101134:	83 ec 08             	sub    $0x8,%esp
80101137:	8d 80 00 e0 ff ff    	lea    -0x2000(%eax),%eax
  for(argc = 0; argv[argc]; argc++) {
8010113d:	89 f3                	mov    %esi,%ebx
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
8010113f:	50                   	push   %eax
80101140:	57                   	push   %edi
  for(argc = 0; argv[argc]; argc++) {
80101141:	31 ff                	xor    %edi,%edi
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80101143:	e8 58 78 00 00       	call   801089a0 <clearpteu>
  for(argc = 0; argv[argc]; argc++) {
80101148:	8b 45 0c             	mov    0xc(%ebp),%eax
8010114b:	83 c4 10             	add    $0x10,%esp
8010114e:	8d 95 58 ff ff ff    	lea    -0xa8(%ebp),%edx
80101154:	8b 00                	mov    (%eax),%eax
80101156:	85 c0                	test   %eax,%eax
80101158:	0f 84 8b 00 00 00    	je     801011e9 <exec+0x279>
8010115e:	89 b5 f0 fe ff ff    	mov    %esi,-0x110(%ebp)
80101164:	8b b5 f4 fe ff ff    	mov    -0x10c(%ebp),%esi
8010116a:	eb 23                	jmp    8010118f <exec+0x21f>
8010116c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80101170:	8b 45 0c             	mov    0xc(%ebp),%eax
    ustack[3+argc] = sp;
80101173:	89 9c bd 64 ff ff ff 	mov    %ebx,-0x9c(%ebp,%edi,4)
  for(argc = 0; argv[argc]; argc++) {
8010117a:	83 c7 01             	add    $0x1,%edi
    ustack[3+argc] = sp;
8010117d:	8d 95 58 ff ff ff    	lea    -0xa8(%ebp),%edx
  for(argc = 0; argv[argc]; argc++) {
80101183:	8b 04 b8             	mov    (%eax,%edi,4),%eax
80101186:	85 c0                	test   %eax,%eax
80101188:	74 59                	je     801011e3 <exec+0x273>
    if(argc >= MAXARG)
8010118a:	83 ff 20             	cmp    $0x20,%edi
8010118d:	74 39                	je     801011c8 <exec+0x258>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
8010118f:	83 ec 0c             	sub    $0xc,%esp
80101192:	50                   	push   %eax
80101193:	e8 88 4d 00 00       	call   80105f20 <strlen>
80101198:	f7 d0                	not    %eax
8010119a:	01 c3                	add    %eax,%ebx
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
8010119c:	58                   	pop    %eax
8010119d:	8b 45 0c             	mov    0xc(%ebp),%eax
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
801011a0:	83 e3 fc             	and    $0xfffffffc,%ebx
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
801011a3:	ff 34 b8             	pushl  (%eax,%edi,4)
801011a6:	e8 75 4d 00 00       	call   80105f20 <strlen>
801011ab:	83 c0 01             	add    $0x1,%eax
801011ae:	50                   	push   %eax
801011af:	8b 45 0c             	mov    0xc(%ebp),%eax
801011b2:	ff 34 b8             	pushl  (%eax,%edi,4)
801011b5:	53                   	push   %ebx
801011b6:	56                   	push   %esi
801011b7:	e8 44 79 00 00       	call   80108b00 <copyout>
801011bc:	83 c4 20             	add    $0x20,%esp
801011bf:	85 c0                	test   %eax,%eax
801011c1:	79 ad                	jns    80101170 <exec+0x200>
801011c3:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801011c7:	90                   	nop
    freevm(pgdir);
801011c8:	83 ec 0c             	sub    $0xc,%esp
801011cb:	ff b5 f4 fe ff ff    	pushl  -0x10c(%ebp)
801011d1:	e8 aa 76 00 00       	call   80108880 <freevm>
801011d6:	83 c4 10             	add    $0x10,%esp
  return -1;
801011d9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801011de:	e9 fd fd ff ff       	jmp    80100fe0 <exec+0x70>
801011e3:	8b b5 f0 fe ff ff    	mov    -0x110(%ebp),%esi
  ustack[2] = sp - (argc+1)*4;  // argv pointer
801011e9:	8d 04 bd 04 00 00 00 	lea    0x4(,%edi,4),%eax
801011f0:	89 d9                	mov    %ebx,%ecx
  ustack[3+argc] = 0;
801011f2:	c7 84 bd 64 ff ff ff 	movl   $0x0,-0x9c(%ebp,%edi,4)
801011f9:	00 00 00 00 
  ustack[2] = sp - (argc+1)*4;  // argv pointer
801011fd:	29 c1                	sub    %eax,%ecx
  sp -= (3+argc+1) * 4;
801011ff:	83 c0 0c             	add    $0xc,%eax
  ustack[1] = argc;
80101202:	89 bd 5c ff ff ff    	mov    %edi,-0xa4(%ebp)
  sp -= (3+argc+1) * 4;
80101208:	29 c3                	sub    %eax,%ebx
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
8010120a:	50                   	push   %eax
8010120b:	52                   	push   %edx
8010120c:	53                   	push   %ebx
8010120d:	ff b5 f4 fe ff ff    	pushl  -0x10c(%ebp)
  ustack[0] = 0xffffffff;  // fake return PC
80101213:	c7 85 58 ff ff ff ff 	movl   $0xffffffff,-0xa8(%ebp)
8010121a:	ff ff ff 
  ustack[2] = sp - (argc+1)*4;  // argv pointer
8010121d:	89 8d 60 ff ff ff    	mov    %ecx,-0xa0(%ebp)
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
80101223:	e8 d8 78 00 00       	call   80108b00 <copyout>
80101228:	83 c4 10             	add    $0x10,%esp
8010122b:	85 c0                	test   %eax,%eax
8010122d:	78 99                	js     801011c8 <exec+0x258>
  for(last=s=path; *s; s++)
8010122f:	8b 45 08             	mov    0x8(%ebp),%eax
80101232:	8b 55 08             	mov    0x8(%ebp),%edx
80101235:	0f b6 00             	movzbl (%eax),%eax
80101238:	84 c0                	test   %al,%al
8010123a:	74 13                	je     8010124f <exec+0x2df>
8010123c:	89 d1                	mov    %edx,%ecx
8010123e:	66 90                	xchg   %ax,%ax
    if(*s == '/')
80101240:	83 c1 01             	add    $0x1,%ecx
80101243:	3c 2f                	cmp    $0x2f,%al
  for(last=s=path; *s; s++)
80101245:	0f b6 01             	movzbl (%ecx),%eax
    if(*s == '/')
80101248:	0f 44 d1             	cmove  %ecx,%edx
  for(last=s=path; *s; s++)
8010124b:	84 c0                	test   %al,%al
8010124d:	75 f1                	jne    80101240 <exec+0x2d0>
  safestrcpy(curproc->name, last, sizeof(curproc->name));
8010124f:	8b bd ec fe ff ff    	mov    -0x114(%ebp),%edi
80101255:	83 ec 04             	sub    $0x4,%esp
80101258:	6a 10                	push   $0x10
8010125a:	89 f8                	mov    %edi,%eax
8010125c:	52                   	push   %edx
8010125d:	83 c0 6c             	add    $0x6c,%eax
80101260:	50                   	push   %eax
80101261:	e8 7a 4c 00 00       	call   80105ee0 <safestrcpy>
  curproc->pgdir = pgdir;
80101266:	8b 8d f4 fe ff ff    	mov    -0x10c(%ebp),%ecx
  oldpgdir = curproc->pgdir;
8010126c:	89 f8                	mov    %edi,%eax
8010126e:	8b 7f 04             	mov    0x4(%edi),%edi
  curproc->sz = sz;
80101271:	89 30                	mov    %esi,(%eax)
  curproc->pgdir = pgdir;
80101273:	89 48 04             	mov    %ecx,0x4(%eax)
  curproc->tf->eip = elf.entry;  // main
80101276:	89 c1                	mov    %eax,%ecx
80101278:	8b 95 3c ff ff ff    	mov    -0xc4(%ebp),%edx
8010127e:	8b 40 18             	mov    0x18(%eax),%eax
80101281:	89 50 38             	mov    %edx,0x38(%eax)
  curproc->tf->esp = sp;
80101284:	8b 41 18             	mov    0x18(%ecx),%eax
80101287:	89 58 44             	mov    %ebx,0x44(%eax)
  switchuvm(curproc);
8010128a:	89 0c 24             	mov    %ecx,(%esp)
8010128d:	e8 2e 72 00 00       	call   801084c0 <switchuvm>
  freevm(oldpgdir);
80101292:	89 3c 24             	mov    %edi,(%esp)
80101295:	e8 e6 75 00 00       	call   80108880 <freevm>
  return 0;
8010129a:	83 c4 10             	add    $0x10,%esp
8010129d:	31 c0                	xor    %eax,%eax
8010129f:	e9 3c fd ff ff       	jmp    80100fe0 <exec+0x70>
    end_op();
801012a4:	e8 e7 1f 00 00       	call   80103290 <end_op>
    cprintf("exec: fail\n");
801012a9:	83 ec 0c             	sub    $0xc,%esp
801012ac:	68 79 8c 10 80       	push   $0x80108c79
801012b1:	e8 0a f6 ff ff       	call   801008c0 <cprintf>
    return -1;
801012b6:	83 c4 10             	add    $0x10,%esp
801012b9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801012be:	e9 1d fd ff ff       	jmp    80100fe0 <exec+0x70>
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
801012c3:	31 ff                	xor    %edi,%edi
801012c5:	be 00 20 00 00       	mov    $0x2000,%esi
801012ca:	e9 39 fe ff ff       	jmp    80101108 <exec+0x198>
801012cf:	90                   	nop

801012d0 <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
801012d0:	f3 0f 1e fb          	endbr32 
801012d4:	55                   	push   %ebp
801012d5:	89 e5                	mov    %esp,%ebp
801012d7:	83 ec 10             	sub    $0x10,%esp
  initlock(&ftable.lock, "ftable");
801012da:	68 85 8c 10 80       	push   $0x80108c85
801012df:	68 c0 25 11 80       	push   $0x801125c0
801012e4:	e8 a7 47 00 00       	call   80105a90 <initlock>
}
801012e9:	83 c4 10             	add    $0x10,%esp
801012ec:	c9                   	leave  
801012ed:	c3                   	ret    
801012ee:	66 90                	xchg   %ax,%ax

801012f0 <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
801012f0:	f3 0f 1e fb          	endbr32 
801012f4:	55                   	push   %ebp
801012f5:	89 e5                	mov    %esp,%ebp
801012f7:	53                   	push   %ebx
  struct file *f;

  acquire(&ftable.lock);
  for(f = ftable.file; f < ftable.file + NFILE; f++){
801012f8:	bb f4 25 11 80       	mov    $0x801125f4,%ebx
{
801012fd:	83 ec 10             	sub    $0x10,%esp
  acquire(&ftable.lock);
80101300:	68 c0 25 11 80       	push   $0x801125c0
80101305:	e8 06 49 00 00       	call   80105c10 <acquire>
8010130a:	83 c4 10             	add    $0x10,%esp
8010130d:	eb 0c                	jmp    8010131b <filealloc+0x2b>
8010130f:	90                   	nop
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80101310:	83 c3 18             	add    $0x18,%ebx
80101313:	81 fb 54 2f 11 80    	cmp    $0x80112f54,%ebx
80101319:	74 25                	je     80101340 <filealloc+0x50>
    if(f->ref == 0){
8010131b:	8b 43 04             	mov    0x4(%ebx),%eax
8010131e:	85 c0                	test   %eax,%eax
80101320:	75 ee                	jne    80101310 <filealloc+0x20>
      f->ref = 1;
      release(&ftable.lock);
80101322:	83 ec 0c             	sub    $0xc,%esp
      f->ref = 1;
80101325:	c7 43 04 01 00 00 00 	movl   $0x1,0x4(%ebx)
      release(&ftable.lock);
8010132c:	68 c0 25 11 80       	push   $0x801125c0
80101331:	e8 9a 49 00 00       	call   80105cd0 <release>
      return f;
    }
  }
  release(&ftable.lock);
  return 0;
}
80101336:	89 d8                	mov    %ebx,%eax
      return f;
80101338:	83 c4 10             	add    $0x10,%esp
}
8010133b:	8b 5d fc             	mov    -0x4(%ebp),%ebx
8010133e:	c9                   	leave  
8010133f:	c3                   	ret    
  release(&ftable.lock);
80101340:	83 ec 0c             	sub    $0xc,%esp
  return 0;
80101343:	31 db                	xor    %ebx,%ebx
  release(&ftable.lock);
80101345:	68 c0 25 11 80       	push   $0x801125c0
8010134a:	e8 81 49 00 00       	call   80105cd0 <release>
}
8010134f:	89 d8                	mov    %ebx,%eax
  return 0;
80101351:	83 c4 10             	add    $0x10,%esp
}
80101354:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80101357:	c9                   	leave  
80101358:	c3                   	ret    
80101359:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80101360 <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
80101360:	f3 0f 1e fb          	endbr32 
80101364:	55                   	push   %ebp
80101365:	89 e5                	mov    %esp,%ebp
80101367:	53                   	push   %ebx
80101368:	83 ec 10             	sub    $0x10,%esp
8010136b:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&ftable.lock);
8010136e:	68 c0 25 11 80       	push   $0x801125c0
80101373:	e8 98 48 00 00       	call   80105c10 <acquire>
  if(f->ref < 1)
80101378:	8b 43 04             	mov    0x4(%ebx),%eax
8010137b:	83 c4 10             	add    $0x10,%esp
8010137e:	85 c0                	test   %eax,%eax
80101380:	7e 1a                	jle    8010139c <filedup+0x3c>
    panic("filedup");
  f->ref++;
80101382:	83 c0 01             	add    $0x1,%eax
  release(&ftable.lock);
80101385:	83 ec 0c             	sub    $0xc,%esp
  f->ref++;
80101388:	89 43 04             	mov    %eax,0x4(%ebx)
  release(&ftable.lock);
8010138b:	68 c0 25 11 80       	push   $0x801125c0
80101390:	e8 3b 49 00 00       	call   80105cd0 <release>
  return f;
}
80101395:	89 d8                	mov    %ebx,%eax
80101397:	8b 5d fc             	mov    -0x4(%ebp),%ebx
8010139a:	c9                   	leave  
8010139b:	c3                   	ret    
    panic("filedup");
8010139c:	83 ec 0c             	sub    $0xc,%esp
8010139f:	68 8c 8c 10 80       	push   $0x80108c8c
801013a4:	e8 e7 ef ff ff       	call   80100390 <panic>
801013a9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

801013b0 <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
801013b0:	f3 0f 1e fb          	endbr32 
801013b4:	55                   	push   %ebp
801013b5:	89 e5                	mov    %esp,%ebp
801013b7:	57                   	push   %edi
801013b8:	56                   	push   %esi
801013b9:	53                   	push   %ebx
801013ba:	83 ec 28             	sub    $0x28,%esp
801013bd:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct file ff;

  acquire(&ftable.lock);
801013c0:	68 c0 25 11 80       	push   $0x801125c0
801013c5:	e8 46 48 00 00       	call   80105c10 <acquire>
  if(f->ref < 1)
801013ca:	8b 53 04             	mov    0x4(%ebx),%edx
801013cd:	83 c4 10             	add    $0x10,%esp
801013d0:	85 d2                	test   %edx,%edx
801013d2:	0f 8e a1 00 00 00    	jle    80101479 <fileclose+0xc9>
    panic("fileclose");
  if(--f->ref > 0){
801013d8:	83 ea 01             	sub    $0x1,%edx
801013db:	89 53 04             	mov    %edx,0x4(%ebx)
801013de:	75 40                	jne    80101420 <fileclose+0x70>
    release(&ftable.lock);
    return;
  }
  ff = *f;
801013e0:	0f b6 43 09          	movzbl 0x9(%ebx),%eax
  f->ref = 0;
  f->type = FD_NONE;
  release(&ftable.lock);
801013e4:	83 ec 0c             	sub    $0xc,%esp
  ff = *f;
801013e7:	8b 3b                	mov    (%ebx),%edi
  f->type = FD_NONE;
801013e9:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
  ff = *f;
801013ef:	8b 73 0c             	mov    0xc(%ebx),%esi
801013f2:	88 45 e7             	mov    %al,-0x19(%ebp)
801013f5:	8b 43 10             	mov    0x10(%ebx),%eax
  release(&ftable.lock);
801013f8:	68 c0 25 11 80       	push   $0x801125c0
  ff = *f;
801013fd:	89 45 e0             	mov    %eax,-0x20(%ebp)
  release(&ftable.lock);
80101400:	e8 cb 48 00 00       	call   80105cd0 <release>

  if(ff.type == FD_PIPE)
80101405:	83 c4 10             	add    $0x10,%esp
80101408:	83 ff 01             	cmp    $0x1,%edi
8010140b:	74 53                	je     80101460 <fileclose+0xb0>
    pipeclose(ff.pipe, ff.writable);
  else if(ff.type == FD_INODE){
8010140d:	83 ff 02             	cmp    $0x2,%edi
80101410:	74 26                	je     80101438 <fileclose+0x88>
    begin_op();
    iput(ff.ip);
    end_op();
  }
}
80101412:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101415:	5b                   	pop    %ebx
80101416:	5e                   	pop    %esi
80101417:	5f                   	pop    %edi
80101418:	5d                   	pop    %ebp
80101419:	c3                   	ret    
8010141a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    release(&ftable.lock);
80101420:	c7 45 08 c0 25 11 80 	movl   $0x801125c0,0x8(%ebp)
}
80101427:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010142a:	5b                   	pop    %ebx
8010142b:	5e                   	pop    %esi
8010142c:	5f                   	pop    %edi
8010142d:	5d                   	pop    %ebp
    release(&ftable.lock);
8010142e:	e9 9d 48 00 00       	jmp    80105cd0 <release>
80101433:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80101437:	90                   	nop
    begin_op();
80101438:	e8 e3 1d 00 00       	call   80103220 <begin_op>
    iput(ff.ip);
8010143d:	83 ec 0c             	sub    $0xc,%esp
80101440:	ff 75 e0             	pushl  -0x20(%ebp)
80101443:	e8 38 09 00 00       	call   80101d80 <iput>
    end_op();
80101448:	83 c4 10             	add    $0x10,%esp
}
8010144b:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010144e:	5b                   	pop    %ebx
8010144f:	5e                   	pop    %esi
80101450:	5f                   	pop    %edi
80101451:	5d                   	pop    %ebp
    end_op();
80101452:	e9 39 1e 00 00       	jmp    80103290 <end_op>
80101457:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010145e:	66 90                	xchg   %ax,%ax
    pipeclose(ff.pipe, ff.writable);
80101460:	0f be 5d e7          	movsbl -0x19(%ebp),%ebx
80101464:	83 ec 08             	sub    $0x8,%esp
80101467:	53                   	push   %ebx
80101468:	56                   	push   %esi
80101469:	e8 92 25 00 00       	call   80103a00 <pipeclose>
8010146e:	83 c4 10             	add    $0x10,%esp
}
80101471:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101474:	5b                   	pop    %ebx
80101475:	5e                   	pop    %esi
80101476:	5f                   	pop    %edi
80101477:	5d                   	pop    %ebp
80101478:	c3                   	ret    
    panic("fileclose");
80101479:	83 ec 0c             	sub    $0xc,%esp
8010147c:	68 94 8c 10 80       	push   $0x80108c94
80101481:	e8 0a ef ff ff       	call   80100390 <panic>
80101486:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010148d:	8d 76 00             	lea    0x0(%esi),%esi

80101490 <filestat>:

// Get metadata about file f.
int
filestat(struct file *f, struct stat *st)
{
80101490:	f3 0f 1e fb          	endbr32 
80101494:	55                   	push   %ebp
80101495:	89 e5                	mov    %esp,%ebp
80101497:	53                   	push   %ebx
80101498:	83 ec 04             	sub    $0x4,%esp
8010149b:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(f->type == FD_INODE){
8010149e:	83 3b 02             	cmpl   $0x2,(%ebx)
801014a1:	75 2d                	jne    801014d0 <filestat+0x40>
    ilock(f->ip);
801014a3:	83 ec 0c             	sub    $0xc,%esp
801014a6:	ff 73 10             	pushl  0x10(%ebx)
801014a9:	e8 a2 07 00 00       	call   80101c50 <ilock>
    stati(f->ip, st);
801014ae:	58                   	pop    %eax
801014af:	5a                   	pop    %edx
801014b0:	ff 75 0c             	pushl  0xc(%ebp)
801014b3:	ff 73 10             	pushl  0x10(%ebx)
801014b6:	e8 65 0a 00 00       	call   80101f20 <stati>
    iunlock(f->ip);
801014bb:	59                   	pop    %ecx
801014bc:	ff 73 10             	pushl  0x10(%ebx)
801014bf:	e8 6c 08 00 00       	call   80101d30 <iunlock>
    return 0;
  }
  return -1;
}
801014c4:	8b 5d fc             	mov    -0x4(%ebp),%ebx
    return 0;
801014c7:	83 c4 10             	add    $0x10,%esp
801014ca:	31 c0                	xor    %eax,%eax
}
801014cc:	c9                   	leave  
801014cd:	c3                   	ret    
801014ce:	66 90                	xchg   %ax,%ax
801014d0:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  return -1;
801014d3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801014d8:	c9                   	leave  
801014d9:	c3                   	ret    
801014da:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801014e0 <fileread>:

// Read from file f.
int
fileread(struct file *f, char *addr, int n)
{
801014e0:	f3 0f 1e fb          	endbr32 
801014e4:	55                   	push   %ebp
801014e5:	89 e5                	mov    %esp,%ebp
801014e7:	57                   	push   %edi
801014e8:	56                   	push   %esi
801014e9:	53                   	push   %ebx
801014ea:	83 ec 0c             	sub    $0xc,%esp
801014ed:	8b 5d 08             	mov    0x8(%ebp),%ebx
801014f0:	8b 75 0c             	mov    0xc(%ebp),%esi
801014f3:	8b 7d 10             	mov    0x10(%ebp),%edi
  int r;

  if(f->readable == 0)
801014f6:	80 7b 08 00          	cmpb   $0x0,0x8(%ebx)
801014fa:	74 64                	je     80101560 <fileread+0x80>
    return -1;
  if(f->type == FD_PIPE)
801014fc:	8b 03                	mov    (%ebx),%eax
801014fe:	83 f8 01             	cmp    $0x1,%eax
80101501:	74 45                	je     80101548 <fileread+0x68>
    return piperead(f->pipe, addr, n);
  if(f->type == FD_INODE){
80101503:	83 f8 02             	cmp    $0x2,%eax
80101506:	75 5f                	jne    80101567 <fileread+0x87>
    ilock(f->ip);
80101508:	83 ec 0c             	sub    $0xc,%esp
8010150b:	ff 73 10             	pushl  0x10(%ebx)
8010150e:	e8 3d 07 00 00       	call   80101c50 <ilock>
    if((r = readi(f->ip, addr, f->off, n)) > 0)
80101513:	57                   	push   %edi
80101514:	ff 73 14             	pushl  0x14(%ebx)
80101517:	56                   	push   %esi
80101518:	ff 73 10             	pushl  0x10(%ebx)
8010151b:	e8 30 0a 00 00       	call   80101f50 <readi>
80101520:	83 c4 20             	add    $0x20,%esp
80101523:	89 c6                	mov    %eax,%esi
80101525:	85 c0                	test   %eax,%eax
80101527:	7e 03                	jle    8010152c <fileread+0x4c>
      f->off += r;
80101529:	01 43 14             	add    %eax,0x14(%ebx)
    iunlock(f->ip);
8010152c:	83 ec 0c             	sub    $0xc,%esp
8010152f:	ff 73 10             	pushl  0x10(%ebx)
80101532:	e8 f9 07 00 00       	call   80101d30 <iunlock>
    return r;
80101537:	83 c4 10             	add    $0x10,%esp
  }
  panic("fileread");
}
8010153a:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010153d:	89 f0                	mov    %esi,%eax
8010153f:	5b                   	pop    %ebx
80101540:	5e                   	pop    %esi
80101541:	5f                   	pop    %edi
80101542:	5d                   	pop    %ebp
80101543:	c3                   	ret    
80101544:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    return piperead(f->pipe, addr, n);
80101548:	8b 43 0c             	mov    0xc(%ebx),%eax
8010154b:	89 45 08             	mov    %eax,0x8(%ebp)
}
8010154e:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101551:	5b                   	pop    %ebx
80101552:	5e                   	pop    %esi
80101553:	5f                   	pop    %edi
80101554:	5d                   	pop    %ebp
    return piperead(f->pipe, addr, n);
80101555:	e9 46 26 00 00       	jmp    80103ba0 <piperead>
8010155a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    return -1;
80101560:	be ff ff ff ff       	mov    $0xffffffff,%esi
80101565:	eb d3                	jmp    8010153a <fileread+0x5a>
  panic("fileread");
80101567:	83 ec 0c             	sub    $0xc,%esp
8010156a:	68 9e 8c 10 80       	push   $0x80108c9e
8010156f:	e8 1c ee ff ff       	call   80100390 <panic>
80101574:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010157b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010157f:	90                   	nop

80101580 <filewrite>:

//PAGEBREAK!
// Write to file f.
int
filewrite(struct file *f, char *addr, int n)
{
80101580:	f3 0f 1e fb          	endbr32 
80101584:	55                   	push   %ebp
80101585:	89 e5                	mov    %esp,%ebp
80101587:	57                   	push   %edi
80101588:	56                   	push   %esi
80101589:	53                   	push   %ebx
8010158a:	83 ec 1c             	sub    $0x1c,%esp
8010158d:	8b 45 0c             	mov    0xc(%ebp),%eax
80101590:	8b 75 08             	mov    0x8(%ebp),%esi
80101593:	89 45 dc             	mov    %eax,-0x24(%ebp)
80101596:	8b 45 10             	mov    0x10(%ebp),%eax
  int r;

  if(f->writable == 0)
80101599:	80 7e 09 00          	cmpb   $0x0,0x9(%esi)
{
8010159d:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  if(f->writable == 0)
801015a0:	0f 84 c1 00 00 00    	je     80101667 <filewrite+0xe7>
    return -1;
  if(f->type == FD_PIPE)
801015a6:	8b 06                	mov    (%esi),%eax
801015a8:	83 f8 01             	cmp    $0x1,%eax
801015ab:	0f 84 c3 00 00 00    	je     80101674 <filewrite+0xf4>
    return pipewrite(f->pipe, addr, n);
  if(f->type == FD_INODE){
801015b1:	83 f8 02             	cmp    $0x2,%eax
801015b4:	0f 85 cc 00 00 00    	jne    80101686 <filewrite+0x106>
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((MAXOPBLOCKS-1-1-2) / 2) * 512;
    int i = 0;
    while(i < n){
801015ba:	8b 45 e4             	mov    -0x1c(%ebp),%eax
    int i = 0;
801015bd:	31 ff                	xor    %edi,%edi
    while(i < n){
801015bf:	85 c0                	test   %eax,%eax
801015c1:	7f 34                	jg     801015f7 <filewrite+0x77>
801015c3:	e9 98 00 00 00       	jmp    80101660 <filewrite+0xe0>
801015c8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801015cf:	90                   	nop
        n1 = max;

      begin_op();
      ilock(f->ip);
      if ((r = writei(f->ip, addr + i, f->off, n1)) > 0)
        f->off += r;
801015d0:	01 46 14             	add    %eax,0x14(%esi)
      iunlock(f->ip);
801015d3:	83 ec 0c             	sub    $0xc,%esp
801015d6:	ff 76 10             	pushl  0x10(%esi)
        f->off += r;
801015d9:	89 45 e0             	mov    %eax,-0x20(%ebp)
      iunlock(f->ip);
801015dc:	e8 4f 07 00 00       	call   80101d30 <iunlock>
      end_op();
801015e1:	e8 aa 1c 00 00       	call   80103290 <end_op>

      if(r < 0)
        break;
      if(r != n1)
801015e6:	8b 45 e0             	mov    -0x20(%ebp),%eax
801015e9:	83 c4 10             	add    $0x10,%esp
801015ec:	39 c3                	cmp    %eax,%ebx
801015ee:	75 60                	jne    80101650 <filewrite+0xd0>
        panic("short filewrite");
      i += r;
801015f0:	01 df                	add    %ebx,%edi
    while(i < n){
801015f2:	39 7d e4             	cmp    %edi,-0x1c(%ebp)
801015f5:	7e 69                	jle    80101660 <filewrite+0xe0>
      int n1 = n - i;
801015f7:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
801015fa:	b8 00 06 00 00       	mov    $0x600,%eax
801015ff:	29 fb                	sub    %edi,%ebx
      if(n1 > max)
80101601:	81 fb 00 06 00 00    	cmp    $0x600,%ebx
80101607:	0f 4f d8             	cmovg  %eax,%ebx
      begin_op();
8010160a:	e8 11 1c 00 00       	call   80103220 <begin_op>
      ilock(f->ip);
8010160f:	83 ec 0c             	sub    $0xc,%esp
80101612:	ff 76 10             	pushl  0x10(%esi)
80101615:	e8 36 06 00 00       	call   80101c50 <ilock>
      if ((r = writei(f->ip, addr + i, f->off, n1)) > 0)
8010161a:	8b 45 dc             	mov    -0x24(%ebp),%eax
8010161d:	53                   	push   %ebx
8010161e:	ff 76 14             	pushl  0x14(%esi)
80101621:	01 f8                	add    %edi,%eax
80101623:	50                   	push   %eax
80101624:	ff 76 10             	pushl  0x10(%esi)
80101627:	e8 24 0a 00 00       	call   80102050 <writei>
8010162c:	83 c4 20             	add    $0x20,%esp
8010162f:	85 c0                	test   %eax,%eax
80101631:	7f 9d                	jg     801015d0 <filewrite+0x50>
      iunlock(f->ip);
80101633:	83 ec 0c             	sub    $0xc,%esp
80101636:	ff 76 10             	pushl  0x10(%esi)
80101639:	89 45 e4             	mov    %eax,-0x1c(%ebp)
8010163c:	e8 ef 06 00 00       	call   80101d30 <iunlock>
      end_op();
80101641:	e8 4a 1c 00 00       	call   80103290 <end_op>
      if(r < 0)
80101646:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80101649:	83 c4 10             	add    $0x10,%esp
8010164c:	85 c0                	test   %eax,%eax
8010164e:	75 17                	jne    80101667 <filewrite+0xe7>
        panic("short filewrite");
80101650:	83 ec 0c             	sub    $0xc,%esp
80101653:	68 a7 8c 10 80       	push   $0x80108ca7
80101658:	e8 33 ed ff ff       	call   80100390 <panic>
8010165d:	8d 76 00             	lea    0x0(%esi),%esi
    }
    return i == n ? n : -1;
80101660:	89 f8                	mov    %edi,%eax
80101662:	3b 7d e4             	cmp    -0x1c(%ebp),%edi
80101665:	74 05                	je     8010166c <filewrite+0xec>
80101667:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
  }
  panic("filewrite");
}
8010166c:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010166f:	5b                   	pop    %ebx
80101670:	5e                   	pop    %esi
80101671:	5f                   	pop    %edi
80101672:	5d                   	pop    %ebp
80101673:	c3                   	ret    
    return pipewrite(f->pipe, addr, n);
80101674:	8b 46 0c             	mov    0xc(%esi),%eax
80101677:	89 45 08             	mov    %eax,0x8(%ebp)
}
8010167a:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010167d:	5b                   	pop    %ebx
8010167e:	5e                   	pop    %esi
8010167f:	5f                   	pop    %edi
80101680:	5d                   	pop    %ebp
    return pipewrite(f->pipe, addr, n);
80101681:	e9 1a 24 00 00       	jmp    80103aa0 <pipewrite>
  panic("filewrite");
80101686:	83 ec 0c             	sub    $0xc,%esp
80101689:	68 ad 8c 10 80       	push   $0x80108cad
8010168e:	e8 fd ec ff ff       	call   80100390 <panic>
80101693:	66 90                	xchg   %ax,%ax
80101695:	66 90                	xchg   %ax,%ax
80101697:	66 90                	xchg   %ax,%ax
80101699:	66 90                	xchg   %ax,%ax
8010169b:	66 90                	xchg   %ax,%ax
8010169d:	66 90                	xchg   %ax,%ax
8010169f:	90                   	nop

801016a0 <bfree>:
}

// Free a disk block.
static void
bfree(int dev, uint b)
{
801016a0:	55                   	push   %ebp
801016a1:	89 c1                	mov    %eax,%ecx
  struct buf *bp;
  int bi, m;

  bp = bread(dev, BBLOCK(b, sb));
801016a3:	89 d0                	mov    %edx,%eax
801016a5:	c1 e8 0c             	shr    $0xc,%eax
801016a8:	03 05 d8 2f 11 80    	add    0x80112fd8,%eax
{
801016ae:	89 e5                	mov    %esp,%ebp
801016b0:	56                   	push   %esi
801016b1:	53                   	push   %ebx
801016b2:	89 d3                	mov    %edx,%ebx
  bp = bread(dev, BBLOCK(b, sb));
801016b4:	83 ec 08             	sub    $0x8,%esp
801016b7:	50                   	push   %eax
801016b8:	51                   	push   %ecx
801016b9:	e8 12 ea ff ff       	call   801000d0 <bread>
  bi = b % BPB;
  m = 1 << (bi % 8);
801016be:	89 d9                	mov    %ebx,%ecx
  if((bp->data[bi/8] & m) == 0)
801016c0:	c1 fb 03             	sar    $0x3,%ebx
  m = 1 << (bi % 8);
801016c3:	ba 01 00 00 00       	mov    $0x1,%edx
801016c8:	83 e1 07             	and    $0x7,%ecx
  if((bp->data[bi/8] & m) == 0)
801016cb:	81 e3 ff 01 00 00    	and    $0x1ff,%ebx
801016d1:	83 c4 10             	add    $0x10,%esp
  m = 1 << (bi % 8);
801016d4:	d3 e2                	shl    %cl,%edx
  if((bp->data[bi/8] & m) == 0)
801016d6:	0f b6 4c 18 5c       	movzbl 0x5c(%eax,%ebx,1),%ecx
801016db:	85 d1                	test   %edx,%ecx
801016dd:	74 25                	je     80101704 <bfree+0x64>
    panic("freeing free block");
  bp->data[bi/8] &= ~m;
801016df:	f7 d2                	not    %edx
  log_write(bp);
801016e1:	83 ec 0c             	sub    $0xc,%esp
801016e4:	89 c6                	mov    %eax,%esi
  bp->data[bi/8] &= ~m;
801016e6:	21 ca                	and    %ecx,%edx
801016e8:	88 54 18 5c          	mov    %dl,0x5c(%eax,%ebx,1)
  log_write(bp);
801016ec:	50                   	push   %eax
801016ed:	e8 0e 1d 00 00       	call   80103400 <log_write>
  brelse(bp);
801016f2:	89 34 24             	mov    %esi,(%esp)
801016f5:	e8 f6 ea ff ff       	call   801001f0 <brelse>
}
801016fa:	83 c4 10             	add    $0x10,%esp
801016fd:	8d 65 f8             	lea    -0x8(%ebp),%esp
80101700:	5b                   	pop    %ebx
80101701:	5e                   	pop    %esi
80101702:	5d                   	pop    %ebp
80101703:	c3                   	ret    
    panic("freeing free block");
80101704:	83 ec 0c             	sub    $0xc,%esp
80101707:	68 b7 8c 10 80       	push   $0x80108cb7
8010170c:	e8 7f ec ff ff       	call   80100390 <panic>
80101711:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80101718:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010171f:	90                   	nop

80101720 <balloc>:
{
80101720:	55                   	push   %ebp
80101721:	89 e5                	mov    %esp,%ebp
80101723:	57                   	push   %edi
80101724:	56                   	push   %esi
80101725:	53                   	push   %ebx
80101726:	83 ec 1c             	sub    $0x1c,%esp
  for(b = 0; b < sb.size; b += BPB){
80101729:	8b 0d c0 2f 11 80    	mov    0x80112fc0,%ecx
{
8010172f:	89 45 d8             	mov    %eax,-0x28(%ebp)
  for(b = 0; b < sb.size; b += BPB){
80101732:	85 c9                	test   %ecx,%ecx
80101734:	0f 84 87 00 00 00    	je     801017c1 <balloc+0xa1>
8010173a:	c7 45 dc 00 00 00 00 	movl   $0x0,-0x24(%ebp)
    bp = bread(dev, BBLOCK(b, sb));
80101741:	8b 75 dc             	mov    -0x24(%ebp),%esi
80101744:	83 ec 08             	sub    $0x8,%esp
80101747:	89 f0                	mov    %esi,%eax
80101749:	c1 f8 0c             	sar    $0xc,%eax
8010174c:	03 05 d8 2f 11 80    	add    0x80112fd8,%eax
80101752:	50                   	push   %eax
80101753:	ff 75 d8             	pushl  -0x28(%ebp)
80101756:	e8 75 e9 ff ff       	call   801000d0 <bread>
8010175b:	83 c4 10             	add    $0x10,%esp
8010175e:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
80101761:	a1 c0 2f 11 80       	mov    0x80112fc0,%eax
80101766:	89 45 e0             	mov    %eax,-0x20(%ebp)
80101769:	31 c0                	xor    %eax,%eax
8010176b:	eb 2f                	jmp    8010179c <balloc+0x7c>
8010176d:	8d 76 00             	lea    0x0(%esi),%esi
      m = 1 << (bi % 8);
80101770:	89 c1                	mov    %eax,%ecx
80101772:	bb 01 00 00 00       	mov    $0x1,%ebx
      if((bp->data[bi/8] & m) == 0){  // Is block free?
80101777:	8b 55 e4             	mov    -0x1c(%ebp),%edx
      m = 1 << (bi % 8);
8010177a:	83 e1 07             	and    $0x7,%ecx
8010177d:	d3 e3                	shl    %cl,%ebx
      if((bp->data[bi/8] & m) == 0){  // Is block free?
8010177f:	89 c1                	mov    %eax,%ecx
80101781:	c1 f9 03             	sar    $0x3,%ecx
80101784:	0f b6 7c 0a 5c       	movzbl 0x5c(%edx,%ecx,1),%edi
80101789:	89 fa                	mov    %edi,%edx
8010178b:	85 df                	test   %ebx,%edi
8010178d:	74 41                	je     801017d0 <balloc+0xb0>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
8010178f:	83 c0 01             	add    $0x1,%eax
80101792:	83 c6 01             	add    $0x1,%esi
80101795:	3d 00 10 00 00       	cmp    $0x1000,%eax
8010179a:	74 05                	je     801017a1 <balloc+0x81>
8010179c:	39 75 e0             	cmp    %esi,-0x20(%ebp)
8010179f:	77 cf                	ja     80101770 <balloc+0x50>
    brelse(bp);
801017a1:	83 ec 0c             	sub    $0xc,%esp
801017a4:	ff 75 e4             	pushl  -0x1c(%ebp)
801017a7:	e8 44 ea ff ff       	call   801001f0 <brelse>
  for(b = 0; b < sb.size; b += BPB){
801017ac:	81 45 dc 00 10 00 00 	addl   $0x1000,-0x24(%ebp)
801017b3:	83 c4 10             	add    $0x10,%esp
801017b6:	8b 45 dc             	mov    -0x24(%ebp),%eax
801017b9:	39 05 c0 2f 11 80    	cmp    %eax,0x80112fc0
801017bf:	77 80                	ja     80101741 <balloc+0x21>
  panic("balloc: out of blocks");
801017c1:	83 ec 0c             	sub    $0xc,%esp
801017c4:	68 ca 8c 10 80       	push   $0x80108cca
801017c9:	e8 c2 eb ff ff       	call   80100390 <panic>
801017ce:	66 90                	xchg   %ax,%ax
        bp->data[bi/8] |= m;  // Mark block in use.
801017d0:	8b 7d e4             	mov    -0x1c(%ebp),%edi
        log_write(bp);
801017d3:	83 ec 0c             	sub    $0xc,%esp
        bp->data[bi/8] |= m;  // Mark block in use.
801017d6:	09 da                	or     %ebx,%edx
801017d8:	88 54 0f 5c          	mov    %dl,0x5c(%edi,%ecx,1)
        log_write(bp);
801017dc:	57                   	push   %edi
801017dd:	e8 1e 1c 00 00       	call   80103400 <log_write>
        brelse(bp);
801017e2:	89 3c 24             	mov    %edi,(%esp)
801017e5:	e8 06 ea ff ff       	call   801001f0 <brelse>
  bp = bread(dev, bno);
801017ea:	58                   	pop    %eax
801017eb:	5a                   	pop    %edx
801017ec:	56                   	push   %esi
801017ed:	ff 75 d8             	pushl  -0x28(%ebp)
801017f0:	e8 db e8 ff ff       	call   801000d0 <bread>
  memset(bp->data, 0, BSIZE);
801017f5:	83 c4 0c             	add    $0xc,%esp
  bp = bread(dev, bno);
801017f8:	89 c3                	mov    %eax,%ebx
  memset(bp->data, 0, BSIZE);
801017fa:	8d 40 5c             	lea    0x5c(%eax),%eax
801017fd:	68 00 02 00 00       	push   $0x200
80101802:	6a 00                	push   $0x0
80101804:	50                   	push   %eax
80101805:	e8 16 45 00 00       	call   80105d20 <memset>
  log_write(bp);
8010180a:	89 1c 24             	mov    %ebx,(%esp)
8010180d:	e8 ee 1b 00 00       	call   80103400 <log_write>
  brelse(bp);
80101812:	89 1c 24             	mov    %ebx,(%esp)
80101815:	e8 d6 e9 ff ff       	call   801001f0 <brelse>
}
8010181a:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010181d:	89 f0                	mov    %esi,%eax
8010181f:	5b                   	pop    %ebx
80101820:	5e                   	pop    %esi
80101821:	5f                   	pop    %edi
80101822:	5d                   	pop    %ebp
80101823:	c3                   	ret    
80101824:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010182b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010182f:	90                   	nop

80101830 <iget>:
// Find the inode with number inum on device dev
// and return the in-memory copy. Does not lock
// the inode and does not read it from disk.
static struct inode*
iget(uint dev, uint inum)
{
80101830:	55                   	push   %ebp
80101831:	89 e5                	mov    %esp,%ebp
80101833:	57                   	push   %edi
80101834:	89 c7                	mov    %eax,%edi
80101836:	56                   	push   %esi
  struct inode *ip, *empty;

  acquire(&icache.lock);

  // Is the inode already cached?
  empty = 0;
80101837:	31 f6                	xor    %esi,%esi
{
80101839:	53                   	push   %ebx
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
8010183a:	bb 14 30 11 80       	mov    $0x80113014,%ebx
{
8010183f:	83 ec 28             	sub    $0x28,%esp
80101842:	89 55 e4             	mov    %edx,-0x1c(%ebp)
  acquire(&icache.lock);
80101845:	68 e0 2f 11 80       	push   $0x80112fe0
8010184a:	e8 c1 43 00 00       	call   80105c10 <acquire>
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
8010184f:	8b 55 e4             	mov    -0x1c(%ebp),%edx
  acquire(&icache.lock);
80101852:	83 c4 10             	add    $0x10,%esp
80101855:	eb 1b                	jmp    80101872 <iget+0x42>
80101857:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010185e:	66 90                	xchg   %ax,%ax
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
80101860:	39 3b                	cmp    %edi,(%ebx)
80101862:	74 6c                	je     801018d0 <iget+0xa0>
80101864:	81 c3 90 00 00 00    	add    $0x90,%ebx
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
8010186a:	81 fb 34 4c 11 80    	cmp    $0x80114c34,%ebx
80101870:	73 26                	jae    80101898 <iget+0x68>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
80101872:	8b 4b 08             	mov    0x8(%ebx),%ecx
80101875:	85 c9                	test   %ecx,%ecx
80101877:	7f e7                	jg     80101860 <iget+0x30>
      ip->ref++;
      release(&icache.lock);
      return ip;
    }
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
80101879:	85 f6                	test   %esi,%esi
8010187b:	75 e7                	jne    80101864 <iget+0x34>
8010187d:	89 d8                	mov    %ebx,%eax
8010187f:	81 c3 90 00 00 00    	add    $0x90,%ebx
80101885:	85 c9                	test   %ecx,%ecx
80101887:	75 6e                	jne    801018f7 <iget+0xc7>
80101889:	89 c6                	mov    %eax,%esi
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
8010188b:	81 fb 34 4c 11 80    	cmp    $0x80114c34,%ebx
80101891:	72 df                	jb     80101872 <iget+0x42>
80101893:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80101897:	90                   	nop
      empty = ip;
  }

  // Recycle an inode cache entry.
  if(empty == 0)
80101898:	85 f6                	test   %esi,%esi
8010189a:	74 73                	je     8010190f <iget+0xdf>
  ip = empty;
  ip->dev = dev;
  ip->inum = inum;
  ip->ref = 1;
  ip->valid = 0;
  release(&icache.lock);
8010189c:	83 ec 0c             	sub    $0xc,%esp
  ip->dev = dev;
8010189f:	89 3e                	mov    %edi,(%esi)
  ip->inum = inum;
801018a1:	89 56 04             	mov    %edx,0x4(%esi)
  ip->ref = 1;
801018a4:	c7 46 08 01 00 00 00 	movl   $0x1,0x8(%esi)
  ip->valid = 0;
801018ab:	c7 46 4c 00 00 00 00 	movl   $0x0,0x4c(%esi)
  release(&icache.lock);
801018b2:	68 e0 2f 11 80       	push   $0x80112fe0
801018b7:	e8 14 44 00 00       	call   80105cd0 <release>

  return ip;
801018bc:	83 c4 10             	add    $0x10,%esp
}
801018bf:	8d 65 f4             	lea    -0xc(%ebp),%esp
801018c2:	89 f0                	mov    %esi,%eax
801018c4:	5b                   	pop    %ebx
801018c5:	5e                   	pop    %esi
801018c6:	5f                   	pop    %edi
801018c7:	5d                   	pop    %ebp
801018c8:	c3                   	ret    
801018c9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
801018d0:	39 53 04             	cmp    %edx,0x4(%ebx)
801018d3:	75 8f                	jne    80101864 <iget+0x34>
      release(&icache.lock);
801018d5:	83 ec 0c             	sub    $0xc,%esp
      ip->ref++;
801018d8:	83 c1 01             	add    $0x1,%ecx
      return ip;
801018db:	89 de                	mov    %ebx,%esi
      release(&icache.lock);
801018dd:	68 e0 2f 11 80       	push   $0x80112fe0
      ip->ref++;
801018e2:	89 4b 08             	mov    %ecx,0x8(%ebx)
      release(&icache.lock);
801018e5:	e8 e6 43 00 00       	call   80105cd0 <release>
      return ip;
801018ea:	83 c4 10             	add    $0x10,%esp
}
801018ed:	8d 65 f4             	lea    -0xc(%ebp),%esp
801018f0:	89 f0                	mov    %esi,%eax
801018f2:	5b                   	pop    %ebx
801018f3:	5e                   	pop    %esi
801018f4:	5f                   	pop    %edi
801018f5:	5d                   	pop    %ebp
801018f6:	c3                   	ret    
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
801018f7:	81 fb 34 4c 11 80    	cmp    $0x80114c34,%ebx
801018fd:	73 10                	jae    8010190f <iget+0xdf>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
801018ff:	8b 4b 08             	mov    0x8(%ebx),%ecx
80101902:	85 c9                	test   %ecx,%ecx
80101904:	0f 8f 56 ff ff ff    	jg     80101860 <iget+0x30>
8010190a:	e9 6e ff ff ff       	jmp    8010187d <iget+0x4d>
    panic("iget: no inodes");
8010190f:	83 ec 0c             	sub    $0xc,%esp
80101912:	68 e0 8c 10 80       	push   $0x80108ce0
80101917:	e8 74 ea ff ff       	call   80100390 <panic>
8010191c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80101920 <bmap>:

// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
static uint
bmap(struct inode *ip, uint bn)
{
80101920:	55                   	push   %ebp
80101921:	89 e5                	mov    %esp,%ebp
80101923:	57                   	push   %edi
80101924:	56                   	push   %esi
80101925:	89 c6                	mov    %eax,%esi
80101927:	53                   	push   %ebx
80101928:	83 ec 1c             	sub    $0x1c,%esp
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
8010192b:	83 fa 0b             	cmp    $0xb,%edx
8010192e:	0f 86 84 00 00 00    	jbe    801019b8 <bmap+0x98>
    if((addr = ip->addrs[bn]) == 0)
      ip->addrs[bn] = addr = balloc(ip->dev);
    return addr;
  }
  bn -= NDIRECT;
80101934:	8d 5a f4             	lea    -0xc(%edx),%ebx

  if(bn < NINDIRECT){
80101937:	83 fb 7f             	cmp    $0x7f,%ebx
8010193a:	0f 87 98 00 00 00    	ja     801019d8 <bmap+0xb8>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0)
80101940:	8b 80 8c 00 00 00    	mov    0x8c(%eax),%eax
80101946:	8b 16                	mov    (%esi),%edx
80101948:	85 c0                	test   %eax,%eax
8010194a:	74 54                	je     801019a0 <bmap+0x80>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
    bp = bread(ip->dev, addr);
8010194c:	83 ec 08             	sub    $0x8,%esp
8010194f:	50                   	push   %eax
80101950:	52                   	push   %edx
80101951:	e8 7a e7 ff ff       	call   801000d0 <bread>
    a = (uint*)bp->data;
    if((addr = a[bn]) == 0){
80101956:	83 c4 10             	add    $0x10,%esp
80101959:	8d 54 98 5c          	lea    0x5c(%eax,%ebx,4),%edx
    bp = bread(ip->dev, addr);
8010195d:	89 c7                	mov    %eax,%edi
    if((addr = a[bn]) == 0){
8010195f:	8b 1a                	mov    (%edx),%ebx
80101961:	85 db                	test   %ebx,%ebx
80101963:	74 1b                	je     80101980 <bmap+0x60>
      a[bn] = addr = balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);
80101965:	83 ec 0c             	sub    $0xc,%esp
80101968:	57                   	push   %edi
80101969:	e8 82 e8 ff ff       	call   801001f0 <brelse>
    return addr;
8010196e:	83 c4 10             	add    $0x10,%esp
  }

  panic("bmap: out of range");
}
80101971:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101974:	89 d8                	mov    %ebx,%eax
80101976:	5b                   	pop    %ebx
80101977:	5e                   	pop    %esi
80101978:	5f                   	pop    %edi
80101979:	5d                   	pop    %ebp
8010197a:	c3                   	ret    
8010197b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010197f:	90                   	nop
      a[bn] = addr = balloc(ip->dev);
80101980:	8b 06                	mov    (%esi),%eax
80101982:	89 55 e4             	mov    %edx,-0x1c(%ebp)
80101985:	e8 96 fd ff ff       	call   80101720 <balloc>
8010198a:	8b 55 e4             	mov    -0x1c(%ebp),%edx
      log_write(bp);
8010198d:	83 ec 0c             	sub    $0xc,%esp
      a[bn] = addr = balloc(ip->dev);
80101990:	89 c3                	mov    %eax,%ebx
80101992:	89 02                	mov    %eax,(%edx)
      log_write(bp);
80101994:	57                   	push   %edi
80101995:	e8 66 1a 00 00       	call   80103400 <log_write>
8010199a:	83 c4 10             	add    $0x10,%esp
8010199d:	eb c6                	jmp    80101965 <bmap+0x45>
8010199f:	90                   	nop
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
801019a0:	89 d0                	mov    %edx,%eax
801019a2:	e8 79 fd ff ff       	call   80101720 <balloc>
801019a7:	8b 16                	mov    (%esi),%edx
801019a9:	89 86 8c 00 00 00    	mov    %eax,0x8c(%esi)
801019af:	eb 9b                	jmp    8010194c <bmap+0x2c>
801019b1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    if((addr = ip->addrs[bn]) == 0)
801019b8:	8d 3c 90             	lea    (%eax,%edx,4),%edi
801019bb:	8b 5f 5c             	mov    0x5c(%edi),%ebx
801019be:	85 db                	test   %ebx,%ebx
801019c0:	75 af                	jne    80101971 <bmap+0x51>
      ip->addrs[bn] = addr = balloc(ip->dev);
801019c2:	8b 00                	mov    (%eax),%eax
801019c4:	e8 57 fd ff ff       	call   80101720 <balloc>
801019c9:	89 47 5c             	mov    %eax,0x5c(%edi)
801019cc:	89 c3                	mov    %eax,%ebx
}
801019ce:	8d 65 f4             	lea    -0xc(%ebp),%esp
801019d1:	89 d8                	mov    %ebx,%eax
801019d3:	5b                   	pop    %ebx
801019d4:	5e                   	pop    %esi
801019d5:	5f                   	pop    %edi
801019d6:	5d                   	pop    %ebp
801019d7:	c3                   	ret    
  panic("bmap: out of range");
801019d8:	83 ec 0c             	sub    $0xc,%esp
801019db:	68 f0 8c 10 80       	push   $0x80108cf0
801019e0:	e8 ab e9 ff ff       	call   80100390 <panic>
801019e5:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801019ec:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

801019f0 <readsb>:
{
801019f0:	f3 0f 1e fb          	endbr32 
801019f4:	55                   	push   %ebp
801019f5:	89 e5                	mov    %esp,%ebp
801019f7:	56                   	push   %esi
801019f8:	53                   	push   %ebx
801019f9:	8b 75 0c             	mov    0xc(%ebp),%esi
  bp = bread(dev, 1);
801019fc:	83 ec 08             	sub    $0x8,%esp
801019ff:	6a 01                	push   $0x1
80101a01:	ff 75 08             	pushl  0x8(%ebp)
80101a04:	e8 c7 e6 ff ff       	call   801000d0 <bread>
  memmove(sb, bp->data, sizeof(*sb));
80101a09:	83 c4 0c             	add    $0xc,%esp
  bp = bread(dev, 1);
80101a0c:	89 c3                	mov    %eax,%ebx
  memmove(sb, bp->data, sizeof(*sb));
80101a0e:	8d 40 5c             	lea    0x5c(%eax),%eax
80101a11:	6a 1c                	push   $0x1c
80101a13:	50                   	push   %eax
80101a14:	56                   	push   %esi
80101a15:	e8 a6 43 00 00       	call   80105dc0 <memmove>
  brelse(bp);
80101a1a:	89 5d 08             	mov    %ebx,0x8(%ebp)
80101a1d:	83 c4 10             	add    $0x10,%esp
}
80101a20:	8d 65 f8             	lea    -0x8(%ebp),%esp
80101a23:	5b                   	pop    %ebx
80101a24:	5e                   	pop    %esi
80101a25:	5d                   	pop    %ebp
  brelse(bp);
80101a26:	e9 c5 e7 ff ff       	jmp    801001f0 <brelse>
80101a2b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80101a2f:	90                   	nop

80101a30 <iinit>:
{
80101a30:	f3 0f 1e fb          	endbr32 
80101a34:	55                   	push   %ebp
80101a35:	89 e5                	mov    %esp,%ebp
80101a37:	53                   	push   %ebx
80101a38:	bb 20 30 11 80       	mov    $0x80113020,%ebx
80101a3d:	83 ec 0c             	sub    $0xc,%esp
  initlock(&icache.lock, "icache");
80101a40:	68 03 8d 10 80       	push   $0x80108d03
80101a45:	68 e0 2f 11 80       	push   $0x80112fe0
80101a4a:	e8 41 40 00 00       	call   80105a90 <initlock>
  for(i = 0; i < NINODE; i++) {
80101a4f:	83 c4 10             	add    $0x10,%esp
80101a52:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    initsleeplock(&icache.inode[i].lock, "inode");
80101a58:	83 ec 08             	sub    $0x8,%esp
80101a5b:	68 0a 8d 10 80       	push   $0x80108d0a
80101a60:	53                   	push   %ebx
80101a61:	81 c3 90 00 00 00    	add    $0x90,%ebx
80101a67:	e8 c4 3d 00 00       	call   80105830 <initsleeplock>
  for(i = 0; i < NINODE; i++) {
80101a6c:	83 c4 10             	add    $0x10,%esp
80101a6f:	81 fb 40 4c 11 80    	cmp    $0x80114c40,%ebx
80101a75:	75 e1                	jne    80101a58 <iinit+0x28>
  readsb(dev, &sb);
80101a77:	83 ec 08             	sub    $0x8,%esp
80101a7a:	68 c0 2f 11 80       	push   $0x80112fc0
80101a7f:	ff 75 08             	pushl  0x8(%ebp)
80101a82:	e8 69 ff ff ff       	call   801019f0 <readsb>
  cprintf("sb: size %d nblocks %d ninodes %d nlog %d logstart %d\
80101a87:	ff 35 d8 2f 11 80    	pushl  0x80112fd8
80101a8d:	ff 35 d4 2f 11 80    	pushl  0x80112fd4
80101a93:	ff 35 d0 2f 11 80    	pushl  0x80112fd0
80101a99:	ff 35 cc 2f 11 80    	pushl  0x80112fcc
80101a9f:	ff 35 c8 2f 11 80    	pushl  0x80112fc8
80101aa5:	ff 35 c4 2f 11 80    	pushl  0x80112fc4
80101aab:	ff 35 c0 2f 11 80    	pushl  0x80112fc0
80101ab1:	68 70 8d 10 80       	push   $0x80108d70
80101ab6:	e8 05 ee ff ff       	call   801008c0 <cprintf>
}
80101abb:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80101abe:	83 c4 30             	add    $0x30,%esp
80101ac1:	c9                   	leave  
80101ac2:	c3                   	ret    
80101ac3:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80101aca:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80101ad0 <ialloc>:
{
80101ad0:	f3 0f 1e fb          	endbr32 
80101ad4:	55                   	push   %ebp
80101ad5:	89 e5                	mov    %esp,%ebp
80101ad7:	57                   	push   %edi
80101ad8:	56                   	push   %esi
80101ad9:	53                   	push   %ebx
80101ada:	83 ec 1c             	sub    $0x1c,%esp
80101add:	8b 45 0c             	mov    0xc(%ebp),%eax
  for(inum = 1; inum < sb.ninodes; inum++){
80101ae0:	83 3d c8 2f 11 80 01 	cmpl   $0x1,0x80112fc8
{
80101ae7:	8b 75 08             	mov    0x8(%ebp),%esi
80101aea:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  for(inum = 1; inum < sb.ninodes; inum++){
80101aed:	0f 86 8d 00 00 00    	jbe    80101b80 <ialloc+0xb0>
80101af3:	bf 01 00 00 00       	mov    $0x1,%edi
80101af8:	eb 1d                	jmp    80101b17 <ialloc+0x47>
80101afa:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    brelse(bp);
80101b00:	83 ec 0c             	sub    $0xc,%esp
  for(inum = 1; inum < sb.ninodes; inum++){
80101b03:	83 c7 01             	add    $0x1,%edi
    brelse(bp);
80101b06:	53                   	push   %ebx
80101b07:	e8 e4 e6 ff ff       	call   801001f0 <brelse>
  for(inum = 1; inum < sb.ninodes; inum++){
80101b0c:	83 c4 10             	add    $0x10,%esp
80101b0f:	3b 3d c8 2f 11 80    	cmp    0x80112fc8,%edi
80101b15:	73 69                	jae    80101b80 <ialloc+0xb0>
    bp = bread(dev, IBLOCK(inum, sb));
80101b17:	89 f8                	mov    %edi,%eax
80101b19:	83 ec 08             	sub    $0x8,%esp
80101b1c:	c1 e8 03             	shr    $0x3,%eax
80101b1f:	03 05 d4 2f 11 80    	add    0x80112fd4,%eax
80101b25:	50                   	push   %eax
80101b26:	56                   	push   %esi
80101b27:	e8 a4 e5 ff ff       	call   801000d0 <bread>
    if(dip->type == 0){  // a free inode
80101b2c:	83 c4 10             	add    $0x10,%esp
    bp = bread(dev, IBLOCK(inum, sb));
80101b2f:	89 c3                	mov    %eax,%ebx
    dip = (struct dinode*)bp->data + inum%IPB;
80101b31:	89 f8                	mov    %edi,%eax
80101b33:	83 e0 07             	and    $0x7,%eax
80101b36:	c1 e0 06             	shl    $0x6,%eax
80101b39:	8d 4c 03 5c          	lea    0x5c(%ebx,%eax,1),%ecx
    if(dip->type == 0){  // a free inode
80101b3d:	66 83 39 00          	cmpw   $0x0,(%ecx)
80101b41:	75 bd                	jne    80101b00 <ialloc+0x30>
      memset(dip, 0, sizeof(*dip));
80101b43:	83 ec 04             	sub    $0x4,%esp
80101b46:	89 4d e0             	mov    %ecx,-0x20(%ebp)
80101b49:	6a 40                	push   $0x40
80101b4b:	6a 00                	push   $0x0
80101b4d:	51                   	push   %ecx
80101b4e:	e8 cd 41 00 00       	call   80105d20 <memset>
      dip->type = type;
80101b53:	0f b7 45 e4          	movzwl -0x1c(%ebp),%eax
80101b57:	8b 4d e0             	mov    -0x20(%ebp),%ecx
80101b5a:	66 89 01             	mov    %ax,(%ecx)
      log_write(bp);   // mark it allocated on the disk
80101b5d:	89 1c 24             	mov    %ebx,(%esp)
80101b60:	e8 9b 18 00 00       	call   80103400 <log_write>
      brelse(bp);
80101b65:	89 1c 24             	mov    %ebx,(%esp)
80101b68:	e8 83 e6 ff ff       	call   801001f0 <brelse>
      return iget(dev, inum);
80101b6d:	83 c4 10             	add    $0x10,%esp
}
80101b70:	8d 65 f4             	lea    -0xc(%ebp),%esp
      return iget(dev, inum);
80101b73:	89 fa                	mov    %edi,%edx
}
80101b75:	5b                   	pop    %ebx
      return iget(dev, inum);
80101b76:	89 f0                	mov    %esi,%eax
}
80101b78:	5e                   	pop    %esi
80101b79:	5f                   	pop    %edi
80101b7a:	5d                   	pop    %ebp
      return iget(dev, inum);
80101b7b:	e9 b0 fc ff ff       	jmp    80101830 <iget>
  panic("ialloc: no inodes");
80101b80:	83 ec 0c             	sub    $0xc,%esp
80101b83:	68 10 8d 10 80       	push   $0x80108d10
80101b88:	e8 03 e8 ff ff       	call   80100390 <panic>
80101b8d:	8d 76 00             	lea    0x0(%esi),%esi

80101b90 <iupdate>:
{
80101b90:	f3 0f 1e fb          	endbr32 
80101b94:	55                   	push   %ebp
80101b95:	89 e5                	mov    %esp,%ebp
80101b97:	56                   	push   %esi
80101b98:	53                   	push   %ebx
80101b99:	8b 5d 08             	mov    0x8(%ebp),%ebx
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
80101b9c:	8b 43 04             	mov    0x4(%ebx),%eax
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
80101b9f:	83 c3 5c             	add    $0x5c,%ebx
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
80101ba2:	83 ec 08             	sub    $0x8,%esp
80101ba5:	c1 e8 03             	shr    $0x3,%eax
80101ba8:	03 05 d4 2f 11 80    	add    0x80112fd4,%eax
80101bae:	50                   	push   %eax
80101baf:	ff 73 a4             	pushl  -0x5c(%ebx)
80101bb2:	e8 19 e5 ff ff       	call   801000d0 <bread>
  dip->type = ip->type;
80101bb7:	0f b7 53 f4          	movzwl -0xc(%ebx),%edx
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
80101bbb:	83 c4 0c             	add    $0xc,%esp
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
80101bbe:	89 c6                	mov    %eax,%esi
  dip = (struct dinode*)bp->data + ip->inum%IPB;
80101bc0:	8b 43 a8             	mov    -0x58(%ebx),%eax
80101bc3:	83 e0 07             	and    $0x7,%eax
80101bc6:	c1 e0 06             	shl    $0x6,%eax
80101bc9:	8d 44 06 5c          	lea    0x5c(%esi,%eax,1),%eax
  dip->type = ip->type;
80101bcd:	66 89 10             	mov    %dx,(%eax)
  dip->major = ip->major;
80101bd0:	0f b7 53 f6          	movzwl -0xa(%ebx),%edx
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
80101bd4:	83 c0 0c             	add    $0xc,%eax
  dip->major = ip->major;
80101bd7:	66 89 50 f6          	mov    %dx,-0xa(%eax)
  dip->minor = ip->minor;
80101bdb:	0f b7 53 f8          	movzwl -0x8(%ebx),%edx
80101bdf:	66 89 50 f8          	mov    %dx,-0x8(%eax)
  dip->nlink = ip->nlink;
80101be3:	0f b7 53 fa          	movzwl -0x6(%ebx),%edx
80101be7:	66 89 50 fa          	mov    %dx,-0x6(%eax)
  dip->size = ip->size;
80101beb:	8b 53 fc             	mov    -0x4(%ebx),%edx
80101bee:	89 50 fc             	mov    %edx,-0x4(%eax)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
80101bf1:	6a 34                	push   $0x34
80101bf3:	53                   	push   %ebx
80101bf4:	50                   	push   %eax
80101bf5:	e8 c6 41 00 00       	call   80105dc0 <memmove>
  log_write(bp);
80101bfa:	89 34 24             	mov    %esi,(%esp)
80101bfd:	e8 fe 17 00 00       	call   80103400 <log_write>
  brelse(bp);
80101c02:	89 75 08             	mov    %esi,0x8(%ebp)
80101c05:	83 c4 10             	add    $0x10,%esp
}
80101c08:	8d 65 f8             	lea    -0x8(%ebp),%esp
80101c0b:	5b                   	pop    %ebx
80101c0c:	5e                   	pop    %esi
80101c0d:	5d                   	pop    %ebp
  brelse(bp);
80101c0e:	e9 dd e5 ff ff       	jmp    801001f0 <brelse>
80101c13:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80101c1a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80101c20 <idup>:
{
80101c20:	f3 0f 1e fb          	endbr32 
80101c24:	55                   	push   %ebp
80101c25:	89 e5                	mov    %esp,%ebp
80101c27:	53                   	push   %ebx
80101c28:	83 ec 10             	sub    $0x10,%esp
80101c2b:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&icache.lock);
80101c2e:	68 e0 2f 11 80       	push   $0x80112fe0
80101c33:	e8 d8 3f 00 00       	call   80105c10 <acquire>
  ip->ref++;
80101c38:	83 43 08 01          	addl   $0x1,0x8(%ebx)
  release(&icache.lock);
80101c3c:	c7 04 24 e0 2f 11 80 	movl   $0x80112fe0,(%esp)
80101c43:	e8 88 40 00 00       	call   80105cd0 <release>
}
80101c48:	89 d8                	mov    %ebx,%eax
80101c4a:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80101c4d:	c9                   	leave  
80101c4e:	c3                   	ret    
80101c4f:	90                   	nop

80101c50 <ilock>:
{
80101c50:	f3 0f 1e fb          	endbr32 
80101c54:	55                   	push   %ebp
80101c55:	89 e5                	mov    %esp,%ebp
80101c57:	56                   	push   %esi
80101c58:	53                   	push   %ebx
80101c59:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(ip == 0 || ip->ref < 1)
80101c5c:	85 db                	test   %ebx,%ebx
80101c5e:	0f 84 b3 00 00 00    	je     80101d17 <ilock+0xc7>
80101c64:	8b 53 08             	mov    0x8(%ebx),%edx
80101c67:	85 d2                	test   %edx,%edx
80101c69:	0f 8e a8 00 00 00    	jle    80101d17 <ilock+0xc7>
  acquiresleep(&ip->lock);
80101c6f:	83 ec 0c             	sub    $0xc,%esp
80101c72:	8d 43 0c             	lea    0xc(%ebx),%eax
80101c75:	50                   	push   %eax
80101c76:	e8 f5 3b 00 00       	call   80105870 <acquiresleep>
  if(ip->valid == 0){
80101c7b:	8b 43 4c             	mov    0x4c(%ebx),%eax
80101c7e:	83 c4 10             	add    $0x10,%esp
80101c81:	85 c0                	test   %eax,%eax
80101c83:	74 0b                	je     80101c90 <ilock+0x40>
}
80101c85:	8d 65 f8             	lea    -0x8(%ebp),%esp
80101c88:	5b                   	pop    %ebx
80101c89:	5e                   	pop    %esi
80101c8a:	5d                   	pop    %ebp
80101c8b:	c3                   	ret    
80101c8c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    bp = bread(ip->dev, IBLOCK(ip->inum, sb));
80101c90:	8b 43 04             	mov    0x4(%ebx),%eax
80101c93:	83 ec 08             	sub    $0x8,%esp
80101c96:	c1 e8 03             	shr    $0x3,%eax
80101c99:	03 05 d4 2f 11 80    	add    0x80112fd4,%eax
80101c9f:	50                   	push   %eax
80101ca0:	ff 33                	pushl  (%ebx)
80101ca2:	e8 29 e4 ff ff       	call   801000d0 <bread>
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
80101ca7:	83 c4 0c             	add    $0xc,%esp
    bp = bread(ip->dev, IBLOCK(ip->inum, sb));
80101caa:	89 c6                	mov    %eax,%esi
    dip = (struct dinode*)bp->data + ip->inum%IPB;
80101cac:	8b 43 04             	mov    0x4(%ebx),%eax
80101caf:	83 e0 07             	and    $0x7,%eax
80101cb2:	c1 e0 06             	shl    $0x6,%eax
80101cb5:	8d 44 06 5c          	lea    0x5c(%esi,%eax,1),%eax
    ip->type = dip->type;
80101cb9:	0f b7 10             	movzwl (%eax),%edx
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
80101cbc:	83 c0 0c             	add    $0xc,%eax
    ip->type = dip->type;
80101cbf:	66 89 53 50          	mov    %dx,0x50(%ebx)
    ip->major = dip->major;
80101cc3:	0f b7 50 f6          	movzwl -0xa(%eax),%edx
80101cc7:	66 89 53 52          	mov    %dx,0x52(%ebx)
    ip->minor = dip->minor;
80101ccb:	0f b7 50 f8          	movzwl -0x8(%eax),%edx
80101ccf:	66 89 53 54          	mov    %dx,0x54(%ebx)
    ip->nlink = dip->nlink;
80101cd3:	0f b7 50 fa          	movzwl -0x6(%eax),%edx
80101cd7:	66 89 53 56          	mov    %dx,0x56(%ebx)
    ip->size = dip->size;
80101cdb:	8b 50 fc             	mov    -0x4(%eax),%edx
80101cde:	89 53 58             	mov    %edx,0x58(%ebx)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
80101ce1:	6a 34                	push   $0x34
80101ce3:	50                   	push   %eax
80101ce4:	8d 43 5c             	lea    0x5c(%ebx),%eax
80101ce7:	50                   	push   %eax
80101ce8:	e8 d3 40 00 00       	call   80105dc0 <memmove>
    brelse(bp);
80101ced:	89 34 24             	mov    %esi,(%esp)
80101cf0:	e8 fb e4 ff ff       	call   801001f0 <brelse>
    if(ip->type == 0)
80101cf5:	83 c4 10             	add    $0x10,%esp
80101cf8:	66 83 7b 50 00       	cmpw   $0x0,0x50(%ebx)
    ip->valid = 1;
80101cfd:	c7 43 4c 01 00 00 00 	movl   $0x1,0x4c(%ebx)
    if(ip->type == 0)
80101d04:	0f 85 7b ff ff ff    	jne    80101c85 <ilock+0x35>
      panic("ilock: no type");
80101d0a:	83 ec 0c             	sub    $0xc,%esp
80101d0d:	68 28 8d 10 80       	push   $0x80108d28
80101d12:	e8 79 e6 ff ff       	call   80100390 <panic>
    panic("ilock");
80101d17:	83 ec 0c             	sub    $0xc,%esp
80101d1a:	68 22 8d 10 80       	push   $0x80108d22
80101d1f:	e8 6c e6 ff ff       	call   80100390 <panic>
80101d24:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80101d2b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80101d2f:	90                   	nop

80101d30 <iunlock>:
{
80101d30:	f3 0f 1e fb          	endbr32 
80101d34:	55                   	push   %ebp
80101d35:	89 e5                	mov    %esp,%ebp
80101d37:	56                   	push   %esi
80101d38:	53                   	push   %ebx
80101d39:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
80101d3c:	85 db                	test   %ebx,%ebx
80101d3e:	74 28                	je     80101d68 <iunlock+0x38>
80101d40:	83 ec 0c             	sub    $0xc,%esp
80101d43:	8d 73 0c             	lea    0xc(%ebx),%esi
80101d46:	56                   	push   %esi
80101d47:	e8 c4 3b 00 00       	call   80105910 <holdingsleep>
80101d4c:	83 c4 10             	add    $0x10,%esp
80101d4f:	85 c0                	test   %eax,%eax
80101d51:	74 15                	je     80101d68 <iunlock+0x38>
80101d53:	8b 43 08             	mov    0x8(%ebx),%eax
80101d56:	85 c0                	test   %eax,%eax
80101d58:	7e 0e                	jle    80101d68 <iunlock+0x38>
  releasesleep(&ip->lock);
80101d5a:	89 75 08             	mov    %esi,0x8(%ebp)
}
80101d5d:	8d 65 f8             	lea    -0x8(%ebp),%esp
80101d60:	5b                   	pop    %ebx
80101d61:	5e                   	pop    %esi
80101d62:	5d                   	pop    %ebp
  releasesleep(&ip->lock);
80101d63:	e9 68 3b 00 00       	jmp    801058d0 <releasesleep>
    panic("iunlock");
80101d68:	83 ec 0c             	sub    $0xc,%esp
80101d6b:	68 37 8d 10 80       	push   $0x80108d37
80101d70:	e8 1b e6 ff ff       	call   80100390 <panic>
80101d75:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80101d7c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80101d80 <iput>:
{
80101d80:	f3 0f 1e fb          	endbr32 
80101d84:	55                   	push   %ebp
80101d85:	89 e5                	mov    %esp,%ebp
80101d87:	57                   	push   %edi
80101d88:	56                   	push   %esi
80101d89:	53                   	push   %ebx
80101d8a:	83 ec 28             	sub    $0x28,%esp
80101d8d:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquiresleep(&ip->lock);
80101d90:	8d 7b 0c             	lea    0xc(%ebx),%edi
80101d93:	57                   	push   %edi
80101d94:	e8 d7 3a 00 00       	call   80105870 <acquiresleep>
  if(ip->valid && ip->nlink == 0){
80101d99:	8b 53 4c             	mov    0x4c(%ebx),%edx
80101d9c:	83 c4 10             	add    $0x10,%esp
80101d9f:	85 d2                	test   %edx,%edx
80101da1:	74 07                	je     80101daa <iput+0x2a>
80101da3:	66 83 7b 56 00       	cmpw   $0x0,0x56(%ebx)
80101da8:	74 36                	je     80101de0 <iput+0x60>
  releasesleep(&ip->lock);
80101daa:	83 ec 0c             	sub    $0xc,%esp
80101dad:	57                   	push   %edi
80101dae:	e8 1d 3b 00 00       	call   801058d0 <releasesleep>
  acquire(&icache.lock);
80101db3:	c7 04 24 e0 2f 11 80 	movl   $0x80112fe0,(%esp)
80101dba:	e8 51 3e 00 00       	call   80105c10 <acquire>
  ip->ref--;
80101dbf:	83 6b 08 01          	subl   $0x1,0x8(%ebx)
  release(&icache.lock);
80101dc3:	83 c4 10             	add    $0x10,%esp
80101dc6:	c7 45 08 e0 2f 11 80 	movl   $0x80112fe0,0x8(%ebp)
}
80101dcd:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101dd0:	5b                   	pop    %ebx
80101dd1:	5e                   	pop    %esi
80101dd2:	5f                   	pop    %edi
80101dd3:	5d                   	pop    %ebp
  release(&icache.lock);
80101dd4:	e9 f7 3e 00 00       	jmp    80105cd0 <release>
80101dd9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    acquire(&icache.lock);
80101de0:	83 ec 0c             	sub    $0xc,%esp
80101de3:	68 e0 2f 11 80       	push   $0x80112fe0
80101de8:	e8 23 3e 00 00       	call   80105c10 <acquire>
    int r = ip->ref;
80101ded:	8b 73 08             	mov    0x8(%ebx),%esi
    release(&icache.lock);
80101df0:	c7 04 24 e0 2f 11 80 	movl   $0x80112fe0,(%esp)
80101df7:	e8 d4 3e 00 00       	call   80105cd0 <release>
    if(r == 1){
80101dfc:	83 c4 10             	add    $0x10,%esp
80101dff:	83 fe 01             	cmp    $0x1,%esi
80101e02:	75 a6                	jne    80101daa <iput+0x2a>
80101e04:	8d 8b 8c 00 00 00    	lea    0x8c(%ebx),%ecx
80101e0a:	89 7d e4             	mov    %edi,-0x1c(%ebp)
80101e0d:	8d 73 5c             	lea    0x5c(%ebx),%esi
80101e10:	89 cf                	mov    %ecx,%edi
80101e12:	eb 0b                	jmp    80101e1f <iput+0x9f>
80101e14:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
{
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101e18:	83 c6 04             	add    $0x4,%esi
80101e1b:	39 fe                	cmp    %edi,%esi
80101e1d:	74 19                	je     80101e38 <iput+0xb8>
    if(ip->addrs[i]){
80101e1f:	8b 16                	mov    (%esi),%edx
80101e21:	85 d2                	test   %edx,%edx
80101e23:	74 f3                	je     80101e18 <iput+0x98>
      bfree(ip->dev, ip->addrs[i]);
80101e25:	8b 03                	mov    (%ebx),%eax
80101e27:	e8 74 f8 ff ff       	call   801016a0 <bfree>
      ip->addrs[i] = 0;
80101e2c:	c7 06 00 00 00 00    	movl   $0x0,(%esi)
80101e32:	eb e4                	jmp    80101e18 <iput+0x98>
80101e34:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    }
  }

  if(ip->addrs[NDIRECT]){
80101e38:	8b 83 8c 00 00 00    	mov    0x8c(%ebx),%eax
80101e3e:	8b 7d e4             	mov    -0x1c(%ebp),%edi
80101e41:	85 c0                	test   %eax,%eax
80101e43:	75 33                	jne    80101e78 <iput+0xf8>
    bfree(ip->dev, ip->addrs[NDIRECT]);
    ip->addrs[NDIRECT] = 0;
  }

  ip->size = 0;
  iupdate(ip);
80101e45:	83 ec 0c             	sub    $0xc,%esp
  ip->size = 0;
80101e48:	c7 43 58 00 00 00 00 	movl   $0x0,0x58(%ebx)
  iupdate(ip);
80101e4f:	53                   	push   %ebx
80101e50:	e8 3b fd ff ff       	call   80101b90 <iupdate>
      ip->type = 0;
80101e55:	31 c0                	xor    %eax,%eax
80101e57:	66 89 43 50          	mov    %ax,0x50(%ebx)
      iupdate(ip);
80101e5b:	89 1c 24             	mov    %ebx,(%esp)
80101e5e:	e8 2d fd ff ff       	call   80101b90 <iupdate>
      ip->valid = 0;
80101e63:	c7 43 4c 00 00 00 00 	movl   $0x0,0x4c(%ebx)
80101e6a:	83 c4 10             	add    $0x10,%esp
80101e6d:	e9 38 ff ff ff       	jmp    80101daa <iput+0x2a>
80101e72:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
80101e78:	83 ec 08             	sub    $0x8,%esp
80101e7b:	50                   	push   %eax
80101e7c:	ff 33                	pushl  (%ebx)
80101e7e:	e8 4d e2 ff ff       	call   801000d0 <bread>
80101e83:	89 7d e0             	mov    %edi,-0x20(%ebp)
80101e86:	83 c4 10             	add    $0x10,%esp
80101e89:	8d 88 5c 02 00 00    	lea    0x25c(%eax),%ecx
80101e8f:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    for(j = 0; j < NINDIRECT; j++){
80101e92:	8d 70 5c             	lea    0x5c(%eax),%esi
80101e95:	89 cf                	mov    %ecx,%edi
80101e97:	eb 0e                	jmp    80101ea7 <iput+0x127>
80101e99:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80101ea0:	83 c6 04             	add    $0x4,%esi
80101ea3:	39 f7                	cmp    %esi,%edi
80101ea5:	74 19                	je     80101ec0 <iput+0x140>
      if(a[j])
80101ea7:	8b 16                	mov    (%esi),%edx
80101ea9:	85 d2                	test   %edx,%edx
80101eab:	74 f3                	je     80101ea0 <iput+0x120>
        bfree(ip->dev, a[j]);
80101ead:	8b 03                	mov    (%ebx),%eax
80101eaf:	e8 ec f7 ff ff       	call   801016a0 <bfree>
80101eb4:	eb ea                	jmp    80101ea0 <iput+0x120>
80101eb6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80101ebd:	8d 76 00             	lea    0x0(%esi),%esi
    brelse(bp);
80101ec0:	83 ec 0c             	sub    $0xc,%esp
80101ec3:	ff 75 e4             	pushl  -0x1c(%ebp)
80101ec6:	8b 7d e0             	mov    -0x20(%ebp),%edi
80101ec9:	e8 22 e3 ff ff       	call   801001f0 <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
80101ece:	8b 93 8c 00 00 00    	mov    0x8c(%ebx),%edx
80101ed4:	8b 03                	mov    (%ebx),%eax
80101ed6:	e8 c5 f7 ff ff       	call   801016a0 <bfree>
    ip->addrs[NDIRECT] = 0;
80101edb:	83 c4 10             	add    $0x10,%esp
80101ede:	c7 83 8c 00 00 00 00 	movl   $0x0,0x8c(%ebx)
80101ee5:	00 00 00 
80101ee8:	e9 58 ff ff ff       	jmp    80101e45 <iput+0xc5>
80101eed:	8d 76 00             	lea    0x0(%esi),%esi

80101ef0 <iunlockput>:
{
80101ef0:	f3 0f 1e fb          	endbr32 
80101ef4:	55                   	push   %ebp
80101ef5:	89 e5                	mov    %esp,%ebp
80101ef7:	53                   	push   %ebx
80101ef8:	83 ec 10             	sub    $0x10,%esp
80101efb:	8b 5d 08             	mov    0x8(%ebp),%ebx
  iunlock(ip);
80101efe:	53                   	push   %ebx
80101eff:	e8 2c fe ff ff       	call   80101d30 <iunlock>
  iput(ip);
80101f04:	89 5d 08             	mov    %ebx,0x8(%ebp)
80101f07:	83 c4 10             	add    $0x10,%esp
}
80101f0a:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80101f0d:	c9                   	leave  
  iput(ip);
80101f0e:	e9 6d fe ff ff       	jmp    80101d80 <iput>
80101f13:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80101f1a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80101f20 <stati>:

// Copy stat information from inode.
// Caller must hold ip->lock.
void
stati(struct inode *ip, struct stat *st)
{
80101f20:	f3 0f 1e fb          	endbr32 
80101f24:	55                   	push   %ebp
80101f25:	89 e5                	mov    %esp,%ebp
80101f27:	8b 55 08             	mov    0x8(%ebp),%edx
80101f2a:	8b 45 0c             	mov    0xc(%ebp),%eax
  st->dev = ip->dev;
80101f2d:	8b 0a                	mov    (%edx),%ecx
80101f2f:	89 48 04             	mov    %ecx,0x4(%eax)
  st->ino = ip->inum;
80101f32:	8b 4a 04             	mov    0x4(%edx),%ecx
80101f35:	89 48 08             	mov    %ecx,0x8(%eax)
  st->type = ip->type;
80101f38:	0f b7 4a 50          	movzwl 0x50(%edx),%ecx
80101f3c:	66 89 08             	mov    %cx,(%eax)
  st->nlink = ip->nlink;
80101f3f:	0f b7 4a 56          	movzwl 0x56(%edx),%ecx
80101f43:	66 89 48 0c          	mov    %cx,0xc(%eax)
  st->size = ip->size;
80101f47:	8b 52 58             	mov    0x58(%edx),%edx
80101f4a:	89 50 10             	mov    %edx,0x10(%eax)
}
80101f4d:	5d                   	pop    %ebp
80101f4e:	c3                   	ret    
80101f4f:	90                   	nop

80101f50 <readi>:
//PAGEBREAK!
// Read data from inode.
// Caller must hold ip->lock.
int
readi(struct inode *ip, char *dst, uint off, uint n)
{
80101f50:	f3 0f 1e fb          	endbr32 
80101f54:	55                   	push   %ebp
80101f55:	89 e5                	mov    %esp,%ebp
80101f57:	57                   	push   %edi
80101f58:	56                   	push   %esi
80101f59:	53                   	push   %ebx
80101f5a:	83 ec 1c             	sub    $0x1c,%esp
80101f5d:	8b 7d 0c             	mov    0xc(%ebp),%edi
80101f60:	8b 45 08             	mov    0x8(%ebp),%eax
80101f63:	8b 75 10             	mov    0x10(%ebp),%esi
80101f66:	89 7d e0             	mov    %edi,-0x20(%ebp)
80101f69:	8b 7d 14             	mov    0x14(%ebp),%edi
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101f6c:	66 83 78 50 03       	cmpw   $0x3,0x50(%eax)
{
80101f71:	89 45 d8             	mov    %eax,-0x28(%ebp)
80101f74:	89 7d e4             	mov    %edi,-0x1c(%ebp)
  if(ip->type == T_DEV){
80101f77:	0f 84 a3 00 00 00    	je     80102020 <readi+0xd0>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].read)
      return -1;
    return devsw[ip->major].read(ip, dst, n);
  }

  if(off > ip->size || off + n < off)
80101f7d:	8b 45 d8             	mov    -0x28(%ebp),%eax
80101f80:	8b 40 58             	mov    0x58(%eax),%eax
80101f83:	39 c6                	cmp    %eax,%esi
80101f85:	0f 87 b6 00 00 00    	ja     80102041 <readi+0xf1>
80101f8b:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
80101f8e:	31 c9                	xor    %ecx,%ecx
80101f90:	89 da                	mov    %ebx,%edx
80101f92:	01 f2                	add    %esi,%edx
80101f94:	0f 92 c1             	setb   %cl
80101f97:	89 cf                	mov    %ecx,%edi
80101f99:	0f 82 a2 00 00 00    	jb     80102041 <readi+0xf1>
    return -1;
  if(off + n > ip->size)
    n = ip->size - off;
80101f9f:	89 c1                	mov    %eax,%ecx
80101fa1:	29 f1                	sub    %esi,%ecx
80101fa3:	39 d0                	cmp    %edx,%eax
80101fa5:	0f 43 cb             	cmovae %ebx,%ecx
80101fa8:	89 4d e4             	mov    %ecx,-0x1c(%ebp)

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101fab:	85 c9                	test   %ecx,%ecx
80101fad:	74 63                	je     80102012 <readi+0xc2>
80101faf:	90                   	nop
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101fb0:	8b 5d d8             	mov    -0x28(%ebp),%ebx
80101fb3:	89 f2                	mov    %esi,%edx
80101fb5:	c1 ea 09             	shr    $0x9,%edx
80101fb8:	89 d8                	mov    %ebx,%eax
80101fba:	e8 61 f9 ff ff       	call   80101920 <bmap>
80101fbf:	83 ec 08             	sub    $0x8,%esp
80101fc2:	50                   	push   %eax
80101fc3:	ff 33                	pushl  (%ebx)
80101fc5:	e8 06 e1 ff ff       	call   801000d0 <bread>
    m = min(n - tot, BSIZE - off%BSIZE);
80101fca:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
80101fcd:	b9 00 02 00 00       	mov    $0x200,%ecx
80101fd2:	83 c4 0c             	add    $0xc,%esp
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101fd5:	89 c2                	mov    %eax,%edx
    m = min(n - tot, BSIZE - off%BSIZE);
80101fd7:	89 f0                	mov    %esi,%eax
80101fd9:	25 ff 01 00 00       	and    $0x1ff,%eax
80101fde:	29 fb                	sub    %edi,%ebx
    memmove(dst, bp->data + off%BSIZE, m);
80101fe0:	89 55 dc             	mov    %edx,-0x24(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101fe3:	29 c1                	sub    %eax,%ecx
    memmove(dst, bp->data + off%BSIZE, m);
80101fe5:	8d 44 02 5c          	lea    0x5c(%edx,%eax,1),%eax
    m = min(n - tot, BSIZE - off%BSIZE);
80101fe9:	39 d9                	cmp    %ebx,%ecx
80101feb:	0f 46 d9             	cmovbe %ecx,%ebx
    memmove(dst, bp->data + off%BSIZE, m);
80101fee:	53                   	push   %ebx
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101fef:	01 df                	add    %ebx,%edi
80101ff1:	01 de                	add    %ebx,%esi
    memmove(dst, bp->data + off%BSIZE, m);
80101ff3:	50                   	push   %eax
80101ff4:	ff 75 e0             	pushl  -0x20(%ebp)
80101ff7:	e8 c4 3d 00 00       	call   80105dc0 <memmove>
    brelse(bp);
80101ffc:	8b 55 dc             	mov    -0x24(%ebp),%edx
80101fff:	89 14 24             	mov    %edx,(%esp)
80102002:	e8 e9 e1 ff ff       	call   801001f0 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80102007:	01 5d e0             	add    %ebx,-0x20(%ebp)
8010200a:	83 c4 10             	add    $0x10,%esp
8010200d:	39 7d e4             	cmp    %edi,-0x1c(%ebp)
80102010:	77 9e                	ja     80101fb0 <readi+0x60>
  }
  return n;
80102012:	8b 45 e4             	mov    -0x1c(%ebp),%eax
}
80102015:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102018:	5b                   	pop    %ebx
80102019:	5e                   	pop    %esi
8010201a:	5f                   	pop    %edi
8010201b:	5d                   	pop    %ebp
8010201c:	c3                   	ret    
8010201d:	8d 76 00             	lea    0x0(%esi),%esi
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].read)
80102020:	0f bf 40 52          	movswl 0x52(%eax),%eax
80102024:	66 83 f8 09          	cmp    $0x9,%ax
80102028:	77 17                	ja     80102041 <readi+0xf1>
8010202a:	8b 04 c5 60 2f 11 80 	mov    -0x7feed0a0(,%eax,8),%eax
80102031:	85 c0                	test   %eax,%eax
80102033:	74 0c                	je     80102041 <readi+0xf1>
    return devsw[ip->major].read(ip, dst, n);
80102035:	89 7d 10             	mov    %edi,0x10(%ebp)
}
80102038:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010203b:	5b                   	pop    %ebx
8010203c:	5e                   	pop    %esi
8010203d:	5f                   	pop    %edi
8010203e:	5d                   	pop    %ebp
    return devsw[ip->major].read(ip, dst, n);
8010203f:	ff e0                	jmp    *%eax
      return -1;
80102041:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102046:	eb cd                	jmp    80102015 <readi+0xc5>
80102048:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010204f:	90                   	nop

80102050 <writei>:
// PAGEBREAK!
// Write data to inode.
// Caller must hold ip->lock.
int
writei(struct inode *ip, char *src, uint off, uint n)
{
80102050:	f3 0f 1e fb          	endbr32 
80102054:	55                   	push   %ebp
80102055:	89 e5                	mov    %esp,%ebp
80102057:	57                   	push   %edi
80102058:	56                   	push   %esi
80102059:	53                   	push   %ebx
8010205a:	83 ec 1c             	sub    $0x1c,%esp
8010205d:	8b 45 08             	mov    0x8(%ebp),%eax
80102060:	8b 75 0c             	mov    0xc(%ebp),%esi
80102063:	8b 7d 14             	mov    0x14(%ebp),%edi
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80102066:	66 83 78 50 03       	cmpw   $0x3,0x50(%eax)
{
8010206b:	89 75 dc             	mov    %esi,-0x24(%ebp)
8010206e:	89 45 d8             	mov    %eax,-0x28(%ebp)
80102071:	8b 75 10             	mov    0x10(%ebp),%esi
80102074:	89 7d e0             	mov    %edi,-0x20(%ebp)
  if(ip->type == T_DEV){
80102077:	0f 84 b3 00 00 00    	je     80102130 <writei+0xe0>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].write)
      return -1;
    return devsw[ip->major].write(ip, src, n);
  }

  if(off > ip->size || off + n < off)
8010207d:	8b 45 d8             	mov    -0x28(%ebp),%eax
80102080:	39 70 58             	cmp    %esi,0x58(%eax)
80102083:	0f 82 e3 00 00 00    	jb     8010216c <writei+0x11c>
    return -1;
  if(off + n > MAXFILE*BSIZE)
80102089:	8b 7d e0             	mov    -0x20(%ebp),%edi
8010208c:	89 f8                	mov    %edi,%eax
8010208e:	01 f0                	add    %esi,%eax
80102090:	0f 82 d6 00 00 00    	jb     8010216c <writei+0x11c>
80102096:	3d 00 18 01 00       	cmp    $0x11800,%eax
8010209b:	0f 87 cb 00 00 00    	ja     8010216c <writei+0x11c>
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
801020a1:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
801020a8:	85 ff                	test   %edi,%edi
801020aa:	74 75                	je     80102121 <writei+0xd1>
801020ac:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
801020b0:	8b 7d d8             	mov    -0x28(%ebp),%edi
801020b3:	89 f2                	mov    %esi,%edx
801020b5:	c1 ea 09             	shr    $0x9,%edx
801020b8:	89 f8                	mov    %edi,%eax
801020ba:	e8 61 f8 ff ff       	call   80101920 <bmap>
801020bf:	83 ec 08             	sub    $0x8,%esp
801020c2:	50                   	push   %eax
801020c3:	ff 37                	pushl  (%edi)
801020c5:	e8 06 e0 ff ff       	call   801000d0 <bread>
    m = min(n - tot, BSIZE - off%BSIZE);
801020ca:	b9 00 02 00 00       	mov    $0x200,%ecx
801020cf:	8b 5d e0             	mov    -0x20(%ebp),%ebx
801020d2:	2b 5d e4             	sub    -0x1c(%ebp),%ebx
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
801020d5:	89 c7                	mov    %eax,%edi
    m = min(n - tot, BSIZE - off%BSIZE);
801020d7:	89 f0                	mov    %esi,%eax
801020d9:	83 c4 0c             	add    $0xc,%esp
801020dc:	25 ff 01 00 00       	and    $0x1ff,%eax
801020e1:	29 c1                	sub    %eax,%ecx
    memmove(bp->data + off%BSIZE, src, m);
801020e3:	8d 44 07 5c          	lea    0x5c(%edi,%eax,1),%eax
    m = min(n - tot, BSIZE - off%BSIZE);
801020e7:	39 d9                	cmp    %ebx,%ecx
801020e9:	0f 46 d9             	cmovbe %ecx,%ebx
    memmove(bp->data + off%BSIZE, src, m);
801020ec:	53                   	push   %ebx
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
801020ed:	01 de                	add    %ebx,%esi
    memmove(bp->data + off%BSIZE, src, m);
801020ef:	ff 75 dc             	pushl  -0x24(%ebp)
801020f2:	50                   	push   %eax
801020f3:	e8 c8 3c 00 00       	call   80105dc0 <memmove>
    log_write(bp);
801020f8:	89 3c 24             	mov    %edi,(%esp)
801020fb:	e8 00 13 00 00       	call   80103400 <log_write>
    brelse(bp);
80102100:	89 3c 24             	mov    %edi,(%esp)
80102103:	e8 e8 e0 ff ff       	call   801001f0 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80102108:	01 5d e4             	add    %ebx,-0x1c(%ebp)
8010210b:	83 c4 10             	add    $0x10,%esp
8010210e:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80102111:	01 5d dc             	add    %ebx,-0x24(%ebp)
80102114:	39 45 e0             	cmp    %eax,-0x20(%ebp)
80102117:	77 97                	ja     801020b0 <writei+0x60>
  }

  if(n > 0 && off > ip->size){
80102119:	8b 45 d8             	mov    -0x28(%ebp),%eax
8010211c:	3b 70 58             	cmp    0x58(%eax),%esi
8010211f:	77 37                	ja     80102158 <writei+0x108>
    ip->size = off;
    iupdate(ip);
  }
  return n;
80102121:	8b 45 e0             	mov    -0x20(%ebp),%eax
}
80102124:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102127:	5b                   	pop    %ebx
80102128:	5e                   	pop    %esi
80102129:	5f                   	pop    %edi
8010212a:	5d                   	pop    %ebp
8010212b:	c3                   	ret    
8010212c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].write)
80102130:	0f bf 40 52          	movswl 0x52(%eax),%eax
80102134:	66 83 f8 09          	cmp    $0x9,%ax
80102138:	77 32                	ja     8010216c <writei+0x11c>
8010213a:	8b 04 c5 64 2f 11 80 	mov    -0x7feed09c(,%eax,8),%eax
80102141:	85 c0                	test   %eax,%eax
80102143:	74 27                	je     8010216c <writei+0x11c>
    return devsw[ip->major].write(ip, src, n);
80102145:	89 7d 10             	mov    %edi,0x10(%ebp)
}
80102148:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010214b:	5b                   	pop    %ebx
8010214c:	5e                   	pop    %esi
8010214d:	5f                   	pop    %edi
8010214e:	5d                   	pop    %ebp
    return devsw[ip->major].write(ip, src, n);
8010214f:	ff e0                	jmp    *%eax
80102151:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    ip->size = off;
80102158:	8b 45 d8             	mov    -0x28(%ebp),%eax
    iupdate(ip);
8010215b:	83 ec 0c             	sub    $0xc,%esp
    ip->size = off;
8010215e:	89 70 58             	mov    %esi,0x58(%eax)
    iupdate(ip);
80102161:	50                   	push   %eax
80102162:	e8 29 fa ff ff       	call   80101b90 <iupdate>
80102167:	83 c4 10             	add    $0x10,%esp
8010216a:	eb b5                	jmp    80102121 <writei+0xd1>
      return -1;
8010216c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102171:	eb b1                	jmp    80102124 <writei+0xd4>
80102173:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010217a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80102180 <namecmp>:
//PAGEBREAK!
// Directories

int
namecmp(const char *s, const char *t)
{
80102180:	f3 0f 1e fb          	endbr32 
80102184:	55                   	push   %ebp
80102185:	89 e5                	mov    %esp,%ebp
80102187:	83 ec 0c             	sub    $0xc,%esp
  return strncmp(s, t, DIRSIZ);
8010218a:	6a 0e                	push   $0xe
8010218c:	ff 75 0c             	pushl  0xc(%ebp)
8010218f:	ff 75 08             	pushl  0x8(%ebp)
80102192:	e8 99 3c 00 00       	call   80105e30 <strncmp>
}
80102197:	c9                   	leave  
80102198:	c3                   	ret    
80102199:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

801021a0 <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
801021a0:	f3 0f 1e fb          	endbr32 
801021a4:	55                   	push   %ebp
801021a5:	89 e5                	mov    %esp,%ebp
801021a7:	57                   	push   %edi
801021a8:	56                   	push   %esi
801021a9:	53                   	push   %ebx
801021aa:	83 ec 1c             	sub    $0x1c,%esp
801021ad:	8b 5d 08             	mov    0x8(%ebp),%ebx
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
801021b0:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
801021b5:	0f 85 89 00 00 00    	jne    80102244 <dirlookup+0xa4>
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
801021bb:	8b 53 58             	mov    0x58(%ebx),%edx
801021be:	31 ff                	xor    %edi,%edi
801021c0:	8d 75 d8             	lea    -0x28(%ebp),%esi
801021c3:	85 d2                	test   %edx,%edx
801021c5:	74 42                	je     80102209 <dirlookup+0x69>
801021c7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801021ce:	66 90                	xchg   %ax,%ax
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801021d0:	6a 10                	push   $0x10
801021d2:	57                   	push   %edi
801021d3:	56                   	push   %esi
801021d4:	53                   	push   %ebx
801021d5:	e8 76 fd ff ff       	call   80101f50 <readi>
801021da:	83 c4 10             	add    $0x10,%esp
801021dd:	83 f8 10             	cmp    $0x10,%eax
801021e0:	75 55                	jne    80102237 <dirlookup+0x97>
      panic("dirlookup read");
    if(de.inum == 0)
801021e2:	66 83 7d d8 00       	cmpw   $0x0,-0x28(%ebp)
801021e7:	74 18                	je     80102201 <dirlookup+0x61>
  return strncmp(s, t, DIRSIZ);
801021e9:	83 ec 04             	sub    $0x4,%esp
801021ec:	8d 45 da             	lea    -0x26(%ebp),%eax
801021ef:	6a 0e                	push   $0xe
801021f1:	50                   	push   %eax
801021f2:	ff 75 0c             	pushl  0xc(%ebp)
801021f5:	e8 36 3c 00 00       	call   80105e30 <strncmp>
      continue;
    if(namecmp(name, de.name) == 0){
801021fa:	83 c4 10             	add    $0x10,%esp
801021fd:	85 c0                	test   %eax,%eax
801021ff:	74 17                	je     80102218 <dirlookup+0x78>
  for(off = 0; off < dp->size; off += sizeof(de)){
80102201:	83 c7 10             	add    $0x10,%edi
80102204:	3b 7b 58             	cmp    0x58(%ebx),%edi
80102207:	72 c7                	jb     801021d0 <dirlookup+0x30>
      return iget(dp->dev, inum);
    }
  }

  return 0;
}
80102209:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
8010220c:	31 c0                	xor    %eax,%eax
}
8010220e:	5b                   	pop    %ebx
8010220f:	5e                   	pop    %esi
80102210:	5f                   	pop    %edi
80102211:	5d                   	pop    %ebp
80102212:	c3                   	ret    
80102213:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80102217:	90                   	nop
      if(poff)
80102218:	8b 45 10             	mov    0x10(%ebp),%eax
8010221b:	85 c0                	test   %eax,%eax
8010221d:	74 05                	je     80102224 <dirlookup+0x84>
        *poff = off;
8010221f:	8b 45 10             	mov    0x10(%ebp),%eax
80102222:	89 38                	mov    %edi,(%eax)
      inum = de.inum;
80102224:	0f b7 55 d8          	movzwl -0x28(%ebp),%edx
      return iget(dp->dev, inum);
80102228:	8b 03                	mov    (%ebx),%eax
8010222a:	e8 01 f6 ff ff       	call   80101830 <iget>
}
8010222f:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102232:	5b                   	pop    %ebx
80102233:	5e                   	pop    %esi
80102234:	5f                   	pop    %edi
80102235:	5d                   	pop    %ebp
80102236:	c3                   	ret    
      panic("dirlookup read");
80102237:	83 ec 0c             	sub    $0xc,%esp
8010223a:	68 51 8d 10 80       	push   $0x80108d51
8010223f:	e8 4c e1 ff ff       	call   80100390 <panic>
    panic("dirlookup not DIR");
80102244:	83 ec 0c             	sub    $0xc,%esp
80102247:	68 3f 8d 10 80       	push   $0x80108d3f
8010224c:	e8 3f e1 ff ff       	call   80100390 <panic>
80102251:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102258:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010225f:	90                   	nop

80102260 <namex>:
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
// Must be called inside a transaction since it calls iput().
static struct inode*
namex(char *path, int nameiparent, char *name)
{
80102260:	55                   	push   %ebp
80102261:	89 e5                	mov    %esp,%ebp
80102263:	57                   	push   %edi
80102264:	56                   	push   %esi
80102265:	53                   	push   %ebx
80102266:	89 c3                	mov    %eax,%ebx
80102268:	83 ec 1c             	sub    $0x1c,%esp
  struct inode *ip, *next;

  if(*path == '/')
8010226b:	80 38 2f             	cmpb   $0x2f,(%eax)
{
8010226e:	89 55 e0             	mov    %edx,-0x20(%ebp)
80102271:	89 4d e4             	mov    %ecx,-0x1c(%ebp)
  if(*path == '/')
80102274:	0f 84 86 01 00 00    	je     80102400 <namex+0x1a0>
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(myproc()->cwd);
8010227a:	e8 41 1c 00 00       	call   80103ec0 <myproc>
  acquire(&icache.lock);
8010227f:	83 ec 0c             	sub    $0xc,%esp
80102282:	89 df                	mov    %ebx,%edi
    ip = idup(myproc()->cwd);
80102284:	8b 70 68             	mov    0x68(%eax),%esi
  acquire(&icache.lock);
80102287:	68 e0 2f 11 80       	push   $0x80112fe0
8010228c:	e8 7f 39 00 00       	call   80105c10 <acquire>
  ip->ref++;
80102291:	83 46 08 01          	addl   $0x1,0x8(%esi)
  release(&icache.lock);
80102295:	c7 04 24 e0 2f 11 80 	movl   $0x80112fe0,(%esp)
8010229c:	e8 2f 3a 00 00       	call   80105cd0 <release>
801022a1:	83 c4 10             	add    $0x10,%esp
801022a4:	eb 0d                	jmp    801022b3 <namex+0x53>
801022a6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801022ad:	8d 76 00             	lea    0x0(%esi),%esi
    path++;
801022b0:	83 c7 01             	add    $0x1,%edi
  while(*path == '/')
801022b3:	0f b6 07             	movzbl (%edi),%eax
801022b6:	3c 2f                	cmp    $0x2f,%al
801022b8:	74 f6                	je     801022b0 <namex+0x50>
  if(*path == 0)
801022ba:	84 c0                	test   %al,%al
801022bc:	0f 84 ee 00 00 00    	je     801023b0 <namex+0x150>
  while(*path != '/' && *path != 0)
801022c2:	0f b6 07             	movzbl (%edi),%eax
801022c5:	84 c0                	test   %al,%al
801022c7:	0f 84 fb 00 00 00    	je     801023c8 <namex+0x168>
801022cd:	89 fb                	mov    %edi,%ebx
801022cf:	3c 2f                	cmp    $0x2f,%al
801022d1:	0f 84 f1 00 00 00    	je     801023c8 <namex+0x168>
801022d7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801022de:	66 90                	xchg   %ax,%ax
801022e0:	0f b6 43 01          	movzbl 0x1(%ebx),%eax
    path++;
801022e4:	83 c3 01             	add    $0x1,%ebx
  while(*path != '/' && *path != 0)
801022e7:	3c 2f                	cmp    $0x2f,%al
801022e9:	74 04                	je     801022ef <namex+0x8f>
801022eb:	84 c0                	test   %al,%al
801022ed:	75 f1                	jne    801022e0 <namex+0x80>
  len = path - s;
801022ef:	89 d8                	mov    %ebx,%eax
801022f1:	29 f8                	sub    %edi,%eax
  if(len >= DIRSIZ)
801022f3:	83 f8 0d             	cmp    $0xd,%eax
801022f6:	0f 8e 84 00 00 00    	jle    80102380 <namex+0x120>
    memmove(name, s, DIRSIZ);
801022fc:	83 ec 04             	sub    $0x4,%esp
801022ff:	6a 0e                	push   $0xe
80102301:	57                   	push   %edi
    path++;
80102302:	89 df                	mov    %ebx,%edi
    memmove(name, s, DIRSIZ);
80102304:	ff 75 e4             	pushl  -0x1c(%ebp)
80102307:	e8 b4 3a 00 00       	call   80105dc0 <memmove>
8010230c:	83 c4 10             	add    $0x10,%esp
  while(*path == '/')
8010230f:	80 3b 2f             	cmpb   $0x2f,(%ebx)
80102312:	75 0c                	jne    80102320 <namex+0xc0>
80102314:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    path++;
80102318:	83 c7 01             	add    $0x1,%edi
  while(*path == '/')
8010231b:	80 3f 2f             	cmpb   $0x2f,(%edi)
8010231e:	74 f8                	je     80102318 <namex+0xb8>

  while((path = skipelem(path, name)) != 0){
    ilock(ip);
80102320:	83 ec 0c             	sub    $0xc,%esp
80102323:	56                   	push   %esi
80102324:	e8 27 f9 ff ff       	call   80101c50 <ilock>
    if(ip->type != T_DIR){
80102329:	83 c4 10             	add    $0x10,%esp
8010232c:	66 83 7e 50 01       	cmpw   $0x1,0x50(%esi)
80102331:	0f 85 a1 00 00 00    	jne    801023d8 <namex+0x178>
      iunlockput(ip);
      return 0;
    }
    if(nameiparent && *path == '\0'){
80102337:	8b 55 e0             	mov    -0x20(%ebp),%edx
8010233a:	85 d2                	test   %edx,%edx
8010233c:	74 09                	je     80102347 <namex+0xe7>
8010233e:	80 3f 00             	cmpb   $0x0,(%edi)
80102341:	0f 84 d9 00 00 00    	je     80102420 <namex+0x1c0>
      // Stop one level early.
      iunlock(ip);
      return ip;
    }
    if((next = dirlookup(ip, name, 0)) == 0){
80102347:	83 ec 04             	sub    $0x4,%esp
8010234a:	6a 00                	push   $0x0
8010234c:	ff 75 e4             	pushl  -0x1c(%ebp)
8010234f:	56                   	push   %esi
80102350:	e8 4b fe ff ff       	call   801021a0 <dirlookup>
80102355:	83 c4 10             	add    $0x10,%esp
80102358:	89 c3                	mov    %eax,%ebx
8010235a:	85 c0                	test   %eax,%eax
8010235c:	74 7a                	je     801023d8 <namex+0x178>
  iunlock(ip);
8010235e:	83 ec 0c             	sub    $0xc,%esp
80102361:	56                   	push   %esi
80102362:	e8 c9 f9 ff ff       	call   80101d30 <iunlock>
  iput(ip);
80102367:	89 34 24             	mov    %esi,(%esp)
8010236a:	89 de                	mov    %ebx,%esi
8010236c:	e8 0f fa ff ff       	call   80101d80 <iput>
80102371:	83 c4 10             	add    $0x10,%esp
80102374:	e9 3a ff ff ff       	jmp    801022b3 <namex+0x53>
80102379:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102380:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80102383:	8d 0c 02             	lea    (%edx,%eax,1),%ecx
80102386:	89 4d dc             	mov    %ecx,-0x24(%ebp)
    memmove(name, s, len);
80102389:	83 ec 04             	sub    $0x4,%esp
8010238c:	50                   	push   %eax
8010238d:	57                   	push   %edi
    name[len] = 0;
8010238e:	89 df                	mov    %ebx,%edi
    memmove(name, s, len);
80102390:	ff 75 e4             	pushl  -0x1c(%ebp)
80102393:	e8 28 3a 00 00       	call   80105dc0 <memmove>
    name[len] = 0;
80102398:	8b 45 dc             	mov    -0x24(%ebp),%eax
8010239b:	83 c4 10             	add    $0x10,%esp
8010239e:	c6 00 00             	movb   $0x0,(%eax)
801023a1:	e9 69 ff ff ff       	jmp    8010230f <namex+0xaf>
801023a6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801023ad:	8d 76 00             	lea    0x0(%esi),%esi
      return 0;
    }
    iunlockput(ip);
    ip = next;
  }
  if(nameiparent){
801023b0:	8b 45 e0             	mov    -0x20(%ebp),%eax
801023b3:	85 c0                	test   %eax,%eax
801023b5:	0f 85 85 00 00 00    	jne    80102440 <namex+0x1e0>
    iput(ip);
    return 0;
  }
  return ip;
}
801023bb:	8d 65 f4             	lea    -0xc(%ebp),%esp
801023be:	89 f0                	mov    %esi,%eax
801023c0:	5b                   	pop    %ebx
801023c1:	5e                   	pop    %esi
801023c2:	5f                   	pop    %edi
801023c3:	5d                   	pop    %ebp
801023c4:	c3                   	ret    
801023c5:	8d 76 00             	lea    0x0(%esi),%esi
  while(*path != '/' && *path != 0)
801023c8:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801023cb:	89 fb                	mov    %edi,%ebx
801023cd:	89 45 dc             	mov    %eax,-0x24(%ebp)
801023d0:	31 c0                	xor    %eax,%eax
801023d2:	eb b5                	jmp    80102389 <namex+0x129>
801023d4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  iunlock(ip);
801023d8:	83 ec 0c             	sub    $0xc,%esp
801023db:	56                   	push   %esi
801023dc:	e8 4f f9 ff ff       	call   80101d30 <iunlock>
  iput(ip);
801023e1:	89 34 24             	mov    %esi,(%esp)
      return 0;
801023e4:	31 f6                	xor    %esi,%esi
  iput(ip);
801023e6:	e8 95 f9 ff ff       	call   80101d80 <iput>
      return 0;
801023eb:	83 c4 10             	add    $0x10,%esp
}
801023ee:	8d 65 f4             	lea    -0xc(%ebp),%esp
801023f1:	89 f0                	mov    %esi,%eax
801023f3:	5b                   	pop    %ebx
801023f4:	5e                   	pop    %esi
801023f5:	5f                   	pop    %edi
801023f6:	5d                   	pop    %ebp
801023f7:	c3                   	ret    
801023f8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801023ff:	90                   	nop
    ip = iget(ROOTDEV, ROOTINO);
80102400:	ba 01 00 00 00       	mov    $0x1,%edx
80102405:	b8 01 00 00 00       	mov    $0x1,%eax
8010240a:	89 df                	mov    %ebx,%edi
8010240c:	e8 1f f4 ff ff       	call   80101830 <iget>
80102411:	89 c6                	mov    %eax,%esi
80102413:	e9 9b fe ff ff       	jmp    801022b3 <namex+0x53>
80102418:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010241f:	90                   	nop
      iunlock(ip);
80102420:	83 ec 0c             	sub    $0xc,%esp
80102423:	56                   	push   %esi
80102424:	e8 07 f9 ff ff       	call   80101d30 <iunlock>
      return ip;
80102429:	83 c4 10             	add    $0x10,%esp
}
8010242c:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010242f:	89 f0                	mov    %esi,%eax
80102431:	5b                   	pop    %ebx
80102432:	5e                   	pop    %esi
80102433:	5f                   	pop    %edi
80102434:	5d                   	pop    %ebp
80102435:	c3                   	ret    
80102436:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010243d:	8d 76 00             	lea    0x0(%esi),%esi
    iput(ip);
80102440:	83 ec 0c             	sub    $0xc,%esp
80102443:	56                   	push   %esi
    return 0;
80102444:	31 f6                	xor    %esi,%esi
    iput(ip);
80102446:	e8 35 f9 ff ff       	call   80101d80 <iput>
    return 0;
8010244b:	83 c4 10             	add    $0x10,%esp
8010244e:	e9 68 ff ff ff       	jmp    801023bb <namex+0x15b>
80102453:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010245a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80102460 <dirlink>:
{
80102460:	f3 0f 1e fb          	endbr32 
80102464:	55                   	push   %ebp
80102465:	89 e5                	mov    %esp,%ebp
80102467:	57                   	push   %edi
80102468:	56                   	push   %esi
80102469:	53                   	push   %ebx
8010246a:	83 ec 20             	sub    $0x20,%esp
8010246d:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if((ip = dirlookup(dp, name, 0)) != 0){
80102470:	6a 00                	push   $0x0
80102472:	ff 75 0c             	pushl  0xc(%ebp)
80102475:	53                   	push   %ebx
80102476:	e8 25 fd ff ff       	call   801021a0 <dirlookup>
8010247b:	83 c4 10             	add    $0x10,%esp
8010247e:	85 c0                	test   %eax,%eax
80102480:	75 6b                	jne    801024ed <dirlink+0x8d>
  for(off = 0; off < dp->size; off += sizeof(de)){
80102482:	8b 7b 58             	mov    0x58(%ebx),%edi
80102485:	8d 75 d8             	lea    -0x28(%ebp),%esi
80102488:	85 ff                	test   %edi,%edi
8010248a:	74 2d                	je     801024b9 <dirlink+0x59>
8010248c:	31 ff                	xor    %edi,%edi
8010248e:	8d 75 d8             	lea    -0x28(%ebp),%esi
80102491:	eb 0d                	jmp    801024a0 <dirlink+0x40>
80102493:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80102497:	90                   	nop
80102498:	83 c7 10             	add    $0x10,%edi
8010249b:	3b 7b 58             	cmp    0x58(%ebx),%edi
8010249e:	73 19                	jae    801024b9 <dirlink+0x59>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801024a0:	6a 10                	push   $0x10
801024a2:	57                   	push   %edi
801024a3:	56                   	push   %esi
801024a4:	53                   	push   %ebx
801024a5:	e8 a6 fa ff ff       	call   80101f50 <readi>
801024aa:	83 c4 10             	add    $0x10,%esp
801024ad:	83 f8 10             	cmp    $0x10,%eax
801024b0:	75 4e                	jne    80102500 <dirlink+0xa0>
    if(de.inum == 0)
801024b2:	66 83 7d d8 00       	cmpw   $0x0,-0x28(%ebp)
801024b7:	75 df                	jne    80102498 <dirlink+0x38>
  strncpy(de.name, name, DIRSIZ);
801024b9:	83 ec 04             	sub    $0x4,%esp
801024bc:	8d 45 da             	lea    -0x26(%ebp),%eax
801024bf:	6a 0e                	push   $0xe
801024c1:	ff 75 0c             	pushl  0xc(%ebp)
801024c4:	50                   	push   %eax
801024c5:	e8 b6 39 00 00       	call   80105e80 <strncpy>
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801024ca:	6a 10                	push   $0x10
  de.inum = inum;
801024cc:	8b 45 10             	mov    0x10(%ebp),%eax
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801024cf:	57                   	push   %edi
801024d0:	56                   	push   %esi
801024d1:	53                   	push   %ebx
  de.inum = inum;
801024d2:	66 89 45 d8          	mov    %ax,-0x28(%ebp)
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801024d6:	e8 75 fb ff ff       	call   80102050 <writei>
801024db:	83 c4 20             	add    $0x20,%esp
801024de:	83 f8 10             	cmp    $0x10,%eax
801024e1:	75 2a                	jne    8010250d <dirlink+0xad>
  return 0;
801024e3:	31 c0                	xor    %eax,%eax
}
801024e5:	8d 65 f4             	lea    -0xc(%ebp),%esp
801024e8:	5b                   	pop    %ebx
801024e9:	5e                   	pop    %esi
801024ea:	5f                   	pop    %edi
801024eb:	5d                   	pop    %ebp
801024ec:	c3                   	ret    
    iput(ip);
801024ed:	83 ec 0c             	sub    $0xc,%esp
801024f0:	50                   	push   %eax
801024f1:	e8 8a f8 ff ff       	call   80101d80 <iput>
    return -1;
801024f6:	83 c4 10             	add    $0x10,%esp
801024f9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801024fe:	eb e5                	jmp    801024e5 <dirlink+0x85>
      panic("dirlink read");
80102500:	83 ec 0c             	sub    $0xc,%esp
80102503:	68 60 8d 10 80       	push   $0x80108d60
80102508:	e8 83 de ff ff       	call   80100390 <panic>
    panic("dirlink");
8010250d:	83 ec 0c             	sub    $0xc,%esp
80102510:	68 82 95 10 80       	push   $0x80109582
80102515:	e8 76 de ff ff       	call   80100390 <panic>
8010251a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80102520 <namei>:

struct inode*
namei(char *path)
{
80102520:	f3 0f 1e fb          	endbr32 
80102524:	55                   	push   %ebp
  char name[DIRSIZ];
  return namex(path, 0, name);
80102525:	31 d2                	xor    %edx,%edx
{
80102527:	89 e5                	mov    %esp,%ebp
80102529:	83 ec 18             	sub    $0x18,%esp
  return namex(path, 0, name);
8010252c:	8b 45 08             	mov    0x8(%ebp),%eax
8010252f:	8d 4d ea             	lea    -0x16(%ebp),%ecx
80102532:	e8 29 fd ff ff       	call   80102260 <namex>
}
80102537:	c9                   	leave  
80102538:	c3                   	ret    
80102539:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80102540 <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
80102540:	f3 0f 1e fb          	endbr32 
80102544:	55                   	push   %ebp
  return namex(path, 1, name);
80102545:	ba 01 00 00 00       	mov    $0x1,%edx
{
8010254a:	89 e5                	mov    %esp,%ebp
  return namex(path, 1, name);
8010254c:	8b 4d 0c             	mov    0xc(%ebp),%ecx
8010254f:	8b 45 08             	mov    0x8(%ebp),%eax
}
80102552:	5d                   	pop    %ebp
  return namex(path, 1, name);
80102553:	e9 08 fd ff ff       	jmp    80102260 <namex>
80102558:	66 90                	xchg   %ax,%ax
8010255a:	66 90                	xchg   %ax,%ax
8010255c:	66 90                	xchg   %ax,%ax
8010255e:	66 90                	xchg   %ax,%ax

80102560 <idestart>:
}

// Start the request for b.  Caller must hold idelock.
static void
idestart(struct buf *b)
{
80102560:	55                   	push   %ebp
80102561:	89 e5                	mov    %esp,%ebp
80102563:	57                   	push   %edi
80102564:	56                   	push   %esi
80102565:	53                   	push   %ebx
80102566:	83 ec 0c             	sub    $0xc,%esp
  if(b == 0)
80102569:	85 c0                	test   %eax,%eax
8010256b:	0f 84 b4 00 00 00    	je     80102625 <idestart+0xc5>
    panic("idestart");
  if(b->blockno >= FSSIZE)
80102571:	8b 70 08             	mov    0x8(%eax),%esi
80102574:	89 c3                	mov    %eax,%ebx
80102576:	81 fe e7 03 00 00    	cmp    $0x3e7,%esi
8010257c:	0f 87 96 00 00 00    	ja     80102618 <idestart+0xb8>
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102582:	b9 f7 01 00 00       	mov    $0x1f7,%ecx
80102587:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010258e:	66 90                	xchg   %ax,%ax
80102590:	89 ca                	mov    %ecx,%edx
80102592:	ec                   	in     (%dx),%al
  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY)
80102593:	83 e0 c0             	and    $0xffffffc0,%eax
80102596:	3c 40                	cmp    $0x40,%al
80102598:	75 f6                	jne    80102590 <idestart+0x30>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
8010259a:	31 ff                	xor    %edi,%edi
8010259c:	ba f6 03 00 00       	mov    $0x3f6,%edx
801025a1:	89 f8                	mov    %edi,%eax
801025a3:	ee                   	out    %al,(%dx)
801025a4:	b8 01 00 00 00       	mov    $0x1,%eax
801025a9:	ba f2 01 00 00       	mov    $0x1f2,%edx
801025ae:	ee                   	out    %al,(%dx)
801025af:	ba f3 01 00 00       	mov    $0x1f3,%edx
801025b4:	89 f0                	mov    %esi,%eax
801025b6:	ee                   	out    %al,(%dx)

  idewait(0);
  outb(0x3f6, 0);  // generate interrupt
  outb(0x1f2, sector_per_block);  // number of sectors
  outb(0x1f3, sector & 0xff);
  outb(0x1f4, (sector >> 8) & 0xff);
801025b7:	89 f0                	mov    %esi,%eax
801025b9:	ba f4 01 00 00       	mov    $0x1f4,%edx
801025be:	c1 f8 08             	sar    $0x8,%eax
801025c1:	ee                   	out    %al,(%dx)
801025c2:	ba f5 01 00 00       	mov    $0x1f5,%edx
801025c7:	89 f8                	mov    %edi,%eax
801025c9:	ee                   	out    %al,(%dx)
  outb(0x1f5, (sector >> 16) & 0xff);
  outb(0x1f6, 0xe0 | ((b->dev&1)<<4) | ((sector>>24)&0x0f));
801025ca:	0f b6 43 04          	movzbl 0x4(%ebx),%eax
801025ce:	ba f6 01 00 00       	mov    $0x1f6,%edx
801025d3:	c1 e0 04             	shl    $0x4,%eax
801025d6:	83 e0 10             	and    $0x10,%eax
801025d9:	83 c8 e0             	or     $0xffffffe0,%eax
801025dc:	ee                   	out    %al,(%dx)
  if(b->flags & B_DIRTY){
801025dd:	f6 03 04             	testb  $0x4,(%ebx)
801025e0:	75 16                	jne    801025f8 <idestart+0x98>
801025e2:	b8 20 00 00 00       	mov    $0x20,%eax
801025e7:	89 ca                	mov    %ecx,%edx
801025e9:	ee                   	out    %al,(%dx)
    outb(0x1f7, write_cmd);
    outsl(0x1f0, b->data, BSIZE/4);
  } else {
    outb(0x1f7, read_cmd);
  }
}
801025ea:	8d 65 f4             	lea    -0xc(%ebp),%esp
801025ed:	5b                   	pop    %ebx
801025ee:	5e                   	pop    %esi
801025ef:	5f                   	pop    %edi
801025f0:	5d                   	pop    %ebp
801025f1:	c3                   	ret    
801025f2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801025f8:	b8 30 00 00 00       	mov    $0x30,%eax
801025fd:	89 ca                	mov    %ecx,%edx
801025ff:	ee                   	out    %al,(%dx)
  asm volatile("cld; rep outsl" :
80102600:	b9 80 00 00 00       	mov    $0x80,%ecx
    outsl(0x1f0, b->data, BSIZE/4);
80102605:	8d 73 5c             	lea    0x5c(%ebx),%esi
80102608:	ba f0 01 00 00       	mov    $0x1f0,%edx
8010260d:	fc                   	cld    
8010260e:	f3 6f                	rep outsl %ds:(%esi),(%dx)
}
80102610:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102613:	5b                   	pop    %ebx
80102614:	5e                   	pop    %esi
80102615:	5f                   	pop    %edi
80102616:	5d                   	pop    %ebp
80102617:	c3                   	ret    
    panic("incorrect blockno");
80102618:	83 ec 0c             	sub    $0xc,%esp
8010261b:	68 cc 8d 10 80       	push   $0x80108dcc
80102620:	e8 6b dd ff ff       	call   80100390 <panic>
    panic("idestart");
80102625:	83 ec 0c             	sub    $0xc,%esp
80102628:	68 c3 8d 10 80       	push   $0x80108dc3
8010262d:	e8 5e dd ff ff       	call   80100390 <panic>
80102632:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102639:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80102640 <ideinit>:
{
80102640:	f3 0f 1e fb          	endbr32 
80102644:	55                   	push   %ebp
80102645:	89 e5                	mov    %esp,%ebp
80102647:	83 ec 10             	sub    $0x10,%esp
  initlock(&idelock, "ide");
8010264a:	68 de 8d 10 80       	push   $0x80108dde
8010264f:	68 80 c5 10 80       	push   $0x8010c580
80102654:	e8 37 34 00 00       	call   80105a90 <initlock>
  ioapicenable(IRQ_IDE, ncpu - 1);
80102659:	58                   	pop    %eax
8010265a:	a1 00 53 11 80       	mov    0x80115300,%eax
8010265f:	5a                   	pop    %edx
80102660:	83 e8 01             	sub    $0x1,%eax
80102663:	50                   	push   %eax
80102664:	6a 0e                	push   $0xe
80102666:	e8 b5 02 00 00       	call   80102920 <ioapicenable>
  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY)
8010266b:	83 c4 10             	add    $0x10,%esp
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
8010266e:	ba f7 01 00 00       	mov    $0x1f7,%edx
80102673:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80102677:	90                   	nop
80102678:	ec                   	in     (%dx),%al
80102679:	83 e0 c0             	and    $0xffffffc0,%eax
8010267c:	3c 40                	cmp    $0x40,%al
8010267e:	75 f8                	jne    80102678 <ideinit+0x38>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102680:	b8 f0 ff ff ff       	mov    $0xfffffff0,%eax
80102685:	ba f6 01 00 00       	mov    $0x1f6,%edx
8010268a:	ee                   	out    %al,(%dx)
8010268b:	b9 e8 03 00 00       	mov    $0x3e8,%ecx
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102690:	ba f7 01 00 00       	mov    $0x1f7,%edx
80102695:	eb 0e                	jmp    801026a5 <ideinit+0x65>
80102697:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010269e:	66 90                	xchg   %ax,%ax
  for(i=0; i<1000; i++){
801026a0:	83 e9 01             	sub    $0x1,%ecx
801026a3:	74 0f                	je     801026b4 <ideinit+0x74>
801026a5:	ec                   	in     (%dx),%al
    if(inb(0x1f7) != 0){
801026a6:	84 c0                	test   %al,%al
801026a8:	74 f6                	je     801026a0 <ideinit+0x60>
      havedisk1 = 1;
801026aa:	c7 05 60 c5 10 80 01 	movl   $0x1,0x8010c560
801026b1:	00 00 00 
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801026b4:	b8 e0 ff ff ff       	mov    $0xffffffe0,%eax
801026b9:	ba f6 01 00 00       	mov    $0x1f6,%edx
801026be:	ee                   	out    %al,(%dx)
}
801026bf:	c9                   	leave  
801026c0:	c3                   	ret    
801026c1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801026c8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801026cf:	90                   	nop

801026d0 <ideintr>:

// Interrupt handler.
void
ideintr(void)
{
801026d0:	f3 0f 1e fb          	endbr32 
801026d4:	55                   	push   %ebp
801026d5:	89 e5                	mov    %esp,%ebp
801026d7:	57                   	push   %edi
801026d8:	56                   	push   %esi
801026d9:	53                   	push   %ebx
801026da:	83 ec 18             	sub    $0x18,%esp
  struct buf *b;

  // First queued buffer is the active request.
  acquire(&idelock);
801026dd:	68 80 c5 10 80       	push   $0x8010c580
801026e2:	e8 29 35 00 00       	call   80105c10 <acquire>

  if((b = idequeue) == 0){
801026e7:	8b 1d 64 c5 10 80    	mov    0x8010c564,%ebx
801026ed:	83 c4 10             	add    $0x10,%esp
801026f0:	85 db                	test   %ebx,%ebx
801026f2:	74 5f                	je     80102753 <ideintr+0x83>
    release(&idelock);
    return;
  }
  idequeue = b->qnext;
801026f4:	8b 43 58             	mov    0x58(%ebx),%eax
801026f7:	a3 64 c5 10 80       	mov    %eax,0x8010c564

  // Read data if needed.
  if(!(b->flags & B_DIRTY) && idewait(1) >= 0)
801026fc:	8b 33                	mov    (%ebx),%esi
801026fe:	f7 c6 04 00 00 00    	test   $0x4,%esi
80102704:	75 2b                	jne    80102731 <ideintr+0x61>
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102706:	ba f7 01 00 00       	mov    $0x1f7,%edx
8010270b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010270f:	90                   	nop
80102710:	ec                   	in     (%dx),%al
  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY)
80102711:	89 c1                	mov    %eax,%ecx
80102713:	83 e1 c0             	and    $0xffffffc0,%ecx
80102716:	80 f9 40             	cmp    $0x40,%cl
80102719:	75 f5                	jne    80102710 <ideintr+0x40>
  if(checkerr && (r & (IDE_DF|IDE_ERR)) != 0)
8010271b:	a8 21                	test   $0x21,%al
8010271d:	75 12                	jne    80102731 <ideintr+0x61>
    insl(0x1f0, b->data, BSIZE/4);
8010271f:	8d 7b 5c             	lea    0x5c(%ebx),%edi
  asm volatile("cld; rep insl" :
80102722:	b9 80 00 00 00       	mov    $0x80,%ecx
80102727:	ba f0 01 00 00       	mov    $0x1f0,%edx
8010272c:	fc                   	cld    
8010272d:	f3 6d                	rep insl (%dx),%es:(%edi)
8010272f:	8b 33                	mov    (%ebx),%esi

  // Wake process waiting for this buf.
  b->flags |= B_VALID;
  b->flags &= ~B_DIRTY;
80102731:	83 e6 fb             	and    $0xfffffffb,%esi
  wakeup(b);
80102734:	83 ec 0c             	sub    $0xc,%esp
  b->flags &= ~B_DIRTY;
80102737:	83 ce 02             	or     $0x2,%esi
8010273a:	89 33                	mov    %esi,(%ebx)
  wakeup(b);
8010273c:	53                   	push   %ebx
8010273d:	e8 1e 1f 00 00       	call   80104660 <wakeup>

  // Start disk on next buf in queue.
  if(idequeue != 0)
80102742:	a1 64 c5 10 80       	mov    0x8010c564,%eax
80102747:	83 c4 10             	add    $0x10,%esp
8010274a:	85 c0                	test   %eax,%eax
8010274c:	74 05                	je     80102753 <ideintr+0x83>
    idestart(idequeue);
8010274e:	e8 0d fe ff ff       	call   80102560 <idestart>
    release(&idelock);
80102753:	83 ec 0c             	sub    $0xc,%esp
80102756:	68 80 c5 10 80       	push   $0x8010c580
8010275b:	e8 70 35 00 00       	call   80105cd0 <release>

  release(&idelock);
}
80102760:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102763:	5b                   	pop    %ebx
80102764:	5e                   	pop    %esi
80102765:	5f                   	pop    %edi
80102766:	5d                   	pop    %ebp
80102767:	c3                   	ret    
80102768:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010276f:	90                   	nop

80102770 <iderw>:
// Sync buf with disk.
// If B_DIRTY is set, write buf to disk, clear B_DIRTY, set B_VALID.
// Else if B_VALID is not set, read buf from disk, set B_VALID.
void
iderw(struct buf *b)
{
80102770:	f3 0f 1e fb          	endbr32 
80102774:	55                   	push   %ebp
80102775:	89 e5                	mov    %esp,%ebp
80102777:	53                   	push   %ebx
80102778:	83 ec 10             	sub    $0x10,%esp
8010277b:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct buf **pp;

  if(!holdingsleep(&b->lock))
8010277e:	8d 43 0c             	lea    0xc(%ebx),%eax
80102781:	50                   	push   %eax
80102782:	e8 89 31 00 00       	call   80105910 <holdingsleep>
80102787:	83 c4 10             	add    $0x10,%esp
8010278a:	85 c0                	test   %eax,%eax
8010278c:	0f 84 cf 00 00 00    	je     80102861 <iderw+0xf1>
    panic("iderw: buf not locked");
  if((b->flags & (B_VALID|B_DIRTY)) == B_VALID)
80102792:	8b 03                	mov    (%ebx),%eax
80102794:	83 e0 06             	and    $0x6,%eax
80102797:	83 f8 02             	cmp    $0x2,%eax
8010279a:	0f 84 b4 00 00 00    	je     80102854 <iderw+0xe4>
    panic("iderw: nothing to do");
  if(b->dev != 0 && !havedisk1)
801027a0:	8b 53 04             	mov    0x4(%ebx),%edx
801027a3:	85 d2                	test   %edx,%edx
801027a5:	74 0d                	je     801027b4 <iderw+0x44>
801027a7:	a1 60 c5 10 80       	mov    0x8010c560,%eax
801027ac:	85 c0                	test   %eax,%eax
801027ae:	0f 84 93 00 00 00    	je     80102847 <iderw+0xd7>
    panic("iderw: ide disk 1 not present");

  acquire(&idelock);  //DOC:acquire-lock
801027b4:	83 ec 0c             	sub    $0xc,%esp
801027b7:	68 80 c5 10 80       	push   $0x8010c580
801027bc:	e8 4f 34 00 00       	call   80105c10 <acquire>

  // Append b to idequeue.
  b->qnext = 0;
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC:insert-queue
801027c1:	a1 64 c5 10 80       	mov    0x8010c564,%eax
  b->qnext = 0;
801027c6:	c7 43 58 00 00 00 00 	movl   $0x0,0x58(%ebx)
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC:insert-queue
801027cd:	83 c4 10             	add    $0x10,%esp
801027d0:	85 c0                	test   %eax,%eax
801027d2:	74 6c                	je     80102840 <iderw+0xd0>
801027d4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801027d8:	89 c2                	mov    %eax,%edx
801027da:	8b 40 58             	mov    0x58(%eax),%eax
801027dd:	85 c0                	test   %eax,%eax
801027df:	75 f7                	jne    801027d8 <iderw+0x68>
801027e1:	83 c2 58             	add    $0x58,%edx
    ;
  *pp = b;
801027e4:	89 1a                	mov    %ebx,(%edx)

  // Start disk if necessary.
  if(idequeue == b)
801027e6:	39 1d 64 c5 10 80    	cmp    %ebx,0x8010c564
801027ec:	74 42                	je     80102830 <iderw+0xc0>
    idestart(b);

  // Wait for request to finish.
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
801027ee:	8b 03                	mov    (%ebx),%eax
801027f0:	83 e0 06             	and    $0x6,%eax
801027f3:	83 f8 02             	cmp    $0x2,%eax
801027f6:	74 23                	je     8010281b <iderw+0xab>
801027f8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801027ff:	90                   	nop
    sleep(b, &idelock);
80102800:	83 ec 08             	sub    $0x8,%esp
80102803:	68 80 c5 10 80       	push   $0x8010c580
80102808:	53                   	push   %ebx
80102809:	e8 92 1c 00 00       	call   801044a0 <sleep>
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
8010280e:	8b 03                	mov    (%ebx),%eax
80102810:	83 c4 10             	add    $0x10,%esp
80102813:	83 e0 06             	and    $0x6,%eax
80102816:	83 f8 02             	cmp    $0x2,%eax
80102819:	75 e5                	jne    80102800 <iderw+0x90>
  }


  release(&idelock);
8010281b:	c7 45 08 80 c5 10 80 	movl   $0x8010c580,0x8(%ebp)
}
80102822:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80102825:	c9                   	leave  
  release(&idelock);
80102826:	e9 a5 34 00 00       	jmp    80105cd0 <release>
8010282b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010282f:	90                   	nop
    idestart(b);
80102830:	89 d8                	mov    %ebx,%eax
80102832:	e8 29 fd ff ff       	call   80102560 <idestart>
80102837:	eb b5                	jmp    801027ee <iderw+0x7e>
80102839:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC:insert-queue
80102840:	ba 64 c5 10 80       	mov    $0x8010c564,%edx
80102845:	eb 9d                	jmp    801027e4 <iderw+0x74>
    panic("iderw: ide disk 1 not present");
80102847:	83 ec 0c             	sub    $0xc,%esp
8010284a:	68 0d 8e 10 80       	push   $0x80108e0d
8010284f:	e8 3c db ff ff       	call   80100390 <panic>
    panic("iderw: nothing to do");
80102854:	83 ec 0c             	sub    $0xc,%esp
80102857:	68 f8 8d 10 80       	push   $0x80108df8
8010285c:	e8 2f db ff ff       	call   80100390 <panic>
    panic("iderw: buf not locked");
80102861:	83 ec 0c             	sub    $0xc,%esp
80102864:	68 e2 8d 10 80       	push   $0x80108de2
80102869:	e8 22 db ff ff       	call   80100390 <panic>
8010286e:	66 90                	xchg   %ax,%ax

80102870 <ioapicinit>:
  ioapic->data = data;
}

void
ioapicinit(void)
{
80102870:	f3 0f 1e fb          	endbr32 
80102874:	55                   	push   %ebp
  int i, id, maxintr;

  ioapic = (volatile struct ioapic*)IOAPIC;
80102875:	c7 05 34 4c 11 80 00 	movl   $0xfec00000,0x80114c34
8010287c:	00 c0 fe 
{
8010287f:	89 e5                	mov    %esp,%ebp
80102881:	56                   	push   %esi
80102882:	53                   	push   %ebx
  ioapic->reg = reg;
80102883:	c7 05 00 00 c0 fe 01 	movl   $0x1,0xfec00000
8010288a:	00 00 00 
  return ioapic->data;
8010288d:	8b 15 34 4c 11 80    	mov    0x80114c34,%edx
80102893:	8b 72 10             	mov    0x10(%edx),%esi
  ioapic->reg = reg;
80102896:	c7 02 00 00 00 00    	movl   $0x0,(%edx)
  return ioapic->data;
8010289c:	8b 0d 34 4c 11 80    	mov    0x80114c34,%ecx
  maxintr = (ioapicread(REG_VER) >> 16) & 0xFF;
  id = ioapicread(REG_ID) >> 24;
  if(id != ioapicid)
801028a2:	0f b6 15 60 4d 11 80 	movzbl 0x80114d60,%edx
  maxintr = (ioapicread(REG_VER) >> 16) & 0xFF;
801028a9:	c1 ee 10             	shr    $0x10,%esi
801028ac:	89 f0                	mov    %esi,%eax
801028ae:	0f b6 f0             	movzbl %al,%esi
  return ioapic->data;
801028b1:	8b 41 10             	mov    0x10(%ecx),%eax
  id = ioapicread(REG_ID) >> 24;
801028b4:	c1 e8 18             	shr    $0x18,%eax
  if(id != ioapicid)
801028b7:	39 c2                	cmp    %eax,%edx
801028b9:	74 16                	je     801028d1 <ioapicinit+0x61>
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");
801028bb:	83 ec 0c             	sub    $0xc,%esp
801028be:	68 2c 8e 10 80       	push   $0x80108e2c
801028c3:	e8 f8 df ff ff       	call   801008c0 <cprintf>
801028c8:	8b 0d 34 4c 11 80    	mov    0x80114c34,%ecx
801028ce:	83 c4 10             	add    $0x10,%esp
801028d1:	83 c6 21             	add    $0x21,%esi
{
801028d4:	ba 10 00 00 00       	mov    $0x10,%edx
801028d9:	b8 20 00 00 00       	mov    $0x20,%eax
801028de:	66 90                	xchg   %ax,%ax
  ioapic->reg = reg;
801028e0:	89 11                	mov    %edx,(%ecx)

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
801028e2:	89 c3                	mov    %eax,%ebx
  ioapic->data = data;
801028e4:	8b 0d 34 4c 11 80    	mov    0x80114c34,%ecx
801028ea:	83 c0 01             	add    $0x1,%eax
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
801028ed:	81 cb 00 00 01 00    	or     $0x10000,%ebx
  ioapic->data = data;
801028f3:	89 59 10             	mov    %ebx,0x10(%ecx)
  ioapic->reg = reg;
801028f6:	8d 5a 01             	lea    0x1(%edx),%ebx
801028f9:	83 c2 02             	add    $0x2,%edx
801028fc:	89 19                	mov    %ebx,(%ecx)
  ioapic->data = data;
801028fe:	8b 0d 34 4c 11 80    	mov    0x80114c34,%ecx
80102904:	c7 41 10 00 00 00 00 	movl   $0x0,0x10(%ecx)
  for(i = 0; i <= maxintr; i++){
8010290b:	39 f0                	cmp    %esi,%eax
8010290d:	75 d1                	jne    801028e0 <ioapicinit+0x70>
    ioapicwrite(REG_TABLE+2*i+1, 0);
  }
}
8010290f:	8d 65 f8             	lea    -0x8(%ebp),%esp
80102912:	5b                   	pop    %ebx
80102913:	5e                   	pop    %esi
80102914:	5d                   	pop    %ebp
80102915:	c3                   	ret    
80102916:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010291d:	8d 76 00             	lea    0x0(%esi),%esi

80102920 <ioapicenable>:

void
ioapicenable(int irq, int cpunum)
{
80102920:	f3 0f 1e fb          	endbr32 
80102924:	55                   	push   %ebp
  ioapic->reg = reg;
80102925:	8b 0d 34 4c 11 80    	mov    0x80114c34,%ecx
{
8010292b:	89 e5                	mov    %esp,%ebp
8010292d:	8b 45 08             	mov    0x8(%ebp),%eax
  // Mark interrupt edge-triggered, active high,
  // enabled, and routed to the given cpunum,
  // which happens to be that cpu's APIC ID.
  ioapicwrite(REG_TABLE+2*irq, T_IRQ0 + irq);
80102930:	8d 50 20             	lea    0x20(%eax),%edx
80102933:	8d 44 00 10          	lea    0x10(%eax,%eax,1),%eax
  ioapic->reg = reg;
80102937:	89 01                	mov    %eax,(%ecx)
  ioapic->data = data;
80102939:	8b 0d 34 4c 11 80    	mov    0x80114c34,%ecx
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
8010293f:	83 c0 01             	add    $0x1,%eax
  ioapic->data = data;
80102942:	89 51 10             	mov    %edx,0x10(%ecx)
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
80102945:	8b 55 0c             	mov    0xc(%ebp),%edx
  ioapic->reg = reg;
80102948:	89 01                	mov    %eax,(%ecx)
  ioapic->data = data;
8010294a:	a1 34 4c 11 80       	mov    0x80114c34,%eax
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
8010294f:	c1 e2 18             	shl    $0x18,%edx
  ioapic->data = data;
80102952:	89 50 10             	mov    %edx,0x10(%eax)
}
80102955:	5d                   	pop    %ebp
80102956:	c3                   	ret    
80102957:	66 90                	xchg   %ax,%ax
80102959:	66 90                	xchg   %ax,%ax
8010295b:	66 90                	xchg   %ax,%ax
8010295d:	66 90                	xchg   %ax,%ax
8010295f:	90                   	nop

80102960 <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(char *v)
{
80102960:	f3 0f 1e fb          	endbr32 
80102964:	55                   	push   %ebp
80102965:	89 e5                	mov    %esp,%ebp
80102967:	53                   	push   %ebx
80102968:	83 ec 04             	sub    $0x4,%esp
8010296b:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct run *r;

  if((uint)v % PGSIZE || v < end || V2P(v) >= PHYSTOP)
8010296e:	f7 c3 ff 0f 00 00    	test   $0xfff,%ebx
80102974:	75 7a                	jne    801029f0 <kfree+0x90>
80102976:	81 fb a8 86 11 80    	cmp    $0x801186a8,%ebx
8010297c:	72 72                	jb     801029f0 <kfree+0x90>
8010297e:	8d 83 00 00 00 80    	lea    -0x80000000(%ebx),%eax
80102984:	3d ff ff ff 0d       	cmp    $0xdffffff,%eax
80102989:	77 65                	ja     801029f0 <kfree+0x90>
    panic("kfree");

  // Fill with junk to catch dangling refs.
  memset(v, 1, PGSIZE);
8010298b:	83 ec 04             	sub    $0x4,%esp
8010298e:	68 00 10 00 00       	push   $0x1000
80102993:	6a 01                	push   $0x1
80102995:	53                   	push   %ebx
80102996:	e8 85 33 00 00       	call   80105d20 <memset>

  if(kmem.use_lock)
8010299b:	8b 15 74 4c 11 80    	mov    0x80114c74,%edx
801029a1:	83 c4 10             	add    $0x10,%esp
801029a4:	85 d2                	test   %edx,%edx
801029a6:	75 20                	jne    801029c8 <kfree+0x68>
    acquire(&kmem.lock);
  r = (struct run*)v;
  r->next = kmem.freelist;
801029a8:	a1 78 4c 11 80       	mov    0x80114c78,%eax
801029ad:	89 03                	mov    %eax,(%ebx)
  kmem.freelist = r;
  if(kmem.use_lock)
801029af:	a1 74 4c 11 80       	mov    0x80114c74,%eax
  kmem.freelist = r;
801029b4:	89 1d 78 4c 11 80    	mov    %ebx,0x80114c78
  if(kmem.use_lock)
801029ba:	85 c0                	test   %eax,%eax
801029bc:	75 22                	jne    801029e0 <kfree+0x80>
    release(&kmem.lock);
}
801029be:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801029c1:	c9                   	leave  
801029c2:	c3                   	ret    
801029c3:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801029c7:	90                   	nop
    acquire(&kmem.lock);
801029c8:	83 ec 0c             	sub    $0xc,%esp
801029cb:	68 40 4c 11 80       	push   $0x80114c40
801029d0:	e8 3b 32 00 00       	call   80105c10 <acquire>
801029d5:	83 c4 10             	add    $0x10,%esp
801029d8:	eb ce                	jmp    801029a8 <kfree+0x48>
801029da:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    release(&kmem.lock);
801029e0:	c7 45 08 40 4c 11 80 	movl   $0x80114c40,0x8(%ebp)
}
801029e7:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801029ea:	c9                   	leave  
    release(&kmem.lock);
801029eb:	e9 e0 32 00 00       	jmp    80105cd0 <release>
    panic("kfree");
801029f0:	83 ec 0c             	sub    $0xc,%esp
801029f3:	68 5e 8e 10 80       	push   $0x80108e5e
801029f8:	e8 93 d9 ff ff       	call   80100390 <panic>
801029fd:	8d 76 00             	lea    0x0(%esi),%esi

80102a00 <freerange>:
{
80102a00:	f3 0f 1e fb          	endbr32 
80102a04:	55                   	push   %ebp
80102a05:	89 e5                	mov    %esp,%ebp
80102a07:	56                   	push   %esi
  p = (char*)PGROUNDUP((uint)vstart);
80102a08:	8b 45 08             	mov    0x8(%ebp),%eax
{
80102a0b:	8b 75 0c             	mov    0xc(%ebp),%esi
80102a0e:	53                   	push   %ebx
  p = (char*)PGROUNDUP((uint)vstart);
80102a0f:	8d 98 ff 0f 00 00    	lea    0xfff(%eax),%ebx
80102a15:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a1b:	81 c3 00 10 00 00    	add    $0x1000,%ebx
80102a21:	39 de                	cmp    %ebx,%esi
80102a23:	72 1f                	jb     80102a44 <freerange+0x44>
80102a25:	8d 76 00             	lea    0x0(%esi),%esi
    kfree(p);
80102a28:	83 ec 0c             	sub    $0xc,%esp
80102a2b:	8d 83 00 f0 ff ff    	lea    -0x1000(%ebx),%eax
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a31:	81 c3 00 10 00 00    	add    $0x1000,%ebx
    kfree(p);
80102a37:	50                   	push   %eax
80102a38:	e8 23 ff ff ff       	call   80102960 <kfree>
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a3d:	83 c4 10             	add    $0x10,%esp
80102a40:	39 f3                	cmp    %esi,%ebx
80102a42:	76 e4                	jbe    80102a28 <freerange+0x28>
}
80102a44:	8d 65 f8             	lea    -0x8(%ebp),%esp
80102a47:	5b                   	pop    %ebx
80102a48:	5e                   	pop    %esi
80102a49:	5d                   	pop    %ebp
80102a4a:	c3                   	ret    
80102a4b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80102a4f:	90                   	nop

80102a50 <kinit1>:
{
80102a50:	f3 0f 1e fb          	endbr32 
80102a54:	55                   	push   %ebp
80102a55:	89 e5                	mov    %esp,%ebp
80102a57:	56                   	push   %esi
80102a58:	53                   	push   %ebx
80102a59:	8b 75 0c             	mov    0xc(%ebp),%esi
  initlock(&kmem.lock, "kmem");
80102a5c:	83 ec 08             	sub    $0x8,%esp
80102a5f:	68 64 8e 10 80       	push   $0x80108e64
80102a64:	68 40 4c 11 80       	push   $0x80114c40
80102a69:	e8 22 30 00 00       	call   80105a90 <initlock>
  p = (char*)PGROUNDUP((uint)vstart);
80102a6e:	8b 45 08             	mov    0x8(%ebp),%eax
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a71:	83 c4 10             	add    $0x10,%esp
  kmem.use_lock = 0;
80102a74:	c7 05 74 4c 11 80 00 	movl   $0x0,0x80114c74
80102a7b:	00 00 00 
  p = (char*)PGROUNDUP((uint)vstart);
80102a7e:	8d 98 ff 0f 00 00    	lea    0xfff(%eax),%ebx
80102a84:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a8a:	81 c3 00 10 00 00    	add    $0x1000,%ebx
80102a90:	39 de                	cmp    %ebx,%esi
80102a92:	72 20                	jb     80102ab4 <kinit1+0x64>
80102a94:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    kfree(p);
80102a98:	83 ec 0c             	sub    $0xc,%esp
80102a9b:	8d 83 00 f0 ff ff    	lea    -0x1000(%ebx),%eax
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102aa1:	81 c3 00 10 00 00    	add    $0x1000,%ebx
    kfree(p);
80102aa7:	50                   	push   %eax
80102aa8:	e8 b3 fe ff ff       	call   80102960 <kfree>
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102aad:	83 c4 10             	add    $0x10,%esp
80102ab0:	39 de                	cmp    %ebx,%esi
80102ab2:	73 e4                	jae    80102a98 <kinit1+0x48>
}
80102ab4:	8d 65 f8             	lea    -0x8(%ebp),%esp
80102ab7:	5b                   	pop    %ebx
80102ab8:	5e                   	pop    %esi
80102ab9:	5d                   	pop    %ebp
80102aba:	c3                   	ret    
80102abb:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80102abf:	90                   	nop

80102ac0 <kinit2>:
{
80102ac0:	f3 0f 1e fb          	endbr32 
80102ac4:	55                   	push   %ebp
80102ac5:	89 e5                	mov    %esp,%ebp
80102ac7:	56                   	push   %esi
  p = (char*)PGROUNDUP((uint)vstart);
80102ac8:	8b 45 08             	mov    0x8(%ebp),%eax
{
80102acb:	8b 75 0c             	mov    0xc(%ebp),%esi
80102ace:	53                   	push   %ebx
  p = (char*)PGROUNDUP((uint)vstart);
80102acf:	8d 98 ff 0f 00 00    	lea    0xfff(%eax),%ebx
80102ad5:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102adb:	81 c3 00 10 00 00    	add    $0x1000,%ebx
80102ae1:	39 de                	cmp    %ebx,%esi
80102ae3:	72 1f                	jb     80102b04 <kinit2+0x44>
80102ae5:	8d 76 00             	lea    0x0(%esi),%esi
    kfree(p);
80102ae8:	83 ec 0c             	sub    $0xc,%esp
80102aeb:	8d 83 00 f0 ff ff    	lea    -0x1000(%ebx),%eax
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102af1:	81 c3 00 10 00 00    	add    $0x1000,%ebx
    kfree(p);
80102af7:	50                   	push   %eax
80102af8:	e8 63 fe ff ff       	call   80102960 <kfree>
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102afd:	83 c4 10             	add    $0x10,%esp
80102b00:	39 de                	cmp    %ebx,%esi
80102b02:	73 e4                	jae    80102ae8 <kinit2+0x28>
  kmem.use_lock = 1;
80102b04:	c7 05 74 4c 11 80 01 	movl   $0x1,0x80114c74
80102b0b:	00 00 00 
}
80102b0e:	8d 65 f8             	lea    -0x8(%ebp),%esp
80102b11:	5b                   	pop    %ebx
80102b12:	5e                   	pop    %esi
80102b13:	5d                   	pop    %ebp
80102b14:	c3                   	ret    
80102b15:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102b1c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80102b20 <kalloc>:
// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
char*
kalloc(void)
{
80102b20:	f3 0f 1e fb          	endbr32 
  struct run *r;

  if(kmem.use_lock)
80102b24:	a1 74 4c 11 80       	mov    0x80114c74,%eax
80102b29:	85 c0                	test   %eax,%eax
80102b2b:	75 1b                	jne    80102b48 <kalloc+0x28>
    acquire(&kmem.lock);
  r = kmem.freelist;
80102b2d:	a1 78 4c 11 80       	mov    0x80114c78,%eax
  if(r)
80102b32:	85 c0                	test   %eax,%eax
80102b34:	74 0a                	je     80102b40 <kalloc+0x20>
    kmem.freelist = r->next;
80102b36:	8b 10                	mov    (%eax),%edx
80102b38:	89 15 78 4c 11 80    	mov    %edx,0x80114c78
  if(kmem.use_lock)
80102b3e:	c3                   	ret    
80102b3f:	90                   	nop
    release(&kmem.lock);
  return (char*)r;
}
80102b40:	c3                   	ret    
80102b41:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
{
80102b48:	55                   	push   %ebp
80102b49:	89 e5                	mov    %esp,%ebp
80102b4b:	83 ec 24             	sub    $0x24,%esp
    acquire(&kmem.lock);
80102b4e:	68 40 4c 11 80       	push   $0x80114c40
80102b53:	e8 b8 30 00 00       	call   80105c10 <acquire>
  r = kmem.freelist;
80102b58:	a1 78 4c 11 80       	mov    0x80114c78,%eax
  if(r)
80102b5d:	8b 15 74 4c 11 80    	mov    0x80114c74,%edx
80102b63:	83 c4 10             	add    $0x10,%esp
80102b66:	85 c0                	test   %eax,%eax
80102b68:	74 08                	je     80102b72 <kalloc+0x52>
    kmem.freelist = r->next;
80102b6a:	8b 08                	mov    (%eax),%ecx
80102b6c:	89 0d 78 4c 11 80    	mov    %ecx,0x80114c78
  if(kmem.use_lock)
80102b72:	85 d2                	test   %edx,%edx
80102b74:	74 16                	je     80102b8c <kalloc+0x6c>
    release(&kmem.lock);
80102b76:	83 ec 0c             	sub    $0xc,%esp
80102b79:	89 45 f4             	mov    %eax,-0xc(%ebp)
80102b7c:	68 40 4c 11 80       	push   $0x80114c40
80102b81:	e8 4a 31 00 00       	call   80105cd0 <release>
  return (char*)r;
80102b86:	8b 45 f4             	mov    -0xc(%ebp),%eax
    release(&kmem.lock);
80102b89:	83 c4 10             	add    $0x10,%esp
}
80102b8c:	c9                   	leave  
80102b8d:	c3                   	ret    
80102b8e:	66 90                	xchg   %ax,%ax

80102b90 <kbdgetc>:
#include "defs.h"
#include "kbd.h"

int
kbdgetc(void)
{
80102b90:	f3 0f 1e fb          	endbr32 
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102b94:	ba 64 00 00 00       	mov    $0x64,%edx
80102b99:	ec                   	in     (%dx),%al
    normalmap, shiftmap, ctlmap, ctlmap
  };
  uint st, data, c;

  st = inb(KBSTATP);
  if((st & KBS_DIB) == 0)
80102b9a:	a8 01                	test   $0x1,%al
80102b9c:	0f 84 be 00 00 00    	je     80102c60 <kbdgetc+0xd0>
{
80102ba2:	55                   	push   %ebp
80102ba3:	ba 60 00 00 00       	mov    $0x60,%edx
80102ba8:	89 e5                	mov    %esp,%ebp
80102baa:	53                   	push   %ebx
80102bab:	ec                   	in     (%dx),%al
  return data;
80102bac:	8b 1d b4 c5 10 80    	mov    0x8010c5b4,%ebx
    return -1;
  data = inb(KBDATAP);
80102bb2:	0f b6 d0             	movzbl %al,%edx

  if(data == 0xE0){
80102bb5:	3c e0                	cmp    $0xe0,%al
80102bb7:	74 57                	je     80102c10 <kbdgetc+0x80>
    shift |= E0ESC;
    return 0;
  } else if(data & 0x80){
80102bb9:	89 d9                	mov    %ebx,%ecx
80102bbb:	83 e1 40             	and    $0x40,%ecx
80102bbe:	84 c0                	test   %al,%al
80102bc0:	78 5e                	js     80102c20 <kbdgetc+0x90>
    // Key released
    data = (shift & E0ESC ? data : data & 0x7F);
    shift &= ~(shiftcode[data] | E0ESC);
    return 0;
  } else if(shift & E0ESC){
80102bc2:	85 c9                	test   %ecx,%ecx
80102bc4:	74 09                	je     80102bcf <kbdgetc+0x3f>
    // Last character was an E0 escape; or with 0x80
    data |= 0x80;
80102bc6:	83 c8 80             	or     $0xffffff80,%eax
    shift &= ~E0ESC;
80102bc9:	83 e3 bf             	and    $0xffffffbf,%ebx
    data |= 0x80;
80102bcc:	0f b6 d0             	movzbl %al,%edx
  }

  shift |= shiftcode[data];
80102bcf:	0f b6 8a a0 8f 10 80 	movzbl -0x7fef7060(%edx),%ecx
  shift ^= togglecode[data];
80102bd6:	0f b6 82 a0 8e 10 80 	movzbl -0x7fef7160(%edx),%eax
  shift |= shiftcode[data];
80102bdd:	09 d9                	or     %ebx,%ecx
  shift ^= togglecode[data];
80102bdf:	31 c1                	xor    %eax,%ecx
  c = charcode[shift & (CTL | SHIFT)][data];
80102be1:	89 c8                	mov    %ecx,%eax
  shift ^= togglecode[data];
80102be3:	89 0d b4 c5 10 80    	mov    %ecx,0x8010c5b4
  c = charcode[shift & (CTL | SHIFT)][data];
80102be9:	83 e0 03             	and    $0x3,%eax
  if(shift & CAPSLOCK){
80102bec:	83 e1 08             	and    $0x8,%ecx
  c = charcode[shift & (CTL | SHIFT)][data];
80102bef:	8b 04 85 80 8e 10 80 	mov    -0x7fef7180(,%eax,4),%eax
80102bf6:	0f b6 04 10          	movzbl (%eax,%edx,1),%eax
  if(shift & CAPSLOCK){
80102bfa:	74 0b                	je     80102c07 <kbdgetc+0x77>
    if('a' <= c && c <= 'z')
80102bfc:	8d 50 9f             	lea    -0x61(%eax),%edx
80102bff:	83 fa 19             	cmp    $0x19,%edx
80102c02:	77 44                	ja     80102c48 <kbdgetc+0xb8>
      c += 'A' - 'a';
80102c04:	83 e8 20             	sub    $0x20,%eax
    else if('A' <= c && c <= 'Z')
      c += 'a' - 'A';
  }
  return c;
}
80102c07:	5b                   	pop    %ebx
80102c08:	5d                   	pop    %ebp
80102c09:	c3                   	ret    
80102c0a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    shift |= E0ESC;
80102c10:	83 cb 40             	or     $0x40,%ebx
    return 0;
80102c13:	31 c0                	xor    %eax,%eax
    shift |= E0ESC;
80102c15:	89 1d b4 c5 10 80    	mov    %ebx,0x8010c5b4
}
80102c1b:	5b                   	pop    %ebx
80102c1c:	5d                   	pop    %ebp
80102c1d:	c3                   	ret    
80102c1e:	66 90                	xchg   %ax,%ax
    data = (shift & E0ESC ? data : data & 0x7F);
80102c20:	83 e0 7f             	and    $0x7f,%eax
80102c23:	85 c9                	test   %ecx,%ecx
80102c25:	0f 44 d0             	cmove  %eax,%edx
    return 0;
80102c28:	31 c0                	xor    %eax,%eax
    shift &= ~(shiftcode[data] | E0ESC);
80102c2a:	0f b6 8a a0 8f 10 80 	movzbl -0x7fef7060(%edx),%ecx
80102c31:	83 c9 40             	or     $0x40,%ecx
80102c34:	0f b6 c9             	movzbl %cl,%ecx
80102c37:	f7 d1                	not    %ecx
80102c39:	21 d9                	and    %ebx,%ecx
}
80102c3b:	5b                   	pop    %ebx
80102c3c:	5d                   	pop    %ebp
    shift &= ~(shiftcode[data] | E0ESC);
80102c3d:	89 0d b4 c5 10 80    	mov    %ecx,0x8010c5b4
}
80102c43:	c3                   	ret    
80102c44:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    else if('A' <= c && c <= 'Z')
80102c48:	8d 48 bf             	lea    -0x41(%eax),%ecx
      c += 'a' - 'A';
80102c4b:	8d 50 20             	lea    0x20(%eax),%edx
}
80102c4e:	5b                   	pop    %ebx
80102c4f:	5d                   	pop    %ebp
      c += 'a' - 'A';
80102c50:	83 f9 1a             	cmp    $0x1a,%ecx
80102c53:	0f 42 c2             	cmovb  %edx,%eax
}
80102c56:	c3                   	ret    
80102c57:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102c5e:	66 90                	xchg   %ax,%ax
    return -1;
80102c60:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80102c65:	c3                   	ret    
80102c66:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102c6d:	8d 76 00             	lea    0x0(%esi),%esi

80102c70 <kbdintr>:

void
kbdintr(void)
{
80102c70:	f3 0f 1e fb          	endbr32 
80102c74:	55                   	push   %ebp
80102c75:	89 e5                	mov    %esp,%ebp
80102c77:	83 ec 14             	sub    $0x14,%esp
  consoleintr(kbdgetc);
80102c7a:	68 90 2b 10 80       	push   $0x80102b90
80102c7f:	e8 ec dd ff ff       	call   80100a70 <consoleintr>
}
80102c84:	83 c4 10             	add    $0x10,%esp
80102c87:	c9                   	leave  
80102c88:	c3                   	ret    
80102c89:	66 90                	xchg   %ax,%ax
80102c8b:	66 90                	xchg   %ax,%ax
80102c8d:	66 90                	xchg   %ax,%ax
80102c8f:	90                   	nop

80102c90 <lapicinit>:
  lapic[ID];  // wait for write to finish, by reading
}

void
lapicinit(void)
{
80102c90:	f3 0f 1e fb          	endbr32 
  if(!lapic)
80102c94:	a1 7c 4c 11 80       	mov    0x80114c7c,%eax
80102c99:	85 c0                	test   %eax,%eax
80102c9b:	0f 84 c7 00 00 00    	je     80102d68 <lapicinit+0xd8>
  lapic[index] = value;
80102ca1:	c7 80 f0 00 00 00 3f 	movl   $0x13f,0xf0(%eax)
80102ca8:	01 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102cab:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102cae:	c7 80 e0 03 00 00 0b 	movl   $0xb,0x3e0(%eax)
80102cb5:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102cb8:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102cbb:	c7 80 20 03 00 00 20 	movl   $0x20020,0x320(%eax)
80102cc2:	00 02 00 
  lapic[ID];  // wait for write to finish, by reading
80102cc5:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102cc8:	c7 80 80 03 00 00 80 	movl   $0x989680,0x380(%eax)
80102ccf:	96 98 00 
  lapic[ID];  // wait for write to finish, by reading
80102cd2:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102cd5:	c7 80 50 03 00 00 00 	movl   $0x10000,0x350(%eax)
80102cdc:	00 01 00 
  lapic[ID];  // wait for write to finish, by reading
80102cdf:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102ce2:	c7 80 60 03 00 00 00 	movl   $0x10000,0x360(%eax)
80102ce9:	00 01 00 
  lapic[ID];  // wait for write to finish, by reading
80102cec:	8b 50 20             	mov    0x20(%eax),%edx
  lapicw(LINT0, MASKED);
  lapicw(LINT1, MASKED);

  // Disable performance counter overflow interrupts
  // on machines that provide that interrupt entry.
  if(((lapic[VER]>>16) & 0xFF) >= 4)
80102cef:	8b 50 30             	mov    0x30(%eax),%edx
80102cf2:	c1 ea 10             	shr    $0x10,%edx
80102cf5:	81 e2 fc 00 00 00    	and    $0xfc,%edx
80102cfb:	75 73                	jne    80102d70 <lapicinit+0xe0>
  lapic[index] = value;
80102cfd:	c7 80 70 03 00 00 33 	movl   $0x33,0x370(%eax)
80102d04:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102d07:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102d0a:	c7 80 80 02 00 00 00 	movl   $0x0,0x280(%eax)
80102d11:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102d14:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102d17:	c7 80 80 02 00 00 00 	movl   $0x0,0x280(%eax)
80102d1e:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102d21:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102d24:	c7 80 b0 00 00 00 00 	movl   $0x0,0xb0(%eax)
80102d2b:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102d2e:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102d31:	c7 80 10 03 00 00 00 	movl   $0x0,0x310(%eax)
80102d38:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102d3b:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102d3e:	c7 80 00 03 00 00 00 	movl   $0x88500,0x300(%eax)
80102d45:	85 08 00 
  lapic[ID];  // wait for write to finish, by reading
80102d48:	8b 50 20             	mov    0x20(%eax),%edx
80102d4b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80102d4f:	90                   	nop
  lapicw(EOI, 0);

  // Send an Init Level De-Assert to synchronise arbitration ID's.
  lapicw(ICRHI, 0);
  lapicw(ICRLO, BCAST | INIT | LEVEL);
  while(lapic[ICRLO] & DELIVS)
80102d50:	8b 90 00 03 00 00    	mov    0x300(%eax),%edx
80102d56:	80 e6 10             	and    $0x10,%dh
80102d59:	75 f5                	jne    80102d50 <lapicinit+0xc0>
  lapic[index] = value;
80102d5b:	c7 80 80 00 00 00 00 	movl   $0x0,0x80(%eax)
80102d62:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102d65:	8b 40 20             	mov    0x20(%eax),%eax
    ;

  // Enable interrupts on the APIC (but not on the processor).
  lapicw(TPR, 0);
}
80102d68:	c3                   	ret    
80102d69:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  lapic[index] = value;
80102d70:	c7 80 40 03 00 00 00 	movl   $0x10000,0x340(%eax)
80102d77:	00 01 00 
  lapic[ID];  // wait for write to finish, by reading
80102d7a:	8b 50 20             	mov    0x20(%eax),%edx
}
80102d7d:	e9 7b ff ff ff       	jmp    80102cfd <lapicinit+0x6d>
80102d82:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102d89:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80102d90 <lapicid>:

int
lapicid(void)
{
80102d90:	f3 0f 1e fb          	endbr32 
  if (!lapic)
80102d94:	a1 7c 4c 11 80       	mov    0x80114c7c,%eax
80102d99:	85 c0                	test   %eax,%eax
80102d9b:	74 0b                	je     80102da8 <lapicid+0x18>
    return 0;
  return lapic[ID] >> 24;
80102d9d:	8b 40 20             	mov    0x20(%eax),%eax
80102da0:	c1 e8 18             	shr    $0x18,%eax
80102da3:	c3                   	ret    
80102da4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    return 0;
80102da8:	31 c0                	xor    %eax,%eax
}
80102daa:	c3                   	ret    
80102dab:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80102daf:	90                   	nop

80102db0 <lapiceoi>:

// Acknowledge interrupt.
void
lapiceoi(void)
{
80102db0:	f3 0f 1e fb          	endbr32 
  if(lapic)
80102db4:	a1 7c 4c 11 80       	mov    0x80114c7c,%eax
80102db9:	85 c0                	test   %eax,%eax
80102dbb:	74 0d                	je     80102dca <lapiceoi+0x1a>
  lapic[index] = value;
80102dbd:	c7 80 b0 00 00 00 00 	movl   $0x0,0xb0(%eax)
80102dc4:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102dc7:	8b 40 20             	mov    0x20(%eax),%eax
    lapicw(EOI, 0);
}
80102dca:	c3                   	ret    
80102dcb:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80102dcf:	90                   	nop

80102dd0 <microdelay>:

// Spin for a given number of microseconds.
// On real hardware would want to tune this dynamically.
void
microdelay(int us)
{
80102dd0:	f3 0f 1e fb          	endbr32 
}
80102dd4:	c3                   	ret    
80102dd5:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102ddc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80102de0 <lapicstartap>:

// Start additional processor running entry code at addr.
// See Appendix B of MultiProcessor Specification.
void
lapicstartap(uchar apicid, uint addr)
{
80102de0:	f3 0f 1e fb          	endbr32 
80102de4:	55                   	push   %ebp
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102de5:	b8 0f 00 00 00       	mov    $0xf,%eax
80102dea:	ba 70 00 00 00       	mov    $0x70,%edx
80102def:	89 e5                	mov    %esp,%ebp
80102df1:	53                   	push   %ebx
80102df2:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80102df5:	8b 5d 08             	mov    0x8(%ebp),%ebx
80102df8:	ee                   	out    %al,(%dx)
80102df9:	b8 0a 00 00 00       	mov    $0xa,%eax
80102dfe:	ba 71 00 00 00       	mov    $0x71,%edx
80102e03:	ee                   	out    %al,(%dx)
  // and the warm reset vector (DWORD based at 40:67) to point at
  // the AP startup code prior to the [universal startup algorithm]."
  outb(CMOS_PORT, 0xF);  // offset 0xF is shutdown code
  outb(CMOS_PORT+1, 0x0A);
  wrv = (ushort*)P2V((0x40<<4 | 0x67));  // Warm reset vector
  wrv[0] = 0;
80102e04:	31 c0                	xor    %eax,%eax
  wrv[1] = addr >> 4;

  // "Universal startup algorithm."
  // Send INIT (level-triggered) interrupt to reset other CPU.
  lapicw(ICRHI, apicid<<24);
80102e06:	c1 e3 18             	shl    $0x18,%ebx
  wrv[0] = 0;
80102e09:	66 a3 67 04 00 80    	mov    %ax,0x80000467
  wrv[1] = addr >> 4;
80102e0f:	89 c8                	mov    %ecx,%eax
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
    lapicw(ICRHI, apicid<<24);
    lapicw(ICRLO, STARTUP | (addr>>12));
80102e11:	c1 e9 0c             	shr    $0xc,%ecx
  lapicw(ICRHI, apicid<<24);
80102e14:	89 da                	mov    %ebx,%edx
  wrv[1] = addr >> 4;
80102e16:	c1 e8 04             	shr    $0x4,%eax
    lapicw(ICRLO, STARTUP | (addr>>12));
80102e19:	80 cd 06             	or     $0x6,%ch
  wrv[1] = addr >> 4;
80102e1c:	66 a3 69 04 00 80    	mov    %ax,0x80000469
  lapic[index] = value;
80102e22:	a1 7c 4c 11 80       	mov    0x80114c7c,%eax
80102e27:	89 98 10 03 00 00    	mov    %ebx,0x310(%eax)
  lapic[ID];  // wait for write to finish, by reading
80102e2d:	8b 58 20             	mov    0x20(%eax),%ebx
  lapic[index] = value;
80102e30:	c7 80 00 03 00 00 00 	movl   $0xc500,0x300(%eax)
80102e37:	c5 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102e3a:	8b 58 20             	mov    0x20(%eax),%ebx
  lapic[index] = value;
80102e3d:	c7 80 00 03 00 00 00 	movl   $0x8500,0x300(%eax)
80102e44:	85 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102e47:	8b 58 20             	mov    0x20(%eax),%ebx
  lapic[index] = value;
80102e4a:	89 90 10 03 00 00    	mov    %edx,0x310(%eax)
  lapic[ID];  // wait for write to finish, by reading
80102e50:	8b 58 20             	mov    0x20(%eax),%ebx
  lapic[index] = value;
80102e53:	89 88 00 03 00 00    	mov    %ecx,0x300(%eax)
  lapic[ID];  // wait for write to finish, by reading
80102e59:	8b 58 20             	mov    0x20(%eax),%ebx
  lapic[index] = value;
80102e5c:	89 90 10 03 00 00    	mov    %edx,0x310(%eax)
  lapic[ID];  // wait for write to finish, by reading
80102e62:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102e65:	89 88 00 03 00 00    	mov    %ecx,0x300(%eax)
    microdelay(200);
  }
}
80102e6b:	5b                   	pop    %ebx
  lapic[ID];  // wait for write to finish, by reading
80102e6c:	8b 40 20             	mov    0x20(%eax),%eax
}
80102e6f:	5d                   	pop    %ebp
80102e70:	c3                   	ret    
80102e71:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102e78:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102e7f:	90                   	nop

80102e80 <cmostime>:
}

// qemu seems to use 24-hour GWT and the values are BCD encoded
void
cmostime(struct rtcdate *r)
{
80102e80:	f3 0f 1e fb          	endbr32 
80102e84:	55                   	push   %ebp
80102e85:	b8 0b 00 00 00       	mov    $0xb,%eax
80102e8a:	ba 70 00 00 00       	mov    $0x70,%edx
80102e8f:	89 e5                	mov    %esp,%ebp
80102e91:	57                   	push   %edi
80102e92:	56                   	push   %esi
80102e93:	53                   	push   %ebx
80102e94:	83 ec 4c             	sub    $0x4c,%esp
80102e97:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102e98:	ba 71 00 00 00       	mov    $0x71,%edx
80102e9d:	ec                   	in     (%dx),%al
  struct rtcdate t1, t2;
  int sb, bcd;

  sb = cmos_read(CMOS_STATB);

  bcd = (sb & (1 << 2)) == 0;
80102e9e:	83 e0 04             	and    $0x4,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102ea1:	bb 70 00 00 00       	mov    $0x70,%ebx
80102ea6:	88 45 b3             	mov    %al,-0x4d(%ebp)
80102ea9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102eb0:	31 c0                	xor    %eax,%eax
80102eb2:	89 da                	mov    %ebx,%edx
80102eb4:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102eb5:	b9 71 00 00 00       	mov    $0x71,%ecx
80102eba:	89 ca                	mov    %ecx,%edx
80102ebc:	ec                   	in     (%dx),%al
80102ebd:	88 45 b7             	mov    %al,-0x49(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102ec0:	89 da                	mov    %ebx,%edx
80102ec2:	b8 02 00 00 00       	mov    $0x2,%eax
80102ec7:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102ec8:	89 ca                	mov    %ecx,%edx
80102eca:	ec                   	in     (%dx),%al
80102ecb:	88 45 b6             	mov    %al,-0x4a(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102ece:	89 da                	mov    %ebx,%edx
80102ed0:	b8 04 00 00 00       	mov    $0x4,%eax
80102ed5:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102ed6:	89 ca                	mov    %ecx,%edx
80102ed8:	ec                   	in     (%dx),%al
80102ed9:	88 45 b5             	mov    %al,-0x4b(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102edc:	89 da                	mov    %ebx,%edx
80102ede:	b8 07 00 00 00       	mov    $0x7,%eax
80102ee3:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102ee4:	89 ca                	mov    %ecx,%edx
80102ee6:	ec                   	in     (%dx),%al
80102ee7:	88 45 b4             	mov    %al,-0x4c(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102eea:	89 da                	mov    %ebx,%edx
80102eec:	b8 08 00 00 00       	mov    $0x8,%eax
80102ef1:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102ef2:	89 ca                	mov    %ecx,%edx
80102ef4:	ec                   	in     (%dx),%al
80102ef5:	89 c7                	mov    %eax,%edi
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102ef7:	89 da                	mov    %ebx,%edx
80102ef9:	b8 09 00 00 00       	mov    $0x9,%eax
80102efe:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102eff:	89 ca                	mov    %ecx,%edx
80102f01:	ec                   	in     (%dx),%al
80102f02:	89 c6                	mov    %eax,%esi
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102f04:	89 da                	mov    %ebx,%edx
80102f06:	b8 0a 00 00 00       	mov    $0xa,%eax
80102f0b:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102f0c:	89 ca                	mov    %ecx,%edx
80102f0e:	ec                   	in     (%dx),%al

  // make sure CMOS doesn't modify time while we read it
  for(;;) {
    fill_rtcdate(&t1);
    if(cmos_read(CMOS_STATA) & CMOS_UIP)
80102f0f:	84 c0                	test   %al,%al
80102f11:	78 9d                	js     80102eb0 <cmostime+0x30>
  return inb(CMOS_RETURN);
80102f13:	0f b6 45 b7          	movzbl -0x49(%ebp),%eax
80102f17:	89 fa                	mov    %edi,%edx
80102f19:	0f b6 fa             	movzbl %dl,%edi
80102f1c:	89 f2                	mov    %esi,%edx
80102f1e:	89 45 b8             	mov    %eax,-0x48(%ebp)
80102f21:	0f b6 45 b6          	movzbl -0x4a(%ebp),%eax
80102f25:	0f b6 f2             	movzbl %dl,%esi
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102f28:	89 da                	mov    %ebx,%edx
80102f2a:	89 7d c8             	mov    %edi,-0x38(%ebp)
80102f2d:	89 45 bc             	mov    %eax,-0x44(%ebp)
80102f30:	0f b6 45 b5          	movzbl -0x4b(%ebp),%eax
80102f34:	89 75 cc             	mov    %esi,-0x34(%ebp)
80102f37:	89 45 c0             	mov    %eax,-0x40(%ebp)
80102f3a:	0f b6 45 b4          	movzbl -0x4c(%ebp),%eax
80102f3e:	89 45 c4             	mov    %eax,-0x3c(%ebp)
80102f41:	31 c0                	xor    %eax,%eax
80102f43:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102f44:	89 ca                	mov    %ecx,%edx
80102f46:	ec                   	in     (%dx),%al
80102f47:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102f4a:	89 da                	mov    %ebx,%edx
80102f4c:	89 45 d0             	mov    %eax,-0x30(%ebp)
80102f4f:	b8 02 00 00 00       	mov    $0x2,%eax
80102f54:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102f55:	89 ca                	mov    %ecx,%edx
80102f57:	ec                   	in     (%dx),%al
80102f58:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102f5b:	89 da                	mov    %ebx,%edx
80102f5d:	89 45 d4             	mov    %eax,-0x2c(%ebp)
80102f60:	b8 04 00 00 00       	mov    $0x4,%eax
80102f65:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102f66:	89 ca                	mov    %ecx,%edx
80102f68:	ec                   	in     (%dx),%al
80102f69:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102f6c:	89 da                	mov    %ebx,%edx
80102f6e:	89 45 d8             	mov    %eax,-0x28(%ebp)
80102f71:	b8 07 00 00 00       	mov    $0x7,%eax
80102f76:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102f77:	89 ca                	mov    %ecx,%edx
80102f79:	ec                   	in     (%dx),%al
80102f7a:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102f7d:	89 da                	mov    %ebx,%edx
80102f7f:	89 45 dc             	mov    %eax,-0x24(%ebp)
80102f82:	b8 08 00 00 00       	mov    $0x8,%eax
80102f87:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102f88:	89 ca                	mov    %ecx,%edx
80102f8a:	ec                   	in     (%dx),%al
80102f8b:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102f8e:	89 da                	mov    %ebx,%edx
80102f90:	89 45 e0             	mov    %eax,-0x20(%ebp)
80102f93:	b8 09 00 00 00       	mov    $0x9,%eax
80102f98:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102f99:	89 ca                	mov    %ecx,%edx
80102f9b:	ec                   	in     (%dx),%al
80102f9c:	0f b6 c0             	movzbl %al,%eax
        continue;
    fill_rtcdate(&t2);
    if(memcmp(&t1, &t2, sizeof(t1)) == 0)
80102f9f:	83 ec 04             	sub    $0x4,%esp
  return inb(CMOS_RETURN);
80102fa2:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if(memcmp(&t1, &t2, sizeof(t1)) == 0)
80102fa5:	8d 45 d0             	lea    -0x30(%ebp),%eax
80102fa8:	6a 18                	push   $0x18
80102faa:	50                   	push   %eax
80102fab:	8d 45 b8             	lea    -0x48(%ebp),%eax
80102fae:	50                   	push   %eax
80102faf:	e8 bc 2d 00 00       	call   80105d70 <memcmp>
80102fb4:	83 c4 10             	add    $0x10,%esp
80102fb7:	85 c0                	test   %eax,%eax
80102fb9:	0f 85 f1 fe ff ff    	jne    80102eb0 <cmostime+0x30>
      break;
  }

  // convert
  if(bcd) {
80102fbf:	80 7d b3 00          	cmpb   $0x0,-0x4d(%ebp)
80102fc3:	75 78                	jne    8010303d <cmostime+0x1bd>
#define    CONV(x)     (t1.x = ((t1.x >> 4) * 10) + (t1.x & 0xf))
    CONV(second);
80102fc5:	8b 45 b8             	mov    -0x48(%ebp),%eax
80102fc8:	89 c2                	mov    %eax,%edx
80102fca:	83 e0 0f             	and    $0xf,%eax
80102fcd:	c1 ea 04             	shr    $0x4,%edx
80102fd0:	8d 14 92             	lea    (%edx,%edx,4),%edx
80102fd3:	8d 04 50             	lea    (%eax,%edx,2),%eax
80102fd6:	89 45 b8             	mov    %eax,-0x48(%ebp)
    CONV(minute);
80102fd9:	8b 45 bc             	mov    -0x44(%ebp),%eax
80102fdc:	89 c2                	mov    %eax,%edx
80102fde:	83 e0 0f             	and    $0xf,%eax
80102fe1:	c1 ea 04             	shr    $0x4,%edx
80102fe4:	8d 14 92             	lea    (%edx,%edx,4),%edx
80102fe7:	8d 04 50             	lea    (%eax,%edx,2),%eax
80102fea:	89 45 bc             	mov    %eax,-0x44(%ebp)
    CONV(hour  );
80102fed:	8b 45 c0             	mov    -0x40(%ebp),%eax
80102ff0:	89 c2                	mov    %eax,%edx
80102ff2:	83 e0 0f             	and    $0xf,%eax
80102ff5:	c1 ea 04             	shr    $0x4,%edx
80102ff8:	8d 14 92             	lea    (%edx,%edx,4),%edx
80102ffb:	8d 04 50             	lea    (%eax,%edx,2),%eax
80102ffe:	89 45 c0             	mov    %eax,-0x40(%ebp)
    CONV(day   );
80103001:	8b 45 c4             	mov    -0x3c(%ebp),%eax
80103004:	89 c2                	mov    %eax,%edx
80103006:	83 e0 0f             	and    $0xf,%eax
80103009:	c1 ea 04             	shr    $0x4,%edx
8010300c:	8d 14 92             	lea    (%edx,%edx,4),%edx
8010300f:	8d 04 50             	lea    (%eax,%edx,2),%eax
80103012:	89 45 c4             	mov    %eax,-0x3c(%ebp)
    CONV(month );
80103015:	8b 45 c8             	mov    -0x38(%ebp),%eax
80103018:	89 c2                	mov    %eax,%edx
8010301a:	83 e0 0f             	and    $0xf,%eax
8010301d:	c1 ea 04             	shr    $0x4,%edx
80103020:	8d 14 92             	lea    (%edx,%edx,4),%edx
80103023:	8d 04 50             	lea    (%eax,%edx,2),%eax
80103026:	89 45 c8             	mov    %eax,-0x38(%ebp)
    CONV(year  );
80103029:	8b 45 cc             	mov    -0x34(%ebp),%eax
8010302c:	89 c2                	mov    %eax,%edx
8010302e:	83 e0 0f             	and    $0xf,%eax
80103031:	c1 ea 04             	shr    $0x4,%edx
80103034:	8d 14 92             	lea    (%edx,%edx,4),%edx
80103037:	8d 04 50             	lea    (%eax,%edx,2),%eax
8010303a:	89 45 cc             	mov    %eax,-0x34(%ebp)
#undef     CONV
  }

  *r = t1;
8010303d:	8b 75 08             	mov    0x8(%ebp),%esi
80103040:	8b 45 b8             	mov    -0x48(%ebp),%eax
80103043:	89 06                	mov    %eax,(%esi)
80103045:	8b 45 bc             	mov    -0x44(%ebp),%eax
80103048:	89 46 04             	mov    %eax,0x4(%esi)
8010304b:	8b 45 c0             	mov    -0x40(%ebp),%eax
8010304e:	89 46 08             	mov    %eax,0x8(%esi)
80103051:	8b 45 c4             	mov    -0x3c(%ebp),%eax
80103054:	89 46 0c             	mov    %eax,0xc(%esi)
80103057:	8b 45 c8             	mov    -0x38(%ebp),%eax
8010305a:	89 46 10             	mov    %eax,0x10(%esi)
8010305d:	8b 45 cc             	mov    -0x34(%ebp),%eax
80103060:	89 46 14             	mov    %eax,0x14(%esi)
  r->year += 2000;
80103063:	81 46 14 d0 07 00 00 	addl   $0x7d0,0x14(%esi)
}
8010306a:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010306d:	5b                   	pop    %ebx
8010306e:	5e                   	pop    %esi
8010306f:	5f                   	pop    %edi
80103070:	5d                   	pop    %ebp
80103071:	c3                   	ret    
80103072:	66 90                	xchg   %ax,%ax
80103074:	66 90                	xchg   %ax,%ax
80103076:	66 90                	xchg   %ax,%ax
80103078:	66 90                	xchg   %ax,%ax
8010307a:	66 90                	xchg   %ax,%ax
8010307c:	66 90                	xchg   %ax,%ax
8010307e:	66 90                	xchg   %ax,%ax

80103080 <install_trans>:
static void
install_trans(void)
{
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
80103080:	8b 0d c8 4c 11 80    	mov    0x80114cc8,%ecx
80103086:	85 c9                	test   %ecx,%ecx
80103088:	0f 8e 8a 00 00 00    	jle    80103118 <install_trans+0x98>
{
8010308e:	55                   	push   %ebp
8010308f:	89 e5                	mov    %esp,%ebp
80103091:	57                   	push   %edi
  for (tail = 0; tail < log.lh.n; tail++) {
80103092:	31 ff                	xor    %edi,%edi
{
80103094:	56                   	push   %esi
80103095:	53                   	push   %ebx
80103096:	83 ec 0c             	sub    $0xc,%esp
80103099:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
801030a0:	a1 b4 4c 11 80       	mov    0x80114cb4,%eax
801030a5:	83 ec 08             	sub    $0x8,%esp
801030a8:	01 f8                	add    %edi,%eax
801030aa:	83 c0 01             	add    $0x1,%eax
801030ad:	50                   	push   %eax
801030ae:	ff 35 c4 4c 11 80    	pushl  0x80114cc4
801030b4:	e8 17 d0 ff ff       	call   801000d0 <bread>
801030b9:	89 c6                	mov    %eax,%esi
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
801030bb:	58                   	pop    %eax
801030bc:	5a                   	pop    %edx
801030bd:	ff 34 bd cc 4c 11 80 	pushl  -0x7feeb334(,%edi,4)
801030c4:	ff 35 c4 4c 11 80    	pushl  0x80114cc4
  for (tail = 0; tail < log.lh.n; tail++) {
801030ca:	83 c7 01             	add    $0x1,%edi
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
801030cd:	e8 fe cf ff ff       	call   801000d0 <bread>
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
801030d2:	83 c4 0c             	add    $0xc,%esp
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
801030d5:	89 c3                	mov    %eax,%ebx
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
801030d7:	8d 46 5c             	lea    0x5c(%esi),%eax
801030da:	68 00 02 00 00       	push   $0x200
801030df:	50                   	push   %eax
801030e0:	8d 43 5c             	lea    0x5c(%ebx),%eax
801030e3:	50                   	push   %eax
801030e4:	e8 d7 2c 00 00       	call   80105dc0 <memmove>
    bwrite(dbuf);  // write dst to disk
801030e9:	89 1c 24             	mov    %ebx,(%esp)
801030ec:	e8 bf d0 ff ff       	call   801001b0 <bwrite>
    brelse(lbuf);
801030f1:	89 34 24             	mov    %esi,(%esp)
801030f4:	e8 f7 d0 ff ff       	call   801001f0 <brelse>
    brelse(dbuf);
801030f9:	89 1c 24             	mov    %ebx,(%esp)
801030fc:	e8 ef d0 ff ff       	call   801001f0 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
80103101:	83 c4 10             	add    $0x10,%esp
80103104:	39 3d c8 4c 11 80    	cmp    %edi,0x80114cc8
8010310a:	7f 94                	jg     801030a0 <install_trans+0x20>
  }
}
8010310c:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010310f:	5b                   	pop    %ebx
80103110:	5e                   	pop    %esi
80103111:	5f                   	pop    %edi
80103112:	5d                   	pop    %ebp
80103113:	c3                   	ret    
80103114:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80103118:	c3                   	ret    
80103119:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80103120 <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
80103120:	55                   	push   %ebp
80103121:	89 e5                	mov    %esp,%ebp
80103123:	53                   	push   %ebx
80103124:	83 ec 0c             	sub    $0xc,%esp
  struct buf *buf = bread(log.dev, log.start);
80103127:	ff 35 b4 4c 11 80    	pushl  0x80114cb4
8010312d:	ff 35 c4 4c 11 80    	pushl  0x80114cc4
80103133:	e8 98 cf ff ff       	call   801000d0 <bread>
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
  for (i = 0; i < log.lh.n; i++) {
80103138:	83 c4 10             	add    $0x10,%esp
  struct buf *buf = bread(log.dev, log.start);
8010313b:	89 c3                	mov    %eax,%ebx
  hb->n = log.lh.n;
8010313d:	a1 c8 4c 11 80       	mov    0x80114cc8,%eax
80103142:	89 43 5c             	mov    %eax,0x5c(%ebx)
  for (i = 0; i < log.lh.n; i++) {
80103145:	85 c0                	test   %eax,%eax
80103147:	7e 19                	jle    80103162 <write_head+0x42>
80103149:	31 d2                	xor    %edx,%edx
8010314b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010314f:	90                   	nop
    hb->block[i] = log.lh.block[i];
80103150:	8b 0c 95 cc 4c 11 80 	mov    -0x7feeb334(,%edx,4),%ecx
80103157:	89 4c 93 60          	mov    %ecx,0x60(%ebx,%edx,4)
  for (i = 0; i < log.lh.n; i++) {
8010315b:	83 c2 01             	add    $0x1,%edx
8010315e:	39 d0                	cmp    %edx,%eax
80103160:	75 ee                	jne    80103150 <write_head+0x30>
  }
  bwrite(buf);
80103162:	83 ec 0c             	sub    $0xc,%esp
80103165:	53                   	push   %ebx
80103166:	e8 45 d0 ff ff       	call   801001b0 <bwrite>
  brelse(buf);
8010316b:	89 1c 24             	mov    %ebx,(%esp)
8010316e:	e8 7d d0 ff ff       	call   801001f0 <brelse>
}
80103173:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80103176:	83 c4 10             	add    $0x10,%esp
80103179:	c9                   	leave  
8010317a:	c3                   	ret    
8010317b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010317f:	90                   	nop

80103180 <initlog>:
{
80103180:	f3 0f 1e fb          	endbr32 
80103184:	55                   	push   %ebp
80103185:	89 e5                	mov    %esp,%ebp
80103187:	53                   	push   %ebx
80103188:	83 ec 2c             	sub    $0x2c,%esp
8010318b:	8b 5d 08             	mov    0x8(%ebp),%ebx
  initlock(&log.lock, "log");
8010318e:	68 a0 90 10 80       	push   $0x801090a0
80103193:	68 80 4c 11 80       	push   $0x80114c80
80103198:	e8 f3 28 00 00       	call   80105a90 <initlock>
  readsb(dev, &sb);
8010319d:	58                   	pop    %eax
8010319e:	8d 45 dc             	lea    -0x24(%ebp),%eax
801031a1:	5a                   	pop    %edx
801031a2:	50                   	push   %eax
801031a3:	53                   	push   %ebx
801031a4:	e8 47 e8 ff ff       	call   801019f0 <readsb>
  log.start = sb.logstart;
801031a9:	8b 45 ec             	mov    -0x14(%ebp),%eax
  struct buf *buf = bread(log.dev, log.start);
801031ac:	59                   	pop    %ecx
  log.dev = dev;
801031ad:	89 1d c4 4c 11 80    	mov    %ebx,0x80114cc4
  log.size = sb.nlog;
801031b3:	8b 55 e8             	mov    -0x18(%ebp),%edx
  log.start = sb.logstart;
801031b6:	a3 b4 4c 11 80       	mov    %eax,0x80114cb4
  log.size = sb.nlog;
801031bb:	89 15 b8 4c 11 80    	mov    %edx,0x80114cb8
  struct buf *buf = bread(log.dev, log.start);
801031c1:	5a                   	pop    %edx
801031c2:	50                   	push   %eax
801031c3:	53                   	push   %ebx
801031c4:	e8 07 cf ff ff       	call   801000d0 <bread>
  for (i = 0; i < log.lh.n; i++) {
801031c9:	83 c4 10             	add    $0x10,%esp
  log.lh.n = lh->n;
801031cc:	8b 48 5c             	mov    0x5c(%eax),%ecx
801031cf:	89 0d c8 4c 11 80    	mov    %ecx,0x80114cc8
  for (i = 0; i < log.lh.n; i++) {
801031d5:	85 c9                	test   %ecx,%ecx
801031d7:	7e 19                	jle    801031f2 <initlog+0x72>
801031d9:	31 d2                	xor    %edx,%edx
801031db:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801031df:	90                   	nop
    log.lh.block[i] = lh->block[i];
801031e0:	8b 5c 90 60          	mov    0x60(%eax,%edx,4),%ebx
801031e4:	89 1c 95 cc 4c 11 80 	mov    %ebx,-0x7feeb334(,%edx,4)
  for (i = 0; i < log.lh.n; i++) {
801031eb:	83 c2 01             	add    $0x1,%edx
801031ee:	39 d1                	cmp    %edx,%ecx
801031f0:	75 ee                	jne    801031e0 <initlog+0x60>
  brelse(buf);
801031f2:	83 ec 0c             	sub    $0xc,%esp
801031f5:	50                   	push   %eax
801031f6:	e8 f5 cf ff ff       	call   801001f0 <brelse>

static void
recover_from_log(void)
{
  read_head();
  install_trans(); // if committed, copy from log to disk
801031fb:	e8 80 fe ff ff       	call   80103080 <install_trans>
  log.lh.n = 0;
80103200:	c7 05 c8 4c 11 80 00 	movl   $0x0,0x80114cc8
80103207:	00 00 00 
  write_head(); // clear the log
8010320a:	e8 11 ff ff ff       	call   80103120 <write_head>
}
8010320f:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80103212:	83 c4 10             	add    $0x10,%esp
80103215:	c9                   	leave  
80103216:	c3                   	ret    
80103217:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010321e:	66 90                	xchg   %ax,%ax

80103220 <begin_op>:
}

// called at the start of each FS system call.
void
begin_op(void)
{
80103220:	f3 0f 1e fb          	endbr32 
80103224:	55                   	push   %ebp
80103225:	89 e5                	mov    %esp,%ebp
80103227:	83 ec 14             	sub    $0x14,%esp
  acquire(&log.lock);
8010322a:	68 80 4c 11 80       	push   $0x80114c80
8010322f:	e8 dc 29 00 00       	call   80105c10 <acquire>
80103234:	83 c4 10             	add    $0x10,%esp
80103237:	eb 1c                	jmp    80103255 <begin_op+0x35>
80103239:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  while(1){
    if(log.committing){
      sleep(&log, &log.lock);
80103240:	83 ec 08             	sub    $0x8,%esp
80103243:	68 80 4c 11 80       	push   $0x80114c80
80103248:	68 80 4c 11 80       	push   $0x80114c80
8010324d:	e8 4e 12 00 00       	call   801044a0 <sleep>
80103252:	83 c4 10             	add    $0x10,%esp
    if(log.committing){
80103255:	a1 c0 4c 11 80       	mov    0x80114cc0,%eax
8010325a:	85 c0                	test   %eax,%eax
8010325c:	75 e2                	jne    80103240 <begin_op+0x20>
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGSIZE){
8010325e:	a1 bc 4c 11 80       	mov    0x80114cbc,%eax
80103263:	8b 15 c8 4c 11 80    	mov    0x80114cc8,%edx
80103269:	83 c0 01             	add    $0x1,%eax
8010326c:	8d 0c 80             	lea    (%eax,%eax,4),%ecx
8010326f:	8d 14 4a             	lea    (%edx,%ecx,2),%edx
80103272:	83 fa 1e             	cmp    $0x1e,%edx
80103275:	7f c9                	jg     80103240 <begin_op+0x20>
      // this op might exhaust log space; wait for commit.
      sleep(&log, &log.lock);
    } else {
      log.outstanding += 1;
      release(&log.lock);
80103277:	83 ec 0c             	sub    $0xc,%esp
      log.outstanding += 1;
8010327a:	a3 bc 4c 11 80       	mov    %eax,0x80114cbc
      release(&log.lock);
8010327f:	68 80 4c 11 80       	push   $0x80114c80
80103284:	e8 47 2a 00 00       	call   80105cd0 <release>
      break;
    }
  }
}
80103289:	83 c4 10             	add    $0x10,%esp
8010328c:	c9                   	leave  
8010328d:	c3                   	ret    
8010328e:	66 90                	xchg   %ax,%ax

80103290 <end_op>:

// called at the end of each FS system call.
// commits if this was the last outstanding operation.
void
end_op(void)
{
80103290:	f3 0f 1e fb          	endbr32 
80103294:	55                   	push   %ebp
80103295:	89 e5                	mov    %esp,%ebp
80103297:	57                   	push   %edi
80103298:	56                   	push   %esi
80103299:	53                   	push   %ebx
8010329a:	83 ec 18             	sub    $0x18,%esp
  int do_commit = 0;

  acquire(&log.lock);
8010329d:	68 80 4c 11 80       	push   $0x80114c80
801032a2:	e8 69 29 00 00       	call   80105c10 <acquire>
  log.outstanding -= 1;
801032a7:	a1 bc 4c 11 80       	mov    0x80114cbc,%eax
  if(log.committing)
801032ac:	8b 35 c0 4c 11 80    	mov    0x80114cc0,%esi
801032b2:	83 c4 10             	add    $0x10,%esp
  log.outstanding -= 1;
801032b5:	8d 58 ff             	lea    -0x1(%eax),%ebx
801032b8:	89 1d bc 4c 11 80    	mov    %ebx,0x80114cbc
  if(log.committing)
801032be:	85 f6                	test   %esi,%esi
801032c0:	0f 85 1e 01 00 00    	jne    801033e4 <end_op+0x154>
    panic("log.committing");
  if(log.outstanding == 0){
801032c6:	85 db                	test   %ebx,%ebx
801032c8:	0f 85 f2 00 00 00    	jne    801033c0 <end_op+0x130>
    do_commit = 1;
    log.committing = 1;
801032ce:	c7 05 c0 4c 11 80 01 	movl   $0x1,0x80114cc0
801032d5:	00 00 00 
    // begin_op() may be waiting for log space,
    // and decrementing log.outstanding has decreased
    // the amount of reserved space.
    wakeup(&log);
  }
  release(&log.lock);
801032d8:	83 ec 0c             	sub    $0xc,%esp
801032db:	68 80 4c 11 80       	push   $0x80114c80
801032e0:	e8 eb 29 00 00       	call   80105cd0 <release>
}

static void
commit()
{
  if (log.lh.n > 0) {
801032e5:	8b 0d c8 4c 11 80    	mov    0x80114cc8,%ecx
801032eb:	83 c4 10             	add    $0x10,%esp
801032ee:	85 c9                	test   %ecx,%ecx
801032f0:	7f 3e                	jg     80103330 <end_op+0xa0>
    acquire(&log.lock);
801032f2:	83 ec 0c             	sub    $0xc,%esp
801032f5:	68 80 4c 11 80       	push   $0x80114c80
801032fa:	e8 11 29 00 00       	call   80105c10 <acquire>
    wakeup(&log);
801032ff:	c7 04 24 80 4c 11 80 	movl   $0x80114c80,(%esp)
    log.committing = 0;
80103306:	c7 05 c0 4c 11 80 00 	movl   $0x0,0x80114cc0
8010330d:	00 00 00 
    wakeup(&log);
80103310:	e8 4b 13 00 00       	call   80104660 <wakeup>
    release(&log.lock);
80103315:	c7 04 24 80 4c 11 80 	movl   $0x80114c80,(%esp)
8010331c:	e8 af 29 00 00       	call   80105cd0 <release>
80103321:	83 c4 10             	add    $0x10,%esp
}
80103324:	8d 65 f4             	lea    -0xc(%ebp),%esp
80103327:	5b                   	pop    %ebx
80103328:	5e                   	pop    %esi
80103329:	5f                   	pop    %edi
8010332a:	5d                   	pop    %ebp
8010332b:	c3                   	ret    
8010332c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
80103330:	a1 b4 4c 11 80       	mov    0x80114cb4,%eax
80103335:	83 ec 08             	sub    $0x8,%esp
80103338:	01 d8                	add    %ebx,%eax
8010333a:	83 c0 01             	add    $0x1,%eax
8010333d:	50                   	push   %eax
8010333e:	ff 35 c4 4c 11 80    	pushl  0x80114cc4
80103344:	e8 87 cd ff ff       	call   801000d0 <bread>
80103349:	89 c6                	mov    %eax,%esi
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
8010334b:	58                   	pop    %eax
8010334c:	5a                   	pop    %edx
8010334d:	ff 34 9d cc 4c 11 80 	pushl  -0x7feeb334(,%ebx,4)
80103354:	ff 35 c4 4c 11 80    	pushl  0x80114cc4
  for (tail = 0; tail < log.lh.n; tail++) {
8010335a:	83 c3 01             	add    $0x1,%ebx
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
8010335d:	e8 6e cd ff ff       	call   801000d0 <bread>
    memmove(to->data, from->data, BSIZE);
80103362:	83 c4 0c             	add    $0xc,%esp
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
80103365:	89 c7                	mov    %eax,%edi
    memmove(to->data, from->data, BSIZE);
80103367:	8d 40 5c             	lea    0x5c(%eax),%eax
8010336a:	68 00 02 00 00       	push   $0x200
8010336f:	50                   	push   %eax
80103370:	8d 46 5c             	lea    0x5c(%esi),%eax
80103373:	50                   	push   %eax
80103374:	e8 47 2a 00 00       	call   80105dc0 <memmove>
    bwrite(to);  // write the log
80103379:	89 34 24             	mov    %esi,(%esp)
8010337c:	e8 2f ce ff ff       	call   801001b0 <bwrite>
    brelse(from);
80103381:	89 3c 24             	mov    %edi,(%esp)
80103384:	e8 67 ce ff ff       	call   801001f0 <brelse>
    brelse(to);
80103389:	89 34 24             	mov    %esi,(%esp)
8010338c:	e8 5f ce ff ff       	call   801001f0 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
80103391:	83 c4 10             	add    $0x10,%esp
80103394:	3b 1d c8 4c 11 80    	cmp    0x80114cc8,%ebx
8010339a:	7c 94                	jl     80103330 <end_op+0xa0>
    write_log();     // Write modified blocks from cache to log
    write_head();    // Write header to disk -- the real commit
8010339c:	e8 7f fd ff ff       	call   80103120 <write_head>
    install_trans(); // Now install writes to home locations
801033a1:	e8 da fc ff ff       	call   80103080 <install_trans>
    log.lh.n = 0;
801033a6:	c7 05 c8 4c 11 80 00 	movl   $0x0,0x80114cc8
801033ad:	00 00 00 
    write_head();    // Erase the transaction from the log
801033b0:	e8 6b fd ff ff       	call   80103120 <write_head>
801033b5:	e9 38 ff ff ff       	jmp    801032f2 <end_op+0x62>
801033ba:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    wakeup(&log);
801033c0:	83 ec 0c             	sub    $0xc,%esp
801033c3:	68 80 4c 11 80       	push   $0x80114c80
801033c8:	e8 93 12 00 00       	call   80104660 <wakeup>
  release(&log.lock);
801033cd:	c7 04 24 80 4c 11 80 	movl   $0x80114c80,(%esp)
801033d4:	e8 f7 28 00 00       	call   80105cd0 <release>
801033d9:	83 c4 10             	add    $0x10,%esp
}
801033dc:	8d 65 f4             	lea    -0xc(%ebp),%esp
801033df:	5b                   	pop    %ebx
801033e0:	5e                   	pop    %esi
801033e1:	5f                   	pop    %edi
801033e2:	5d                   	pop    %ebp
801033e3:	c3                   	ret    
    panic("log.committing");
801033e4:	83 ec 0c             	sub    $0xc,%esp
801033e7:	68 a4 90 10 80       	push   $0x801090a4
801033ec:	e8 9f cf ff ff       	call   80100390 <panic>
801033f1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801033f8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801033ff:	90                   	nop

80103400 <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
80103400:	f3 0f 1e fb          	endbr32 
80103404:	55                   	push   %ebp
80103405:	89 e5                	mov    %esp,%ebp
80103407:	53                   	push   %ebx
80103408:	83 ec 04             	sub    $0x4,%esp
  int i;

  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
8010340b:	8b 15 c8 4c 11 80    	mov    0x80114cc8,%edx
{
80103411:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
80103414:	83 fa 1d             	cmp    $0x1d,%edx
80103417:	0f 8f 91 00 00 00    	jg     801034ae <log_write+0xae>
8010341d:	a1 b8 4c 11 80       	mov    0x80114cb8,%eax
80103422:	83 e8 01             	sub    $0x1,%eax
80103425:	39 c2                	cmp    %eax,%edx
80103427:	0f 8d 81 00 00 00    	jge    801034ae <log_write+0xae>
    panic("too big a transaction");
  if (log.outstanding < 1)
8010342d:	a1 bc 4c 11 80       	mov    0x80114cbc,%eax
80103432:	85 c0                	test   %eax,%eax
80103434:	0f 8e 81 00 00 00    	jle    801034bb <log_write+0xbb>
    panic("log_write outside of trans");

  acquire(&log.lock);
8010343a:	83 ec 0c             	sub    $0xc,%esp
8010343d:	68 80 4c 11 80       	push   $0x80114c80
80103442:	e8 c9 27 00 00       	call   80105c10 <acquire>
  for (i = 0; i < log.lh.n; i++) {
80103447:	8b 15 c8 4c 11 80    	mov    0x80114cc8,%edx
8010344d:	83 c4 10             	add    $0x10,%esp
80103450:	85 d2                	test   %edx,%edx
80103452:	7e 4e                	jle    801034a2 <log_write+0xa2>
    if (log.lh.block[i] == b->blockno)   // log absorbtion
80103454:	8b 4b 08             	mov    0x8(%ebx),%ecx
  for (i = 0; i < log.lh.n; i++) {
80103457:	31 c0                	xor    %eax,%eax
80103459:	eb 0c                	jmp    80103467 <log_write+0x67>
8010345b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010345f:	90                   	nop
80103460:	83 c0 01             	add    $0x1,%eax
80103463:	39 c2                	cmp    %eax,%edx
80103465:	74 29                	je     80103490 <log_write+0x90>
    if (log.lh.block[i] == b->blockno)   // log absorbtion
80103467:	39 0c 85 cc 4c 11 80 	cmp    %ecx,-0x7feeb334(,%eax,4)
8010346e:	75 f0                	jne    80103460 <log_write+0x60>
      break;
  }
  log.lh.block[i] = b->blockno;
80103470:	89 0c 85 cc 4c 11 80 	mov    %ecx,-0x7feeb334(,%eax,4)
  if (i == log.lh.n)
    log.lh.n++;
  b->flags |= B_DIRTY; // prevent eviction
80103477:	83 0b 04             	orl    $0x4,(%ebx)
  release(&log.lock);
}
8010347a:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  release(&log.lock);
8010347d:	c7 45 08 80 4c 11 80 	movl   $0x80114c80,0x8(%ebp)
}
80103484:	c9                   	leave  
  release(&log.lock);
80103485:	e9 46 28 00 00       	jmp    80105cd0 <release>
8010348a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  log.lh.block[i] = b->blockno;
80103490:	89 0c 95 cc 4c 11 80 	mov    %ecx,-0x7feeb334(,%edx,4)
    log.lh.n++;
80103497:	83 c2 01             	add    $0x1,%edx
8010349a:	89 15 c8 4c 11 80    	mov    %edx,0x80114cc8
801034a0:	eb d5                	jmp    80103477 <log_write+0x77>
  log.lh.block[i] = b->blockno;
801034a2:	8b 43 08             	mov    0x8(%ebx),%eax
801034a5:	a3 cc 4c 11 80       	mov    %eax,0x80114ccc
  if (i == log.lh.n)
801034aa:	75 cb                	jne    80103477 <log_write+0x77>
801034ac:	eb e9                	jmp    80103497 <log_write+0x97>
    panic("too big a transaction");
801034ae:	83 ec 0c             	sub    $0xc,%esp
801034b1:	68 b3 90 10 80       	push   $0x801090b3
801034b6:	e8 d5 ce ff ff       	call   80100390 <panic>
    panic("log_write outside of trans");
801034bb:	83 ec 0c             	sub    $0xc,%esp
801034be:	68 c9 90 10 80       	push   $0x801090c9
801034c3:	e8 c8 ce ff ff       	call   80100390 <panic>
801034c8:	66 90                	xchg   %ax,%ax
801034ca:	66 90                	xchg   %ax,%ax
801034cc:	66 90                	xchg   %ax,%ax
801034ce:	66 90                	xchg   %ax,%ax

801034d0 <mpmain>:
}

// Common CPU setup code.
static void
mpmain(void)
{
801034d0:	55                   	push   %ebp
801034d1:	89 e5                	mov    %esp,%ebp
801034d3:	53                   	push   %ebx
801034d4:	83 ec 04             	sub    $0x4,%esp
  cprintf("cpu%d: starting %d\n", cpuid(), cpuid());
801034d7:	e8 c4 09 00 00       	call   80103ea0 <cpuid>
801034dc:	89 c3                	mov    %eax,%ebx
801034de:	e8 bd 09 00 00       	call   80103ea0 <cpuid>
801034e3:	83 ec 04             	sub    $0x4,%esp
801034e6:	53                   	push   %ebx
801034e7:	50                   	push   %eax
801034e8:	68 e4 90 10 80       	push   $0x801090e4
801034ed:	e8 ce d3 ff ff       	call   801008c0 <cprintf>
  idtinit();       // load idt register
801034f2:	e8 c9 3e 00 00       	call   801073c0 <idtinit>
  xchg(&(mycpu()->started), 1); // tell startothers() we're up
801034f7:	e8 34 09 00 00       	call   80103e30 <mycpu>
801034fc:	89 c2                	mov    %eax,%edx
xchg(volatile uint *addr, uint newval)
{
  uint result;

  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
801034fe:	b8 01 00 00 00       	mov    $0x1,%eax
80103503:	f0 87 82 a0 00 00 00 	lock xchg %eax,0xa0(%edx)
  scheduler();     // start running processes
8010350a:	e8 91 0b 00 00       	call   801040a0 <scheduler>
8010350f:	90                   	nop

80103510 <mpenter>:
{
80103510:	f3 0f 1e fb          	endbr32 
80103514:	55                   	push   %ebp
80103515:	89 e5                	mov    %esp,%ebp
80103517:	83 ec 08             	sub    $0x8,%esp
  switchkvm();
8010351a:	e8 81 4f 00 00       	call   801084a0 <switchkvm>
  seginit();
8010351f:	e8 ec 4e 00 00       	call   80108410 <seginit>
  lapicinit();
80103524:	e8 67 f7 ff ff       	call   80102c90 <lapicinit>
  mpmain();
80103529:	e8 a2 ff ff ff       	call   801034d0 <mpmain>
8010352e:	66 90                	xchg   %ax,%ax

80103530 <main>:
{
80103530:	f3 0f 1e fb          	endbr32 
80103534:	8d 4c 24 04          	lea    0x4(%esp),%ecx
80103538:	83 e4 f0             	and    $0xfffffff0,%esp
8010353b:	ff 71 fc             	pushl  -0x4(%ecx)
8010353e:	55                   	push   %ebp
8010353f:	89 e5                	mov    %esp,%ebp
80103541:	53                   	push   %ebx
80103542:	51                   	push   %ecx
  kinit1(end, P2V(4*1024*1024)); // phys page allocator
80103543:	83 ec 08             	sub    $0x8,%esp
80103546:	68 00 00 40 80       	push   $0x80400000
8010354b:	68 a8 86 11 80       	push   $0x801186a8
80103550:	e8 fb f4 ff ff       	call   80102a50 <kinit1>
  kvmalloc();      // kernel page table
80103555:	e8 26 54 00 00       	call   80108980 <kvmalloc>
  mpinit();        // detect other processors
8010355a:	e8 91 01 00 00       	call   801036f0 <mpinit>
  lapicinit();     // interrupt controller
8010355f:	e8 2c f7 ff ff       	call   80102c90 <lapicinit>
  seginit();       // segment descriptors
80103564:	e8 a7 4e 00 00       	call   80108410 <seginit>
  picinit();       // disable pic
80103569:	e8 62 03 00 00       	call   801038d0 <picinit>
  ioapicinit();    // another interrupt controller
8010356e:	e8 fd f2 ff ff       	call   80102870 <ioapicinit>
  consoleinit();   // console hardware
80103573:	e8 a8 d9 ff ff       	call   80100f20 <consoleinit>
  uartinit();      // serial port
80103578:	e8 53 41 00 00       	call   801076d0 <uartinit>
  pinit();         // process table
8010357d:	e8 8e 08 00 00       	call   80103e10 <pinit>
  tvinit();        // trap vectors
80103582:	e8 b9 3d 00 00       	call   80107340 <tvinit>
  binit();         // buffer cache
80103587:	e8 b4 ca ff ff       	call   80100040 <binit>
  init_queue_test();
8010358c:	e8 7f 22 00 00       	call   80105810 <init_queue_test>
  fileinit();      // file table
80103591:	e8 3a dd ff ff       	call   801012d0 <fileinit>
  ideinit();       // disk 
80103596:	e8 a5 f0 ff ff       	call   80102640 <ideinit>

  // Write entry code to unused memory at 0x7000.
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = P2V(0x7000);
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);
8010359b:	83 c4 0c             	add    $0xc,%esp
8010359e:	68 8a 00 00 00       	push   $0x8a
801035a3:	68 8c c4 10 80       	push   $0x8010c48c
801035a8:	68 00 70 00 80       	push   $0x80007000
801035ad:	e8 0e 28 00 00       	call   80105dc0 <memmove>

  for(c = cpus; c < cpus+ncpu; c++){
801035b2:	83 c4 10             	add    $0x10,%esp
801035b5:	69 05 00 53 11 80 b0 	imul   $0xb0,0x80115300,%eax
801035bc:	00 00 00 
801035bf:	05 80 4d 11 80       	add    $0x80114d80,%eax
801035c4:	3d 80 4d 11 80       	cmp    $0x80114d80,%eax
801035c9:	76 7d                	jbe    80103648 <main+0x118>
801035cb:	bb 80 4d 11 80       	mov    $0x80114d80,%ebx
801035d0:	eb 1f                	jmp    801035f1 <main+0xc1>
801035d2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801035d8:	69 05 00 53 11 80 b0 	imul   $0xb0,0x80115300,%eax
801035df:	00 00 00 
801035e2:	81 c3 b0 00 00 00    	add    $0xb0,%ebx
801035e8:	05 80 4d 11 80       	add    $0x80114d80,%eax
801035ed:	39 c3                	cmp    %eax,%ebx
801035ef:	73 57                	jae    80103648 <main+0x118>
    if(c == mycpu())  // We've started already.
801035f1:	e8 3a 08 00 00       	call   80103e30 <mycpu>
801035f6:	39 c3                	cmp    %eax,%ebx
801035f8:	74 de                	je     801035d8 <main+0xa8>
      continue;

    // Tell entryother.S what stack to use, where to enter, and what
    // pgdir to use. We cannot use kpgdir yet, because the AP processor
    // is running in low  memory, so we use entrypgdir for the APs too.
    stack = kalloc();
801035fa:	e8 21 f5 ff ff       	call   80102b20 <kalloc>
    *(void**)(code-4) = stack + KSTACKSIZE;
    *(void(**)(void))(code-8) = mpenter;
    *(int**)(code-12) = (void *) V2P(entrypgdir);

    lapicstartap(c->apicid, V2P(code));
801035ff:	83 ec 08             	sub    $0x8,%esp
    *(void(**)(void))(code-8) = mpenter;
80103602:	c7 05 f8 6f 00 80 10 	movl   $0x80103510,0x80006ff8
80103609:	35 10 80 
    *(int**)(code-12) = (void *) V2P(entrypgdir);
8010360c:	c7 05 f4 6f 00 80 00 	movl   $0x10b000,0x80006ff4
80103613:	b0 10 00 
    *(void**)(code-4) = stack + KSTACKSIZE;
80103616:	05 00 10 00 00       	add    $0x1000,%eax
8010361b:	a3 fc 6f 00 80       	mov    %eax,0x80006ffc
    lapicstartap(c->apicid, V2P(code));
80103620:	0f b6 03             	movzbl (%ebx),%eax
80103623:	68 00 70 00 00       	push   $0x7000
80103628:	50                   	push   %eax
80103629:	e8 b2 f7 ff ff       	call   80102de0 <lapicstartap>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
8010362e:	83 c4 10             	add    $0x10,%esp
80103631:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103638:	8b 83 a0 00 00 00    	mov    0xa0(%ebx),%eax
8010363e:	85 c0                	test   %eax,%eax
80103640:	74 f6                	je     80103638 <main+0x108>
80103642:	eb 94                	jmp    801035d8 <main+0xa8>
80103644:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  kinit2(P2V(4*1024*1024), P2V(PHYSTOP)); // must come after startothers()
80103648:	83 ec 08             	sub    $0x8,%esp
8010364b:	68 00 00 00 8e       	push   $0x8e000000
80103650:	68 00 00 40 80       	push   $0x80400000
80103655:	e8 66 f4 ff ff       	call   80102ac0 <kinit2>
  userinit();      // first user process
8010365a:	e8 f1 13 00 00       	call   80104a50 <userinit>
  mpmain();        // finish this processor's setup
8010365f:	e8 6c fe ff ff       	call   801034d0 <mpmain>
80103664:	66 90                	xchg   %ax,%ax
80103666:	66 90                	xchg   %ax,%ax
80103668:	66 90                	xchg   %ax,%ax
8010366a:	66 90                	xchg   %ax,%ax
8010366c:	66 90                	xchg   %ax,%ax
8010366e:	66 90                	xchg   %ax,%ax

80103670 <mpsearch1>:
}

// Look for an MP structure in the len bytes at addr.
static struct mp*
mpsearch1(uint a, int len)
{
80103670:	55                   	push   %ebp
80103671:	89 e5                	mov    %esp,%ebp
80103673:	57                   	push   %edi
80103674:	56                   	push   %esi
  uchar *e, *p, *addr;

  addr = P2V(a);
80103675:	8d b0 00 00 00 80    	lea    -0x80000000(%eax),%esi
{
8010367b:	53                   	push   %ebx
  e = addr+len;
8010367c:	8d 1c 16             	lea    (%esi,%edx,1),%ebx
{
8010367f:	83 ec 0c             	sub    $0xc,%esp
  for(p = addr; p < e; p += sizeof(struct mp))
80103682:	39 de                	cmp    %ebx,%esi
80103684:	72 10                	jb     80103696 <mpsearch1+0x26>
80103686:	eb 50                	jmp    801036d8 <mpsearch1+0x68>
80103688:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010368f:	90                   	nop
80103690:	89 fe                	mov    %edi,%esi
80103692:	39 fb                	cmp    %edi,%ebx
80103694:	76 42                	jbe    801036d8 <mpsearch1+0x68>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
80103696:	83 ec 04             	sub    $0x4,%esp
80103699:	8d 7e 10             	lea    0x10(%esi),%edi
8010369c:	6a 04                	push   $0x4
8010369e:	68 f8 90 10 80       	push   $0x801090f8
801036a3:	56                   	push   %esi
801036a4:	e8 c7 26 00 00       	call   80105d70 <memcmp>
801036a9:	83 c4 10             	add    $0x10,%esp
801036ac:	85 c0                	test   %eax,%eax
801036ae:	75 e0                	jne    80103690 <mpsearch1+0x20>
801036b0:	89 f2                	mov    %esi,%edx
801036b2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    sum += addr[i];
801036b8:	0f b6 0a             	movzbl (%edx),%ecx
801036bb:	83 c2 01             	add    $0x1,%edx
801036be:	01 c8                	add    %ecx,%eax
  for(i=0; i<len; i++)
801036c0:	39 fa                	cmp    %edi,%edx
801036c2:	75 f4                	jne    801036b8 <mpsearch1+0x48>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
801036c4:	84 c0                	test   %al,%al
801036c6:	75 c8                	jne    80103690 <mpsearch1+0x20>
      return (struct mp*)p;
  return 0;
}
801036c8:	8d 65 f4             	lea    -0xc(%ebp),%esp
801036cb:	89 f0                	mov    %esi,%eax
801036cd:	5b                   	pop    %ebx
801036ce:	5e                   	pop    %esi
801036cf:	5f                   	pop    %edi
801036d0:	5d                   	pop    %ebp
801036d1:	c3                   	ret    
801036d2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801036d8:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
801036db:	31 f6                	xor    %esi,%esi
}
801036dd:	5b                   	pop    %ebx
801036de:	89 f0                	mov    %esi,%eax
801036e0:	5e                   	pop    %esi
801036e1:	5f                   	pop    %edi
801036e2:	5d                   	pop    %ebp
801036e3:	c3                   	ret    
801036e4:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801036eb:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801036ef:	90                   	nop

801036f0 <mpinit>:
  return conf;
}

void
mpinit(void)
{
801036f0:	f3 0f 1e fb          	endbr32 
801036f4:	55                   	push   %ebp
801036f5:	89 e5                	mov    %esp,%ebp
801036f7:	57                   	push   %edi
801036f8:	56                   	push   %esi
801036f9:	53                   	push   %ebx
801036fa:	83 ec 1c             	sub    $0x1c,%esp
  if((p = ((bda[0x0F]<<8)| bda[0x0E]) << 4)){
801036fd:	0f b6 05 0f 04 00 80 	movzbl 0x8000040f,%eax
80103704:	0f b6 15 0e 04 00 80 	movzbl 0x8000040e,%edx
8010370b:	c1 e0 08             	shl    $0x8,%eax
8010370e:	09 d0                	or     %edx,%eax
80103710:	c1 e0 04             	shl    $0x4,%eax
80103713:	75 1b                	jne    80103730 <mpinit+0x40>
    p = ((bda[0x14]<<8)|bda[0x13])*1024;
80103715:	0f b6 05 14 04 00 80 	movzbl 0x80000414,%eax
8010371c:	0f b6 15 13 04 00 80 	movzbl 0x80000413,%edx
80103723:	c1 e0 08             	shl    $0x8,%eax
80103726:	09 d0                	or     %edx,%eax
80103728:	c1 e0 0a             	shl    $0xa,%eax
    if((mp = mpsearch1(p-1024, 1024)))
8010372b:	2d 00 04 00 00       	sub    $0x400,%eax
    if((mp = mpsearch1(p, 1024)))
80103730:	ba 00 04 00 00       	mov    $0x400,%edx
80103735:	e8 36 ff ff ff       	call   80103670 <mpsearch1>
8010373a:	89 c6                	mov    %eax,%esi
8010373c:	85 c0                	test   %eax,%eax
8010373e:	0f 84 4c 01 00 00    	je     80103890 <mpinit+0x1a0>
  if((mp = mpsearch()) == 0 || mp->physaddr == 0)
80103744:	8b 5e 04             	mov    0x4(%esi),%ebx
80103747:	85 db                	test   %ebx,%ebx
80103749:	0f 84 61 01 00 00    	je     801038b0 <mpinit+0x1c0>
  if(memcmp(conf, "PCMP", 4) != 0)
8010374f:	83 ec 04             	sub    $0x4,%esp
  conf = (struct mpconf*) P2V((uint) mp->physaddr);
80103752:	8d 83 00 00 00 80    	lea    -0x80000000(%ebx),%eax
  if(memcmp(conf, "PCMP", 4) != 0)
80103758:	6a 04                	push   $0x4
8010375a:	68 fd 90 10 80       	push   $0x801090fd
8010375f:	50                   	push   %eax
  conf = (struct mpconf*) P2V((uint) mp->physaddr);
80103760:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  if(memcmp(conf, "PCMP", 4) != 0)
80103763:	e8 08 26 00 00       	call   80105d70 <memcmp>
80103768:	83 c4 10             	add    $0x10,%esp
8010376b:	85 c0                	test   %eax,%eax
8010376d:	0f 85 3d 01 00 00    	jne    801038b0 <mpinit+0x1c0>
  if(conf->version != 1 && conf->version != 4)
80103773:	0f b6 83 06 00 00 80 	movzbl -0x7ffffffa(%ebx),%eax
8010377a:	3c 01                	cmp    $0x1,%al
8010377c:	74 08                	je     80103786 <mpinit+0x96>
8010377e:	3c 04                	cmp    $0x4,%al
80103780:	0f 85 2a 01 00 00    	jne    801038b0 <mpinit+0x1c0>
  if(sum((uchar*)conf, conf->length) != 0)
80103786:	0f b7 93 04 00 00 80 	movzwl -0x7ffffffc(%ebx),%edx
  for(i=0; i<len; i++)
8010378d:	66 85 d2             	test   %dx,%dx
80103790:	74 26                	je     801037b8 <mpinit+0xc8>
80103792:	8d 3c 1a             	lea    (%edx,%ebx,1),%edi
80103795:	89 d8                	mov    %ebx,%eax
  sum = 0;
80103797:	31 d2                	xor    %edx,%edx
80103799:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    sum += addr[i];
801037a0:	0f b6 88 00 00 00 80 	movzbl -0x80000000(%eax),%ecx
801037a7:	83 c0 01             	add    $0x1,%eax
801037aa:	01 ca                	add    %ecx,%edx
  for(i=0; i<len; i++)
801037ac:	39 f8                	cmp    %edi,%eax
801037ae:	75 f0                	jne    801037a0 <mpinit+0xb0>
  if(sum((uchar*)conf, conf->length) != 0)
801037b0:	84 d2                	test   %dl,%dl
801037b2:	0f 85 f8 00 00 00    	jne    801038b0 <mpinit+0x1c0>
  struct mpioapic *ioapic;

  if((conf = mpconfig(&mp)) == 0)
    panic("Expect to run on an SMP");
  ismp = 1;
  lapic = (uint*)conf->lapicaddr;
801037b8:	8b 83 24 00 00 80    	mov    -0x7fffffdc(%ebx),%eax
801037be:	a3 7c 4c 11 80       	mov    %eax,0x80114c7c
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
801037c3:	8d 83 2c 00 00 80    	lea    -0x7fffffd4(%ebx),%eax
801037c9:	0f b7 93 04 00 00 80 	movzwl -0x7ffffffc(%ebx),%edx
  ismp = 1;
801037d0:	bb 01 00 00 00       	mov    $0x1,%ebx
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
801037d5:	03 55 e4             	add    -0x1c(%ebp),%edx
801037d8:	89 5d e4             	mov    %ebx,-0x1c(%ebp)
801037db:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801037df:	90                   	nop
801037e0:	39 c2                	cmp    %eax,%edx
801037e2:	76 15                	jbe    801037f9 <mpinit+0x109>
    switch(*p){
801037e4:	0f b6 08             	movzbl (%eax),%ecx
801037e7:	80 f9 02             	cmp    $0x2,%cl
801037ea:	74 5c                	je     80103848 <mpinit+0x158>
801037ec:	77 42                	ja     80103830 <mpinit+0x140>
801037ee:	84 c9                	test   %cl,%cl
801037f0:	74 6e                	je     80103860 <mpinit+0x170>
      p += sizeof(struct mpioapic);
      continue;
    case MPBUS:
    case MPIOINTR:
    case MPLINTR:
      p += 8;
801037f2:	83 c0 08             	add    $0x8,%eax
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
801037f5:	39 c2                	cmp    %eax,%edx
801037f7:	77 eb                	ja     801037e4 <mpinit+0xf4>
801037f9:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
    default:
      ismp = 0;
      break;
    }
  }
  if(!ismp)
801037fc:	85 db                	test   %ebx,%ebx
801037fe:	0f 84 b9 00 00 00    	je     801038bd <mpinit+0x1cd>
    panic("Didn't find a suitable machine");

  if(mp->imcrp){
80103804:	80 7e 0c 00          	cmpb   $0x0,0xc(%esi)
80103808:	74 15                	je     8010381f <mpinit+0x12f>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
8010380a:	b8 70 00 00 00       	mov    $0x70,%eax
8010380f:	ba 22 00 00 00       	mov    $0x22,%edx
80103814:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80103815:	ba 23 00 00 00       	mov    $0x23,%edx
8010381a:	ec                   	in     (%dx),%al
    // Bochs doesn't support IMCR, so this doesn't run on Bochs.
    // But it would on real hardware.
    outb(0x22, 0x70);   // Select IMCR
    outb(0x23, inb(0x23) | 1);  // Mask external interrupts.
8010381b:	83 c8 01             	or     $0x1,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
8010381e:	ee                   	out    %al,(%dx)
  }
}
8010381f:	8d 65 f4             	lea    -0xc(%ebp),%esp
80103822:	5b                   	pop    %ebx
80103823:	5e                   	pop    %esi
80103824:	5f                   	pop    %edi
80103825:	5d                   	pop    %ebp
80103826:	c3                   	ret    
80103827:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010382e:	66 90                	xchg   %ax,%ax
    switch(*p){
80103830:	83 e9 03             	sub    $0x3,%ecx
80103833:	80 f9 01             	cmp    $0x1,%cl
80103836:	76 ba                	jbe    801037f2 <mpinit+0x102>
80103838:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
8010383f:	eb 9f                	jmp    801037e0 <mpinit+0xf0>
80103841:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
      ioapicid = ioapic->apicno;
80103848:	0f b6 48 01          	movzbl 0x1(%eax),%ecx
      p += sizeof(struct mpioapic);
8010384c:	83 c0 08             	add    $0x8,%eax
      ioapicid = ioapic->apicno;
8010384f:	88 0d 60 4d 11 80    	mov    %cl,0x80114d60
      continue;
80103855:	eb 89                	jmp    801037e0 <mpinit+0xf0>
80103857:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010385e:	66 90                	xchg   %ax,%ax
      if(ncpu < NCPU) {
80103860:	8b 0d 00 53 11 80    	mov    0x80115300,%ecx
80103866:	83 f9 07             	cmp    $0x7,%ecx
80103869:	7f 19                	jg     80103884 <mpinit+0x194>
        cpus[ncpu].apicid = proc->apicid;  // apicid may differ from ncpu
8010386b:	69 f9 b0 00 00 00    	imul   $0xb0,%ecx,%edi
80103871:	0f b6 58 01          	movzbl 0x1(%eax),%ebx
        ncpu++;
80103875:	83 c1 01             	add    $0x1,%ecx
80103878:	89 0d 00 53 11 80    	mov    %ecx,0x80115300
        cpus[ncpu].apicid = proc->apicid;  // apicid may differ from ncpu
8010387e:	88 9f 80 4d 11 80    	mov    %bl,-0x7feeb280(%edi)
      p += sizeof(struct mpproc);
80103884:	83 c0 14             	add    $0x14,%eax
      continue;
80103887:	e9 54 ff ff ff       	jmp    801037e0 <mpinit+0xf0>
8010388c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  return mpsearch1(0xF0000, 0x10000);
80103890:	ba 00 00 01 00       	mov    $0x10000,%edx
80103895:	b8 00 00 0f 00       	mov    $0xf0000,%eax
8010389a:	e8 d1 fd ff ff       	call   80103670 <mpsearch1>
8010389f:	89 c6                	mov    %eax,%esi
  if((mp = mpsearch()) == 0 || mp->physaddr == 0)
801038a1:	85 c0                	test   %eax,%eax
801038a3:	0f 85 9b fe ff ff    	jne    80103744 <mpinit+0x54>
801038a9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    panic("Expect to run on an SMP");
801038b0:	83 ec 0c             	sub    $0xc,%esp
801038b3:	68 02 91 10 80       	push   $0x80109102
801038b8:	e8 d3 ca ff ff       	call   80100390 <panic>
    panic("Didn't find a suitable machine");
801038bd:	83 ec 0c             	sub    $0xc,%esp
801038c0:	68 1c 91 10 80       	push   $0x8010911c
801038c5:	e8 c6 ca ff ff       	call   80100390 <panic>
801038ca:	66 90                	xchg   %ax,%ax
801038cc:	66 90                	xchg   %ax,%ax
801038ce:	66 90                	xchg   %ax,%ax

801038d0 <picinit>:
#define IO_PIC2         0xA0    // Slave (IRQs 8-15)

// Don't use the 8259A interrupt controllers.  Xv6 assumes SMP hardware.
void
picinit(void)
{
801038d0:	f3 0f 1e fb          	endbr32 
801038d4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801038d9:	ba 21 00 00 00       	mov    $0x21,%edx
801038de:	ee                   	out    %al,(%dx)
801038df:	ba a1 00 00 00       	mov    $0xa1,%edx
801038e4:	ee                   	out    %al,(%dx)
  // mask all interrupts
  outb(IO_PIC1+1, 0xFF);
  outb(IO_PIC2+1, 0xFF);
}
801038e5:	c3                   	ret    
801038e6:	66 90                	xchg   %ax,%ax
801038e8:	66 90                	xchg   %ax,%ax
801038ea:	66 90                	xchg   %ax,%ax
801038ec:	66 90                	xchg   %ax,%ax
801038ee:	66 90                	xchg   %ax,%ax

801038f0 <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
801038f0:	f3 0f 1e fb          	endbr32 
801038f4:	55                   	push   %ebp
801038f5:	89 e5                	mov    %esp,%ebp
801038f7:	57                   	push   %edi
801038f8:	56                   	push   %esi
801038f9:	53                   	push   %ebx
801038fa:	83 ec 0c             	sub    $0xc,%esp
801038fd:	8b 5d 08             	mov    0x8(%ebp),%ebx
80103900:	8b 75 0c             	mov    0xc(%ebp),%esi
  struct pipe *p;

  p = 0;
  *f0 = *f1 = 0;
80103903:	c7 06 00 00 00 00    	movl   $0x0,(%esi)
80103909:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
8010390f:	e8 dc d9 ff ff       	call   801012f0 <filealloc>
80103914:	89 03                	mov    %eax,(%ebx)
80103916:	85 c0                	test   %eax,%eax
80103918:	0f 84 ac 00 00 00    	je     801039ca <pipealloc+0xda>
8010391e:	e8 cd d9 ff ff       	call   801012f0 <filealloc>
80103923:	89 06                	mov    %eax,(%esi)
80103925:	85 c0                	test   %eax,%eax
80103927:	0f 84 8b 00 00 00    	je     801039b8 <pipealloc+0xc8>
    goto bad;
  if((p = (struct pipe*)kalloc()) == 0)
8010392d:	e8 ee f1 ff ff       	call   80102b20 <kalloc>
80103932:	89 c7                	mov    %eax,%edi
80103934:	85 c0                	test   %eax,%eax
80103936:	0f 84 b4 00 00 00    	je     801039f0 <pipealloc+0x100>
    goto bad;
  p->readopen = 1;
8010393c:	c7 80 3c 02 00 00 01 	movl   $0x1,0x23c(%eax)
80103943:	00 00 00 
  p->writeopen = 1;
  p->nwrite = 0;
  p->nread = 0;
  initlock(&p->lock, "pipe");
80103946:	83 ec 08             	sub    $0x8,%esp
  p->writeopen = 1;
80103949:	c7 80 40 02 00 00 01 	movl   $0x1,0x240(%eax)
80103950:	00 00 00 
  p->nwrite = 0;
80103953:	c7 80 38 02 00 00 00 	movl   $0x0,0x238(%eax)
8010395a:	00 00 00 
  p->nread = 0;
8010395d:	c7 80 34 02 00 00 00 	movl   $0x0,0x234(%eax)
80103964:	00 00 00 
  initlock(&p->lock, "pipe");
80103967:	68 3b 91 10 80       	push   $0x8010913b
8010396c:	50                   	push   %eax
8010396d:	e8 1e 21 00 00       	call   80105a90 <initlock>
  (*f0)->type = FD_PIPE;
80103972:	8b 03                	mov    (%ebx),%eax
  (*f0)->pipe = p;
  (*f1)->type = FD_PIPE;
  (*f1)->readable = 0;
  (*f1)->writable = 1;
  (*f1)->pipe = p;
  return 0;
80103974:	83 c4 10             	add    $0x10,%esp
  (*f0)->type = FD_PIPE;
80103977:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f0)->readable = 1;
8010397d:	8b 03                	mov    (%ebx),%eax
8010397f:	c6 40 08 01          	movb   $0x1,0x8(%eax)
  (*f0)->writable = 0;
80103983:	8b 03                	mov    (%ebx),%eax
80103985:	c6 40 09 00          	movb   $0x0,0x9(%eax)
  (*f0)->pipe = p;
80103989:	8b 03                	mov    (%ebx),%eax
8010398b:	89 78 0c             	mov    %edi,0xc(%eax)
  (*f1)->type = FD_PIPE;
8010398e:	8b 06                	mov    (%esi),%eax
80103990:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f1)->readable = 0;
80103996:	8b 06                	mov    (%esi),%eax
80103998:	c6 40 08 00          	movb   $0x0,0x8(%eax)
  (*f1)->writable = 1;
8010399c:	8b 06                	mov    (%esi),%eax
8010399e:	c6 40 09 01          	movb   $0x1,0x9(%eax)
  (*f1)->pipe = p;
801039a2:	8b 06                	mov    (%esi),%eax
801039a4:	89 78 0c             	mov    %edi,0xc(%eax)
  if(*f0)
    fileclose(*f0);
  if(*f1)
    fileclose(*f1);
  return -1;
}
801039a7:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
801039aa:	31 c0                	xor    %eax,%eax
}
801039ac:	5b                   	pop    %ebx
801039ad:	5e                   	pop    %esi
801039ae:	5f                   	pop    %edi
801039af:	5d                   	pop    %ebp
801039b0:	c3                   	ret    
801039b1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  if(*f0)
801039b8:	8b 03                	mov    (%ebx),%eax
801039ba:	85 c0                	test   %eax,%eax
801039bc:	74 1e                	je     801039dc <pipealloc+0xec>
    fileclose(*f0);
801039be:	83 ec 0c             	sub    $0xc,%esp
801039c1:	50                   	push   %eax
801039c2:	e8 e9 d9 ff ff       	call   801013b0 <fileclose>
801039c7:	83 c4 10             	add    $0x10,%esp
  if(*f1)
801039ca:	8b 06                	mov    (%esi),%eax
801039cc:	85 c0                	test   %eax,%eax
801039ce:	74 0c                	je     801039dc <pipealloc+0xec>
    fileclose(*f1);
801039d0:	83 ec 0c             	sub    $0xc,%esp
801039d3:	50                   	push   %eax
801039d4:	e8 d7 d9 ff ff       	call   801013b0 <fileclose>
801039d9:	83 c4 10             	add    $0x10,%esp
}
801039dc:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return -1;
801039df:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801039e4:	5b                   	pop    %ebx
801039e5:	5e                   	pop    %esi
801039e6:	5f                   	pop    %edi
801039e7:	5d                   	pop    %ebp
801039e8:	c3                   	ret    
801039e9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  if(*f0)
801039f0:	8b 03                	mov    (%ebx),%eax
801039f2:	85 c0                	test   %eax,%eax
801039f4:	75 c8                	jne    801039be <pipealloc+0xce>
801039f6:	eb d2                	jmp    801039ca <pipealloc+0xda>
801039f8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801039ff:	90                   	nop

80103a00 <pipeclose>:

void
pipeclose(struct pipe *p, int writable)
{
80103a00:	f3 0f 1e fb          	endbr32 
80103a04:	55                   	push   %ebp
80103a05:	89 e5                	mov    %esp,%ebp
80103a07:	56                   	push   %esi
80103a08:	53                   	push   %ebx
80103a09:	8b 5d 08             	mov    0x8(%ebp),%ebx
80103a0c:	8b 75 0c             	mov    0xc(%ebp),%esi
  acquire(&p->lock);
80103a0f:	83 ec 0c             	sub    $0xc,%esp
80103a12:	53                   	push   %ebx
80103a13:	e8 f8 21 00 00       	call   80105c10 <acquire>
  if(writable){
80103a18:	83 c4 10             	add    $0x10,%esp
80103a1b:	85 f6                	test   %esi,%esi
80103a1d:	74 41                	je     80103a60 <pipeclose+0x60>
    p->writeopen = 0;
    wakeup(&p->nread);
80103a1f:	83 ec 0c             	sub    $0xc,%esp
80103a22:	8d 83 34 02 00 00    	lea    0x234(%ebx),%eax
    p->writeopen = 0;
80103a28:	c7 83 40 02 00 00 00 	movl   $0x0,0x240(%ebx)
80103a2f:	00 00 00 
    wakeup(&p->nread);
80103a32:	50                   	push   %eax
80103a33:	e8 28 0c 00 00       	call   80104660 <wakeup>
80103a38:	83 c4 10             	add    $0x10,%esp
  } else {
    p->readopen = 0;
    wakeup(&p->nwrite);
  }
  if(p->readopen == 0 && p->writeopen == 0){
80103a3b:	8b 93 3c 02 00 00    	mov    0x23c(%ebx),%edx
80103a41:	85 d2                	test   %edx,%edx
80103a43:	75 0a                	jne    80103a4f <pipeclose+0x4f>
80103a45:	8b 83 40 02 00 00    	mov    0x240(%ebx),%eax
80103a4b:	85 c0                	test   %eax,%eax
80103a4d:	74 31                	je     80103a80 <pipeclose+0x80>
    release(&p->lock);
    kfree((char*)p);
  } else
    release(&p->lock);
80103a4f:	89 5d 08             	mov    %ebx,0x8(%ebp)
}
80103a52:	8d 65 f8             	lea    -0x8(%ebp),%esp
80103a55:	5b                   	pop    %ebx
80103a56:	5e                   	pop    %esi
80103a57:	5d                   	pop    %ebp
    release(&p->lock);
80103a58:	e9 73 22 00 00       	jmp    80105cd0 <release>
80103a5d:	8d 76 00             	lea    0x0(%esi),%esi
    wakeup(&p->nwrite);
80103a60:	83 ec 0c             	sub    $0xc,%esp
80103a63:	8d 83 38 02 00 00    	lea    0x238(%ebx),%eax
    p->readopen = 0;
80103a69:	c7 83 3c 02 00 00 00 	movl   $0x0,0x23c(%ebx)
80103a70:	00 00 00 
    wakeup(&p->nwrite);
80103a73:	50                   	push   %eax
80103a74:	e8 e7 0b 00 00       	call   80104660 <wakeup>
80103a79:	83 c4 10             	add    $0x10,%esp
80103a7c:	eb bd                	jmp    80103a3b <pipeclose+0x3b>
80103a7e:	66 90                	xchg   %ax,%ax
    release(&p->lock);
80103a80:	83 ec 0c             	sub    $0xc,%esp
80103a83:	53                   	push   %ebx
80103a84:	e8 47 22 00 00       	call   80105cd0 <release>
    kfree((char*)p);
80103a89:	89 5d 08             	mov    %ebx,0x8(%ebp)
80103a8c:	83 c4 10             	add    $0x10,%esp
}
80103a8f:	8d 65 f8             	lea    -0x8(%ebp),%esp
80103a92:	5b                   	pop    %ebx
80103a93:	5e                   	pop    %esi
80103a94:	5d                   	pop    %ebp
    kfree((char*)p);
80103a95:	e9 c6 ee ff ff       	jmp    80102960 <kfree>
80103a9a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80103aa0 <pipewrite>:

//PAGEBREAK: 40
int
pipewrite(struct pipe *p, char *addr, int n)
{
80103aa0:	f3 0f 1e fb          	endbr32 
80103aa4:	55                   	push   %ebp
80103aa5:	89 e5                	mov    %esp,%ebp
80103aa7:	57                   	push   %edi
80103aa8:	56                   	push   %esi
80103aa9:	53                   	push   %ebx
80103aaa:	83 ec 28             	sub    $0x28,%esp
80103aad:	8b 5d 08             	mov    0x8(%ebp),%ebx
  int i;

  acquire(&p->lock);
80103ab0:	53                   	push   %ebx
80103ab1:	e8 5a 21 00 00       	call   80105c10 <acquire>
  for(i = 0; i < n; i++){
80103ab6:	8b 45 10             	mov    0x10(%ebp),%eax
80103ab9:	83 c4 10             	add    $0x10,%esp
80103abc:	85 c0                	test   %eax,%eax
80103abe:	0f 8e bc 00 00 00    	jle    80103b80 <pipewrite+0xe0>
80103ac4:	8b 45 0c             	mov    0xc(%ebp),%eax
80103ac7:	8b 8b 38 02 00 00    	mov    0x238(%ebx),%ecx
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
      if(p->readopen == 0 || myproc()->killed){
        release(&p->lock);
        return -1;
      }
      wakeup(&p->nread);
80103acd:	8d bb 34 02 00 00    	lea    0x234(%ebx),%edi
80103ad3:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80103ad6:	03 45 10             	add    0x10(%ebp),%eax
80103ad9:	89 45 e0             	mov    %eax,-0x20(%ebp)
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80103adc:	8b 83 34 02 00 00    	mov    0x234(%ebx),%eax
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
80103ae2:	8d b3 38 02 00 00    	lea    0x238(%ebx),%esi
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80103ae8:	89 ca                	mov    %ecx,%edx
80103aea:	05 00 02 00 00       	add    $0x200,%eax
80103aef:	39 c1                	cmp    %eax,%ecx
80103af1:	74 3b                	je     80103b2e <pipewrite+0x8e>
80103af3:	eb 63                	jmp    80103b58 <pipewrite+0xb8>
80103af5:	8d 76 00             	lea    0x0(%esi),%esi
      if(p->readopen == 0 || myproc()->killed){
80103af8:	e8 c3 03 00 00       	call   80103ec0 <myproc>
80103afd:	8b 48 24             	mov    0x24(%eax),%ecx
80103b00:	85 c9                	test   %ecx,%ecx
80103b02:	75 34                	jne    80103b38 <pipewrite+0x98>
      wakeup(&p->nread);
80103b04:	83 ec 0c             	sub    $0xc,%esp
80103b07:	57                   	push   %edi
80103b08:	e8 53 0b 00 00       	call   80104660 <wakeup>
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
80103b0d:	58                   	pop    %eax
80103b0e:	5a                   	pop    %edx
80103b0f:	53                   	push   %ebx
80103b10:	56                   	push   %esi
80103b11:	e8 8a 09 00 00       	call   801044a0 <sleep>
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80103b16:	8b 83 34 02 00 00    	mov    0x234(%ebx),%eax
80103b1c:	8b 93 38 02 00 00    	mov    0x238(%ebx),%edx
80103b22:	83 c4 10             	add    $0x10,%esp
80103b25:	05 00 02 00 00       	add    $0x200,%eax
80103b2a:	39 c2                	cmp    %eax,%edx
80103b2c:	75 2a                	jne    80103b58 <pipewrite+0xb8>
      if(p->readopen == 0 || myproc()->killed){
80103b2e:	8b 83 3c 02 00 00    	mov    0x23c(%ebx),%eax
80103b34:	85 c0                	test   %eax,%eax
80103b36:	75 c0                	jne    80103af8 <pipewrite+0x58>
        release(&p->lock);
80103b38:	83 ec 0c             	sub    $0xc,%esp
80103b3b:	53                   	push   %ebx
80103b3c:	e8 8f 21 00 00       	call   80105cd0 <release>
        return -1;
80103b41:	83 c4 10             	add    $0x10,%esp
80103b44:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
  }
  wakeup(&p->nread);  //DOC: pipewrite-wakeup1
  release(&p->lock);
  return n;
}
80103b49:	8d 65 f4             	lea    -0xc(%ebp),%esp
80103b4c:	5b                   	pop    %ebx
80103b4d:	5e                   	pop    %esi
80103b4e:	5f                   	pop    %edi
80103b4f:	5d                   	pop    %ebp
80103b50:	c3                   	ret    
80103b51:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
80103b58:	8b 75 e4             	mov    -0x1c(%ebp),%esi
80103b5b:	8d 4a 01             	lea    0x1(%edx),%ecx
80103b5e:	81 e2 ff 01 00 00    	and    $0x1ff,%edx
80103b64:	89 8b 38 02 00 00    	mov    %ecx,0x238(%ebx)
80103b6a:	0f b6 06             	movzbl (%esi),%eax
80103b6d:	83 c6 01             	add    $0x1,%esi
80103b70:	89 75 e4             	mov    %esi,-0x1c(%ebp)
80103b73:	88 44 13 34          	mov    %al,0x34(%ebx,%edx,1)
  for(i = 0; i < n; i++){
80103b77:	3b 75 e0             	cmp    -0x20(%ebp),%esi
80103b7a:	0f 85 5c ff ff ff    	jne    80103adc <pipewrite+0x3c>
  wakeup(&p->nread);  //DOC: pipewrite-wakeup1
80103b80:	83 ec 0c             	sub    $0xc,%esp
80103b83:	8d 83 34 02 00 00    	lea    0x234(%ebx),%eax
80103b89:	50                   	push   %eax
80103b8a:	e8 d1 0a 00 00       	call   80104660 <wakeup>
  release(&p->lock);
80103b8f:	89 1c 24             	mov    %ebx,(%esp)
80103b92:	e8 39 21 00 00       	call   80105cd0 <release>
  return n;
80103b97:	8b 45 10             	mov    0x10(%ebp),%eax
80103b9a:	83 c4 10             	add    $0x10,%esp
80103b9d:	eb aa                	jmp    80103b49 <pipewrite+0xa9>
80103b9f:	90                   	nop

80103ba0 <piperead>:

int
piperead(struct pipe *p, char *addr, int n)
{
80103ba0:	f3 0f 1e fb          	endbr32 
80103ba4:	55                   	push   %ebp
80103ba5:	89 e5                	mov    %esp,%ebp
80103ba7:	57                   	push   %edi
80103ba8:	56                   	push   %esi
80103ba9:	53                   	push   %ebx
80103baa:	83 ec 18             	sub    $0x18,%esp
80103bad:	8b 75 08             	mov    0x8(%ebp),%esi
80103bb0:	8b 7d 0c             	mov    0xc(%ebp),%edi
  int i;

  acquire(&p->lock);
80103bb3:	56                   	push   %esi
80103bb4:	8d 9e 34 02 00 00    	lea    0x234(%esi),%ebx
80103bba:	e8 51 20 00 00       	call   80105c10 <acquire>
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
80103bbf:	8b 86 34 02 00 00    	mov    0x234(%esi),%eax
80103bc5:	83 c4 10             	add    $0x10,%esp
80103bc8:	39 86 38 02 00 00    	cmp    %eax,0x238(%esi)
80103bce:	74 33                	je     80103c03 <piperead+0x63>
80103bd0:	eb 3b                	jmp    80103c0d <piperead+0x6d>
80103bd2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    if(myproc()->killed){
80103bd8:	e8 e3 02 00 00       	call   80103ec0 <myproc>
80103bdd:	8b 48 24             	mov    0x24(%eax),%ecx
80103be0:	85 c9                	test   %ecx,%ecx
80103be2:	0f 85 88 00 00 00    	jne    80103c70 <piperead+0xd0>
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
80103be8:	83 ec 08             	sub    $0x8,%esp
80103beb:	56                   	push   %esi
80103bec:	53                   	push   %ebx
80103bed:	e8 ae 08 00 00       	call   801044a0 <sleep>
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
80103bf2:	8b 86 38 02 00 00    	mov    0x238(%esi),%eax
80103bf8:	83 c4 10             	add    $0x10,%esp
80103bfb:	39 86 34 02 00 00    	cmp    %eax,0x234(%esi)
80103c01:	75 0a                	jne    80103c0d <piperead+0x6d>
80103c03:	8b 86 40 02 00 00    	mov    0x240(%esi),%eax
80103c09:	85 c0                	test   %eax,%eax
80103c0b:	75 cb                	jne    80103bd8 <piperead+0x38>
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
80103c0d:	8b 55 10             	mov    0x10(%ebp),%edx
80103c10:	31 db                	xor    %ebx,%ebx
80103c12:	85 d2                	test   %edx,%edx
80103c14:	7f 28                	jg     80103c3e <piperead+0x9e>
80103c16:	eb 34                	jmp    80103c4c <piperead+0xac>
80103c18:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103c1f:	90                   	nop
    if(p->nread == p->nwrite)
      break;
    addr[i] = p->data[p->nread++ % PIPESIZE];
80103c20:	8d 48 01             	lea    0x1(%eax),%ecx
80103c23:	25 ff 01 00 00       	and    $0x1ff,%eax
80103c28:	89 8e 34 02 00 00    	mov    %ecx,0x234(%esi)
80103c2e:	0f b6 44 06 34       	movzbl 0x34(%esi,%eax,1),%eax
80103c33:	88 04 1f             	mov    %al,(%edi,%ebx,1)
  for(i = 0; i < n; i++){  //DOC: piperead-copy
80103c36:	83 c3 01             	add    $0x1,%ebx
80103c39:	39 5d 10             	cmp    %ebx,0x10(%ebp)
80103c3c:	74 0e                	je     80103c4c <piperead+0xac>
    if(p->nread == p->nwrite)
80103c3e:	8b 86 34 02 00 00    	mov    0x234(%esi),%eax
80103c44:	3b 86 38 02 00 00    	cmp    0x238(%esi),%eax
80103c4a:	75 d4                	jne    80103c20 <piperead+0x80>
  }
  wakeup(&p->nwrite);  //DOC: piperead-wakeup
80103c4c:	83 ec 0c             	sub    $0xc,%esp
80103c4f:	8d 86 38 02 00 00    	lea    0x238(%esi),%eax
80103c55:	50                   	push   %eax
80103c56:	e8 05 0a 00 00       	call   80104660 <wakeup>
  release(&p->lock);
80103c5b:	89 34 24             	mov    %esi,(%esp)
80103c5e:	e8 6d 20 00 00       	call   80105cd0 <release>
  return i;
80103c63:	83 c4 10             	add    $0x10,%esp
}
80103c66:	8d 65 f4             	lea    -0xc(%ebp),%esp
80103c69:	89 d8                	mov    %ebx,%eax
80103c6b:	5b                   	pop    %ebx
80103c6c:	5e                   	pop    %esi
80103c6d:	5f                   	pop    %edi
80103c6e:	5d                   	pop    %ebp
80103c6f:	c3                   	ret    
      release(&p->lock);
80103c70:	83 ec 0c             	sub    $0xc,%esp
      return -1;
80103c73:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
      release(&p->lock);
80103c78:	56                   	push   %esi
80103c79:	e8 52 20 00 00       	call   80105cd0 <release>
      return -1;
80103c7e:	83 c4 10             	add    $0x10,%esp
}
80103c81:	8d 65 f4             	lea    -0xc(%ebp),%esp
80103c84:	89 d8                	mov    %ebx,%eax
80103c86:	5b                   	pop    %ebx
80103c87:	5e                   	pop    %esi
80103c88:	5f                   	pop    %edi
80103c89:	5d                   	pop    %ebp
80103c8a:	c3                   	ret    
80103c8b:	66 90                	xchg   %ax,%ax
80103c8d:	66 90                	xchg   %ax,%ax
80103c8f:	90                   	nop

80103c90 <allocproc>:
// If found, change state to EMBRYO and initialize
// state required to run in the kernel.
// Otherwise return 0.
static struct proc*
allocproc(void)
{
80103c90:	55                   	push   %ebp
80103c91:	89 e5                	mov    %esp,%ebp
80103c93:	53                   	push   %ebx
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80103c94:	bb 54 53 11 80       	mov    $0x80115354,%ebx
{
80103c99:	83 ec 10             	sub    $0x10,%esp
  acquire(&ptable.lock);
80103c9c:	68 20 53 11 80       	push   $0x80115320
80103ca1:	e8 6a 1f 00 00       	call   80105c10 <acquire>
80103ca6:	83 c4 10             	add    $0x10,%esp
80103ca9:	eb 17                	jmp    80103cc2 <allocproc+0x32>
80103cab:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80103caf:	90                   	nop
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80103cb0:	81 c3 ac 00 00 00    	add    $0xac,%ebx
80103cb6:	81 fb 54 7e 11 80    	cmp    $0x80117e54,%ebx
80103cbc:	0f 84 ce 00 00 00    	je     80103d90 <allocproc+0x100>
    if(p->state == UNUSED)
80103cc2:	8b 43 0c             	mov    0xc(%ebx),%eax
80103cc5:	85 c0                	test   %eax,%eax
80103cc7:	75 e7                	jne    80103cb0 <allocproc+0x20>
  release(&ptable.lock);
  return 0;

found:
  p->state = EMBRYO;
  p->pid = nextpid++;
80103cc9:	a1 04 c0 10 80       	mov    0x8010c004,%eax

  release(&ptable.lock);
80103cce:	83 ec 0c             	sub    $0xc,%esp
  p->state = EMBRYO;
80103cd1:	c7 43 0c 01 00 00 00 	movl   $0x1,0xc(%ebx)
  p->pid = nextpid++;
80103cd8:	89 43 10             	mov    %eax,0x10(%ebx)
80103cdb:	8d 50 01             	lea    0x1(%eax),%edx
  release(&ptable.lock);
80103cde:	68 20 53 11 80       	push   $0x80115320
  p->pid = nextpid++;
80103ce3:	89 15 04 c0 10 80    	mov    %edx,0x8010c004
  release(&ptable.lock);
80103ce9:	e8 e2 1f 00 00       	call   80105cd0 <release>

  // Allocate kernel stack.
  if((p->kstack = kalloc()) == 0){
80103cee:	e8 2d ee ff ff       	call   80102b20 <kalloc>
80103cf3:	83 c4 10             	add    $0x10,%esp
80103cf6:	89 43 08             	mov    %eax,0x8(%ebx)
80103cf9:	85 c0                	test   %eax,%eax
80103cfb:	0f 84 a8 00 00 00    	je     80103da9 <allocproc+0x119>
    return 0;
  }
  sp = p->kstack + KSTACKSIZE;

  // Leave room for trap frame.
  sp -= sizeof *p->tf;
80103d01:	8d 90 b4 0f 00 00    	lea    0xfb4(%eax),%edx
  sp -= 4;
  *(uint*)sp = (uint)trapret;

  sp -= sizeof *p->context;
  p->context = (struct context*)sp;
  memset(p->context, 0, sizeof *p->context);
80103d07:	83 ec 04             	sub    $0x4,%esp
  sp -= sizeof *p->context;
80103d0a:	05 9c 0f 00 00       	add    $0xf9c,%eax
  sp -= sizeof *p->tf;
80103d0f:	89 53 18             	mov    %edx,0x18(%ebx)
  *(uint*)sp = (uint)trapret;
80103d12:	c7 40 14 27 73 10 80 	movl   $0x80107327,0x14(%eax)
  p->context = (struct context*)sp;
80103d19:	89 43 1c             	mov    %eax,0x1c(%ebx)
  memset(p->context, 0, sizeof *p->context);
80103d1c:	6a 14                	push   $0x14
80103d1e:	6a 00                	push   $0x0
80103d20:	50                   	push   %eax
80103d21:	e8 fa 1f 00 00       	call   80105d20 <memset>
  p->context->eip = (uint)forkret;
80103d26:	8b 43 1c             	mov    0x1c(%ebx),%eax
  p->sched_info.bjf.executed_cycle = 0;
  p->sched_info.bjf.executed_cycle_ratio = 1;
  p->sched_info.bjf.process_size = p->sz;
  p->sched_info.bjf.process_size_ratio = 1;
  p->start_time = ticks;
  return p;
80103d29:	83 c4 10             	add    $0x10,%esp
  p->sched_info.bjf.priority_ratio = 1;
80103d2c:	d9 e8                	fld1   
  p->context->eip = (uint)forkret;
80103d2e:	c7 40 10 c0 3d 10 80 	movl   $0x80103dc0,0x10(%eax)
  p->sched_info.bjf.arrival_time = ticks;
80103d35:	a1 a0 86 11 80       	mov    0x801186a0,%eax
  p->sched_info.bjf.priority_ratio = 1;
80103d3a:	d9 93 8c 00 00 00    	fsts   0x8c(%ebx)
  p->sched_info.bjf.process_size = p->sz;
80103d40:	8b 13                	mov    (%ebx),%edx
  p->sched_info.bjf.arrival_time_ratio = 1;
80103d42:	d9 93 94 00 00 00    	fsts   0x94(%ebx)
  p->sched_info.bjf.executed_cycle_ratio = 1;
80103d48:	d9 93 9c 00 00 00    	fsts   0x9c(%ebx)
  p->sched_info.bjf.arrival_time = ticks;
80103d4e:	89 83 90 00 00 00    	mov    %eax,0x90(%ebx)
  p->start_time = ticks;
80103d54:	89 43 7c             	mov    %eax,0x7c(%ebx)
}
80103d57:	89 d8                	mov    %ebx,%eax
  p->sched_info.queue = UNSET;
80103d59:	c7 83 80 00 00 00 00 	movl   $0x0,0x80(%ebx)
80103d60:	00 00 00 
  p->sched_info.bjf.priority = 3;
80103d63:	c7 83 88 00 00 00 03 	movl   $0x3,0x88(%ebx)
80103d6a:	00 00 00 
  p->sched_info.bjf.executed_cycle = 0;
80103d6d:	c7 83 98 00 00 00 00 	movl   $0x0,0x98(%ebx)
80103d74:	00 00 00 
  p->sched_info.bjf.process_size = p->sz;
80103d77:	89 93 a0 00 00 00    	mov    %edx,0xa0(%ebx)
  p->sched_info.bjf.process_size_ratio = 1;
80103d7d:	d9 9b a4 00 00 00    	fstps  0xa4(%ebx)
}
80103d83:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80103d86:	c9                   	leave  
80103d87:	c3                   	ret    
80103d88:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103d8f:	90                   	nop
  release(&ptable.lock);
80103d90:	83 ec 0c             	sub    $0xc,%esp
  return 0;
80103d93:	31 db                	xor    %ebx,%ebx
  release(&ptable.lock);
80103d95:	68 20 53 11 80       	push   $0x80115320
80103d9a:	e8 31 1f 00 00       	call   80105cd0 <release>
}
80103d9f:	89 d8                	mov    %ebx,%eax
  return 0;
80103da1:	83 c4 10             	add    $0x10,%esp
}
80103da4:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80103da7:	c9                   	leave  
80103da8:	c3                   	ret    
    p->state = UNUSED;
80103da9:	c7 43 0c 00 00 00 00 	movl   $0x0,0xc(%ebx)
    return 0;
80103db0:	31 db                	xor    %ebx,%ebx
}
80103db2:	89 d8                	mov    %ebx,%eax
80103db4:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80103db7:	c9                   	leave  
80103db8:	c3                   	ret    
80103db9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80103dc0 <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch here.  "Return" to user space.
void
forkret(void)
{
80103dc0:	f3 0f 1e fb          	endbr32 
80103dc4:	55                   	push   %ebp
80103dc5:	89 e5                	mov    %esp,%ebp
80103dc7:	83 ec 14             	sub    $0x14,%esp
  static int first = 1;
  // Still holding ptable.lock from scheduler.
  release(&ptable.lock);
80103dca:	68 20 53 11 80       	push   $0x80115320
80103dcf:	e8 fc 1e 00 00       	call   80105cd0 <release>

  if (first) {
80103dd4:	a1 00 c0 10 80       	mov    0x8010c000,%eax
80103dd9:	83 c4 10             	add    $0x10,%esp
80103ddc:	85 c0                	test   %eax,%eax
80103dde:	75 08                	jne    80103de8 <forkret+0x28>
    iinit(ROOTDEV);
    initlog(ROOTDEV);
  }

  // Return to "caller", actually trapret (see allocproc).
}
80103de0:	c9                   	leave  
80103de1:	c3                   	ret    
80103de2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    first = 0;
80103de8:	c7 05 00 c0 10 80 00 	movl   $0x0,0x8010c000
80103def:	00 00 00 
    iinit(ROOTDEV);
80103df2:	83 ec 0c             	sub    $0xc,%esp
80103df5:	6a 01                	push   $0x1
80103df7:	e8 34 dc ff ff       	call   80101a30 <iinit>
    initlog(ROOTDEV);
80103dfc:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80103e03:	e8 78 f3 ff ff       	call   80103180 <initlog>
}
80103e08:	83 c4 10             	add    $0x10,%esp
80103e0b:	c9                   	leave  
80103e0c:	c3                   	ret    
80103e0d:	8d 76 00             	lea    0x0(%esi),%esi

80103e10 <pinit>:
{
80103e10:	f3 0f 1e fb          	endbr32 
80103e14:	55                   	push   %ebp
80103e15:	89 e5                	mov    %esp,%ebp
80103e17:	83 ec 10             	sub    $0x10,%esp
  initlock(&ptable.lock, "ptable");
80103e1a:	68 40 91 10 80       	push   $0x80109140
80103e1f:	68 20 53 11 80       	push   $0x80115320
80103e24:	e8 67 1c 00 00       	call   80105a90 <initlock>
}
80103e29:	83 c4 10             	add    $0x10,%esp
80103e2c:	c9                   	leave  
80103e2d:	c3                   	ret    
80103e2e:	66 90                	xchg   %ax,%ax

80103e30 <mycpu>:
{
80103e30:	f3 0f 1e fb          	endbr32 
80103e34:	55                   	push   %ebp
80103e35:	89 e5                	mov    %esp,%ebp
80103e37:	56                   	push   %esi
80103e38:	53                   	push   %ebx
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80103e39:	9c                   	pushf  
80103e3a:	58                   	pop    %eax
  if(readeflags()&FL_IF)
80103e3b:	f6 c4 02             	test   $0x2,%ah
80103e3e:	75 4a                	jne    80103e8a <mycpu+0x5a>
  apicid = lapicid();
80103e40:	e8 4b ef ff ff       	call   80102d90 <lapicid>
  for (i = 0; i < ncpu; ++i) {
80103e45:	8b 35 00 53 11 80    	mov    0x80115300,%esi
  apicid = lapicid();
80103e4b:	89 c3                	mov    %eax,%ebx
  for (i = 0; i < ncpu; ++i) {
80103e4d:	85 f6                	test   %esi,%esi
80103e4f:	7e 2c                	jle    80103e7d <mycpu+0x4d>
80103e51:	31 d2                	xor    %edx,%edx
80103e53:	eb 0a                	jmp    80103e5f <mycpu+0x2f>
80103e55:	8d 76 00             	lea    0x0(%esi),%esi
80103e58:	83 c2 01             	add    $0x1,%edx
80103e5b:	39 f2                	cmp    %esi,%edx
80103e5d:	74 1e                	je     80103e7d <mycpu+0x4d>
    if (cpus[i].apicid == apicid)
80103e5f:	69 ca b0 00 00 00    	imul   $0xb0,%edx,%ecx
80103e65:	0f b6 81 80 4d 11 80 	movzbl -0x7feeb280(%ecx),%eax
80103e6c:	39 d8                	cmp    %ebx,%eax
80103e6e:	75 e8                	jne    80103e58 <mycpu+0x28>
}
80103e70:	8d 65 f8             	lea    -0x8(%ebp),%esp
      return &cpus[i];
80103e73:	8d 81 80 4d 11 80    	lea    -0x7feeb280(%ecx),%eax
}
80103e79:	5b                   	pop    %ebx
80103e7a:	5e                   	pop    %esi
80103e7b:	5d                   	pop    %ebp
80103e7c:	c3                   	ret    
  panic("unknown apicid\n");
80103e7d:	83 ec 0c             	sub    $0xc,%esp
80103e80:	68 47 91 10 80       	push   $0x80109147
80103e85:	e8 06 c5 ff ff       	call   80100390 <panic>
    panic("mycpu called with interrupts enabled\n");
80103e8a:	83 ec 0c             	sub    $0xc,%esp
80103e8d:	68 a0 92 10 80       	push   $0x801092a0
80103e92:	e8 f9 c4 ff ff       	call   80100390 <panic>
80103e97:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103e9e:	66 90                	xchg   %ax,%ax

80103ea0 <cpuid>:
cpuid() {
80103ea0:	f3 0f 1e fb          	endbr32 
80103ea4:	55                   	push   %ebp
80103ea5:	89 e5                	mov    %esp,%ebp
80103ea7:	83 ec 08             	sub    $0x8,%esp
  return mycpu()-cpus;
80103eaa:	e8 81 ff ff ff       	call   80103e30 <mycpu>
}
80103eaf:	c9                   	leave  
  return mycpu()-cpus;
80103eb0:	2d 80 4d 11 80       	sub    $0x80114d80,%eax
80103eb5:	c1 f8 04             	sar    $0x4,%eax
80103eb8:	69 c0 a3 8b 2e ba    	imul   $0xba2e8ba3,%eax,%eax
}
80103ebe:	c3                   	ret    
80103ebf:	90                   	nop

80103ec0 <myproc>:
myproc(void) {
80103ec0:	f3 0f 1e fb          	endbr32 
80103ec4:	55                   	push   %ebp
80103ec5:	89 e5                	mov    %esp,%ebp
80103ec7:	53                   	push   %ebx
80103ec8:	83 ec 04             	sub    $0x4,%esp
  pushcli();
80103ecb:	e8 40 1c 00 00       	call   80105b10 <pushcli>
  c = mycpu();
80103ed0:	e8 5b ff ff ff       	call   80103e30 <mycpu>
  p = c->proc;
80103ed5:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
80103edb:	e8 80 1c 00 00       	call   80105b60 <popcli>
}
80103ee0:	83 c4 04             	add    $0x4,%esp
80103ee3:	89 d8                	mov    %ebx,%eax
80103ee5:	5b                   	pop    %ebx
80103ee6:	5d                   	pop    %ebp
80103ee7:	c3                   	ret    
80103ee8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103eef:	90                   	nop

80103ef0 <growproc>:
{
80103ef0:	f3 0f 1e fb          	endbr32 
80103ef4:	55                   	push   %ebp
80103ef5:	89 e5                	mov    %esp,%ebp
80103ef7:	56                   	push   %esi
80103ef8:	53                   	push   %ebx
80103ef9:	8b 75 08             	mov    0x8(%ebp),%esi
  pushcli();
80103efc:	e8 0f 1c 00 00       	call   80105b10 <pushcli>
  c = mycpu();
80103f01:	e8 2a ff ff ff       	call   80103e30 <mycpu>
  p = c->proc;
80103f06:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
80103f0c:	e8 4f 1c 00 00       	call   80105b60 <popcli>
  sz = curproc->sz;
80103f11:	8b 03                	mov    (%ebx),%eax
  if(n > 0){
80103f13:	85 f6                	test   %esi,%esi
80103f15:	7f 19                	jg     80103f30 <growproc+0x40>
  } else if(n < 0){
80103f17:	75 37                	jne    80103f50 <growproc+0x60>
  switchuvm(curproc);
80103f19:	83 ec 0c             	sub    $0xc,%esp
  curproc->sz = sz;
80103f1c:	89 03                	mov    %eax,(%ebx)
  switchuvm(curproc);
80103f1e:	53                   	push   %ebx
80103f1f:	e8 9c 45 00 00       	call   801084c0 <switchuvm>
  return 0;
80103f24:	83 c4 10             	add    $0x10,%esp
80103f27:	31 c0                	xor    %eax,%eax
}
80103f29:	8d 65 f8             	lea    -0x8(%ebp),%esp
80103f2c:	5b                   	pop    %ebx
80103f2d:	5e                   	pop    %esi
80103f2e:	5d                   	pop    %ebp
80103f2f:	c3                   	ret    
    if((sz = allocuvm(curproc->pgdir, sz, sz + n)) == 0)
80103f30:	83 ec 04             	sub    $0x4,%esp
80103f33:	01 c6                	add    %eax,%esi
80103f35:	56                   	push   %esi
80103f36:	50                   	push   %eax
80103f37:	ff 73 04             	pushl  0x4(%ebx)
80103f3a:	e8 e1 47 00 00       	call   80108720 <allocuvm>
80103f3f:	83 c4 10             	add    $0x10,%esp
80103f42:	85 c0                	test   %eax,%eax
80103f44:	75 d3                	jne    80103f19 <growproc+0x29>
      return -1;
80103f46:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103f4b:	eb dc                	jmp    80103f29 <growproc+0x39>
80103f4d:	8d 76 00             	lea    0x0(%esi),%esi
    if((sz = deallocuvm(curproc->pgdir, sz, sz + n)) == 0)
80103f50:	83 ec 04             	sub    $0x4,%esp
80103f53:	01 c6                	add    %eax,%esi
80103f55:	56                   	push   %esi
80103f56:	50                   	push   %eax
80103f57:	ff 73 04             	pushl  0x4(%ebx)
80103f5a:	e8 f1 48 00 00       	call   80108850 <deallocuvm>
80103f5f:	83 c4 10             	add    $0x10,%esp
80103f62:	85 c0                	test   %eax,%eax
80103f64:	75 b3                	jne    80103f19 <growproc+0x29>
80103f66:	eb de                	jmp    80103f46 <growproc+0x56>
80103f68:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103f6f:	90                   	nop

80103f70 <bjfrank>:
{
80103f70:	f3 0f 1e fb          	endbr32 
80103f74:	55                   	push   %ebp
80103f75:	89 e5                	mov    %esp,%ebp
80103f77:	8b 45 08             	mov    0x8(%ebp),%eax
}
80103f7a:	5d                   	pop    %ebp
  return p->sched_info.bjf.priority * p->sched_info.bjf.priority_ratio +
80103f7b:	db 80 88 00 00 00    	fildl  0x88(%eax)
80103f81:	d8 88 8c 00 00 00    	fmuls  0x8c(%eax)
         p->sched_info.bjf.arrival_time * p->sched_info.bjf.arrival_time_ratio +
80103f87:	db 80 90 00 00 00    	fildl  0x90(%eax)
80103f8d:	d8 88 94 00 00 00    	fmuls  0x94(%eax)
  return p->sched_info.bjf.priority * p->sched_info.bjf.priority_ratio +
80103f93:	de c1                	faddp  %st,%st(1)
         p->sched_info.bjf.executed_cycle * p->sched_info.bjf.executed_cycle_ratio +
80103f95:	d9 80 98 00 00 00    	flds   0x98(%eax)
80103f9b:	d8 88 9c 00 00 00    	fmuls  0x9c(%eax)
         p->sched_info.bjf.arrival_time * p->sched_info.bjf.arrival_time_ratio +
80103fa1:	de c1                	faddp  %st,%st(1)
         p->sched_info.bjf.process_size * p->sched_info.bjf.process_size_ratio;
80103fa3:	db 80 a0 00 00 00    	fildl  0xa0(%eax)
80103fa9:	d8 88 a4 00 00 00    	fmuls  0xa4(%eax)
         p->sched_info.bjf.executed_cycle * p->sched_info.bjf.executed_cycle_ratio +
80103faf:	de c1                	faddp  %st,%st(1)
}
80103fb1:	c3                   	ret    
80103fb2:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103fb9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80103fc0 <lcfs>:
{
80103fc0:	f3 0f 1e fb          	endbr32 
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80103fc4:	b8 54 53 11 80       	mov    $0x80115354,%eax
  struct proc *result = 0;
80103fc9:	31 d2                	xor    %edx,%edx
80103fcb:	eb 1f                	jmp    80103fec <lcfs+0x2c>
80103fcd:	8d 76 00             	lea    0x0(%esi),%esi
      if (result->sched_info.arrival_queue_time < p->sched_info.arrival_queue_time)
80103fd0:	8b 88 a8 00 00 00    	mov    0xa8(%eax),%ecx
80103fd6:	39 8a a8 00 00 00    	cmp    %ecx,0xa8(%edx)
80103fdc:	0f 4c d0             	cmovl  %eax,%edx
80103fdf:	90                   	nop
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80103fe0:	05 ac 00 00 00       	add    $0xac,%eax
80103fe5:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
80103fea:	74 21                	je     8010400d <lcfs+0x4d>
    if (p->state != RUNNABLE || p->sched_info.queue != LCFS)
80103fec:	83 78 0c 03          	cmpl   $0x3,0xc(%eax)
80103ff0:	75 ee                	jne    80103fe0 <lcfs+0x20>
80103ff2:	83 b8 80 00 00 00 02 	cmpl   $0x2,0x80(%eax)
80103ff9:	75 e5                	jne    80103fe0 <lcfs+0x20>
    if (result != 0)
80103ffb:	85 d2                	test   %edx,%edx
80103ffd:	75 d1                	jne    80103fd0 <lcfs+0x10>
80103fff:	89 c2                	mov    %eax,%edx
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104001:	05 ac 00 00 00       	add    $0xac,%eax
80104006:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
8010400b:	75 df                	jne    80103fec <lcfs+0x2c>
}
8010400d:	89 d0                	mov    %edx,%eax
8010400f:	c3                   	ret    

80104010 <bestjobfirst>:
{
80104010:	f3 0f 1e fb          	endbr32 
  float min_rank = 2e6;
80104014:	d9 05 5c 94 10 80    	flds   0x8010945c
  struct proc *min_p = 0;
8010401a:	31 d2                	xor    %edx,%edx
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
8010401c:	b8 54 53 11 80       	mov    $0x80115354,%eax
80104021:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    if (p->state != RUNNABLE || p->sched_info.queue != BJF)
80104028:	83 78 0c 03          	cmpl   $0x3,0xc(%eax)
8010402c:	75 5a                	jne    80104088 <bestjobfirst+0x78>
8010402e:	83 b8 80 00 00 00 03 	cmpl   $0x3,0x80(%eax)
80104035:	75 51                	jne    80104088 <bestjobfirst+0x78>
  return p->sched_info.bjf.priority * p->sched_info.bjf.priority_ratio +
80104037:	db 80 88 00 00 00    	fildl  0x88(%eax)
8010403d:	d8 88 8c 00 00 00    	fmuls  0x8c(%eax)
         p->sched_info.bjf.arrival_time * p->sched_info.bjf.arrival_time_ratio +
80104043:	db 80 90 00 00 00    	fildl  0x90(%eax)
80104049:	d8 88 94 00 00 00    	fmuls  0x94(%eax)
  return p->sched_info.bjf.priority * p->sched_info.bjf.priority_ratio +
8010404f:	de c1                	faddp  %st,%st(1)
         p->sched_info.bjf.executed_cycle * p->sched_info.bjf.executed_cycle_ratio +
80104051:	d9 80 98 00 00 00    	flds   0x98(%eax)
80104057:	d8 88 9c 00 00 00    	fmuls  0x9c(%eax)
         p->sched_info.bjf.arrival_time * p->sched_info.bjf.arrival_time_ratio +
8010405d:	de c1                	faddp  %st,%st(1)
         p->sched_info.bjf.process_size * p->sched_info.bjf.process_size_ratio;
8010405f:	db 80 a0 00 00 00    	fildl  0xa0(%eax)
80104065:	d8 88 a4 00 00 00    	fmuls  0xa4(%eax)
         p->sched_info.bjf.executed_cycle * p->sched_info.bjf.executed_cycle_ratio +
8010406b:	de c1                	faddp  %st,%st(1)
8010406d:	d9 c9                	fxch   %st(1)
    if (p_rank < min_rank)
8010406f:	db f1                	fcomi  %st(1),%st
80104071:	76 0d                	jbe    80104080 <bestjobfirst+0x70>
80104073:	dd d8                	fstp   %st(0)
80104075:	89 c2                	mov    %eax,%edx
80104077:	eb 0f                	jmp    80104088 <bestjobfirst+0x78>
80104079:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104080:	dd d9                	fstp   %st(1)
80104082:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104088:	05 ac 00 00 00       	add    $0xac,%eax
8010408d:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
80104092:	75 94                	jne    80104028 <bestjobfirst+0x18>
80104094:	dd d8                	fstp   %st(0)
}
80104096:	89 d0                	mov    %edx,%eax
80104098:	c3                   	ret    
80104099:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

801040a0 <scheduler>:
{
801040a0:	f3 0f 1e fb          	endbr32 
801040a4:	55                   	push   %ebp
801040a5:	89 e5                	mov    %esp,%ebp
801040a7:	57                   	push   %edi
      p = ptable.proc;
801040a8:	bf 54 53 11 80       	mov    $0x80115354,%edi
{
801040ad:	56                   	push   %esi
  struct proc *last_scheduled_RR = &ptable.proc[NPROC - 1];
801040ae:	be a8 7d 11 80       	mov    $0x80117da8,%esi
{
801040b3:	53                   	push   %ebx
801040b4:	83 ec 1c             	sub    $0x1c,%esp
  struct cpu *c = mycpu();
801040b7:	e8 74 fd ff ff       	call   80103e30 <mycpu>
  c->proc = 0;
801040bc:	c7 80 ac 00 00 00 00 	movl   $0x0,0xac(%eax)
801040c3:	00 00 00 
  struct cpu *c = mycpu();
801040c6:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  c->proc = 0;
801040c9:	83 c0 04             	add    $0x4,%eax
801040cc:	89 45 e0             	mov    %eax,-0x20(%ebp)
801040cf:	90                   	nop
  asm volatile("sti");
801040d0:	fb                   	sti    
    acquire(&ptable.lock);
801040d1:	83 ec 0c             	sub    $0xc,%esp
801040d4:	89 f3                	mov    %esi,%ebx
801040d6:	68 20 53 11 80       	push   $0x80115320
801040db:	e8 30 1b 00 00       	call   80105c10 <acquire>
801040e0:	83 c4 10             	add    $0x10,%esp
801040e3:	eb 0b                	jmp    801040f0 <scheduler+0x50>
801040e5:	8d 76 00             	lea    0x0(%esi),%esi
    if (p == last_scheduled)
801040e8:	39 de                	cmp    %ebx,%esi
801040ea:	0f 84 90 00 00 00    	je     80104180 <scheduler+0xe0>
    p++;
801040f0:	81 c3 ac 00 00 00    	add    $0xac,%ebx
      p = ptable.proc;
801040f6:	81 fb 54 7e 11 80    	cmp    $0x80117e54,%ebx
801040fc:	0f 43 df             	cmovae %edi,%ebx
    if (p->state == RUNNABLE && p->sched_info.queue == ROUND_ROBIN)
801040ff:	83 7b 0c 03          	cmpl   $0x3,0xc(%ebx)
80104103:	75 e3                	jne    801040e8 <scheduler+0x48>
80104105:	83 bb 80 00 00 00 01 	cmpl   $0x1,0x80(%ebx)
8010410c:	75 da                	jne    801040e8 <scheduler+0x48>
8010410e:	89 de                	mov    %ebx,%esi
    c->proc = p;
80104110:	8b 45 e4             	mov    -0x1c(%ebp),%eax
    switchuvm(p);
80104113:	83 ec 0c             	sub    $0xc,%esp
    c->proc = p;
80104116:	89 98 ac 00 00 00    	mov    %ebx,0xac(%eax)
    switchuvm(p);
8010411c:	53                   	push   %ebx
8010411d:	e8 9e 43 00 00       	call   801084c0 <switchuvm>
    p->sched_info.bjf.executed_cycle += 0.1f;
80104122:	d9 05 60 94 10 80    	flds   0x80109460
80104128:	d8 83 98 00 00 00    	fadds  0x98(%ebx)
    p->state = RUNNING;
8010412e:	c7 43 0c 04 00 00 00 	movl   $0x4,0xc(%ebx)
    p->sched_info.last_run = ticks;
80104135:	a1 a0 86 11 80       	mov    0x801186a0,%eax
8010413a:	89 83 84 00 00 00    	mov    %eax,0x84(%ebx)
    p->sched_info.bjf.executed_cycle += 0.1f;
80104140:	d9 9b 98 00 00 00    	fstps  0x98(%ebx)
    swtch(&(c->scheduler), p->context);
80104146:	58                   	pop    %eax
80104147:	5a                   	pop    %edx
80104148:	ff 73 1c             	pushl  0x1c(%ebx)
8010414b:	ff 75 e0             	pushl  -0x20(%ebp)
8010414e:	e8 f0 1d 00 00       	call   80105f43 <swtch>
    switchkvm();
80104153:	e8 48 43 00 00       	call   801084a0 <switchkvm>
    c->proc = 0;
80104158:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010415b:	c7 80 ac 00 00 00 00 	movl   $0x0,0xac(%eax)
80104162:	00 00 00 
  release(&ptable.lock);
80104165:	c7 04 24 20 53 11 80 	movl   $0x80115320,(%esp)
8010416c:	e8 5f 1b 00 00       	call   80105cd0 <release>
80104171:	83 c4 10             	add    $0x10,%esp
80104174:	e9 57 ff ff ff       	jmp    801040d0 <scheduler+0x30>
80104179:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  struct proc *result = 0;
80104180:	31 db                	xor    %ebx,%ebx
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104182:	b8 54 53 11 80       	mov    $0x80115354,%eax
80104187:	eb 23                	jmp    801041ac <scheduler+0x10c>
80104189:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
      if (result->sched_info.arrival_queue_time < p->sched_info.arrival_queue_time)
80104190:	8b 90 a8 00 00 00    	mov    0xa8(%eax),%edx
80104196:	39 93 a8 00 00 00    	cmp    %edx,0xa8(%ebx)
8010419c:	0f 4c d8             	cmovl  %eax,%ebx
8010419f:	90                   	nop
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
801041a0:	05 ac 00 00 00       	add    $0xac,%eax
801041a5:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
801041aa:	74 24                	je     801041d0 <scheduler+0x130>
    if (p->state != RUNNABLE || p->sched_info.queue != LCFS)
801041ac:	83 78 0c 03          	cmpl   $0x3,0xc(%eax)
801041b0:	75 ee                	jne    801041a0 <scheduler+0x100>
801041b2:	83 b8 80 00 00 00 02 	cmpl   $0x2,0x80(%eax)
801041b9:	75 e5                	jne    801041a0 <scheduler+0x100>
    if (result != 0)
801041bb:	85 db                	test   %ebx,%ebx
801041bd:	75 d1                	jne    80104190 <scheduler+0xf0>
801041bf:	89 c3                	mov    %eax,%ebx
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
801041c1:	05 ac 00 00 00       	add    $0xac,%eax
801041c6:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
801041cb:	75 df                	jne    801041ac <scheduler+0x10c>
801041cd:	8d 76 00             	lea    0x0(%esi),%esi
      if (!p)
801041d0:	85 db                	test   %ebx,%ebx
801041d2:	0f 85 38 ff ff ff    	jne    80104110 <scheduler+0x70>
        p = bestjobfirst();
801041d8:	e8 33 fe ff ff       	call   80104010 <bestjobfirst>
801041dd:	89 c3                	mov    %eax,%ebx
        if (!p)
801041df:	85 c0                	test   %eax,%eax
801041e1:	0f 85 29 ff ff ff    	jne    80104110 <scheduler+0x70>
          release(&ptable.lock);
801041e7:	83 ec 0c             	sub    $0xc,%esp
801041ea:	68 20 53 11 80       	push   $0x80115320
801041ef:	e8 dc 1a 00 00       	call   80105cd0 <release>
          continue;
801041f4:	83 c4 10             	add    $0x10,%esp
801041f7:	e9 d4 fe ff ff       	jmp    801040d0 <scheduler+0x30>
801041fc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80104200 <roundrobin>:
{
80104200:	f3 0f 1e fb          	endbr32 
80104204:	55                   	push   %ebp
      p = ptable.proc;
80104205:	b9 54 53 11 80       	mov    $0x80115354,%ecx
{
8010420a:	89 e5                	mov    %esp,%ebp
8010420c:	8b 55 08             	mov    0x8(%ebp),%edx
  struct proc *p = last_scheduled;
8010420f:	89 d0                	mov    %edx,%eax
80104211:	eb 09                	jmp    8010421c <roundrobin+0x1c>
80104213:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104217:	90                   	nop
    if (p == last_scheduled)
80104218:	39 d0                	cmp    %edx,%eax
8010421a:	74 24                	je     80104240 <roundrobin+0x40>
    p++;
8010421c:	05 ac 00 00 00       	add    $0xac,%eax
      p = ptable.proc;
80104221:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
80104226:	0f 43 c1             	cmovae %ecx,%eax
    if (p->state == RUNNABLE && p->sched_info.queue == ROUND_ROBIN)
80104229:	83 78 0c 03          	cmpl   $0x3,0xc(%eax)
8010422d:	75 e9                	jne    80104218 <roundrobin+0x18>
8010422f:	83 b8 80 00 00 00 01 	cmpl   $0x1,0x80(%eax)
80104236:	75 e0                	jne    80104218 <roundrobin+0x18>
}
80104238:	5d                   	pop    %ebp
80104239:	c3                   	ret    
8010423a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
      return 0;
80104240:	31 c0                	xor    %eax,%eax
}
80104242:	5d                   	pop    %ebp
80104243:	c3                   	ret    
80104244:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010424b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010424f:	90                   	nop

80104250 <sched>:
{
80104250:	f3 0f 1e fb          	endbr32 
80104254:	55                   	push   %ebp
80104255:	89 e5                	mov    %esp,%ebp
80104257:	56                   	push   %esi
80104258:	53                   	push   %ebx
  pushcli();
80104259:	e8 b2 18 00 00       	call   80105b10 <pushcli>
  c = mycpu();
8010425e:	e8 cd fb ff ff       	call   80103e30 <mycpu>
  p = c->proc;
80104263:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
80104269:	e8 f2 18 00 00       	call   80105b60 <popcli>
  if(!holding(&ptable.lock))
8010426e:	83 ec 0c             	sub    $0xc,%esp
80104271:	68 20 53 11 80       	push   $0x80115320
80104276:	e8 45 19 00 00       	call   80105bc0 <holding>
8010427b:	83 c4 10             	add    $0x10,%esp
8010427e:	85 c0                	test   %eax,%eax
80104280:	74 4f                	je     801042d1 <sched+0x81>
  if(mycpu()->ncli != 1)
80104282:	e8 a9 fb ff ff       	call   80103e30 <mycpu>
80104287:	83 b8 a4 00 00 00 01 	cmpl   $0x1,0xa4(%eax)
8010428e:	75 68                	jne    801042f8 <sched+0xa8>
  if(p->state == RUNNING)
80104290:	83 7b 0c 04          	cmpl   $0x4,0xc(%ebx)
80104294:	74 55                	je     801042eb <sched+0x9b>
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80104296:	9c                   	pushf  
80104297:	58                   	pop    %eax
  if(readeflags()&FL_IF)
80104298:	f6 c4 02             	test   $0x2,%ah
8010429b:	75 41                	jne    801042de <sched+0x8e>
  intena = mycpu()->intena;
8010429d:	e8 8e fb ff ff       	call   80103e30 <mycpu>
  swtch(&p->context, mycpu()->scheduler);
801042a2:	83 c3 1c             	add    $0x1c,%ebx
  intena = mycpu()->intena;
801042a5:	8b b0 a8 00 00 00    	mov    0xa8(%eax),%esi
  swtch(&p->context, mycpu()->scheduler);
801042ab:	e8 80 fb ff ff       	call   80103e30 <mycpu>
801042b0:	83 ec 08             	sub    $0x8,%esp
801042b3:	ff 70 04             	pushl  0x4(%eax)
801042b6:	53                   	push   %ebx
801042b7:	e8 87 1c 00 00       	call   80105f43 <swtch>
  mycpu()->intena = intena;
801042bc:	e8 6f fb ff ff       	call   80103e30 <mycpu>
}
801042c1:	83 c4 10             	add    $0x10,%esp
  mycpu()->intena = intena;
801042c4:	89 b0 a8 00 00 00    	mov    %esi,0xa8(%eax)
}
801042ca:	8d 65 f8             	lea    -0x8(%ebp),%esp
801042cd:	5b                   	pop    %ebx
801042ce:	5e                   	pop    %esi
801042cf:	5d                   	pop    %ebp
801042d0:	c3                   	ret    
    panic("sched ptable.lock");
801042d1:	83 ec 0c             	sub    $0xc,%esp
801042d4:	68 57 91 10 80       	push   $0x80109157
801042d9:	e8 b2 c0 ff ff       	call   80100390 <panic>
    panic("sched interruptible");
801042de:	83 ec 0c             	sub    $0xc,%esp
801042e1:	68 83 91 10 80       	push   $0x80109183
801042e6:	e8 a5 c0 ff ff       	call   80100390 <panic>
    panic("sched running");
801042eb:	83 ec 0c             	sub    $0xc,%esp
801042ee:	68 75 91 10 80       	push   $0x80109175
801042f3:	e8 98 c0 ff ff       	call   80100390 <panic>
    panic("sched locks");
801042f8:	83 ec 0c             	sub    $0xc,%esp
801042fb:	68 69 91 10 80       	push   $0x80109169
80104300:	e8 8b c0 ff ff       	call   80100390 <panic>
80104305:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010430c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80104310 <exit>:
{
80104310:	f3 0f 1e fb          	endbr32 
80104314:	55                   	push   %ebp
80104315:	89 e5                	mov    %esp,%ebp
80104317:	57                   	push   %edi
80104318:	56                   	push   %esi
80104319:	53                   	push   %ebx
8010431a:	83 ec 0c             	sub    $0xc,%esp
  pushcli();
8010431d:	e8 ee 17 00 00       	call   80105b10 <pushcli>
  c = mycpu();
80104322:	e8 09 fb ff ff       	call   80103e30 <mycpu>
  p = c->proc;
80104327:	8b b0 ac 00 00 00    	mov    0xac(%eax),%esi
  popcli();
8010432d:	e8 2e 18 00 00       	call   80105b60 <popcli>
  if(curproc == initproc)
80104332:	8d 5e 28             	lea    0x28(%esi),%ebx
80104335:	8d 7e 68             	lea    0x68(%esi),%edi
80104338:	39 35 fc c5 10 80    	cmp    %esi,0x8010c5fc
8010433e:	0f 84 fd 00 00 00    	je     80104441 <exit+0x131>
80104344:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    if(curproc->ofile[fd]){
80104348:	8b 03                	mov    (%ebx),%eax
8010434a:	85 c0                	test   %eax,%eax
8010434c:	74 12                	je     80104360 <exit+0x50>
      fileclose(curproc->ofile[fd]);
8010434e:	83 ec 0c             	sub    $0xc,%esp
80104351:	50                   	push   %eax
80104352:	e8 59 d0 ff ff       	call   801013b0 <fileclose>
      curproc->ofile[fd] = 0;
80104357:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
8010435d:	83 c4 10             	add    $0x10,%esp
  for(fd = 0; fd < NOFILE; fd++){
80104360:	83 c3 04             	add    $0x4,%ebx
80104363:	39 df                	cmp    %ebx,%edi
80104365:	75 e1                	jne    80104348 <exit+0x38>
  begin_op();
80104367:	e8 b4 ee ff ff       	call   80103220 <begin_op>
  iput(curproc->cwd);
8010436c:	83 ec 0c             	sub    $0xc,%esp
8010436f:	ff 76 68             	pushl  0x68(%esi)
80104372:	e8 09 da ff ff       	call   80101d80 <iput>
  end_op();
80104377:	e8 14 ef ff ff       	call   80103290 <end_op>
  curproc->cwd = 0;
8010437c:	c7 46 68 00 00 00 00 	movl   $0x0,0x68(%esi)
  acquire(&ptable.lock);
80104383:	c7 04 24 20 53 11 80 	movl   $0x80115320,(%esp)
8010438a:	e8 81 18 00 00       	call   80105c10 <acquire>
  wakeup1(curproc->parent);
8010438f:	8b 56 14             	mov    0x14(%esi),%edx
80104392:	83 c4 10             	add    $0x10,%esp
static void
wakeup1(void *chan)
{
  struct proc *p;

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104395:	b8 54 53 11 80       	mov    $0x80115354,%eax
8010439a:	eb 10                	jmp    801043ac <exit+0x9c>
8010439c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801043a0:	05 ac 00 00 00       	add    $0xac,%eax
801043a5:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
801043aa:	74 1e                	je     801043ca <exit+0xba>
    if(p->state == SLEEPING && p->chan == chan)
801043ac:	83 78 0c 02          	cmpl   $0x2,0xc(%eax)
801043b0:	75 ee                	jne    801043a0 <exit+0x90>
801043b2:	3b 50 20             	cmp    0x20(%eax),%edx
801043b5:	75 e9                	jne    801043a0 <exit+0x90>
      p->state = RUNNABLE;
801043b7:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
801043be:	05 ac 00 00 00       	add    $0xac,%eax
801043c3:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
801043c8:	75 e2                	jne    801043ac <exit+0x9c>
      p->parent = initproc;
801043ca:	8b 0d fc c5 10 80    	mov    0x8010c5fc,%ecx
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801043d0:	ba 54 53 11 80       	mov    $0x80115354,%edx
801043d5:	eb 17                	jmp    801043ee <exit+0xde>
801043d7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801043de:	66 90                	xchg   %ax,%ax
801043e0:	81 c2 ac 00 00 00    	add    $0xac,%edx
801043e6:	81 fa 54 7e 11 80    	cmp    $0x80117e54,%edx
801043ec:	74 3a                	je     80104428 <exit+0x118>
    if(p->parent == curproc){
801043ee:	39 72 14             	cmp    %esi,0x14(%edx)
801043f1:	75 ed                	jne    801043e0 <exit+0xd0>
      if(p->state == ZOMBIE)
801043f3:	83 7a 0c 05          	cmpl   $0x5,0xc(%edx)
      p->parent = initproc;
801043f7:	89 4a 14             	mov    %ecx,0x14(%edx)
      if(p->state == ZOMBIE)
801043fa:	75 e4                	jne    801043e0 <exit+0xd0>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
801043fc:	b8 54 53 11 80       	mov    $0x80115354,%eax
80104401:	eb 11                	jmp    80104414 <exit+0x104>
80104403:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104407:	90                   	nop
80104408:	05 ac 00 00 00       	add    $0xac,%eax
8010440d:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
80104412:	74 cc                	je     801043e0 <exit+0xd0>
    if(p->state == SLEEPING && p->chan == chan)
80104414:	83 78 0c 02          	cmpl   $0x2,0xc(%eax)
80104418:	75 ee                	jne    80104408 <exit+0xf8>
8010441a:	3b 48 20             	cmp    0x20(%eax),%ecx
8010441d:	75 e9                	jne    80104408 <exit+0xf8>
      p->state = RUNNABLE;
8010441f:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
80104426:	eb e0                	jmp    80104408 <exit+0xf8>
  curproc->state = ZOMBIE;
80104428:	c7 46 0c 05 00 00 00 	movl   $0x5,0xc(%esi)
  sched();
8010442f:	e8 1c fe ff ff       	call   80104250 <sched>
  panic("zombie exit");
80104434:	83 ec 0c             	sub    $0xc,%esp
80104437:	68 a4 91 10 80       	push   $0x801091a4
8010443c:	e8 4f bf ff ff       	call   80100390 <panic>
    panic("init exiting");
80104441:	83 ec 0c             	sub    $0xc,%esp
80104444:	68 97 91 10 80       	push   $0x80109197
80104449:	e8 42 bf ff ff       	call   80100390 <panic>
8010444e:	66 90                	xchg   %ax,%ax

80104450 <yield>:
{
80104450:	f3 0f 1e fb          	endbr32 
80104454:	55                   	push   %ebp
80104455:	89 e5                	mov    %esp,%ebp
80104457:	53                   	push   %ebx
80104458:	83 ec 10             	sub    $0x10,%esp
  acquire(&ptable.lock);  //DOC: yieldlock
8010445b:	68 20 53 11 80       	push   $0x80115320
80104460:	e8 ab 17 00 00       	call   80105c10 <acquire>
  pushcli();
80104465:	e8 a6 16 00 00       	call   80105b10 <pushcli>
  c = mycpu();
8010446a:	e8 c1 f9 ff ff       	call   80103e30 <mycpu>
  p = c->proc;
8010446f:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
80104475:	e8 e6 16 00 00       	call   80105b60 <popcli>
  myproc()->state = RUNNABLE;
8010447a:	c7 43 0c 03 00 00 00 	movl   $0x3,0xc(%ebx)
  sched();
80104481:	e8 ca fd ff ff       	call   80104250 <sched>
  release(&ptable.lock);
80104486:	c7 04 24 20 53 11 80 	movl   $0x80115320,(%esp)
8010448d:	e8 3e 18 00 00       	call   80105cd0 <release>
}
80104492:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80104495:	83 c4 10             	add    $0x10,%esp
80104498:	c9                   	leave  
80104499:	c3                   	ret    
8010449a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801044a0 <sleep>:
{
801044a0:	f3 0f 1e fb          	endbr32 
801044a4:	55                   	push   %ebp
801044a5:	89 e5                	mov    %esp,%ebp
801044a7:	57                   	push   %edi
801044a8:	56                   	push   %esi
801044a9:	53                   	push   %ebx
801044aa:	83 ec 0c             	sub    $0xc,%esp
801044ad:	8b 7d 08             	mov    0x8(%ebp),%edi
801044b0:	8b 75 0c             	mov    0xc(%ebp),%esi
  pushcli();
801044b3:	e8 58 16 00 00       	call   80105b10 <pushcli>
  c = mycpu();
801044b8:	e8 73 f9 ff ff       	call   80103e30 <mycpu>
  p = c->proc;
801044bd:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
801044c3:	e8 98 16 00 00       	call   80105b60 <popcli>
  if(p == 0)
801044c8:	85 db                	test   %ebx,%ebx
801044ca:	0f 84 83 00 00 00    	je     80104553 <sleep+0xb3>
  if(lk == 0)
801044d0:	85 f6                	test   %esi,%esi
801044d2:	74 72                	je     80104546 <sleep+0xa6>
  if(lk != &ptable.lock){  //DOC: sleeplock0
801044d4:	81 fe 20 53 11 80    	cmp    $0x80115320,%esi
801044da:	74 4c                	je     80104528 <sleep+0x88>
    acquire(&ptable.lock);  //DOC: sleeplock1
801044dc:	83 ec 0c             	sub    $0xc,%esp
801044df:	68 20 53 11 80       	push   $0x80115320
801044e4:	e8 27 17 00 00       	call   80105c10 <acquire>
    release(lk);
801044e9:	89 34 24             	mov    %esi,(%esp)
801044ec:	e8 df 17 00 00       	call   80105cd0 <release>
  p->chan = chan;
801044f1:	89 7b 20             	mov    %edi,0x20(%ebx)
  p->state = SLEEPING;
801044f4:	c7 43 0c 02 00 00 00 	movl   $0x2,0xc(%ebx)
  sched();
801044fb:	e8 50 fd ff ff       	call   80104250 <sched>
  p->chan = 0;
80104500:	c7 43 20 00 00 00 00 	movl   $0x0,0x20(%ebx)
    release(&ptable.lock);
80104507:	c7 04 24 20 53 11 80 	movl   $0x80115320,(%esp)
8010450e:	e8 bd 17 00 00       	call   80105cd0 <release>
    acquire(lk);
80104513:	89 75 08             	mov    %esi,0x8(%ebp)
80104516:	83 c4 10             	add    $0x10,%esp
}
80104519:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010451c:	5b                   	pop    %ebx
8010451d:	5e                   	pop    %esi
8010451e:	5f                   	pop    %edi
8010451f:	5d                   	pop    %ebp
    acquire(lk);
80104520:	e9 eb 16 00 00       	jmp    80105c10 <acquire>
80104525:	8d 76 00             	lea    0x0(%esi),%esi
  p->chan = chan;
80104528:	89 7b 20             	mov    %edi,0x20(%ebx)
  p->state = SLEEPING;
8010452b:	c7 43 0c 02 00 00 00 	movl   $0x2,0xc(%ebx)
  sched();
80104532:	e8 19 fd ff ff       	call   80104250 <sched>
  p->chan = 0;
80104537:	c7 43 20 00 00 00 00 	movl   $0x0,0x20(%ebx)
}
8010453e:	8d 65 f4             	lea    -0xc(%ebp),%esp
80104541:	5b                   	pop    %ebx
80104542:	5e                   	pop    %esi
80104543:	5f                   	pop    %edi
80104544:	5d                   	pop    %ebp
80104545:	c3                   	ret    
    panic("sleep without lk");
80104546:	83 ec 0c             	sub    $0xc,%esp
80104549:	68 b6 91 10 80       	push   $0x801091b6
8010454e:	e8 3d be ff ff       	call   80100390 <panic>
    panic("sleep");
80104553:	83 ec 0c             	sub    $0xc,%esp
80104556:	68 b0 91 10 80       	push   $0x801091b0
8010455b:	e8 30 be ff ff       	call   80100390 <panic>

80104560 <wait>:
{
80104560:	f3 0f 1e fb          	endbr32 
80104564:	55                   	push   %ebp
80104565:	89 e5                	mov    %esp,%ebp
80104567:	56                   	push   %esi
80104568:	53                   	push   %ebx
  pushcli();
80104569:	e8 a2 15 00 00       	call   80105b10 <pushcli>
  c = mycpu();
8010456e:	e8 bd f8 ff ff       	call   80103e30 <mycpu>
  p = c->proc;
80104573:	8b b0 ac 00 00 00    	mov    0xac(%eax),%esi
  popcli();
80104579:	e8 e2 15 00 00       	call   80105b60 <popcli>
  acquire(&ptable.lock);
8010457e:	83 ec 0c             	sub    $0xc,%esp
80104581:	68 20 53 11 80       	push   $0x80115320
80104586:	e8 85 16 00 00       	call   80105c10 <acquire>
8010458b:	83 c4 10             	add    $0x10,%esp
    havekids = 0;
8010458e:	31 c0                	xor    %eax,%eax
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104590:	bb 54 53 11 80       	mov    $0x80115354,%ebx
80104595:	eb 17                	jmp    801045ae <wait+0x4e>
80104597:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010459e:	66 90                	xchg   %ax,%ax
801045a0:	81 c3 ac 00 00 00    	add    $0xac,%ebx
801045a6:	81 fb 54 7e 11 80    	cmp    $0x80117e54,%ebx
801045ac:	74 1e                	je     801045cc <wait+0x6c>
      if(p->parent != curproc)
801045ae:	39 73 14             	cmp    %esi,0x14(%ebx)
801045b1:	75 ed                	jne    801045a0 <wait+0x40>
      if(p->state == ZOMBIE){
801045b3:	83 7b 0c 05          	cmpl   $0x5,0xc(%ebx)
801045b7:	74 37                	je     801045f0 <wait+0x90>
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801045b9:	81 c3 ac 00 00 00    	add    $0xac,%ebx
      havekids = 1;
801045bf:	b8 01 00 00 00       	mov    $0x1,%eax
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801045c4:	81 fb 54 7e 11 80    	cmp    $0x80117e54,%ebx
801045ca:	75 e2                	jne    801045ae <wait+0x4e>
    if(!havekids || curproc->killed){
801045cc:	85 c0                	test   %eax,%eax
801045ce:	74 76                	je     80104646 <wait+0xe6>
801045d0:	8b 46 24             	mov    0x24(%esi),%eax
801045d3:	85 c0                	test   %eax,%eax
801045d5:	75 6f                	jne    80104646 <wait+0xe6>
    sleep(curproc, &ptable.lock);  //DOC: wait-sleep
801045d7:	83 ec 08             	sub    $0x8,%esp
801045da:	68 20 53 11 80       	push   $0x80115320
801045df:	56                   	push   %esi
801045e0:	e8 bb fe ff ff       	call   801044a0 <sleep>
    havekids = 0;
801045e5:	83 c4 10             	add    $0x10,%esp
801045e8:	eb a4                	jmp    8010458e <wait+0x2e>
801045ea:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
        kfree(p->kstack);
801045f0:	83 ec 0c             	sub    $0xc,%esp
801045f3:	ff 73 08             	pushl  0x8(%ebx)
        pid = p->pid;
801045f6:	8b 73 10             	mov    0x10(%ebx),%esi
        kfree(p->kstack);
801045f9:	e8 62 e3 ff ff       	call   80102960 <kfree>
        freevm(p->pgdir);
801045fe:	5a                   	pop    %edx
801045ff:	ff 73 04             	pushl  0x4(%ebx)
        p->kstack = 0;
80104602:	c7 43 08 00 00 00 00 	movl   $0x0,0x8(%ebx)
        freevm(p->pgdir);
80104609:	e8 72 42 00 00       	call   80108880 <freevm>
        release(&ptable.lock);
8010460e:	c7 04 24 20 53 11 80 	movl   $0x80115320,(%esp)
        p->pid = 0;
80104615:	c7 43 10 00 00 00 00 	movl   $0x0,0x10(%ebx)
        p->parent = 0;
8010461c:	c7 43 14 00 00 00 00 	movl   $0x0,0x14(%ebx)
        p->name[0] = 0;
80104623:	c6 43 6c 00          	movb   $0x0,0x6c(%ebx)
        p->killed = 0;
80104627:	c7 43 24 00 00 00 00 	movl   $0x0,0x24(%ebx)
        p->state = UNUSED;
8010462e:	c7 43 0c 00 00 00 00 	movl   $0x0,0xc(%ebx)
        release(&ptable.lock);
80104635:	e8 96 16 00 00       	call   80105cd0 <release>
        return pid;
8010463a:	83 c4 10             	add    $0x10,%esp
}
8010463d:	8d 65 f8             	lea    -0x8(%ebp),%esp
80104640:	89 f0                	mov    %esi,%eax
80104642:	5b                   	pop    %ebx
80104643:	5e                   	pop    %esi
80104644:	5d                   	pop    %ebp
80104645:	c3                   	ret    
      release(&ptable.lock);
80104646:	83 ec 0c             	sub    $0xc,%esp
      return -1;
80104649:	be ff ff ff ff       	mov    $0xffffffff,%esi
      release(&ptable.lock);
8010464e:	68 20 53 11 80       	push   $0x80115320
80104653:	e8 78 16 00 00       	call   80105cd0 <release>
      return -1;
80104658:	83 c4 10             	add    $0x10,%esp
8010465b:	eb e0                	jmp    8010463d <wait+0xdd>
8010465d:	8d 76 00             	lea    0x0(%esi),%esi

80104660 <wakeup>:
}

// Wake up all processes sleeping on chan.
void
wakeup(void *chan)
{
80104660:	f3 0f 1e fb          	endbr32 
80104664:	55                   	push   %ebp
80104665:	89 e5                	mov    %esp,%ebp
80104667:	53                   	push   %ebx
80104668:	83 ec 10             	sub    $0x10,%esp
8010466b:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&ptable.lock);
8010466e:	68 20 53 11 80       	push   $0x80115320
80104673:	e8 98 15 00 00       	call   80105c10 <acquire>
80104678:	83 c4 10             	add    $0x10,%esp
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
8010467b:	b8 54 53 11 80       	mov    $0x80115354,%eax
80104680:	eb 12                	jmp    80104694 <wakeup+0x34>
80104682:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80104688:	05 ac 00 00 00       	add    $0xac,%eax
8010468d:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
80104692:	74 1e                	je     801046b2 <wakeup+0x52>
    if(p->state == SLEEPING && p->chan == chan)
80104694:	83 78 0c 02          	cmpl   $0x2,0xc(%eax)
80104698:	75 ee                	jne    80104688 <wakeup+0x28>
8010469a:	3b 58 20             	cmp    0x20(%eax),%ebx
8010469d:	75 e9                	jne    80104688 <wakeup+0x28>
      p->state = RUNNABLE;
8010469f:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
801046a6:	05 ac 00 00 00       	add    $0xac,%eax
801046ab:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
801046b0:	75 e2                	jne    80104694 <wakeup+0x34>
  wakeup1(chan);
  release(&ptable.lock);
801046b2:	c7 45 08 20 53 11 80 	movl   $0x80115320,0x8(%ebp)
}
801046b9:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801046bc:	c9                   	leave  
  release(&ptable.lock);
801046bd:	e9 0e 16 00 00       	jmp    80105cd0 <release>
801046c2:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801046c9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

801046d0 <kill>:
// Kill the process with the given pid.
// Process won't exit until it returns
// to user space (see trap in trap.c).
int
kill(int pid)
{
801046d0:	f3 0f 1e fb          	endbr32 
801046d4:	55                   	push   %ebp
801046d5:	89 e5                	mov    %esp,%ebp
801046d7:	53                   	push   %ebx
801046d8:	83 ec 10             	sub    $0x10,%esp
801046db:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct proc *p;

  acquire(&ptable.lock);
801046de:	68 20 53 11 80       	push   $0x80115320
801046e3:	e8 28 15 00 00       	call   80105c10 <acquire>
801046e8:	83 c4 10             	add    $0x10,%esp
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801046eb:	b8 54 53 11 80       	mov    $0x80115354,%eax
801046f0:	eb 12                	jmp    80104704 <kill+0x34>
801046f2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801046f8:	05 ac 00 00 00       	add    $0xac,%eax
801046fd:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
80104702:	74 34                	je     80104738 <kill+0x68>
    if(p->pid == pid){
80104704:	39 58 10             	cmp    %ebx,0x10(%eax)
80104707:	75 ef                	jne    801046f8 <kill+0x28>
      p->killed = 1;
      // Wake process from sleep if necessary.
      if(p->state == SLEEPING)
80104709:	83 78 0c 02          	cmpl   $0x2,0xc(%eax)
      p->killed = 1;
8010470d:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
      if(p->state == SLEEPING)
80104714:	75 07                	jne    8010471d <kill+0x4d>
        p->state = RUNNABLE;
80104716:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
      release(&ptable.lock);
8010471d:	83 ec 0c             	sub    $0xc,%esp
80104720:	68 20 53 11 80       	push   $0x80115320
80104725:	e8 a6 15 00 00       	call   80105cd0 <release>
      return 0;
    }
  }
  release(&ptable.lock);
  return -1;
}
8010472a:	8b 5d fc             	mov    -0x4(%ebp),%ebx
      return 0;
8010472d:	83 c4 10             	add    $0x10,%esp
80104730:	31 c0                	xor    %eax,%eax
}
80104732:	c9                   	leave  
80104733:	c3                   	ret    
80104734:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  release(&ptable.lock);
80104738:	83 ec 0c             	sub    $0xc,%esp
8010473b:	68 20 53 11 80       	push   $0x80115320
80104740:	e8 8b 15 00 00       	call   80105cd0 <release>
}
80104745:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  return -1;
80104748:	83 c4 10             	add    $0x10,%esp
8010474b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104750:	c9                   	leave  
80104751:	c3                   	ret    
80104752:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104759:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80104760 <get_uncle_count>:
    
}*/


//get_uncle_count
int get_uncle_count(int pid){
80104760:	f3 0f 1e fb          	endbr32 
80104764:	55                   	push   %ebp
80104765:	89 e5                	mov    %esp,%ebp
80104767:	53                   	push   %ebx
80104768:	83 ec 10             	sub    $0x10,%esp
8010476b:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct proc *p;
  struct proc *p_parent;
  struct proc *p_grandParent = 0;
  acquire(&ptable.lock);
8010476e:	68 20 53 11 80       	push   $0x80115320
80104773:	e8 98 14 00 00       	call   80105c10 <acquire>
  if(pid < 0 || pid >= NPROC){
80104778:	83 c4 10             	add    $0x10,%esp
8010477b:	83 fb 3f             	cmp    $0x3f,%ebx
8010477e:	0f 87 a8 00 00 00    	ja     8010482c <get_uncle_count+0xcc>
  struct proc *p_grandParent = 0;
80104784:	31 c9                	xor    %ecx,%ecx
      return -1;
      }
  int count = 0;
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104786:	b8 54 53 11 80       	mov    $0x80115354,%eax
8010478b:	eb 0f                	jmp    8010479c <get_uncle_count+0x3c>
8010478d:	8d 76 00             	lea    0x0(%esi),%esi
80104790:	05 ac 00 00 00       	add    $0xac,%eax
80104795:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
8010479a:	74 34                	je     801047d0 <get_uncle_count+0x70>
    count ++;
    if((p->pid) == pid){
8010479c:	39 58 10             	cmp    %ebx,0x10(%eax)
8010479f:	75 ef                	jne    80104790 <get_uncle_count+0x30>
      p_parent = p->parent;
801047a1:	8b 50 14             	mov    0x14(%eax),%edx
      if(p_parent != 0){
801047a4:	85 d2                	test   %edx,%edx
801047a6:	74 68                	je     80104810 <get_uncle_count+0xb0>
        p_grandParent = p_parent->parent;
801047a8:	8b 4a 14             	mov    0x14(%edx),%ecx
        if(p_grandParent == 0){
801047ab:	85 c9                	test   %ecx,%ecx
801047ad:	75 e1                	jne    80104790 <get_uncle_count+0x30>
          cprintf("grandparent is zero.");
801047af:	83 ec 0c             	sub    $0xc,%esp
          return -1;
801047b2:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
          cprintf("grandparent is zero.");
801047b7:	68 c7 91 10 80       	push   $0x801091c7
801047bc:	e8 ff c0 ff ff       	call   801008c0 <cprintf>
          return -1;
801047c1:	83 c4 10             	add    $0x10,%esp
    }
  }
  release(&ptable.lock);
  return siblings;

}
801047c4:	89 d8                	mov    %ebx,%eax
801047c6:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801047c9:	c9                   	leave  
801047ca:	c3                   	ret    
801047cb:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801047cf:	90                   	nop
  for(struct proc *i = ptable.proc; i < &ptable.proc[NPROC]; i++){
801047d0:	b8 54 53 11 80       	mov    $0x80115354,%eax
  int siblings = 0;
801047d5:	31 db                	xor    %ebx,%ebx
801047d7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801047de:	66 90                	xchg   %ax,%ax
      siblings++;
801047e0:	31 d2                	xor    %edx,%edx
801047e2:	39 48 14             	cmp    %ecx,0x14(%eax)
801047e5:	0f 94 c2             	sete   %dl
  for(struct proc *i = ptable.proc; i < &ptable.proc[NPROC]; i++){
801047e8:	05 ac 00 00 00       	add    $0xac,%eax
      siblings++;
801047ed:	01 d3                	add    %edx,%ebx
  for(struct proc *i = ptable.proc; i < &ptable.proc[NPROC]; i++){
801047ef:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
801047f4:	75 ea                	jne    801047e0 <get_uncle_count+0x80>
  release(&ptable.lock);
801047f6:	83 ec 0c             	sub    $0xc,%esp
801047f9:	68 20 53 11 80       	push   $0x80115320
801047fe:	e8 cd 14 00 00       	call   80105cd0 <release>
}
80104803:	89 d8                	mov    %ebx,%eax
  return siblings;
80104805:	83 c4 10             	add    $0x10,%esp
}
80104808:	8b 5d fc             	mov    -0x4(%ebp),%ebx
8010480b:	c9                   	leave  
8010480c:	c3                   	ret    
8010480d:	8d 76 00             	lea    0x0(%esi),%esi
        cprintf("parent is zero.");
80104810:	83 ec 0c             	sub    $0xc,%esp
        return -1;
80104813:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
        cprintf("parent is zero.");
80104818:	68 cc 91 10 80       	push   $0x801091cc
8010481d:	e8 9e c0 ff ff       	call   801008c0 <cprintf>
}
80104822:	89 d8                	mov    %ebx,%eax
        return -1;
80104824:	83 c4 10             	add    $0x10,%esp
}
80104827:	8b 5d fc             	mov    -0x4(%ebp),%ebx
8010482a:	c9                   	leave  
8010482b:	c3                   	ret    
      return -1;
8010482c:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
80104831:	eb 91                	jmp    801047c4 <get_uncle_count+0x64>
80104833:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010483a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80104840 <procdump>:
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void
procdump(void)
{
80104840:	f3 0f 1e fb          	endbr32 
80104844:	55                   	push   %ebp
80104845:	89 e5                	mov    %esp,%ebp
80104847:	57                   	push   %edi
80104848:	56                   	push   %esi
80104849:	8d 75 e8             	lea    -0x18(%ebp),%esi
8010484c:	53                   	push   %ebx
8010484d:	bb c0 53 11 80       	mov    $0x801153c0,%ebx
80104852:	83 ec 3c             	sub    $0x3c,%esp
80104855:	eb 2b                	jmp    80104882 <procdump+0x42>
80104857:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010485e:	66 90                	xchg   %ax,%ax
    if(p->state == SLEEPING){
      getcallerpcs((uint*)p->context->ebp+2, pc);
      for(i=0; i<10 && pc[i] != 0; i++)
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
80104860:	83 ec 0c             	sub    $0xc,%esp
80104863:	68 25 92 10 80       	push   $0x80109225
80104868:	e8 53 c0 ff ff       	call   801008c0 <cprintf>
8010486d:	83 c4 10             	add    $0x10,%esp
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104870:	81 c3 ac 00 00 00    	add    $0xac,%ebx
80104876:	81 fb c0 7e 11 80    	cmp    $0x80117ec0,%ebx
8010487c:	0f 84 8e 00 00 00    	je     80104910 <procdump+0xd0>
    if(p->state == UNUSED)
80104882:	8b 43 a0             	mov    -0x60(%ebx),%eax
80104885:	85 c0                	test   %eax,%eax
80104887:	74 e7                	je     80104870 <procdump+0x30>
      state = "???";
80104889:	ba dc 91 10 80       	mov    $0x801091dc,%edx
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
8010488e:	83 f8 05             	cmp    $0x5,%eax
80104891:	77 11                	ja     801048a4 <procdump+0x64>
80104893:	8b 14 85 44 94 10 80 	mov    -0x7fef6bbc(,%eax,4),%edx
      state = "???";
8010489a:	b8 dc 91 10 80       	mov    $0x801091dc,%eax
8010489f:	85 d2                	test   %edx,%edx
801048a1:	0f 44 d0             	cmove  %eax,%edx
    cprintf("%d %s %s", p->pid, state, p->name);
801048a4:	53                   	push   %ebx
801048a5:	52                   	push   %edx
801048a6:	ff 73 a4             	pushl  -0x5c(%ebx)
801048a9:	68 e0 91 10 80       	push   $0x801091e0
801048ae:	e8 0d c0 ff ff       	call   801008c0 <cprintf>
    if(p->state == SLEEPING){
801048b3:	83 c4 10             	add    $0x10,%esp
801048b6:	83 7b a0 02          	cmpl   $0x2,-0x60(%ebx)
801048ba:	75 a4                	jne    80104860 <procdump+0x20>
      getcallerpcs((uint*)p->context->ebp+2, pc);
801048bc:	83 ec 08             	sub    $0x8,%esp
801048bf:	8d 45 c0             	lea    -0x40(%ebp),%eax
801048c2:	8d 7d c0             	lea    -0x40(%ebp),%edi
801048c5:	50                   	push   %eax
801048c6:	8b 43 b0             	mov    -0x50(%ebx),%eax
801048c9:	8b 40 0c             	mov    0xc(%eax),%eax
801048cc:	83 c0 08             	add    $0x8,%eax
801048cf:	50                   	push   %eax
801048d0:	e8 db 11 00 00       	call   80105ab0 <getcallerpcs>
      for(i=0; i<10 && pc[i] != 0; i++)
801048d5:	83 c4 10             	add    $0x10,%esp
801048d8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801048df:	90                   	nop
801048e0:	8b 17                	mov    (%edi),%edx
801048e2:	85 d2                	test   %edx,%edx
801048e4:	0f 84 76 ff ff ff    	je     80104860 <procdump+0x20>
        cprintf(" %p", pc[i]);
801048ea:	83 ec 08             	sub    $0x8,%esp
801048ed:	83 c7 04             	add    $0x4,%edi
801048f0:	52                   	push   %edx
801048f1:	68 e1 8b 10 80       	push   $0x80108be1
801048f6:	e8 c5 bf ff ff       	call   801008c0 <cprintf>
      for(i=0; i<10 && pc[i] != 0; i++)
801048fb:	83 c4 10             	add    $0x10,%esp
801048fe:	39 fe                	cmp    %edi,%esi
80104900:	75 de                	jne    801048e0 <procdump+0xa0>
80104902:	e9 59 ff ff ff       	jmp    80104860 <procdump+0x20>
80104907:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010490e:	66 90                	xchg   %ax,%ax
  }
}
80104910:	8d 65 f4             	lea    -0xc(%ebp),%esp
80104913:	5b                   	pop    %ebx
80104914:	5e                   	pop    %esi
80104915:	5f                   	pop    %edi
80104916:	5d                   	pop    %ebp
80104917:	c3                   	ret    
80104918:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010491f:	90                   	nop

80104920 <find_digital_root>:

int 
find_digital_root(int n){
80104920:	f3 0f 1e fb          	endbr32 
80104924:	55                   	push   %ebp
80104925:	89 e5                	mov    %esp,%ebp
80104927:	57                   	push   %edi
80104928:	56                   	push   %esi
80104929:	53                   	push   %ebx
8010492a:	8b 5d 08             	mov    0x8(%ebp),%ebx
  while(n>9){
8010492d:	83 fb 09             	cmp    $0x9,%ebx
80104930:	7e 3d                	jle    8010496f <find_digital_root+0x4f>
    int sum = 0 ;
    while(n > 0){
      int digit = n%10;
80104932:	be 67 66 66 66       	mov    $0x66666667,%esi
80104937:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010493e:	66 90                	xchg   %ax,%ax
find_digital_root(int n){
80104940:	89 d9                	mov    %ebx,%ecx
    int sum = 0 ;
80104942:	31 db                	xor    %ebx,%ebx
80104944:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      int digit = n%10;
80104948:	89 c8                	mov    %ecx,%eax
8010494a:	89 cf                	mov    %ecx,%edi
8010494c:	f7 ee                	imul   %esi
8010494e:	89 c8                	mov    %ecx,%eax
80104950:	c1 f8 1f             	sar    $0x1f,%eax
80104953:	c1 fa 02             	sar    $0x2,%edx
80104956:	29 c2                	sub    %eax,%edx
80104958:	8d 04 92             	lea    (%edx,%edx,4),%eax
8010495b:	01 c0                	add    %eax,%eax
8010495d:	29 c7                	sub    %eax,%edi
8010495f:	89 c8                	mov    %ecx,%eax
      sum += digit;
      n = n/10;
80104961:	89 d1                	mov    %edx,%ecx
      sum += digit;
80104963:	01 fb                	add    %edi,%ebx
    while(n > 0){
80104965:	83 f8 09             	cmp    $0x9,%eax
80104968:	7f de                	jg     80104948 <find_digital_root+0x28>
  while(n>9){
8010496a:	83 fb 09             	cmp    $0x9,%ebx
8010496d:	7f d1                	jg     80104940 <find_digital_root+0x20>
    }
    n = sum;
  }
  
  return n;
}
8010496f:	89 d8                	mov    %ebx,%eax
80104971:	5b                   	pop    %ebx
80104972:	5e                   	pop    %esi
80104973:	5f                   	pop    %edi
80104974:	5d                   	pop    %ebp
80104975:	c3                   	ret    
80104976:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010497d:	8d 76 00             	lea    0x0(%esi),%esi

80104980 <get_process_lifetime>:

int
get_process_lifetime(void){
80104980:	f3 0f 1e fb          	endbr32 
80104984:	55                   	push   %ebp
80104985:	89 e5                	mov    %esp,%ebp
80104987:	56                   	push   %esi
80104988:	53                   	push   %ebx
  return sys_uptime() - myproc()->start_time ; 
80104989:	e8 e2 27 00 00       	call   80107170 <sys_uptime>
8010498e:	89 c3                	mov    %eax,%ebx
  pushcli();
80104990:	e8 7b 11 00 00       	call   80105b10 <pushcli>
  c = mycpu();
80104995:	e8 96 f4 ff ff       	call   80103e30 <mycpu>
  p = c->proc;
8010499a:	8b b0 ac 00 00 00    	mov    0xac(%eax),%esi
  popcli();
801049a0:	e8 bb 11 00 00       	call   80105b60 <popcli>
  return sys_uptime() - myproc()->start_time ; 
801049a5:	89 d8                	mov    %ebx,%eax
}
801049a7:	5b                   	pop    %ebx
  return sys_uptime() - myproc()->start_time ; 
801049a8:	2b 46 7c             	sub    0x7c(%esi),%eax
}
801049ab:	5e                   	pop    %esi
801049ac:	5d                   	pop    %ebp
801049ad:	c3                   	ret    
801049ae:	66 90                	xchg   %ax,%ax

801049b0 <change_Q>:
  }
  release(&ptable.lock);
}

int change_Q(int pid, int new_queue)
{
801049b0:	f3 0f 1e fb          	endbr32 
801049b4:	55                   	push   %ebp
801049b5:	89 e5                	mov    %esp,%ebp
801049b7:	57                   	push   %edi
801049b8:	56                   	push   %esi
801049b9:	53                   	push   %ebx
801049ba:	83 ec 0c             	sub    $0xc,%esp
801049bd:	8b 75 0c             	mov    0xc(%ebp),%esi
801049c0:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct proc *p;
  int old_queue = -1;

  if (new_queue == UNSET)
801049c3:	85 f6                	test   %esi,%esi
801049c5:	75 0c                	jne    801049d3 <change_Q+0x23>
  {
    if (pid == 1)
801049c7:	83 fb 01             	cmp    $0x1,%ebx
801049ca:	74 69                	je     80104a35 <change_Q+0x85>
      new_queue = ROUND_ROBIN;
    else if (pid > 1)
801049cc:	7e 6e                	jle    80104a3c <change_Q+0x8c>
      new_queue = LCFS;
801049ce:	be 02 00 00 00       	mov    $0x2,%esi
    else
      return -1;
  }
  acquire(&ptable.lock);
801049d3:	83 ec 0c             	sub    $0xc,%esp
  int old_queue = -1;
801049d6:	bf ff ff ff ff       	mov    $0xffffffff,%edi
  acquire(&ptable.lock);
801049db:	68 20 53 11 80       	push   $0x80115320
801049e0:	e8 2b 12 00 00       	call   80105c10 <acquire>
    if (p->pid == pid)
    {
      old_queue = p->sched_info.queue;
      p->sched_info.queue = new_queue;

      p->sched_info.arrival_queue_time = ticks;
801049e5:	8b 15 a0 86 11 80    	mov    0x801186a0,%edx
801049eb:	83 c4 10             	add    $0x10,%esp
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
801049ee:	b8 54 53 11 80       	mov    $0x80115354,%eax
801049f3:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801049f7:	90                   	nop
    if (p->pid == pid)
801049f8:	39 58 10             	cmp    %ebx,0x10(%eax)
801049fb:	75 12                	jne    80104a0f <change_Q+0x5f>
      old_queue = p->sched_info.queue;
801049fd:	8b b8 80 00 00 00    	mov    0x80(%eax),%edi
      p->sched_info.arrival_queue_time = ticks;
80104a03:	89 90 a8 00 00 00    	mov    %edx,0xa8(%eax)
      p->sched_info.queue = new_queue;
80104a09:	89 b0 80 00 00 00    	mov    %esi,0x80(%eax)
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104a0f:	05 ac 00 00 00       	add    $0xac,%eax
80104a14:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
80104a19:	75 dd                	jne    801049f8 <change_Q+0x48>
    }
  }
  release(&ptable.lock);
80104a1b:	83 ec 0c             	sub    $0xc,%esp
80104a1e:	68 20 53 11 80       	push   $0x80115320
80104a23:	e8 a8 12 00 00       	call   80105cd0 <release>
  return old_queue;
80104a28:	83 c4 10             	add    $0x10,%esp
}
80104a2b:	8d 65 f4             	lea    -0xc(%ebp),%esp
80104a2e:	89 f8                	mov    %edi,%eax
80104a30:	5b                   	pop    %ebx
80104a31:	5e                   	pop    %esi
80104a32:	5f                   	pop    %edi
80104a33:	5d                   	pop    %ebp
80104a34:	c3                   	ret    
      new_queue = ROUND_ROBIN;
80104a35:	be 01 00 00 00       	mov    $0x1,%esi
80104a3a:	eb 97                	jmp    801049d3 <change_Q+0x23>
      return -1;
80104a3c:	bf ff ff ff ff       	mov    $0xffffffff,%edi
80104a41:	eb e8                	jmp    80104a2b <change_Q+0x7b>
80104a43:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104a4a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80104a50 <userinit>:
{
80104a50:	f3 0f 1e fb          	endbr32 
80104a54:	55                   	push   %ebp
80104a55:	89 e5                	mov    %esp,%ebp
80104a57:	53                   	push   %ebx
80104a58:	83 ec 04             	sub    $0x4,%esp
  p = allocproc();
80104a5b:	e8 30 f2 ff ff       	call   80103c90 <allocproc>
80104a60:	89 c3                	mov    %eax,%ebx
  initproc = p;
80104a62:	a3 fc c5 10 80       	mov    %eax,0x8010c5fc
  if((p->pgdir = setupkvm()) == 0)
80104a67:	e8 94 3e 00 00       	call   80108900 <setupkvm>
80104a6c:	89 43 04             	mov    %eax,0x4(%ebx)
80104a6f:	85 c0                	test   %eax,%eax
80104a71:	0f 84 c9 00 00 00    	je     80104b40 <userinit+0xf0>
  inituvm(p->pgdir, _binary_initcode_start, (int)_binary_initcode_size);
80104a77:	83 ec 04             	sub    $0x4,%esp
80104a7a:	68 2c 00 00 00       	push   $0x2c
80104a7f:	68 60 c4 10 80       	push   $0x8010c460
80104a84:	50                   	push   %eax
80104a85:	e8 46 3b 00 00       	call   801085d0 <inituvm>
  memset(p->tf, 0, sizeof(*p->tf));
80104a8a:	83 c4 0c             	add    $0xc,%esp
  p->sz = PGSIZE;
80104a8d:	c7 03 00 10 00 00    	movl   $0x1000,(%ebx)
  memset(p->tf, 0, sizeof(*p->tf));
80104a93:	6a 4c                	push   $0x4c
80104a95:	6a 00                	push   $0x0
80104a97:	ff 73 18             	pushl  0x18(%ebx)
80104a9a:	e8 81 12 00 00       	call   80105d20 <memset>
  p->tf->cs = (SEG_UCODE << 3) | DPL_USER;
80104a9f:	8b 43 18             	mov    0x18(%ebx),%eax
80104aa2:	ba 1b 00 00 00       	mov    $0x1b,%edx
  safestrcpy(p->name, "initcode", sizeof(p->name));
80104aa7:	83 c4 0c             	add    $0xc,%esp
  p->tf->ds = (SEG_UDATA << 3) | DPL_USER;
80104aaa:	b9 23 00 00 00       	mov    $0x23,%ecx
  p->tf->cs = (SEG_UCODE << 3) | DPL_USER;
80104aaf:	66 89 50 3c          	mov    %dx,0x3c(%eax)
  p->tf->ds = (SEG_UDATA << 3) | DPL_USER;
80104ab3:	8b 43 18             	mov    0x18(%ebx),%eax
80104ab6:	66 89 48 2c          	mov    %cx,0x2c(%eax)
  p->tf->es = p->tf->ds;
80104aba:	8b 43 18             	mov    0x18(%ebx),%eax
80104abd:	0f b7 50 2c          	movzwl 0x2c(%eax),%edx
80104ac1:	66 89 50 28          	mov    %dx,0x28(%eax)
  p->tf->ss = p->tf->ds;
80104ac5:	8b 43 18             	mov    0x18(%ebx),%eax
80104ac8:	0f b7 50 2c          	movzwl 0x2c(%eax),%edx
80104acc:	66 89 50 48          	mov    %dx,0x48(%eax)
  p->tf->eflags = FL_IF;
80104ad0:	8b 43 18             	mov    0x18(%ebx),%eax
80104ad3:	c7 40 40 00 02 00 00 	movl   $0x200,0x40(%eax)
  p->tf->esp = PGSIZE;
80104ada:	8b 43 18             	mov    0x18(%ebx),%eax
80104add:	c7 40 44 00 10 00 00 	movl   $0x1000,0x44(%eax)
  p->tf->eip = 0;  // beginning of initcode.S
80104ae4:	8b 43 18             	mov    0x18(%ebx),%eax
80104ae7:	c7 40 38 00 00 00 00 	movl   $0x0,0x38(%eax)
  safestrcpy(p->name, "initcode", sizeof(p->name));
80104aee:	8d 43 6c             	lea    0x6c(%ebx),%eax
80104af1:	6a 10                	push   $0x10
80104af3:	68 02 92 10 80       	push   $0x80109202
80104af8:	50                   	push   %eax
80104af9:	e8 e2 13 00 00       	call   80105ee0 <safestrcpy>
  p->cwd = namei("/");
80104afe:	c7 04 24 0b 92 10 80 	movl   $0x8010920b,(%esp)
80104b05:	e8 16 da ff ff       	call   80102520 <namei>
80104b0a:	89 43 68             	mov    %eax,0x68(%ebx)
  acquire(&ptable.lock);
80104b0d:	c7 04 24 20 53 11 80 	movl   $0x80115320,(%esp)
80104b14:	e8 f7 10 00 00       	call   80105c10 <acquire>
  p->state = RUNNABLE;
80104b19:	c7 43 0c 03 00 00 00 	movl   $0x3,0xc(%ebx)
  release(&ptable.lock);
80104b20:	c7 04 24 20 53 11 80 	movl   $0x80115320,(%esp)
80104b27:	e8 a4 11 00 00       	call   80105cd0 <release>
  change_Q(p->pid, UNSET);
80104b2c:	58                   	pop    %eax
80104b2d:	5a                   	pop    %edx
80104b2e:	6a 00                	push   $0x0
80104b30:	ff 73 10             	pushl  0x10(%ebx)
80104b33:	e8 78 fe ff ff       	call   801049b0 <change_Q>
}
80104b38:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80104b3b:	83 c4 10             	add    $0x10,%esp
80104b3e:	c9                   	leave  
80104b3f:	c3                   	ret    
    panic("userinit: out of memory?");
80104b40:	83 ec 0c             	sub    $0xc,%esp
80104b43:	68 e9 91 10 80       	push   $0x801091e9
80104b48:	e8 43 b8 ff ff       	call   80100390 <panic>
80104b4d:	8d 76 00             	lea    0x0(%esi),%esi

80104b50 <fork>:
{
80104b50:	f3 0f 1e fb          	endbr32 
80104b54:	55                   	push   %ebp
80104b55:	89 e5                	mov    %esp,%ebp
80104b57:	57                   	push   %edi
80104b58:	56                   	push   %esi
80104b59:	53                   	push   %ebx
80104b5a:	83 ec 1c             	sub    $0x1c,%esp
  pushcli();
80104b5d:	e8 ae 0f 00 00       	call   80105b10 <pushcli>
  c = mycpu();
80104b62:	e8 c9 f2 ff ff       	call   80103e30 <mycpu>
  p = c->proc;
80104b67:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
80104b6d:	e8 ee 0f 00 00       	call   80105b60 <popcli>
  if((np = allocproc()) == 0){
80104b72:	e8 19 f1 ff ff       	call   80103c90 <allocproc>
80104b77:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80104b7a:	85 c0                	test   %eax,%eax
80104b7c:	0f 84 c7 00 00 00    	je     80104c49 <fork+0xf9>
  if((np->pgdir = copyuvm(curproc->pgdir, curproc->sz)) == 0){
80104b82:	83 ec 08             	sub    $0x8,%esp
80104b85:	ff 33                	pushl  (%ebx)
80104b87:	89 c7                	mov    %eax,%edi
80104b89:	ff 73 04             	pushl  0x4(%ebx)
80104b8c:	e8 3f 3e 00 00       	call   801089d0 <copyuvm>
80104b91:	83 c4 10             	add    $0x10,%esp
80104b94:	89 47 04             	mov    %eax,0x4(%edi)
80104b97:	85 c0                	test   %eax,%eax
80104b99:	0f 84 b1 00 00 00    	je     80104c50 <fork+0x100>
  np->sz = curproc->sz;
80104b9f:	8b 03                	mov    (%ebx),%eax
80104ba1:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
80104ba4:	89 01                	mov    %eax,(%ecx)
  *np->tf = *curproc->tf;
80104ba6:	8b 79 18             	mov    0x18(%ecx),%edi
  np->parent = curproc;
80104ba9:	89 c8                	mov    %ecx,%eax
80104bab:	89 59 14             	mov    %ebx,0x14(%ecx)
  *np->tf = *curproc->tf;
80104bae:	b9 13 00 00 00       	mov    $0x13,%ecx
80104bb3:	8b 73 18             	mov    0x18(%ebx),%esi
80104bb6:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
  for(i = 0; i < NOFILE; i++)
80104bb8:	31 f6                	xor    %esi,%esi
  np->tf->eax = 0;
80104bba:	8b 40 18             	mov    0x18(%eax),%eax
80104bbd:	c7 40 1c 00 00 00 00 	movl   $0x0,0x1c(%eax)
  for(i = 0; i < NOFILE; i++)
80104bc4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    if(curproc->ofile[i])
80104bc8:	8b 44 b3 28          	mov    0x28(%ebx,%esi,4),%eax
80104bcc:	85 c0                	test   %eax,%eax
80104bce:	74 13                	je     80104be3 <fork+0x93>
      np->ofile[i] = filedup(curproc->ofile[i]);
80104bd0:	83 ec 0c             	sub    $0xc,%esp
80104bd3:	50                   	push   %eax
80104bd4:	e8 87 c7 ff ff       	call   80101360 <filedup>
80104bd9:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80104bdc:	83 c4 10             	add    $0x10,%esp
80104bdf:	89 44 b2 28          	mov    %eax,0x28(%edx,%esi,4)
  for(i = 0; i < NOFILE; i++)
80104be3:	83 c6 01             	add    $0x1,%esi
80104be6:	83 fe 10             	cmp    $0x10,%esi
80104be9:	75 dd                	jne    80104bc8 <fork+0x78>
  np->cwd = idup(curproc->cwd);
80104beb:	83 ec 0c             	sub    $0xc,%esp
80104bee:	ff 73 68             	pushl  0x68(%ebx)
  safestrcpy(np->name, curproc->name, sizeof(curproc->name));
80104bf1:	83 c3 6c             	add    $0x6c,%ebx
  np->cwd = idup(curproc->cwd);
80104bf4:	e8 27 d0 ff ff       	call   80101c20 <idup>
80104bf9:	8b 7d e4             	mov    -0x1c(%ebp),%edi
  safestrcpy(np->name, curproc->name, sizeof(curproc->name));
80104bfc:	83 c4 0c             	add    $0xc,%esp
  np->cwd = idup(curproc->cwd);
80104bff:	89 47 68             	mov    %eax,0x68(%edi)
  safestrcpy(np->name, curproc->name, sizeof(curproc->name));
80104c02:	8d 47 6c             	lea    0x6c(%edi),%eax
80104c05:	6a 10                	push   $0x10
80104c07:	53                   	push   %ebx
80104c08:	50                   	push   %eax
80104c09:	e8 d2 12 00 00       	call   80105ee0 <safestrcpy>
  pid = np->pid;
80104c0e:	8b 5f 10             	mov    0x10(%edi),%ebx
  acquire(&ptable.lock);
80104c11:	c7 04 24 20 53 11 80 	movl   $0x80115320,(%esp)
80104c18:	e8 f3 0f 00 00       	call   80105c10 <acquire>
  np->state = RUNNABLE;
80104c1d:	c7 47 0c 03 00 00 00 	movl   $0x3,0xc(%edi)
  release(&ptable.lock);
80104c24:	c7 04 24 20 53 11 80 	movl   $0x80115320,(%esp)
80104c2b:	e8 a0 10 00 00       	call   80105cd0 <release>
  change_Q(np->pid, UNSET);
80104c30:	58                   	pop    %eax
80104c31:	5a                   	pop    %edx
80104c32:	6a 00                	push   $0x0
80104c34:	ff 77 10             	pushl  0x10(%edi)
80104c37:	e8 74 fd ff ff       	call   801049b0 <change_Q>
  return pid;
80104c3c:	83 c4 10             	add    $0x10,%esp
}
80104c3f:	8d 65 f4             	lea    -0xc(%ebp),%esp
80104c42:	89 d8                	mov    %ebx,%eax
80104c44:	5b                   	pop    %ebx
80104c45:	5e                   	pop    %esi
80104c46:	5f                   	pop    %edi
80104c47:	5d                   	pop    %ebp
80104c48:	c3                   	ret    
    return -1;
80104c49:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
80104c4e:	eb ef                	jmp    80104c3f <fork+0xef>
    kfree(np->kstack);
80104c50:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
80104c53:	83 ec 0c             	sub    $0xc,%esp
80104c56:	ff 73 08             	pushl  0x8(%ebx)
80104c59:	e8 02 dd ff ff       	call   80102960 <kfree>
    np->kstack = 0;
80104c5e:	c7 43 08 00 00 00 00 	movl   $0x0,0x8(%ebx)
    return -1;
80104c65:	83 c4 10             	add    $0x10,%esp
    np->state = UNUSED;
80104c68:	c7 43 0c 00 00 00 00 	movl   $0x0,0xc(%ebx)
    return -1;
80104c6f:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
80104c74:	eb c9                	jmp    80104c3f <fork+0xef>
80104c76:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104c7d:	8d 76 00             	lea    0x0(%esi),%esi

80104c80 <ageprocs>:
{
80104c80:	f3 0f 1e fb          	endbr32 
80104c84:	55                   	push   %ebp
80104c85:	89 e5                	mov    %esp,%ebp
80104c87:	56                   	push   %esi
80104c88:	53                   	push   %ebx
80104c89:	8b 75 08             	mov    0x8(%ebp),%esi
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104c8c:	bb 54 53 11 80       	mov    $0x80115354,%ebx
  acquire(&ptable.lock);
80104c91:	83 ec 0c             	sub    $0xc,%esp
80104c94:	68 20 53 11 80       	push   $0x80115320
80104c99:	e8 72 0f 00 00       	call   80105c10 <acquire>
80104c9e:	83 c4 10             	add    $0x10,%esp
80104ca1:	eb 13                	jmp    80104cb6 <ageprocs+0x36>
80104ca3:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104ca7:	90                   	nop
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104ca8:	81 c3 ac 00 00 00    	add    $0xac,%ebx
80104cae:	81 fb 54 7e 11 80    	cmp    $0x80117e54,%ebx
80104cb4:	74 57                	je     80104d0d <ageprocs+0x8d>
    if (p->state == RUNNABLE && p->sched_info.queue != ROUND_ROBIN)
80104cb6:	83 7b 0c 03          	cmpl   $0x3,0xc(%ebx)
80104cba:	75 ec                	jne    80104ca8 <ageprocs+0x28>
80104cbc:	83 bb 80 00 00 00 01 	cmpl   $0x1,0x80(%ebx)
80104cc3:	74 e3                	je     80104ca8 <ageprocs+0x28>
      if (os_ticks - p->sched_info.last_run > AGING_THRESHOLD)
80104cc5:	89 f0                	mov    %esi,%eax
80104cc7:	2b 83 84 00 00 00    	sub    0x84(%ebx),%eax
80104ccd:	3d 40 1f 00 00       	cmp    $0x1f40,%eax
80104cd2:	7e d4                	jle    80104ca8 <ageprocs+0x28>
        release(&ptable.lock);
80104cd4:	83 ec 0c             	sub    $0xc,%esp
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104cd7:	81 c3 ac 00 00 00    	add    $0xac,%ebx
        release(&ptable.lock);
80104cdd:	68 20 53 11 80       	push   $0x80115320
80104ce2:	e8 e9 0f 00 00       	call   80105cd0 <release>
        change_Q(p->pid, ROUND_ROBIN);
80104ce7:	58                   	pop    %eax
80104ce8:	5a                   	pop    %edx
80104ce9:	6a 01                	push   $0x1
80104ceb:	ff b3 64 ff ff ff    	pushl  -0x9c(%ebx)
80104cf1:	e8 ba fc ff ff       	call   801049b0 <change_Q>
        acquire(&ptable.lock);
80104cf6:	c7 04 24 20 53 11 80 	movl   $0x80115320,(%esp)
80104cfd:	e8 0e 0f 00 00       	call   80105c10 <acquire>
80104d02:	83 c4 10             	add    $0x10,%esp
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104d05:	81 fb 54 7e 11 80    	cmp    $0x80117e54,%ebx
80104d0b:	75 a9                	jne    80104cb6 <ageprocs+0x36>
  release(&ptable.lock);
80104d0d:	c7 45 08 20 53 11 80 	movl   $0x80115320,0x8(%ebp)
}
80104d14:	8d 65 f8             	lea    -0x8(%ebp),%esp
80104d17:	5b                   	pop    %ebx
80104d18:	5e                   	pop    %esi
80104d19:	5d                   	pop    %ebp
  release(&ptable.lock);
80104d1a:	e9 b1 0f 00 00       	jmp    80105cd0 <release>
80104d1f:	90                   	nop

80104d20 <num_digits>:

int num_digits(int n) {
80104d20:	f3 0f 1e fb          	endbr32 
80104d24:	55                   	push   %ebp
80104d25:	89 e5                	mov    %esp,%ebp
80104d27:	56                   	push   %esi
80104d28:	8b 4d 08             	mov    0x8(%ebp),%ecx
80104d2b:	53                   	push   %ebx
  int num = 0;
80104d2c:	31 db                	xor    %ebx,%ebx
  while(n!= 0) {
80104d2e:	85 c9                	test   %ecx,%ecx
80104d30:	74 21                	je     80104d53 <num_digits+0x33>
    n/=10;
80104d32:	be 67 66 66 66       	mov    $0x66666667,%esi
80104d37:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104d3e:	66 90                	xchg   %ax,%ax
80104d40:	89 c8                	mov    %ecx,%eax
80104d42:	c1 f9 1f             	sar    $0x1f,%ecx
    num += 1;
80104d45:	83 c3 01             	add    $0x1,%ebx
    n/=10;
80104d48:	f7 ee                	imul   %esi
80104d4a:	c1 fa 02             	sar    $0x2,%edx
  while(n!= 0) {
80104d4d:	29 ca                	sub    %ecx,%edx
80104d4f:	89 d1                	mov    %edx,%ecx
80104d51:	75 ed                	jne    80104d40 <num_digits+0x20>
  }
  return num;
}
80104d53:	89 d8                	mov    %ebx,%eax
80104d55:	5b                   	pop    %ebx
80104d56:	5e                   	pop    %esi
80104d57:	5d                   	pop    %ebp
80104d58:	c3                   	ret    
80104d59:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80104d60 <space>:

void
space(int count)
{
80104d60:	f3 0f 1e fb          	endbr32 
80104d64:	55                   	push   %ebp
80104d65:	89 e5                	mov    %esp,%ebp
80104d67:	56                   	push   %esi
80104d68:	8b 75 08             	mov    0x8(%ebp),%esi
80104d6b:	53                   	push   %ebx
  for(int i = 0; i < count; ++i)
80104d6c:	85 f6                	test   %esi,%esi
80104d6e:	7e 1f                	jle    80104d8f <space+0x2f>
80104d70:	31 db                	xor    %ebx,%ebx
80104d72:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    cprintf(" ");
80104d78:	83 ec 0c             	sub    $0xc,%esp
  for(int i = 0; i < count; ++i)
80104d7b:	83 c3 01             	add    $0x1,%ebx
    cprintf(" ");
80104d7e:	68 9e 92 10 80       	push   $0x8010929e
80104d83:	e8 38 bb ff ff       	call   801008c0 <cprintf>
  for(int i = 0; i < count; ++i)
80104d88:	83 c4 10             	add    $0x10,%esp
80104d8b:	39 de                	cmp    %ebx,%esi
80104d8d:	75 e9                	jne    80104d78 <space+0x18>
}
80104d8f:	8d 65 f8             	lea    -0x8(%ebp),%esp
80104d92:	5b                   	pop    %ebx
80104d93:	5e                   	pop    %esi
80104d94:	5d                   	pop    %ebp
80104d95:	c3                   	ret    
80104d96:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104d9d:	8d 76 00             	lea    0x0(%esi),%esi

80104da0 <set_proc_bjf_params>:

int set_proc_bjf_params(int pid, float priority_ratio, float arrival_time_ratio, float executed_cycle_ratio, float process_size_ratio)
{
80104da0:	f3 0f 1e fb          	endbr32 
80104da4:	55                   	push   %ebp
80104da5:	89 e5                	mov    %esp,%ebp
80104da7:	53                   	push   %ebx
80104da8:	83 ec 10             	sub    $0x10,%esp
80104dab:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct proc *p;

  acquire(&ptable.lock);
80104dae:	68 20 53 11 80       	push   $0x80115320
80104db3:	e8 58 0e 00 00       	call   80105c10 <acquire>
80104db8:	83 c4 10             	add    $0x10,%esp
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104dbb:	b8 54 53 11 80       	mov    $0x80115354,%eax
80104dc0:	eb 12                	jmp    80104dd4 <set_proc_bjf_params+0x34>
80104dc2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80104dc8:	05 ac 00 00 00       	add    $0xac,%eax
80104dcd:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
80104dd2:	74 44                	je     80104e18 <set_proc_bjf_params+0x78>
  {
    if (p->pid == pid)
80104dd4:	39 58 10             	cmp    %ebx,0x10(%eax)
80104dd7:	75 ef                	jne    80104dc8 <set_proc_bjf_params+0x28>
    {
      p->sched_info.bjf.priority_ratio = priority_ratio;
80104dd9:	d9 45 0c             	flds   0xc(%ebp)
      p->sched_info.bjf.arrival_time_ratio = arrival_time_ratio;
      p->sched_info.bjf.executed_cycle_ratio = executed_cycle_ratio;
      p->sched_info.bjf.process_size_ratio = process_size_ratio;
      release(&ptable.lock);
80104ddc:	83 ec 0c             	sub    $0xc,%esp
      p->sched_info.bjf.priority_ratio = priority_ratio;
80104ddf:	d9 98 8c 00 00 00    	fstps  0x8c(%eax)
      p->sched_info.bjf.arrival_time_ratio = arrival_time_ratio;
80104de5:	d9 45 10             	flds   0x10(%ebp)
80104de8:	d9 98 94 00 00 00    	fstps  0x94(%eax)
      p->sched_info.bjf.executed_cycle_ratio = executed_cycle_ratio;
80104dee:	d9 45 14             	flds   0x14(%ebp)
80104df1:	d9 98 9c 00 00 00    	fstps  0x9c(%eax)
      p->sched_info.bjf.process_size_ratio = process_size_ratio;
80104df7:	d9 45 18             	flds   0x18(%ebp)
80104dfa:	d9 98 a4 00 00 00    	fstps  0xa4(%eax)
      release(&ptable.lock);
80104e00:	68 20 53 11 80       	push   $0x80115320
80104e05:	e8 c6 0e 00 00       	call   80105cd0 <release>
      return 0;
    }
  }
  release(&ptable.lock);
  return -1;
}
80104e0a:	8b 5d fc             	mov    -0x4(%ebp),%ebx
      return 0;
80104e0d:	83 c4 10             	add    $0x10,%esp
80104e10:	31 c0                	xor    %eax,%eax
}
80104e12:	c9                   	leave  
80104e13:	c3                   	ret    
80104e14:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  release(&ptable.lock);
80104e18:	83 ec 0c             	sub    $0xc,%esp
80104e1b:	68 20 53 11 80       	push   $0x80115320
80104e20:	e8 ab 0e 00 00       	call   80105cd0 <release>
}
80104e25:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  return -1;
80104e28:	83 c4 10             	add    $0x10,%esp
80104e2b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104e30:	c9                   	leave  
80104e31:	c3                   	ret    
80104e32:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104e39:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80104e40 <set_system_bjf_params>:

int set_system_bjf_params(float priority_ratio, float arrival_time_ratio, float executed_cycle_ratio, float process_size_ratio)
{
80104e40:	f3 0f 1e fb          	endbr32 
80104e44:	55                   	push   %ebp
80104e45:	89 e5                	mov    %esp,%ebp
80104e47:	83 ec 14             	sub    $0x14,%esp
  struct proc *p;

  acquire(&ptable.lock);
80104e4a:	68 20 53 11 80       	push   $0x80115320
80104e4f:	e8 bc 0d 00 00       	call   80105c10 <acquire>
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104e54:	d9 45 08             	flds   0x8(%ebp)
80104e57:	d9 45 0c             	flds   0xc(%ebp)
  acquire(&ptable.lock);
80104e5a:	83 c4 10             	add    $0x10,%esp
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104e5d:	d9 45 10             	flds   0x10(%ebp)
80104e60:	d9 45 14             	flds   0x14(%ebp)
80104e63:	d9 cb                	fxch   %st(3)
80104e65:	b8 54 53 11 80       	mov    $0x80115354,%eax
80104e6a:	eb 0a                	jmp    80104e76 <set_system_bjf_params+0x36>
80104e6c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104e70:	d9 cb                	fxch   %st(3)
80104e72:	d9 c9                	fxch   %st(1)
80104e74:	d9 ca                	fxch   %st(2)
  {
    p->sched_info.bjf.priority_ratio = priority_ratio;
80104e76:	d9 90 8c 00 00 00    	fsts   0x8c(%eax)
80104e7c:	d9 ca                	fxch   %st(2)
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104e7e:	05 ac 00 00 00       	add    $0xac,%eax
    p->sched_info.bjf.arrival_time_ratio = arrival_time_ratio;
80104e83:	d9 50 e8             	fsts   -0x18(%eax)
80104e86:	d9 c9                	fxch   %st(1)
    p->sched_info.bjf.executed_cycle_ratio = executed_cycle_ratio;
80104e88:	d9 50 f0             	fsts   -0x10(%eax)
80104e8b:	d9 cb                	fxch   %st(3)
    p->sched_info.bjf.process_size_ratio = process_size_ratio;
80104e8d:	d9 50 f8             	fsts   -0x8(%eax)
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104e90:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
80104e95:	75 d9                	jne    80104e70 <set_system_bjf_params+0x30>
80104e97:	dd d8                	fstp   %st(0)
80104e99:	dd d8                	fstp   %st(0)
80104e9b:	dd d8                	fstp   %st(0)
80104e9d:	dd d8                	fstp   %st(0)
  }
  release(&ptable.lock);
80104e9f:	83 ec 0c             	sub    $0xc,%esp
80104ea2:	68 20 53 11 80       	push   $0x80115320
80104ea7:	e8 24 0e 00 00       	call   80105cd0 <release>
  return 0;
}
80104eac:	31 c0                	xor    %eax,%eax
80104eae:	c9                   	leave  
80104eaf:	c3                   	ret    

80104eb0 <show_process_info>:

void show_process_info()
{
80104eb0:	f3 0f 1e fb          	endbr32 
80104eb4:	55                   	push   %ebp
80104eb5:	89 e5                	mov    %esp,%ebp
80104eb7:	57                   	push   %edi
80104eb8:	56                   	push   %esi
  static int columns[] = {16, 8, 9, 8, 8, 8, 9, 8, 8, 8, 8};
  cprintf("Process_Name    PID     State    Queue   Cycle   Arrival Priority R_Prty  R_Arvl  R_Exec  R_Size  Rank\n"
          "------------------------------------------------------------------------------------------------------\n");

  struct proc *p;
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104eb9:	be 54 53 11 80       	mov    $0x80115354,%esi
{
80104ebe:	53                   	push   %ebx
    n/=10;
80104ebf:	bb 67 66 66 66       	mov    $0x66666667,%ebx
{
80104ec4:	83 ec 28             	sub    $0x28,%esp
  cprintf("Process_Name    PID     State    Queue   Cycle   Arrival Priority R_Prty  R_Arvl  R_Exec  R_Size  Rank\n"
80104ec7:	68 c8 92 10 80       	push   $0x801092c8
80104ecc:	e8 ef b9 ff ff       	call   801008c0 <cprintf>
80104ed1:	83 c4 10             	add    $0x10,%esp
80104ed4:	eb 1c                	jmp    80104ef2 <show_process_info+0x42>
80104ed6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104edd:	8d 76 00             	lea    0x0(%esi),%esi
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104ee0:	81 c6 ac 00 00 00    	add    $0xac,%esi
80104ee6:	81 fe 54 7e 11 80    	cmp    $0x80117e54,%esi
80104eec:	0f 84 7a 06 00 00    	je     8010556c <show_process_info+0x6bc>
  {
    if (p->state == UNUSED)
80104ef2:	8b 46 0c             	mov    0xc(%esi),%eax
80104ef5:	85 c0                	test   %eax,%eax
80104ef7:	74 e7                	je     80104ee0 <show_process_info+0x30>

    const char *state;
    if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
      state = states[p->state];
    else
      state = "unknown state";
80104ef9:	c7 45 e0 0d 92 10 80 	movl   $0x8010920d,-0x20(%ebp)
    if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
80104f00:	83 f8 05             	cmp    $0x5,%eax
80104f03:	77 14                	ja     80104f19 <show_process_info+0x69>
80104f05:	8b 3c 85 2c 94 10 80 	mov    -0x7fef6bd4(,%eax,4),%edi
      state = "unknown state";
80104f0c:	b8 0d 92 10 80       	mov    $0x8010920d,%eax
80104f11:	85 ff                	test   %edi,%edi
80104f13:	0f 45 c7             	cmovne %edi,%eax
80104f16:	89 45 e0             	mov    %eax,-0x20(%ebp)

    cprintf("%s", p->name);
80104f19:	83 ec 08             	sub    $0x8,%esp
80104f1c:	8d 7e 6c             	lea    0x6c(%esi),%edi
80104f1f:	57                   	push   %edi
80104f20:	68 e6 91 10 80       	push   $0x801091e6
80104f25:	e8 96 b9 ff ff       	call   801008c0 <cprintf>
    space(columns[0] - strlen(p->name));
80104f2a:	89 3c 24             	mov    %edi,(%esp)
80104f2d:	bf 10 00 00 00       	mov    $0x10,%edi
80104f32:	e8 e9 0f 00 00       	call   80105f20 <strlen>
  for(int i = 0; i < count; ++i)
80104f37:	83 c4 10             	add    $0x10,%esp
    space(columns[0] - strlen(p->name));
80104f3a:	29 c7                	sub    %eax,%edi
80104f3c:	89 f8                	mov    %edi,%eax
  for(int i = 0; i < count; ++i)
80104f3e:	31 ff                	xor    %edi,%edi
80104f40:	85 c0                	test   %eax,%eax
80104f42:	7e 26                	jle    80104f6a <show_process_info+0xba>
80104f44:	89 75 dc             	mov    %esi,-0x24(%ebp)
80104f47:	89 fe                	mov    %edi,%esi
80104f49:	89 c7                	mov    %eax,%edi
80104f4b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104f4f:	90                   	nop
    cprintf(" ");
80104f50:	83 ec 0c             	sub    $0xc,%esp
  for(int i = 0; i < count; ++i)
80104f53:	83 c6 01             	add    $0x1,%esi
    cprintf(" ");
80104f56:	68 9e 92 10 80       	push   $0x8010929e
80104f5b:	e8 60 b9 ff ff       	call   801008c0 <cprintf>
  for(int i = 0; i < count; ++i)
80104f60:	83 c4 10             	add    $0x10,%esp
80104f63:	39 f7                	cmp    %esi,%edi
80104f65:	75 e9                	jne    80104f50 <show_process_info+0xa0>
80104f67:	8b 75 dc             	mov    -0x24(%ebp),%esi

    cprintf("%d", p->pid);
80104f6a:	83 ec 08             	sub    $0x8,%esp
80104f6d:	ff 76 10             	pushl  0x10(%esi)
  int num = 0;
80104f70:	31 ff                	xor    %edi,%edi
    cprintf("%d", p->pid);
80104f72:	68 1b 92 10 80       	push   $0x8010921b
80104f77:	e8 44 b9 ff ff       	call   801008c0 <cprintf>
    space(columns[1] - num_digits(p->pid));
80104f7c:	8b 4e 10             	mov    0x10(%esi),%ecx
  while(n!= 0) {
80104f7f:	83 c4 10             	add    $0x10,%esp
    space(columns[1] - num_digits(p->pid));
80104f82:	b8 08 00 00 00       	mov    $0x8,%eax
  while(n!= 0) {
80104f87:	85 c9                	test   %ecx,%ecx
80104f89:	74 23                	je     80104fae <show_process_info+0xfe>
80104f8b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104f8f:	90                   	nop
    n/=10;
80104f90:	89 c8                	mov    %ecx,%eax
80104f92:	c1 f9 1f             	sar    $0x1f,%ecx
    num += 1;
80104f95:	83 c7 01             	add    $0x1,%edi
    n/=10;
80104f98:	f7 eb                	imul   %ebx
80104f9a:	c1 fa 02             	sar    $0x2,%edx
  while(n!= 0) {
80104f9d:	29 ca                	sub    %ecx,%edx
80104f9f:	89 d1                	mov    %edx,%ecx
80104fa1:	75 ed                	jne    80104f90 <show_process_info+0xe0>
    space(columns[1] - num_digits(p->pid));
80104fa3:	b8 08 00 00 00       	mov    $0x8,%eax
80104fa8:	29 f8                	sub    %edi,%eax
  for(int i = 0; i < count; ++i)
80104faa:	85 c0                	test   %eax,%eax
80104fac:	7e 2c                	jle    80104fda <show_process_info+0x12a>
    space(columns[1] - num_digits(p->pid));
80104fae:	31 ff                	xor    %edi,%edi
80104fb0:	89 75 dc             	mov    %esi,-0x24(%ebp)
80104fb3:	89 fe                	mov    %edi,%esi
80104fb5:	89 c7                	mov    %eax,%edi
80104fb7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104fbe:	66 90                	xchg   %ax,%ax
    cprintf(" ");
80104fc0:	83 ec 0c             	sub    $0xc,%esp
  for(int i = 0; i < count; ++i)
80104fc3:	83 c6 01             	add    $0x1,%esi
    cprintf(" ");
80104fc6:	68 9e 92 10 80       	push   $0x8010929e
80104fcb:	e8 f0 b8 ff ff       	call   801008c0 <cprintf>
  for(int i = 0; i < count; ++i)
80104fd0:	83 c4 10             	add    $0x10,%esp
80104fd3:	39 fe                	cmp    %edi,%esi
80104fd5:	7c e9                	jl     80104fc0 <show_process_info+0x110>
80104fd7:	8b 75 dc             	mov    -0x24(%ebp),%esi

    cprintf("%s", state);
80104fda:	8b 7d e0             	mov    -0x20(%ebp),%edi
80104fdd:	83 ec 08             	sub    $0x8,%esp
80104fe0:	57                   	push   %edi
80104fe1:	68 e6 91 10 80       	push   $0x801091e6
80104fe6:	e8 d5 b8 ff ff       	call   801008c0 <cprintf>
    space(columns[2] - strlen(state));
80104feb:	89 3c 24             	mov    %edi,(%esp)
80104fee:	bf 09 00 00 00       	mov    $0x9,%edi
80104ff3:	e8 28 0f 00 00       	call   80105f20 <strlen>
  for(int i = 0; i < count; ++i)
80104ff8:	83 c4 10             	add    $0x10,%esp
    space(columns[2] - strlen(state));
80104ffb:	29 c7                	sub    %eax,%edi
80104ffd:	89 f8                	mov    %edi,%eax
  for(int i = 0; i < count; ++i)
80104fff:	31 ff                	xor    %edi,%edi
80105001:	85 c0                	test   %eax,%eax
80105003:	7e 25                	jle    8010502a <show_process_info+0x17a>
80105005:	89 75 e0             	mov    %esi,-0x20(%ebp)
80105008:	89 fe                	mov    %edi,%esi
8010500a:	89 c7                	mov    %eax,%edi
8010500c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    cprintf(" ");
80105010:	83 ec 0c             	sub    $0xc,%esp
  for(int i = 0; i < count; ++i)
80105013:	83 c6 01             	add    $0x1,%esi
    cprintf(" ");
80105016:	68 9e 92 10 80       	push   $0x8010929e
8010501b:	e8 a0 b8 ff ff       	call   801008c0 <cprintf>
  for(int i = 0; i < count; ++i)
80105020:	83 c4 10             	add    $0x10,%esp
80105023:	39 f7                	cmp    %esi,%edi
80105025:	75 e9                	jne    80105010 <show_process_info+0x160>
80105027:	8b 75 e0             	mov    -0x20(%ebp),%esi

    cprintf("%d", p->sched_info.queue);
8010502a:	83 ec 08             	sub    $0x8,%esp
8010502d:	ff b6 80 00 00 00    	pushl  0x80(%esi)
  int num = 0;
80105033:	31 ff                	xor    %edi,%edi
    cprintf("%d", p->sched_info.queue);
80105035:	68 1b 92 10 80       	push   $0x8010921b
8010503a:	e8 81 b8 ff ff       	call   801008c0 <cprintf>
    space(columns[3] - num_digits(p->sched_info.queue));
8010503f:	8b 8e 80 00 00 00    	mov    0x80(%esi),%ecx
  while(n!= 0) {
80105045:	83 c4 10             	add    $0x10,%esp
    space(columns[3] - num_digits(p->sched_info.queue));
80105048:	b8 08 00 00 00       	mov    $0x8,%eax
  while(n!= 0) {
8010504d:	85 c9                	test   %ecx,%ecx
8010504f:	74 25                	je     80105076 <show_process_info+0x1c6>
80105051:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    n/=10;
80105058:	89 c8                	mov    %ecx,%eax
8010505a:	c1 f9 1f             	sar    $0x1f,%ecx
    num += 1;
8010505d:	83 c7 01             	add    $0x1,%edi
    n/=10;
80105060:	f7 eb                	imul   %ebx
80105062:	c1 fa 02             	sar    $0x2,%edx
  while(n!= 0) {
80105065:	29 ca                	sub    %ecx,%edx
80105067:	89 d1                	mov    %edx,%ecx
80105069:	75 ed                	jne    80105058 <show_process_info+0x1a8>
    space(columns[3] - num_digits(p->sched_info.queue));
8010506b:	b8 08 00 00 00       	mov    $0x8,%eax
80105070:	29 f8                	sub    %edi,%eax
  for(int i = 0; i < count; ++i)
80105072:	85 c0                	test   %eax,%eax
80105074:	7e 24                	jle    8010509a <show_process_info+0x1ea>
    space(columns[3] - num_digits(p->sched_info.queue));
80105076:	31 ff                	xor    %edi,%edi
80105078:	89 75 e0             	mov    %esi,-0x20(%ebp)
8010507b:	89 fe                	mov    %edi,%esi
8010507d:	89 c7                	mov    %eax,%edi
8010507f:	90                   	nop
    cprintf(" ");
80105080:	83 ec 0c             	sub    $0xc,%esp
  for(int i = 0; i < count; ++i)
80105083:	83 c6 01             	add    $0x1,%esi
    cprintf(" ");
80105086:	68 9e 92 10 80       	push   $0x8010929e
8010508b:	e8 30 b8 ff ff       	call   801008c0 <cprintf>
  for(int i = 0; i < count; ++i)
80105090:	83 c4 10             	add    $0x10,%esp
80105093:	39 f7                	cmp    %esi,%edi
80105095:	7f e9                	jg     80105080 <show_process_info+0x1d0>
80105097:	8b 75 e0             	mov    -0x20(%ebp),%esi

    cprintf("%d", (int)p->sched_info.bjf.executed_cycle);
8010509a:	d9 86 98 00 00 00    	flds   0x98(%esi)
801050a0:	83 ec 08             	sub    $0x8,%esp
  int num = 0;
801050a3:	31 ff                	xor    %edi,%edi
    cprintf("%d", (int)p->sched_info.bjf.executed_cycle);
801050a5:	d9 7d e6             	fnstcw -0x1a(%ebp)
801050a8:	0f b7 45 e6          	movzwl -0x1a(%ebp),%eax
801050ac:	80 cc 0c             	or     $0xc,%ah
801050af:	66 89 45 e4          	mov    %ax,-0x1c(%ebp)
801050b3:	d9 6d e4             	fldcw  -0x1c(%ebp)
801050b6:	db 5d e0             	fistpl -0x20(%ebp)
801050b9:	d9 6d e6             	fldcw  -0x1a(%ebp)
801050bc:	8b 45 e0             	mov    -0x20(%ebp),%eax
801050bf:	50                   	push   %eax
801050c0:	68 1b 92 10 80       	push   $0x8010921b
801050c5:	e8 f6 b7 ff ff       	call   801008c0 <cprintf>
    space(columns[4] - num_digits((int)p->sched_info.bjf.executed_cycle));
801050ca:	d9 86 98 00 00 00    	flds   0x98(%esi)
  while(n!= 0) {
801050d0:	83 c4 10             	add    $0x10,%esp
    space(columns[4] - num_digits((int)p->sched_info.bjf.executed_cycle));
801050d3:	d9 7d e6             	fnstcw -0x1a(%ebp)
801050d6:	0f b7 45 e6          	movzwl -0x1a(%ebp),%eax
801050da:	80 cc 0c             	or     $0xc,%ah
801050dd:	66 89 45 e4          	mov    %ax,-0x1c(%ebp)
801050e1:	b8 08 00 00 00       	mov    $0x8,%eax
801050e6:	d9 6d e4             	fldcw  -0x1c(%ebp)
801050e9:	db 5d e0             	fistpl -0x20(%ebp)
801050ec:	d9 6d e6             	fldcw  -0x1a(%ebp)
801050ef:	8b 4d e0             	mov    -0x20(%ebp),%ecx
  while(n!= 0) {
801050f2:	85 c9                	test   %ecx,%ecx
801050f4:	74 28                	je     8010511e <show_process_info+0x26e>
801050f6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801050fd:	8d 76 00             	lea    0x0(%esi),%esi
    n/=10;
80105100:	89 c8                	mov    %ecx,%eax
80105102:	c1 f9 1f             	sar    $0x1f,%ecx
    num += 1;
80105105:	83 c7 01             	add    $0x1,%edi
    n/=10;
80105108:	f7 eb                	imul   %ebx
8010510a:	c1 fa 02             	sar    $0x2,%edx
  while(n!= 0) {
8010510d:	29 ca                	sub    %ecx,%edx
8010510f:	89 d1                	mov    %edx,%ecx
80105111:	75 ed                	jne    80105100 <show_process_info+0x250>
    space(columns[4] - num_digits((int)p->sched_info.bjf.executed_cycle));
80105113:	b8 08 00 00 00       	mov    $0x8,%eax
80105118:	29 f8                	sub    %edi,%eax
  for(int i = 0; i < count; ++i)
8010511a:	85 c0                	test   %eax,%eax
8010511c:	7e 2c                	jle    8010514a <show_process_info+0x29a>
    space(columns[4] - num_digits((int)p->sched_info.bjf.executed_cycle));
8010511e:	31 ff                	xor    %edi,%edi
80105120:	89 75 e0             	mov    %esi,-0x20(%ebp)
80105123:	89 fe                	mov    %edi,%esi
80105125:	89 c7                	mov    %eax,%edi
80105127:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010512e:	66 90                	xchg   %ax,%ax
    cprintf(" ");
80105130:	83 ec 0c             	sub    $0xc,%esp
  for(int i = 0; i < count; ++i)
80105133:	83 c6 01             	add    $0x1,%esi
    cprintf(" ");
80105136:	68 9e 92 10 80       	push   $0x8010929e
8010513b:	e8 80 b7 ff ff       	call   801008c0 <cprintf>
  for(int i = 0; i < count; ++i)
80105140:	83 c4 10             	add    $0x10,%esp
80105143:	39 fe                	cmp    %edi,%esi
80105145:	7c e9                	jl     80105130 <show_process_info+0x280>
80105147:	8b 75 e0             	mov    -0x20(%ebp),%esi

    cprintf("%d", p->sched_info.bjf.arrival_time);
8010514a:	83 ec 08             	sub    $0x8,%esp
8010514d:	ff b6 90 00 00 00    	pushl  0x90(%esi)
  int num = 0;
80105153:	31 ff                	xor    %edi,%edi
    cprintf("%d", p->sched_info.bjf.arrival_time);
80105155:	68 1b 92 10 80       	push   $0x8010921b
8010515a:	e8 61 b7 ff ff       	call   801008c0 <cprintf>
    space(columns[5] - num_digits(p->sched_info.bjf.arrival_time));
8010515f:	8b 8e 90 00 00 00    	mov    0x90(%esi),%ecx
  while(n!= 0) {
80105165:	83 c4 10             	add    $0x10,%esp
    space(columns[5] - num_digits(p->sched_info.bjf.arrival_time));
80105168:	b8 08 00 00 00       	mov    $0x8,%eax
  while(n!= 0) {
8010516d:	85 c9                	test   %ecx,%ecx
8010516f:	74 25                	je     80105196 <show_process_info+0x2e6>
80105171:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    n/=10;
80105178:	89 c8                	mov    %ecx,%eax
8010517a:	c1 f9 1f             	sar    $0x1f,%ecx
    num += 1;
8010517d:	83 c7 01             	add    $0x1,%edi
    n/=10;
80105180:	f7 eb                	imul   %ebx
80105182:	c1 fa 02             	sar    $0x2,%edx
  while(n!= 0) {
80105185:	29 ca                	sub    %ecx,%edx
80105187:	89 d1                	mov    %edx,%ecx
80105189:	75 ed                	jne    80105178 <show_process_info+0x2c8>
    space(columns[5] - num_digits(p->sched_info.bjf.arrival_time));
8010518b:	b8 08 00 00 00       	mov    $0x8,%eax
80105190:	29 f8                	sub    %edi,%eax
  for(int i = 0; i < count; ++i)
80105192:	85 c0                	test   %eax,%eax
80105194:	7e 24                	jle    801051ba <show_process_info+0x30a>
    space(columns[5] - num_digits(p->sched_info.bjf.arrival_time));
80105196:	31 ff                	xor    %edi,%edi
80105198:	89 75 e0             	mov    %esi,-0x20(%ebp)
8010519b:	89 fe                	mov    %edi,%esi
8010519d:	89 c7                	mov    %eax,%edi
8010519f:	90                   	nop
    cprintf(" ");
801051a0:	83 ec 0c             	sub    $0xc,%esp
  for(int i = 0; i < count; ++i)
801051a3:	83 c6 01             	add    $0x1,%esi
    cprintf(" ");
801051a6:	68 9e 92 10 80       	push   $0x8010929e
801051ab:	e8 10 b7 ff ff       	call   801008c0 <cprintf>
  for(int i = 0; i < count; ++i)
801051b0:	83 c4 10             	add    $0x10,%esp
801051b3:	39 fe                	cmp    %edi,%esi
801051b5:	7c e9                	jl     801051a0 <show_process_info+0x2f0>
801051b7:	8b 75 e0             	mov    -0x20(%ebp),%esi

    cprintf("%d", p->sched_info.bjf.priority);
801051ba:	83 ec 08             	sub    $0x8,%esp
801051bd:	ff b6 88 00 00 00    	pushl  0x88(%esi)
  int num = 0;
801051c3:	31 ff                	xor    %edi,%edi
    cprintf("%d", p->sched_info.bjf.priority);
801051c5:	68 1b 92 10 80       	push   $0x8010921b
801051ca:	e8 f1 b6 ff ff       	call   801008c0 <cprintf>
    space(columns[6] - num_digits(p->sched_info.bjf.priority));
801051cf:	8b 8e 88 00 00 00    	mov    0x88(%esi),%ecx
  while(n!= 0) {
801051d5:	83 c4 10             	add    $0x10,%esp
    space(columns[6] - num_digits(p->sched_info.bjf.priority));
801051d8:	b8 09 00 00 00       	mov    $0x9,%eax
  while(n!= 0) {
801051dd:	85 c9                	test   %ecx,%ecx
801051df:	74 25                	je     80105206 <show_process_info+0x356>
801051e1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    n/=10;
801051e8:	89 c8                	mov    %ecx,%eax
801051ea:	c1 f9 1f             	sar    $0x1f,%ecx
    num += 1;
801051ed:	83 c7 01             	add    $0x1,%edi
    n/=10;
801051f0:	f7 eb                	imul   %ebx
801051f2:	c1 fa 02             	sar    $0x2,%edx
  while(n!= 0) {
801051f5:	29 ca                	sub    %ecx,%edx
801051f7:	89 d1                	mov    %edx,%ecx
801051f9:	75 ed                	jne    801051e8 <show_process_info+0x338>
    space(columns[6] - num_digits(p->sched_info.bjf.priority));
801051fb:	b8 09 00 00 00       	mov    $0x9,%eax
80105200:	29 f8                	sub    %edi,%eax
  for(int i = 0; i < count; ++i)
80105202:	85 c0                	test   %eax,%eax
80105204:	7e 24                	jle    8010522a <show_process_info+0x37a>
    space(columns[6] - num_digits(p->sched_info.bjf.priority));
80105206:	31 ff                	xor    %edi,%edi
80105208:	89 75 e0             	mov    %esi,-0x20(%ebp)
8010520b:	89 fe                	mov    %edi,%esi
8010520d:	89 c7                	mov    %eax,%edi
8010520f:	90                   	nop
    cprintf(" ");
80105210:	83 ec 0c             	sub    $0xc,%esp
  for(int i = 0; i < count; ++i)
80105213:	83 c6 01             	add    $0x1,%esi
    cprintf(" ");
80105216:	68 9e 92 10 80       	push   $0x8010929e
8010521b:	e8 a0 b6 ff ff       	call   801008c0 <cprintf>
  for(int i = 0; i < count; ++i)
80105220:	83 c4 10             	add    $0x10,%esp
80105223:	39 fe                	cmp    %edi,%esi
80105225:	7c e9                	jl     80105210 <show_process_info+0x360>
80105227:	8b 75 e0             	mov    -0x20(%ebp),%esi

    cprintf("%d", (int)p->sched_info.bjf.priority_ratio);
8010522a:	d9 86 8c 00 00 00    	flds   0x8c(%esi)
80105230:	83 ec 08             	sub    $0x8,%esp
  int num = 0;
80105233:	31 ff                	xor    %edi,%edi
    cprintf("%d", (int)p->sched_info.bjf.priority_ratio);
80105235:	d9 7d e6             	fnstcw -0x1a(%ebp)
80105238:	0f b7 45 e6          	movzwl -0x1a(%ebp),%eax
8010523c:	80 cc 0c             	or     $0xc,%ah
8010523f:	66 89 45 e4          	mov    %ax,-0x1c(%ebp)
80105243:	d9 6d e4             	fldcw  -0x1c(%ebp)
80105246:	db 5d e0             	fistpl -0x20(%ebp)
80105249:	d9 6d e6             	fldcw  -0x1a(%ebp)
8010524c:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010524f:	50                   	push   %eax
80105250:	68 1b 92 10 80       	push   $0x8010921b
80105255:	e8 66 b6 ff ff       	call   801008c0 <cprintf>
    space(columns[7] - num_digits((int)p->sched_info.bjf.priority_ratio));
8010525a:	d9 86 8c 00 00 00    	flds   0x8c(%esi)
  while(n!= 0) {
80105260:	83 c4 10             	add    $0x10,%esp
    space(columns[7] - num_digits((int)p->sched_info.bjf.priority_ratio));
80105263:	d9 7d e6             	fnstcw -0x1a(%ebp)
80105266:	0f b7 45 e6          	movzwl -0x1a(%ebp),%eax
8010526a:	80 cc 0c             	or     $0xc,%ah
8010526d:	66 89 45 e4          	mov    %ax,-0x1c(%ebp)
80105271:	b8 08 00 00 00       	mov    $0x8,%eax
80105276:	d9 6d e4             	fldcw  -0x1c(%ebp)
80105279:	db 5d e0             	fistpl -0x20(%ebp)
8010527c:	d9 6d e6             	fldcw  -0x1a(%ebp)
8010527f:	8b 4d e0             	mov    -0x20(%ebp),%ecx
  while(n!= 0) {
80105282:	85 c9                	test   %ecx,%ecx
80105284:	74 28                	je     801052ae <show_process_info+0x3fe>
80105286:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010528d:	8d 76 00             	lea    0x0(%esi),%esi
    n/=10;
80105290:	89 c8                	mov    %ecx,%eax
80105292:	c1 f9 1f             	sar    $0x1f,%ecx
    num += 1;
80105295:	83 c7 01             	add    $0x1,%edi
    n/=10;
80105298:	f7 eb                	imul   %ebx
8010529a:	c1 fa 02             	sar    $0x2,%edx
  while(n!= 0) {
8010529d:	29 ca                	sub    %ecx,%edx
8010529f:	89 d1                	mov    %edx,%ecx
801052a1:	75 ed                	jne    80105290 <show_process_info+0x3e0>
    space(columns[7] - num_digits((int)p->sched_info.bjf.priority_ratio));
801052a3:	b8 08 00 00 00       	mov    $0x8,%eax
801052a8:	29 f8                	sub    %edi,%eax
  for(int i = 0; i < count; ++i)
801052aa:	85 c0                	test   %eax,%eax
801052ac:	7e 3a                	jle    801052e8 <show_process_info+0x438>
    space(columns[7] - num_digits((int)p->sched_info.bjf.priority_ratio));
801052ae:	31 ff                	xor    %edi,%edi
801052b0:	89 75 e0             	mov    %esi,-0x20(%ebp)
801052b3:	89 fe                	mov    %edi,%esi
801052b5:	89 c7                	mov    %eax,%edi
801052b7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801052be:	66 90                	xchg   %ax,%ax
    cprintf(" ");
801052c0:	83 ec 0c             	sub    $0xc,%esp
  for(int i = 0; i < count; ++i)
801052c3:	83 c6 01             	add    $0x1,%esi
    cprintf(" ");
801052c6:	68 9e 92 10 80       	push   $0x8010929e
801052cb:	e8 f0 b5 ff ff       	call   801008c0 <cprintf>
  for(int i = 0; i < count; ++i)
801052d0:	83 c4 10             	add    $0x10,%esp
801052d3:	39 fe                	cmp    %edi,%esi
801052d5:	7c e9                	jl     801052c0 <show_process_info+0x410>
801052d7:	d9 7d e6             	fnstcw -0x1a(%ebp)
801052da:	8b 75 e0             	mov    -0x20(%ebp),%esi
801052dd:	0f b7 45 e6          	movzwl -0x1a(%ebp),%eax
801052e1:	80 cc 0c             	or     $0xc,%ah
801052e4:	66 89 45 e4          	mov    %ax,-0x1c(%ebp)

    cprintf("%d", (int)p->sched_info.bjf.arrival_time_ratio);
801052e8:	d9 86 94 00 00 00    	flds   0x94(%esi)
801052ee:	83 ec 08             	sub    $0x8,%esp
  int num = 0;
801052f1:	31 ff                	xor    %edi,%edi
    cprintf("%d", (int)p->sched_info.bjf.arrival_time_ratio);
801052f3:	d9 6d e4             	fldcw  -0x1c(%ebp)
801052f6:	db 5d e0             	fistpl -0x20(%ebp)
801052f9:	d9 6d e6             	fldcw  -0x1a(%ebp)
801052fc:	8b 45 e0             	mov    -0x20(%ebp),%eax
801052ff:	50                   	push   %eax
80105300:	68 1b 92 10 80       	push   $0x8010921b
80105305:	e8 b6 b5 ff ff       	call   801008c0 <cprintf>
    space(columns[8] - num_digits((int)p->sched_info.bjf.arrival_time_ratio));
8010530a:	d9 86 94 00 00 00    	flds   0x94(%esi)
  while(n!= 0) {
80105310:	83 c4 10             	add    $0x10,%esp
    space(columns[8] - num_digits((int)p->sched_info.bjf.arrival_time_ratio));
80105313:	d9 7d e6             	fnstcw -0x1a(%ebp)
80105316:	0f b7 45 e6          	movzwl -0x1a(%ebp),%eax
8010531a:	80 cc 0c             	or     $0xc,%ah
8010531d:	66 89 45 e4          	mov    %ax,-0x1c(%ebp)
80105321:	b8 08 00 00 00       	mov    $0x8,%eax
80105326:	d9 6d e4             	fldcw  -0x1c(%ebp)
80105329:	db 5d e0             	fistpl -0x20(%ebp)
8010532c:	d9 6d e6             	fldcw  -0x1a(%ebp)
8010532f:	8b 4d e0             	mov    -0x20(%ebp),%ecx
  while(n!= 0) {
80105332:	85 c9                	test   %ecx,%ecx
80105334:	74 28                	je     8010535e <show_process_info+0x4ae>
80105336:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010533d:	8d 76 00             	lea    0x0(%esi),%esi
    n/=10;
80105340:	89 c8                	mov    %ecx,%eax
80105342:	c1 f9 1f             	sar    $0x1f,%ecx
    num += 1;
80105345:	83 c7 01             	add    $0x1,%edi
    n/=10;
80105348:	f7 eb                	imul   %ebx
8010534a:	c1 fa 02             	sar    $0x2,%edx
  while(n!= 0) {
8010534d:	29 ca                	sub    %ecx,%edx
8010534f:	89 d1                	mov    %edx,%ecx
80105351:	75 ed                	jne    80105340 <show_process_info+0x490>
    space(columns[8] - num_digits((int)p->sched_info.bjf.arrival_time_ratio));
80105353:	b8 08 00 00 00       	mov    $0x8,%eax
80105358:	29 f8                	sub    %edi,%eax
  for(int i = 0; i < count; ++i)
8010535a:	85 c0                	test   %eax,%eax
8010535c:	7e 3a                	jle    80105398 <show_process_info+0x4e8>
    space(columns[8] - num_digits((int)p->sched_info.bjf.arrival_time_ratio));
8010535e:	31 ff                	xor    %edi,%edi
80105360:	89 75 e0             	mov    %esi,-0x20(%ebp)
80105363:	89 fe                	mov    %edi,%esi
80105365:	89 c7                	mov    %eax,%edi
80105367:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010536e:	66 90                	xchg   %ax,%ax
    cprintf(" ");
80105370:	83 ec 0c             	sub    $0xc,%esp
  for(int i = 0; i < count; ++i)
80105373:	83 c6 01             	add    $0x1,%esi
    cprintf(" ");
80105376:	68 9e 92 10 80       	push   $0x8010929e
8010537b:	e8 40 b5 ff ff       	call   801008c0 <cprintf>
  for(int i = 0; i < count; ++i)
80105380:	83 c4 10             	add    $0x10,%esp
80105383:	39 fe                	cmp    %edi,%esi
80105385:	7c e9                	jl     80105370 <show_process_info+0x4c0>
80105387:	d9 7d e6             	fnstcw -0x1a(%ebp)
8010538a:	8b 75 e0             	mov    -0x20(%ebp),%esi
8010538d:	0f b7 45 e6          	movzwl -0x1a(%ebp),%eax
80105391:	80 cc 0c             	or     $0xc,%ah
80105394:	66 89 45 e4          	mov    %ax,-0x1c(%ebp)

    cprintf("%d", (int)p->sched_info.bjf.executed_cycle_ratio);
80105398:	d9 86 9c 00 00 00    	flds   0x9c(%esi)
8010539e:	83 ec 08             	sub    $0x8,%esp
  int num = 0;
801053a1:	31 ff                	xor    %edi,%edi
    cprintf("%d", (int)p->sched_info.bjf.executed_cycle_ratio);
801053a3:	d9 6d e4             	fldcw  -0x1c(%ebp)
801053a6:	db 5d e0             	fistpl -0x20(%ebp)
801053a9:	d9 6d e6             	fldcw  -0x1a(%ebp)
801053ac:	8b 45 e0             	mov    -0x20(%ebp),%eax
801053af:	50                   	push   %eax
801053b0:	68 1b 92 10 80       	push   $0x8010921b
801053b5:	e8 06 b5 ff ff       	call   801008c0 <cprintf>
    space(columns[9] - num_digits((int)p->sched_info.bjf.executed_cycle_ratio));
801053ba:	d9 86 9c 00 00 00    	flds   0x9c(%esi)
  while(n!= 0) {
801053c0:	83 c4 10             	add    $0x10,%esp
    space(columns[9] - num_digits((int)p->sched_info.bjf.executed_cycle_ratio));
801053c3:	d9 7d e6             	fnstcw -0x1a(%ebp)
801053c6:	0f b7 45 e6          	movzwl -0x1a(%ebp),%eax
801053ca:	80 cc 0c             	or     $0xc,%ah
801053cd:	66 89 45 e4          	mov    %ax,-0x1c(%ebp)
801053d1:	b8 08 00 00 00       	mov    $0x8,%eax
801053d6:	d9 6d e4             	fldcw  -0x1c(%ebp)
801053d9:	db 5d e0             	fistpl -0x20(%ebp)
801053dc:	d9 6d e6             	fldcw  -0x1a(%ebp)
801053df:	8b 4d e0             	mov    -0x20(%ebp),%ecx
  while(n!= 0) {
801053e2:	85 c9                	test   %ecx,%ecx
801053e4:	74 28                	je     8010540e <show_process_info+0x55e>
801053e6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801053ed:	8d 76 00             	lea    0x0(%esi),%esi
    n/=10;
801053f0:	89 c8                	mov    %ecx,%eax
801053f2:	c1 f9 1f             	sar    $0x1f,%ecx
    num += 1;
801053f5:	83 c7 01             	add    $0x1,%edi
    n/=10;
801053f8:	f7 eb                	imul   %ebx
801053fa:	c1 fa 02             	sar    $0x2,%edx
  while(n!= 0) {
801053fd:	29 ca                	sub    %ecx,%edx
801053ff:	89 d1                	mov    %edx,%ecx
80105401:	75 ed                	jne    801053f0 <show_process_info+0x540>
    space(columns[9] - num_digits((int)p->sched_info.bjf.executed_cycle_ratio));
80105403:	b8 08 00 00 00       	mov    $0x8,%eax
80105408:	29 f8                	sub    %edi,%eax
  for(int i = 0; i < count; ++i)
8010540a:	85 c0                	test   %eax,%eax
8010540c:	7e 3a                	jle    80105448 <show_process_info+0x598>
    space(columns[9] - num_digits((int)p->sched_info.bjf.executed_cycle_ratio));
8010540e:	31 ff                	xor    %edi,%edi
80105410:	89 75 e0             	mov    %esi,-0x20(%ebp)
80105413:	89 fe                	mov    %edi,%esi
80105415:	89 c7                	mov    %eax,%edi
80105417:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010541e:	66 90                	xchg   %ax,%ax
    cprintf(" ");
80105420:	83 ec 0c             	sub    $0xc,%esp
  for(int i = 0; i < count; ++i)
80105423:	83 c6 01             	add    $0x1,%esi
    cprintf(" ");
80105426:	68 9e 92 10 80       	push   $0x8010929e
8010542b:	e8 90 b4 ff ff       	call   801008c0 <cprintf>
  for(int i = 0; i < count; ++i)
80105430:	83 c4 10             	add    $0x10,%esp
80105433:	39 fe                	cmp    %edi,%esi
80105435:	7c e9                	jl     80105420 <show_process_info+0x570>
80105437:	d9 7d e6             	fnstcw -0x1a(%ebp)
8010543a:	8b 75 e0             	mov    -0x20(%ebp),%esi
8010543d:	0f b7 45 e6          	movzwl -0x1a(%ebp),%eax
80105441:	80 cc 0c             	or     $0xc,%ah
80105444:	66 89 45 e4          	mov    %ax,-0x1c(%ebp)

    cprintf("%d", (int)p->sched_info.bjf.process_size_ratio);
80105448:	d9 86 a4 00 00 00    	flds   0xa4(%esi)
8010544e:	83 ec 08             	sub    $0x8,%esp
  int num = 0;
80105451:	31 ff                	xor    %edi,%edi
    cprintf("%d", (int)p->sched_info.bjf.process_size_ratio);
80105453:	d9 6d e4             	fldcw  -0x1c(%ebp)
80105456:	db 5d e0             	fistpl -0x20(%ebp)
80105459:	d9 6d e6             	fldcw  -0x1a(%ebp)
8010545c:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010545f:	50                   	push   %eax
80105460:	68 1b 92 10 80       	push   $0x8010921b
80105465:	e8 56 b4 ff ff       	call   801008c0 <cprintf>
    space(columns[10] - num_digits((int)p->sched_info.bjf.process_size_ratio));
8010546a:	d9 86 a4 00 00 00    	flds   0xa4(%esi)
  while(n!= 0) {
80105470:	83 c4 10             	add    $0x10,%esp
    space(columns[10] - num_digits((int)p->sched_info.bjf.process_size_ratio));
80105473:	d9 7d e6             	fnstcw -0x1a(%ebp)
80105476:	0f b7 45 e6          	movzwl -0x1a(%ebp),%eax
8010547a:	80 cc 0c             	or     $0xc,%ah
8010547d:	66 89 45 e4          	mov    %ax,-0x1c(%ebp)
80105481:	b8 08 00 00 00       	mov    $0x8,%eax
80105486:	d9 6d e4             	fldcw  -0x1c(%ebp)
80105489:	db 55 e0             	fistl  -0x20(%ebp)
8010548c:	d9 6d e6             	fldcw  -0x1a(%ebp)
8010548f:	8b 4d e0             	mov    -0x20(%ebp),%ecx
  while(n!= 0) {
80105492:	85 c9                	test   %ecx,%ecx
80105494:	74 32                	je     801054c8 <show_process_info+0x618>
80105496:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010549d:	8d 76 00             	lea    0x0(%esi),%esi
    n/=10;
801054a0:	89 c8                	mov    %ecx,%eax
801054a2:	c1 f9 1f             	sar    $0x1f,%ecx
    num += 1;
801054a5:	83 c7 01             	add    $0x1,%edi
    n/=10;
801054a8:	f7 eb                	imul   %ebx
801054aa:	c1 fa 02             	sar    $0x2,%edx
  while(n!= 0) {
801054ad:	29 ca                	sub    %ecx,%edx
801054af:	89 d1                	mov    %edx,%ecx
801054b1:	75 ed                	jne    801054a0 <show_process_info+0x5f0>
    space(columns[10] - num_digits((int)p->sched_info.bjf.process_size_ratio));
801054b3:	b8 08 00 00 00       	mov    $0x8,%eax
801054b8:	29 f8                	sub    %edi,%eax
  for(int i = 0; i < count; ++i)
801054ba:	85 c0                	test   %eax,%eax
801054bc:	7e 50                	jle    8010550e <show_process_info+0x65e>
801054be:	dd d8                	fstp   %st(0)
801054c0:	eb 0e                	jmp    801054d0 <show_process_info+0x620>
801054c2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801054c8:	dd d8                	fstp   %st(0)
801054ca:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    space(columns[10] - num_digits((int)p->sched_info.bjf.process_size_ratio));
801054d0:	31 ff                	xor    %edi,%edi
801054d2:	89 75 e0             	mov    %esi,-0x20(%ebp)
801054d5:	89 fe                	mov    %edi,%esi
801054d7:	89 c7                	mov    %eax,%edi
801054d9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    cprintf(" ");
801054e0:	83 ec 0c             	sub    $0xc,%esp
  for(int i = 0; i < count; ++i)
801054e3:	83 c6 01             	add    $0x1,%esi
    cprintf(" ");
801054e6:	68 9e 92 10 80       	push   $0x8010929e
801054eb:	e8 d0 b3 ff ff       	call   801008c0 <cprintf>
  for(int i = 0; i < count; ++i)
801054f0:	83 c4 10             	add    $0x10,%esp
801054f3:	39 fe                	cmp    %edi,%esi
801054f5:	7c e9                	jl     801054e0 <show_process_info+0x630>
801054f7:	8b 75 e0             	mov    -0x20(%ebp),%esi
801054fa:	d9 86 a4 00 00 00    	flds   0xa4(%esi)
80105500:	d9 7d e6             	fnstcw -0x1a(%ebp)
80105503:	0f b7 45 e6          	movzwl -0x1a(%ebp),%eax
80105507:	80 cc 0c             	or     $0xc,%ah
8010550a:	66 89 45 e4          	mov    %ax,-0x1c(%ebp)
         p->sched_info.bjf.arrival_time * p->sched_info.bjf.arrival_time_ratio +
8010550e:	db 86 90 00 00 00    	fildl  0x90(%esi)
80105514:	d8 8e 94 00 00 00    	fmuls  0x94(%esi)

    cprintf("%d", (int)bjfrank(p));
8010551a:	83 ec 08             	sub    $0x8,%esp
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
8010551d:	81 c6 ac 00 00 00    	add    $0xac,%esi
  return p->sched_info.bjf.priority * p->sched_info.bjf.priority_ratio +
80105523:	db 46 dc             	fildl  -0x24(%esi)
80105526:	d8 4e e0             	fmuls  -0x20(%esi)
80105529:	de c1                	faddp  %st,%st(1)
         p->sched_info.bjf.executed_cycle * p->sched_info.bjf.executed_cycle_ratio +
8010552b:	d9 46 ec             	flds   -0x14(%esi)
8010552e:	d8 4e f0             	fmuls  -0x10(%esi)
         p->sched_info.bjf.arrival_time * p->sched_info.bjf.arrival_time_ratio +
80105531:	de c1                	faddp  %st,%st(1)
         p->sched_info.bjf.process_size * p->sched_info.bjf.process_size_ratio;
80105533:	db 46 f4             	fildl  -0xc(%esi)
80105536:	de ca                	fmulp  %st,%st(2)
         p->sched_info.bjf.executed_cycle * p->sched_info.bjf.executed_cycle_ratio +
80105538:	de c1                	faddp  %st,%st(1)
    cprintf("%d", (int)bjfrank(p));
8010553a:	d9 6d e4             	fldcw  -0x1c(%ebp)
8010553d:	db 5d e0             	fistpl -0x20(%ebp)
80105540:	d9 6d e6             	fldcw  -0x1a(%ebp)
80105543:	8b 45 e0             	mov    -0x20(%ebp),%eax
80105546:	50                   	push   %eax
80105547:	68 1b 92 10 80       	push   $0x8010921b
8010554c:	e8 6f b3 ff ff       	call   801008c0 <cprintf>
    cprintf("\n");
80105551:	c7 04 24 25 92 10 80 	movl   $0x80109225,(%esp)
80105558:	e8 63 b3 ff ff       	call   801008c0 <cprintf>
8010555d:	83 c4 10             	add    $0x10,%esp
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80105560:	81 fe 54 7e 11 80    	cmp    $0x80117e54,%esi
80105566:	0f 85 86 f9 ff ff    	jne    80104ef2 <show_process_info+0x42>
  }
}
8010556c:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010556f:	5b                   	pop    %ebx
80105570:	5e                   	pop    %esi
80105571:	5f                   	pop    %edi
80105572:	5d                   	pop    %ebp
80105573:	c3                   	ret    
80105574:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010557b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010557f:	90                   	nop

80105580 <priority_wakeup>:

void priority_wakeup(void* chan){
80105580:	f3 0f 1e fb          	endbr32 
80105584:	55                   	push   %ebp
80105585:	89 e5                	mov    %esp,%ebp
80105587:	56                   	push   %esi
80105588:	53                   	push   %ebx
80105589:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&ptable.lock);
8010558c:	83 ec 0c             	sub    $0xc,%esp
8010558f:	68 20 53 11 80       	push   $0x80115320
80105594:	e8 77 06 00 00       	call   80105c10 <acquire>
80105599:	83 c4 10             	add    $0x10,%esp
  struct proc *p;
  struct proc * p_max_pid = 0 ;
  int first = 1 ; 
8010559c:	b9 01 00 00 00       	mov    $0x1,%ecx
  struct proc * p_max_pid = 0 ;
801055a1:	31 d2                	xor    %edx,%edx
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801055a3:	b8 54 53 11 80       	mov    $0x80115354,%eax
801055a8:	eb 12                	jmp    801055bc <priority_wakeup+0x3c>
801055aa:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801055b0:	05 ac 00 00 00       	add    $0xac,%eax
801055b5:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
801055ba:	74 24                	je     801055e0 <priority_wakeup+0x60>
    if(p->state == SLEEPING && p->chan == chan){
801055bc:	83 78 0c 02          	cmpl   $0x2,0xc(%eax)
801055c0:	75 ee                	jne    801055b0 <priority_wakeup+0x30>
801055c2:	39 58 20             	cmp    %ebx,0x20(%eax)
801055c5:	75 e9                	jne    801055b0 <priority_wakeup+0x30>
      if(first){
801055c7:	85 c9                	test   %ecx,%ecx
801055c9:	75 35                	jne    80105600 <priority_wakeup+0x80>
        p_max_pid = p;
        first = 0;
      }
      else{
        if(p->pid > p_max_pid->pid){
801055cb:	8b 72 10             	mov    0x10(%edx),%esi
801055ce:	39 70 10             	cmp    %esi,0x10(%eax)
801055d1:	0f 4f d0             	cmovg  %eax,%edx
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801055d4:	05 ac 00 00 00       	add    $0xac,%eax
801055d9:	3d 54 7e 11 80       	cmp    $0x80117e54,%eax
801055de:	75 dc                	jne    801055bc <priority_wakeup+0x3c>
          p_max_pid = p;
        }
      }
    }
  }
  if (p_max_pid)
801055e0:	85 d2                	test   %edx,%edx
801055e2:	74 07                	je     801055eb <priority_wakeup+0x6b>
  {
      p_max_pid->state = RUNNABLE;
801055e4:	c7 42 0c 03 00 00 00 	movl   $0x3,0xc(%edx)

    
  }
  
  release(&ptable.lock);
801055eb:	c7 45 08 20 53 11 80 	movl   $0x80115320,0x8(%ebp)
}
801055f2:	8d 65 f8             	lea    -0x8(%ebp),%esp
801055f5:	5b                   	pop    %ebx
801055f6:	5e                   	pop    %esi
801055f7:	5d                   	pop    %ebp
  release(&ptable.lock);
801055f8:	e9 d3 06 00 00       	jmp    80105cd0 <release>
801055fd:	8d 76 00             	lea    0x0(%esi),%esi
80105600:	89 c2                	mov    %eax,%edx
        first = 0;
80105602:	31 c9                	xor    %ecx,%ecx
80105604:	eb aa                	jmp    801055b0 <priority_wakeup+0x30>
80105606:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010560d:	8d 76 00             	lea    0x0(%esi),%esi

80105610 <compare_pid>:

int compare_pid(const void *a, const void *b) {
80105610:	f3 0f 1e fb          	endbr32 
80105614:	55                   	push   %ebp
80105615:	89 e5                	mov    %esp,%ebp
  struct proc *procA = (struct proc *)a;
  struct proc *procB = (struct proc *)b;
  return procA->pid - procB->pid;
80105617:	8b 45 08             	mov    0x8(%ebp),%eax
8010561a:	8b 55 0c             	mov    0xc(%ebp),%edx
}
8010561d:	5d                   	pop    %ebp
  return procA->pid - procB->pid;
8010561e:	8b 40 10             	mov    0x10(%eax),%eax
80105621:	2b 42 10             	sub    0x10(%edx),%eax
}
80105624:	c3                   	ret    
80105625:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010562c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80105630 <make_priority_queue>:


void make_priority_queue(void * chan) {
80105630:	f3 0f 1e fb          	endbr32 
80105634:	55                   	push   %ebp
80105635:	89 e5                	mov    %esp,%ebp
80105637:	57                   	push   %edi
    // Handle the allocation failure 
    release(&ptable.lock);
    return;
  }*/

  int first = 1;
80105638:	bf 01 00 00 00       	mov    $0x1,%edi
void make_priority_queue(void * chan) {
8010563d:	56                   	push   %esi
8010563e:	53                   	push   %ebx
  int i = 0;
  cprintf("Queue: \n" );
  // Populate the queue with processes that match the channel
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
8010563f:	bb 54 53 11 80       	mov    $0x80115354,%ebx
void make_priority_queue(void * chan) {
80105644:	83 ec 18             	sub    $0x18,%esp
80105647:	8b 75 08             	mov    0x8(%ebp),%esi
  acquire(&ptable.lock); // Lock the ptable before making changes
8010564a:	68 20 53 11 80       	push   $0x80115320
8010564f:	e8 bc 05 00 00       	call   80105c10 <acquire>
  cprintf("Queue: \n" );
80105654:	c7 04 24 1e 92 10 80 	movl   $0x8010921e,(%esp)
8010565b:	e8 60 b2 ff ff       	call   801008c0 <cprintf>
80105660:	83 c4 10             	add    $0x10,%esp
  struct proc *proc_queue = 0;
80105663:	31 d2                	xor    %edx,%edx
80105665:	eb 17                	jmp    8010567e <make_priority_queue+0x4e>
80105667:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010566e:	66 90                	xchg   %ax,%ax
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
80105670:	81 c3 ac 00 00 00    	add    $0xac,%ebx
80105676:	81 fb 54 7e 11 80    	cmp    $0x80117e54,%ebx
8010567c:	74 42                	je     801056c0 <make_priority_queue+0x90>
    if (p->state == SLEEPING && p->chan == chan) {
8010567e:	83 7b 0c 02          	cmpl   $0x2,0xc(%ebx)
80105682:	75 ec                	jne    80105670 <make_priority_queue+0x40>
80105684:	39 73 20             	cmp    %esi,0x20(%ebx)
80105687:	75 e7                	jne    80105670 <make_priority_queue+0x40>
      if(first){
80105689:	85 ff                	test   %edi,%edi
8010568b:	75 63                	jne    801056f0 <make_priority_queue+0xc0>
8010568d:	8b 43 10             	mov    0x10(%ebx),%eax
        proc_queue = p ;
        first = 0;
        i ++;
        cprintf("Pid %d , Name: %s \n", p->pid , p->name );
      }
      else if(p->pid > proc_queue->pid){
80105690:	39 42 10             	cmp    %eax,0x10(%edx)
80105693:	7d db                	jge    80105670 <make_priority_queue+0x40>
        proc_queue = p ;
        i ++ ;
        cprintf("Pid %d , Name: %s \n", p->pid , p->name );
80105695:	83 ec 04             	sub    $0x4,%esp
80105698:	8d 53 6c             	lea    0x6c(%ebx),%edx
8010569b:	52                   	push   %edx
8010569c:	50                   	push   %eax
8010569d:	68 27 92 10 80       	push   $0x80109227
801056a2:	e8 19 b2 ff ff       	call   801008c0 <cprintf>
801056a7:	89 da                	mov    %ebx,%edx
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
801056a9:	81 c3 ac 00 00 00    	add    $0xac,%ebx
        cprintf("Pid %d , Name: %s \n", p->pid , p->name );
801056af:	83 c4 10             	add    $0x10,%esp
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
801056b2:	81 fb 54 7e 11 80    	cmp    $0x80117e54,%ebx
801056b8:	75 c4                	jne    8010567e <make_priority_queue+0x4e>
801056ba:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
      }
    }
  }
  if(first){
801056c0:	85 ff                	test   %edi,%edi
801056c2:	74 4c                	je     80105710 <make_priority_queue+0xe0>
    cprintf("Queue is empty\n");
801056c4:	83 ec 0c             	sub    $0xc,%esp
801056c7:	68 3b 92 10 80       	push   $0x8010923b
801056cc:	e8 ef b1 ff ff       	call   801008c0 <cprintf>
801056d1:	83 c4 10             	add    $0x10,%esp
  // Sort the queue by pid using the comparison function
  //qsort(proc_queue, i, sizeof(struct proc), compare_pid);
  


  release(&ptable.lock); // Release the lock after modifications are done
801056d4:	c7 45 08 20 53 11 80 	movl   $0x80115320,0x8(%ebp)
}
801056db:	8d 65 f4             	lea    -0xc(%ebp),%esp
801056de:	5b                   	pop    %ebx
801056df:	5e                   	pop    %esi
801056e0:	5f                   	pop    %edi
801056e1:	5d                   	pop    %ebp
  release(&ptable.lock); // Release the lock after modifications are done
801056e2:	e9 e9 05 00 00       	jmp    80105cd0 <release>
801056e7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801056ee:	66 90                	xchg   %ax,%ax
        cprintf("Pid %d , Name: %s \n", p->pid , p->name );
801056f0:	83 ec 04             	sub    $0x4,%esp
801056f3:	8d 43 6c             	lea    0x6c(%ebx),%eax
        first = 0;
801056f6:	31 ff                	xor    %edi,%edi
        cprintf("Pid %d , Name: %s \n", p->pid , p->name );
801056f8:	50                   	push   %eax
801056f9:	ff 73 10             	pushl  0x10(%ebx)
801056fc:	68 27 92 10 80       	push   $0x80109227
80105701:	e8 ba b1 ff ff       	call   801008c0 <cprintf>
80105706:	83 c4 10             	add    $0x10,%esp
80105709:	89 da                	mov    %ebx,%edx
8010570b:	e9 60 ff ff ff       	jmp    80105670 <make_priority_queue+0x40>
    cprintf("Now it is pid %d's turn\n");
80105710:	83 ec 0c             	sub    $0xc,%esp
80105713:	68 4b 92 10 80       	push   $0x8010924b
80105718:	e8 a3 b1 ff ff       	call   801008c0 <cprintf>
8010571d:	83 c4 10             	add    $0x10,%esp
80105720:	eb b2                	jmp    801056d4 <make_priority_queue+0xa4>
80105722:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105729:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80105730 <priorityLock_test>:


void priorityLock_test(){
80105730:	f3 0f 1e fb          	endbr32 
80105734:	55                   	push   %ebp
80105735:	89 e5                	mov    %esp,%ebp
80105737:	53                   	push   %ebx
80105738:	83 ec 14             	sub    $0x14,%esp
  pushcli();
8010573b:	e8 d0 03 00 00       	call   80105b10 <pushcli>
  c = mycpu();
80105740:	e8 eb e6 ff ff       	call   80103e30 <mycpu>
  p = c->proc;
80105745:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
8010574b:	e8 10 04 00 00       	call   80105b60 <popcli>
  cprintf("Process pid:%d want access to critical section\n" , myproc()->pid);
80105750:	83 ec 08             	sub    $0x8,%esp
80105753:	ff 73 10             	pushl  0x10(%ebx)
80105756:	68 98 93 10 80       	push   $0x80109398
8010575b:	e8 60 b1 ff ff       	call   801008c0 <cprintf>
  acquirePriorityLock(&lock);
80105760:	c7 04 24 c0 c5 10 80 	movl   $0x8010c5c0,(%esp)
80105767:	e8 04 02 00 00       	call   80105970 <acquirePriorityLock>
  pushcli();
8010576c:	e8 9f 03 00 00       	call   80105b10 <pushcli>
  c = mycpu();
80105771:	e8 ba e6 ff ff       	call   80103e30 <mycpu>
  p = c->proc;
80105776:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
8010577c:	e8 df 03 00 00       	call   80105b60 <popcli>
  cprintf("Process pid:%d acquired access to critical section\n" , myproc()->pid);
80105781:	59                   	pop    %ecx
80105782:	58                   	pop    %eax
80105783:	ff 73 10             	pushl  0x10(%ebx)
80105786:	68 c8 93 10 80       	push   $0x801093c8
8010578b:	e8 30 b1 ff ff       	call   801008c0 <cprintf>
      }
    }
    i ++ ;
    i -- ;
  }*/
  volatile long long temp = 0;
80105790:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80105797:	83 c4 10             	add    $0x10,%esp
8010579a:	31 c9                	xor    %ecx,%ecx
8010579c:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  for (long long l = 0; l < 200000000; l++){
801057a3:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801057a7:	90                   	nop
    temp += 5 * 7;
801057a8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801057ab:	8b 55 f4             	mov    -0xc(%ebp),%edx
801057ae:	83 c0 23             	add    $0x23,%eax
801057b1:	83 d2 00             	adc    $0x0,%edx
801057b4:	83 c1 01             	add    $0x1,%ecx
801057b7:	89 45 f0             	mov    %eax,-0x10(%ebp)
801057ba:	89 55 f4             	mov    %edx,-0xc(%ebp)
  for (long long l = 0; l < 200000000; l++){
801057bd:	81 f9 00 c2 eb 0b    	cmp    $0xbebc200,%ecx
801057c3:	75 e3                	jne    801057a8 <priorityLock_test+0x78>
  }
            
  make_priority_queue(&lock);
801057c5:	83 ec 0c             	sub    $0xc,%esp
801057c8:	68 c0 c5 10 80       	push   $0x8010c5c0
801057cd:	e8 5e fe ff ff       	call   80105630 <make_priority_queue>
  releasePriorityLock(&lock);
801057d2:	c7 04 24 c0 c5 10 80 	movl   $0x8010c5c0,(%esp)
801057d9:	e8 f2 01 00 00       	call   801059d0 <releasePriorityLock>
  pushcli();
801057de:	e8 2d 03 00 00       	call   80105b10 <pushcli>
  c = mycpu();
801057e3:	e8 48 e6 ff ff       	call   80103e30 <mycpu>
  p = c->proc;
801057e8:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
801057ee:	e8 6d 03 00 00       	call   80105b60 <popcli>
  cprintf("Process pid: %d exited from critical section\n" , myproc()->pid);
801057f3:	58                   	pop    %eax
801057f4:	5a                   	pop    %edx
801057f5:	ff 73 10             	pushl  0x10(%ebx)
801057f8:	68 fc 93 10 80       	push   $0x801093fc
801057fd:	e8 be b0 ff ff       	call   801008c0 <cprintf>
}
80105802:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80105805:	83 c4 10             	add    $0x10,%esp
80105808:	c9                   	leave  
80105809:	c3                   	ret    
8010580a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80105810 <init_queue_test>:

void
init_queue_test(void){
80105810:	f3 0f 1e fb          	endbr32 
80105814:	55                   	push   %ebp
80105815:	89 e5                	mov    %esp,%ebp
80105817:	83 ec 14             	sub    $0x14,%esp
  initPriorityLock(&lock);
8010581a:	68 c0 c5 10 80       	push   $0x8010c5c0
8010581f:	e8 ec 01 00 00       	call   80105a10 <initPriorityLock>
80105824:	83 c4 10             	add    $0x10,%esp
80105827:	c9                   	leave  
80105828:	c3                   	ret    
80105829:	66 90                	xchg   %ax,%ax
8010582b:	66 90                	xchg   %ax,%ax
8010582d:	66 90                	xchg   %ax,%ax
8010582f:	90                   	nop

80105830 <initsleeplock>:
#include "spinlock.h"
#include "sleeplock.h"

void
initsleeplock(struct sleeplock *lk, char *name)
{
80105830:	f3 0f 1e fb          	endbr32 
80105834:	55                   	push   %ebp
80105835:	89 e5                	mov    %esp,%ebp
80105837:	53                   	push   %ebx
80105838:	83 ec 0c             	sub    $0xc,%esp
8010583b:	8b 5d 08             	mov    0x8(%ebp),%ebx
  initlock(&lk->lk, "sleep lock");
8010583e:	68 64 94 10 80       	push   $0x80109464
80105843:	8d 43 04             	lea    0x4(%ebx),%eax
80105846:	50                   	push   %eax
80105847:	e8 44 02 00 00       	call   80105a90 <initlock>
  lk->name = name;
8010584c:	8b 45 0c             	mov    0xc(%ebp),%eax
  lk->locked = 0;
8010584f:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
  lk->pid = 0;
}
80105855:	83 c4 10             	add    $0x10,%esp
  lk->pid = 0;
80105858:	c7 43 3c 00 00 00 00 	movl   $0x0,0x3c(%ebx)
  lk->name = name;
8010585f:	89 43 38             	mov    %eax,0x38(%ebx)
}
80105862:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80105865:	c9                   	leave  
80105866:	c3                   	ret    
80105867:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010586e:	66 90                	xchg   %ax,%ax

80105870 <acquiresleep>:

void
acquiresleep(struct sleeplock *lk)
{
80105870:	f3 0f 1e fb          	endbr32 
80105874:	55                   	push   %ebp
80105875:	89 e5                	mov    %esp,%ebp
80105877:	56                   	push   %esi
80105878:	53                   	push   %ebx
80105879:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&lk->lk);
8010587c:	8d 73 04             	lea    0x4(%ebx),%esi
8010587f:	83 ec 0c             	sub    $0xc,%esp
80105882:	56                   	push   %esi
80105883:	e8 88 03 00 00       	call   80105c10 <acquire>
  while (lk->locked) {
80105888:	8b 13                	mov    (%ebx),%edx
8010588a:	83 c4 10             	add    $0x10,%esp
8010588d:	85 d2                	test   %edx,%edx
8010588f:	74 1a                	je     801058ab <acquiresleep+0x3b>
80105891:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    sleep(lk, &lk->lk);
80105898:	83 ec 08             	sub    $0x8,%esp
8010589b:	56                   	push   %esi
8010589c:	53                   	push   %ebx
8010589d:	e8 fe eb ff ff       	call   801044a0 <sleep>
  while (lk->locked) {
801058a2:	8b 03                	mov    (%ebx),%eax
801058a4:	83 c4 10             	add    $0x10,%esp
801058a7:	85 c0                	test   %eax,%eax
801058a9:	75 ed                	jne    80105898 <acquiresleep+0x28>
  }
  lk->locked = 1;
801058ab:	c7 03 01 00 00 00    	movl   $0x1,(%ebx)
  lk->pid = myproc()->pid;
801058b1:	e8 0a e6 ff ff       	call   80103ec0 <myproc>
801058b6:	8b 40 10             	mov    0x10(%eax),%eax
801058b9:	89 43 3c             	mov    %eax,0x3c(%ebx)
  release(&lk->lk);
801058bc:	89 75 08             	mov    %esi,0x8(%ebp)
}
801058bf:	8d 65 f8             	lea    -0x8(%ebp),%esp
801058c2:	5b                   	pop    %ebx
801058c3:	5e                   	pop    %esi
801058c4:	5d                   	pop    %ebp
  release(&lk->lk);
801058c5:	e9 06 04 00 00       	jmp    80105cd0 <release>
801058ca:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801058d0 <releasesleep>:

void
releasesleep(struct sleeplock *lk)
{
801058d0:	f3 0f 1e fb          	endbr32 
801058d4:	55                   	push   %ebp
801058d5:	89 e5                	mov    %esp,%ebp
801058d7:	56                   	push   %esi
801058d8:	53                   	push   %ebx
801058d9:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&lk->lk);
801058dc:	8d 73 04             	lea    0x4(%ebx),%esi
801058df:	83 ec 0c             	sub    $0xc,%esp
801058e2:	56                   	push   %esi
801058e3:	e8 28 03 00 00       	call   80105c10 <acquire>
  lk->locked = 0;
801058e8:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
  lk->pid = 0;
801058ee:	c7 43 3c 00 00 00 00 	movl   $0x0,0x3c(%ebx)
  wakeup(lk);
801058f5:	89 1c 24             	mov    %ebx,(%esp)
801058f8:	e8 63 ed ff ff       	call   80104660 <wakeup>
  release(&lk->lk);
801058fd:	89 75 08             	mov    %esi,0x8(%ebp)
80105900:	83 c4 10             	add    $0x10,%esp
}
80105903:	8d 65 f8             	lea    -0x8(%ebp),%esp
80105906:	5b                   	pop    %ebx
80105907:	5e                   	pop    %esi
80105908:	5d                   	pop    %ebp
  release(&lk->lk);
80105909:	e9 c2 03 00 00       	jmp    80105cd0 <release>
8010590e:	66 90                	xchg   %ax,%ax

80105910 <holdingsleep>:

int
holdingsleep(struct sleeplock *lk)
{
80105910:	f3 0f 1e fb          	endbr32 
80105914:	55                   	push   %ebp
80105915:	89 e5                	mov    %esp,%ebp
80105917:	57                   	push   %edi
80105918:	31 ff                	xor    %edi,%edi
8010591a:	56                   	push   %esi
8010591b:	53                   	push   %ebx
8010591c:	83 ec 18             	sub    $0x18,%esp
8010591f:	8b 5d 08             	mov    0x8(%ebp),%ebx
  int r;
  
  acquire(&lk->lk);
80105922:	8d 73 04             	lea    0x4(%ebx),%esi
80105925:	56                   	push   %esi
80105926:	e8 e5 02 00 00       	call   80105c10 <acquire>
  r = lk->locked && (lk->pid == myproc()->pid);
8010592b:	8b 03                	mov    (%ebx),%eax
8010592d:	83 c4 10             	add    $0x10,%esp
80105930:	85 c0                	test   %eax,%eax
80105932:	75 1c                	jne    80105950 <holdingsleep+0x40>
  release(&lk->lk);
80105934:	83 ec 0c             	sub    $0xc,%esp
80105937:	56                   	push   %esi
80105938:	e8 93 03 00 00       	call   80105cd0 <release>
  return r;
}
8010593d:	8d 65 f4             	lea    -0xc(%ebp),%esp
80105940:	89 f8                	mov    %edi,%eax
80105942:	5b                   	pop    %ebx
80105943:	5e                   	pop    %esi
80105944:	5f                   	pop    %edi
80105945:	5d                   	pop    %ebp
80105946:	c3                   	ret    
80105947:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010594e:	66 90                	xchg   %ax,%ax
  r = lk->locked && (lk->pid == myproc()->pid);
80105950:	8b 5b 3c             	mov    0x3c(%ebx),%ebx
80105953:	e8 68 e5 ff ff       	call   80103ec0 <myproc>
80105958:	39 58 10             	cmp    %ebx,0x10(%eax)
8010595b:	0f 94 c0             	sete   %al
8010595e:	0f b6 c0             	movzbl %al,%eax
80105961:	89 c7                	mov    %eax,%edi
80105963:	eb cf                	jmp    80105934 <holdingsleep+0x24>
80105965:	66 90                	xchg   %ax,%ax
80105967:	66 90                	xchg   %ax,%ax
80105969:	66 90                	xchg   %ax,%ax
8010596b:	66 90                	xchg   %ax,%ax
8010596d:	66 90                	xchg   %ax,%ax
8010596f:	90                   	nop

80105970 <acquirePriorityLock>:
#include "proc.h"
#include "spinlock.h"
#include "priorityLock.h"

void acquirePriorityLock(struct PriorityLock *lock)
{
80105970:	f3 0f 1e fb          	endbr32 
80105974:	55                   	push   %ebp
80105975:	89 e5                	mov    %esp,%ebp
80105977:	53                   	push   %ebx
80105978:	83 ec 10             	sub    $0x10,%esp
8010597b:	8b 5d 08             	mov    0x8(%ebp),%ebx
	acquire(&lock->lock);
8010597e:	53                   	push   %ebx
8010597f:	e8 8c 02 00 00       	call   80105c10 <acquire>
	while(lock->is_lock){
80105984:	8b 53 34             	mov    0x34(%ebx),%edx
80105987:	83 c4 10             	add    $0x10,%esp
8010598a:	85 d2                	test   %edx,%edx
8010598c:	74 16                	je     801059a4 <acquirePriorityLock+0x34>
8010598e:	66 90                	xchg   %ax,%ax
		sleep(lock, &lock->lock);
80105990:	83 ec 08             	sub    $0x8,%esp
80105993:	53                   	push   %ebx
80105994:	53                   	push   %ebx
80105995:	e8 06 eb ff ff       	call   801044a0 <sleep>
	while(lock->is_lock){
8010599a:	8b 43 34             	mov    0x34(%ebx),%eax
8010599d:	83 c4 10             	add    $0x10,%esp
801059a0:	85 c0                	test   %eax,%eax
801059a2:	75 ec                	jne    80105990 <acquirePriorityLock+0x20>
	}
	lock->is_lock = 1;
801059a4:	c7 43 34 01 00 00 00 	movl   $0x1,0x34(%ebx)
	lock->pid = myproc()->pid;	
801059ab:	e8 10 e5 ff ff       	call   80103ec0 <myproc>
801059b0:	8b 40 10             	mov    0x10(%eax),%eax
801059b3:	89 43 38             	mov    %eax,0x38(%ebx)
	release(&lock->lock);
801059b6:	89 5d 08             	mov    %ebx,0x8(%ebp)

}
801059b9:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801059bc:	c9                   	leave  
	release(&lock->lock);
801059bd:	e9 0e 03 00 00       	jmp    80105cd0 <release>
801059c2:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801059c9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

801059d0 <releasePriorityLock>:


void releasePriorityLock(struct PriorityLock *lock)
{
801059d0:	f3 0f 1e fb          	endbr32 
801059d4:	55                   	push   %ebp
801059d5:	89 e5                	mov    %esp,%ebp
801059d7:	53                   	push   %ebx
801059d8:	83 ec 10             	sub    $0x10,%esp
801059db:	8b 5d 08             	mov    0x8(%ebp),%ebx
	acquire(&lock->lock);
801059de:	53                   	push   %ebx
801059df:	e8 2c 02 00 00       	call   80105c10 <acquire>
	lock->is_lock = 0;
801059e4:	c7 43 34 00 00 00 00 	movl   $0x0,0x34(%ebx)
	lock->pid = 0;
801059eb:	c7 43 38 00 00 00 00 	movl   $0x0,0x38(%ebx)
	priority_wakeup(lock);
801059f2:	89 1c 24             	mov    %ebx,(%esp)
801059f5:	e8 86 fb ff ff       	call   80105580 <priority_wakeup>
	release(&lock->lock);
801059fa:	89 5d 08             	mov    %ebx,0x8(%ebp)
801059fd:	83 c4 10             	add    $0x10,%esp
}
80105a00:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80105a03:	c9                   	leave  
	release(&lock->lock);
80105a04:	e9 c7 02 00 00       	jmp    80105cd0 <release>
80105a09:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80105a10 <initPriorityLock>:

void initPriorityLock(struct PriorityLock * lock){
80105a10:	f3 0f 1e fb          	endbr32 
80105a14:	55                   	push   %ebp
80105a15:	89 e5                	mov    %esp,%ebp
80105a17:	53                   	push   %ebx
80105a18:	83 ec 0c             	sub    $0xc,%esp
80105a1b:	8b 5d 08             	mov    0x8(%ebp),%ebx
	initlock(&lock->lock , "priority lock");
80105a1e:	68 6f 94 10 80       	push   $0x8010946f
80105a23:	53                   	push   %ebx
80105a24:	e8 67 00 00 00       	call   80105a90 <initlock>
	lock->is_lock = 0;
80105a29:	c7 43 34 00 00 00 00 	movl   $0x0,0x34(%ebx)
	lock->pid = 0;

}
80105a30:	83 c4 10             	add    $0x10,%esp
	lock->pid = 0;
80105a33:	c7 43 38 00 00 00 00 	movl   $0x0,0x38(%ebx)
}
80105a3a:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80105a3d:	c9                   	leave  
80105a3e:	c3                   	ret    
80105a3f:	90                   	nop

80105a40 <holdingPriorityLock>:

int
holdingPriorityLock(struct PriorityLock * lock)
{
80105a40:	f3 0f 1e fb          	endbr32 
80105a44:	55                   	push   %ebp
80105a45:	89 e5                	mov    %esp,%ebp
80105a47:	57                   	push   %edi
80105a48:	56                   	push   %esi
80105a49:	31 f6                	xor    %esi,%esi
80105a4b:	53                   	push   %ebx
80105a4c:	83 ec 18             	sub    $0x18,%esp
80105a4f:	8b 5d 08             	mov    0x8(%ebp),%ebx
	int ret = 0;
	acquire(&lock->lock);
80105a52:	53                   	push   %ebx
80105a53:	e8 b8 01 00 00       	call   80105c10 <acquire>
	ret = (lock->pid == myproc()->pid)&& lock->is_lock;
80105a58:	8b 7b 38             	mov    0x38(%ebx),%edi
80105a5b:	e8 60 e4 ff ff       	call   80103ec0 <myproc>
80105a60:	83 c4 10             	add    $0x10,%esp
80105a63:	3b 78 10             	cmp    0x10(%eax),%edi
80105a66:	75 0c                	jne    80105a74 <holdingPriorityLock+0x34>
80105a68:	8b 53 34             	mov    0x34(%ebx),%edx
80105a6b:	31 c0                	xor    %eax,%eax
80105a6d:	85 d2                	test   %edx,%edx
80105a6f:	0f 95 c0             	setne  %al
80105a72:	89 c6                	mov    %eax,%esi
	release(&lock->lock);
80105a74:	83 ec 0c             	sub    $0xc,%esp
80105a77:	53                   	push   %ebx
80105a78:	e8 53 02 00 00       	call   80105cd0 <release>
	return ret;
80105a7d:	8d 65 f4             	lea    -0xc(%ebp),%esp
80105a80:	89 f0                	mov    %esi,%eax
80105a82:	5b                   	pop    %ebx
80105a83:	5e                   	pop    %esi
80105a84:	5f                   	pop    %edi
80105a85:	5d                   	pop    %ebp
80105a86:	c3                   	ret    
80105a87:	66 90                	xchg   %ax,%ax
80105a89:	66 90                	xchg   %ax,%ax
80105a8b:	66 90                	xchg   %ax,%ax
80105a8d:	66 90                	xchg   %ax,%ax
80105a8f:	90                   	nop

80105a90 <initlock>:
#include "proc.h"
#include "spinlock.h"

void
initlock(struct spinlock *lk, char *name)
{
80105a90:	f3 0f 1e fb          	endbr32 
80105a94:	55                   	push   %ebp
80105a95:	89 e5                	mov    %esp,%ebp
80105a97:	8b 45 08             	mov    0x8(%ebp),%eax
  lk->name = name;
80105a9a:	8b 55 0c             	mov    0xc(%ebp),%edx
  lk->locked = 0;
80105a9d:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  lk->name = name;
80105aa3:	89 50 04             	mov    %edx,0x4(%eax)
  lk->cpu = 0;
80105aa6:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
}
80105aad:	5d                   	pop    %ebp
80105aae:	c3                   	ret    
80105aaf:	90                   	nop

80105ab0 <getcallerpcs>:
}

// Record the current call stack in pcs[] by following the %ebp chain.
void
getcallerpcs(void *v, uint pcs[])
{
80105ab0:	f3 0f 1e fb          	endbr32 
80105ab4:	55                   	push   %ebp
  uint *ebp;
  int i;

  ebp = (uint*)v - 2;
  for(i = 0; i < 10; i++){
80105ab5:	31 d2                	xor    %edx,%edx
{
80105ab7:	89 e5                	mov    %esp,%ebp
80105ab9:	53                   	push   %ebx
  ebp = (uint*)v - 2;
80105aba:	8b 45 08             	mov    0x8(%ebp),%eax
{
80105abd:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  ebp = (uint*)v - 2;
80105ac0:	83 e8 08             	sub    $0x8,%eax
  for(i = 0; i < 10; i++){
80105ac3:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80105ac7:	90                   	nop
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
80105ac8:	8d 98 00 00 00 80    	lea    -0x80000000(%eax),%ebx
80105ace:	81 fb fe ff ff 7f    	cmp    $0x7ffffffe,%ebx
80105ad4:	77 1a                	ja     80105af0 <getcallerpcs+0x40>
      break;
    pcs[i] = ebp[1];     // saved %eip
80105ad6:	8b 58 04             	mov    0x4(%eax),%ebx
80105ad9:	89 1c 91             	mov    %ebx,(%ecx,%edx,4)
  for(i = 0; i < 10; i++){
80105adc:	83 c2 01             	add    $0x1,%edx
    ebp = (uint*)ebp[0]; // saved %ebp
80105adf:	8b 00                	mov    (%eax),%eax
  for(i = 0; i < 10; i++){
80105ae1:	83 fa 0a             	cmp    $0xa,%edx
80105ae4:	75 e2                	jne    80105ac8 <getcallerpcs+0x18>
  }
  for(; i < 10; i++)
    pcs[i] = 0;
}
80105ae6:	5b                   	pop    %ebx
80105ae7:	5d                   	pop    %ebp
80105ae8:	c3                   	ret    
80105ae9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  for(; i < 10; i++)
80105af0:	8d 04 91             	lea    (%ecx,%edx,4),%eax
80105af3:	8d 51 28             	lea    0x28(%ecx),%edx
80105af6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105afd:	8d 76 00             	lea    0x0(%esi),%esi
    pcs[i] = 0;
80105b00:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  for(; i < 10; i++)
80105b06:	83 c0 04             	add    $0x4,%eax
80105b09:	39 d0                	cmp    %edx,%eax
80105b0b:	75 f3                	jne    80105b00 <getcallerpcs+0x50>
}
80105b0d:	5b                   	pop    %ebx
80105b0e:	5d                   	pop    %ebp
80105b0f:	c3                   	ret    

80105b10 <pushcli>:
// it takes two popcli to undo two pushcli.  Also, if interrupts
// are off, then pushcli, popcli leaves them off.

void
pushcli(void)
{
80105b10:	f3 0f 1e fb          	endbr32 
80105b14:	55                   	push   %ebp
80105b15:	89 e5                	mov    %esp,%ebp
80105b17:	53                   	push   %ebx
80105b18:	83 ec 04             	sub    $0x4,%esp
80105b1b:	9c                   	pushf  
80105b1c:	5b                   	pop    %ebx
  asm volatile("cli");
80105b1d:	fa                   	cli    
  int eflags;

  eflags = readeflags();
  cli();
  if(mycpu()->ncli == 0)
80105b1e:	e8 0d e3 ff ff       	call   80103e30 <mycpu>
80105b23:	8b 80 a4 00 00 00    	mov    0xa4(%eax),%eax
80105b29:	85 c0                	test   %eax,%eax
80105b2b:	74 13                	je     80105b40 <pushcli+0x30>
    mycpu()->intena = eflags & FL_IF;
  mycpu()->ncli += 1;
80105b2d:	e8 fe e2 ff ff       	call   80103e30 <mycpu>
80105b32:	83 80 a4 00 00 00 01 	addl   $0x1,0xa4(%eax)
}
80105b39:	83 c4 04             	add    $0x4,%esp
80105b3c:	5b                   	pop    %ebx
80105b3d:	5d                   	pop    %ebp
80105b3e:	c3                   	ret    
80105b3f:	90                   	nop
    mycpu()->intena = eflags & FL_IF;
80105b40:	e8 eb e2 ff ff       	call   80103e30 <mycpu>
80105b45:	81 e3 00 02 00 00    	and    $0x200,%ebx
80105b4b:	89 98 a8 00 00 00    	mov    %ebx,0xa8(%eax)
80105b51:	eb da                	jmp    80105b2d <pushcli+0x1d>
80105b53:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105b5a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80105b60 <popcli>:

void
popcli(void)
{
80105b60:	f3 0f 1e fb          	endbr32 
80105b64:	55                   	push   %ebp
80105b65:	89 e5                	mov    %esp,%ebp
80105b67:	83 ec 08             	sub    $0x8,%esp
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80105b6a:	9c                   	pushf  
80105b6b:	58                   	pop    %eax
  if(readeflags()&FL_IF)
80105b6c:	f6 c4 02             	test   $0x2,%ah
80105b6f:	75 31                	jne    80105ba2 <popcli+0x42>
    panic("popcli - interruptible");
  if(--mycpu()->ncli < 0)
80105b71:	e8 ba e2 ff ff       	call   80103e30 <mycpu>
80105b76:	83 a8 a4 00 00 00 01 	subl   $0x1,0xa4(%eax)
80105b7d:	78 30                	js     80105baf <popcli+0x4f>
    panic("popcli");
  if(mycpu()->ncli == 0 && mycpu()->intena)
80105b7f:	e8 ac e2 ff ff       	call   80103e30 <mycpu>
80105b84:	8b 90 a4 00 00 00    	mov    0xa4(%eax),%edx
80105b8a:	85 d2                	test   %edx,%edx
80105b8c:	74 02                	je     80105b90 <popcli+0x30>
    sti();
}
80105b8e:	c9                   	leave  
80105b8f:	c3                   	ret    
  if(mycpu()->ncli == 0 && mycpu()->intena)
80105b90:	e8 9b e2 ff ff       	call   80103e30 <mycpu>
80105b95:	8b 80 a8 00 00 00    	mov    0xa8(%eax),%eax
80105b9b:	85 c0                	test   %eax,%eax
80105b9d:	74 ef                	je     80105b8e <popcli+0x2e>
  asm volatile("sti");
80105b9f:	fb                   	sti    
}
80105ba0:	c9                   	leave  
80105ba1:	c3                   	ret    
    panic("popcli - interruptible");
80105ba2:	83 ec 0c             	sub    $0xc,%esp
80105ba5:	68 7d 94 10 80       	push   $0x8010947d
80105baa:	e8 e1 a7 ff ff       	call   80100390 <panic>
    panic("popcli");
80105baf:	83 ec 0c             	sub    $0xc,%esp
80105bb2:	68 94 94 10 80       	push   $0x80109494
80105bb7:	e8 d4 a7 ff ff       	call   80100390 <panic>
80105bbc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80105bc0 <holding>:
{
80105bc0:	f3 0f 1e fb          	endbr32 
80105bc4:	55                   	push   %ebp
80105bc5:	89 e5                	mov    %esp,%ebp
80105bc7:	56                   	push   %esi
80105bc8:	53                   	push   %ebx
80105bc9:	8b 75 08             	mov    0x8(%ebp),%esi
80105bcc:	31 db                	xor    %ebx,%ebx
  pushcli();
80105bce:	e8 3d ff ff ff       	call   80105b10 <pushcli>
  r = lock->locked && lock->cpu == mycpu();
80105bd3:	8b 06                	mov    (%esi),%eax
80105bd5:	85 c0                	test   %eax,%eax
80105bd7:	75 0f                	jne    80105be8 <holding+0x28>
  popcli();
80105bd9:	e8 82 ff ff ff       	call   80105b60 <popcli>
}
80105bde:	89 d8                	mov    %ebx,%eax
80105be0:	5b                   	pop    %ebx
80105be1:	5e                   	pop    %esi
80105be2:	5d                   	pop    %ebp
80105be3:	c3                   	ret    
80105be4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  r = lock->locked && lock->cpu == mycpu();
80105be8:	8b 5e 08             	mov    0x8(%esi),%ebx
80105beb:	e8 40 e2 ff ff       	call   80103e30 <mycpu>
80105bf0:	39 c3                	cmp    %eax,%ebx
80105bf2:	0f 94 c3             	sete   %bl
  popcli();
80105bf5:	e8 66 ff ff ff       	call   80105b60 <popcli>
  r = lock->locked && lock->cpu == mycpu();
80105bfa:	0f b6 db             	movzbl %bl,%ebx
}
80105bfd:	89 d8                	mov    %ebx,%eax
80105bff:	5b                   	pop    %ebx
80105c00:	5e                   	pop    %esi
80105c01:	5d                   	pop    %ebp
80105c02:	c3                   	ret    
80105c03:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105c0a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80105c10 <acquire>:
{
80105c10:	f3 0f 1e fb          	endbr32 
80105c14:	55                   	push   %ebp
80105c15:	89 e5                	mov    %esp,%ebp
80105c17:	56                   	push   %esi
80105c18:	53                   	push   %ebx
  pushcli(); // disable interrupts to avoid deadlock.
80105c19:	e8 f2 fe ff ff       	call   80105b10 <pushcli>
  if(holding(lk))
80105c1e:	8b 5d 08             	mov    0x8(%ebp),%ebx
80105c21:	83 ec 0c             	sub    $0xc,%esp
80105c24:	53                   	push   %ebx
80105c25:	e8 96 ff ff ff       	call   80105bc0 <holding>
80105c2a:	83 c4 10             	add    $0x10,%esp
80105c2d:	85 c0                	test   %eax,%eax
80105c2f:	0f 85 7f 00 00 00    	jne    80105cb4 <acquire+0xa4>
80105c35:	89 c6                	mov    %eax,%esi
  asm volatile("lock; xchgl %0, %1" :
80105c37:	ba 01 00 00 00       	mov    $0x1,%edx
80105c3c:	eb 05                	jmp    80105c43 <acquire+0x33>
80105c3e:	66 90                	xchg   %ax,%ax
80105c40:	8b 5d 08             	mov    0x8(%ebp),%ebx
80105c43:	89 d0                	mov    %edx,%eax
80105c45:	f0 87 03             	lock xchg %eax,(%ebx)
  while(xchg(&lk->locked, 1) != 0)
80105c48:	85 c0                	test   %eax,%eax
80105c4a:	75 f4                	jne    80105c40 <acquire+0x30>
  __sync_synchronize();
80105c4c:	f0 83 0c 24 00       	lock orl $0x0,(%esp)
  lk->cpu = mycpu();
80105c51:	8b 5d 08             	mov    0x8(%ebp),%ebx
80105c54:	e8 d7 e1 ff ff       	call   80103e30 <mycpu>
80105c59:	89 43 08             	mov    %eax,0x8(%ebx)
  ebp = (uint*)v - 2;
80105c5c:	89 e8                	mov    %ebp,%eax
80105c5e:	66 90                	xchg   %ax,%ax
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
80105c60:	8d 90 00 00 00 80    	lea    -0x80000000(%eax),%edx
80105c66:	81 fa fe ff ff 7f    	cmp    $0x7ffffffe,%edx
80105c6c:	77 22                	ja     80105c90 <acquire+0x80>
    pcs[i] = ebp[1];     // saved %eip
80105c6e:	8b 50 04             	mov    0x4(%eax),%edx
80105c71:	89 54 b3 0c          	mov    %edx,0xc(%ebx,%esi,4)
  for(i = 0; i < 10; i++){
80105c75:	83 c6 01             	add    $0x1,%esi
    ebp = (uint*)ebp[0]; // saved %ebp
80105c78:	8b 00                	mov    (%eax),%eax
  for(i = 0; i < 10; i++){
80105c7a:	83 fe 0a             	cmp    $0xa,%esi
80105c7d:	75 e1                	jne    80105c60 <acquire+0x50>
}
80105c7f:	8d 65 f8             	lea    -0x8(%ebp),%esp
80105c82:	5b                   	pop    %ebx
80105c83:	5e                   	pop    %esi
80105c84:	5d                   	pop    %ebp
80105c85:	c3                   	ret    
80105c86:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105c8d:	8d 76 00             	lea    0x0(%esi),%esi
  for(; i < 10; i++)
80105c90:	8d 44 b3 0c          	lea    0xc(%ebx,%esi,4),%eax
80105c94:	83 c3 34             	add    $0x34,%ebx
80105c97:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105c9e:	66 90                	xchg   %ax,%ax
    pcs[i] = 0;
80105ca0:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  for(; i < 10; i++)
80105ca6:	83 c0 04             	add    $0x4,%eax
80105ca9:	39 d8                	cmp    %ebx,%eax
80105cab:	75 f3                	jne    80105ca0 <acquire+0x90>
}
80105cad:	8d 65 f8             	lea    -0x8(%ebp),%esp
80105cb0:	5b                   	pop    %ebx
80105cb1:	5e                   	pop    %esi
80105cb2:	5d                   	pop    %ebp
80105cb3:	c3                   	ret    
    panic("acquire");
80105cb4:	83 ec 0c             	sub    $0xc,%esp
80105cb7:	68 9b 94 10 80       	push   $0x8010949b
80105cbc:	e8 cf a6 ff ff       	call   80100390 <panic>
80105cc1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105cc8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105ccf:	90                   	nop

80105cd0 <release>:
{
80105cd0:	f3 0f 1e fb          	endbr32 
80105cd4:	55                   	push   %ebp
80105cd5:	89 e5                	mov    %esp,%ebp
80105cd7:	53                   	push   %ebx
80105cd8:	83 ec 10             	sub    $0x10,%esp
80105cdb:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(!holding(lk))
80105cde:	53                   	push   %ebx
80105cdf:	e8 dc fe ff ff       	call   80105bc0 <holding>
80105ce4:	83 c4 10             	add    $0x10,%esp
80105ce7:	85 c0                	test   %eax,%eax
80105ce9:	74 22                	je     80105d0d <release+0x3d>
  lk->pcs[0] = 0;
80105ceb:	c7 43 0c 00 00 00 00 	movl   $0x0,0xc(%ebx)
  lk->cpu = 0;
80105cf2:	c7 43 08 00 00 00 00 	movl   $0x0,0x8(%ebx)
  __sync_synchronize();
80105cf9:	f0 83 0c 24 00       	lock orl $0x0,(%esp)
  asm volatile("movl $0, %0" : "+m" (lk->locked) : );
80105cfe:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
}
80105d04:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80105d07:	c9                   	leave  
  popcli();
80105d08:	e9 53 fe ff ff       	jmp    80105b60 <popcli>
    panic("release");
80105d0d:	83 ec 0c             	sub    $0xc,%esp
80105d10:	68 a3 94 10 80       	push   $0x801094a3
80105d15:	e8 76 a6 ff ff       	call   80100390 <panic>
80105d1a:	66 90                	xchg   %ax,%ax
80105d1c:	66 90                	xchg   %ax,%ax
80105d1e:	66 90                	xchg   %ax,%ax

80105d20 <memset>:
#include "types.h"
#include "x86.h"

void*
memset(void *dst, int c, uint n)
{
80105d20:	f3 0f 1e fb          	endbr32 
80105d24:	55                   	push   %ebp
80105d25:	89 e5                	mov    %esp,%ebp
80105d27:	57                   	push   %edi
80105d28:	8b 55 08             	mov    0x8(%ebp),%edx
80105d2b:	8b 4d 10             	mov    0x10(%ebp),%ecx
80105d2e:	53                   	push   %ebx
80105d2f:	8b 45 0c             	mov    0xc(%ebp),%eax
  if ((int)dst%4 == 0 && n%4 == 0){
80105d32:	89 d7                	mov    %edx,%edi
80105d34:	09 cf                	or     %ecx,%edi
80105d36:	83 e7 03             	and    $0x3,%edi
80105d39:	75 25                	jne    80105d60 <memset+0x40>
    c &= 0xFF;
80105d3b:	0f b6 f8             	movzbl %al,%edi
    stosl(dst, (c<<24)|(c<<16)|(c<<8)|c, n/4);
80105d3e:	c1 e0 18             	shl    $0x18,%eax
80105d41:	89 fb                	mov    %edi,%ebx
80105d43:	c1 e9 02             	shr    $0x2,%ecx
80105d46:	c1 e3 10             	shl    $0x10,%ebx
80105d49:	09 d8                	or     %ebx,%eax
80105d4b:	09 f8                	or     %edi,%eax
80105d4d:	c1 e7 08             	shl    $0x8,%edi
80105d50:	09 f8                	or     %edi,%eax
  asm volatile("cld; rep stosl" :
80105d52:	89 d7                	mov    %edx,%edi
80105d54:	fc                   	cld    
80105d55:	f3 ab                	rep stos %eax,%es:(%edi)
  } else
    stosb(dst, c, n);
  return dst;
}
80105d57:	5b                   	pop    %ebx
80105d58:	89 d0                	mov    %edx,%eax
80105d5a:	5f                   	pop    %edi
80105d5b:	5d                   	pop    %ebp
80105d5c:	c3                   	ret    
80105d5d:	8d 76 00             	lea    0x0(%esi),%esi
  asm volatile("cld; rep stosb" :
80105d60:	89 d7                	mov    %edx,%edi
80105d62:	fc                   	cld    
80105d63:	f3 aa                	rep stos %al,%es:(%edi)
80105d65:	5b                   	pop    %ebx
80105d66:	89 d0                	mov    %edx,%eax
80105d68:	5f                   	pop    %edi
80105d69:	5d                   	pop    %ebp
80105d6a:	c3                   	ret    
80105d6b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80105d6f:	90                   	nop

80105d70 <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
80105d70:	f3 0f 1e fb          	endbr32 
80105d74:	55                   	push   %ebp
80105d75:	89 e5                	mov    %esp,%ebp
80105d77:	56                   	push   %esi
80105d78:	8b 75 10             	mov    0x10(%ebp),%esi
80105d7b:	8b 55 08             	mov    0x8(%ebp),%edx
80105d7e:	53                   	push   %ebx
80105d7f:	8b 45 0c             	mov    0xc(%ebp),%eax
  const uchar *s1, *s2;

  s1 = v1;
  s2 = v2;
  while(n-- > 0){
80105d82:	85 f6                	test   %esi,%esi
80105d84:	74 2a                	je     80105db0 <memcmp+0x40>
80105d86:	01 c6                	add    %eax,%esi
80105d88:	eb 10                	jmp    80105d9a <memcmp+0x2a>
80105d8a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    if(*s1 != *s2)
      return *s1 - *s2;
    s1++, s2++;
80105d90:	83 c0 01             	add    $0x1,%eax
80105d93:	83 c2 01             	add    $0x1,%edx
  while(n-- > 0){
80105d96:	39 f0                	cmp    %esi,%eax
80105d98:	74 16                	je     80105db0 <memcmp+0x40>
    if(*s1 != *s2)
80105d9a:	0f b6 0a             	movzbl (%edx),%ecx
80105d9d:	0f b6 18             	movzbl (%eax),%ebx
80105da0:	38 d9                	cmp    %bl,%cl
80105da2:	74 ec                	je     80105d90 <memcmp+0x20>
      return *s1 - *s2;
80105da4:	0f b6 c1             	movzbl %cl,%eax
80105da7:	29 d8                	sub    %ebx,%eax
  }

  return 0;
}
80105da9:	5b                   	pop    %ebx
80105daa:	5e                   	pop    %esi
80105dab:	5d                   	pop    %ebp
80105dac:	c3                   	ret    
80105dad:	8d 76 00             	lea    0x0(%esi),%esi
80105db0:	5b                   	pop    %ebx
  return 0;
80105db1:	31 c0                	xor    %eax,%eax
}
80105db3:	5e                   	pop    %esi
80105db4:	5d                   	pop    %ebp
80105db5:	c3                   	ret    
80105db6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105dbd:	8d 76 00             	lea    0x0(%esi),%esi

80105dc0 <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
80105dc0:	f3 0f 1e fb          	endbr32 
80105dc4:	55                   	push   %ebp
80105dc5:	89 e5                	mov    %esp,%ebp
80105dc7:	57                   	push   %edi
80105dc8:	8b 55 08             	mov    0x8(%ebp),%edx
80105dcb:	8b 4d 10             	mov    0x10(%ebp),%ecx
80105dce:	56                   	push   %esi
80105dcf:	8b 75 0c             	mov    0xc(%ebp),%esi
  const char *s;
  char *d;

  s = src;
  d = dst;
  if(s < d && s + n > d){
80105dd2:	39 d6                	cmp    %edx,%esi
80105dd4:	73 2a                	jae    80105e00 <memmove+0x40>
80105dd6:	8d 3c 0e             	lea    (%esi,%ecx,1),%edi
80105dd9:	39 fa                	cmp    %edi,%edx
80105ddb:	73 23                	jae    80105e00 <memmove+0x40>
80105ddd:	8d 41 ff             	lea    -0x1(%ecx),%eax
    s += n;
    d += n;
    while(n-- > 0)
80105de0:	85 c9                	test   %ecx,%ecx
80105de2:	74 13                	je     80105df7 <memmove+0x37>
80105de4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      *--d = *--s;
80105de8:	0f b6 0c 06          	movzbl (%esi,%eax,1),%ecx
80105dec:	88 0c 02             	mov    %cl,(%edx,%eax,1)
    while(n-- > 0)
80105def:	83 e8 01             	sub    $0x1,%eax
80105df2:	83 f8 ff             	cmp    $0xffffffff,%eax
80105df5:	75 f1                	jne    80105de8 <memmove+0x28>
  } else
    while(n-- > 0)
      *d++ = *s++;

  return dst;
}
80105df7:	5e                   	pop    %esi
80105df8:	89 d0                	mov    %edx,%eax
80105dfa:	5f                   	pop    %edi
80105dfb:	5d                   	pop    %ebp
80105dfc:	c3                   	ret    
80105dfd:	8d 76 00             	lea    0x0(%esi),%esi
    while(n-- > 0)
80105e00:	8d 04 0e             	lea    (%esi,%ecx,1),%eax
80105e03:	89 d7                	mov    %edx,%edi
80105e05:	85 c9                	test   %ecx,%ecx
80105e07:	74 ee                	je     80105df7 <memmove+0x37>
80105e09:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
      *d++ = *s++;
80105e10:	a4                   	movsb  %ds:(%esi),%es:(%edi)
    while(n-- > 0)
80105e11:	39 f0                	cmp    %esi,%eax
80105e13:	75 fb                	jne    80105e10 <memmove+0x50>
}
80105e15:	5e                   	pop    %esi
80105e16:	89 d0                	mov    %edx,%eax
80105e18:	5f                   	pop    %edi
80105e19:	5d                   	pop    %ebp
80105e1a:	c3                   	ret    
80105e1b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80105e1f:	90                   	nop

80105e20 <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
80105e20:	f3 0f 1e fb          	endbr32 
  return memmove(dst, src, n);
80105e24:	eb 9a                	jmp    80105dc0 <memmove>
80105e26:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105e2d:	8d 76 00             	lea    0x0(%esi),%esi

80105e30 <strncmp>:
}

int
strncmp(const char *p, const char *q, uint n)
{
80105e30:	f3 0f 1e fb          	endbr32 
80105e34:	55                   	push   %ebp
80105e35:	89 e5                	mov    %esp,%ebp
80105e37:	56                   	push   %esi
80105e38:	8b 75 10             	mov    0x10(%ebp),%esi
80105e3b:	8b 4d 08             	mov    0x8(%ebp),%ecx
80105e3e:	53                   	push   %ebx
80105e3f:	8b 45 0c             	mov    0xc(%ebp),%eax
  while(n > 0 && *p && *p == *q)
80105e42:	85 f6                	test   %esi,%esi
80105e44:	74 32                	je     80105e78 <strncmp+0x48>
80105e46:	01 c6                	add    %eax,%esi
80105e48:	eb 14                	jmp    80105e5e <strncmp+0x2e>
80105e4a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80105e50:	38 da                	cmp    %bl,%dl
80105e52:	75 14                	jne    80105e68 <strncmp+0x38>
    n--, p++, q++;
80105e54:	83 c0 01             	add    $0x1,%eax
80105e57:	83 c1 01             	add    $0x1,%ecx
  while(n > 0 && *p && *p == *q)
80105e5a:	39 f0                	cmp    %esi,%eax
80105e5c:	74 1a                	je     80105e78 <strncmp+0x48>
80105e5e:	0f b6 11             	movzbl (%ecx),%edx
80105e61:	0f b6 18             	movzbl (%eax),%ebx
80105e64:	84 d2                	test   %dl,%dl
80105e66:	75 e8                	jne    80105e50 <strncmp+0x20>
  if(n == 0)
    return 0;
  return (uchar)*p - (uchar)*q;
80105e68:	0f b6 c2             	movzbl %dl,%eax
80105e6b:	29 d8                	sub    %ebx,%eax
}
80105e6d:	5b                   	pop    %ebx
80105e6e:	5e                   	pop    %esi
80105e6f:	5d                   	pop    %ebp
80105e70:	c3                   	ret    
80105e71:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105e78:	5b                   	pop    %ebx
    return 0;
80105e79:	31 c0                	xor    %eax,%eax
}
80105e7b:	5e                   	pop    %esi
80105e7c:	5d                   	pop    %ebp
80105e7d:	c3                   	ret    
80105e7e:	66 90                	xchg   %ax,%ax

80105e80 <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
80105e80:	f3 0f 1e fb          	endbr32 
80105e84:	55                   	push   %ebp
80105e85:	89 e5                	mov    %esp,%ebp
80105e87:	57                   	push   %edi
80105e88:	56                   	push   %esi
80105e89:	8b 75 08             	mov    0x8(%ebp),%esi
80105e8c:	53                   	push   %ebx
80105e8d:	8b 45 10             	mov    0x10(%ebp),%eax
  char *os;

  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
80105e90:	89 f2                	mov    %esi,%edx
80105e92:	eb 1b                	jmp    80105eaf <strncpy+0x2f>
80105e94:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80105e98:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
80105e9c:	8b 7d 0c             	mov    0xc(%ebp),%edi
80105e9f:	83 c2 01             	add    $0x1,%edx
80105ea2:	0f b6 7f ff          	movzbl -0x1(%edi),%edi
80105ea6:	89 f9                	mov    %edi,%ecx
80105ea8:	88 4a ff             	mov    %cl,-0x1(%edx)
80105eab:	84 c9                	test   %cl,%cl
80105ead:	74 09                	je     80105eb8 <strncpy+0x38>
80105eaf:	89 c3                	mov    %eax,%ebx
80105eb1:	83 e8 01             	sub    $0x1,%eax
80105eb4:	85 db                	test   %ebx,%ebx
80105eb6:	7f e0                	jg     80105e98 <strncpy+0x18>
    ;
  while(n-- > 0)
80105eb8:	89 d1                	mov    %edx,%ecx
80105eba:	85 c0                	test   %eax,%eax
80105ebc:	7e 15                	jle    80105ed3 <strncpy+0x53>
80105ebe:	66 90                	xchg   %ax,%ax
    *s++ = 0;
80105ec0:	83 c1 01             	add    $0x1,%ecx
80105ec3:	c6 41 ff 00          	movb   $0x0,-0x1(%ecx)
  while(n-- > 0)
80105ec7:	89 c8                	mov    %ecx,%eax
80105ec9:	f7 d0                	not    %eax
80105ecb:	01 d0                	add    %edx,%eax
80105ecd:	01 d8                	add    %ebx,%eax
80105ecf:	85 c0                	test   %eax,%eax
80105ed1:	7f ed                	jg     80105ec0 <strncpy+0x40>
  return os;
}
80105ed3:	5b                   	pop    %ebx
80105ed4:	89 f0                	mov    %esi,%eax
80105ed6:	5e                   	pop    %esi
80105ed7:	5f                   	pop    %edi
80105ed8:	5d                   	pop    %ebp
80105ed9:	c3                   	ret    
80105eda:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80105ee0 <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
80105ee0:	f3 0f 1e fb          	endbr32 
80105ee4:	55                   	push   %ebp
80105ee5:	89 e5                	mov    %esp,%ebp
80105ee7:	56                   	push   %esi
80105ee8:	8b 55 10             	mov    0x10(%ebp),%edx
80105eeb:	8b 75 08             	mov    0x8(%ebp),%esi
80105eee:	53                   	push   %ebx
80105eef:	8b 45 0c             	mov    0xc(%ebp),%eax
  char *os;

  os = s;
  if(n <= 0)
80105ef2:	85 d2                	test   %edx,%edx
80105ef4:	7e 21                	jle    80105f17 <safestrcpy+0x37>
80105ef6:	8d 5c 10 ff          	lea    -0x1(%eax,%edx,1),%ebx
80105efa:	89 f2                	mov    %esi,%edx
80105efc:	eb 12                	jmp    80105f10 <safestrcpy+0x30>
80105efe:	66 90                	xchg   %ax,%ax
    return os;
  while(--n > 0 && (*s++ = *t++) != 0)
80105f00:	0f b6 08             	movzbl (%eax),%ecx
80105f03:	83 c0 01             	add    $0x1,%eax
80105f06:	83 c2 01             	add    $0x1,%edx
80105f09:	88 4a ff             	mov    %cl,-0x1(%edx)
80105f0c:	84 c9                	test   %cl,%cl
80105f0e:	74 04                	je     80105f14 <safestrcpy+0x34>
80105f10:	39 d8                	cmp    %ebx,%eax
80105f12:	75 ec                	jne    80105f00 <safestrcpy+0x20>
    ;
  *s = 0;
80105f14:	c6 02 00             	movb   $0x0,(%edx)
  return os;
}
80105f17:	89 f0                	mov    %esi,%eax
80105f19:	5b                   	pop    %ebx
80105f1a:	5e                   	pop    %esi
80105f1b:	5d                   	pop    %ebp
80105f1c:	c3                   	ret    
80105f1d:	8d 76 00             	lea    0x0(%esi),%esi

80105f20 <strlen>:

int
strlen(const char *s)
{
80105f20:	f3 0f 1e fb          	endbr32 
80105f24:	55                   	push   %ebp
  int n;

  for(n = 0; s[n]; n++)
80105f25:	31 c0                	xor    %eax,%eax
{
80105f27:	89 e5                	mov    %esp,%ebp
80105f29:	8b 55 08             	mov    0x8(%ebp),%edx
  for(n = 0; s[n]; n++)
80105f2c:	80 3a 00             	cmpb   $0x0,(%edx)
80105f2f:	74 10                	je     80105f41 <strlen+0x21>
80105f31:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105f38:	83 c0 01             	add    $0x1,%eax
80105f3b:	80 3c 02 00          	cmpb   $0x0,(%edx,%eax,1)
80105f3f:	75 f7                	jne    80105f38 <strlen+0x18>
    ;
  return n;
}
80105f41:	5d                   	pop    %ebp
80105f42:	c3                   	ret    

80105f43 <swtch>:
# a struct context, and save its address in *old.
# Switch stacks to new and pop previously-saved registers.

.globl swtch
swtch:
  movl 4(%esp), %eax
80105f43:	8b 44 24 04          	mov    0x4(%esp),%eax
  movl 8(%esp), %edx
80105f47:	8b 54 24 08          	mov    0x8(%esp),%edx

  # Save old callee-saved registers
  pushl %ebp
80105f4b:	55                   	push   %ebp
  pushl %ebx
80105f4c:	53                   	push   %ebx
  pushl %esi
80105f4d:	56                   	push   %esi
  pushl %edi
80105f4e:	57                   	push   %edi

  # Switch stacks
  movl %esp, (%eax)
80105f4f:	89 20                	mov    %esp,(%eax)
  movl %edx, %esp
80105f51:	89 d4                	mov    %edx,%esp

  # Load new callee-saved registers
  popl %edi
80105f53:	5f                   	pop    %edi
  popl %esi
80105f54:	5e                   	pop    %esi
  popl %ebx
80105f55:	5b                   	pop    %ebx
  popl %ebp
80105f56:	5d                   	pop    %ebp
  ret
80105f57:	c3                   	ret    
80105f58:	66 90                	xchg   %ax,%ax
80105f5a:	66 90                	xchg   %ax,%ax
80105f5c:	66 90                	xchg   %ax,%ax
80105f5e:	66 90                	xchg   %ax,%ax

80105f60 <fetchfloat>:
#include "x86.h"
#include "syscall.h"

int
fetchfloat(uint addr, float *fp)
{
80105f60:	f3 0f 1e fb          	endbr32 
80105f64:	55                   	push   %ebp
80105f65:	89 e5                	mov    %esp,%ebp
80105f67:	53                   	push   %ebx
80105f68:	83 ec 04             	sub    $0x4,%esp
80105f6b:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct proc *curproc = myproc();
80105f6e:	e8 4d df ff ff       	call   80103ec0 <myproc>

  if(addr >= curproc->sz || addr+4 > curproc->sz)
80105f73:	8b 00                	mov    (%eax),%eax
80105f75:	39 d8                	cmp    %ebx,%eax
80105f77:	76 17                	jbe    80105f90 <fetchfloat+0x30>
80105f79:	8d 53 04             	lea    0x4(%ebx),%edx
80105f7c:	39 d0                	cmp    %edx,%eax
80105f7e:	72 10                	jb     80105f90 <fetchfloat+0x30>
    return -1;
  *fp = *(float*)(addr);
80105f80:	d9 03                	flds   (%ebx)
80105f82:	8b 45 0c             	mov    0xc(%ebp),%eax
80105f85:	d9 18                	fstps  (%eax)
  return 0;
80105f87:	31 c0                	xor    %eax,%eax
}
80105f89:	83 c4 04             	add    $0x4,%esp
80105f8c:	5b                   	pop    %ebx
80105f8d:	5d                   	pop    %ebp
80105f8e:	c3                   	ret    
80105f8f:	90                   	nop
    return -1;
80105f90:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105f95:	eb f2                	jmp    80105f89 <fetchfloat+0x29>
80105f97:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105f9e:	66 90                	xchg   %ax,%ax

80105fa0 <argfloat>:


int
argfloat(int n, float *fp)
{
80105fa0:	f3 0f 1e fb          	endbr32 
80105fa4:	55                   	push   %ebp
80105fa5:	89 e5                	mov    %esp,%ebp
80105fa7:	56                   	push   %esi
80105fa8:	53                   	push   %ebx
  return fetchfloat((myproc()->tf->esp) + 4 + 4*n, fp);
80105fa9:	e8 12 df ff ff       	call   80103ec0 <myproc>
80105fae:	8b 55 08             	mov    0x8(%ebp),%edx
80105fb1:	8b 40 18             	mov    0x18(%eax),%eax
80105fb4:	8b 40 44             	mov    0x44(%eax),%eax
80105fb7:	8d 1c 90             	lea    (%eax,%edx,4),%ebx
  struct proc *curproc = myproc();
80105fba:	e8 01 df ff ff       	call   80103ec0 <myproc>
  return fetchfloat((myproc()->tf->esp) + 4 + 4*n, fp);
80105fbf:	8d 73 04             	lea    0x4(%ebx),%esi
  if(addr >= curproc->sz || addr+4 > curproc->sz)
80105fc2:	8b 00                	mov    (%eax),%eax
80105fc4:	39 c6                	cmp    %eax,%esi
80105fc6:	73 18                	jae    80105fe0 <argfloat+0x40>
80105fc8:	8d 53 08             	lea    0x8(%ebx),%edx
80105fcb:	39 d0                	cmp    %edx,%eax
80105fcd:	72 11                	jb     80105fe0 <argfloat+0x40>
  *fp = *(float*)(addr);
80105fcf:	d9 43 04             	flds   0x4(%ebx)
80105fd2:	8b 45 0c             	mov    0xc(%ebp),%eax
80105fd5:	d9 18                	fstps  (%eax)
  return 0;
80105fd7:	31 c0                	xor    %eax,%eax
}
80105fd9:	5b                   	pop    %ebx
80105fda:	5e                   	pop    %esi
80105fdb:	5d                   	pop    %ebp
80105fdc:	c3                   	ret    
80105fdd:	8d 76 00             	lea    0x0(%esi),%esi
    return -1;
80105fe0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
  return fetchfloat((myproc()->tf->esp) + 4 + 4*n, fp);
80105fe5:	eb f2                	jmp    80105fd9 <argfloat+0x39>
80105fe7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105fee:	66 90                	xchg   %ax,%ax

80105ff0 <fetchint>:
// to a saved program counter, and then the first argument.

// Fetch the int at addr from the current process.
int
fetchint(uint addr, int *ip)
{
80105ff0:	f3 0f 1e fb          	endbr32 
80105ff4:	55                   	push   %ebp
80105ff5:	89 e5                	mov    %esp,%ebp
80105ff7:	53                   	push   %ebx
80105ff8:	83 ec 04             	sub    $0x4,%esp
80105ffb:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct proc *curproc = myproc();
80105ffe:	e8 bd de ff ff       	call   80103ec0 <myproc>

  if(addr >= curproc->sz || addr+4 > curproc->sz)
80106003:	8b 00                	mov    (%eax),%eax
80106005:	39 d8                	cmp    %ebx,%eax
80106007:	76 17                	jbe    80106020 <fetchint+0x30>
80106009:	8d 53 04             	lea    0x4(%ebx),%edx
8010600c:	39 d0                	cmp    %edx,%eax
8010600e:	72 10                	jb     80106020 <fetchint+0x30>
    return -1;
  *ip = *(int*)(addr);
80106010:	8b 45 0c             	mov    0xc(%ebp),%eax
80106013:	8b 13                	mov    (%ebx),%edx
80106015:	89 10                	mov    %edx,(%eax)
  return 0;
80106017:	31 c0                	xor    %eax,%eax
}
80106019:	83 c4 04             	add    $0x4,%esp
8010601c:	5b                   	pop    %ebx
8010601d:	5d                   	pop    %ebp
8010601e:	c3                   	ret    
8010601f:	90                   	nop
    return -1;
80106020:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106025:	eb f2                	jmp    80106019 <fetchint+0x29>
80106027:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010602e:	66 90                	xchg   %ax,%ax

80106030 <fetchstr>:
// Fetch the nul-terminated string at addr from the current process.
// Doesn't actually copy the string - just sets *pp to point at it.
// Returns length of string, not including nul.
int
fetchstr(uint addr, char **pp)
{
80106030:	f3 0f 1e fb          	endbr32 
80106034:	55                   	push   %ebp
80106035:	89 e5                	mov    %esp,%ebp
80106037:	53                   	push   %ebx
80106038:	83 ec 04             	sub    $0x4,%esp
8010603b:	8b 5d 08             	mov    0x8(%ebp),%ebx
  char *s, *ep;
  struct proc *curproc = myproc();
8010603e:	e8 7d de ff ff       	call   80103ec0 <myproc>

  if(addr >= curproc->sz)
80106043:	39 18                	cmp    %ebx,(%eax)
80106045:	76 31                	jbe    80106078 <fetchstr+0x48>
    return -1;
  *pp = (char*)addr;
80106047:	8b 55 0c             	mov    0xc(%ebp),%edx
8010604a:	89 1a                	mov    %ebx,(%edx)
  ep = (char*)curproc->sz;
8010604c:	8b 10                	mov    (%eax),%edx
  for(s = *pp; s < ep; s++){
8010604e:	39 d3                	cmp    %edx,%ebx
80106050:	73 26                	jae    80106078 <fetchstr+0x48>
80106052:	89 d8                	mov    %ebx,%eax
80106054:	eb 11                	jmp    80106067 <fetchstr+0x37>
80106056:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010605d:	8d 76 00             	lea    0x0(%esi),%esi
80106060:	83 c0 01             	add    $0x1,%eax
80106063:	39 c2                	cmp    %eax,%edx
80106065:	76 11                	jbe    80106078 <fetchstr+0x48>
    if(*s == 0)
80106067:	80 38 00             	cmpb   $0x0,(%eax)
8010606a:	75 f4                	jne    80106060 <fetchstr+0x30>
      return s - *pp;
  }
  return -1;
}
8010606c:	83 c4 04             	add    $0x4,%esp
      return s - *pp;
8010606f:	29 d8                	sub    %ebx,%eax
}
80106071:	5b                   	pop    %ebx
80106072:	5d                   	pop    %ebp
80106073:	c3                   	ret    
80106074:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80106078:	83 c4 04             	add    $0x4,%esp
    return -1;
8010607b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106080:	5b                   	pop    %ebx
80106081:	5d                   	pop    %ebp
80106082:	c3                   	ret    
80106083:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010608a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80106090 <argint>:

// Fetch the nth 32-bit system call argument.
int
argint(int n, int *ip)
{
80106090:	f3 0f 1e fb          	endbr32 
80106094:	55                   	push   %ebp
80106095:	89 e5                	mov    %esp,%ebp
80106097:	56                   	push   %esi
80106098:	53                   	push   %ebx
  return fetchint((myproc()->tf->esp) + 4 + 4*n, ip);
80106099:	e8 22 de ff ff       	call   80103ec0 <myproc>
8010609e:	8b 55 08             	mov    0x8(%ebp),%edx
801060a1:	8b 40 18             	mov    0x18(%eax),%eax
801060a4:	8b 40 44             	mov    0x44(%eax),%eax
801060a7:	8d 1c 90             	lea    (%eax,%edx,4),%ebx
  struct proc *curproc = myproc();
801060aa:	e8 11 de ff ff       	call   80103ec0 <myproc>
  return fetchint((myproc()->tf->esp) + 4 + 4*n, ip);
801060af:	8d 73 04             	lea    0x4(%ebx),%esi
  if(addr >= curproc->sz || addr+4 > curproc->sz)
801060b2:	8b 00                	mov    (%eax),%eax
801060b4:	39 c6                	cmp    %eax,%esi
801060b6:	73 18                	jae    801060d0 <argint+0x40>
801060b8:	8d 53 08             	lea    0x8(%ebx),%edx
801060bb:	39 d0                	cmp    %edx,%eax
801060bd:	72 11                	jb     801060d0 <argint+0x40>
  *ip = *(int*)(addr);
801060bf:	8b 45 0c             	mov    0xc(%ebp),%eax
801060c2:	8b 53 04             	mov    0x4(%ebx),%edx
801060c5:	89 10                	mov    %edx,(%eax)
  return 0;
801060c7:	31 c0                	xor    %eax,%eax
}
801060c9:	5b                   	pop    %ebx
801060ca:	5e                   	pop    %esi
801060cb:	5d                   	pop    %ebp
801060cc:	c3                   	ret    
801060cd:	8d 76 00             	lea    0x0(%esi),%esi
    return -1;
801060d0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
  return fetchint((myproc()->tf->esp) + 4 + 4*n, ip);
801060d5:	eb f2                	jmp    801060c9 <argint+0x39>
801060d7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801060de:	66 90                	xchg   %ax,%ax

801060e0 <argptr>:
// Fetch the nth word-sized system call argument as a pointer
// to a block of memory of size bytes.  Check that the pointer
// lies within the process address space.
int
argptr(int n, char **pp, int size)
{
801060e0:	f3 0f 1e fb          	endbr32 
801060e4:	55                   	push   %ebp
801060e5:	89 e5                	mov    %esp,%ebp
801060e7:	56                   	push   %esi
801060e8:	53                   	push   %ebx
801060e9:	83 ec 10             	sub    $0x10,%esp
801060ec:	8b 5d 10             	mov    0x10(%ebp),%ebx
  int i;
  struct proc *curproc = myproc();
801060ef:	e8 cc dd ff ff       	call   80103ec0 <myproc>
 
  if(argint(n, &i) < 0)
801060f4:	83 ec 08             	sub    $0x8,%esp
  struct proc *curproc = myproc();
801060f7:	89 c6                	mov    %eax,%esi
  if(argint(n, &i) < 0)
801060f9:	8d 45 f4             	lea    -0xc(%ebp),%eax
801060fc:	50                   	push   %eax
801060fd:	ff 75 08             	pushl  0x8(%ebp)
80106100:	e8 8b ff ff ff       	call   80106090 <argint>
    return -1;
  if(size < 0 || (uint)i >= curproc->sz || (uint)i+size > curproc->sz)
80106105:	83 c4 10             	add    $0x10,%esp
80106108:	85 c0                	test   %eax,%eax
8010610a:	78 24                	js     80106130 <argptr+0x50>
8010610c:	85 db                	test   %ebx,%ebx
8010610e:	78 20                	js     80106130 <argptr+0x50>
80106110:	8b 16                	mov    (%esi),%edx
80106112:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106115:	39 c2                	cmp    %eax,%edx
80106117:	76 17                	jbe    80106130 <argptr+0x50>
80106119:	01 c3                	add    %eax,%ebx
8010611b:	39 da                	cmp    %ebx,%edx
8010611d:	72 11                	jb     80106130 <argptr+0x50>
    return -1;
  *pp = (char*)i;
8010611f:	8b 55 0c             	mov    0xc(%ebp),%edx
80106122:	89 02                	mov    %eax,(%edx)
  return 0;
80106124:	31 c0                	xor    %eax,%eax
}
80106126:	8d 65 f8             	lea    -0x8(%ebp),%esp
80106129:	5b                   	pop    %ebx
8010612a:	5e                   	pop    %esi
8010612b:	5d                   	pop    %ebp
8010612c:	c3                   	ret    
8010612d:	8d 76 00             	lea    0x0(%esi),%esi
    return -1;
80106130:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106135:	eb ef                	jmp    80106126 <argptr+0x46>
80106137:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010613e:	66 90                	xchg   %ax,%ax

80106140 <argstr>:
// Check that the pointer is valid and the string is nul-terminated.
// (There is no shared writable memory, so the string can't change
// between this check and being used by the kernel.)
int
argstr(int n, char **pp)
{
80106140:	f3 0f 1e fb          	endbr32 
80106144:	55                   	push   %ebp
80106145:	89 e5                	mov    %esp,%ebp
80106147:	83 ec 20             	sub    $0x20,%esp
  int addr;
  if(argint(n, &addr) < 0)
8010614a:	8d 45 f4             	lea    -0xc(%ebp),%eax
8010614d:	50                   	push   %eax
8010614e:	ff 75 08             	pushl  0x8(%ebp)
80106151:	e8 3a ff ff ff       	call   80106090 <argint>
80106156:	83 c4 10             	add    $0x10,%esp
80106159:	85 c0                	test   %eax,%eax
8010615b:	78 13                	js     80106170 <argstr+0x30>
    return -1;
  return fetchstr(addr, pp);
8010615d:	83 ec 08             	sub    $0x8,%esp
80106160:	ff 75 0c             	pushl  0xc(%ebp)
80106163:	ff 75 f4             	pushl  -0xc(%ebp)
80106166:	e8 c5 fe ff ff       	call   80106030 <fetchstr>
8010616b:	83 c4 10             	add    $0x10,%esp
}
8010616e:	c9                   	leave  
8010616f:	c3                   	ret    
80106170:	c9                   	leave  
    return -1;
80106171:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106176:	c3                   	ret    
80106177:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010617e:	66 90                	xchg   %ax,%ax

80106180 <syscall>:

};

void
syscall(void)
{
80106180:	f3 0f 1e fb          	endbr32 
80106184:	55                   	push   %ebp
80106185:	89 e5                	mov    %esp,%ebp
80106187:	53                   	push   %ebx
80106188:	83 ec 04             	sub    $0x4,%esp
  int num;
  struct proc *curproc = myproc();
8010618b:	e8 30 dd ff ff       	call   80103ec0 <myproc>
80106190:	89 c3                	mov    %eax,%ebx
  num = curproc->tf->eax;
80106192:	8b 40 18             	mov    0x18(%eax),%eax
80106195:	8b 40 1c             	mov    0x1c(%eax),%eax
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
80106198:	8d 50 ff             	lea    -0x1(%eax),%edx
8010619b:	83 fa 1d             	cmp    $0x1d,%edx
8010619e:	77 20                	ja     801061c0 <syscall+0x40>
801061a0:	8b 14 85 e0 94 10 80 	mov    -0x7fef6b20(,%eax,4),%edx
801061a7:	85 d2                	test   %edx,%edx
801061a9:	74 15                	je     801061c0 <syscall+0x40>
    curproc->tf->eax = syscalls[num]();
801061ab:	ff d2                	call   *%edx
801061ad:	89 c2                	mov    %eax,%edx
801061af:	8b 43 18             	mov    0x18(%ebx),%eax
801061b2:	89 50 1c             	mov    %edx,0x1c(%eax)
  } else {
    cprintf("%d %s: unknown sys call %d\n",
            curproc->pid, curproc->name, num);
    curproc->tf->eax = -1;
  }
}
801061b5:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801061b8:	c9                   	leave  
801061b9:	c3                   	ret    
801061ba:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    cprintf("%d %s: unknown sys call %d\n",
801061c0:	50                   	push   %eax
            curproc->pid, curproc->name, num);
801061c1:	8d 43 6c             	lea    0x6c(%ebx),%eax
    cprintf("%d %s: unknown sys call %d\n",
801061c4:	50                   	push   %eax
801061c5:	ff 73 10             	pushl  0x10(%ebx)
801061c8:	68 ab 94 10 80       	push   $0x801094ab
801061cd:	e8 ee a6 ff ff       	call   801008c0 <cprintf>
    curproc->tf->eax = -1;
801061d2:	8b 43 18             	mov    0x18(%ebx),%eax
801061d5:	83 c4 10             	add    $0x10,%esp
801061d8:	c7 40 1c ff ff ff ff 	movl   $0xffffffff,0x1c(%eax)
}
801061df:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801061e2:	c9                   	leave  
801061e3:	c3                   	ret    
801061e4:	66 90                	xchg   %ax,%ax
801061e6:	66 90                	xchg   %ax,%ax
801061e8:	66 90                	xchg   %ax,%ax
801061ea:	66 90                	xchg   %ax,%ax
801061ec:	66 90                	xchg   %ax,%ax
801061ee:	66 90                	xchg   %ax,%ax

801061f0 <create>:
  return -1;
}

static struct inode*
create(char *path, short type, short major, short minor)
{
801061f0:	55                   	push   %ebp
801061f1:	89 e5                	mov    %esp,%ebp
801061f3:	57                   	push   %edi
801061f4:	56                   	push   %esi
  struct inode *ip, *dp;
  char name[DIRSIZ];

  if((dp = nameiparent(path, name)) == 0)
801061f5:	8d 7d da             	lea    -0x26(%ebp),%edi
{
801061f8:	53                   	push   %ebx
801061f9:	83 ec 34             	sub    $0x34,%esp
801061fc:	89 4d d0             	mov    %ecx,-0x30(%ebp)
801061ff:	8b 4d 08             	mov    0x8(%ebp),%ecx
  if((dp = nameiparent(path, name)) == 0)
80106202:	57                   	push   %edi
80106203:	50                   	push   %eax
{
80106204:	89 55 d4             	mov    %edx,-0x2c(%ebp)
80106207:	89 4d cc             	mov    %ecx,-0x34(%ebp)
  if((dp = nameiparent(path, name)) == 0)
8010620a:	e8 31 c3 ff ff       	call   80102540 <nameiparent>
8010620f:	83 c4 10             	add    $0x10,%esp
80106212:	85 c0                	test   %eax,%eax
80106214:	0f 84 46 01 00 00    	je     80106360 <create+0x170>
    return 0;
  ilock(dp);
8010621a:	83 ec 0c             	sub    $0xc,%esp
8010621d:	89 c3                	mov    %eax,%ebx
8010621f:	50                   	push   %eax
80106220:	e8 2b ba ff ff       	call   80101c50 <ilock>

  if((ip = dirlookup(dp, name, 0)) != 0){
80106225:	83 c4 0c             	add    $0xc,%esp
80106228:	6a 00                	push   $0x0
8010622a:	57                   	push   %edi
8010622b:	53                   	push   %ebx
8010622c:	e8 6f bf ff ff       	call   801021a0 <dirlookup>
80106231:	83 c4 10             	add    $0x10,%esp
80106234:	89 c6                	mov    %eax,%esi
80106236:	85 c0                	test   %eax,%eax
80106238:	74 56                	je     80106290 <create+0xa0>
    iunlockput(dp);
8010623a:	83 ec 0c             	sub    $0xc,%esp
8010623d:	53                   	push   %ebx
8010623e:	e8 ad bc ff ff       	call   80101ef0 <iunlockput>
    ilock(ip);
80106243:	89 34 24             	mov    %esi,(%esp)
80106246:	e8 05 ba ff ff       	call   80101c50 <ilock>
    if(type == T_FILE && ip->type == T_FILE)
8010624b:	83 c4 10             	add    $0x10,%esp
8010624e:	66 83 7d d4 02       	cmpw   $0x2,-0x2c(%ebp)
80106253:	75 1b                	jne    80106270 <create+0x80>
80106255:	66 83 7e 50 02       	cmpw   $0x2,0x50(%esi)
8010625a:	75 14                	jne    80106270 <create+0x80>
    panic("create: dirlink");

  iunlockput(dp);

  return ip;
}
8010625c:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010625f:	89 f0                	mov    %esi,%eax
80106261:	5b                   	pop    %ebx
80106262:	5e                   	pop    %esi
80106263:	5f                   	pop    %edi
80106264:	5d                   	pop    %ebp
80106265:	c3                   	ret    
80106266:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010626d:	8d 76 00             	lea    0x0(%esi),%esi
    iunlockput(ip);
80106270:	83 ec 0c             	sub    $0xc,%esp
80106273:	56                   	push   %esi
    return 0;
80106274:	31 f6                	xor    %esi,%esi
    iunlockput(ip);
80106276:	e8 75 bc ff ff       	call   80101ef0 <iunlockput>
    return 0;
8010627b:	83 c4 10             	add    $0x10,%esp
}
8010627e:	8d 65 f4             	lea    -0xc(%ebp),%esp
80106281:	89 f0                	mov    %esi,%eax
80106283:	5b                   	pop    %ebx
80106284:	5e                   	pop    %esi
80106285:	5f                   	pop    %edi
80106286:	5d                   	pop    %ebp
80106287:	c3                   	ret    
80106288:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010628f:	90                   	nop
  if((ip = ialloc(dp->dev, type)) == 0)
80106290:	0f bf 45 d4          	movswl -0x2c(%ebp),%eax
80106294:	83 ec 08             	sub    $0x8,%esp
80106297:	50                   	push   %eax
80106298:	ff 33                	pushl  (%ebx)
8010629a:	e8 31 b8 ff ff       	call   80101ad0 <ialloc>
8010629f:	83 c4 10             	add    $0x10,%esp
801062a2:	89 c6                	mov    %eax,%esi
801062a4:	85 c0                	test   %eax,%eax
801062a6:	0f 84 cd 00 00 00    	je     80106379 <create+0x189>
  ilock(ip);
801062ac:	83 ec 0c             	sub    $0xc,%esp
801062af:	50                   	push   %eax
801062b0:	e8 9b b9 ff ff       	call   80101c50 <ilock>
  ip->major = major;
801062b5:	0f b7 45 d0          	movzwl -0x30(%ebp),%eax
801062b9:	66 89 46 52          	mov    %ax,0x52(%esi)
  ip->minor = minor;
801062bd:	0f b7 45 cc          	movzwl -0x34(%ebp),%eax
801062c1:	66 89 46 54          	mov    %ax,0x54(%esi)
  ip->nlink = 1;
801062c5:	b8 01 00 00 00       	mov    $0x1,%eax
801062ca:	66 89 46 56          	mov    %ax,0x56(%esi)
  iupdate(ip);
801062ce:	89 34 24             	mov    %esi,(%esp)
801062d1:	e8 ba b8 ff ff       	call   80101b90 <iupdate>
  if(type == T_DIR){  // Create . and .. entries.
801062d6:	83 c4 10             	add    $0x10,%esp
801062d9:	66 83 7d d4 01       	cmpw   $0x1,-0x2c(%ebp)
801062de:	74 30                	je     80106310 <create+0x120>
  if(dirlink(dp, name, ip->inum) < 0)
801062e0:	83 ec 04             	sub    $0x4,%esp
801062e3:	ff 76 04             	pushl  0x4(%esi)
801062e6:	57                   	push   %edi
801062e7:	53                   	push   %ebx
801062e8:	e8 73 c1 ff ff       	call   80102460 <dirlink>
801062ed:	83 c4 10             	add    $0x10,%esp
801062f0:	85 c0                	test   %eax,%eax
801062f2:	78 78                	js     8010636c <create+0x17c>
  iunlockput(dp);
801062f4:	83 ec 0c             	sub    $0xc,%esp
801062f7:	53                   	push   %ebx
801062f8:	e8 f3 bb ff ff       	call   80101ef0 <iunlockput>
  return ip;
801062fd:	83 c4 10             	add    $0x10,%esp
}
80106300:	8d 65 f4             	lea    -0xc(%ebp),%esp
80106303:	89 f0                	mov    %esi,%eax
80106305:	5b                   	pop    %ebx
80106306:	5e                   	pop    %esi
80106307:	5f                   	pop    %edi
80106308:	5d                   	pop    %ebp
80106309:	c3                   	ret    
8010630a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    iupdate(dp);
80106310:	83 ec 0c             	sub    $0xc,%esp
    dp->nlink++;  // for ".."
80106313:	66 83 43 56 01       	addw   $0x1,0x56(%ebx)
    iupdate(dp);
80106318:	53                   	push   %ebx
80106319:	e8 72 b8 ff ff       	call   80101b90 <iupdate>
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
8010631e:	83 c4 0c             	add    $0xc,%esp
80106321:	ff 76 04             	pushl  0x4(%esi)
80106324:	68 78 95 10 80       	push   $0x80109578
80106329:	56                   	push   %esi
8010632a:	e8 31 c1 ff ff       	call   80102460 <dirlink>
8010632f:	83 c4 10             	add    $0x10,%esp
80106332:	85 c0                	test   %eax,%eax
80106334:	78 18                	js     8010634e <create+0x15e>
80106336:	83 ec 04             	sub    $0x4,%esp
80106339:	ff 73 04             	pushl  0x4(%ebx)
8010633c:	68 77 95 10 80       	push   $0x80109577
80106341:	56                   	push   %esi
80106342:	e8 19 c1 ff ff       	call   80102460 <dirlink>
80106347:	83 c4 10             	add    $0x10,%esp
8010634a:	85 c0                	test   %eax,%eax
8010634c:	79 92                	jns    801062e0 <create+0xf0>
      panic("create dots");
8010634e:	83 ec 0c             	sub    $0xc,%esp
80106351:	68 6b 95 10 80       	push   $0x8010956b
80106356:	e8 35 a0 ff ff       	call   80100390 <panic>
8010635b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010635f:	90                   	nop
}
80106360:	8d 65 f4             	lea    -0xc(%ebp),%esp
    return 0;
80106363:	31 f6                	xor    %esi,%esi
}
80106365:	5b                   	pop    %ebx
80106366:	89 f0                	mov    %esi,%eax
80106368:	5e                   	pop    %esi
80106369:	5f                   	pop    %edi
8010636a:	5d                   	pop    %ebp
8010636b:	c3                   	ret    
    panic("create: dirlink");
8010636c:	83 ec 0c             	sub    $0xc,%esp
8010636f:	68 7a 95 10 80       	push   $0x8010957a
80106374:	e8 17 a0 ff ff       	call   80100390 <panic>
    panic("create: ialloc");
80106379:	83 ec 0c             	sub    $0xc,%esp
8010637c:	68 5c 95 10 80       	push   $0x8010955c
80106381:	e8 0a a0 ff ff       	call   80100390 <panic>
80106386:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010638d:	8d 76 00             	lea    0x0(%esi),%esi

80106390 <argfd.constprop.0>:
argfd(int n, int *pfd, struct file **pf)
80106390:	55                   	push   %ebp
80106391:	89 e5                	mov    %esp,%ebp
80106393:	56                   	push   %esi
80106394:	89 d6                	mov    %edx,%esi
80106396:	53                   	push   %ebx
80106397:	89 c3                	mov    %eax,%ebx
  if(argint(n, &fd) < 0)
80106399:	8d 45 f4             	lea    -0xc(%ebp),%eax
argfd(int n, int *pfd, struct file **pf)
8010639c:	83 ec 18             	sub    $0x18,%esp
  if(argint(n, &fd) < 0)
8010639f:	50                   	push   %eax
801063a0:	6a 00                	push   $0x0
801063a2:	e8 e9 fc ff ff       	call   80106090 <argint>
801063a7:	83 c4 10             	add    $0x10,%esp
801063aa:	85 c0                	test   %eax,%eax
801063ac:	78 2a                	js     801063d8 <argfd.constprop.0+0x48>
  if(fd < 0 || fd >= NOFILE || (f=myproc()->ofile[fd]) == 0)
801063ae:	83 7d f4 0f          	cmpl   $0xf,-0xc(%ebp)
801063b2:	77 24                	ja     801063d8 <argfd.constprop.0+0x48>
801063b4:	e8 07 db ff ff       	call   80103ec0 <myproc>
801063b9:	8b 55 f4             	mov    -0xc(%ebp),%edx
801063bc:	8b 44 90 28          	mov    0x28(%eax,%edx,4),%eax
801063c0:	85 c0                	test   %eax,%eax
801063c2:	74 14                	je     801063d8 <argfd.constprop.0+0x48>
  if(pfd)
801063c4:	85 db                	test   %ebx,%ebx
801063c6:	74 02                	je     801063ca <argfd.constprop.0+0x3a>
    *pfd = fd;
801063c8:	89 13                	mov    %edx,(%ebx)
    *pf = f;
801063ca:	89 06                	mov    %eax,(%esi)
  return 0;
801063cc:	31 c0                	xor    %eax,%eax
}
801063ce:	8d 65 f8             	lea    -0x8(%ebp),%esp
801063d1:	5b                   	pop    %ebx
801063d2:	5e                   	pop    %esi
801063d3:	5d                   	pop    %ebp
801063d4:	c3                   	ret    
801063d5:	8d 76 00             	lea    0x0(%esi),%esi
    return -1;
801063d8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801063dd:	eb ef                	jmp    801063ce <argfd.constprop.0+0x3e>
801063df:	90                   	nop

801063e0 <sys_dup>:
{
801063e0:	f3 0f 1e fb          	endbr32 
801063e4:	55                   	push   %ebp
  if(argfd(0, 0, &f) < 0)
801063e5:	31 c0                	xor    %eax,%eax
{
801063e7:	89 e5                	mov    %esp,%ebp
801063e9:	56                   	push   %esi
801063ea:	53                   	push   %ebx
  if(argfd(0, 0, &f) < 0)
801063eb:	8d 55 f4             	lea    -0xc(%ebp),%edx
{
801063ee:	83 ec 10             	sub    $0x10,%esp
  if(argfd(0, 0, &f) < 0)
801063f1:	e8 9a ff ff ff       	call   80106390 <argfd.constprop.0>
801063f6:	85 c0                	test   %eax,%eax
801063f8:	78 1e                	js     80106418 <sys_dup+0x38>
  if((fd=fdalloc(f)) < 0)
801063fa:	8b 75 f4             	mov    -0xc(%ebp),%esi
  for(fd = 0; fd < NOFILE; fd++){
801063fd:	31 db                	xor    %ebx,%ebx
  struct proc *curproc = myproc();
801063ff:	e8 bc da ff ff       	call   80103ec0 <myproc>
  for(fd = 0; fd < NOFILE; fd++){
80106404:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    if(curproc->ofile[fd] == 0){
80106408:	8b 54 98 28          	mov    0x28(%eax,%ebx,4),%edx
8010640c:	85 d2                	test   %edx,%edx
8010640e:	74 20                	je     80106430 <sys_dup+0x50>
  for(fd = 0; fd < NOFILE; fd++){
80106410:	83 c3 01             	add    $0x1,%ebx
80106413:	83 fb 10             	cmp    $0x10,%ebx
80106416:	75 f0                	jne    80106408 <sys_dup+0x28>
}
80106418:	8d 65 f8             	lea    -0x8(%ebp),%esp
    return -1;
8010641b:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
}
80106420:	89 d8                	mov    %ebx,%eax
80106422:	5b                   	pop    %ebx
80106423:	5e                   	pop    %esi
80106424:	5d                   	pop    %ebp
80106425:	c3                   	ret    
80106426:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010642d:	8d 76 00             	lea    0x0(%esi),%esi
      curproc->ofile[fd] = f;
80106430:	89 74 98 28          	mov    %esi,0x28(%eax,%ebx,4)
  filedup(f);
80106434:	83 ec 0c             	sub    $0xc,%esp
80106437:	ff 75 f4             	pushl  -0xc(%ebp)
8010643a:	e8 21 af ff ff       	call   80101360 <filedup>
  return fd;
8010643f:	83 c4 10             	add    $0x10,%esp
}
80106442:	8d 65 f8             	lea    -0x8(%ebp),%esp
80106445:	89 d8                	mov    %ebx,%eax
80106447:	5b                   	pop    %ebx
80106448:	5e                   	pop    %esi
80106449:	5d                   	pop    %ebp
8010644a:	c3                   	ret    
8010644b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010644f:	90                   	nop

80106450 <sys_read>:
{
80106450:	f3 0f 1e fb          	endbr32 
80106454:	55                   	push   %ebp
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
80106455:	31 c0                	xor    %eax,%eax
{
80106457:	89 e5                	mov    %esp,%ebp
80106459:	83 ec 18             	sub    $0x18,%esp
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
8010645c:	8d 55 ec             	lea    -0x14(%ebp),%edx
8010645f:	e8 2c ff ff ff       	call   80106390 <argfd.constprop.0>
80106464:	85 c0                	test   %eax,%eax
80106466:	78 48                	js     801064b0 <sys_read+0x60>
80106468:	83 ec 08             	sub    $0x8,%esp
8010646b:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010646e:	50                   	push   %eax
8010646f:	6a 02                	push   $0x2
80106471:	e8 1a fc ff ff       	call   80106090 <argint>
80106476:	83 c4 10             	add    $0x10,%esp
80106479:	85 c0                	test   %eax,%eax
8010647b:	78 33                	js     801064b0 <sys_read+0x60>
8010647d:	83 ec 04             	sub    $0x4,%esp
80106480:	8d 45 f4             	lea    -0xc(%ebp),%eax
80106483:	ff 75 f0             	pushl  -0x10(%ebp)
80106486:	50                   	push   %eax
80106487:	6a 01                	push   $0x1
80106489:	e8 52 fc ff ff       	call   801060e0 <argptr>
8010648e:	83 c4 10             	add    $0x10,%esp
80106491:	85 c0                	test   %eax,%eax
80106493:	78 1b                	js     801064b0 <sys_read+0x60>
  return fileread(f, p, n);
80106495:	83 ec 04             	sub    $0x4,%esp
80106498:	ff 75 f0             	pushl  -0x10(%ebp)
8010649b:	ff 75 f4             	pushl  -0xc(%ebp)
8010649e:	ff 75 ec             	pushl  -0x14(%ebp)
801064a1:	e8 3a b0 ff ff       	call   801014e0 <fileread>
801064a6:	83 c4 10             	add    $0x10,%esp
}
801064a9:	c9                   	leave  
801064aa:	c3                   	ret    
801064ab:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801064af:	90                   	nop
801064b0:	c9                   	leave  
    return -1;
801064b1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801064b6:	c3                   	ret    
801064b7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801064be:	66 90                	xchg   %ax,%ax

801064c0 <sys_write>:
{
801064c0:	f3 0f 1e fb          	endbr32 
801064c4:	55                   	push   %ebp
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
801064c5:	31 c0                	xor    %eax,%eax
{
801064c7:	89 e5                	mov    %esp,%ebp
801064c9:	83 ec 18             	sub    $0x18,%esp
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
801064cc:	8d 55 ec             	lea    -0x14(%ebp),%edx
801064cf:	e8 bc fe ff ff       	call   80106390 <argfd.constprop.0>
801064d4:	85 c0                	test   %eax,%eax
801064d6:	78 48                	js     80106520 <sys_write+0x60>
801064d8:	83 ec 08             	sub    $0x8,%esp
801064db:	8d 45 f0             	lea    -0x10(%ebp),%eax
801064de:	50                   	push   %eax
801064df:	6a 02                	push   $0x2
801064e1:	e8 aa fb ff ff       	call   80106090 <argint>
801064e6:	83 c4 10             	add    $0x10,%esp
801064e9:	85 c0                	test   %eax,%eax
801064eb:	78 33                	js     80106520 <sys_write+0x60>
801064ed:	83 ec 04             	sub    $0x4,%esp
801064f0:	8d 45 f4             	lea    -0xc(%ebp),%eax
801064f3:	ff 75 f0             	pushl  -0x10(%ebp)
801064f6:	50                   	push   %eax
801064f7:	6a 01                	push   $0x1
801064f9:	e8 e2 fb ff ff       	call   801060e0 <argptr>
801064fe:	83 c4 10             	add    $0x10,%esp
80106501:	85 c0                	test   %eax,%eax
80106503:	78 1b                	js     80106520 <sys_write+0x60>
  return filewrite(f, p, n);
80106505:	83 ec 04             	sub    $0x4,%esp
80106508:	ff 75 f0             	pushl  -0x10(%ebp)
8010650b:	ff 75 f4             	pushl  -0xc(%ebp)
8010650e:	ff 75 ec             	pushl  -0x14(%ebp)
80106511:	e8 6a b0 ff ff       	call   80101580 <filewrite>
80106516:	83 c4 10             	add    $0x10,%esp
}
80106519:	c9                   	leave  
8010651a:	c3                   	ret    
8010651b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010651f:	90                   	nop
80106520:	c9                   	leave  
    return -1;
80106521:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106526:	c3                   	ret    
80106527:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010652e:	66 90                	xchg   %ax,%ax

80106530 <sys_close>:
{
80106530:	f3 0f 1e fb          	endbr32 
80106534:	55                   	push   %ebp
80106535:	89 e5                	mov    %esp,%ebp
80106537:	83 ec 18             	sub    $0x18,%esp
  if(argfd(0, &fd, &f) < 0)
8010653a:	8d 55 f4             	lea    -0xc(%ebp),%edx
8010653d:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106540:	e8 4b fe ff ff       	call   80106390 <argfd.constprop.0>
80106545:	85 c0                	test   %eax,%eax
80106547:	78 27                	js     80106570 <sys_close+0x40>
  myproc()->ofile[fd] = 0;
80106549:	e8 72 d9 ff ff       	call   80103ec0 <myproc>
8010654e:	8b 55 f0             	mov    -0x10(%ebp),%edx
  fileclose(f);
80106551:	83 ec 0c             	sub    $0xc,%esp
  myproc()->ofile[fd] = 0;
80106554:	c7 44 90 28 00 00 00 	movl   $0x0,0x28(%eax,%edx,4)
8010655b:	00 
  fileclose(f);
8010655c:	ff 75 f4             	pushl  -0xc(%ebp)
8010655f:	e8 4c ae ff ff       	call   801013b0 <fileclose>
  return 0;
80106564:	83 c4 10             	add    $0x10,%esp
80106567:	31 c0                	xor    %eax,%eax
}
80106569:	c9                   	leave  
8010656a:	c3                   	ret    
8010656b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010656f:	90                   	nop
80106570:	c9                   	leave  
    return -1;
80106571:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106576:	c3                   	ret    
80106577:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010657e:	66 90                	xchg   %ax,%ax

80106580 <sys_fstat>:
{
80106580:	f3 0f 1e fb          	endbr32 
80106584:	55                   	push   %ebp
  if(argfd(0, 0, &f) < 0 || argptr(1, (void*)&st, sizeof(*st)) < 0)
80106585:	31 c0                	xor    %eax,%eax
{
80106587:	89 e5                	mov    %esp,%ebp
80106589:	83 ec 18             	sub    $0x18,%esp
  if(argfd(0, 0, &f) < 0 || argptr(1, (void*)&st, sizeof(*st)) < 0)
8010658c:	8d 55 f0             	lea    -0x10(%ebp),%edx
8010658f:	e8 fc fd ff ff       	call   80106390 <argfd.constprop.0>
80106594:	85 c0                	test   %eax,%eax
80106596:	78 30                	js     801065c8 <sys_fstat+0x48>
80106598:	83 ec 04             	sub    $0x4,%esp
8010659b:	8d 45 f4             	lea    -0xc(%ebp),%eax
8010659e:	6a 14                	push   $0x14
801065a0:	50                   	push   %eax
801065a1:	6a 01                	push   $0x1
801065a3:	e8 38 fb ff ff       	call   801060e0 <argptr>
801065a8:	83 c4 10             	add    $0x10,%esp
801065ab:	85 c0                	test   %eax,%eax
801065ad:	78 19                	js     801065c8 <sys_fstat+0x48>
  return filestat(f, st);
801065af:	83 ec 08             	sub    $0x8,%esp
801065b2:	ff 75 f4             	pushl  -0xc(%ebp)
801065b5:	ff 75 f0             	pushl  -0x10(%ebp)
801065b8:	e8 d3 ae ff ff       	call   80101490 <filestat>
801065bd:	83 c4 10             	add    $0x10,%esp
}
801065c0:	c9                   	leave  
801065c1:	c3                   	ret    
801065c2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801065c8:	c9                   	leave  
    return -1;
801065c9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801065ce:	c3                   	ret    
801065cf:	90                   	nop

801065d0 <sys_link>:
{
801065d0:	f3 0f 1e fb          	endbr32 
801065d4:	55                   	push   %ebp
801065d5:	89 e5                	mov    %esp,%ebp
801065d7:	57                   	push   %edi
801065d8:	56                   	push   %esi
  if(argstr(0, &old) < 0 || argstr(1, &new) < 0)
801065d9:	8d 45 d4             	lea    -0x2c(%ebp),%eax
{
801065dc:	53                   	push   %ebx
801065dd:	83 ec 34             	sub    $0x34,%esp
  if(argstr(0, &old) < 0 || argstr(1, &new) < 0)
801065e0:	50                   	push   %eax
801065e1:	6a 00                	push   $0x0
801065e3:	e8 58 fb ff ff       	call   80106140 <argstr>
801065e8:	83 c4 10             	add    $0x10,%esp
801065eb:	85 c0                	test   %eax,%eax
801065ed:	0f 88 ff 00 00 00    	js     801066f2 <sys_link+0x122>
801065f3:	83 ec 08             	sub    $0x8,%esp
801065f6:	8d 45 d0             	lea    -0x30(%ebp),%eax
801065f9:	50                   	push   %eax
801065fa:	6a 01                	push   $0x1
801065fc:	e8 3f fb ff ff       	call   80106140 <argstr>
80106601:	83 c4 10             	add    $0x10,%esp
80106604:	85 c0                	test   %eax,%eax
80106606:	0f 88 e6 00 00 00    	js     801066f2 <sys_link+0x122>
  begin_op();
8010660c:	e8 0f cc ff ff       	call   80103220 <begin_op>
  if((ip = namei(old)) == 0){
80106611:	83 ec 0c             	sub    $0xc,%esp
80106614:	ff 75 d4             	pushl  -0x2c(%ebp)
80106617:	e8 04 bf ff ff       	call   80102520 <namei>
8010661c:	83 c4 10             	add    $0x10,%esp
8010661f:	89 c3                	mov    %eax,%ebx
80106621:	85 c0                	test   %eax,%eax
80106623:	0f 84 e8 00 00 00    	je     80106711 <sys_link+0x141>
  ilock(ip);
80106629:	83 ec 0c             	sub    $0xc,%esp
8010662c:	50                   	push   %eax
8010662d:	e8 1e b6 ff ff       	call   80101c50 <ilock>
  if(ip->type == T_DIR){
80106632:	83 c4 10             	add    $0x10,%esp
80106635:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
8010663a:	0f 84 b9 00 00 00    	je     801066f9 <sys_link+0x129>
  iupdate(ip);
80106640:	83 ec 0c             	sub    $0xc,%esp
  ip->nlink++;
80106643:	66 83 43 56 01       	addw   $0x1,0x56(%ebx)
  if((dp = nameiparent(new, name)) == 0)
80106648:	8d 7d da             	lea    -0x26(%ebp),%edi
  iupdate(ip);
8010664b:	53                   	push   %ebx
8010664c:	e8 3f b5 ff ff       	call   80101b90 <iupdate>
  iunlock(ip);
80106651:	89 1c 24             	mov    %ebx,(%esp)
80106654:	e8 d7 b6 ff ff       	call   80101d30 <iunlock>
  if((dp = nameiparent(new, name)) == 0)
80106659:	58                   	pop    %eax
8010665a:	5a                   	pop    %edx
8010665b:	57                   	push   %edi
8010665c:	ff 75 d0             	pushl  -0x30(%ebp)
8010665f:	e8 dc be ff ff       	call   80102540 <nameiparent>
80106664:	83 c4 10             	add    $0x10,%esp
80106667:	89 c6                	mov    %eax,%esi
80106669:	85 c0                	test   %eax,%eax
8010666b:	74 5f                	je     801066cc <sys_link+0xfc>
  ilock(dp);
8010666d:	83 ec 0c             	sub    $0xc,%esp
80106670:	50                   	push   %eax
80106671:	e8 da b5 ff ff       	call   80101c50 <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
80106676:	8b 03                	mov    (%ebx),%eax
80106678:	83 c4 10             	add    $0x10,%esp
8010667b:	39 06                	cmp    %eax,(%esi)
8010667d:	75 41                	jne    801066c0 <sys_link+0xf0>
8010667f:	83 ec 04             	sub    $0x4,%esp
80106682:	ff 73 04             	pushl  0x4(%ebx)
80106685:	57                   	push   %edi
80106686:	56                   	push   %esi
80106687:	e8 d4 bd ff ff       	call   80102460 <dirlink>
8010668c:	83 c4 10             	add    $0x10,%esp
8010668f:	85 c0                	test   %eax,%eax
80106691:	78 2d                	js     801066c0 <sys_link+0xf0>
  iunlockput(dp);
80106693:	83 ec 0c             	sub    $0xc,%esp
80106696:	56                   	push   %esi
80106697:	e8 54 b8 ff ff       	call   80101ef0 <iunlockput>
  iput(ip);
8010669c:	89 1c 24             	mov    %ebx,(%esp)
8010669f:	e8 dc b6 ff ff       	call   80101d80 <iput>
  end_op();
801066a4:	e8 e7 cb ff ff       	call   80103290 <end_op>
  return 0;
801066a9:	83 c4 10             	add    $0x10,%esp
801066ac:	31 c0                	xor    %eax,%eax
}
801066ae:	8d 65 f4             	lea    -0xc(%ebp),%esp
801066b1:	5b                   	pop    %ebx
801066b2:	5e                   	pop    %esi
801066b3:	5f                   	pop    %edi
801066b4:	5d                   	pop    %ebp
801066b5:	c3                   	ret    
801066b6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801066bd:	8d 76 00             	lea    0x0(%esi),%esi
    iunlockput(dp);
801066c0:	83 ec 0c             	sub    $0xc,%esp
801066c3:	56                   	push   %esi
801066c4:	e8 27 b8 ff ff       	call   80101ef0 <iunlockput>
    goto bad;
801066c9:	83 c4 10             	add    $0x10,%esp
  ilock(ip);
801066cc:	83 ec 0c             	sub    $0xc,%esp
801066cf:	53                   	push   %ebx
801066d0:	e8 7b b5 ff ff       	call   80101c50 <ilock>
  ip->nlink--;
801066d5:	66 83 6b 56 01       	subw   $0x1,0x56(%ebx)
  iupdate(ip);
801066da:	89 1c 24             	mov    %ebx,(%esp)
801066dd:	e8 ae b4 ff ff       	call   80101b90 <iupdate>
  iunlockput(ip);
801066e2:	89 1c 24             	mov    %ebx,(%esp)
801066e5:	e8 06 b8 ff ff       	call   80101ef0 <iunlockput>
  end_op();
801066ea:	e8 a1 cb ff ff       	call   80103290 <end_op>
  return -1;
801066ef:	83 c4 10             	add    $0x10,%esp
801066f2:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801066f7:	eb b5                	jmp    801066ae <sys_link+0xde>
    iunlockput(ip);
801066f9:	83 ec 0c             	sub    $0xc,%esp
801066fc:	53                   	push   %ebx
801066fd:	e8 ee b7 ff ff       	call   80101ef0 <iunlockput>
    end_op();
80106702:	e8 89 cb ff ff       	call   80103290 <end_op>
    return -1;
80106707:	83 c4 10             	add    $0x10,%esp
8010670a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010670f:	eb 9d                	jmp    801066ae <sys_link+0xde>
    end_op();
80106711:	e8 7a cb ff ff       	call   80103290 <end_op>
    return -1;
80106716:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010671b:	eb 91                	jmp    801066ae <sys_link+0xde>
8010671d:	8d 76 00             	lea    0x0(%esi),%esi

80106720 <sys_unlink>:
{
80106720:	f3 0f 1e fb          	endbr32 
80106724:	55                   	push   %ebp
80106725:	89 e5                	mov    %esp,%ebp
80106727:	57                   	push   %edi
80106728:	56                   	push   %esi
  if(argstr(0, &path) < 0)
80106729:	8d 45 c0             	lea    -0x40(%ebp),%eax
{
8010672c:	53                   	push   %ebx
8010672d:	83 ec 54             	sub    $0x54,%esp
  if(argstr(0, &path) < 0)
80106730:	50                   	push   %eax
80106731:	6a 00                	push   $0x0
80106733:	e8 08 fa ff ff       	call   80106140 <argstr>
80106738:	83 c4 10             	add    $0x10,%esp
8010673b:	85 c0                	test   %eax,%eax
8010673d:	0f 88 7d 01 00 00    	js     801068c0 <sys_unlink+0x1a0>
  begin_op();
80106743:	e8 d8 ca ff ff       	call   80103220 <begin_op>
  if((dp = nameiparent(path, name)) == 0){
80106748:	8d 5d ca             	lea    -0x36(%ebp),%ebx
8010674b:	83 ec 08             	sub    $0x8,%esp
8010674e:	53                   	push   %ebx
8010674f:	ff 75 c0             	pushl  -0x40(%ebp)
80106752:	e8 e9 bd ff ff       	call   80102540 <nameiparent>
80106757:	83 c4 10             	add    $0x10,%esp
8010675a:	89 c6                	mov    %eax,%esi
8010675c:	85 c0                	test   %eax,%eax
8010675e:	0f 84 66 01 00 00    	je     801068ca <sys_unlink+0x1aa>
  ilock(dp);
80106764:	83 ec 0c             	sub    $0xc,%esp
80106767:	50                   	push   %eax
80106768:	e8 e3 b4 ff ff       	call   80101c50 <ilock>
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
8010676d:	58                   	pop    %eax
8010676e:	5a                   	pop    %edx
8010676f:	68 78 95 10 80       	push   $0x80109578
80106774:	53                   	push   %ebx
80106775:	e8 06 ba ff ff       	call   80102180 <namecmp>
8010677a:	83 c4 10             	add    $0x10,%esp
8010677d:	85 c0                	test   %eax,%eax
8010677f:	0f 84 03 01 00 00    	je     80106888 <sys_unlink+0x168>
80106785:	83 ec 08             	sub    $0x8,%esp
80106788:	68 77 95 10 80       	push   $0x80109577
8010678d:	53                   	push   %ebx
8010678e:	e8 ed b9 ff ff       	call   80102180 <namecmp>
80106793:	83 c4 10             	add    $0x10,%esp
80106796:	85 c0                	test   %eax,%eax
80106798:	0f 84 ea 00 00 00    	je     80106888 <sys_unlink+0x168>
  if((ip = dirlookup(dp, name, &off)) == 0)
8010679e:	83 ec 04             	sub    $0x4,%esp
801067a1:	8d 45 c4             	lea    -0x3c(%ebp),%eax
801067a4:	50                   	push   %eax
801067a5:	53                   	push   %ebx
801067a6:	56                   	push   %esi
801067a7:	e8 f4 b9 ff ff       	call   801021a0 <dirlookup>
801067ac:	83 c4 10             	add    $0x10,%esp
801067af:	89 c3                	mov    %eax,%ebx
801067b1:	85 c0                	test   %eax,%eax
801067b3:	0f 84 cf 00 00 00    	je     80106888 <sys_unlink+0x168>
  ilock(ip);
801067b9:	83 ec 0c             	sub    $0xc,%esp
801067bc:	50                   	push   %eax
801067bd:	e8 8e b4 ff ff       	call   80101c50 <ilock>
  if(ip->nlink < 1)
801067c2:	83 c4 10             	add    $0x10,%esp
801067c5:	66 83 7b 56 00       	cmpw   $0x0,0x56(%ebx)
801067ca:	0f 8e 23 01 00 00    	jle    801068f3 <sys_unlink+0x1d3>
  if(ip->type == T_DIR && !isdirempty(ip)){
801067d0:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
801067d5:	8d 7d d8             	lea    -0x28(%ebp),%edi
801067d8:	74 66                	je     80106840 <sys_unlink+0x120>
  memset(&de, 0, sizeof(de));
801067da:	83 ec 04             	sub    $0x4,%esp
801067dd:	6a 10                	push   $0x10
801067df:	6a 00                	push   $0x0
801067e1:	57                   	push   %edi
801067e2:	e8 39 f5 ff ff       	call   80105d20 <memset>
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801067e7:	6a 10                	push   $0x10
801067e9:	ff 75 c4             	pushl  -0x3c(%ebp)
801067ec:	57                   	push   %edi
801067ed:	56                   	push   %esi
801067ee:	e8 5d b8 ff ff       	call   80102050 <writei>
801067f3:	83 c4 20             	add    $0x20,%esp
801067f6:	83 f8 10             	cmp    $0x10,%eax
801067f9:	0f 85 e7 00 00 00    	jne    801068e6 <sys_unlink+0x1c6>
  if(ip->type == T_DIR){
801067ff:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
80106804:	0f 84 96 00 00 00    	je     801068a0 <sys_unlink+0x180>
  iunlockput(dp);
8010680a:	83 ec 0c             	sub    $0xc,%esp
8010680d:	56                   	push   %esi
8010680e:	e8 dd b6 ff ff       	call   80101ef0 <iunlockput>
  ip->nlink--;
80106813:	66 83 6b 56 01       	subw   $0x1,0x56(%ebx)
  iupdate(ip);
80106818:	89 1c 24             	mov    %ebx,(%esp)
8010681b:	e8 70 b3 ff ff       	call   80101b90 <iupdate>
  iunlockput(ip);
80106820:	89 1c 24             	mov    %ebx,(%esp)
80106823:	e8 c8 b6 ff ff       	call   80101ef0 <iunlockput>
  end_op();
80106828:	e8 63 ca ff ff       	call   80103290 <end_op>
  return 0;
8010682d:	83 c4 10             	add    $0x10,%esp
80106830:	31 c0                	xor    %eax,%eax
}
80106832:	8d 65 f4             	lea    -0xc(%ebp),%esp
80106835:	5b                   	pop    %ebx
80106836:	5e                   	pop    %esi
80106837:	5f                   	pop    %edi
80106838:	5d                   	pop    %ebp
80106839:	c3                   	ret    
8010683a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
80106840:	83 7b 58 20          	cmpl   $0x20,0x58(%ebx)
80106844:	76 94                	jbe    801067da <sys_unlink+0xba>
80106846:	ba 20 00 00 00       	mov    $0x20,%edx
8010684b:	eb 0b                	jmp    80106858 <sys_unlink+0x138>
8010684d:	8d 76 00             	lea    0x0(%esi),%esi
80106850:	83 c2 10             	add    $0x10,%edx
80106853:	39 53 58             	cmp    %edx,0x58(%ebx)
80106856:	76 82                	jbe    801067da <sys_unlink+0xba>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80106858:	6a 10                	push   $0x10
8010685a:	52                   	push   %edx
8010685b:	57                   	push   %edi
8010685c:	53                   	push   %ebx
8010685d:	89 55 b4             	mov    %edx,-0x4c(%ebp)
80106860:	e8 eb b6 ff ff       	call   80101f50 <readi>
80106865:	83 c4 10             	add    $0x10,%esp
80106868:	8b 55 b4             	mov    -0x4c(%ebp),%edx
8010686b:	83 f8 10             	cmp    $0x10,%eax
8010686e:	75 69                	jne    801068d9 <sys_unlink+0x1b9>
    if(de.inum != 0)
80106870:	66 83 7d d8 00       	cmpw   $0x0,-0x28(%ebp)
80106875:	74 d9                	je     80106850 <sys_unlink+0x130>
    iunlockput(ip);
80106877:	83 ec 0c             	sub    $0xc,%esp
8010687a:	53                   	push   %ebx
8010687b:	e8 70 b6 ff ff       	call   80101ef0 <iunlockput>
    goto bad;
80106880:	83 c4 10             	add    $0x10,%esp
80106883:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80106887:	90                   	nop
  iunlockput(dp);
80106888:	83 ec 0c             	sub    $0xc,%esp
8010688b:	56                   	push   %esi
8010688c:	e8 5f b6 ff ff       	call   80101ef0 <iunlockput>
  end_op();
80106891:	e8 fa c9 ff ff       	call   80103290 <end_op>
  return -1;
80106896:	83 c4 10             	add    $0x10,%esp
80106899:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010689e:	eb 92                	jmp    80106832 <sys_unlink+0x112>
    iupdate(dp);
801068a0:	83 ec 0c             	sub    $0xc,%esp
    dp->nlink--;
801068a3:	66 83 6e 56 01       	subw   $0x1,0x56(%esi)
    iupdate(dp);
801068a8:	56                   	push   %esi
801068a9:	e8 e2 b2 ff ff       	call   80101b90 <iupdate>
801068ae:	83 c4 10             	add    $0x10,%esp
801068b1:	e9 54 ff ff ff       	jmp    8010680a <sys_unlink+0xea>
801068b6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801068bd:	8d 76 00             	lea    0x0(%esi),%esi
    return -1;
801068c0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801068c5:	e9 68 ff ff ff       	jmp    80106832 <sys_unlink+0x112>
    end_op();
801068ca:	e8 c1 c9 ff ff       	call   80103290 <end_op>
    return -1;
801068cf:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801068d4:	e9 59 ff ff ff       	jmp    80106832 <sys_unlink+0x112>
      panic("isdirempty: readi");
801068d9:	83 ec 0c             	sub    $0xc,%esp
801068dc:	68 9c 95 10 80       	push   $0x8010959c
801068e1:	e8 aa 9a ff ff       	call   80100390 <panic>
    panic("unlink: writei");
801068e6:	83 ec 0c             	sub    $0xc,%esp
801068e9:	68 ae 95 10 80       	push   $0x801095ae
801068ee:	e8 9d 9a ff ff       	call   80100390 <panic>
    panic("unlink: nlink < 1");
801068f3:	83 ec 0c             	sub    $0xc,%esp
801068f6:	68 8a 95 10 80       	push   $0x8010958a
801068fb:	e8 90 9a ff ff       	call   80100390 <panic>

80106900 <sys_open>:

int
sys_open(void)
{
80106900:	f3 0f 1e fb          	endbr32 
80106904:	55                   	push   %ebp
80106905:	89 e5                	mov    %esp,%ebp
80106907:	57                   	push   %edi
80106908:	56                   	push   %esi
  char *path;
  int fd, omode;
  struct file *f;
  struct inode *ip;

  if(argstr(0, &path) < 0 || argint(1, &omode) < 0)
80106909:	8d 45 e0             	lea    -0x20(%ebp),%eax
{
8010690c:	53                   	push   %ebx
8010690d:	83 ec 24             	sub    $0x24,%esp
  if(argstr(0, &path) < 0 || argint(1, &omode) < 0)
80106910:	50                   	push   %eax
80106911:	6a 00                	push   $0x0
80106913:	e8 28 f8 ff ff       	call   80106140 <argstr>
80106918:	83 c4 10             	add    $0x10,%esp
8010691b:	85 c0                	test   %eax,%eax
8010691d:	0f 88 8a 00 00 00    	js     801069ad <sys_open+0xad>
80106923:	83 ec 08             	sub    $0x8,%esp
80106926:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80106929:	50                   	push   %eax
8010692a:	6a 01                	push   $0x1
8010692c:	e8 5f f7 ff ff       	call   80106090 <argint>
80106931:	83 c4 10             	add    $0x10,%esp
80106934:	85 c0                	test   %eax,%eax
80106936:	78 75                	js     801069ad <sys_open+0xad>
    return -1;

  begin_op();
80106938:	e8 e3 c8 ff ff       	call   80103220 <begin_op>

  if(omode & O_CREATE){
8010693d:	f6 45 e5 02          	testb  $0x2,-0x1b(%ebp)
80106941:	75 75                	jne    801069b8 <sys_open+0xb8>
    if(ip == 0){
      end_op();
      return -1;
    }
  } else {
    if((ip = namei(path)) == 0){
80106943:	83 ec 0c             	sub    $0xc,%esp
80106946:	ff 75 e0             	pushl  -0x20(%ebp)
80106949:	e8 d2 bb ff ff       	call   80102520 <namei>
8010694e:	83 c4 10             	add    $0x10,%esp
80106951:	89 c6                	mov    %eax,%esi
80106953:	85 c0                	test   %eax,%eax
80106955:	74 7e                	je     801069d5 <sys_open+0xd5>
      end_op();
      return -1;
    }
    ilock(ip);
80106957:	83 ec 0c             	sub    $0xc,%esp
8010695a:	50                   	push   %eax
8010695b:	e8 f0 b2 ff ff       	call   80101c50 <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
80106960:	83 c4 10             	add    $0x10,%esp
80106963:	66 83 7e 50 01       	cmpw   $0x1,0x50(%esi)
80106968:	0f 84 c2 00 00 00    	je     80106a30 <sys_open+0x130>
      end_op();
      return -1;
    }
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
8010696e:	e8 7d a9 ff ff       	call   801012f0 <filealloc>
80106973:	89 c7                	mov    %eax,%edi
80106975:	85 c0                	test   %eax,%eax
80106977:	74 23                	je     8010699c <sys_open+0x9c>
  struct proc *curproc = myproc();
80106979:	e8 42 d5 ff ff       	call   80103ec0 <myproc>
  for(fd = 0; fd < NOFILE; fd++){
8010697e:	31 db                	xor    %ebx,%ebx
    if(curproc->ofile[fd] == 0){
80106980:	8b 54 98 28          	mov    0x28(%eax,%ebx,4),%edx
80106984:	85 d2                	test   %edx,%edx
80106986:	74 60                	je     801069e8 <sys_open+0xe8>
  for(fd = 0; fd < NOFILE; fd++){
80106988:	83 c3 01             	add    $0x1,%ebx
8010698b:	83 fb 10             	cmp    $0x10,%ebx
8010698e:	75 f0                	jne    80106980 <sys_open+0x80>
    if(f)
      fileclose(f);
80106990:	83 ec 0c             	sub    $0xc,%esp
80106993:	57                   	push   %edi
80106994:	e8 17 aa ff ff       	call   801013b0 <fileclose>
80106999:	83 c4 10             	add    $0x10,%esp
    iunlockput(ip);
8010699c:	83 ec 0c             	sub    $0xc,%esp
8010699f:	56                   	push   %esi
801069a0:	e8 4b b5 ff ff       	call   80101ef0 <iunlockput>
    end_op();
801069a5:	e8 e6 c8 ff ff       	call   80103290 <end_op>
    return -1;
801069aa:	83 c4 10             	add    $0x10,%esp
801069ad:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
801069b2:	eb 6d                	jmp    80106a21 <sys_open+0x121>
801069b4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    ip = create(path, T_FILE, 0, 0);
801069b8:	83 ec 0c             	sub    $0xc,%esp
801069bb:	8b 45 e0             	mov    -0x20(%ebp),%eax
801069be:	31 c9                	xor    %ecx,%ecx
801069c0:	ba 02 00 00 00       	mov    $0x2,%edx
801069c5:	6a 00                	push   $0x0
801069c7:	e8 24 f8 ff ff       	call   801061f0 <create>
    if(ip == 0){
801069cc:	83 c4 10             	add    $0x10,%esp
    ip = create(path, T_FILE, 0, 0);
801069cf:	89 c6                	mov    %eax,%esi
    if(ip == 0){
801069d1:	85 c0                	test   %eax,%eax
801069d3:	75 99                	jne    8010696e <sys_open+0x6e>
      end_op();
801069d5:	e8 b6 c8 ff ff       	call   80103290 <end_op>
      return -1;
801069da:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
801069df:	eb 40                	jmp    80106a21 <sys_open+0x121>
801069e1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  }
  iunlock(ip);
801069e8:	83 ec 0c             	sub    $0xc,%esp
      curproc->ofile[fd] = f;
801069eb:	89 7c 98 28          	mov    %edi,0x28(%eax,%ebx,4)
  iunlock(ip);
801069ef:	56                   	push   %esi
801069f0:	e8 3b b3 ff ff       	call   80101d30 <iunlock>
  end_op();
801069f5:	e8 96 c8 ff ff       	call   80103290 <end_op>

  f->type = FD_INODE;
801069fa:	c7 07 02 00 00 00    	movl   $0x2,(%edi)
  f->ip = ip;
  f->off = 0;
  f->readable = !(omode & O_WRONLY);
80106a00:	8b 55 e4             	mov    -0x1c(%ebp),%edx
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
80106a03:	83 c4 10             	add    $0x10,%esp
  f->ip = ip;
80106a06:	89 77 10             	mov    %esi,0x10(%edi)
  f->readable = !(omode & O_WRONLY);
80106a09:	89 d0                	mov    %edx,%eax
  f->off = 0;
80106a0b:	c7 47 14 00 00 00 00 	movl   $0x0,0x14(%edi)
  f->readable = !(omode & O_WRONLY);
80106a12:	f7 d0                	not    %eax
80106a14:	83 e0 01             	and    $0x1,%eax
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
80106a17:	83 e2 03             	and    $0x3,%edx
  f->readable = !(omode & O_WRONLY);
80106a1a:	88 47 08             	mov    %al,0x8(%edi)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
80106a1d:	0f 95 47 09          	setne  0x9(%edi)
  return fd;
}
80106a21:	8d 65 f4             	lea    -0xc(%ebp),%esp
80106a24:	89 d8                	mov    %ebx,%eax
80106a26:	5b                   	pop    %ebx
80106a27:	5e                   	pop    %esi
80106a28:	5f                   	pop    %edi
80106a29:	5d                   	pop    %ebp
80106a2a:	c3                   	ret    
80106a2b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80106a2f:	90                   	nop
    if(ip->type == T_DIR && omode != O_RDONLY){
80106a30:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
80106a33:	85 c9                	test   %ecx,%ecx
80106a35:	0f 84 33 ff ff ff    	je     8010696e <sys_open+0x6e>
80106a3b:	e9 5c ff ff ff       	jmp    8010699c <sys_open+0x9c>

80106a40 <sys_mkdir>:

int
sys_mkdir(void)
{
80106a40:	f3 0f 1e fb          	endbr32 
80106a44:	55                   	push   %ebp
80106a45:	89 e5                	mov    %esp,%ebp
80106a47:	83 ec 18             	sub    $0x18,%esp
  char *path;
  struct inode *ip;

  begin_op();
80106a4a:	e8 d1 c7 ff ff       	call   80103220 <begin_op>
  if(argstr(0, &path) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
80106a4f:	83 ec 08             	sub    $0x8,%esp
80106a52:	8d 45 f4             	lea    -0xc(%ebp),%eax
80106a55:	50                   	push   %eax
80106a56:	6a 00                	push   $0x0
80106a58:	e8 e3 f6 ff ff       	call   80106140 <argstr>
80106a5d:	83 c4 10             	add    $0x10,%esp
80106a60:	85 c0                	test   %eax,%eax
80106a62:	78 34                	js     80106a98 <sys_mkdir+0x58>
80106a64:	83 ec 0c             	sub    $0xc,%esp
80106a67:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106a6a:	31 c9                	xor    %ecx,%ecx
80106a6c:	ba 01 00 00 00       	mov    $0x1,%edx
80106a71:	6a 00                	push   $0x0
80106a73:	e8 78 f7 ff ff       	call   801061f0 <create>
80106a78:	83 c4 10             	add    $0x10,%esp
80106a7b:	85 c0                	test   %eax,%eax
80106a7d:	74 19                	je     80106a98 <sys_mkdir+0x58>
    end_op();
    return -1;
  }
  iunlockput(ip);
80106a7f:	83 ec 0c             	sub    $0xc,%esp
80106a82:	50                   	push   %eax
80106a83:	e8 68 b4 ff ff       	call   80101ef0 <iunlockput>
  end_op();
80106a88:	e8 03 c8 ff ff       	call   80103290 <end_op>
  return 0;
80106a8d:	83 c4 10             	add    $0x10,%esp
80106a90:	31 c0                	xor    %eax,%eax
}
80106a92:	c9                   	leave  
80106a93:	c3                   	ret    
80106a94:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    end_op();
80106a98:	e8 f3 c7 ff ff       	call   80103290 <end_op>
    return -1;
80106a9d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106aa2:	c9                   	leave  
80106aa3:	c3                   	ret    
80106aa4:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106aab:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80106aaf:	90                   	nop

80106ab0 <sys_mknod>:

int
sys_mknod(void)
{
80106ab0:	f3 0f 1e fb          	endbr32 
80106ab4:	55                   	push   %ebp
80106ab5:	89 e5                	mov    %esp,%ebp
80106ab7:	83 ec 18             	sub    $0x18,%esp
  struct inode *ip;
  char *path;
  int major, minor;

  begin_op();
80106aba:	e8 61 c7 ff ff       	call   80103220 <begin_op>
  if((argstr(0, &path)) < 0 ||
80106abf:	83 ec 08             	sub    $0x8,%esp
80106ac2:	8d 45 ec             	lea    -0x14(%ebp),%eax
80106ac5:	50                   	push   %eax
80106ac6:	6a 00                	push   $0x0
80106ac8:	e8 73 f6 ff ff       	call   80106140 <argstr>
80106acd:	83 c4 10             	add    $0x10,%esp
80106ad0:	85 c0                	test   %eax,%eax
80106ad2:	78 64                	js     80106b38 <sys_mknod+0x88>
     argint(1, &major) < 0 ||
80106ad4:	83 ec 08             	sub    $0x8,%esp
80106ad7:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106ada:	50                   	push   %eax
80106adb:	6a 01                	push   $0x1
80106add:	e8 ae f5 ff ff       	call   80106090 <argint>
  if((argstr(0, &path)) < 0 ||
80106ae2:	83 c4 10             	add    $0x10,%esp
80106ae5:	85 c0                	test   %eax,%eax
80106ae7:	78 4f                	js     80106b38 <sys_mknod+0x88>
     argint(2, &minor) < 0 ||
80106ae9:	83 ec 08             	sub    $0x8,%esp
80106aec:	8d 45 f4             	lea    -0xc(%ebp),%eax
80106aef:	50                   	push   %eax
80106af0:	6a 02                	push   $0x2
80106af2:	e8 99 f5 ff ff       	call   80106090 <argint>
     argint(1, &major) < 0 ||
80106af7:	83 c4 10             	add    $0x10,%esp
80106afa:	85 c0                	test   %eax,%eax
80106afc:	78 3a                	js     80106b38 <sys_mknod+0x88>
     (ip = create(path, T_DEV, major, minor)) == 0){
80106afe:	0f bf 45 f4          	movswl -0xc(%ebp),%eax
80106b02:	83 ec 0c             	sub    $0xc,%esp
80106b05:	0f bf 4d f0          	movswl -0x10(%ebp),%ecx
80106b09:	ba 03 00 00 00       	mov    $0x3,%edx
80106b0e:	50                   	push   %eax
80106b0f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80106b12:	e8 d9 f6 ff ff       	call   801061f0 <create>
     argint(2, &minor) < 0 ||
80106b17:	83 c4 10             	add    $0x10,%esp
80106b1a:	85 c0                	test   %eax,%eax
80106b1c:	74 1a                	je     80106b38 <sys_mknod+0x88>
    end_op();
    return -1;
  }
  iunlockput(ip);
80106b1e:	83 ec 0c             	sub    $0xc,%esp
80106b21:	50                   	push   %eax
80106b22:	e8 c9 b3 ff ff       	call   80101ef0 <iunlockput>
  end_op();
80106b27:	e8 64 c7 ff ff       	call   80103290 <end_op>
  return 0;
80106b2c:	83 c4 10             	add    $0x10,%esp
80106b2f:	31 c0                	xor    %eax,%eax
}
80106b31:	c9                   	leave  
80106b32:	c3                   	ret    
80106b33:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80106b37:	90                   	nop
    end_op();
80106b38:	e8 53 c7 ff ff       	call   80103290 <end_op>
    return -1;
80106b3d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106b42:	c9                   	leave  
80106b43:	c3                   	ret    
80106b44:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106b4b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80106b4f:	90                   	nop

80106b50 <sys_chdir>:

int
sys_chdir(void)
{
80106b50:	f3 0f 1e fb          	endbr32 
80106b54:	55                   	push   %ebp
80106b55:	89 e5                	mov    %esp,%ebp
80106b57:	56                   	push   %esi
80106b58:	53                   	push   %ebx
80106b59:	83 ec 10             	sub    $0x10,%esp
  char *path;
  struct inode *ip;
  struct proc *curproc = myproc();
80106b5c:	e8 5f d3 ff ff       	call   80103ec0 <myproc>
80106b61:	89 c6                	mov    %eax,%esi
  
  begin_op();
80106b63:	e8 b8 c6 ff ff       	call   80103220 <begin_op>
  if(argstr(0, &path) < 0 || (ip = namei(path)) == 0){
80106b68:	83 ec 08             	sub    $0x8,%esp
80106b6b:	8d 45 f4             	lea    -0xc(%ebp),%eax
80106b6e:	50                   	push   %eax
80106b6f:	6a 00                	push   $0x0
80106b71:	e8 ca f5 ff ff       	call   80106140 <argstr>
80106b76:	83 c4 10             	add    $0x10,%esp
80106b79:	85 c0                	test   %eax,%eax
80106b7b:	78 73                	js     80106bf0 <sys_chdir+0xa0>
80106b7d:	83 ec 0c             	sub    $0xc,%esp
80106b80:	ff 75 f4             	pushl  -0xc(%ebp)
80106b83:	e8 98 b9 ff ff       	call   80102520 <namei>
80106b88:	83 c4 10             	add    $0x10,%esp
80106b8b:	89 c3                	mov    %eax,%ebx
80106b8d:	85 c0                	test   %eax,%eax
80106b8f:	74 5f                	je     80106bf0 <sys_chdir+0xa0>
    end_op();
    return -1;
  }
  ilock(ip);
80106b91:	83 ec 0c             	sub    $0xc,%esp
80106b94:	50                   	push   %eax
80106b95:	e8 b6 b0 ff ff       	call   80101c50 <ilock>
  if(ip->type != T_DIR){
80106b9a:	83 c4 10             	add    $0x10,%esp
80106b9d:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
80106ba2:	75 2c                	jne    80106bd0 <sys_chdir+0x80>
    iunlockput(ip);
    end_op();
    return -1;
  }
  iunlock(ip);
80106ba4:	83 ec 0c             	sub    $0xc,%esp
80106ba7:	53                   	push   %ebx
80106ba8:	e8 83 b1 ff ff       	call   80101d30 <iunlock>
  iput(curproc->cwd);
80106bad:	58                   	pop    %eax
80106bae:	ff 76 68             	pushl  0x68(%esi)
80106bb1:	e8 ca b1 ff ff       	call   80101d80 <iput>
  end_op();
80106bb6:	e8 d5 c6 ff ff       	call   80103290 <end_op>
  curproc->cwd = ip;
80106bbb:	89 5e 68             	mov    %ebx,0x68(%esi)
  return 0;
80106bbe:	83 c4 10             	add    $0x10,%esp
80106bc1:	31 c0                	xor    %eax,%eax
}
80106bc3:	8d 65 f8             	lea    -0x8(%ebp),%esp
80106bc6:	5b                   	pop    %ebx
80106bc7:	5e                   	pop    %esi
80106bc8:	5d                   	pop    %ebp
80106bc9:	c3                   	ret    
80106bca:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    iunlockput(ip);
80106bd0:	83 ec 0c             	sub    $0xc,%esp
80106bd3:	53                   	push   %ebx
80106bd4:	e8 17 b3 ff ff       	call   80101ef0 <iunlockput>
    end_op();
80106bd9:	e8 b2 c6 ff ff       	call   80103290 <end_op>
    return -1;
80106bde:	83 c4 10             	add    $0x10,%esp
80106be1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106be6:	eb db                	jmp    80106bc3 <sys_chdir+0x73>
80106be8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106bef:	90                   	nop
    end_op();
80106bf0:	e8 9b c6 ff ff       	call   80103290 <end_op>
    return -1;
80106bf5:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106bfa:	eb c7                	jmp    80106bc3 <sys_chdir+0x73>
80106bfc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80106c00 <sys_exec>:

int
sys_exec(void)
{
80106c00:	f3 0f 1e fb          	endbr32 
80106c04:	55                   	push   %ebp
80106c05:	89 e5                	mov    %esp,%ebp
80106c07:	57                   	push   %edi
80106c08:	56                   	push   %esi
  char *path, *argv[MAXARG];
  int i;
  uint uargv, uarg;

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
80106c09:	8d 85 5c ff ff ff    	lea    -0xa4(%ebp),%eax
{
80106c0f:	53                   	push   %ebx
80106c10:	81 ec a4 00 00 00    	sub    $0xa4,%esp
  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
80106c16:	50                   	push   %eax
80106c17:	6a 00                	push   $0x0
80106c19:	e8 22 f5 ff ff       	call   80106140 <argstr>
80106c1e:	83 c4 10             	add    $0x10,%esp
80106c21:	85 c0                	test   %eax,%eax
80106c23:	0f 88 8b 00 00 00    	js     80106cb4 <sys_exec+0xb4>
80106c29:	83 ec 08             	sub    $0x8,%esp
80106c2c:	8d 85 60 ff ff ff    	lea    -0xa0(%ebp),%eax
80106c32:	50                   	push   %eax
80106c33:	6a 01                	push   $0x1
80106c35:	e8 56 f4 ff ff       	call   80106090 <argint>
80106c3a:	83 c4 10             	add    $0x10,%esp
80106c3d:	85 c0                	test   %eax,%eax
80106c3f:	78 73                	js     80106cb4 <sys_exec+0xb4>
    return -1;
  }
  memset(argv, 0, sizeof(argv));
80106c41:	83 ec 04             	sub    $0x4,%esp
80106c44:	8d 85 68 ff ff ff    	lea    -0x98(%ebp),%eax
  for(i=0;; i++){
80106c4a:	31 db                	xor    %ebx,%ebx
  memset(argv, 0, sizeof(argv));
80106c4c:	68 80 00 00 00       	push   $0x80
80106c51:	8d bd 64 ff ff ff    	lea    -0x9c(%ebp),%edi
80106c57:	6a 00                	push   $0x0
80106c59:	50                   	push   %eax
80106c5a:	e8 c1 f0 ff ff       	call   80105d20 <memset>
80106c5f:	83 c4 10             	add    $0x10,%esp
80106c62:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    if(i >= NELEM(argv))
      return -1;
    if(fetchint(uargv+4*i, (int*)&uarg) < 0)
80106c68:	8b 85 60 ff ff ff    	mov    -0xa0(%ebp),%eax
80106c6e:	8d 34 9d 00 00 00 00 	lea    0x0(,%ebx,4),%esi
80106c75:	83 ec 08             	sub    $0x8,%esp
80106c78:	57                   	push   %edi
80106c79:	01 f0                	add    %esi,%eax
80106c7b:	50                   	push   %eax
80106c7c:	e8 6f f3 ff ff       	call   80105ff0 <fetchint>
80106c81:	83 c4 10             	add    $0x10,%esp
80106c84:	85 c0                	test   %eax,%eax
80106c86:	78 2c                	js     80106cb4 <sys_exec+0xb4>
      return -1;
    if(uarg == 0){
80106c88:	8b 85 64 ff ff ff    	mov    -0x9c(%ebp),%eax
80106c8e:	85 c0                	test   %eax,%eax
80106c90:	74 36                	je     80106cc8 <sys_exec+0xc8>
      argv[i] = 0;
      break;
    }
    if(fetchstr(uarg, &argv[i]) < 0)
80106c92:	8d 8d 68 ff ff ff    	lea    -0x98(%ebp),%ecx
80106c98:	83 ec 08             	sub    $0x8,%esp
80106c9b:	8d 14 31             	lea    (%ecx,%esi,1),%edx
80106c9e:	52                   	push   %edx
80106c9f:	50                   	push   %eax
80106ca0:	e8 8b f3 ff ff       	call   80106030 <fetchstr>
80106ca5:	83 c4 10             	add    $0x10,%esp
80106ca8:	85 c0                	test   %eax,%eax
80106caa:	78 08                	js     80106cb4 <sys_exec+0xb4>
  for(i=0;; i++){
80106cac:	83 c3 01             	add    $0x1,%ebx
    if(i >= NELEM(argv))
80106caf:	83 fb 20             	cmp    $0x20,%ebx
80106cb2:	75 b4                	jne    80106c68 <sys_exec+0x68>
      return -1;
  }
  return exec(path, argv);
}
80106cb4:	8d 65 f4             	lea    -0xc(%ebp),%esp
    return -1;
80106cb7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106cbc:	5b                   	pop    %ebx
80106cbd:	5e                   	pop    %esi
80106cbe:	5f                   	pop    %edi
80106cbf:	5d                   	pop    %ebp
80106cc0:	c3                   	ret    
80106cc1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  return exec(path, argv);
80106cc8:	83 ec 08             	sub    $0x8,%esp
80106ccb:	8d 85 68 ff ff ff    	lea    -0x98(%ebp),%eax
      argv[i] = 0;
80106cd1:	c7 84 9d 68 ff ff ff 	movl   $0x0,-0x98(%ebp,%ebx,4)
80106cd8:	00 00 00 00 
  return exec(path, argv);
80106cdc:	50                   	push   %eax
80106cdd:	ff b5 5c ff ff ff    	pushl  -0xa4(%ebp)
80106ce3:	e8 88 a2 ff ff       	call   80100f70 <exec>
80106ce8:	83 c4 10             	add    $0x10,%esp
}
80106ceb:	8d 65 f4             	lea    -0xc(%ebp),%esp
80106cee:	5b                   	pop    %ebx
80106cef:	5e                   	pop    %esi
80106cf0:	5f                   	pop    %edi
80106cf1:	5d                   	pop    %ebp
80106cf2:	c3                   	ret    
80106cf3:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106cfa:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80106d00 <sys_pipe>:

int
sys_pipe(void)
{
80106d00:	f3 0f 1e fb          	endbr32 
80106d04:	55                   	push   %ebp
80106d05:	89 e5                	mov    %esp,%ebp
80106d07:	57                   	push   %edi
80106d08:	56                   	push   %esi
  int *fd;
  struct file *rf, *wf;
  int fd0, fd1;

  if(argptr(0, (void*)&fd, 2*sizeof(fd[0])) < 0)
80106d09:	8d 45 dc             	lea    -0x24(%ebp),%eax
{
80106d0c:	53                   	push   %ebx
80106d0d:	83 ec 20             	sub    $0x20,%esp
  if(argptr(0, (void*)&fd, 2*sizeof(fd[0])) < 0)
80106d10:	6a 08                	push   $0x8
80106d12:	50                   	push   %eax
80106d13:	6a 00                	push   $0x0
80106d15:	e8 c6 f3 ff ff       	call   801060e0 <argptr>
80106d1a:	83 c4 10             	add    $0x10,%esp
80106d1d:	85 c0                	test   %eax,%eax
80106d1f:	78 4e                	js     80106d6f <sys_pipe+0x6f>
    return -1;
  if(pipealloc(&rf, &wf) < 0)
80106d21:	83 ec 08             	sub    $0x8,%esp
80106d24:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80106d27:	50                   	push   %eax
80106d28:	8d 45 e0             	lea    -0x20(%ebp),%eax
80106d2b:	50                   	push   %eax
80106d2c:	e8 bf cb ff ff       	call   801038f0 <pipealloc>
80106d31:	83 c4 10             	add    $0x10,%esp
80106d34:	85 c0                	test   %eax,%eax
80106d36:	78 37                	js     80106d6f <sys_pipe+0x6f>
    return -1;
  fd0 = -1;
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
80106d38:	8b 7d e0             	mov    -0x20(%ebp),%edi
  for(fd = 0; fd < NOFILE; fd++){
80106d3b:	31 db                	xor    %ebx,%ebx
  struct proc *curproc = myproc();
80106d3d:	e8 7e d1 ff ff       	call   80103ec0 <myproc>
  for(fd = 0; fd < NOFILE; fd++){
80106d42:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    if(curproc->ofile[fd] == 0){
80106d48:	8b 74 98 28          	mov    0x28(%eax,%ebx,4),%esi
80106d4c:	85 f6                	test   %esi,%esi
80106d4e:	74 30                	je     80106d80 <sys_pipe+0x80>
  for(fd = 0; fd < NOFILE; fd++){
80106d50:	83 c3 01             	add    $0x1,%ebx
80106d53:	83 fb 10             	cmp    $0x10,%ebx
80106d56:	75 f0                	jne    80106d48 <sys_pipe+0x48>
    if(fd0 >= 0)
      myproc()->ofile[fd0] = 0;
    fileclose(rf);
80106d58:	83 ec 0c             	sub    $0xc,%esp
80106d5b:	ff 75 e0             	pushl  -0x20(%ebp)
80106d5e:	e8 4d a6 ff ff       	call   801013b0 <fileclose>
    fileclose(wf);
80106d63:	58                   	pop    %eax
80106d64:	ff 75 e4             	pushl  -0x1c(%ebp)
80106d67:	e8 44 a6 ff ff       	call   801013b0 <fileclose>
    return -1;
80106d6c:	83 c4 10             	add    $0x10,%esp
80106d6f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106d74:	eb 5b                	jmp    80106dd1 <sys_pipe+0xd1>
80106d76:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106d7d:	8d 76 00             	lea    0x0(%esi),%esi
      curproc->ofile[fd] = f;
80106d80:	8d 73 08             	lea    0x8(%ebx),%esi
80106d83:	89 7c b0 08          	mov    %edi,0x8(%eax,%esi,4)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
80106d87:	8b 7d e4             	mov    -0x1c(%ebp),%edi
  struct proc *curproc = myproc();
80106d8a:	e8 31 d1 ff ff       	call   80103ec0 <myproc>
  for(fd = 0; fd < NOFILE; fd++){
80106d8f:	31 d2                	xor    %edx,%edx
80106d91:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    if(curproc->ofile[fd] == 0){
80106d98:	8b 4c 90 28          	mov    0x28(%eax,%edx,4),%ecx
80106d9c:	85 c9                	test   %ecx,%ecx
80106d9e:	74 20                	je     80106dc0 <sys_pipe+0xc0>
  for(fd = 0; fd < NOFILE; fd++){
80106da0:	83 c2 01             	add    $0x1,%edx
80106da3:	83 fa 10             	cmp    $0x10,%edx
80106da6:	75 f0                	jne    80106d98 <sys_pipe+0x98>
      myproc()->ofile[fd0] = 0;
80106da8:	e8 13 d1 ff ff       	call   80103ec0 <myproc>
80106dad:	c7 44 b0 08 00 00 00 	movl   $0x0,0x8(%eax,%esi,4)
80106db4:	00 
80106db5:	eb a1                	jmp    80106d58 <sys_pipe+0x58>
80106db7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106dbe:	66 90                	xchg   %ax,%ax
      curproc->ofile[fd] = f;
80106dc0:	89 7c 90 28          	mov    %edi,0x28(%eax,%edx,4)
  }
  fd[0] = fd0;
80106dc4:	8b 45 dc             	mov    -0x24(%ebp),%eax
80106dc7:	89 18                	mov    %ebx,(%eax)
  fd[1] = fd1;
80106dc9:	8b 45 dc             	mov    -0x24(%ebp),%eax
80106dcc:	89 50 04             	mov    %edx,0x4(%eax)
  return 0;
80106dcf:	31 c0                	xor    %eax,%eax
}
80106dd1:	8d 65 f4             	lea    -0xc(%ebp),%esp
80106dd4:	5b                   	pop    %ebx
80106dd5:	5e                   	pop    %esi
80106dd6:	5f                   	pop    %edi
80106dd7:	5d                   	pop    %ebp
80106dd8:	c3                   	ret    
80106dd9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80106de0 <sys_copy_file>:

int 
sys_copy_file(void) {
80106de0:	f3 0f 1e fb          	endbr32 
80106de4:	55                   	push   %ebp
80106de5:	89 e5                	mov    %esp,%ebp
80106de7:	57                   	push   %edi
80106de8:	56                   	push   %esi
  struct inode* ip_dst;
  struct inode* ip_src;
  int bytesRead;
  char buf[1024];

  if (argstr(0, &src_path) < 0 || argstr(1, &dst_path) < 0)
80106de9:	8d 85 e0 fb ff ff    	lea    -0x420(%ebp),%eax
sys_copy_file(void) {
80106def:	53                   	push   %ebx
80106df0:	81 ec 34 04 00 00    	sub    $0x434,%esp
  if (argstr(0, &src_path) < 0 || argstr(1, &dst_path) < 0)
80106df6:	50                   	push   %eax
80106df7:	6a 00                	push   $0x0
80106df9:	e8 42 f3 ff ff       	call   80106140 <argstr>
80106dfe:	83 c4 10             	add    $0x10,%esp
80106e01:	85 c0                	test   %eax,%eax
80106e03:	0f 88 fa 00 00 00    	js     80106f03 <sys_copy_file+0x123>
80106e09:	83 ec 08             	sub    $0x8,%esp
80106e0c:	8d 85 e4 fb ff ff    	lea    -0x41c(%ebp),%eax
80106e12:	50                   	push   %eax
80106e13:	6a 01                	push   $0x1
80106e15:	e8 26 f3 ff ff       	call   80106140 <argstr>
80106e1a:	83 c4 10             	add    $0x10,%esp
80106e1d:	85 c0                	test   %eax,%eax
80106e1f:	0f 88 de 00 00 00    	js     80106f03 <sys_copy_file+0x123>
    return -1;
  begin_op();
80106e25:	e8 f6 c3 ff ff       	call   80103220 <begin_op>

  if ((ip_src = namei(src_path)) == 0) { // Check if source file exists
80106e2a:	83 ec 0c             	sub    $0xc,%esp
80106e2d:	ff b5 e0 fb ff ff    	pushl  -0x420(%ebp)
80106e33:	e8 e8 b6 ff ff       	call   80102520 <namei>
80106e38:	83 c4 10             	add    $0x10,%esp
80106e3b:	89 85 d4 fb ff ff    	mov    %eax,-0x42c(%ebp)
80106e41:	85 c0                	test   %eax,%eax
80106e43:	0f 84 b5 00 00 00    	je     80106efe <sys_copy_file+0x11e>
    end_op();
    return -1;
  }
 
  ip_dst = namei(dst_path);
80106e49:	83 ec 0c             	sub    $0xc,%esp
80106e4c:	ff b5 e4 fb ff ff    	pushl  -0x41c(%ebp)
80106e52:	e8 c9 b6 ff ff       	call   80102520 <namei>
  if (ip_dst > 0) { // Check if destination file already exists
80106e57:	83 c4 10             	add    $0x10,%esp
80106e5a:	85 c0                	test   %eax,%eax
80106e5c:	0f 85 9c 00 00 00    	jne    80106efe <sys_copy_file+0x11e>
    end_op();
    return -1;
  }
  ip_dst = create(dst_path, T_FILE, 0, 0);
80106e62:	83 ec 0c             	sub    $0xc,%esp
80106e65:	8b 85 e4 fb ff ff    	mov    -0x41c(%ebp),%eax
80106e6b:	31 c9                	xor    %ecx,%ecx
80106e6d:	ba 02 00 00 00       	mov    $0x2,%edx
80106e72:	6a 00                	push   $0x0

  int bytesWrote = 0;
  int readOffset = 0;
  int writeOffset = 0;
80106e74:	31 f6                	xor    %esi,%esi
  int readOffset = 0;
80106e76:	31 db                	xor    %ebx,%ebx
80106e78:	8d bd e8 fb ff ff    	lea    -0x418(%ebp),%edi
  ip_dst = create(dst_path, T_FILE, 0, 0);
80106e7e:	e8 6d f3 ff ff       	call   801061f0 <create>
  ilock(ip_src);
80106e83:	5a                   	pop    %edx
80106e84:	ff b5 d4 fb ff ff    	pushl  -0x42c(%ebp)
  ip_dst = create(dst_path, T_FILE, 0, 0);
80106e8a:	89 85 d0 fb ff ff    	mov    %eax,-0x430(%ebp)
  ilock(ip_src);
80106e90:	e8 bb ad ff ff       	call   80101c50 <ilock>
  while ((bytesRead = readi(ip_src, buf, readOffset, sizeof(buf))) > 0) {
80106e95:	83 c4 10             	add    $0x10,%esp
80106e98:	eb 1f                	jmp    80106eb9 <sys_copy_file+0xd9>
80106e9a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    readOffset += bytesRead;
    if ((bytesWrote = writei(ip_dst , buf,writeOffset,  bytesRead)) <= 0)
80106ea0:	50                   	push   %eax
    readOffset += bytesRead;
80106ea1:	01 c3                	add    %eax,%ebx
    if ((bytesWrote = writei(ip_dst , buf,writeOffset,  bytesRead)) <= 0)
80106ea3:	56                   	push   %esi
80106ea4:	57                   	push   %edi
80106ea5:	ff b5 d0 fb ff ff    	pushl  -0x430(%ebp)
80106eab:	e8 a0 b1 ff ff       	call   80102050 <writei>
80106eb0:	83 c4 10             	add    $0x10,%esp
80106eb3:	85 c0                	test   %eax,%eax
80106eb5:	7e 4c                	jle    80106f03 <sys_copy_file+0x123>
      return -1;
    writeOffset += bytesWrote;
80106eb7:	01 c6                	add    %eax,%esi
  while ((bytesRead = readi(ip_src, buf, readOffset, sizeof(buf))) > 0) {
80106eb9:	68 00 04 00 00       	push   $0x400
80106ebe:	53                   	push   %ebx
80106ebf:	57                   	push   %edi
80106ec0:	ff b5 d4 fb ff ff    	pushl  -0x42c(%ebp)
80106ec6:	e8 85 b0 ff ff       	call   80101f50 <readi>
80106ecb:	83 c4 10             	add    $0x10,%esp
80106ece:	85 c0                	test   %eax,%eax
80106ed0:	7f ce                	jg     80106ea0 <sys_copy_file+0xc0>
   
}

  iunlock(ip_src);
80106ed2:	83 ec 0c             	sub    $0xc,%esp
80106ed5:	ff b5 d4 fb ff ff    	pushl  -0x42c(%ebp)
80106edb:	e8 50 ae ff ff       	call   80101d30 <iunlock>
  iunlock(ip_dst);
80106ee0:	58                   	pop    %eax
80106ee1:	ff b5 d0 fb ff ff    	pushl  -0x430(%ebp)
80106ee7:	e8 44 ae ff ff       	call   80101d30 <iunlock>
  end_op();
80106eec:	e8 9f c3 ff ff       	call   80103290 <end_op>

  return 0;
80106ef1:	83 c4 10             	add    $0x10,%esp
}
80106ef4:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
80106ef7:	31 c0                	xor    %eax,%eax
}
80106ef9:	5b                   	pop    %ebx
80106efa:	5e                   	pop    %esi
80106efb:	5f                   	pop    %edi
80106efc:	5d                   	pop    %ebp
80106efd:	c3                   	ret    
    end_op();
80106efe:	e8 8d c3 ff ff       	call   80103290 <end_op>
}
80106f03:	8d 65 f4             	lea    -0xc(%ebp),%esp
    return -1;
80106f06:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106f0b:	5b                   	pop    %ebx
80106f0c:	5e                   	pop    %esi
80106f0d:	5f                   	pop    %edi
80106f0e:	5d                   	pop    %ebp
80106f0f:	c3                   	ret    

80106f10 <sys_get_process_lifetime>:
    return -1;
  //cprintf("sysproc.h %d", pid);
  return get_process_lifetime(pid);
}*/

int sys_get_process_lifetime(void){
80106f10:	f3 0f 1e fb          	endbr32 
  return get_process_lifetime();
80106f14:	e9 67 da ff ff       	jmp    80104980 <get_process_lifetime>
80106f19:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80106f20 <sys_get_uncle_count>:
}

int sys_get_uncle_count(void){
80106f20:	f3 0f 1e fb          	endbr32 
80106f24:	55                   	push   %ebp
80106f25:	89 e5                	mov    %esp,%ebp
80106f27:	83 ec 20             	sub    $0x20,%esp
  int pid;
  if(argint(0, &pid) < 0){
80106f2a:	8d 45 f4             	lea    -0xc(%ebp),%eax
80106f2d:	50                   	push   %eax
80106f2e:	6a 00                	push   $0x0
80106f30:	e8 5b f1 ff ff       	call   80106090 <argint>
80106f35:	83 c4 10             	add    $0x10,%esp
80106f38:	85 c0                	test   %eax,%eax
80106f3a:	78 14                	js     80106f50 <sys_get_uncle_count+0x30>
    return -1;
  }
  return get_uncle_count(pid);
80106f3c:	83 ec 0c             	sub    $0xc,%esp
80106f3f:	ff 75 f4             	pushl  -0xc(%ebp)
80106f42:	e8 19 d8 ff ff       	call   80104760 <get_uncle_count>
80106f47:	83 c4 10             	add    $0x10,%esp
}
80106f4a:	c9                   	leave  
80106f4b:	c3                   	ret    
80106f4c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80106f50:	c9                   	leave  
    return -1;
80106f51:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106f56:	c3                   	ret    
80106f57:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106f5e:	66 90                	xchg   %ax,%ax

80106f60 <sys_fork>:

int
sys_fork(void)
{
80106f60:	f3 0f 1e fb          	endbr32 
  return fork();
80106f64:	e9 e7 db ff ff       	jmp    80104b50 <fork>
80106f69:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80106f70 <sys_exit>:
}

int
sys_exit(void)
{
80106f70:	f3 0f 1e fb          	endbr32 
80106f74:	55                   	push   %ebp
80106f75:	89 e5                	mov    %esp,%ebp
80106f77:	83 ec 08             	sub    $0x8,%esp
  exit();
80106f7a:	e8 91 d3 ff ff       	call   80104310 <exit>
  return 0;  // not reached
}
80106f7f:	31 c0                	xor    %eax,%eax
80106f81:	c9                   	leave  
80106f82:	c3                   	ret    
80106f83:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106f8a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80106f90 <sys_wait>:

int
sys_wait(void)
{
80106f90:	f3 0f 1e fb          	endbr32 
  return wait();
80106f94:	e9 c7 d5 ff ff       	jmp    80104560 <wait>
80106f99:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80106fa0 <sys_kill>:
}

int
sys_kill(void)
{
80106fa0:	f3 0f 1e fb          	endbr32 
80106fa4:	55                   	push   %ebp
80106fa5:	89 e5                	mov    %esp,%ebp
80106fa7:	83 ec 20             	sub    $0x20,%esp
  int pid;

  if(argint(0, &pid) < 0)
80106faa:	8d 45 f4             	lea    -0xc(%ebp),%eax
80106fad:	50                   	push   %eax
80106fae:	6a 00                	push   $0x0
80106fb0:	e8 db f0 ff ff       	call   80106090 <argint>
80106fb5:	83 c4 10             	add    $0x10,%esp
80106fb8:	85 c0                	test   %eax,%eax
80106fba:	78 14                	js     80106fd0 <sys_kill+0x30>
    return -1;
  return kill(pid);
80106fbc:	83 ec 0c             	sub    $0xc,%esp
80106fbf:	ff 75 f4             	pushl  -0xc(%ebp)
80106fc2:	e8 09 d7 ff ff       	call   801046d0 <kill>
80106fc7:	83 c4 10             	add    $0x10,%esp
}
80106fca:	c9                   	leave  
80106fcb:	c3                   	ret    
80106fcc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80106fd0:	c9                   	leave  
    return -1;
80106fd1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106fd6:	c3                   	ret    
80106fd7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106fde:	66 90                	xchg   %ax,%ax

80106fe0 <sys_getpid>:

int
sys_getpid(void)
{
80106fe0:	f3 0f 1e fb          	endbr32 
80106fe4:	55                   	push   %ebp
80106fe5:	89 e5                	mov    %esp,%ebp
80106fe7:	83 ec 08             	sub    $0x8,%esp
  return myproc()->pid;
80106fea:	e8 d1 ce ff ff       	call   80103ec0 <myproc>
80106fef:	8b 40 10             	mov    0x10(%eax),%eax
}
80106ff2:	c9                   	leave  
80106ff3:	c3                   	ret    
80106ff4:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106ffb:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80106fff:	90                   	nop

80107000 <sys_sbrk>:

int
sys_sbrk(void)
{
80107000:	f3 0f 1e fb          	endbr32 
80107004:	55                   	push   %ebp
80107005:	89 e5                	mov    %esp,%ebp
80107007:	53                   	push   %ebx
  int addr;
  int n;

  if(argint(0, &n) < 0)
80107008:	8d 45 f4             	lea    -0xc(%ebp),%eax
{
8010700b:	83 ec 1c             	sub    $0x1c,%esp
  if(argint(0, &n) < 0)
8010700e:	50                   	push   %eax
8010700f:	6a 00                	push   $0x0
80107011:	e8 7a f0 ff ff       	call   80106090 <argint>
80107016:	83 c4 10             	add    $0x10,%esp
80107019:	85 c0                	test   %eax,%eax
8010701b:	78 23                	js     80107040 <sys_sbrk+0x40>
    return -1;
  addr = myproc()->sz;
8010701d:	e8 9e ce ff ff       	call   80103ec0 <myproc>
  if(growproc(n) < 0)
80107022:	83 ec 0c             	sub    $0xc,%esp
  addr = myproc()->sz;
80107025:	8b 18                	mov    (%eax),%ebx
  if(growproc(n) < 0)
80107027:	ff 75 f4             	pushl  -0xc(%ebp)
8010702a:	e8 c1 ce ff ff       	call   80103ef0 <growproc>
8010702f:	83 c4 10             	add    $0x10,%esp
80107032:	85 c0                	test   %eax,%eax
80107034:	78 0a                	js     80107040 <sys_sbrk+0x40>
    return -1;
  return addr;
}
80107036:	89 d8                	mov    %ebx,%eax
80107038:	8b 5d fc             	mov    -0x4(%ebp),%ebx
8010703b:	c9                   	leave  
8010703c:	c3                   	ret    
8010703d:	8d 76 00             	lea    0x0(%esi),%esi
    return -1;
80107040:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
80107045:	eb ef                	jmp    80107036 <sys_sbrk+0x36>
80107047:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010704e:	66 90                	xchg   %ax,%ax

80107050 <sys_sleep>:

int
sys_sleep(void)
{
80107050:	f3 0f 1e fb          	endbr32 
80107054:	55                   	push   %ebp
80107055:	89 e5                	mov    %esp,%ebp
80107057:	53                   	push   %ebx
  int n;
  uint ticks0;

  if(argint(0, &n) < 0)
80107058:	8d 45 f4             	lea    -0xc(%ebp),%eax
{
8010705b:	83 ec 1c             	sub    $0x1c,%esp
  if(argint(0, &n) < 0)
8010705e:	50                   	push   %eax
8010705f:	6a 00                	push   $0x0
80107061:	e8 2a f0 ff ff       	call   80106090 <argint>
80107066:	83 c4 10             	add    $0x10,%esp
80107069:	85 c0                	test   %eax,%eax
8010706b:	0f 88 86 00 00 00    	js     801070f7 <sys_sleep+0xa7>
    return -1;
  acquire(&tickslock);
80107071:	83 ec 0c             	sub    $0xc,%esp
80107074:	68 60 7e 11 80       	push   $0x80117e60
80107079:	e8 92 eb ff ff       	call   80105c10 <acquire>
  ticks0 = ticks;
  while(ticks - ticks0 < n){
8010707e:	8b 55 f4             	mov    -0xc(%ebp),%edx
  ticks0 = ticks;
80107081:	8b 1d a0 86 11 80    	mov    0x801186a0,%ebx
  while(ticks - ticks0 < n){
80107087:	83 c4 10             	add    $0x10,%esp
8010708a:	85 d2                	test   %edx,%edx
8010708c:	75 23                	jne    801070b1 <sys_sleep+0x61>
8010708e:	eb 50                	jmp    801070e0 <sys_sleep+0x90>
    if(myproc()->killed){
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
80107090:	83 ec 08             	sub    $0x8,%esp
80107093:	68 60 7e 11 80       	push   $0x80117e60
80107098:	68 a0 86 11 80       	push   $0x801186a0
8010709d:	e8 fe d3 ff ff       	call   801044a0 <sleep>
  while(ticks - ticks0 < n){
801070a2:	a1 a0 86 11 80       	mov    0x801186a0,%eax
801070a7:	83 c4 10             	add    $0x10,%esp
801070aa:	29 d8                	sub    %ebx,%eax
801070ac:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801070af:	73 2f                	jae    801070e0 <sys_sleep+0x90>
    if(myproc()->killed){
801070b1:	e8 0a ce ff ff       	call   80103ec0 <myproc>
801070b6:	8b 40 24             	mov    0x24(%eax),%eax
801070b9:	85 c0                	test   %eax,%eax
801070bb:	74 d3                	je     80107090 <sys_sleep+0x40>
      release(&tickslock);
801070bd:	83 ec 0c             	sub    $0xc,%esp
801070c0:	68 60 7e 11 80       	push   $0x80117e60
801070c5:	e8 06 ec ff ff       	call   80105cd0 <release>
  }
  release(&tickslock);
  return 0;
}
801070ca:	8b 5d fc             	mov    -0x4(%ebp),%ebx
      return -1;
801070cd:	83 c4 10             	add    $0x10,%esp
801070d0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801070d5:	c9                   	leave  
801070d6:	c3                   	ret    
801070d7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801070de:	66 90                	xchg   %ax,%ax
  release(&tickslock);
801070e0:	83 ec 0c             	sub    $0xc,%esp
801070e3:	68 60 7e 11 80       	push   $0x80117e60
801070e8:	e8 e3 eb ff ff       	call   80105cd0 <release>
  return 0;
801070ed:	83 c4 10             	add    $0x10,%esp
801070f0:	31 c0                	xor    %eax,%eax
}
801070f2:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801070f5:	c9                   	leave  
801070f6:	c3                   	ret    
    return -1;
801070f7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801070fc:	eb f4                	jmp    801070f2 <sys_sleep+0xa2>
801070fe:	66 90                	xchg   %ax,%ax

80107100 <sys_change_sched_Q>:
int
sys_change_sched_Q(void)
{
80107100:	f3 0f 1e fb          	endbr32 
80107104:	55                   	push   %ebp
80107105:	89 e5                	mov    %esp,%ebp
80107107:	83 ec 20             	sub    $0x20,%esp
  int queue_number, pid;
  if(argint(0, &pid) < 0 || argint(1, &queue_number) < 0)
8010710a:	8d 45 f4             	lea    -0xc(%ebp),%eax
8010710d:	50                   	push   %eax
8010710e:	6a 00                	push   $0x0
80107110:	e8 7b ef ff ff       	call   80106090 <argint>
80107115:	83 c4 10             	add    $0x10,%esp
80107118:	85 c0                	test   %eax,%eax
8010711a:	78 34                	js     80107150 <sys_change_sched_Q+0x50>
8010711c:	83 ec 08             	sub    $0x8,%esp
8010711f:	8d 45 f0             	lea    -0x10(%ebp),%eax
80107122:	50                   	push   %eax
80107123:	6a 01                	push   $0x1
80107125:	e8 66 ef ff ff       	call   80106090 <argint>
8010712a:	83 c4 10             	add    $0x10,%esp
8010712d:	85 c0                	test   %eax,%eax
8010712f:	78 1f                	js     80107150 <sys_change_sched_Q+0x50>
    return -1;

  if(queue_number < ROUND_ROBIN || queue_number > BJF)
80107131:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107134:	8d 50 ff             	lea    -0x1(%eax),%edx
80107137:	83 fa 02             	cmp    $0x2,%edx
8010713a:	77 14                	ja     80107150 <sys_change_sched_Q+0x50>
    return -1;

  return change_Q(pid, queue_number);
8010713c:	83 ec 08             	sub    $0x8,%esp
8010713f:	50                   	push   %eax
80107140:	ff 75 f4             	pushl  -0xc(%ebp)
80107143:	e8 68 d8 ff ff       	call   801049b0 <change_Q>
80107148:	83 c4 10             	add    $0x10,%esp
}
8010714b:	c9                   	leave  
8010714c:	c3                   	ret    
8010714d:	8d 76 00             	lea    0x0(%esi),%esi
80107150:	c9                   	leave  
    return -1;
80107151:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80107156:	c3                   	ret    
80107157:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010715e:	66 90                	xchg   %ax,%ax

80107160 <sys_show_process_info>:

void sys_show_process_info(void) {
80107160:	f3 0f 1e fb          	endbr32 
  show_process_info();
80107164:	e9 47 dd ff ff       	jmp    80104eb0 <show_process_info>
80107169:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80107170 <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
int
sys_uptime(void)
{
80107170:	f3 0f 1e fb          	endbr32 
80107174:	55                   	push   %ebp
80107175:	89 e5                	mov    %esp,%ebp
80107177:	53                   	push   %ebx
80107178:	83 ec 10             	sub    $0x10,%esp
  uint xticks;

  acquire(&tickslock);
8010717b:	68 60 7e 11 80       	push   $0x80117e60
80107180:	e8 8b ea ff ff       	call   80105c10 <acquire>
  xticks = ticks;
80107185:	8b 1d a0 86 11 80    	mov    0x801186a0,%ebx
  release(&tickslock);
8010718b:	c7 04 24 60 7e 11 80 	movl   $0x80117e60,(%esp)
80107192:	e8 39 eb ff ff       	call   80105cd0 <release>
  return xticks;
}
80107197:	89 d8                	mov    %ebx,%eax
80107199:	8b 5d fc             	mov    -0x4(%ebp),%ebx
8010719c:	c9                   	leave  
8010719d:	c3                   	ret    
8010719e:	66 90                	xchg   %ax,%ax

801071a0 <sys_find_digital_root>:

int
sys_find_digital_root(void)
{
801071a0:	f3 0f 1e fb          	endbr32 
801071a4:	55                   	push   %ebp
801071a5:	89 e5                	mov    %esp,%ebp
801071a7:	53                   	push   %ebx
801071a8:	83 ec 04             	sub    $0x4,%esp
  int n = myproc()->tf->ebx;
801071ab:	e8 10 cd ff ff       	call   80103ec0 <myproc>
  cprintf("KERNEL: sys_find_digital_root(%d)\n", n);
801071b0:	83 ec 08             	sub    $0x8,%esp
  int n = myproc()->tf->ebx;
801071b3:	8b 40 18             	mov    0x18(%eax),%eax
801071b6:	8b 58 10             	mov    0x10(%eax),%ebx
  cprintf("KERNEL: sys_find_digital_root(%d)\n", n);
801071b9:	53                   	push   %ebx
801071ba:	68 c0 95 10 80       	push   $0x801095c0
801071bf:	e8 fc 96 ff ff       	call   801008c0 <cprintf>
  return find_digital_root(n);
801071c4:	89 1c 24             	mov    %ebx,(%esp)
801071c7:	e8 54 d7 ff ff       	call   80104920 <find_digital_root>

}
801071cc:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801071cf:	c9                   	leave  
801071d0:	c3                   	ret    
801071d1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801071d8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801071df:	90                   	nop

801071e0 <sys_priorityLock_test>:


void
sys_priorityLock_test(void){
801071e0:	f3 0f 1e fb          	endbr32 
  priorityLock_test();
801071e4:	e9 47 e5 ff ff       	jmp    80105730 <priorityLock_test>
801071e9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

801071f0 <sys_set_proc_bjf_params>:
//   return 0;
// }

int
sys_set_proc_bjf_params(void)
{
801071f0:	f3 0f 1e fb          	endbr32 
801071f4:	55                   	push   %ebp
801071f5:	89 e5                	mov    %esp,%ebp
801071f7:	83 ec 30             	sub    $0x30,%esp
  int pid;
  float priority_ratio, arrival_time_ratio, executed_cycle_ratio, process_size_ratio;
  if(argint(0, &pid) < 0 ||
801071fa:	8d 45 e4             	lea    -0x1c(%ebp),%eax
801071fd:	50                   	push   %eax
801071fe:	6a 00                	push   $0x0
80107200:	e8 8b ee ff ff       	call   80106090 <argint>
80107205:	83 c4 10             	add    $0x10,%esp
80107208:	85 c0                	test   %eax,%eax
8010720a:	78 74                	js     80107280 <sys_set_proc_bjf_params+0x90>
     argfloat(1, &priority_ratio) < 0 ||
8010720c:	83 ec 08             	sub    $0x8,%esp
8010720f:	8d 45 e8             	lea    -0x18(%ebp),%eax
80107212:	50                   	push   %eax
80107213:	6a 01                	push   $0x1
80107215:	e8 86 ed ff ff       	call   80105fa0 <argfloat>
  if(argint(0, &pid) < 0 ||
8010721a:	83 c4 10             	add    $0x10,%esp
8010721d:	85 c0                	test   %eax,%eax
8010721f:	78 5f                	js     80107280 <sys_set_proc_bjf_params+0x90>
     argfloat(2, &arrival_time_ratio) < 0 ||
80107221:	83 ec 08             	sub    $0x8,%esp
80107224:	8d 45 ec             	lea    -0x14(%ebp),%eax
80107227:	50                   	push   %eax
80107228:	6a 02                	push   $0x2
8010722a:	e8 71 ed ff ff       	call   80105fa0 <argfloat>
     argfloat(1, &priority_ratio) < 0 ||
8010722f:	83 c4 10             	add    $0x10,%esp
80107232:	85 c0                	test   %eax,%eax
80107234:	78 4a                	js     80107280 <sys_set_proc_bjf_params+0x90>
     argfloat(3, &executed_cycle_ratio) < 0||
80107236:	83 ec 08             	sub    $0x8,%esp
80107239:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010723c:	50                   	push   %eax
8010723d:	6a 03                	push   $0x3
8010723f:	e8 5c ed ff ff       	call   80105fa0 <argfloat>
     argfloat(2, &arrival_time_ratio) < 0 ||
80107244:	83 c4 10             	add    $0x10,%esp
80107247:	85 c0                	test   %eax,%eax
80107249:	78 35                	js     80107280 <sys_set_proc_bjf_params+0x90>
     argfloat(4, &process_size_ratio)<0 ){
8010724b:	83 ec 08             	sub    $0x8,%esp
8010724e:	8d 45 f4             	lea    -0xc(%ebp),%eax
80107251:	50                   	push   %eax
80107252:	6a 04                	push   $0x4
80107254:	e8 47 ed ff ff       	call   80105fa0 <argfloat>
     argfloat(3, &executed_cycle_ratio) < 0||
80107259:	83 c4 10             	add    $0x10,%esp
8010725c:	85 c0                	test   %eax,%eax
8010725e:	78 20                	js     80107280 <sys_set_proc_bjf_params+0x90>
    return -1;
  }

  return set_proc_bjf_params(pid, priority_ratio, arrival_time_ratio, executed_cycle_ratio,process_size_ratio);
80107260:	83 ec 0c             	sub    $0xc,%esp
80107263:	ff 75 f4             	pushl  -0xc(%ebp)
80107266:	ff 75 f0             	pushl  -0x10(%ebp)
80107269:	ff 75 ec             	pushl  -0x14(%ebp)
8010726c:	ff 75 e8             	pushl  -0x18(%ebp)
8010726f:	ff 75 e4             	pushl  -0x1c(%ebp)
80107272:	e8 29 db ff ff       	call   80104da0 <set_proc_bjf_params>
80107277:	83 c4 20             	add    $0x20,%esp
}
8010727a:	c9                   	leave  
8010727b:	c3                   	ret    
8010727c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80107280:	c9                   	leave  
    return -1;
80107281:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80107286:	c3                   	ret    
80107287:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010728e:	66 90                	xchg   %ax,%ax

80107290 <sys_set_system_bjf_params>:

int
sys_set_system_bjf_params(void)
{
80107290:	f3 0f 1e fb          	endbr32 
80107294:	55                   	push   %ebp
80107295:	89 e5                	mov    %esp,%ebp
80107297:	83 ec 20             	sub    $0x20,%esp
  float priority_ratio, arrival_time_ratio, executed_cycle_ratio,process_size_ratio;
  if(argfloat(0, &priority_ratio) < 0 ||
8010729a:	8d 45 e8             	lea    -0x18(%ebp),%eax
8010729d:	50                   	push   %eax
8010729e:	6a 00                	push   $0x0
801072a0:	e8 fb ec ff ff       	call   80105fa0 <argfloat>
801072a5:	83 c4 10             	add    $0x10,%esp
801072a8:	85 c0                	test   %eax,%eax
801072aa:	78 5c                	js     80107308 <sys_set_system_bjf_params+0x78>
     argfloat(1, &arrival_time_ratio) < 0 ||
801072ac:	83 ec 08             	sub    $0x8,%esp
801072af:	8d 45 ec             	lea    -0x14(%ebp),%eax
801072b2:	50                   	push   %eax
801072b3:	6a 01                	push   $0x1
801072b5:	e8 e6 ec ff ff       	call   80105fa0 <argfloat>
  if(argfloat(0, &priority_ratio) < 0 ||
801072ba:	83 c4 10             	add    $0x10,%esp
801072bd:	85 c0                	test   %eax,%eax
801072bf:	78 47                	js     80107308 <sys_set_system_bjf_params+0x78>
     argfloat(2, &executed_cycle_ratio) < 0||
801072c1:	83 ec 08             	sub    $0x8,%esp
801072c4:	8d 45 f0             	lea    -0x10(%ebp),%eax
801072c7:	50                   	push   %eax
801072c8:	6a 02                	push   $0x2
801072ca:	e8 d1 ec ff ff       	call   80105fa0 <argfloat>
     argfloat(1, &arrival_time_ratio) < 0 ||
801072cf:	83 c4 10             	add    $0x10,%esp
801072d2:	85 c0                	test   %eax,%eax
801072d4:	78 32                	js     80107308 <sys_set_system_bjf_params+0x78>
     argfloat(3,&process_size_ratio)<0){
801072d6:	83 ec 08             	sub    $0x8,%esp
801072d9:	8d 45 f4             	lea    -0xc(%ebp),%eax
801072dc:	50                   	push   %eax
801072dd:	6a 03                	push   $0x3
801072df:	e8 bc ec ff ff       	call   80105fa0 <argfloat>
     argfloat(2, &executed_cycle_ratio) < 0||
801072e4:	83 c4 10             	add    $0x10,%esp
801072e7:	85 c0                	test   %eax,%eax
801072e9:	78 1d                	js     80107308 <sys_set_system_bjf_params+0x78>
    return -1;
  }

  set_system_bjf_params(priority_ratio, arrival_time_ratio, executed_cycle_ratio,process_size_ratio);
801072eb:	ff 75 f4             	pushl  -0xc(%ebp)
801072ee:	ff 75 f0             	pushl  -0x10(%ebp)
801072f1:	ff 75 ec             	pushl  -0x14(%ebp)
801072f4:	ff 75 e8             	pushl  -0x18(%ebp)
801072f7:	e8 44 db ff ff       	call   80104e40 <set_system_bjf_params>
  return 0;
801072fc:	83 c4 10             	add    $0x10,%esp
801072ff:	31 c0                	xor    %eax,%eax
}
80107301:	c9                   	leave  
80107302:	c3                   	ret    
80107303:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80107307:	90                   	nop
80107308:	c9                   	leave  
    return -1;
80107309:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010730e:	c3                   	ret    

8010730f <alltraps>:

  # vectors.S sends all traps here.
.globl alltraps
alltraps:
  # Build trap frame.
  pushl %ds
8010730f:	1e                   	push   %ds
  pushl %es
80107310:	06                   	push   %es
  pushl %fs
80107311:	0f a0                	push   %fs
  pushl %gs
80107313:	0f a8                	push   %gs
  pushal
80107315:	60                   	pusha  
  
  # Set up data segments.
  movw $(SEG_KDATA<<3), %ax
80107316:	66 b8 10 00          	mov    $0x10,%ax
  movw %ax, %ds
8010731a:	8e d8                	mov    %eax,%ds
  movw %ax, %es
8010731c:	8e c0                	mov    %eax,%es

  # Call trap(tf), where tf=%esp
  pushl %esp
8010731e:	54                   	push   %esp
  call trap
8010731f:	e8 cc 00 00 00       	call   801073f0 <trap>
  addl $4, %esp
80107324:	83 c4 04             	add    $0x4,%esp

80107327 <trapret>:

  # Return falls through to trapret...
.globl trapret
trapret:
  popal
80107327:	61                   	popa   
  popl %gs
80107328:	0f a9                	pop    %gs
  popl %fs
8010732a:	0f a1                	pop    %fs
  popl %es
8010732c:	07                   	pop    %es
  popl %ds
8010732d:	1f                   	pop    %ds
  addl $0x8, %esp  # trapno and errcode
8010732e:	83 c4 08             	add    $0x8,%esp
  iret
80107331:	cf                   	iret   
80107332:	66 90                	xchg   %ax,%ax
80107334:	66 90                	xchg   %ax,%ax
80107336:	66 90                	xchg   %ax,%ax
80107338:	66 90                	xchg   %ax,%ax
8010733a:	66 90                	xchg   %ax,%ax
8010733c:	66 90                	xchg   %ax,%ax
8010733e:	66 90                	xchg   %ax,%ax

80107340 <tvinit>:
struct spinlock tickslock;
uint ticks;

void
tvinit(void)
{
80107340:	f3 0f 1e fb          	endbr32 
80107344:	55                   	push   %ebp
  int i;

  for(i = 0; i < 256; i++)
80107345:	31 c0                	xor    %eax,%eax
{
80107347:	89 e5                	mov    %esp,%ebp
80107349:	83 ec 08             	sub    $0x8,%esp
8010734c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
80107350:	8b 14 85 08 c0 10 80 	mov    -0x7fef3ff8(,%eax,4),%edx
80107357:	c7 04 c5 a2 7e 11 80 	movl   $0x8e000008,-0x7fee815e(,%eax,8)
8010735e:	08 00 00 8e 
80107362:	66 89 14 c5 a0 7e 11 	mov    %dx,-0x7fee8160(,%eax,8)
80107369:	80 
8010736a:	c1 ea 10             	shr    $0x10,%edx
8010736d:	66 89 14 c5 a6 7e 11 	mov    %dx,-0x7fee815a(,%eax,8)
80107374:	80 
  for(i = 0; i < 256; i++)
80107375:	83 c0 01             	add    $0x1,%eax
80107378:	3d 00 01 00 00       	cmp    $0x100,%eax
8010737d:	75 d1                	jne    80107350 <tvinit+0x10>
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);

  initlock(&tickslock, "time");
8010737f:	83 ec 08             	sub    $0x8,%esp
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);
80107382:	a1 08 c1 10 80       	mov    0x8010c108,%eax
80107387:	c7 05 a2 80 11 80 08 	movl   $0xef000008,0x801180a2
8010738e:	00 00 ef 
  initlock(&tickslock, "time");
80107391:	68 e3 95 10 80       	push   $0x801095e3
80107396:	68 60 7e 11 80       	push   $0x80117e60
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);
8010739b:	66 a3 a0 80 11 80    	mov    %ax,0x801180a0
801073a1:	c1 e8 10             	shr    $0x10,%eax
801073a4:	66 a3 a6 80 11 80    	mov    %ax,0x801180a6
  initlock(&tickslock, "time");
801073aa:	e8 e1 e6 ff ff       	call   80105a90 <initlock>
}
801073af:	83 c4 10             	add    $0x10,%esp
801073b2:	c9                   	leave  
801073b3:	c3                   	ret    
801073b4:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801073bb:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801073bf:	90                   	nop

801073c0 <idtinit>:

void
idtinit(void)
{
801073c0:	f3 0f 1e fb          	endbr32 
801073c4:	55                   	push   %ebp
  pd[0] = size-1;
801073c5:	b8 ff 07 00 00       	mov    $0x7ff,%eax
801073ca:	89 e5                	mov    %esp,%ebp
801073cc:	83 ec 10             	sub    $0x10,%esp
801073cf:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
801073d3:	b8 a0 7e 11 80       	mov    $0x80117ea0,%eax
801073d8:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
801073dc:	c1 e8 10             	shr    $0x10,%eax
801073df:	66 89 45 fe          	mov    %ax,-0x2(%ebp)
  asm volatile("lidt (%0)" : : "r" (pd));
801073e3:	8d 45 fa             	lea    -0x6(%ebp),%eax
801073e6:	0f 01 18             	lidtl  (%eax)
  lidt(idt, sizeof(idt));
}
801073e9:	c9                   	leave  
801073ea:	c3                   	ret    
801073eb:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801073ef:	90                   	nop

801073f0 <trap>:

//PAGEBREAK: 41
void
trap(struct trapframe *tf)
{
801073f0:	f3 0f 1e fb          	endbr32 
801073f4:	55                   	push   %ebp
801073f5:	89 e5                	mov    %esp,%ebp
801073f7:	57                   	push   %edi
801073f8:	56                   	push   %esi
801073f9:	53                   	push   %ebx
801073fa:	83 ec 1c             	sub    $0x1c,%esp
801073fd:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(tf->trapno == T_SYSCALL){
80107400:	8b 43 30             	mov    0x30(%ebx),%eax
80107403:	83 f8 40             	cmp    $0x40,%eax
80107406:	0f 84 cc 01 00 00    	je     801075d8 <trap+0x1e8>
    if(myproc()->killed)
      exit();
    return;
  }
  int os_tick;
  switch(tf->trapno){
8010740c:	83 e8 20             	sub    $0x20,%eax
8010740f:	83 f8 1f             	cmp    $0x1f,%eax
80107412:	77 08                	ja     8010741c <trap+0x2c>
80107414:	3e ff 24 85 8c 96 10 	notrack jmp *-0x7fef6974(,%eax,4)
8010741b:	80 
    lapiceoi();
    break;

  //PAGEBREAK: 13
  default:
    if(myproc() == 0 || (tf->cs&3) == 0){
8010741c:	e8 9f ca ff ff       	call   80103ec0 <myproc>
80107421:	8b 7b 38             	mov    0x38(%ebx),%edi
80107424:	85 c0                	test   %eax,%eax
80107426:	0f 84 fb 01 00 00    	je     80107627 <trap+0x237>
8010742c:	f6 43 3c 03          	testb  $0x3,0x3c(%ebx)
80107430:	0f 84 f1 01 00 00    	je     80107627 <trap+0x237>

static inline uint
rcr2(void)
{
  uint val;
  asm volatile("movl %%cr2,%0" : "=r" (val));
80107436:	0f 20 d1             	mov    %cr2,%ecx
80107439:	89 4d d8             	mov    %ecx,-0x28(%ebp)
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpuid(), tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
8010743c:	e8 5f ca ff ff       	call   80103ea0 <cpuid>
80107441:	8b 73 30             	mov    0x30(%ebx),%esi
80107444:	89 45 dc             	mov    %eax,-0x24(%ebp)
80107447:	8b 43 34             	mov    0x34(%ebx),%eax
8010744a:	89 45 e4             	mov    %eax,-0x1c(%ebp)
            "eip 0x%x addr 0x%x--kill proc\n",
            myproc()->pid, myproc()->name, tf->trapno,
8010744d:	e8 6e ca ff ff       	call   80103ec0 <myproc>
80107452:	89 45 e0             	mov    %eax,-0x20(%ebp)
80107455:	e8 66 ca ff ff       	call   80103ec0 <myproc>
    cprintf("pid %d %s: trap %d err %d on cpu %d "
8010745a:	8b 4d d8             	mov    -0x28(%ebp),%ecx
8010745d:	8b 55 dc             	mov    -0x24(%ebp),%edx
80107460:	51                   	push   %ecx
80107461:	57                   	push   %edi
80107462:	52                   	push   %edx
80107463:	ff 75 e4             	pushl  -0x1c(%ebp)
80107466:	56                   	push   %esi
            myproc()->pid, myproc()->name, tf->trapno,
80107467:	8b 75 e0             	mov    -0x20(%ebp),%esi
8010746a:	83 c6 6c             	add    $0x6c,%esi
    cprintf("pid %d %s: trap %d err %d on cpu %d "
8010746d:	56                   	push   %esi
8010746e:	ff 70 10             	pushl  0x10(%eax)
80107471:	68 48 96 10 80       	push   $0x80109648
80107476:	e8 45 94 ff ff       	call   801008c0 <cprintf>
            tf->err, cpuid(), tf->eip, rcr2());
    myproc()->killed = 1;
8010747b:	83 c4 20             	add    $0x20,%esp
8010747e:	e8 3d ca ff ff       	call   80103ec0 <myproc>
80107483:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
  }

  // Force process exit if it has been killed and is in user space.
  // (If it is still executing in the kernel, let it keep running
  // until it gets to the regular system call return.)
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
8010748a:	e8 31 ca ff ff       	call   80103ec0 <myproc>
8010748f:	85 c0                	test   %eax,%eax
80107491:	74 1d                	je     801074b0 <trap+0xc0>
80107493:	e8 28 ca ff ff       	call   80103ec0 <myproc>
80107498:	8b 50 24             	mov    0x24(%eax),%edx
8010749b:	85 d2                	test   %edx,%edx
8010749d:	74 11                	je     801074b0 <trap+0xc0>
8010749f:	0f b7 43 3c          	movzwl 0x3c(%ebx),%eax
801074a3:	83 e0 03             	and    $0x3,%eax
801074a6:	66 83 f8 03          	cmp    $0x3,%ax
801074aa:	0f 84 60 01 00 00    	je     80107610 <trap+0x220>
    exit();

  // Force process to give up CPU on clock tick.
  // If interrupts were on while locks held, would need to check nlock.
  if(myproc() && myproc()->state == RUNNING &&
801074b0:	e8 0b ca ff ff       	call   80103ec0 <myproc>
801074b5:	85 c0                	test   %eax,%eax
801074b7:	74 0f                	je     801074c8 <trap+0xd8>
801074b9:	e8 02 ca ff ff       	call   80103ec0 <myproc>
801074be:	83 78 0c 04          	cmpl   $0x4,0xc(%eax)
801074c2:	0f 84 f8 00 00 00    	je     801075c0 <trap+0x1d0>
     tf->trapno == T_IRQ0+IRQ_TIMER)
    yield();

  // Check if the process has been killed since we yielded
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
801074c8:	e8 f3 c9 ff ff       	call   80103ec0 <myproc>
801074cd:	85 c0                	test   %eax,%eax
801074cf:	74 1d                	je     801074ee <trap+0xfe>
801074d1:	e8 ea c9 ff ff       	call   80103ec0 <myproc>
801074d6:	8b 40 24             	mov    0x24(%eax),%eax
801074d9:	85 c0                	test   %eax,%eax
801074db:	74 11                	je     801074ee <trap+0xfe>
801074dd:	0f b7 43 3c          	movzwl 0x3c(%ebx),%eax
801074e1:	83 e0 03             	and    $0x3,%eax
801074e4:	66 83 f8 03          	cmp    $0x3,%ax
801074e8:	0f 84 13 01 00 00    	je     80107601 <trap+0x211>
    exit();
}
801074ee:	8d 65 f4             	lea    -0xc(%ebp),%esp
801074f1:	5b                   	pop    %ebx
801074f2:	5e                   	pop    %esi
801074f3:	5f                   	pop    %edi
801074f4:	5d                   	pop    %ebp
801074f5:	c3                   	ret    
    ideintr();
801074f6:	e8 d5 b1 ff ff       	call   801026d0 <ideintr>
    lapiceoi();
801074fb:	e8 b0 b8 ff ff       	call   80102db0 <lapiceoi>
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
80107500:	e8 bb c9 ff ff       	call   80103ec0 <myproc>
80107505:	85 c0                	test   %eax,%eax
80107507:	75 8a                	jne    80107493 <trap+0xa3>
80107509:	eb a5                	jmp    801074b0 <trap+0xc0>
    if(cpuid() == 0){
8010750b:	e8 90 c9 ff ff       	call   80103ea0 <cpuid>
80107510:	85 c0                	test   %eax,%eax
80107512:	75 e7                	jne    801074fb <trap+0x10b>
      acquire(&tickslock);
80107514:	83 ec 0c             	sub    $0xc,%esp
80107517:	68 60 7e 11 80       	push   $0x80117e60
8010751c:	e8 ef e6 ff ff       	call   80105c10 <acquire>
      ticks++;
80107521:	a1 a0 86 11 80       	mov    0x801186a0,%eax
      wakeup(&ticks);
80107526:	c7 04 24 a0 86 11 80 	movl   $0x801186a0,(%esp)
      ticks++;
8010752d:	8d 70 01             	lea    0x1(%eax),%esi
80107530:	89 35 a0 86 11 80    	mov    %esi,0x801186a0
      wakeup(&ticks);
80107536:	e8 25 d1 ff ff       	call   80104660 <wakeup>
      release(&tickslock);
8010753b:	c7 04 24 60 7e 11 80 	movl   $0x80117e60,(%esp)
80107542:	e8 89 e7 ff ff       	call   80105cd0 <release>
      ageprocs(os_tick);
80107547:	89 34 24             	mov    %esi,(%esp)
8010754a:	e8 31 d7 ff ff       	call   80104c80 <ageprocs>
8010754f:	83 c4 10             	add    $0x10,%esp
    lapiceoi();
80107552:	eb a7                	jmp    801074fb <trap+0x10b>
    kbdintr();
80107554:	e8 17 b7 ff ff       	call   80102c70 <kbdintr>
    lapiceoi();
80107559:	e8 52 b8 ff ff       	call   80102db0 <lapiceoi>
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
8010755e:	e8 5d c9 ff ff       	call   80103ec0 <myproc>
80107563:	85 c0                	test   %eax,%eax
80107565:	0f 85 28 ff ff ff    	jne    80107493 <trap+0xa3>
8010756b:	e9 40 ff ff ff       	jmp    801074b0 <trap+0xc0>
    uartintr();
80107570:	e8 4b 02 00 00       	call   801077c0 <uartintr>
    lapiceoi();
80107575:	e8 36 b8 ff ff       	call   80102db0 <lapiceoi>
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
8010757a:	e8 41 c9 ff ff       	call   80103ec0 <myproc>
8010757f:	85 c0                	test   %eax,%eax
80107581:	0f 85 0c ff ff ff    	jne    80107493 <trap+0xa3>
80107587:	e9 24 ff ff ff       	jmp    801074b0 <trap+0xc0>
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
8010758c:	8b 7b 38             	mov    0x38(%ebx),%edi
8010758f:	0f b7 73 3c          	movzwl 0x3c(%ebx),%esi
80107593:	e8 08 c9 ff ff       	call   80103ea0 <cpuid>
80107598:	57                   	push   %edi
80107599:	56                   	push   %esi
8010759a:	50                   	push   %eax
8010759b:	68 f0 95 10 80       	push   $0x801095f0
801075a0:	e8 1b 93 ff ff       	call   801008c0 <cprintf>
    lapiceoi();
801075a5:	e8 06 b8 ff ff       	call   80102db0 <lapiceoi>
    break;
801075aa:	83 c4 10             	add    $0x10,%esp
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
801075ad:	e8 0e c9 ff ff       	call   80103ec0 <myproc>
801075b2:	85 c0                	test   %eax,%eax
801075b4:	0f 85 d9 fe ff ff    	jne    80107493 <trap+0xa3>
801075ba:	e9 f1 fe ff ff       	jmp    801074b0 <trap+0xc0>
801075bf:	90                   	nop
  if(myproc() && myproc()->state == RUNNING &&
801075c0:	83 7b 30 20          	cmpl   $0x20,0x30(%ebx)
801075c4:	0f 85 fe fe ff ff    	jne    801074c8 <trap+0xd8>
    yield();
801075ca:	e8 81 ce ff ff       	call   80104450 <yield>
801075cf:	e9 f4 fe ff ff       	jmp    801074c8 <trap+0xd8>
801075d4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    if(myproc()->killed)
801075d8:	e8 e3 c8 ff ff       	call   80103ec0 <myproc>
801075dd:	8b 70 24             	mov    0x24(%eax),%esi
801075e0:	85 f6                	test   %esi,%esi
801075e2:	75 3c                	jne    80107620 <trap+0x230>
    myproc()->tf = tf;
801075e4:	e8 d7 c8 ff ff       	call   80103ec0 <myproc>
801075e9:	89 58 18             	mov    %ebx,0x18(%eax)
    syscall();
801075ec:	e8 8f eb ff ff       	call   80106180 <syscall>
    if(myproc()->killed)
801075f1:	e8 ca c8 ff ff       	call   80103ec0 <myproc>
801075f6:	8b 48 24             	mov    0x24(%eax),%ecx
801075f9:	85 c9                	test   %ecx,%ecx
801075fb:	0f 84 ed fe ff ff    	je     801074ee <trap+0xfe>
}
80107601:	8d 65 f4             	lea    -0xc(%ebp),%esp
80107604:	5b                   	pop    %ebx
80107605:	5e                   	pop    %esi
80107606:	5f                   	pop    %edi
80107607:	5d                   	pop    %ebp
      exit();
80107608:	e9 03 cd ff ff       	jmp    80104310 <exit>
8010760d:	8d 76 00             	lea    0x0(%esi),%esi
    exit();
80107610:	e8 fb cc ff ff       	call   80104310 <exit>
80107615:	e9 96 fe ff ff       	jmp    801074b0 <trap+0xc0>
8010761a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
      exit();
80107620:	e8 eb cc ff ff       	call   80104310 <exit>
80107625:	eb bd                	jmp    801075e4 <trap+0x1f4>
80107627:	0f 20 d6             	mov    %cr2,%esi
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
8010762a:	e8 71 c8 ff ff       	call   80103ea0 <cpuid>
8010762f:	83 ec 0c             	sub    $0xc,%esp
80107632:	56                   	push   %esi
80107633:	57                   	push   %edi
80107634:	50                   	push   %eax
80107635:	ff 73 30             	pushl  0x30(%ebx)
80107638:	68 14 96 10 80       	push   $0x80109614
8010763d:	e8 7e 92 ff ff       	call   801008c0 <cprintf>
      panic("trap");
80107642:	83 c4 14             	add    $0x14,%esp
80107645:	68 e8 95 10 80       	push   $0x801095e8
8010764a:	e8 41 8d ff ff       	call   80100390 <panic>
8010764f:	90                   	nop

80107650 <uartgetc>:
  outb(COM1+0, c);
}

static int
uartgetc(void)
{
80107650:	f3 0f 1e fb          	endbr32 
  if(!uart)
80107654:	a1 00 c6 10 80       	mov    0x8010c600,%eax
80107659:	85 c0                	test   %eax,%eax
8010765b:	74 1b                	je     80107678 <uartgetc+0x28>
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
8010765d:	ba fd 03 00 00       	mov    $0x3fd,%edx
80107662:	ec                   	in     (%dx),%al
    return -1;
  if(!(inb(COM1+5) & 0x01))
80107663:	a8 01                	test   $0x1,%al
80107665:	74 11                	je     80107678 <uartgetc+0x28>
80107667:	ba f8 03 00 00       	mov    $0x3f8,%edx
8010766c:	ec                   	in     (%dx),%al
    return -1;
  return inb(COM1+0);
8010766d:	0f b6 c0             	movzbl %al,%eax
80107670:	c3                   	ret    
80107671:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    return -1;
80107678:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010767d:	c3                   	ret    
8010767e:	66 90                	xchg   %ax,%ax

80107680 <uartputc.part.0>:
uartputc(int c)
80107680:	55                   	push   %ebp
80107681:	89 e5                	mov    %esp,%ebp
80107683:	57                   	push   %edi
80107684:	89 c7                	mov    %eax,%edi
80107686:	56                   	push   %esi
80107687:	be fd 03 00 00       	mov    $0x3fd,%esi
8010768c:	53                   	push   %ebx
8010768d:	bb 80 00 00 00       	mov    $0x80,%ebx
80107692:	83 ec 0c             	sub    $0xc,%esp
80107695:	eb 1b                	jmp    801076b2 <uartputc.part.0+0x32>
80107697:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010769e:	66 90                	xchg   %ax,%ax
    microdelay(10);
801076a0:	83 ec 0c             	sub    $0xc,%esp
801076a3:	6a 0a                	push   $0xa
801076a5:	e8 26 b7 ff ff       	call   80102dd0 <microdelay>
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
801076aa:	83 c4 10             	add    $0x10,%esp
801076ad:	83 eb 01             	sub    $0x1,%ebx
801076b0:	74 07                	je     801076b9 <uartputc.part.0+0x39>
801076b2:	89 f2                	mov    %esi,%edx
801076b4:	ec                   	in     (%dx),%al
801076b5:	a8 20                	test   $0x20,%al
801076b7:	74 e7                	je     801076a0 <uartputc.part.0+0x20>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801076b9:	ba f8 03 00 00       	mov    $0x3f8,%edx
801076be:	89 f8                	mov    %edi,%eax
801076c0:	ee                   	out    %al,(%dx)
}
801076c1:	8d 65 f4             	lea    -0xc(%ebp),%esp
801076c4:	5b                   	pop    %ebx
801076c5:	5e                   	pop    %esi
801076c6:	5f                   	pop    %edi
801076c7:	5d                   	pop    %ebp
801076c8:	c3                   	ret    
801076c9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

801076d0 <uartinit>:
{
801076d0:	f3 0f 1e fb          	endbr32 
801076d4:	55                   	push   %ebp
801076d5:	31 c9                	xor    %ecx,%ecx
801076d7:	89 c8                	mov    %ecx,%eax
801076d9:	89 e5                	mov    %esp,%ebp
801076db:	57                   	push   %edi
801076dc:	56                   	push   %esi
801076dd:	53                   	push   %ebx
801076de:	bb fa 03 00 00       	mov    $0x3fa,%ebx
801076e3:	89 da                	mov    %ebx,%edx
801076e5:	83 ec 0c             	sub    $0xc,%esp
801076e8:	ee                   	out    %al,(%dx)
801076e9:	bf fb 03 00 00       	mov    $0x3fb,%edi
801076ee:	b8 80 ff ff ff       	mov    $0xffffff80,%eax
801076f3:	89 fa                	mov    %edi,%edx
801076f5:	ee                   	out    %al,(%dx)
801076f6:	b8 0c 00 00 00       	mov    $0xc,%eax
801076fb:	ba f8 03 00 00       	mov    $0x3f8,%edx
80107700:	ee                   	out    %al,(%dx)
80107701:	be f9 03 00 00       	mov    $0x3f9,%esi
80107706:	89 c8                	mov    %ecx,%eax
80107708:	89 f2                	mov    %esi,%edx
8010770a:	ee                   	out    %al,(%dx)
8010770b:	b8 03 00 00 00       	mov    $0x3,%eax
80107710:	89 fa                	mov    %edi,%edx
80107712:	ee                   	out    %al,(%dx)
80107713:	ba fc 03 00 00       	mov    $0x3fc,%edx
80107718:	89 c8                	mov    %ecx,%eax
8010771a:	ee                   	out    %al,(%dx)
8010771b:	b8 01 00 00 00       	mov    $0x1,%eax
80107720:	89 f2                	mov    %esi,%edx
80107722:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80107723:	ba fd 03 00 00       	mov    $0x3fd,%edx
80107728:	ec                   	in     (%dx),%al
  if(inb(COM1+5) == 0xFF)
80107729:	3c ff                	cmp    $0xff,%al
8010772b:	74 52                	je     8010777f <uartinit+0xaf>
  uart = 1;
8010772d:	c7 05 00 c6 10 80 01 	movl   $0x1,0x8010c600
80107734:	00 00 00 
80107737:	89 da                	mov    %ebx,%edx
80107739:	ec                   	in     (%dx),%al
8010773a:	ba f8 03 00 00       	mov    $0x3f8,%edx
8010773f:	ec                   	in     (%dx),%al
  ioapicenable(IRQ_COM1, 0);
80107740:	83 ec 08             	sub    $0x8,%esp
80107743:	be 76 00 00 00       	mov    $0x76,%esi
  for(p="xv6...\n"; *p; p++)
80107748:	bb 0c 97 10 80       	mov    $0x8010970c,%ebx
  ioapicenable(IRQ_COM1, 0);
8010774d:	6a 00                	push   $0x0
8010774f:	6a 04                	push   $0x4
80107751:	e8 ca b1 ff ff       	call   80102920 <ioapicenable>
80107756:	83 c4 10             	add    $0x10,%esp
  for(p="xv6...\n"; *p; p++)
80107759:	b8 78 00 00 00       	mov    $0x78,%eax
8010775e:	eb 04                	jmp    80107764 <uartinit+0x94>
80107760:	0f b6 73 01          	movzbl 0x1(%ebx),%esi
  if(!uart)
80107764:	8b 15 00 c6 10 80    	mov    0x8010c600,%edx
8010776a:	85 d2                	test   %edx,%edx
8010776c:	74 08                	je     80107776 <uartinit+0xa6>
    uartputc(*p);
8010776e:	0f be c0             	movsbl %al,%eax
80107771:	e8 0a ff ff ff       	call   80107680 <uartputc.part.0>
  for(p="xv6...\n"; *p; p++)
80107776:	89 f0                	mov    %esi,%eax
80107778:	83 c3 01             	add    $0x1,%ebx
8010777b:	84 c0                	test   %al,%al
8010777d:	75 e1                	jne    80107760 <uartinit+0x90>
}
8010777f:	8d 65 f4             	lea    -0xc(%ebp),%esp
80107782:	5b                   	pop    %ebx
80107783:	5e                   	pop    %esi
80107784:	5f                   	pop    %edi
80107785:	5d                   	pop    %ebp
80107786:	c3                   	ret    
80107787:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010778e:	66 90                	xchg   %ax,%ax

80107790 <uartputc>:
{
80107790:	f3 0f 1e fb          	endbr32 
80107794:	55                   	push   %ebp
  if(!uart)
80107795:	8b 15 00 c6 10 80    	mov    0x8010c600,%edx
{
8010779b:	89 e5                	mov    %esp,%ebp
8010779d:	8b 45 08             	mov    0x8(%ebp),%eax
  if(!uart)
801077a0:	85 d2                	test   %edx,%edx
801077a2:	74 0c                	je     801077b0 <uartputc+0x20>
}
801077a4:	5d                   	pop    %ebp
801077a5:	e9 d6 fe ff ff       	jmp    80107680 <uartputc.part.0>
801077aa:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801077b0:	5d                   	pop    %ebp
801077b1:	c3                   	ret    
801077b2:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801077b9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

801077c0 <uartintr>:

void
uartintr(void)
{
801077c0:	f3 0f 1e fb          	endbr32 
801077c4:	55                   	push   %ebp
801077c5:	89 e5                	mov    %esp,%ebp
801077c7:	83 ec 14             	sub    $0x14,%esp
  consoleintr(uartgetc);
801077ca:	68 50 76 10 80       	push   $0x80107650
801077cf:	e8 9c 92 ff ff       	call   80100a70 <consoleintr>
}
801077d4:	83 c4 10             	add    $0x10,%esp
801077d7:	c9                   	leave  
801077d8:	c3                   	ret    

801077d9 <vector0>:
# generated by vectors.pl - do not edit
# handlers
.globl alltraps
.globl vector0
vector0:
  pushl $0
801077d9:	6a 00                	push   $0x0
  pushl $0
801077db:	6a 00                	push   $0x0
  jmp alltraps
801077dd:	e9 2d fb ff ff       	jmp    8010730f <alltraps>

801077e2 <vector1>:
.globl vector1
vector1:
  pushl $0
801077e2:	6a 00                	push   $0x0
  pushl $1
801077e4:	6a 01                	push   $0x1
  jmp alltraps
801077e6:	e9 24 fb ff ff       	jmp    8010730f <alltraps>

801077eb <vector2>:
.globl vector2
vector2:
  pushl $0
801077eb:	6a 00                	push   $0x0
  pushl $2
801077ed:	6a 02                	push   $0x2
  jmp alltraps
801077ef:	e9 1b fb ff ff       	jmp    8010730f <alltraps>

801077f4 <vector3>:
.globl vector3
vector3:
  pushl $0
801077f4:	6a 00                	push   $0x0
  pushl $3
801077f6:	6a 03                	push   $0x3
  jmp alltraps
801077f8:	e9 12 fb ff ff       	jmp    8010730f <alltraps>

801077fd <vector4>:
.globl vector4
vector4:
  pushl $0
801077fd:	6a 00                	push   $0x0
  pushl $4
801077ff:	6a 04                	push   $0x4
  jmp alltraps
80107801:	e9 09 fb ff ff       	jmp    8010730f <alltraps>

80107806 <vector5>:
.globl vector5
vector5:
  pushl $0
80107806:	6a 00                	push   $0x0
  pushl $5
80107808:	6a 05                	push   $0x5
  jmp alltraps
8010780a:	e9 00 fb ff ff       	jmp    8010730f <alltraps>

8010780f <vector6>:
.globl vector6
vector6:
  pushl $0
8010780f:	6a 00                	push   $0x0
  pushl $6
80107811:	6a 06                	push   $0x6
  jmp alltraps
80107813:	e9 f7 fa ff ff       	jmp    8010730f <alltraps>

80107818 <vector7>:
.globl vector7
vector7:
  pushl $0
80107818:	6a 00                	push   $0x0
  pushl $7
8010781a:	6a 07                	push   $0x7
  jmp alltraps
8010781c:	e9 ee fa ff ff       	jmp    8010730f <alltraps>

80107821 <vector8>:
.globl vector8
vector8:
  pushl $8
80107821:	6a 08                	push   $0x8
  jmp alltraps
80107823:	e9 e7 fa ff ff       	jmp    8010730f <alltraps>

80107828 <vector9>:
.globl vector9
vector9:
  pushl $0
80107828:	6a 00                	push   $0x0
  pushl $9
8010782a:	6a 09                	push   $0x9
  jmp alltraps
8010782c:	e9 de fa ff ff       	jmp    8010730f <alltraps>

80107831 <vector10>:
.globl vector10
vector10:
  pushl $10
80107831:	6a 0a                	push   $0xa
  jmp alltraps
80107833:	e9 d7 fa ff ff       	jmp    8010730f <alltraps>

80107838 <vector11>:
.globl vector11
vector11:
  pushl $11
80107838:	6a 0b                	push   $0xb
  jmp alltraps
8010783a:	e9 d0 fa ff ff       	jmp    8010730f <alltraps>

8010783f <vector12>:
.globl vector12
vector12:
  pushl $12
8010783f:	6a 0c                	push   $0xc
  jmp alltraps
80107841:	e9 c9 fa ff ff       	jmp    8010730f <alltraps>

80107846 <vector13>:
.globl vector13
vector13:
  pushl $13
80107846:	6a 0d                	push   $0xd
  jmp alltraps
80107848:	e9 c2 fa ff ff       	jmp    8010730f <alltraps>

8010784d <vector14>:
.globl vector14
vector14:
  pushl $14
8010784d:	6a 0e                	push   $0xe
  jmp alltraps
8010784f:	e9 bb fa ff ff       	jmp    8010730f <alltraps>

80107854 <vector15>:
.globl vector15
vector15:
  pushl $0
80107854:	6a 00                	push   $0x0
  pushl $15
80107856:	6a 0f                	push   $0xf
  jmp alltraps
80107858:	e9 b2 fa ff ff       	jmp    8010730f <alltraps>

8010785d <vector16>:
.globl vector16
vector16:
  pushl $0
8010785d:	6a 00                	push   $0x0
  pushl $16
8010785f:	6a 10                	push   $0x10
  jmp alltraps
80107861:	e9 a9 fa ff ff       	jmp    8010730f <alltraps>

80107866 <vector17>:
.globl vector17
vector17:
  pushl $17
80107866:	6a 11                	push   $0x11
  jmp alltraps
80107868:	e9 a2 fa ff ff       	jmp    8010730f <alltraps>

8010786d <vector18>:
.globl vector18
vector18:
  pushl $0
8010786d:	6a 00                	push   $0x0
  pushl $18
8010786f:	6a 12                	push   $0x12
  jmp alltraps
80107871:	e9 99 fa ff ff       	jmp    8010730f <alltraps>

80107876 <vector19>:
.globl vector19
vector19:
  pushl $0
80107876:	6a 00                	push   $0x0
  pushl $19
80107878:	6a 13                	push   $0x13
  jmp alltraps
8010787a:	e9 90 fa ff ff       	jmp    8010730f <alltraps>

8010787f <vector20>:
.globl vector20
vector20:
  pushl $0
8010787f:	6a 00                	push   $0x0
  pushl $20
80107881:	6a 14                	push   $0x14
  jmp alltraps
80107883:	e9 87 fa ff ff       	jmp    8010730f <alltraps>

80107888 <vector21>:
.globl vector21
vector21:
  pushl $0
80107888:	6a 00                	push   $0x0
  pushl $21
8010788a:	6a 15                	push   $0x15
  jmp alltraps
8010788c:	e9 7e fa ff ff       	jmp    8010730f <alltraps>

80107891 <vector22>:
.globl vector22
vector22:
  pushl $0
80107891:	6a 00                	push   $0x0
  pushl $22
80107893:	6a 16                	push   $0x16
  jmp alltraps
80107895:	e9 75 fa ff ff       	jmp    8010730f <alltraps>

8010789a <vector23>:
.globl vector23
vector23:
  pushl $0
8010789a:	6a 00                	push   $0x0
  pushl $23
8010789c:	6a 17                	push   $0x17
  jmp alltraps
8010789e:	e9 6c fa ff ff       	jmp    8010730f <alltraps>

801078a3 <vector24>:
.globl vector24
vector24:
  pushl $0
801078a3:	6a 00                	push   $0x0
  pushl $24
801078a5:	6a 18                	push   $0x18
  jmp alltraps
801078a7:	e9 63 fa ff ff       	jmp    8010730f <alltraps>

801078ac <vector25>:
.globl vector25
vector25:
  pushl $0
801078ac:	6a 00                	push   $0x0
  pushl $25
801078ae:	6a 19                	push   $0x19
  jmp alltraps
801078b0:	e9 5a fa ff ff       	jmp    8010730f <alltraps>

801078b5 <vector26>:
.globl vector26
vector26:
  pushl $0
801078b5:	6a 00                	push   $0x0
  pushl $26
801078b7:	6a 1a                	push   $0x1a
  jmp alltraps
801078b9:	e9 51 fa ff ff       	jmp    8010730f <alltraps>

801078be <vector27>:
.globl vector27
vector27:
  pushl $0
801078be:	6a 00                	push   $0x0
  pushl $27
801078c0:	6a 1b                	push   $0x1b
  jmp alltraps
801078c2:	e9 48 fa ff ff       	jmp    8010730f <alltraps>

801078c7 <vector28>:
.globl vector28
vector28:
  pushl $0
801078c7:	6a 00                	push   $0x0
  pushl $28
801078c9:	6a 1c                	push   $0x1c
  jmp alltraps
801078cb:	e9 3f fa ff ff       	jmp    8010730f <alltraps>

801078d0 <vector29>:
.globl vector29
vector29:
  pushl $0
801078d0:	6a 00                	push   $0x0
  pushl $29
801078d2:	6a 1d                	push   $0x1d
  jmp alltraps
801078d4:	e9 36 fa ff ff       	jmp    8010730f <alltraps>

801078d9 <vector30>:
.globl vector30
vector30:
  pushl $0
801078d9:	6a 00                	push   $0x0
  pushl $30
801078db:	6a 1e                	push   $0x1e
  jmp alltraps
801078dd:	e9 2d fa ff ff       	jmp    8010730f <alltraps>

801078e2 <vector31>:
.globl vector31
vector31:
  pushl $0
801078e2:	6a 00                	push   $0x0
  pushl $31
801078e4:	6a 1f                	push   $0x1f
  jmp alltraps
801078e6:	e9 24 fa ff ff       	jmp    8010730f <alltraps>

801078eb <vector32>:
.globl vector32
vector32:
  pushl $0
801078eb:	6a 00                	push   $0x0
  pushl $32
801078ed:	6a 20                	push   $0x20
  jmp alltraps
801078ef:	e9 1b fa ff ff       	jmp    8010730f <alltraps>

801078f4 <vector33>:
.globl vector33
vector33:
  pushl $0
801078f4:	6a 00                	push   $0x0
  pushl $33
801078f6:	6a 21                	push   $0x21
  jmp alltraps
801078f8:	e9 12 fa ff ff       	jmp    8010730f <alltraps>

801078fd <vector34>:
.globl vector34
vector34:
  pushl $0
801078fd:	6a 00                	push   $0x0
  pushl $34
801078ff:	6a 22                	push   $0x22
  jmp alltraps
80107901:	e9 09 fa ff ff       	jmp    8010730f <alltraps>

80107906 <vector35>:
.globl vector35
vector35:
  pushl $0
80107906:	6a 00                	push   $0x0
  pushl $35
80107908:	6a 23                	push   $0x23
  jmp alltraps
8010790a:	e9 00 fa ff ff       	jmp    8010730f <alltraps>

8010790f <vector36>:
.globl vector36
vector36:
  pushl $0
8010790f:	6a 00                	push   $0x0
  pushl $36
80107911:	6a 24                	push   $0x24
  jmp alltraps
80107913:	e9 f7 f9 ff ff       	jmp    8010730f <alltraps>

80107918 <vector37>:
.globl vector37
vector37:
  pushl $0
80107918:	6a 00                	push   $0x0
  pushl $37
8010791a:	6a 25                	push   $0x25
  jmp alltraps
8010791c:	e9 ee f9 ff ff       	jmp    8010730f <alltraps>

80107921 <vector38>:
.globl vector38
vector38:
  pushl $0
80107921:	6a 00                	push   $0x0
  pushl $38
80107923:	6a 26                	push   $0x26
  jmp alltraps
80107925:	e9 e5 f9 ff ff       	jmp    8010730f <alltraps>

8010792a <vector39>:
.globl vector39
vector39:
  pushl $0
8010792a:	6a 00                	push   $0x0
  pushl $39
8010792c:	6a 27                	push   $0x27
  jmp alltraps
8010792e:	e9 dc f9 ff ff       	jmp    8010730f <alltraps>

80107933 <vector40>:
.globl vector40
vector40:
  pushl $0
80107933:	6a 00                	push   $0x0
  pushl $40
80107935:	6a 28                	push   $0x28
  jmp alltraps
80107937:	e9 d3 f9 ff ff       	jmp    8010730f <alltraps>

8010793c <vector41>:
.globl vector41
vector41:
  pushl $0
8010793c:	6a 00                	push   $0x0
  pushl $41
8010793e:	6a 29                	push   $0x29
  jmp alltraps
80107940:	e9 ca f9 ff ff       	jmp    8010730f <alltraps>

80107945 <vector42>:
.globl vector42
vector42:
  pushl $0
80107945:	6a 00                	push   $0x0
  pushl $42
80107947:	6a 2a                	push   $0x2a
  jmp alltraps
80107949:	e9 c1 f9 ff ff       	jmp    8010730f <alltraps>

8010794e <vector43>:
.globl vector43
vector43:
  pushl $0
8010794e:	6a 00                	push   $0x0
  pushl $43
80107950:	6a 2b                	push   $0x2b
  jmp alltraps
80107952:	e9 b8 f9 ff ff       	jmp    8010730f <alltraps>

80107957 <vector44>:
.globl vector44
vector44:
  pushl $0
80107957:	6a 00                	push   $0x0
  pushl $44
80107959:	6a 2c                	push   $0x2c
  jmp alltraps
8010795b:	e9 af f9 ff ff       	jmp    8010730f <alltraps>

80107960 <vector45>:
.globl vector45
vector45:
  pushl $0
80107960:	6a 00                	push   $0x0
  pushl $45
80107962:	6a 2d                	push   $0x2d
  jmp alltraps
80107964:	e9 a6 f9 ff ff       	jmp    8010730f <alltraps>

80107969 <vector46>:
.globl vector46
vector46:
  pushl $0
80107969:	6a 00                	push   $0x0
  pushl $46
8010796b:	6a 2e                	push   $0x2e
  jmp alltraps
8010796d:	e9 9d f9 ff ff       	jmp    8010730f <alltraps>

80107972 <vector47>:
.globl vector47
vector47:
  pushl $0
80107972:	6a 00                	push   $0x0
  pushl $47
80107974:	6a 2f                	push   $0x2f
  jmp alltraps
80107976:	e9 94 f9 ff ff       	jmp    8010730f <alltraps>

8010797b <vector48>:
.globl vector48
vector48:
  pushl $0
8010797b:	6a 00                	push   $0x0
  pushl $48
8010797d:	6a 30                	push   $0x30
  jmp alltraps
8010797f:	e9 8b f9 ff ff       	jmp    8010730f <alltraps>

80107984 <vector49>:
.globl vector49
vector49:
  pushl $0
80107984:	6a 00                	push   $0x0
  pushl $49
80107986:	6a 31                	push   $0x31
  jmp alltraps
80107988:	e9 82 f9 ff ff       	jmp    8010730f <alltraps>

8010798d <vector50>:
.globl vector50
vector50:
  pushl $0
8010798d:	6a 00                	push   $0x0
  pushl $50
8010798f:	6a 32                	push   $0x32
  jmp alltraps
80107991:	e9 79 f9 ff ff       	jmp    8010730f <alltraps>

80107996 <vector51>:
.globl vector51
vector51:
  pushl $0
80107996:	6a 00                	push   $0x0
  pushl $51
80107998:	6a 33                	push   $0x33
  jmp alltraps
8010799a:	e9 70 f9 ff ff       	jmp    8010730f <alltraps>

8010799f <vector52>:
.globl vector52
vector52:
  pushl $0
8010799f:	6a 00                	push   $0x0
  pushl $52
801079a1:	6a 34                	push   $0x34
  jmp alltraps
801079a3:	e9 67 f9 ff ff       	jmp    8010730f <alltraps>

801079a8 <vector53>:
.globl vector53
vector53:
  pushl $0
801079a8:	6a 00                	push   $0x0
  pushl $53
801079aa:	6a 35                	push   $0x35
  jmp alltraps
801079ac:	e9 5e f9 ff ff       	jmp    8010730f <alltraps>

801079b1 <vector54>:
.globl vector54
vector54:
  pushl $0
801079b1:	6a 00                	push   $0x0
  pushl $54
801079b3:	6a 36                	push   $0x36
  jmp alltraps
801079b5:	e9 55 f9 ff ff       	jmp    8010730f <alltraps>

801079ba <vector55>:
.globl vector55
vector55:
  pushl $0
801079ba:	6a 00                	push   $0x0
  pushl $55
801079bc:	6a 37                	push   $0x37
  jmp alltraps
801079be:	e9 4c f9 ff ff       	jmp    8010730f <alltraps>

801079c3 <vector56>:
.globl vector56
vector56:
  pushl $0
801079c3:	6a 00                	push   $0x0
  pushl $56
801079c5:	6a 38                	push   $0x38
  jmp alltraps
801079c7:	e9 43 f9 ff ff       	jmp    8010730f <alltraps>

801079cc <vector57>:
.globl vector57
vector57:
  pushl $0
801079cc:	6a 00                	push   $0x0
  pushl $57
801079ce:	6a 39                	push   $0x39
  jmp alltraps
801079d0:	e9 3a f9 ff ff       	jmp    8010730f <alltraps>

801079d5 <vector58>:
.globl vector58
vector58:
  pushl $0
801079d5:	6a 00                	push   $0x0
  pushl $58
801079d7:	6a 3a                	push   $0x3a
  jmp alltraps
801079d9:	e9 31 f9 ff ff       	jmp    8010730f <alltraps>

801079de <vector59>:
.globl vector59
vector59:
  pushl $0
801079de:	6a 00                	push   $0x0
  pushl $59
801079e0:	6a 3b                	push   $0x3b
  jmp alltraps
801079e2:	e9 28 f9 ff ff       	jmp    8010730f <alltraps>

801079e7 <vector60>:
.globl vector60
vector60:
  pushl $0
801079e7:	6a 00                	push   $0x0
  pushl $60
801079e9:	6a 3c                	push   $0x3c
  jmp alltraps
801079eb:	e9 1f f9 ff ff       	jmp    8010730f <alltraps>

801079f0 <vector61>:
.globl vector61
vector61:
  pushl $0
801079f0:	6a 00                	push   $0x0
  pushl $61
801079f2:	6a 3d                	push   $0x3d
  jmp alltraps
801079f4:	e9 16 f9 ff ff       	jmp    8010730f <alltraps>

801079f9 <vector62>:
.globl vector62
vector62:
  pushl $0
801079f9:	6a 00                	push   $0x0
  pushl $62
801079fb:	6a 3e                	push   $0x3e
  jmp alltraps
801079fd:	e9 0d f9 ff ff       	jmp    8010730f <alltraps>

80107a02 <vector63>:
.globl vector63
vector63:
  pushl $0
80107a02:	6a 00                	push   $0x0
  pushl $63
80107a04:	6a 3f                	push   $0x3f
  jmp alltraps
80107a06:	e9 04 f9 ff ff       	jmp    8010730f <alltraps>

80107a0b <vector64>:
.globl vector64
vector64:
  pushl $0
80107a0b:	6a 00                	push   $0x0
  pushl $64
80107a0d:	6a 40                	push   $0x40
  jmp alltraps
80107a0f:	e9 fb f8 ff ff       	jmp    8010730f <alltraps>

80107a14 <vector65>:
.globl vector65
vector65:
  pushl $0
80107a14:	6a 00                	push   $0x0
  pushl $65
80107a16:	6a 41                	push   $0x41
  jmp alltraps
80107a18:	e9 f2 f8 ff ff       	jmp    8010730f <alltraps>

80107a1d <vector66>:
.globl vector66
vector66:
  pushl $0
80107a1d:	6a 00                	push   $0x0
  pushl $66
80107a1f:	6a 42                	push   $0x42
  jmp alltraps
80107a21:	e9 e9 f8 ff ff       	jmp    8010730f <alltraps>

80107a26 <vector67>:
.globl vector67
vector67:
  pushl $0
80107a26:	6a 00                	push   $0x0
  pushl $67
80107a28:	6a 43                	push   $0x43
  jmp alltraps
80107a2a:	e9 e0 f8 ff ff       	jmp    8010730f <alltraps>

80107a2f <vector68>:
.globl vector68
vector68:
  pushl $0
80107a2f:	6a 00                	push   $0x0
  pushl $68
80107a31:	6a 44                	push   $0x44
  jmp alltraps
80107a33:	e9 d7 f8 ff ff       	jmp    8010730f <alltraps>

80107a38 <vector69>:
.globl vector69
vector69:
  pushl $0
80107a38:	6a 00                	push   $0x0
  pushl $69
80107a3a:	6a 45                	push   $0x45
  jmp alltraps
80107a3c:	e9 ce f8 ff ff       	jmp    8010730f <alltraps>

80107a41 <vector70>:
.globl vector70
vector70:
  pushl $0
80107a41:	6a 00                	push   $0x0
  pushl $70
80107a43:	6a 46                	push   $0x46
  jmp alltraps
80107a45:	e9 c5 f8 ff ff       	jmp    8010730f <alltraps>

80107a4a <vector71>:
.globl vector71
vector71:
  pushl $0
80107a4a:	6a 00                	push   $0x0
  pushl $71
80107a4c:	6a 47                	push   $0x47
  jmp alltraps
80107a4e:	e9 bc f8 ff ff       	jmp    8010730f <alltraps>

80107a53 <vector72>:
.globl vector72
vector72:
  pushl $0
80107a53:	6a 00                	push   $0x0
  pushl $72
80107a55:	6a 48                	push   $0x48
  jmp alltraps
80107a57:	e9 b3 f8 ff ff       	jmp    8010730f <alltraps>

80107a5c <vector73>:
.globl vector73
vector73:
  pushl $0
80107a5c:	6a 00                	push   $0x0
  pushl $73
80107a5e:	6a 49                	push   $0x49
  jmp alltraps
80107a60:	e9 aa f8 ff ff       	jmp    8010730f <alltraps>

80107a65 <vector74>:
.globl vector74
vector74:
  pushl $0
80107a65:	6a 00                	push   $0x0
  pushl $74
80107a67:	6a 4a                	push   $0x4a
  jmp alltraps
80107a69:	e9 a1 f8 ff ff       	jmp    8010730f <alltraps>

80107a6e <vector75>:
.globl vector75
vector75:
  pushl $0
80107a6e:	6a 00                	push   $0x0
  pushl $75
80107a70:	6a 4b                	push   $0x4b
  jmp alltraps
80107a72:	e9 98 f8 ff ff       	jmp    8010730f <alltraps>

80107a77 <vector76>:
.globl vector76
vector76:
  pushl $0
80107a77:	6a 00                	push   $0x0
  pushl $76
80107a79:	6a 4c                	push   $0x4c
  jmp alltraps
80107a7b:	e9 8f f8 ff ff       	jmp    8010730f <alltraps>

80107a80 <vector77>:
.globl vector77
vector77:
  pushl $0
80107a80:	6a 00                	push   $0x0
  pushl $77
80107a82:	6a 4d                	push   $0x4d
  jmp alltraps
80107a84:	e9 86 f8 ff ff       	jmp    8010730f <alltraps>

80107a89 <vector78>:
.globl vector78
vector78:
  pushl $0
80107a89:	6a 00                	push   $0x0
  pushl $78
80107a8b:	6a 4e                	push   $0x4e
  jmp alltraps
80107a8d:	e9 7d f8 ff ff       	jmp    8010730f <alltraps>

80107a92 <vector79>:
.globl vector79
vector79:
  pushl $0
80107a92:	6a 00                	push   $0x0
  pushl $79
80107a94:	6a 4f                	push   $0x4f
  jmp alltraps
80107a96:	e9 74 f8 ff ff       	jmp    8010730f <alltraps>

80107a9b <vector80>:
.globl vector80
vector80:
  pushl $0
80107a9b:	6a 00                	push   $0x0
  pushl $80
80107a9d:	6a 50                	push   $0x50
  jmp alltraps
80107a9f:	e9 6b f8 ff ff       	jmp    8010730f <alltraps>

80107aa4 <vector81>:
.globl vector81
vector81:
  pushl $0
80107aa4:	6a 00                	push   $0x0
  pushl $81
80107aa6:	6a 51                	push   $0x51
  jmp alltraps
80107aa8:	e9 62 f8 ff ff       	jmp    8010730f <alltraps>

80107aad <vector82>:
.globl vector82
vector82:
  pushl $0
80107aad:	6a 00                	push   $0x0
  pushl $82
80107aaf:	6a 52                	push   $0x52
  jmp alltraps
80107ab1:	e9 59 f8 ff ff       	jmp    8010730f <alltraps>

80107ab6 <vector83>:
.globl vector83
vector83:
  pushl $0
80107ab6:	6a 00                	push   $0x0
  pushl $83
80107ab8:	6a 53                	push   $0x53
  jmp alltraps
80107aba:	e9 50 f8 ff ff       	jmp    8010730f <alltraps>

80107abf <vector84>:
.globl vector84
vector84:
  pushl $0
80107abf:	6a 00                	push   $0x0
  pushl $84
80107ac1:	6a 54                	push   $0x54
  jmp alltraps
80107ac3:	e9 47 f8 ff ff       	jmp    8010730f <alltraps>

80107ac8 <vector85>:
.globl vector85
vector85:
  pushl $0
80107ac8:	6a 00                	push   $0x0
  pushl $85
80107aca:	6a 55                	push   $0x55
  jmp alltraps
80107acc:	e9 3e f8 ff ff       	jmp    8010730f <alltraps>

80107ad1 <vector86>:
.globl vector86
vector86:
  pushl $0
80107ad1:	6a 00                	push   $0x0
  pushl $86
80107ad3:	6a 56                	push   $0x56
  jmp alltraps
80107ad5:	e9 35 f8 ff ff       	jmp    8010730f <alltraps>

80107ada <vector87>:
.globl vector87
vector87:
  pushl $0
80107ada:	6a 00                	push   $0x0
  pushl $87
80107adc:	6a 57                	push   $0x57
  jmp alltraps
80107ade:	e9 2c f8 ff ff       	jmp    8010730f <alltraps>

80107ae3 <vector88>:
.globl vector88
vector88:
  pushl $0
80107ae3:	6a 00                	push   $0x0
  pushl $88
80107ae5:	6a 58                	push   $0x58
  jmp alltraps
80107ae7:	e9 23 f8 ff ff       	jmp    8010730f <alltraps>

80107aec <vector89>:
.globl vector89
vector89:
  pushl $0
80107aec:	6a 00                	push   $0x0
  pushl $89
80107aee:	6a 59                	push   $0x59
  jmp alltraps
80107af0:	e9 1a f8 ff ff       	jmp    8010730f <alltraps>

80107af5 <vector90>:
.globl vector90
vector90:
  pushl $0
80107af5:	6a 00                	push   $0x0
  pushl $90
80107af7:	6a 5a                	push   $0x5a
  jmp alltraps
80107af9:	e9 11 f8 ff ff       	jmp    8010730f <alltraps>

80107afe <vector91>:
.globl vector91
vector91:
  pushl $0
80107afe:	6a 00                	push   $0x0
  pushl $91
80107b00:	6a 5b                	push   $0x5b
  jmp alltraps
80107b02:	e9 08 f8 ff ff       	jmp    8010730f <alltraps>

80107b07 <vector92>:
.globl vector92
vector92:
  pushl $0
80107b07:	6a 00                	push   $0x0
  pushl $92
80107b09:	6a 5c                	push   $0x5c
  jmp alltraps
80107b0b:	e9 ff f7 ff ff       	jmp    8010730f <alltraps>

80107b10 <vector93>:
.globl vector93
vector93:
  pushl $0
80107b10:	6a 00                	push   $0x0
  pushl $93
80107b12:	6a 5d                	push   $0x5d
  jmp alltraps
80107b14:	e9 f6 f7 ff ff       	jmp    8010730f <alltraps>

80107b19 <vector94>:
.globl vector94
vector94:
  pushl $0
80107b19:	6a 00                	push   $0x0
  pushl $94
80107b1b:	6a 5e                	push   $0x5e
  jmp alltraps
80107b1d:	e9 ed f7 ff ff       	jmp    8010730f <alltraps>

80107b22 <vector95>:
.globl vector95
vector95:
  pushl $0
80107b22:	6a 00                	push   $0x0
  pushl $95
80107b24:	6a 5f                	push   $0x5f
  jmp alltraps
80107b26:	e9 e4 f7 ff ff       	jmp    8010730f <alltraps>

80107b2b <vector96>:
.globl vector96
vector96:
  pushl $0
80107b2b:	6a 00                	push   $0x0
  pushl $96
80107b2d:	6a 60                	push   $0x60
  jmp alltraps
80107b2f:	e9 db f7 ff ff       	jmp    8010730f <alltraps>

80107b34 <vector97>:
.globl vector97
vector97:
  pushl $0
80107b34:	6a 00                	push   $0x0
  pushl $97
80107b36:	6a 61                	push   $0x61
  jmp alltraps
80107b38:	e9 d2 f7 ff ff       	jmp    8010730f <alltraps>

80107b3d <vector98>:
.globl vector98
vector98:
  pushl $0
80107b3d:	6a 00                	push   $0x0
  pushl $98
80107b3f:	6a 62                	push   $0x62
  jmp alltraps
80107b41:	e9 c9 f7 ff ff       	jmp    8010730f <alltraps>

80107b46 <vector99>:
.globl vector99
vector99:
  pushl $0
80107b46:	6a 00                	push   $0x0
  pushl $99
80107b48:	6a 63                	push   $0x63
  jmp alltraps
80107b4a:	e9 c0 f7 ff ff       	jmp    8010730f <alltraps>

80107b4f <vector100>:
.globl vector100
vector100:
  pushl $0
80107b4f:	6a 00                	push   $0x0
  pushl $100
80107b51:	6a 64                	push   $0x64
  jmp alltraps
80107b53:	e9 b7 f7 ff ff       	jmp    8010730f <alltraps>

80107b58 <vector101>:
.globl vector101
vector101:
  pushl $0
80107b58:	6a 00                	push   $0x0
  pushl $101
80107b5a:	6a 65                	push   $0x65
  jmp alltraps
80107b5c:	e9 ae f7 ff ff       	jmp    8010730f <alltraps>

80107b61 <vector102>:
.globl vector102
vector102:
  pushl $0
80107b61:	6a 00                	push   $0x0
  pushl $102
80107b63:	6a 66                	push   $0x66
  jmp alltraps
80107b65:	e9 a5 f7 ff ff       	jmp    8010730f <alltraps>

80107b6a <vector103>:
.globl vector103
vector103:
  pushl $0
80107b6a:	6a 00                	push   $0x0
  pushl $103
80107b6c:	6a 67                	push   $0x67
  jmp alltraps
80107b6e:	e9 9c f7 ff ff       	jmp    8010730f <alltraps>

80107b73 <vector104>:
.globl vector104
vector104:
  pushl $0
80107b73:	6a 00                	push   $0x0
  pushl $104
80107b75:	6a 68                	push   $0x68
  jmp alltraps
80107b77:	e9 93 f7 ff ff       	jmp    8010730f <alltraps>

80107b7c <vector105>:
.globl vector105
vector105:
  pushl $0
80107b7c:	6a 00                	push   $0x0
  pushl $105
80107b7e:	6a 69                	push   $0x69
  jmp alltraps
80107b80:	e9 8a f7 ff ff       	jmp    8010730f <alltraps>

80107b85 <vector106>:
.globl vector106
vector106:
  pushl $0
80107b85:	6a 00                	push   $0x0
  pushl $106
80107b87:	6a 6a                	push   $0x6a
  jmp alltraps
80107b89:	e9 81 f7 ff ff       	jmp    8010730f <alltraps>

80107b8e <vector107>:
.globl vector107
vector107:
  pushl $0
80107b8e:	6a 00                	push   $0x0
  pushl $107
80107b90:	6a 6b                	push   $0x6b
  jmp alltraps
80107b92:	e9 78 f7 ff ff       	jmp    8010730f <alltraps>

80107b97 <vector108>:
.globl vector108
vector108:
  pushl $0
80107b97:	6a 00                	push   $0x0
  pushl $108
80107b99:	6a 6c                	push   $0x6c
  jmp alltraps
80107b9b:	e9 6f f7 ff ff       	jmp    8010730f <alltraps>

80107ba0 <vector109>:
.globl vector109
vector109:
  pushl $0
80107ba0:	6a 00                	push   $0x0
  pushl $109
80107ba2:	6a 6d                	push   $0x6d
  jmp alltraps
80107ba4:	e9 66 f7 ff ff       	jmp    8010730f <alltraps>

80107ba9 <vector110>:
.globl vector110
vector110:
  pushl $0
80107ba9:	6a 00                	push   $0x0
  pushl $110
80107bab:	6a 6e                	push   $0x6e
  jmp alltraps
80107bad:	e9 5d f7 ff ff       	jmp    8010730f <alltraps>

80107bb2 <vector111>:
.globl vector111
vector111:
  pushl $0
80107bb2:	6a 00                	push   $0x0
  pushl $111
80107bb4:	6a 6f                	push   $0x6f
  jmp alltraps
80107bb6:	e9 54 f7 ff ff       	jmp    8010730f <alltraps>

80107bbb <vector112>:
.globl vector112
vector112:
  pushl $0
80107bbb:	6a 00                	push   $0x0
  pushl $112
80107bbd:	6a 70                	push   $0x70
  jmp alltraps
80107bbf:	e9 4b f7 ff ff       	jmp    8010730f <alltraps>

80107bc4 <vector113>:
.globl vector113
vector113:
  pushl $0
80107bc4:	6a 00                	push   $0x0
  pushl $113
80107bc6:	6a 71                	push   $0x71
  jmp alltraps
80107bc8:	e9 42 f7 ff ff       	jmp    8010730f <alltraps>

80107bcd <vector114>:
.globl vector114
vector114:
  pushl $0
80107bcd:	6a 00                	push   $0x0
  pushl $114
80107bcf:	6a 72                	push   $0x72
  jmp alltraps
80107bd1:	e9 39 f7 ff ff       	jmp    8010730f <alltraps>

80107bd6 <vector115>:
.globl vector115
vector115:
  pushl $0
80107bd6:	6a 00                	push   $0x0
  pushl $115
80107bd8:	6a 73                	push   $0x73
  jmp alltraps
80107bda:	e9 30 f7 ff ff       	jmp    8010730f <alltraps>

80107bdf <vector116>:
.globl vector116
vector116:
  pushl $0
80107bdf:	6a 00                	push   $0x0
  pushl $116
80107be1:	6a 74                	push   $0x74
  jmp alltraps
80107be3:	e9 27 f7 ff ff       	jmp    8010730f <alltraps>

80107be8 <vector117>:
.globl vector117
vector117:
  pushl $0
80107be8:	6a 00                	push   $0x0
  pushl $117
80107bea:	6a 75                	push   $0x75
  jmp alltraps
80107bec:	e9 1e f7 ff ff       	jmp    8010730f <alltraps>

80107bf1 <vector118>:
.globl vector118
vector118:
  pushl $0
80107bf1:	6a 00                	push   $0x0
  pushl $118
80107bf3:	6a 76                	push   $0x76
  jmp alltraps
80107bf5:	e9 15 f7 ff ff       	jmp    8010730f <alltraps>

80107bfa <vector119>:
.globl vector119
vector119:
  pushl $0
80107bfa:	6a 00                	push   $0x0
  pushl $119
80107bfc:	6a 77                	push   $0x77
  jmp alltraps
80107bfe:	e9 0c f7 ff ff       	jmp    8010730f <alltraps>

80107c03 <vector120>:
.globl vector120
vector120:
  pushl $0
80107c03:	6a 00                	push   $0x0
  pushl $120
80107c05:	6a 78                	push   $0x78
  jmp alltraps
80107c07:	e9 03 f7 ff ff       	jmp    8010730f <alltraps>

80107c0c <vector121>:
.globl vector121
vector121:
  pushl $0
80107c0c:	6a 00                	push   $0x0
  pushl $121
80107c0e:	6a 79                	push   $0x79
  jmp alltraps
80107c10:	e9 fa f6 ff ff       	jmp    8010730f <alltraps>

80107c15 <vector122>:
.globl vector122
vector122:
  pushl $0
80107c15:	6a 00                	push   $0x0
  pushl $122
80107c17:	6a 7a                	push   $0x7a
  jmp alltraps
80107c19:	e9 f1 f6 ff ff       	jmp    8010730f <alltraps>

80107c1e <vector123>:
.globl vector123
vector123:
  pushl $0
80107c1e:	6a 00                	push   $0x0
  pushl $123
80107c20:	6a 7b                	push   $0x7b
  jmp alltraps
80107c22:	e9 e8 f6 ff ff       	jmp    8010730f <alltraps>

80107c27 <vector124>:
.globl vector124
vector124:
  pushl $0
80107c27:	6a 00                	push   $0x0
  pushl $124
80107c29:	6a 7c                	push   $0x7c
  jmp alltraps
80107c2b:	e9 df f6 ff ff       	jmp    8010730f <alltraps>

80107c30 <vector125>:
.globl vector125
vector125:
  pushl $0
80107c30:	6a 00                	push   $0x0
  pushl $125
80107c32:	6a 7d                	push   $0x7d
  jmp alltraps
80107c34:	e9 d6 f6 ff ff       	jmp    8010730f <alltraps>

80107c39 <vector126>:
.globl vector126
vector126:
  pushl $0
80107c39:	6a 00                	push   $0x0
  pushl $126
80107c3b:	6a 7e                	push   $0x7e
  jmp alltraps
80107c3d:	e9 cd f6 ff ff       	jmp    8010730f <alltraps>

80107c42 <vector127>:
.globl vector127
vector127:
  pushl $0
80107c42:	6a 00                	push   $0x0
  pushl $127
80107c44:	6a 7f                	push   $0x7f
  jmp alltraps
80107c46:	e9 c4 f6 ff ff       	jmp    8010730f <alltraps>

80107c4b <vector128>:
.globl vector128
vector128:
  pushl $0
80107c4b:	6a 00                	push   $0x0
  pushl $128
80107c4d:	68 80 00 00 00       	push   $0x80
  jmp alltraps
80107c52:	e9 b8 f6 ff ff       	jmp    8010730f <alltraps>

80107c57 <vector129>:
.globl vector129
vector129:
  pushl $0
80107c57:	6a 00                	push   $0x0
  pushl $129
80107c59:	68 81 00 00 00       	push   $0x81
  jmp alltraps
80107c5e:	e9 ac f6 ff ff       	jmp    8010730f <alltraps>

80107c63 <vector130>:
.globl vector130
vector130:
  pushl $0
80107c63:	6a 00                	push   $0x0
  pushl $130
80107c65:	68 82 00 00 00       	push   $0x82
  jmp alltraps
80107c6a:	e9 a0 f6 ff ff       	jmp    8010730f <alltraps>

80107c6f <vector131>:
.globl vector131
vector131:
  pushl $0
80107c6f:	6a 00                	push   $0x0
  pushl $131
80107c71:	68 83 00 00 00       	push   $0x83
  jmp alltraps
80107c76:	e9 94 f6 ff ff       	jmp    8010730f <alltraps>

80107c7b <vector132>:
.globl vector132
vector132:
  pushl $0
80107c7b:	6a 00                	push   $0x0
  pushl $132
80107c7d:	68 84 00 00 00       	push   $0x84
  jmp alltraps
80107c82:	e9 88 f6 ff ff       	jmp    8010730f <alltraps>

80107c87 <vector133>:
.globl vector133
vector133:
  pushl $0
80107c87:	6a 00                	push   $0x0
  pushl $133
80107c89:	68 85 00 00 00       	push   $0x85
  jmp alltraps
80107c8e:	e9 7c f6 ff ff       	jmp    8010730f <alltraps>

80107c93 <vector134>:
.globl vector134
vector134:
  pushl $0
80107c93:	6a 00                	push   $0x0
  pushl $134
80107c95:	68 86 00 00 00       	push   $0x86
  jmp alltraps
80107c9a:	e9 70 f6 ff ff       	jmp    8010730f <alltraps>

80107c9f <vector135>:
.globl vector135
vector135:
  pushl $0
80107c9f:	6a 00                	push   $0x0
  pushl $135
80107ca1:	68 87 00 00 00       	push   $0x87
  jmp alltraps
80107ca6:	e9 64 f6 ff ff       	jmp    8010730f <alltraps>

80107cab <vector136>:
.globl vector136
vector136:
  pushl $0
80107cab:	6a 00                	push   $0x0
  pushl $136
80107cad:	68 88 00 00 00       	push   $0x88
  jmp alltraps
80107cb2:	e9 58 f6 ff ff       	jmp    8010730f <alltraps>

80107cb7 <vector137>:
.globl vector137
vector137:
  pushl $0
80107cb7:	6a 00                	push   $0x0
  pushl $137
80107cb9:	68 89 00 00 00       	push   $0x89
  jmp alltraps
80107cbe:	e9 4c f6 ff ff       	jmp    8010730f <alltraps>

80107cc3 <vector138>:
.globl vector138
vector138:
  pushl $0
80107cc3:	6a 00                	push   $0x0
  pushl $138
80107cc5:	68 8a 00 00 00       	push   $0x8a
  jmp alltraps
80107cca:	e9 40 f6 ff ff       	jmp    8010730f <alltraps>

80107ccf <vector139>:
.globl vector139
vector139:
  pushl $0
80107ccf:	6a 00                	push   $0x0
  pushl $139
80107cd1:	68 8b 00 00 00       	push   $0x8b
  jmp alltraps
80107cd6:	e9 34 f6 ff ff       	jmp    8010730f <alltraps>

80107cdb <vector140>:
.globl vector140
vector140:
  pushl $0
80107cdb:	6a 00                	push   $0x0
  pushl $140
80107cdd:	68 8c 00 00 00       	push   $0x8c
  jmp alltraps
80107ce2:	e9 28 f6 ff ff       	jmp    8010730f <alltraps>

80107ce7 <vector141>:
.globl vector141
vector141:
  pushl $0
80107ce7:	6a 00                	push   $0x0
  pushl $141
80107ce9:	68 8d 00 00 00       	push   $0x8d
  jmp alltraps
80107cee:	e9 1c f6 ff ff       	jmp    8010730f <alltraps>

80107cf3 <vector142>:
.globl vector142
vector142:
  pushl $0
80107cf3:	6a 00                	push   $0x0
  pushl $142
80107cf5:	68 8e 00 00 00       	push   $0x8e
  jmp alltraps
80107cfa:	e9 10 f6 ff ff       	jmp    8010730f <alltraps>

80107cff <vector143>:
.globl vector143
vector143:
  pushl $0
80107cff:	6a 00                	push   $0x0
  pushl $143
80107d01:	68 8f 00 00 00       	push   $0x8f
  jmp alltraps
80107d06:	e9 04 f6 ff ff       	jmp    8010730f <alltraps>

80107d0b <vector144>:
.globl vector144
vector144:
  pushl $0
80107d0b:	6a 00                	push   $0x0
  pushl $144
80107d0d:	68 90 00 00 00       	push   $0x90
  jmp alltraps
80107d12:	e9 f8 f5 ff ff       	jmp    8010730f <alltraps>

80107d17 <vector145>:
.globl vector145
vector145:
  pushl $0
80107d17:	6a 00                	push   $0x0
  pushl $145
80107d19:	68 91 00 00 00       	push   $0x91
  jmp alltraps
80107d1e:	e9 ec f5 ff ff       	jmp    8010730f <alltraps>

80107d23 <vector146>:
.globl vector146
vector146:
  pushl $0
80107d23:	6a 00                	push   $0x0
  pushl $146
80107d25:	68 92 00 00 00       	push   $0x92
  jmp alltraps
80107d2a:	e9 e0 f5 ff ff       	jmp    8010730f <alltraps>

80107d2f <vector147>:
.globl vector147
vector147:
  pushl $0
80107d2f:	6a 00                	push   $0x0
  pushl $147
80107d31:	68 93 00 00 00       	push   $0x93
  jmp alltraps
80107d36:	e9 d4 f5 ff ff       	jmp    8010730f <alltraps>

80107d3b <vector148>:
.globl vector148
vector148:
  pushl $0
80107d3b:	6a 00                	push   $0x0
  pushl $148
80107d3d:	68 94 00 00 00       	push   $0x94
  jmp alltraps
80107d42:	e9 c8 f5 ff ff       	jmp    8010730f <alltraps>

80107d47 <vector149>:
.globl vector149
vector149:
  pushl $0
80107d47:	6a 00                	push   $0x0
  pushl $149
80107d49:	68 95 00 00 00       	push   $0x95
  jmp alltraps
80107d4e:	e9 bc f5 ff ff       	jmp    8010730f <alltraps>

80107d53 <vector150>:
.globl vector150
vector150:
  pushl $0
80107d53:	6a 00                	push   $0x0
  pushl $150
80107d55:	68 96 00 00 00       	push   $0x96
  jmp alltraps
80107d5a:	e9 b0 f5 ff ff       	jmp    8010730f <alltraps>

80107d5f <vector151>:
.globl vector151
vector151:
  pushl $0
80107d5f:	6a 00                	push   $0x0
  pushl $151
80107d61:	68 97 00 00 00       	push   $0x97
  jmp alltraps
80107d66:	e9 a4 f5 ff ff       	jmp    8010730f <alltraps>

80107d6b <vector152>:
.globl vector152
vector152:
  pushl $0
80107d6b:	6a 00                	push   $0x0
  pushl $152
80107d6d:	68 98 00 00 00       	push   $0x98
  jmp alltraps
80107d72:	e9 98 f5 ff ff       	jmp    8010730f <alltraps>

80107d77 <vector153>:
.globl vector153
vector153:
  pushl $0
80107d77:	6a 00                	push   $0x0
  pushl $153
80107d79:	68 99 00 00 00       	push   $0x99
  jmp alltraps
80107d7e:	e9 8c f5 ff ff       	jmp    8010730f <alltraps>

80107d83 <vector154>:
.globl vector154
vector154:
  pushl $0
80107d83:	6a 00                	push   $0x0
  pushl $154
80107d85:	68 9a 00 00 00       	push   $0x9a
  jmp alltraps
80107d8a:	e9 80 f5 ff ff       	jmp    8010730f <alltraps>

80107d8f <vector155>:
.globl vector155
vector155:
  pushl $0
80107d8f:	6a 00                	push   $0x0
  pushl $155
80107d91:	68 9b 00 00 00       	push   $0x9b
  jmp alltraps
80107d96:	e9 74 f5 ff ff       	jmp    8010730f <alltraps>

80107d9b <vector156>:
.globl vector156
vector156:
  pushl $0
80107d9b:	6a 00                	push   $0x0
  pushl $156
80107d9d:	68 9c 00 00 00       	push   $0x9c
  jmp alltraps
80107da2:	e9 68 f5 ff ff       	jmp    8010730f <alltraps>

80107da7 <vector157>:
.globl vector157
vector157:
  pushl $0
80107da7:	6a 00                	push   $0x0
  pushl $157
80107da9:	68 9d 00 00 00       	push   $0x9d
  jmp alltraps
80107dae:	e9 5c f5 ff ff       	jmp    8010730f <alltraps>

80107db3 <vector158>:
.globl vector158
vector158:
  pushl $0
80107db3:	6a 00                	push   $0x0
  pushl $158
80107db5:	68 9e 00 00 00       	push   $0x9e
  jmp alltraps
80107dba:	e9 50 f5 ff ff       	jmp    8010730f <alltraps>

80107dbf <vector159>:
.globl vector159
vector159:
  pushl $0
80107dbf:	6a 00                	push   $0x0
  pushl $159
80107dc1:	68 9f 00 00 00       	push   $0x9f
  jmp alltraps
80107dc6:	e9 44 f5 ff ff       	jmp    8010730f <alltraps>

80107dcb <vector160>:
.globl vector160
vector160:
  pushl $0
80107dcb:	6a 00                	push   $0x0
  pushl $160
80107dcd:	68 a0 00 00 00       	push   $0xa0
  jmp alltraps
80107dd2:	e9 38 f5 ff ff       	jmp    8010730f <alltraps>

80107dd7 <vector161>:
.globl vector161
vector161:
  pushl $0
80107dd7:	6a 00                	push   $0x0
  pushl $161
80107dd9:	68 a1 00 00 00       	push   $0xa1
  jmp alltraps
80107dde:	e9 2c f5 ff ff       	jmp    8010730f <alltraps>

80107de3 <vector162>:
.globl vector162
vector162:
  pushl $0
80107de3:	6a 00                	push   $0x0
  pushl $162
80107de5:	68 a2 00 00 00       	push   $0xa2
  jmp alltraps
80107dea:	e9 20 f5 ff ff       	jmp    8010730f <alltraps>

80107def <vector163>:
.globl vector163
vector163:
  pushl $0
80107def:	6a 00                	push   $0x0
  pushl $163
80107df1:	68 a3 00 00 00       	push   $0xa3
  jmp alltraps
80107df6:	e9 14 f5 ff ff       	jmp    8010730f <alltraps>

80107dfb <vector164>:
.globl vector164
vector164:
  pushl $0
80107dfb:	6a 00                	push   $0x0
  pushl $164
80107dfd:	68 a4 00 00 00       	push   $0xa4
  jmp alltraps
80107e02:	e9 08 f5 ff ff       	jmp    8010730f <alltraps>

80107e07 <vector165>:
.globl vector165
vector165:
  pushl $0
80107e07:	6a 00                	push   $0x0
  pushl $165
80107e09:	68 a5 00 00 00       	push   $0xa5
  jmp alltraps
80107e0e:	e9 fc f4 ff ff       	jmp    8010730f <alltraps>

80107e13 <vector166>:
.globl vector166
vector166:
  pushl $0
80107e13:	6a 00                	push   $0x0
  pushl $166
80107e15:	68 a6 00 00 00       	push   $0xa6
  jmp alltraps
80107e1a:	e9 f0 f4 ff ff       	jmp    8010730f <alltraps>

80107e1f <vector167>:
.globl vector167
vector167:
  pushl $0
80107e1f:	6a 00                	push   $0x0
  pushl $167
80107e21:	68 a7 00 00 00       	push   $0xa7
  jmp alltraps
80107e26:	e9 e4 f4 ff ff       	jmp    8010730f <alltraps>

80107e2b <vector168>:
.globl vector168
vector168:
  pushl $0
80107e2b:	6a 00                	push   $0x0
  pushl $168
80107e2d:	68 a8 00 00 00       	push   $0xa8
  jmp alltraps
80107e32:	e9 d8 f4 ff ff       	jmp    8010730f <alltraps>

80107e37 <vector169>:
.globl vector169
vector169:
  pushl $0
80107e37:	6a 00                	push   $0x0
  pushl $169
80107e39:	68 a9 00 00 00       	push   $0xa9
  jmp alltraps
80107e3e:	e9 cc f4 ff ff       	jmp    8010730f <alltraps>

80107e43 <vector170>:
.globl vector170
vector170:
  pushl $0
80107e43:	6a 00                	push   $0x0
  pushl $170
80107e45:	68 aa 00 00 00       	push   $0xaa
  jmp alltraps
80107e4a:	e9 c0 f4 ff ff       	jmp    8010730f <alltraps>

80107e4f <vector171>:
.globl vector171
vector171:
  pushl $0
80107e4f:	6a 00                	push   $0x0
  pushl $171
80107e51:	68 ab 00 00 00       	push   $0xab
  jmp alltraps
80107e56:	e9 b4 f4 ff ff       	jmp    8010730f <alltraps>

80107e5b <vector172>:
.globl vector172
vector172:
  pushl $0
80107e5b:	6a 00                	push   $0x0
  pushl $172
80107e5d:	68 ac 00 00 00       	push   $0xac
  jmp alltraps
80107e62:	e9 a8 f4 ff ff       	jmp    8010730f <alltraps>

80107e67 <vector173>:
.globl vector173
vector173:
  pushl $0
80107e67:	6a 00                	push   $0x0
  pushl $173
80107e69:	68 ad 00 00 00       	push   $0xad
  jmp alltraps
80107e6e:	e9 9c f4 ff ff       	jmp    8010730f <alltraps>

80107e73 <vector174>:
.globl vector174
vector174:
  pushl $0
80107e73:	6a 00                	push   $0x0
  pushl $174
80107e75:	68 ae 00 00 00       	push   $0xae
  jmp alltraps
80107e7a:	e9 90 f4 ff ff       	jmp    8010730f <alltraps>

80107e7f <vector175>:
.globl vector175
vector175:
  pushl $0
80107e7f:	6a 00                	push   $0x0
  pushl $175
80107e81:	68 af 00 00 00       	push   $0xaf
  jmp alltraps
80107e86:	e9 84 f4 ff ff       	jmp    8010730f <alltraps>

80107e8b <vector176>:
.globl vector176
vector176:
  pushl $0
80107e8b:	6a 00                	push   $0x0
  pushl $176
80107e8d:	68 b0 00 00 00       	push   $0xb0
  jmp alltraps
80107e92:	e9 78 f4 ff ff       	jmp    8010730f <alltraps>

80107e97 <vector177>:
.globl vector177
vector177:
  pushl $0
80107e97:	6a 00                	push   $0x0
  pushl $177
80107e99:	68 b1 00 00 00       	push   $0xb1
  jmp alltraps
80107e9e:	e9 6c f4 ff ff       	jmp    8010730f <alltraps>

80107ea3 <vector178>:
.globl vector178
vector178:
  pushl $0
80107ea3:	6a 00                	push   $0x0
  pushl $178
80107ea5:	68 b2 00 00 00       	push   $0xb2
  jmp alltraps
80107eaa:	e9 60 f4 ff ff       	jmp    8010730f <alltraps>

80107eaf <vector179>:
.globl vector179
vector179:
  pushl $0
80107eaf:	6a 00                	push   $0x0
  pushl $179
80107eb1:	68 b3 00 00 00       	push   $0xb3
  jmp alltraps
80107eb6:	e9 54 f4 ff ff       	jmp    8010730f <alltraps>

80107ebb <vector180>:
.globl vector180
vector180:
  pushl $0
80107ebb:	6a 00                	push   $0x0
  pushl $180
80107ebd:	68 b4 00 00 00       	push   $0xb4
  jmp alltraps
80107ec2:	e9 48 f4 ff ff       	jmp    8010730f <alltraps>

80107ec7 <vector181>:
.globl vector181
vector181:
  pushl $0
80107ec7:	6a 00                	push   $0x0
  pushl $181
80107ec9:	68 b5 00 00 00       	push   $0xb5
  jmp alltraps
80107ece:	e9 3c f4 ff ff       	jmp    8010730f <alltraps>

80107ed3 <vector182>:
.globl vector182
vector182:
  pushl $0
80107ed3:	6a 00                	push   $0x0
  pushl $182
80107ed5:	68 b6 00 00 00       	push   $0xb6
  jmp alltraps
80107eda:	e9 30 f4 ff ff       	jmp    8010730f <alltraps>

80107edf <vector183>:
.globl vector183
vector183:
  pushl $0
80107edf:	6a 00                	push   $0x0
  pushl $183
80107ee1:	68 b7 00 00 00       	push   $0xb7
  jmp alltraps
80107ee6:	e9 24 f4 ff ff       	jmp    8010730f <alltraps>

80107eeb <vector184>:
.globl vector184
vector184:
  pushl $0
80107eeb:	6a 00                	push   $0x0
  pushl $184
80107eed:	68 b8 00 00 00       	push   $0xb8
  jmp alltraps
80107ef2:	e9 18 f4 ff ff       	jmp    8010730f <alltraps>

80107ef7 <vector185>:
.globl vector185
vector185:
  pushl $0
80107ef7:	6a 00                	push   $0x0
  pushl $185
80107ef9:	68 b9 00 00 00       	push   $0xb9
  jmp alltraps
80107efe:	e9 0c f4 ff ff       	jmp    8010730f <alltraps>

80107f03 <vector186>:
.globl vector186
vector186:
  pushl $0
80107f03:	6a 00                	push   $0x0
  pushl $186
80107f05:	68 ba 00 00 00       	push   $0xba
  jmp alltraps
80107f0a:	e9 00 f4 ff ff       	jmp    8010730f <alltraps>

80107f0f <vector187>:
.globl vector187
vector187:
  pushl $0
80107f0f:	6a 00                	push   $0x0
  pushl $187
80107f11:	68 bb 00 00 00       	push   $0xbb
  jmp alltraps
80107f16:	e9 f4 f3 ff ff       	jmp    8010730f <alltraps>

80107f1b <vector188>:
.globl vector188
vector188:
  pushl $0
80107f1b:	6a 00                	push   $0x0
  pushl $188
80107f1d:	68 bc 00 00 00       	push   $0xbc
  jmp alltraps
80107f22:	e9 e8 f3 ff ff       	jmp    8010730f <alltraps>

80107f27 <vector189>:
.globl vector189
vector189:
  pushl $0
80107f27:	6a 00                	push   $0x0
  pushl $189
80107f29:	68 bd 00 00 00       	push   $0xbd
  jmp alltraps
80107f2e:	e9 dc f3 ff ff       	jmp    8010730f <alltraps>

80107f33 <vector190>:
.globl vector190
vector190:
  pushl $0
80107f33:	6a 00                	push   $0x0
  pushl $190
80107f35:	68 be 00 00 00       	push   $0xbe
  jmp alltraps
80107f3a:	e9 d0 f3 ff ff       	jmp    8010730f <alltraps>

80107f3f <vector191>:
.globl vector191
vector191:
  pushl $0
80107f3f:	6a 00                	push   $0x0
  pushl $191
80107f41:	68 bf 00 00 00       	push   $0xbf
  jmp alltraps
80107f46:	e9 c4 f3 ff ff       	jmp    8010730f <alltraps>

80107f4b <vector192>:
.globl vector192
vector192:
  pushl $0
80107f4b:	6a 00                	push   $0x0
  pushl $192
80107f4d:	68 c0 00 00 00       	push   $0xc0
  jmp alltraps
80107f52:	e9 b8 f3 ff ff       	jmp    8010730f <alltraps>

80107f57 <vector193>:
.globl vector193
vector193:
  pushl $0
80107f57:	6a 00                	push   $0x0
  pushl $193
80107f59:	68 c1 00 00 00       	push   $0xc1
  jmp alltraps
80107f5e:	e9 ac f3 ff ff       	jmp    8010730f <alltraps>

80107f63 <vector194>:
.globl vector194
vector194:
  pushl $0
80107f63:	6a 00                	push   $0x0
  pushl $194
80107f65:	68 c2 00 00 00       	push   $0xc2
  jmp alltraps
80107f6a:	e9 a0 f3 ff ff       	jmp    8010730f <alltraps>

80107f6f <vector195>:
.globl vector195
vector195:
  pushl $0
80107f6f:	6a 00                	push   $0x0
  pushl $195
80107f71:	68 c3 00 00 00       	push   $0xc3
  jmp alltraps
80107f76:	e9 94 f3 ff ff       	jmp    8010730f <alltraps>

80107f7b <vector196>:
.globl vector196
vector196:
  pushl $0
80107f7b:	6a 00                	push   $0x0
  pushl $196
80107f7d:	68 c4 00 00 00       	push   $0xc4
  jmp alltraps
80107f82:	e9 88 f3 ff ff       	jmp    8010730f <alltraps>

80107f87 <vector197>:
.globl vector197
vector197:
  pushl $0
80107f87:	6a 00                	push   $0x0
  pushl $197
80107f89:	68 c5 00 00 00       	push   $0xc5
  jmp alltraps
80107f8e:	e9 7c f3 ff ff       	jmp    8010730f <alltraps>

80107f93 <vector198>:
.globl vector198
vector198:
  pushl $0
80107f93:	6a 00                	push   $0x0
  pushl $198
80107f95:	68 c6 00 00 00       	push   $0xc6
  jmp alltraps
80107f9a:	e9 70 f3 ff ff       	jmp    8010730f <alltraps>

80107f9f <vector199>:
.globl vector199
vector199:
  pushl $0
80107f9f:	6a 00                	push   $0x0
  pushl $199
80107fa1:	68 c7 00 00 00       	push   $0xc7
  jmp alltraps
80107fa6:	e9 64 f3 ff ff       	jmp    8010730f <alltraps>

80107fab <vector200>:
.globl vector200
vector200:
  pushl $0
80107fab:	6a 00                	push   $0x0
  pushl $200
80107fad:	68 c8 00 00 00       	push   $0xc8
  jmp alltraps
80107fb2:	e9 58 f3 ff ff       	jmp    8010730f <alltraps>

80107fb7 <vector201>:
.globl vector201
vector201:
  pushl $0
80107fb7:	6a 00                	push   $0x0
  pushl $201
80107fb9:	68 c9 00 00 00       	push   $0xc9
  jmp alltraps
80107fbe:	e9 4c f3 ff ff       	jmp    8010730f <alltraps>

80107fc3 <vector202>:
.globl vector202
vector202:
  pushl $0
80107fc3:	6a 00                	push   $0x0
  pushl $202
80107fc5:	68 ca 00 00 00       	push   $0xca
  jmp alltraps
80107fca:	e9 40 f3 ff ff       	jmp    8010730f <alltraps>

80107fcf <vector203>:
.globl vector203
vector203:
  pushl $0
80107fcf:	6a 00                	push   $0x0
  pushl $203
80107fd1:	68 cb 00 00 00       	push   $0xcb
  jmp alltraps
80107fd6:	e9 34 f3 ff ff       	jmp    8010730f <alltraps>

80107fdb <vector204>:
.globl vector204
vector204:
  pushl $0
80107fdb:	6a 00                	push   $0x0
  pushl $204
80107fdd:	68 cc 00 00 00       	push   $0xcc
  jmp alltraps
80107fe2:	e9 28 f3 ff ff       	jmp    8010730f <alltraps>

80107fe7 <vector205>:
.globl vector205
vector205:
  pushl $0
80107fe7:	6a 00                	push   $0x0
  pushl $205
80107fe9:	68 cd 00 00 00       	push   $0xcd
  jmp alltraps
80107fee:	e9 1c f3 ff ff       	jmp    8010730f <alltraps>

80107ff3 <vector206>:
.globl vector206
vector206:
  pushl $0
80107ff3:	6a 00                	push   $0x0
  pushl $206
80107ff5:	68 ce 00 00 00       	push   $0xce
  jmp alltraps
80107ffa:	e9 10 f3 ff ff       	jmp    8010730f <alltraps>

80107fff <vector207>:
.globl vector207
vector207:
  pushl $0
80107fff:	6a 00                	push   $0x0
  pushl $207
80108001:	68 cf 00 00 00       	push   $0xcf
  jmp alltraps
80108006:	e9 04 f3 ff ff       	jmp    8010730f <alltraps>

8010800b <vector208>:
.globl vector208
vector208:
  pushl $0
8010800b:	6a 00                	push   $0x0
  pushl $208
8010800d:	68 d0 00 00 00       	push   $0xd0
  jmp alltraps
80108012:	e9 f8 f2 ff ff       	jmp    8010730f <alltraps>

80108017 <vector209>:
.globl vector209
vector209:
  pushl $0
80108017:	6a 00                	push   $0x0
  pushl $209
80108019:	68 d1 00 00 00       	push   $0xd1
  jmp alltraps
8010801e:	e9 ec f2 ff ff       	jmp    8010730f <alltraps>

80108023 <vector210>:
.globl vector210
vector210:
  pushl $0
80108023:	6a 00                	push   $0x0
  pushl $210
80108025:	68 d2 00 00 00       	push   $0xd2
  jmp alltraps
8010802a:	e9 e0 f2 ff ff       	jmp    8010730f <alltraps>

8010802f <vector211>:
.globl vector211
vector211:
  pushl $0
8010802f:	6a 00                	push   $0x0
  pushl $211
80108031:	68 d3 00 00 00       	push   $0xd3
  jmp alltraps
80108036:	e9 d4 f2 ff ff       	jmp    8010730f <alltraps>

8010803b <vector212>:
.globl vector212
vector212:
  pushl $0
8010803b:	6a 00                	push   $0x0
  pushl $212
8010803d:	68 d4 00 00 00       	push   $0xd4
  jmp alltraps
80108042:	e9 c8 f2 ff ff       	jmp    8010730f <alltraps>

80108047 <vector213>:
.globl vector213
vector213:
  pushl $0
80108047:	6a 00                	push   $0x0
  pushl $213
80108049:	68 d5 00 00 00       	push   $0xd5
  jmp alltraps
8010804e:	e9 bc f2 ff ff       	jmp    8010730f <alltraps>

80108053 <vector214>:
.globl vector214
vector214:
  pushl $0
80108053:	6a 00                	push   $0x0
  pushl $214
80108055:	68 d6 00 00 00       	push   $0xd6
  jmp alltraps
8010805a:	e9 b0 f2 ff ff       	jmp    8010730f <alltraps>

8010805f <vector215>:
.globl vector215
vector215:
  pushl $0
8010805f:	6a 00                	push   $0x0
  pushl $215
80108061:	68 d7 00 00 00       	push   $0xd7
  jmp alltraps
80108066:	e9 a4 f2 ff ff       	jmp    8010730f <alltraps>

8010806b <vector216>:
.globl vector216
vector216:
  pushl $0
8010806b:	6a 00                	push   $0x0
  pushl $216
8010806d:	68 d8 00 00 00       	push   $0xd8
  jmp alltraps
80108072:	e9 98 f2 ff ff       	jmp    8010730f <alltraps>

80108077 <vector217>:
.globl vector217
vector217:
  pushl $0
80108077:	6a 00                	push   $0x0
  pushl $217
80108079:	68 d9 00 00 00       	push   $0xd9
  jmp alltraps
8010807e:	e9 8c f2 ff ff       	jmp    8010730f <alltraps>

80108083 <vector218>:
.globl vector218
vector218:
  pushl $0
80108083:	6a 00                	push   $0x0
  pushl $218
80108085:	68 da 00 00 00       	push   $0xda
  jmp alltraps
8010808a:	e9 80 f2 ff ff       	jmp    8010730f <alltraps>

8010808f <vector219>:
.globl vector219
vector219:
  pushl $0
8010808f:	6a 00                	push   $0x0
  pushl $219
80108091:	68 db 00 00 00       	push   $0xdb
  jmp alltraps
80108096:	e9 74 f2 ff ff       	jmp    8010730f <alltraps>

8010809b <vector220>:
.globl vector220
vector220:
  pushl $0
8010809b:	6a 00                	push   $0x0
  pushl $220
8010809d:	68 dc 00 00 00       	push   $0xdc
  jmp alltraps
801080a2:	e9 68 f2 ff ff       	jmp    8010730f <alltraps>

801080a7 <vector221>:
.globl vector221
vector221:
  pushl $0
801080a7:	6a 00                	push   $0x0
  pushl $221
801080a9:	68 dd 00 00 00       	push   $0xdd
  jmp alltraps
801080ae:	e9 5c f2 ff ff       	jmp    8010730f <alltraps>

801080b3 <vector222>:
.globl vector222
vector222:
  pushl $0
801080b3:	6a 00                	push   $0x0
  pushl $222
801080b5:	68 de 00 00 00       	push   $0xde
  jmp alltraps
801080ba:	e9 50 f2 ff ff       	jmp    8010730f <alltraps>

801080bf <vector223>:
.globl vector223
vector223:
  pushl $0
801080bf:	6a 00                	push   $0x0
  pushl $223
801080c1:	68 df 00 00 00       	push   $0xdf
  jmp alltraps
801080c6:	e9 44 f2 ff ff       	jmp    8010730f <alltraps>

801080cb <vector224>:
.globl vector224
vector224:
  pushl $0
801080cb:	6a 00                	push   $0x0
  pushl $224
801080cd:	68 e0 00 00 00       	push   $0xe0
  jmp alltraps
801080d2:	e9 38 f2 ff ff       	jmp    8010730f <alltraps>

801080d7 <vector225>:
.globl vector225
vector225:
  pushl $0
801080d7:	6a 00                	push   $0x0
  pushl $225
801080d9:	68 e1 00 00 00       	push   $0xe1
  jmp alltraps
801080de:	e9 2c f2 ff ff       	jmp    8010730f <alltraps>

801080e3 <vector226>:
.globl vector226
vector226:
  pushl $0
801080e3:	6a 00                	push   $0x0
  pushl $226
801080e5:	68 e2 00 00 00       	push   $0xe2
  jmp alltraps
801080ea:	e9 20 f2 ff ff       	jmp    8010730f <alltraps>

801080ef <vector227>:
.globl vector227
vector227:
  pushl $0
801080ef:	6a 00                	push   $0x0
  pushl $227
801080f1:	68 e3 00 00 00       	push   $0xe3
  jmp alltraps
801080f6:	e9 14 f2 ff ff       	jmp    8010730f <alltraps>

801080fb <vector228>:
.globl vector228
vector228:
  pushl $0
801080fb:	6a 00                	push   $0x0
  pushl $228
801080fd:	68 e4 00 00 00       	push   $0xe4
  jmp alltraps
80108102:	e9 08 f2 ff ff       	jmp    8010730f <alltraps>

80108107 <vector229>:
.globl vector229
vector229:
  pushl $0
80108107:	6a 00                	push   $0x0
  pushl $229
80108109:	68 e5 00 00 00       	push   $0xe5
  jmp alltraps
8010810e:	e9 fc f1 ff ff       	jmp    8010730f <alltraps>

80108113 <vector230>:
.globl vector230
vector230:
  pushl $0
80108113:	6a 00                	push   $0x0
  pushl $230
80108115:	68 e6 00 00 00       	push   $0xe6
  jmp alltraps
8010811a:	e9 f0 f1 ff ff       	jmp    8010730f <alltraps>

8010811f <vector231>:
.globl vector231
vector231:
  pushl $0
8010811f:	6a 00                	push   $0x0
  pushl $231
80108121:	68 e7 00 00 00       	push   $0xe7
  jmp alltraps
80108126:	e9 e4 f1 ff ff       	jmp    8010730f <alltraps>

8010812b <vector232>:
.globl vector232
vector232:
  pushl $0
8010812b:	6a 00                	push   $0x0
  pushl $232
8010812d:	68 e8 00 00 00       	push   $0xe8
  jmp alltraps
80108132:	e9 d8 f1 ff ff       	jmp    8010730f <alltraps>

80108137 <vector233>:
.globl vector233
vector233:
  pushl $0
80108137:	6a 00                	push   $0x0
  pushl $233
80108139:	68 e9 00 00 00       	push   $0xe9
  jmp alltraps
8010813e:	e9 cc f1 ff ff       	jmp    8010730f <alltraps>

80108143 <vector234>:
.globl vector234
vector234:
  pushl $0
80108143:	6a 00                	push   $0x0
  pushl $234
80108145:	68 ea 00 00 00       	push   $0xea
  jmp alltraps
8010814a:	e9 c0 f1 ff ff       	jmp    8010730f <alltraps>

8010814f <vector235>:
.globl vector235
vector235:
  pushl $0
8010814f:	6a 00                	push   $0x0
  pushl $235
80108151:	68 eb 00 00 00       	push   $0xeb
  jmp alltraps
80108156:	e9 b4 f1 ff ff       	jmp    8010730f <alltraps>

8010815b <vector236>:
.globl vector236
vector236:
  pushl $0
8010815b:	6a 00                	push   $0x0
  pushl $236
8010815d:	68 ec 00 00 00       	push   $0xec
  jmp alltraps
80108162:	e9 a8 f1 ff ff       	jmp    8010730f <alltraps>

80108167 <vector237>:
.globl vector237
vector237:
  pushl $0
80108167:	6a 00                	push   $0x0
  pushl $237
80108169:	68 ed 00 00 00       	push   $0xed
  jmp alltraps
8010816e:	e9 9c f1 ff ff       	jmp    8010730f <alltraps>

80108173 <vector238>:
.globl vector238
vector238:
  pushl $0
80108173:	6a 00                	push   $0x0
  pushl $238
80108175:	68 ee 00 00 00       	push   $0xee
  jmp alltraps
8010817a:	e9 90 f1 ff ff       	jmp    8010730f <alltraps>

8010817f <vector239>:
.globl vector239
vector239:
  pushl $0
8010817f:	6a 00                	push   $0x0
  pushl $239
80108181:	68 ef 00 00 00       	push   $0xef
  jmp alltraps
80108186:	e9 84 f1 ff ff       	jmp    8010730f <alltraps>

8010818b <vector240>:
.globl vector240
vector240:
  pushl $0
8010818b:	6a 00                	push   $0x0
  pushl $240
8010818d:	68 f0 00 00 00       	push   $0xf0
  jmp alltraps
80108192:	e9 78 f1 ff ff       	jmp    8010730f <alltraps>

80108197 <vector241>:
.globl vector241
vector241:
  pushl $0
80108197:	6a 00                	push   $0x0
  pushl $241
80108199:	68 f1 00 00 00       	push   $0xf1
  jmp alltraps
8010819e:	e9 6c f1 ff ff       	jmp    8010730f <alltraps>

801081a3 <vector242>:
.globl vector242
vector242:
  pushl $0
801081a3:	6a 00                	push   $0x0
  pushl $242
801081a5:	68 f2 00 00 00       	push   $0xf2
  jmp alltraps
801081aa:	e9 60 f1 ff ff       	jmp    8010730f <alltraps>

801081af <vector243>:
.globl vector243
vector243:
  pushl $0
801081af:	6a 00                	push   $0x0
  pushl $243
801081b1:	68 f3 00 00 00       	push   $0xf3
  jmp alltraps
801081b6:	e9 54 f1 ff ff       	jmp    8010730f <alltraps>

801081bb <vector244>:
.globl vector244
vector244:
  pushl $0
801081bb:	6a 00                	push   $0x0
  pushl $244
801081bd:	68 f4 00 00 00       	push   $0xf4
  jmp alltraps
801081c2:	e9 48 f1 ff ff       	jmp    8010730f <alltraps>

801081c7 <vector245>:
.globl vector245
vector245:
  pushl $0
801081c7:	6a 00                	push   $0x0
  pushl $245
801081c9:	68 f5 00 00 00       	push   $0xf5
  jmp alltraps
801081ce:	e9 3c f1 ff ff       	jmp    8010730f <alltraps>

801081d3 <vector246>:
.globl vector246
vector246:
  pushl $0
801081d3:	6a 00                	push   $0x0
  pushl $246
801081d5:	68 f6 00 00 00       	push   $0xf6
  jmp alltraps
801081da:	e9 30 f1 ff ff       	jmp    8010730f <alltraps>

801081df <vector247>:
.globl vector247
vector247:
  pushl $0
801081df:	6a 00                	push   $0x0
  pushl $247
801081e1:	68 f7 00 00 00       	push   $0xf7
  jmp alltraps
801081e6:	e9 24 f1 ff ff       	jmp    8010730f <alltraps>

801081eb <vector248>:
.globl vector248
vector248:
  pushl $0
801081eb:	6a 00                	push   $0x0
  pushl $248
801081ed:	68 f8 00 00 00       	push   $0xf8
  jmp alltraps
801081f2:	e9 18 f1 ff ff       	jmp    8010730f <alltraps>

801081f7 <vector249>:
.globl vector249
vector249:
  pushl $0
801081f7:	6a 00                	push   $0x0
  pushl $249
801081f9:	68 f9 00 00 00       	push   $0xf9
  jmp alltraps
801081fe:	e9 0c f1 ff ff       	jmp    8010730f <alltraps>

80108203 <vector250>:
.globl vector250
vector250:
  pushl $0
80108203:	6a 00                	push   $0x0
  pushl $250
80108205:	68 fa 00 00 00       	push   $0xfa
  jmp alltraps
8010820a:	e9 00 f1 ff ff       	jmp    8010730f <alltraps>

8010820f <vector251>:
.globl vector251
vector251:
  pushl $0
8010820f:	6a 00                	push   $0x0
  pushl $251
80108211:	68 fb 00 00 00       	push   $0xfb
  jmp alltraps
80108216:	e9 f4 f0 ff ff       	jmp    8010730f <alltraps>

8010821b <vector252>:
.globl vector252
vector252:
  pushl $0
8010821b:	6a 00                	push   $0x0
  pushl $252
8010821d:	68 fc 00 00 00       	push   $0xfc
  jmp alltraps
80108222:	e9 e8 f0 ff ff       	jmp    8010730f <alltraps>

80108227 <vector253>:
.globl vector253
vector253:
  pushl $0
80108227:	6a 00                	push   $0x0
  pushl $253
80108229:	68 fd 00 00 00       	push   $0xfd
  jmp alltraps
8010822e:	e9 dc f0 ff ff       	jmp    8010730f <alltraps>

80108233 <vector254>:
.globl vector254
vector254:
  pushl $0
80108233:	6a 00                	push   $0x0
  pushl $254
80108235:	68 fe 00 00 00       	push   $0xfe
  jmp alltraps
8010823a:	e9 d0 f0 ff ff       	jmp    8010730f <alltraps>

8010823f <vector255>:
.globl vector255
vector255:
  pushl $0
8010823f:	6a 00                	push   $0x0
  pushl $255
80108241:	68 ff 00 00 00       	push   $0xff
  jmp alltraps
80108246:	e9 c4 f0 ff ff       	jmp    8010730f <alltraps>
8010824b:	66 90                	xchg   %ax,%ax
8010824d:	66 90                	xchg   %ax,%ax
8010824f:	90                   	nop

80108250 <walkpgdir>:
// Return the address of the PTE in page table pgdir
// that corresponds to virtual address va.  If alloc!=0,
// create any required page table pages.
static pte_t *
walkpgdir(pde_t *pgdir, const void *va, int alloc)
{
80108250:	55                   	push   %ebp
80108251:	89 e5                	mov    %esp,%ebp
80108253:	57                   	push   %edi
80108254:	56                   	push   %esi
80108255:	89 d6                	mov    %edx,%esi
  pde_t *pde;
  pte_t *pgtab;

  pde = &pgdir[PDX(va)];
80108257:	c1 ea 16             	shr    $0x16,%edx
{
8010825a:	53                   	push   %ebx
  pde = &pgdir[PDX(va)];
8010825b:	8d 3c 90             	lea    (%eax,%edx,4),%edi
{
8010825e:	83 ec 0c             	sub    $0xc,%esp
  if(*pde & PTE_P){
80108261:	8b 1f                	mov    (%edi),%ebx
80108263:	f6 c3 01             	test   $0x1,%bl
80108266:	74 28                	je     80108290 <walkpgdir+0x40>
    pgtab = (pte_t*)P2V(PTE_ADDR(*pde));
80108268:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
8010826e:	81 c3 00 00 00 80    	add    $0x80000000,%ebx
    // The permissions here are overly generous, but they can
    // be further restricted by the permissions in the page table
    // entries, if necessary.
    *pde = V2P(pgtab) | PTE_P | PTE_W | PTE_U;
  }
  return &pgtab[PTX(va)];
80108274:	89 f0                	mov    %esi,%eax
}
80108276:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return &pgtab[PTX(va)];
80108279:	c1 e8 0a             	shr    $0xa,%eax
8010827c:	25 fc 0f 00 00       	and    $0xffc,%eax
80108281:	01 d8                	add    %ebx,%eax
}
80108283:	5b                   	pop    %ebx
80108284:	5e                   	pop    %esi
80108285:	5f                   	pop    %edi
80108286:	5d                   	pop    %ebp
80108287:	c3                   	ret    
80108288:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010828f:	90                   	nop
    if(!alloc || (pgtab = (pte_t*)kalloc()) == 0)
80108290:	85 c9                	test   %ecx,%ecx
80108292:	74 2c                	je     801082c0 <walkpgdir+0x70>
80108294:	e8 87 a8 ff ff       	call   80102b20 <kalloc>
80108299:	89 c3                	mov    %eax,%ebx
8010829b:	85 c0                	test   %eax,%eax
8010829d:	74 21                	je     801082c0 <walkpgdir+0x70>
    memset(pgtab, 0, PGSIZE);
8010829f:	83 ec 04             	sub    $0x4,%esp
801082a2:	68 00 10 00 00       	push   $0x1000
801082a7:	6a 00                	push   $0x0
801082a9:	50                   	push   %eax
801082aa:	e8 71 da ff ff       	call   80105d20 <memset>
    *pde = V2P(pgtab) | PTE_P | PTE_W | PTE_U;
801082af:	8d 83 00 00 00 80    	lea    -0x80000000(%ebx),%eax
801082b5:	83 c4 10             	add    $0x10,%esp
801082b8:	83 c8 07             	or     $0x7,%eax
801082bb:	89 07                	mov    %eax,(%edi)
801082bd:	eb b5                	jmp    80108274 <walkpgdir+0x24>
801082bf:	90                   	nop
}
801082c0:	8d 65 f4             	lea    -0xc(%ebp),%esp
      return 0;
801082c3:	31 c0                	xor    %eax,%eax
}
801082c5:	5b                   	pop    %ebx
801082c6:	5e                   	pop    %esi
801082c7:	5f                   	pop    %edi
801082c8:	5d                   	pop    %ebp
801082c9:	c3                   	ret    
801082ca:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801082d0 <mappages>:
// Create PTEs for virtual addresses starting at va that refer to
// physical addresses starting at pa. va and size might not
// be page-aligned.
static int
mappages(pde_t *pgdir, void *va, uint size, uint pa, int perm)
{
801082d0:	55                   	push   %ebp
801082d1:	89 e5                	mov    %esp,%ebp
801082d3:	57                   	push   %edi
801082d4:	89 c7                	mov    %eax,%edi
  char *a, *last;
  pte_t *pte;

  a = (char*)PGROUNDDOWN((uint)va);
  last = (char*)PGROUNDDOWN(((uint)va) + size - 1);
801082d6:	8d 44 0a ff          	lea    -0x1(%edx,%ecx,1),%eax
{
801082da:	56                   	push   %esi
  last = (char*)PGROUNDDOWN(((uint)va) + size - 1);
801082db:	25 00 f0 ff ff       	and    $0xfffff000,%eax
  a = (char*)PGROUNDDOWN((uint)va);
801082e0:	89 d6                	mov    %edx,%esi
{
801082e2:	53                   	push   %ebx
  a = (char*)PGROUNDDOWN((uint)va);
801082e3:	81 e6 00 f0 ff ff    	and    $0xfffff000,%esi
{
801082e9:	83 ec 1c             	sub    $0x1c,%esp
  last = (char*)PGROUNDDOWN(((uint)va) + size - 1);
801082ec:	89 45 e0             	mov    %eax,-0x20(%ebp)
801082ef:	8b 45 08             	mov    0x8(%ebp),%eax
801082f2:	29 f0                	sub    %esi,%eax
801082f4:	89 45 e4             	mov    %eax,-0x1c(%ebp)
801082f7:	eb 1f                	jmp    80108318 <mappages+0x48>
801082f9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  for(;;){
    if((pte = walkpgdir(pgdir, a, 1)) == 0)
      return -1;
    if(*pte & PTE_P)
80108300:	f6 00 01             	testb  $0x1,(%eax)
80108303:	75 45                	jne    8010834a <mappages+0x7a>
      panic("remap");
    *pte = pa | perm | PTE_P;
80108305:	0b 5d 0c             	or     0xc(%ebp),%ebx
80108308:	83 cb 01             	or     $0x1,%ebx
8010830b:	89 18                	mov    %ebx,(%eax)
    if(a == last)
8010830d:	3b 75 e0             	cmp    -0x20(%ebp),%esi
80108310:	74 2e                	je     80108340 <mappages+0x70>
      break;
    a += PGSIZE;
80108312:	81 c6 00 10 00 00    	add    $0x1000,%esi
  for(;;){
80108318:	8b 45 e4             	mov    -0x1c(%ebp),%eax
    if((pte = walkpgdir(pgdir, a, 1)) == 0)
8010831b:	b9 01 00 00 00       	mov    $0x1,%ecx
80108320:	89 f2                	mov    %esi,%edx
80108322:	8d 1c 06             	lea    (%esi,%eax,1),%ebx
80108325:	89 f8                	mov    %edi,%eax
80108327:	e8 24 ff ff ff       	call   80108250 <walkpgdir>
8010832c:	85 c0                	test   %eax,%eax
8010832e:	75 d0                	jne    80108300 <mappages+0x30>
    pa += PGSIZE;
  }
  return 0;
}
80108330:	8d 65 f4             	lea    -0xc(%ebp),%esp
      return -1;
80108333:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80108338:	5b                   	pop    %ebx
80108339:	5e                   	pop    %esi
8010833a:	5f                   	pop    %edi
8010833b:	5d                   	pop    %ebp
8010833c:	c3                   	ret    
8010833d:	8d 76 00             	lea    0x0(%esi),%esi
80108340:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
80108343:	31 c0                	xor    %eax,%eax
}
80108345:	5b                   	pop    %ebx
80108346:	5e                   	pop    %esi
80108347:	5f                   	pop    %edi
80108348:	5d                   	pop    %ebp
80108349:	c3                   	ret    
      panic("remap");
8010834a:	83 ec 0c             	sub    $0xc,%esp
8010834d:	68 14 97 10 80       	push   $0x80109714
80108352:	e8 39 80 ff ff       	call   80100390 <panic>
80108357:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010835e:	66 90                	xchg   %ax,%ax

80108360 <deallocuvm.part.0>:
// Deallocate user pages to bring the process size from oldsz to
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
int
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
80108360:	55                   	push   %ebp
80108361:	89 e5                	mov    %esp,%ebp
80108363:	57                   	push   %edi
80108364:	56                   	push   %esi
80108365:	89 c6                	mov    %eax,%esi
80108367:	53                   	push   %ebx
80108368:	89 d3                	mov    %edx,%ebx
  uint a, pa;

  if(newsz >= oldsz)
    return oldsz;

  a = PGROUNDUP(newsz);
8010836a:	8d 91 ff 0f 00 00    	lea    0xfff(%ecx),%edx
80108370:	81 e2 00 f0 ff ff    	and    $0xfffff000,%edx
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
80108376:	83 ec 1c             	sub    $0x1c,%esp
80108379:	89 4d e0             	mov    %ecx,-0x20(%ebp)
  for(; a  < oldsz; a += PGSIZE){
8010837c:	39 da                	cmp    %ebx,%edx
8010837e:	73 5b                	jae    801083db <deallocuvm.part.0+0x7b>
80108380:	89 5d e4             	mov    %ebx,-0x1c(%ebp)
80108383:	89 d7                	mov    %edx,%edi
80108385:	eb 14                	jmp    8010839b <deallocuvm.part.0+0x3b>
80108387:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010838e:	66 90                	xchg   %ax,%ax
80108390:	81 c7 00 10 00 00    	add    $0x1000,%edi
80108396:	39 7d e4             	cmp    %edi,-0x1c(%ebp)
80108399:	76 40                	jbe    801083db <deallocuvm.part.0+0x7b>
    pte = walkpgdir(pgdir, (char*)a, 0);
8010839b:	31 c9                	xor    %ecx,%ecx
8010839d:	89 fa                	mov    %edi,%edx
8010839f:	89 f0                	mov    %esi,%eax
801083a1:	e8 aa fe ff ff       	call   80108250 <walkpgdir>
801083a6:	89 c3                	mov    %eax,%ebx
    if(!pte)
801083a8:	85 c0                	test   %eax,%eax
801083aa:	74 44                	je     801083f0 <deallocuvm.part.0+0x90>
      a = PGADDR(PDX(a) + 1, 0, 0) - PGSIZE;
    else if((*pte & PTE_P) != 0){
801083ac:	8b 00                	mov    (%eax),%eax
801083ae:	a8 01                	test   $0x1,%al
801083b0:	74 de                	je     80108390 <deallocuvm.part.0+0x30>
      pa = PTE_ADDR(*pte);
      if(pa == 0)
801083b2:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801083b7:	74 47                	je     80108400 <deallocuvm.part.0+0xa0>
        panic("kfree");
      char *v = P2V(pa);
      kfree(v);
801083b9:	83 ec 0c             	sub    $0xc,%esp
      char *v = P2V(pa);
801083bc:	05 00 00 00 80       	add    $0x80000000,%eax
801083c1:	81 c7 00 10 00 00    	add    $0x1000,%edi
      kfree(v);
801083c7:	50                   	push   %eax
801083c8:	e8 93 a5 ff ff       	call   80102960 <kfree>
      *pte = 0;
801083cd:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
801083d3:	83 c4 10             	add    $0x10,%esp
  for(; a  < oldsz; a += PGSIZE){
801083d6:	39 7d e4             	cmp    %edi,-0x1c(%ebp)
801083d9:	77 c0                	ja     8010839b <deallocuvm.part.0+0x3b>
    }
  }
  return newsz;
}
801083db:	8b 45 e0             	mov    -0x20(%ebp),%eax
801083de:	8d 65 f4             	lea    -0xc(%ebp),%esp
801083e1:	5b                   	pop    %ebx
801083e2:	5e                   	pop    %esi
801083e3:	5f                   	pop    %edi
801083e4:	5d                   	pop    %ebp
801083e5:	c3                   	ret    
801083e6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801083ed:	8d 76 00             	lea    0x0(%esi),%esi
      a = PGADDR(PDX(a) + 1, 0, 0) - PGSIZE;
801083f0:	89 fa                	mov    %edi,%edx
801083f2:	81 e2 00 00 c0 ff    	and    $0xffc00000,%edx
801083f8:	8d ba 00 00 40 00    	lea    0x400000(%edx),%edi
801083fe:	eb 96                	jmp    80108396 <deallocuvm.part.0+0x36>
        panic("kfree");
80108400:	83 ec 0c             	sub    $0xc,%esp
80108403:	68 5e 8e 10 80       	push   $0x80108e5e
80108408:	e8 83 7f ff ff       	call   80100390 <panic>
8010840d:	8d 76 00             	lea    0x0(%esi),%esi

80108410 <seginit>:
{
80108410:	f3 0f 1e fb          	endbr32 
80108414:	55                   	push   %ebp
80108415:	89 e5                	mov    %esp,%ebp
80108417:	83 ec 18             	sub    $0x18,%esp
  c = &cpus[cpuid()];
8010841a:	e8 81 ba ff ff       	call   80103ea0 <cpuid>
  pd[0] = size-1;
8010841f:	ba 2f 00 00 00       	mov    $0x2f,%edx
80108424:	69 c0 b0 00 00 00    	imul   $0xb0,%eax,%eax
8010842a:	66 89 55 f2          	mov    %dx,-0xe(%ebp)
  c->gdt[SEG_KCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, 0);
8010842e:	c7 80 f8 4d 11 80 ff 	movl   $0xffff,-0x7feeb208(%eax)
80108435:	ff 00 00 
80108438:	c7 80 fc 4d 11 80 00 	movl   $0xcf9a00,-0x7feeb204(%eax)
8010843f:	9a cf 00 
  c->gdt[SEG_KDATA] = SEG(STA_W, 0, 0xffffffff, 0);
80108442:	c7 80 00 4e 11 80 ff 	movl   $0xffff,-0x7feeb200(%eax)
80108449:	ff 00 00 
8010844c:	c7 80 04 4e 11 80 00 	movl   $0xcf9200,-0x7feeb1fc(%eax)
80108453:	92 cf 00 
  c->gdt[SEG_UCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, DPL_USER);
80108456:	c7 80 08 4e 11 80 ff 	movl   $0xffff,-0x7feeb1f8(%eax)
8010845d:	ff 00 00 
80108460:	c7 80 0c 4e 11 80 00 	movl   $0xcffa00,-0x7feeb1f4(%eax)
80108467:	fa cf 00 
  c->gdt[SEG_UDATA] = SEG(STA_W, 0, 0xffffffff, DPL_USER);
8010846a:	c7 80 10 4e 11 80 ff 	movl   $0xffff,-0x7feeb1f0(%eax)
80108471:	ff 00 00 
80108474:	c7 80 14 4e 11 80 00 	movl   $0xcff200,-0x7feeb1ec(%eax)
8010847b:	f2 cf 00 
  lgdt(c->gdt, sizeof(c->gdt));
8010847e:	05 f0 4d 11 80       	add    $0x80114df0,%eax
  pd[1] = (uint)p;
80108483:	66 89 45 f4          	mov    %ax,-0xc(%ebp)
  pd[2] = (uint)p >> 16;
80108487:	c1 e8 10             	shr    $0x10,%eax
8010848a:	66 89 45 f6          	mov    %ax,-0xa(%ebp)
  asm volatile("lgdt (%0)" : : "r" (pd));
8010848e:	8d 45 f2             	lea    -0xe(%ebp),%eax
80108491:	0f 01 10             	lgdtl  (%eax)
}
80108494:	c9                   	leave  
80108495:	c3                   	ret    
80108496:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010849d:	8d 76 00             	lea    0x0(%esi),%esi

801084a0 <switchkvm>:
{
801084a0:	f3 0f 1e fb          	endbr32 
  lcr3(V2P(kpgdir));   // switch to the kernel page table
801084a4:	a1 a4 86 11 80       	mov    0x801186a4,%eax
801084a9:	05 00 00 00 80       	add    $0x80000000,%eax
}

static inline void
lcr3(uint val)
{
  asm volatile("movl %0,%%cr3" : : "r" (val));
801084ae:	0f 22 d8             	mov    %eax,%cr3
}
801084b1:	c3                   	ret    
801084b2:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801084b9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

801084c0 <switchuvm>:
{
801084c0:	f3 0f 1e fb          	endbr32 
801084c4:	55                   	push   %ebp
801084c5:	89 e5                	mov    %esp,%ebp
801084c7:	57                   	push   %edi
801084c8:	56                   	push   %esi
801084c9:	53                   	push   %ebx
801084ca:	83 ec 1c             	sub    $0x1c,%esp
801084cd:	8b 75 08             	mov    0x8(%ebp),%esi
  if(p == 0)
801084d0:	85 f6                	test   %esi,%esi
801084d2:	0f 84 cb 00 00 00    	je     801085a3 <switchuvm+0xe3>
  if(p->kstack == 0)
801084d8:	8b 46 08             	mov    0x8(%esi),%eax
801084db:	85 c0                	test   %eax,%eax
801084dd:	0f 84 da 00 00 00    	je     801085bd <switchuvm+0xfd>
  if(p->pgdir == 0)
801084e3:	8b 46 04             	mov    0x4(%esi),%eax
801084e6:	85 c0                	test   %eax,%eax
801084e8:	0f 84 c2 00 00 00    	je     801085b0 <switchuvm+0xf0>
  pushcli();
801084ee:	e8 1d d6 ff ff       	call   80105b10 <pushcli>
  mycpu()->gdt[SEG_TSS] = SEG16(STS_T32A, &mycpu()->ts,
801084f3:	e8 38 b9 ff ff       	call   80103e30 <mycpu>
801084f8:	89 c3                	mov    %eax,%ebx
801084fa:	e8 31 b9 ff ff       	call   80103e30 <mycpu>
801084ff:	89 c7                	mov    %eax,%edi
80108501:	e8 2a b9 ff ff       	call   80103e30 <mycpu>
80108506:	83 c7 08             	add    $0x8,%edi
80108509:	89 45 e4             	mov    %eax,-0x1c(%ebp)
8010850c:	e8 1f b9 ff ff       	call   80103e30 <mycpu>
80108511:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
80108514:	ba 67 00 00 00       	mov    $0x67,%edx
80108519:	66 89 bb 9a 00 00 00 	mov    %di,0x9a(%ebx)
80108520:	83 c0 08             	add    $0x8,%eax
80108523:	66 89 93 98 00 00 00 	mov    %dx,0x98(%ebx)
  mycpu()->ts.iomb = (ushort) 0xFFFF;
8010852a:	bf ff ff ff ff       	mov    $0xffffffff,%edi
  mycpu()->gdt[SEG_TSS] = SEG16(STS_T32A, &mycpu()->ts,
8010852f:	83 c1 08             	add    $0x8,%ecx
80108532:	c1 e8 18             	shr    $0x18,%eax
80108535:	c1 e9 10             	shr    $0x10,%ecx
80108538:	88 83 9f 00 00 00    	mov    %al,0x9f(%ebx)
8010853e:	88 8b 9c 00 00 00    	mov    %cl,0x9c(%ebx)
80108544:	b9 99 40 00 00       	mov    $0x4099,%ecx
80108549:	66 89 8b 9d 00 00 00 	mov    %cx,0x9d(%ebx)
  mycpu()->ts.ss0 = SEG_KDATA << 3;
80108550:	bb 10 00 00 00       	mov    $0x10,%ebx
  mycpu()->gdt[SEG_TSS].s = 0;
80108555:	e8 d6 b8 ff ff       	call   80103e30 <mycpu>
8010855a:	80 a0 9d 00 00 00 ef 	andb   $0xef,0x9d(%eax)
  mycpu()->ts.ss0 = SEG_KDATA << 3;
80108561:	e8 ca b8 ff ff       	call   80103e30 <mycpu>
80108566:	66 89 58 10          	mov    %bx,0x10(%eax)
  mycpu()->ts.esp0 = (uint)p->kstack + KSTACKSIZE;
8010856a:	8b 5e 08             	mov    0x8(%esi),%ebx
8010856d:	81 c3 00 10 00 00    	add    $0x1000,%ebx
80108573:	e8 b8 b8 ff ff       	call   80103e30 <mycpu>
80108578:	89 58 0c             	mov    %ebx,0xc(%eax)
  mycpu()->ts.iomb = (ushort) 0xFFFF;
8010857b:	e8 b0 b8 ff ff       	call   80103e30 <mycpu>
80108580:	66 89 78 6e          	mov    %di,0x6e(%eax)
  asm volatile("ltr %0" : : "r" (sel));
80108584:	b8 28 00 00 00       	mov    $0x28,%eax
80108589:	0f 00 d8             	ltr    %ax
  lcr3(V2P(p->pgdir));  // switch to process's address space
8010858c:	8b 46 04             	mov    0x4(%esi),%eax
8010858f:	05 00 00 00 80       	add    $0x80000000,%eax
  asm volatile("movl %0,%%cr3" : : "r" (val));
80108594:	0f 22 d8             	mov    %eax,%cr3
}
80108597:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010859a:	5b                   	pop    %ebx
8010859b:	5e                   	pop    %esi
8010859c:	5f                   	pop    %edi
8010859d:	5d                   	pop    %ebp
  popcli();
8010859e:	e9 bd d5 ff ff       	jmp    80105b60 <popcli>
    panic("switchuvm: no process");
801085a3:	83 ec 0c             	sub    $0xc,%esp
801085a6:	68 1a 97 10 80       	push   $0x8010971a
801085ab:	e8 e0 7d ff ff       	call   80100390 <panic>
    panic("switchuvm: no pgdir");
801085b0:	83 ec 0c             	sub    $0xc,%esp
801085b3:	68 45 97 10 80       	push   $0x80109745
801085b8:	e8 d3 7d ff ff       	call   80100390 <panic>
    panic("switchuvm: no kstack");
801085bd:	83 ec 0c             	sub    $0xc,%esp
801085c0:	68 30 97 10 80       	push   $0x80109730
801085c5:	e8 c6 7d ff ff       	call   80100390 <panic>
801085ca:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801085d0 <inituvm>:
{
801085d0:	f3 0f 1e fb          	endbr32 
801085d4:	55                   	push   %ebp
801085d5:	89 e5                	mov    %esp,%ebp
801085d7:	57                   	push   %edi
801085d8:	56                   	push   %esi
801085d9:	53                   	push   %ebx
801085da:	83 ec 1c             	sub    $0x1c,%esp
801085dd:	8b 45 0c             	mov    0xc(%ebp),%eax
801085e0:	8b 75 10             	mov    0x10(%ebp),%esi
801085e3:	8b 7d 08             	mov    0x8(%ebp),%edi
801085e6:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  if(sz >= PGSIZE)
801085e9:	81 fe ff 0f 00 00    	cmp    $0xfff,%esi
801085ef:	77 4b                	ja     8010863c <inituvm+0x6c>
  mem = kalloc();
801085f1:	e8 2a a5 ff ff       	call   80102b20 <kalloc>
  memset(mem, 0, PGSIZE);
801085f6:	83 ec 04             	sub    $0x4,%esp
801085f9:	68 00 10 00 00       	push   $0x1000
  mem = kalloc();
801085fe:	89 c3                	mov    %eax,%ebx
  memset(mem, 0, PGSIZE);
80108600:	6a 00                	push   $0x0
80108602:	50                   	push   %eax
80108603:	e8 18 d7 ff ff       	call   80105d20 <memset>
  mappages(pgdir, 0, PGSIZE, V2P(mem), PTE_W|PTE_U);
80108608:	58                   	pop    %eax
80108609:	8d 83 00 00 00 80    	lea    -0x80000000(%ebx),%eax
8010860f:	5a                   	pop    %edx
80108610:	6a 06                	push   $0x6
80108612:	b9 00 10 00 00       	mov    $0x1000,%ecx
80108617:	31 d2                	xor    %edx,%edx
80108619:	50                   	push   %eax
8010861a:	89 f8                	mov    %edi,%eax
8010861c:	e8 af fc ff ff       	call   801082d0 <mappages>
  memmove(mem, init, sz);
80108621:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80108624:	89 75 10             	mov    %esi,0x10(%ebp)
80108627:	83 c4 10             	add    $0x10,%esp
8010862a:	89 5d 08             	mov    %ebx,0x8(%ebp)
8010862d:	89 45 0c             	mov    %eax,0xc(%ebp)
}
80108630:	8d 65 f4             	lea    -0xc(%ebp),%esp
80108633:	5b                   	pop    %ebx
80108634:	5e                   	pop    %esi
80108635:	5f                   	pop    %edi
80108636:	5d                   	pop    %ebp
  memmove(mem, init, sz);
80108637:	e9 84 d7 ff ff       	jmp    80105dc0 <memmove>
    panic("inituvm: more than a page");
8010863c:	83 ec 0c             	sub    $0xc,%esp
8010863f:	68 59 97 10 80       	push   $0x80109759
80108644:	e8 47 7d ff ff       	call   80100390 <panic>
80108649:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80108650 <loaduvm>:
{
80108650:	f3 0f 1e fb          	endbr32 
80108654:	55                   	push   %ebp
80108655:	89 e5                	mov    %esp,%ebp
80108657:	57                   	push   %edi
80108658:	56                   	push   %esi
80108659:	53                   	push   %ebx
8010865a:	83 ec 1c             	sub    $0x1c,%esp
8010865d:	8b 45 0c             	mov    0xc(%ebp),%eax
80108660:	8b 75 18             	mov    0x18(%ebp),%esi
  if((uint) addr % PGSIZE != 0)
80108663:	a9 ff 0f 00 00       	test   $0xfff,%eax
80108668:	0f 85 99 00 00 00    	jne    80108707 <loaduvm+0xb7>
  for(i = 0; i < sz; i += PGSIZE){
8010866e:	01 f0                	add    %esi,%eax
80108670:	89 f3                	mov    %esi,%ebx
80108672:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if(readi(ip, P2V(pa), offset+i, n) != n)
80108675:	8b 45 14             	mov    0x14(%ebp),%eax
80108678:	01 f0                	add    %esi,%eax
8010867a:	89 45 e0             	mov    %eax,-0x20(%ebp)
  for(i = 0; i < sz; i += PGSIZE){
8010867d:	85 f6                	test   %esi,%esi
8010867f:	75 15                	jne    80108696 <loaduvm+0x46>
80108681:	eb 6d                	jmp    801086f0 <loaduvm+0xa0>
80108683:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80108687:	90                   	nop
80108688:	81 eb 00 10 00 00    	sub    $0x1000,%ebx
8010868e:	89 f0                	mov    %esi,%eax
80108690:	29 d8                	sub    %ebx,%eax
80108692:	39 c6                	cmp    %eax,%esi
80108694:	76 5a                	jbe    801086f0 <loaduvm+0xa0>
    if((pte = walkpgdir(pgdir, addr+i, 0)) == 0)
80108696:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80108699:	8b 45 08             	mov    0x8(%ebp),%eax
8010869c:	31 c9                	xor    %ecx,%ecx
8010869e:	29 da                	sub    %ebx,%edx
801086a0:	e8 ab fb ff ff       	call   80108250 <walkpgdir>
801086a5:	85 c0                	test   %eax,%eax
801086a7:	74 51                	je     801086fa <loaduvm+0xaa>
    pa = PTE_ADDR(*pte);
801086a9:	8b 00                	mov    (%eax),%eax
    if(readi(ip, P2V(pa), offset+i, n) != n)
801086ab:	8b 4d e0             	mov    -0x20(%ebp),%ecx
    if(sz - i < PGSIZE)
801086ae:	bf 00 10 00 00       	mov    $0x1000,%edi
    pa = PTE_ADDR(*pte);
801086b3:	25 00 f0 ff ff       	and    $0xfffff000,%eax
    if(sz - i < PGSIZE)
801086b8:	81 fb ff 0f 00 00    	cmp    $0xfff,%ebx
801086be:	0f 46 fb             	cmovbe %ebx,%edi
    if(readi(ip, P2V(pa), offset+i, n) != n)
801086c1:	29 d9                	sub    %ebx,%ecx
801086c3:	05 00 00 00 80       	add    $0x80000000,%eax
801086c8:	57                   	push   %edi
801086c9:	51                   	push   %ecx
801086ca:	50                   	push   %eax
801086cb:	ff 75 10             	pushl  0x10(%ebp)
801086ce:	e8 7d 98 ff ff       	call   80101f50 <readi>
801086d3:	83 c4 10             	add    $0x10,%esp
801086d6:	39 f8                	cmp    %edi,%eax
801086d8:	74 ae                	je     80108688 <loaduvm+0x38>
}
801086da:	8d 65 f4             	lea    -0xc(%ebp),%esp
      return -1;
801086dd:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801086e2:	5b                   	pop    %ebx
801086e3:	5e                   	pop    %esi
801086e4:	5f                   	pop    %edi
801086e5:	5d                   	pop    %ebp
801086e6:	c3                   	ret    
801086e7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801086ee:	66 90                	xchg   %ax,%ax
801086f0:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
801086f3:	31 c0                	xor    %eax,%eax
}
801086f5:	5b                   	pop    %ebx
801086f6:	5e                   	pop    %esi
801086f7:	5f                   	pop    %edi
801086f8:	5d                   	pop    %ebp
801086f9:	c3                   	ret    
      panic("loaduvm: address should exist");
801086fa:	83 ec 0c             	sub    $0xc,%esp
801086fd:	68 73 97 10 80       	push   $0x80109773
80108702:	e8 89 7c ff ff       	call   80100390 <panic>
    panic("loaduvm: addr must be page aligned");
80108707:	83 ec 0c             	sub    $0xc,%esp
8010870a:	68 14 98 10 80       	push   $0x80109814
8010870f:	e8 7c 7c ff ff       	call   80100390 <panic>
80108714:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010871b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010871f:	90                   	nop

80108720 <allocuvm>:
{
80108720:	f3 0f 1e fb          	endbr32 
80108724:	55                   	push   %ebp
80108725:	89 e5                	mov    %esp,%ebp
80108727:	57                   	push   %edi
80108728:	56                   	push   %esi
80108729:	53                   	push   %ebx
8010872a:	83 ec 1c             	sub    $0x1c,%esp
  if(newsz >= KERNBASE)
8010872d:	8b 45 10             	mov    0x10(%ebp),%eax
{
80108730:	8b 7d 08             	mov    0x8(%ebp),%edi
  if(newsz >= KERNBASE)
80108733:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80108736:	85 c0                	test   %eax,%eax
80108738:	0f 88 b2 00 00 00    	js     801087f0 <allocuvm+0xd0>
  if(newsz < oldsz)
8010873e:	3b 45 0c             	cmp    0xc(%ebp),%eax
    return oldsz;
80108741:	8b 45 0c             	mov    0xc(%ebp),%eax
  if(newsz < oldsz)
80108744:	0f 82 96 00 00 00    	jb     801087e0 <allocuvm+0xc0>
  a = PGROUNDUP(oldsz);
8010874a:	8d b0 ff 0f 00 00    	lea    0xfff(%eax),%esi
80108750:	81 e6 00 f0 ff ff    	and    $0xfffff000,%esi
  for(; a < newsz; a += PGSIZE){
80108756:	39 75 10             	cmp    %esi,0x10(%ebp)
80108759:	77 40                	ja     8010879b <allocuvm+0x7b>
8010875b:	e9 83 00 00 00       	jmp    801087e3 <allocuvm+0xc3>
    memset(mem, 0, PGSIZE);
80108760:	83 ec 04             	sub    $0x4,%esp
80108763:	68 00 10 00 00       	push   $0x1000
80108768:	6a 00                	push   $0x0
8010876a:	50                   	push   %eax
8010876b:	e8 b0 d5 ff ff       	call   80105d20 <memset>
    if(mappages(pgdir, (char*)a, PGSIZE, V2P(mem), PTE_W|PTE_U) < 0){
80108770:	58                   	pop    %eax
80108771:	8d 83 00 00 00 80    	lea    -0x80000000(%ebx),%eax
80108777:	5a                   	pop    %edx
80108778:	6a 06                	push   $0x6
8010877a:	b9 00 10 00 00       	mov    $0x1000,%ecx
8010877f:	89 f2                	mov    %esi,%edx
80108781:	50                   	push   %eax
80108782:	89 f8                	mov    %edi,%eax
80108784:	e8 47 fb ff ff       	call   801082d0 <mappages>
80108789:	83 c4 10             	add    $0x10,%esp
8010878c:	85 c0                	test   %eax,%eax
8010878e:	78 78                	js     80108808 <allocuvm+0xe8>
  for(; a < newsz; a += PGSIZE){
80108790:	81 c6 00 10 00 00    	add    $0x1000,%esi
80108796:	39 75 10             	cmp    %esi,0x10(%ebp)
80108799:	76 48                	jbe    801087e3 <allocuvm+0xc3>
    mem = kalloc();
8010879b:	e8 80 a3 ff ff       	call   80102b20 <kalloc>
801087a0:	89 c3                	mov    %eax,%ebx
    if(mem == 0){
801087a2:	85 c0                	test   %eax,%eax
801087a4:	75 ba                	jne    80108760 <allocuvm+0x40>
      cprintf("allocuvm out of memory\n");
801087a6:	83 ec 0c             	sub    $0xc,%esp
801087a9:	68 91 97 10 80       	push   $0x80109791
801087ae:	e8 0d 81 ff ff       	call   801008c0 <cprintf>
  if(newsz >= oldsz)
801087b3:	8b 45 0c             	mov    0xc(%ebp),%eax
801087b6:	83 c4 10             	add    $0x10,%esp
801087b9:	39 45 10             	cmp    %eax,0x10(%ebp)
801087bc:	74 32                	je     801087f0 <allocuvm+0xd0>
801087be:	8b 55 10             	mov    0x10(%ebp),%edx
801087c1:	89 c1                	mov    %eax,%ecx
801087c3:	89 f8                	mov    %edi,%eax
801087c5:	e8 96 fb ff ff       	call   80108360 <deallocuvm.part.0>
      return 0;
801087ca:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
}
801087d1:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801087d4:	8d 65 f4             	lea    -0xc(%ebp),%esp
801087d7:	5b                   	pop    %ebx
801087d8:	5e                   	pop    %esi
801087d9:	5f                   	pop    %edi
801087da:	5d                   	pop    %ebp
801087db:	c3                   	ret    
801087dc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    return oldsz;
801087e0:	89 45 e4             	mov    %eax,-0x1c(%ebp)
}
801087e3:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801087e6:	8d 65 f4             	lea    -0xc(%ebp),%esp
801087e9:	5b                   	pop    %ebx
801087ea:	5e                   	pop    %esi
801087eb:	5f                   	pop    %edi
801087ec:	5d                   	pop    %ebp
801087ed:	c3                   	ret    
801087ee:	66 90                	xchg   %ax,%ax
    return 0;
801087f0:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
}
801087f7:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801087fa:	8d 65 f4             	lea    -0xc(%ebp),%esp
801087fd:	5b                   	pop    %ebx
801087fe:	5e                   	pop    %esi
801087ff:	5f                   	pop    %edi
80108800:	5d                   	pop    %ebp
80108801:	c3                   	ret    
80108802:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
      cprintf("allocuvm out of memory (2)\n");
80108808:	83 ec 0c             	sub    $0xc,%esp
8010880b:	68 a9 97 10 80       	push   $0x801097a9
80108810:	e8 ab 80 ff ff       	call   801008c0 <cprintf>
  if(newsz >= oldsz)
80108815:	8b 45 0c             	mov    0xc(%ebp),%eax
80108818:	83 c4 10             	add    $0x10,%esp
8010881b:	39 45 10             	cmp    %eax,0x10(%ebp)
8010881e:	74 0c                	je     8010882c <allocuvm+0x10c>
80108820:	8b 55 10             	mov    0x10(%ebp),%edx
80108823:	89 c1                	mov    %eax,%ecx
80108825:	89 f8                	mov    %edi,%eax
80108827:	e8 34 fb ff ff       	call   80108360 <deallocuvm.part.0>
      kfree(mem);
8010882c:	83 ec 0c             	sub    $0xc,%esp
8010882f:	53                   	push   %ebx
80108830:	e8 2b a1 ff ff       	call   80102960 <kfree>
      return 0;
80108835:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
8010883c:	83 c4 10             	add    $0x10,%esp
}
8010883f:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80108842:	8d 65 f4             	lea    -0xc(%ebp),%esp
80108845:	5b                   	pop    %ebx
80108846:	5e                   	pop    %esi
80108847:	5f                   	pop    %edi
80108848:	5d                   	pop    %ebp
80108849:	c3                   	ret    
8010884a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80108850 <deallocuvm>:
{
80108850:	f3 0f 1e fb          	endbr32 
80108854:	55                   	push   %ebp
80108855:	89 e5                	mov    %esp,%ebp
80108857:	8b 55 0c             	mov    0xc(%ebp),%edx
8010885a:	8b 4d 10             	mov    0x10(%ebp),%ecx
8010885d:	8b 45 08             	mov    0x8(%ebp),%eax
  if(newsz >= oldsz)
80108860:	39 d1                	cmp    %edx,%ecx
80108862:	73 0c                	jae    80108870 <deallocuvm+0x20>
}
80108864:	5d                   	pop    %ebp
80108865:	e9 f6 fa ff ff       	jmp    80108360 <deallocuvm.part.0>
8010886a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80108870:	89 d0                	mov    %edx,%eax
80108872:	5d                   	pop    %ebp
80108873:	c3                   	ret    
80108874:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010887b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010887f:	90                   	nop

80108880 <freevm>:

// Free a page table and all the physical memory pages
// in the user part.
void
freevm(pde_t *pgdir)
{
80108880:	f3 0f 1e fb          	endbr32 
80108884:	55                   	push   %ebp
80108885:	89 e5                	mov    %esp,%ebp
80108887:	57                   	push   %edi
80108888:	56                   	push   %esi
80108889:	53                   	push   %ebx
8010888a:	83 ec 0c             	sub    $0xc,%esp
8010888d:	8b 75 08             	mov    0x8(%ebp),%esi
  uint i;

  if(pgdir == 0)
80108890:	85 f6                	test   %esi,%esi
80108892:	74 55                	je     801088e9 <freevm+0x69>
  if(newsz >= oldsz)
80108894:	31 c9                	xor    %ecx,%ecx
80108896:	ba 00 00 00 80       	mov    $0x80000000,%edx
8010889b:	89 f0                	mov    %esi,%eax
8010889d:	89 f3                	mov    %esi,%ebx
8010889f:	e8 bc fa ff ff       	call   80108360 <deallocuvm.part.0>
    panic("freevm: no pgdir");
  deallocuvm(pgdir, KERNBASE, 0);
  for(i = 0; i < NPDENTRIES; i++){
801088a4:	8d be 00 10 00 00    	lea    0x1000(%esi),%edi
801088aa:	eb 0b                	jmp    801088b7 <freevm+0x37>
801088ac:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801088b0:	83 c3 04             	add    $0x4,%ebx
801088b3:	39 df                	cmp    %ebx,%edi
801088b5:	74 23                	je     801088da <freevm+0x5a>
    if(pgdir[i] & PTE_P){
801088b7:	8b 03                	mov    (%ebx),%eax
801088b9:	a8 01                	test   $0x1,%al
801088bb:	74 f3                	je     801088b0 <freevm+0x30>
      char * v = P2V(PTE_ADDR(pgdir[i]));
801088bd:	25 00 f0 ff ff       	and    $0xfffff000,%eax
      kfree(v);
801088c2:	83 ec 0c             	sub    $0xc,%esp
801088c5:	83 c3 04             	add    $0x4,%ebx
      char * v = P2V(PTE_ADDR(pgdir[i]));
801088c8:	05 00 00 00 80       	add    $0x80000000,%eax
      kfree(v);
801088cd:	50                   	push   %eax
801088ce:	e8 8d a0 ff ff       	call   80102960 <kfree>
801088d3:	83 c4 10             	add    $0x10,%esp
  for(i = 0; i < NPDENTRIES; i++){
801088d6:	39 df                	cmp    %ebx,%edi
801088d8:	75 dd                	jne    801088b7 <freevm+0x37>
    }
  }
  kfree((char*)pgdir);
801088da:	89 75 08             	mov    %esi,0x8(%ebp)
}
801088dd:	8d 65 f4             	lea    -0xc(%ebp),%esp
801088e0:	5b                   	pop    %ebx
801088e1:	5e                   	pop    %esi
801088e2:	5f                   	pop    %edi
801088e3:	5d                   	pop    %ebp
  kfree((char*)pgdir);
801088e4:	e9 77 a0 ff ff       	jmp    80102960 <kfree>
    panic("freevm: no pgdir");
801088e9:	83 ec 0c             	sub    $0xc,%esp
801088ec:	68 c5 97 10 80       	push   $0x801097c5
801088f1:	e8 9a 7a ff ff       	call   80100390 <panic>
801088f6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801088fd:	8d 76 00             	lea    0x0(%esi),%esi

80108900 <setupkvm>:
{
80108900:	f3 0f 1e fb          	endbr32 
80108904:	55                   	push   %ebp
80108905:	89 e5                	mov    %esp,%ebp
80108907:	56                   	push   %esi
80108908:	53                   	push   %ebx
  if((pgdir = (pde_t*)kalloc()) == 0)
80108909:	e8 12 a2 ff ff       	call   80102b20 <kalloc>
8010890e:	89 c6                	mov    %eax,%esi
80108910:	85 c0                	test   %eax,%eax
80108912:	74 42                	je     80108956 <setupkvm+0x56>
  memset(pgdir, 0, PGSIZE);
80108914:	83 ec 04             	sub    $0x4,%esp
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
80108917:	bb 20 c4 10 80       	mov    $0x8010c420,%ebx
  memset(pgdir, 0, PGSIZE);
8010891c:	68 00 10 00 00       	push   $0x1000
80108921:	6a 00                	push   $0x0
80108923:	50                   	push   %eax
80108924:	e8 f7 d3 ff ff       	call   80105d20 <memset>
80108929:	83 c4 10             	add    $0x10,%esp
                (uint)k->phys_start, k->perm) < 0) {
8010892c:	8b 43 04             	mov    0x4(%ebx),%eax
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start,
8010892f:	83 ec 08             	sub    $0x8,%esp
80108932:	8b 4b 08             	mov    0x8(%ebx),%ecx
80108935:	ff 73 0c             	pushl  0xc(%ebx)
80108938:	8b 13                	mov    (%ebx),%edx
8010893a:	50                   	push   %eax
8010893b:	29 c1                	sub    %eax,%ecx
8010893d:	89 f0                	mov    %esi,%eax
8010893f:	e8 8c f9 ff ff       	call   801082d0 <mappages>
80108944:	83 c4 10             	add    $0x10,%esp
80108947:	85 c0                	test   %eax,%eax
80108949:	78 15                	js     80108960 <setupkvm+0x60>
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
8010894b:	83 c3 10             	add    $0x10,%ebx
8010894e:	81 fb 60 c4 10 80    	cmp    $0x8010c460,%ebx
80108954:	75 d6                	jne    8010892c <setupkvm+0x2c>
}
80108956:	8d 65 f8             	lea    -0x8(%ebp),%esp
80108959:	89 f0                	mov    %esi,%eax
8010895b:	5b                   	pop    %ebx
8010895c:	5e                   	pop    %esi
8010895d:	5d                   	pop    %ebp
8010895e:	c3                   	ret    
8010895f:	90                   	nop
      freevm(pgdir);
80108960:	83 ec 0c             	sub    $0xc,%esp
80108963:	56                   	push   %esi
      return 0;
80108964:	31 f6                	xor    %esi,%esi
      freevm(pgdir);
80108966:	e8 15 ff ff ff       	call   80108880 <freevm>
      return 0;
8010896b:	83 c4 10             	add    $0x10,%esp
}
8010896e:	8d 65 f8             	lea    -0x8(%ebp),%esp
80108971:	89 f0                	mov    %esi,%eax
80108973:	5b                   	pop    %ebx
80108974:	5e                   	pop    %esi
80108975:	5d                   	pop    %ebp
80108976:	c3                   	ret    
80108977:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010897e:	66 90                	xchg   %ax,%ax

80108980 <kvmalloc>:
{
80108980:	f3 0f 1e fb          	endbr32 
80108984:	55                   	push   %ebp
80108985:	89 e5                	mov    %esp,%ebp
80108987:	83 ec 08             	sub    $0x8,%esp
  kpgdir = setupkvm();
8010898a:	e8 71 ff ff ff       	call   80108900 <setupkvm>
8010898f:	a3 a4 86 11 80       	mov    %eax,0x801186a4
  lcr3(V2P(kpgdir));   // switch to the kernel page table
80108994:	05 00 00 00 80       	add    $0x80000000,%eax
80108999:	0f 22 d8             	mov    %eax,%cr3
}
8010899c:	c9                   	leave  
8010899d:	c3                   	ret    
8010899e:	66 90                	xchg   %ax,%ax

801089a0 <clearpteu>:

// Clear PTE_U on a page. Used to create an inaccessible
// page beneath the user stack.
void
clearpteu(pde_t *pgdir, char *uva)
{
801089a0:	f3 0f 1e fb          	endbr32 
801089a4:	55                   	push   %ebp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
801089a5:	31 c9                	xor    %ecx,%ecx
{
801089a7:	89 e5                	mov    %esp,%ebp
801089a9:	83 ec 08             	sub    $0x8,%esp
  pte = walkpgdir(pgdir, uva, 0);
801089ac:	8b 55 0c             	mov    0xc(%ebp),%edx
801089af:	8b 45 08             	mov    0x8(%ebp),%eax
801089b2:	e8 99 f8 ff ff       	call   80108250 <walkpgdir>
  if(pte == 0)
801089b7:	85 c0                	test   %eax,%eax
801089b9:	74 05                	je     801089c0 <clearpteu+0x20>
    panic("clearpteu");
  *pte &= ~PTE_U;
801089bb:	83 20 fb             	andl   $0xfffffffb,(%eax)
}
801089be:	c9                   	leave  
801089bf:	c3                   	ret    
    panic("clearpteu");
801089c0:	83 ec 0c             	sub    $0xc,%esp
801089c3:	68 d6 97 10 80       	push   $0x801097d6
801089c8:	e8 c3 79 ff ff       	call   80100390 <panic>
801089cd:	8d 76 00             	lea    0x0(%esi),%esi

801089d0 <copyuvm>:

// Given a parent process's page table, create a copy
// of it for a child.
pde_t*
copyuvm(pde_t *pgdir, uint sz)
{
801089d0:	f3 0f 1e fb          	endbr32 
801089d4:	55                   	push   %ebp
801089d5:	89 e5                	mov    %esp,%ebp
801089d7:	57                   	push   %edi
801089d8:	56                   	push   %esi
801089d9:	53                   	push   %ebx
801089da:	83 ec 1c             	sub    $0x1c,%esp
  pde_t *d;
  pte_t *pte;
  uint pa, i, flags;
  char *mem;

  if((d = setupkvm()) == 0)
801089dd:	e8 1e ff ff ff       	call   80108900 <setupkvm>
801089e2:	89 45 e0             	mov    %eax,-0x20(%ebp)
801089e5:	85 c0                	test   %eax,%eax
801089e7:	0f 84 9b 00 00 00    	je     80108a88 <copyuvm+0xb8>
    return 0;
  for(i = 0; i < sz; i += PGSIZE){
801089ed:	8b 4d 0c             	mov    0xc(%ebp),%ecx
801089f0:	85 c9                	test   %ecx,%ecx
801089f2:	0f 84 90 00 00 00    	je     80108a88 <copyuvm+0xb8>
801089f8:	31 f6                	xor    %esi,%esi
801089fa:	eb 46                	jmp    80108a42 <copyuvm+0x72>
801089fc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      panic("copyuvm: page not present");
    pa = PTE_ADDR(*pte);
    flags = PTE_FLAGS(*pte);
    if((mem = kalloc()) == 0)
      goto bad;
    memmove(mem, (char*)P2V(pa), PGSIZE);
80108a00:	83 ec 04             	sub    $0x4,%esp
80108a03:	81 c7 00 00 00 80    	add    $0x80000000,%edi
80108a09:	68 00 10 00 00       	push   $0x1000
80108a0e:	57                   	push   %edi
80108a0f:	50                   	push   %eax
80108a10:	e8 ab d3 ff ff       	call   80105dc0 <memmove>
    if(mappages(d, (void*)i, PGSIZE, V2P(mem), flags) < 0) {
80108a15:	58                   	pop    %eax
80108a16:	8d 83 00 00 00 80    	lea    -0x80000000(%ebx),%eax
80108a1c:	5a                   	pop    %edx
80108a1d:	ff 75 e4             	pushl  -0x1c(%ebp)
80108a20:	b9 00 10 00 00       	mov    $0x1000,%ecx
80108a25:	89 f2                	mov    %esi,%edx
80108a27:	50                   	push   %eax
80108a28:	8b 45 e0             	mov    -0x20(%ebp),%eax
80108a2b:	e8 a0 f8 ff ff       	call   801082d0 <mappages>
80108a30:	83 c4 10             	add    $0x10,%esp
80108a33:	85 c0                	test   %eax,%eax
80108a35:	78 61                	js     80108a98 <copyuvm+0xc8>
  for(i = 0; i < sz; i += PGSIZE){
80108a37:	81 c6 00 10 00 00    	add    $0x1000,%esi
80108a3d:	39 75 0c             	cmp    %esi,0xc(%ebp)
80108a40:	76 46                	jbe    80108a88 <copyuvm+0xb8>
    if((pte = walkpgdir(pgdir, (void *) i, 0)) == 0)
80108a42:	8b 45 08             	mov    0x8(%ebp),%eax
80108a45:	31 c9                	xor    %ecx,%ecx
80108a47:	89 f2                	mov    %esi,%edx
80108a49:	e8 02 f8 ff ff       	call   80108250 <walkpgdir>
80108a4e:	85 c0                	test   %eax,%eax
80108a50:	74 61                	je     80108ab3 <copyuvm+0xe3>
    if(!(*pte & PTE_P))
80108a52:	8b 00                	mov    (%eax),%eax
80108a54:	a8 01                	test   $0x1,%al
80108a56:	74 4e                	je     80108aa6 <copyuvm+0xd6>
    pa = PTE_ADDR(*pte);
80108a58:	89 c7                	mov    %eax,%edi
    flags = PTE_FLAGS(*pte);
80108a5a:	25 ff 0f 00 00       	and    $0xfff,%eax
80108a5f:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    pa = PTE_ADDR(*pte);
80108a62:	81 e7 00 f0 ff ff    	and    $0xfffff000,%edi
    if((mem = kalloc()) == 0)
80108a68:	e8 b3 a0 ff ff       	call   80102b20 <kalloc>
80108a6d:	89 c3                	mov    %eax,%ebx
80108a6f:	85 c0                	test   %eax,%eax
80108a71:	75 8d                	jne    80108a00 <copyuvm+0x30>
    }
  }
  return d;

bad:
  freevm(d);
80108a73:	83 ec 0c             	sub    $0xc,%esp
80108a76:	ff 75 e0             	pushl  -0x20(%ebp)
80108a79:	e8 02 fe ff ff       	call   80108880 <freevm>
  return 0;
80108a7e:	c7 45 e0 00 00 00 00 	movl   $0x0,-0x20(%ebp)
80108a85:	83 c4 10             	add    $0x10,%esp
}
80108a88:	8b 45 e0             	mov    -0x20(%ebp),%eax
80108a8b:	8d 65 f4             	lea    -0xc(%ebp),%esp
80108a8e:	5b                   	pop    %ebx
80108a8f:	5e                   	pop    %esi
80108a90:	5f                   	pop    %edi
80108a91:	5d                   	pop    %ebp
80108a92:	c3                   	ret    
80108a93:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80108a97:	90                   	nop
      kfree(mem);
80108a98:	83 ec 0c             	sub    $0xc,%esp
80108a9b:	53                   	push   %ebx
80108a9c:	e8 bf 9e ff ff       	call   80102960 <kfree>
      goto bad;
80108aa1:	83 c4 10             	add    $0x10,%esp
80108aa4:	eb cd                	jmp    80108a73 <copyuvm+0xa3>
      panic("copyuvm: page not present");
80108aa6:	83 ec 0c             	sub    $0xc,%esp
80108aa9:	68 fa 97 10 80       	push   $0x801097fa
80108aae:	e8 dd 78 ff ff       	call   80100390 <panic>
      panic("copyuvm: pte should exist");
80108ab3:	83 ec 0c             	sub    $0xc,%esp
80108ab6:	68 e0 97 10 80       	push   $0x801097e0
80108abb:	e8 d0 78 ff ff       	call   80100390 <panic>

80108ac0 <uva2ka>:

//PAGEBREAK!
// Map user virtual address to kernel address.
char*
uva2ka(pde_t *pgdir, char *uva)
{
80108ac0:	f3 0f 1e fb          	endbr32 
80108ac4:	55                   	push   %ebp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
80108ac5:	31 c9                	xor    %ecx,%ecx
{
80108ac7:	89 e5                	mov    %esp,%ebp
80108ac9:	83 ec 08             	sub    $0x8,%esp
  pte = walkpgdir(pgdir, uva, 0);
80108acc:	8b 55 0c             	mov    0xc(%ebp),%edx
80108acf:	8b 45 08             	mov    0x8(%ebp),%eax
80108ad2:	e8 79 f7 ff ff       	call   80108250 <walkpgdir>
  if((*pte & PTE_P) == 0)
80108ad7:	8b 00                	mov    (%eax),%eax
    return 0;
  if((*pte & PTE_U) == 0)
    return 0;
  return (char*)P2V(PTE_ADDR(*pte));
}
80108ad9:	c9                   	leave  
  if((*pte & PTE_U) == 0)
80108ada:	89 c2                	mov    %eax,%edx
  return (char*)P2V(PTE_ADDR(*pte));
80108adc:	25 00 f0 ff ff       	and    $0xfffff000,%eax
  if((*pte & PTE_U) == 0)
80108ae1:	83 e2 05             	and    $0x5,%edx
  return (char*)P2V(PTE_ADDR(*pte));
80108ae4:	05 00 00 00 80       	add    $0x80000000,%eax
80108ae9:	83 fa 05             	cmp    $0x5,%edx
80108aec:	ba 00 00 00 00       	mov    $0x0,%edx
80108af1:	0f 45 c2             	cmovne %edx,%eax
}
80108af4:	c3                   	ret    
80108af5:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80108afc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80108b00 <copyout>:
// Copy len bytes from p to user address va in page table pgdir.
// Most useful when pgdir is not the current page table.
// uva2ka ensures this only works for PTE_U pages.
int
copyout(pde_t *pgdir, uint va, void *p, uint len)
{
80108b00:	f3 0f 1e fb          	endbr32 
80108b04:	55                   	push   %ebp
80108b05:	89 e5                	mov    %esp,%ebp
80108b07:	57                   	push   %edi
80108b08:	56                   	push   %esi
80108b09:	53                   	push   %ebx
80108b0a:	83 ec 0c             	sub    $0xc,%esp
80108b0d:	8b 75 14             	mov    0x14(%ebp),%esi
80108b10:	8b 55 0c             	mov    0xc(%ebp),%edx
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
  while(len > 0){
80108b13:	85 f6                	test   %esi,%esi
80108b15:	75 3c                	jne    80108b53 <copyout+0x53>
80108b17:	eb 67                	jmp    80108b80 <copyout+0x80>
80108b19:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    va0 = (uint)PGROUNDDOWN(va);
    pa0 = uva2ka(pgdir, (char*)va0);
    if(pa0 == 0)
      return -1;
    n = PGSIZE - (va - va0);
80108b20:	8b 55 0c             	mov    0xc(%ebp),%edx
80108b23:	89 fb                	mov    %edi,%ebx
80108b25:	29 d3                	sub    %edx,%ebx
80108b27:	81 c3 00 10 00 00    	add    $0x1000,%ebx
    if(n > len)
80108b2d:	39 f3                	cmp    %esi,%ebx
80108b2f:	0f 47 de             	cmova  %esi,%ebx
      n = len;
    memmove(pa0 + (va - va0), buf, n);
80108b32:	29 fa                	sub    %edi,%edx
80108b34:	83 ec 04             	sub    $0x4,%esp
80108b37:	01 c2                	add    %eax,%edx
80108b39:	53                   	push   %ebx
80108b3a:	ff 75 10             	pushl  0x10(%ebp)
80108b3d:	52                   	push   %edx
80108b3e:	e8 7d d2 ff ff       	call   80105dc0 <memmove>
    len -= n;
    buf += n;
80108b43:	01 5d 10             	add    %ebx,0x10(%ebp)
    va = va0 + PGSIZE;
80108b46:	8d 97 00 10 00 00    	lea    0x1000(%edi),%edx
  while(len > 0){
80108b4c:	83 c4 10             	add    $0x10,%esp
80108b4f:	29 de                	sub    %ebx,%esi
80108b51:	74 2d                	je     80108b80 <copyout+0x80>
    va0 = (uint)PGROUNDDOWN(va);
80108b53:	89 d7                	mov    %edx,%edi
    pa0 = uva2ka(pgdir, (char*)va0);
80108b55:	83 ec 08             	sub    $0x8,%esp
    va0 = (uint)PGROUNDDOWN(va);
80108b58:	89 55 0c             	mov    %edx,0xc(%ebp)
80108b5b:	81 e7 00 f0 ff ff    	and    $0xfffff000,%edi
    pa0 = uva2ka(pgdir, (char*)va0);
80108b61:	57                   	push   %edi
80108b62:	ff 75 08             	pushl  0x8(%ebp)
80108b65:	e8 56 ff ff ff       	call   80108ac0 <uva2ka>
    if(pa0 == 0)
80108b6a:	83 c4 10             	add    $0x10,%esp
80108b6d:	85 c0                	test   %eax,%eax
80108b6f:	75 af                	jne    80108b20 <copyout+0x20>
  }
  return 0;
}
80108b71:	8d 65 f4             	lea    -0xc(%ebp),%esp
      return -1;
80108b74:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80108b79:	5b                   	pop    %ebx
80108b7a:	5e                   	pop    %esi
80108b7b:	5f                   	pop    %edi
80108b7c:	5d                   	pop    %ebp
80108b7d:	c3                   	ret    
80108b7e:	66 90                	xchg   %ax,%ax
80108b80:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
80108b83:	31 c0                	xor    %eax,%eax
}
80108b85:	5b                   	pop    %ebx
80108b86:	5e                   	pop    %esi
80108b87:	5f                   	pop    %edi
80108b88:	5d                   	pop    %ebp
80108b89:	c3                   	ret    
